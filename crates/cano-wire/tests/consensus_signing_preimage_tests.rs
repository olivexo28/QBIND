...
fn vote_signing_preimage_stable_for_fields() {
    ...
    // Updated for T101 to include epoch field
...
fn vote_signing_preimage_length_is_correct() {
    ...
    // validator_index: 2
    // suite_id: 2
    // Total: 12 + 1 + 4 + 8 + 8 + 8 + 1 + 32 + 2 + 2 = 78
    let expected_len = 12 + 1 + 4 + 8 + 8 + 8 + 1 + 32 + 2 + 2;
...
fn proposal_signing_preimage_stable_for_fields() {
    ...
    let expected_bytes: Vec<u8> = vec![
        // "CANO:PROPOSAL:v1"
        0x43, 0x41, 0x4e, 0x4f, 0x3a, 0x50, 0x52, 0x4f, 0x50, 0x4f, 0x53, 0x41, 0x4c, 0x3a, 0x76,
        0x31, // version: 1
        0x01, // chain_id: 42 (LE)
        0x2a, 0x00, 0x00, 0x00, // epoch: 0 (LE) - T101
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // height: 100 (LE)
...
fn proposal_signing_preimage_length_no_qc_no_txs() {
    ...
    // next_epoch: 8 (T102.1)
    // qc_len: 4
    // Total: 16 + 1 + 4 + 8 + 8 + 8 + 32 + 32 + 2 + 2 + 4 + 8 + 1 + 8 + 4 = 138
    let expected_len = 16 + 1 + 4 + 8 + 8 + 8 + 32 + 32 + 2 + 2 + 4 + 8 + 1 + 8 + 4;
    assert_eq!(preimage.len(), expected_len);
}
