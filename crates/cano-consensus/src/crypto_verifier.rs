...
/// - `cano_consensus_sig_verifications_total{kind="vote",suite="<name>"}`
/// - `cano_consensus_sig_verifications_total{kind="proposal",suite="<name>"}`
/// - `cano_consensus_sig_verify_duration_ms_bucket{suite="<name>",le="<threshold>"}`
///
...
impl ConsensusSigMetrics {
    ...
    fn find_or_allocate_suite_slot(&self, suite_id: ConsensusSigSuiteId) -> Option<usize> {
        ...
        for i in 0..MAX_PER_SUITE_SLOTS {
            ...
            if result.is_ok() {
                ...
                return Some(i);
            } else if result.unwrap_err() == suite_id_u64 {
    ...
    pub fn format_per_suite_metrics(&self) -> String {
            ...
            self.all_per_suite_metrics()
        {
            ...
            let suite_name = cano_crypto::suite_name(suite_id);

            // Vote/proposal counts
            output.push_str(&format!(
                "cano_consensus_sig_verifications_total{{kind=\"vote\",suite=\"{}\"}} {}\n",
                suite_name, vote_count
            ));
            output.push_str(&format!(
                "cano_consensus_sig_verifications_total{{kind=\"proposal\",suite=\"{}\"}} {}\n",
            ...
            if total_latency_count > 0 {
                output.push_str(&format!(
                    "cano_consensus_sig_verify_duration_ms_bucket{{suite=\"{}\",le=\"1\"}} {}\n",
                    suite_name, under_1ms
                ));
                output.push_str(&format!(
                    "cano_consensus_sig_verify_duration_ms_bucket{{suite=\"{}\",le=\"10\"}} {}\n",
                    suite_name,
                    under_1ms + to_10ms
                ));
                output.push_str(&format!(
                    "cano_consensus_sig_verify_duration_ms_bucket{{suite=\"{}\",le=\"100\"}} {}\n",
                    suite_name,
                    under_1ms + to_10ms + to_100ms
                ));
                output.push_str(&format!(
                    "cano_consensus_sig_verify_duration_ms_bucket{{suite=\"{}\",le=\"+Inf\"}} {}\n",
                    suite_name, total_latency_count
                ));
                output.push_str(&format!(
                    "cano_consensus_sig_verify_duration_ms_count{{suite=\"{}\"}} {}\n",
                    suite_name, total_latency_count
                ));
            }
        }
...
impl SingleSuiteKeyProviderAdapter {
    ...
    pub fn with_toy_sha3(inner: Arc<dyn ValidatorKeyProvider>) -> Self {
        Self::new(inner, cano_crypto::SUITE_TOY_SHA3)
    }
}
...
impl CryptoConsensusVerifier {
    ...
    pub fn with_key_provider(
        ...
        let backend_registry =
            SimpleBackendRegistry::with_backend(cano_crypto::SUITE_TOY_SHA3, backend);
