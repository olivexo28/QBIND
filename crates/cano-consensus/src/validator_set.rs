...
///
/// ```ignore
/// use cano_consensus::validator_set::{EpochId, EpochState, ConsensusValidatorSet, ValidatorSetEntry};
...
impl EpochState {
    ...
    pub fn epoch_suite_id<G>(
        &self,
        governance: &G,
    ) -> Result<Option<cano_crypto::ConsensusSigSuiteId>, Vec<cano_crypto::ConsensusSigSuiteId>>
    where
        G: crate::governed_key_registry::ConsensusKeyGovernance + ?Sized,
    {
        let mut suite_ids: std::collections::HashSet<cano_crypto::ConsensusSigSuiteId> =
    ...
    ///
    /// ```ignore
    /// use cano_consensus::validator_set::EpochState;
    ...
    where
        G: crate::governed_key_registry::ConsensusKeyGovernance + ?Sized,
        F: Fn(cano_crypto::ConsensusSigSuiteId) -> bool,
    {
        for entry in self.validator_set.iter() {
...
///
/// ```ignore
/// use cano_consensus::validator_set::{EpochId, EpochState, StaticEpochStateProvider};
///
