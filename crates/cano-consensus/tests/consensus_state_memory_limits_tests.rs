...
fn eviction_does_not_break_commit_safety() {
    ...
    engine.register_block(block1.clone(), 1, Some(block0), None);

    // Block 2 (child of 1)
    let block2 = make_block_id(2);
    engine.register_block(block2.clone(), 2, Some(block1.clone()), None);

    // Block 3 (child of 2) - will form 3-chain with QCs
    let block3 = make_block_id(3);
    engine.register_block(block3.clone(), 3, Some(block2.clone()), None);
...
fn committed_blocks_never_evicted() {
    ...
    let block0 = make_block_id(0);
    engine.register_block(block0.clone(), 0, None, None);

    let block1 = make_block_id(1);
    engine.register_block(block1.clone(), 1, Some(block0.clone()), None);
