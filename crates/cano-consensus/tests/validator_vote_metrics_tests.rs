...
impl ValidatorVoteRecorder for TestValidatorVoteRecorder {
    fn on_validator_vote(&self, validator_id: ValidatorId, view: u64) {
        ...
        let counters = validators
            .entry(validator_id)
            .or_insert_with(|| TestValidatorCounters::default());
...
fn two_node_records_votes_from_both_validators() {
    ...
    let block_id = match &actions[0] {
        cano_consensus::ConsensusEngineAction::BroadcastProposal(proposal) => {
            let mut id = [0u8; 32];
...
fn extract_block_id(actions: &[cano_consensus::ConsensusEngineAction<ValidatorId>]) -> [u8; 32] {
    match &actions[0] {
        cano_consensus::ConsensusEngineAction::BroadcastProposal(proposal) => {
