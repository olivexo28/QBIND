...
use cano_net::{
    ClientHandshake, ClientHandshakeConfig, KemOpMetrics, KemPrivateKey, ServerHandshake,
...
fn multi_handshake_metrics_aggregate() {
    ...
    for i in 0..N {
        ...
        let client_init = client
            .start(validator_id, &server_kem_pk)
            .expect(&format!("handshake {}: client start should succeed", i));

        let (server_accept, _server_result) = server
            .handle_client_init(&*provider, &client_init)
            .expect(&format!(
                "handshake {}: server handle_client_init should succeed",
                i
            ));
