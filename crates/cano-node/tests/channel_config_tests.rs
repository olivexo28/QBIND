...
//!
//! ```bash
//! cargo test --package cano-node --test channel_config_tests -- --test-threads=1
//! ```
//!
...
use cano_node::async_peer_manager::AsyncPeerManagerConfig;
use cano_node::channel_config::ChannelCapacityConfig;
use cano_node::consensus_net_worker::{AsyncConsensusNetAdapter, AsyncNetSender};
...
fn from_env_uses_defaults_when_vars_unset() {
    ...
    std::env::remove_var("CANO_CONSENSUS_EVENT_CHANNEL_CAPACITY");
    std::env::remove_var("CANO_OUTBOUND_COMMAND_CHANNEL_CAPACITY");
    std::env::remove_var("CANO_ASYNC_PEER_INBOUND_CAPACITY");
    std::env::remove_var("CANO_ASYNC_PEER_OUTBOUND_CAPACITY");
...
fn from_env_parses_valid_values() {
    ...
    std::env::set_var("CANO_CONSENSUS_EVENT_CHANNEL_CAPACITY", "2048");
    std::env::set_var("CANO_OUTBOUND_COMMAND_CHANNEL_CAPACITY", "512");
    std::env::set_var("CANO_ASYNC_PEER_INBOUND_CAPACITY", "4096");
    std::env::set_var("CANO_ASYNC_PEER_OUTBOUND_CAPACITY", "128");
...
fn from_env_rejects_zero_value() {
    ...
    std::env::set_var("CANO_CONSENSUS_EVENT_CHANNEL_CAPACITY", "0");
...
fn from_env_rejects_non_numeric_value() {
    // Set an invalid value
    std::env::set_var("CANO_CONSENSUS_EVENT_CHANNEL_CAPACITY", "not_a_number");
