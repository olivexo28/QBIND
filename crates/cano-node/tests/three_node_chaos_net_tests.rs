...
//!
//! This module provides test-only adversarial networking scenarios for the Canonot
...
//!
//! ```bash
//! cargo test -p cano-node --test three_node_chaos_net_tests
//! ```
...
mod unit_tests {
    ...
    fn should_drop_pass_through_never_drops() {
        ...
        for i in 0..100 {
            assert!(!FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, i));
        }
    }
    ...
    fn should_drop_every_nth_works() {
        ...
        assert!(FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, 0));
        assert!(!FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, 1));
        assert!(!FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, 4));
        assert!(FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, 5));
        assert!(FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, 10));
    }
    ...
    fn should_drop_percentage_50_drops_approximately_half() {
        ...
        for i in 0..1000 {
            if FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, i) {
                drop_count += 1;
            }
        }
    ...
    fn should_drop_percentage_is_deterministic() {
        ...
        let results_1: Vec<bool> = (0..100)
            .map(|i| FaultyNetworkFacade::<cano_node::NullNetworkFacade>::should_drop(&mode, i))
            .collect();
