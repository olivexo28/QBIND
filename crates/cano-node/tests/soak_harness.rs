...
use cano_consensus::hotstuff_state_engine::HotStuffStateEngine;
use cano_consensus::ids::ValidatorId;
use cano_consensus::validator_set::{ConsensusValidatorSet, ValidatorSetEntry};
use cano_consensus::vote_accumulator::ConsensusLimitsConfig;
...
pub fn run_three_node_soak(config: &SoakConfig) -> SoakResult {
    ...
    let mut nodes: Vec<SoakNode> = (0..3)
        .map(|i| SoakNode::new(i, validator_set.clone(), limits.clone()))
        .collect();
