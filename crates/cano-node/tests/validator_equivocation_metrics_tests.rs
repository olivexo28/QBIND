//! Tests for per-validator equivocation metrics (T129) in cano-node.
//!
...
use cano_node::{NodeMetrics, ValidatorEquivocationMetrics, MAX_TRACKED_VALIDATORS};
...
fn format_metrics_empty_when_no_equivocations() {
    ...
    // Should not contain any cano_consensus_validator_equivocations_total lines
    assert!(!output.contains("cano_consensus_validator_equivocations_total{"));
}
...
fn format_metrics_includes_equivocating_validators() {
    ...
    assert!(output.contains("cano_consensus_validator_equivocations_total{validator=\"1\"} 2"));
    assert!(output.contains("cano_consensus_validator_equivocating{validator=\"1\"} 1"));
    assert!(output.contains("cano_consensus_validator_equivocations_total{validator=\"42\"} 1"));
    assert!(output.contains("cano_consensus_validator_equivocating{validator=\"42\"} 1"));
}
...
fn overflow_shown_in_format_metrics() {
    ...
    let output = metrics.format_metrics();
    assert!(output.contains("cano_consensus_validator_equivocation_overflow_count 1"));
}
...
fn node_metrics_format_includes_equivocation_metrics() {
    ...
    assert!(output.contains("# Per-validator equivocation metrics (T129)"));
    assert!(output.contains("cano_consensus_validator_equivocations_total{validator=\"1\"} 1"));
    assert!(output.contains("cano_consensus_validator_equivocating{validator=\"1\"} 1"));
}
