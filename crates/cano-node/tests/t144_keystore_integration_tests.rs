...
fn keystore_returns_not_found_for_missing_entry() {
    ...
    match result {
        Err(cano_node::keystore::KeystoreError::NotFound(entry)) => {
...
fn keystore_returns_invalid_key_for_wrong_suite() {
    ...
    match result {
        Err(cano_node::keystore::KeystoreError::InvalidKey) => {}
...
fn validator_keystore_config_is_clone() {
    let config = ValidatorKeystoreConfig {
        keystore_root: std::path::PathBuf::from("/etc/cano/keystore"),
