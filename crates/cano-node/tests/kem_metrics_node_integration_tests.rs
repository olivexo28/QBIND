...
//!
//! ```bash
//! cargo test -p cano-node --test kem_metrics_node_integration_tests
//! ```
...
async fn kem_metrics_recorded_during_handshake() {
    use cano_net::handshake::{ClientHandshake, ServerHandshake};
...
fn kem_metrics_formatted_in_http_endpoint() {
    ...
    assert!(formatted.contains("cano_net_kem_encaps_total 1"));
