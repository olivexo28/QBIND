...
//!
//! ```bash
//! cargo test -p cano-node --test consensus_net_worker_tests -- --test-threads=1
//! ```
...
use cano_node::async_runner::{AsyncNodeRunner, ConsensusEvent};
use cano_node::consensus_net_worker::{ConsensusNetService, ConsensusNetWorker};
        ...
        res = net_worker.run() => {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    Err(cano_node::AsyncNodeError::Cancelled)
                }
            }
        }
    };
