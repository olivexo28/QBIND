...
//!
//! ```bash
//! cargo test -p cano-node --test t132_three_node_mldsa44_consensus_tests -- --test-threads=1
//! ```
...
use cano_consensus::validator_set::{ConsensusValidatorSet, EpochState, ValidatorSetEntry};
use cano_consensus::verify::ConsensusVerifier;
use cano_consensus::{
    GovernedValidatorKeyRegistry, MultiSuiteCryptoVerifier, SimpleBackendRegistry,
};
use cano_crypto::consensus_sig::ConsensusSigVerifier;
use cano_crypto::{ConsensusSigSuiteId, MlDsa44Backend, SUITE_PQ_RESERVED_1};
use cano_node::peer::PeerId;
use cano_node::startup_validation::{ConsensusStartupValidator, SuitePolicy, ValidatorEnumerator};
...
fn mldsa44_keygen_produces_valid_keys() {
    ...
    for (i, key) in keys.iter().enumerate() {
        ...
        assert_eq!(
            key.public_key.len(),
            cano_crypto::ml_dsa44::ML_DSA_44_PUBLIC_KEY_SIZE,
            "public key should be correct size"
        );
        assert_eq!(
            key.secret_key.len(),
            cano_crypto::ml_dsa44::ML_DSA_44_SECRET_KEY_SIZE,
...
fn mldsa44_epoch_rejects_mixed_suites() {
    use cano_crypto::SUITE_TOY_SHA3;
...
fn mldsa44_verifier_rejects_tampered_vote() {
    use cano_consensus::verify::VerificationError;
...
fn toy_suite_rejected_under_prod_policy() {
    use cano_crypto::SUITE_TOY_SHA3;
    use cano_node::startup_validation::StartupValidationError;
    ...
    impl ConsensusSigVerifier for NoopVerifier {
        fn verify_vote(
            ...
            _: &[u8],
            _: &[u8],
        ) -> Result<(), cano_crypto::consensus_sig::ConsensusSigError> {
            Ok(())
        }
        fn verify_proposal(
            &self,
