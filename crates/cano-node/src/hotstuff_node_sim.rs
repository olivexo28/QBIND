...
pub enum NodeHotstuffHarnessError {
    ...
    /// - Validation failures
    EpochTransition(cano_consensus::validator_set::EpochTransitionError),
    ...
    RuntimeSuiteDowngrade {
        ...
        from_suite: cano_crypto::ConsensusSigSuiteId,
        to_suite: cano_crypto::ConsensusSigSuiteId,
    },
}
...
impl From<cano_consensus::validator_set::EpochTransitionError> for NodeHotstuffHarnessError {
    fn from(e: cano_consensus::validator_set::EpochTransitionError) -> Self {
...
pub struct NodeHotstuffHarness {
    ...
    governance: Option<
        Arc<dyn cano_consensus::governed_key_registry::ConsensusKeyGovernance + Send + Sync>,
    >,
    ...
    signing_key: Option<std::sync::Arc<cano_crypto::ValidatorSigningKey>>,
...
impl NodeHotstuffHarness {
    ...
    ///
    /// ```ignore
    /// use cano_consensus::validator_set::{EpochId, EpochState};
    ///
    ...
    ///
    /// ```ignore
    /// use cano_consensus::validator_set::{EpochState, StaticEpochStateProvider};
    ///
    ...
    pub fn with_governance(
        mut self,
        governance: Arc<
            dyn cano_consensus::governed_key_registry::ConsensusKeyGovernance + Send + Sync,
        >,
    ) -> Self {
    ...
    ///
    /// ```ignore
    /// use cano_node::validator_signer::{LocalKeySigner, ValidatorSigner};
    /// use std::sync::Arc;
    ///
    ...
    ///
    /// ```ignore
    /// use cano_consensus::TimeoutPacemakerConfig;
    /// use std::time::Duration;
    ///
    ...
    ///
    /// ```ignore
    /// use cano_node::verify_pool::ConsensusVerifyPoolConfig;
    ///
    /// let harness = NodeHotstuffHarness::new(...)?
    ...
    pub fn load_persisted_state(&mut self) -> Result<Option<[u8; 32]>, NodeHotstuffHarnessError> {
        use cano_consensus::qc::QuorumCertificate;
    ...
    where
        ...
        BR: cano_consensus::crypto_verifier::ConsensusSigBackendRegistry + Send + Sync,
    {
    ...
    fn emit_timeout_msg(&mut self, view: u64) -> Result<(), NodeHotstuffHarnessError> {
        use cano_consensus::timeout::TIMEOUT_SUITE_ID;
        ...
        let signature = if let Some(ref signer) = self.signer {
            ...
            use cano_consensus::timeout_signing_bytes;
    ...
    fn handle_potential_reconfig_commit(
        ...
        if proposal.header.payload_kind != cano_wire::PAYLOAD_KIND_RECONFIG {
        ...
        if let Some(ref signer) = self.signer {
            ...
            match &mut action {
                ConsensusEngineAction::BroadcastProposal(ref mut proposal) => {
                    ...
                    use cano_crypto::suite_catalog::SUITE_PQ_RESERVED_1;
