...
//! # Design (T89)
//!
//! Following the existing `ConsensusSigMetrics` pattern in `cano-consensus`,
...
use cano_net::kem_metrics::KemOpMetrics;
...
impl PeerNetworkMetrics {
    ...
    pub fn format_metrics(&self) -> String {
        ...
        for peer_id in peer_ids {
            if let Some(counters) = peers.get(&peer_id) {
                ...
                if rate_limit_drop > 0 {
                    output.push_str(&format!(
                        "cano_net_per_peer_drops_total{{peer=\"{}\",reason=\"rate_limit\"}} {}\n",
        ...
        if overflow_count > 0 {
            ...
            if rate_limit_drop > 0 {
                output.push_str(&format!(
                    "cano_net_per_peer_drops_total{{peer=\"overflow\",reason=\"rate_limit\"}} {}\n",
                    rate_limit_drop
                ));
            }
        }
...
impl cano_consensus::ViewDurationRecorder for ConsensusRoundMetrics {
...
/// Output format in `format_metrics()`:
/// ```text
/// cano_consensus_qcs_formed_total 42
/// cano_consensus_votes_observed_total 1234
/// cano_consensus_votes_observed_current_view 12
/// cano_consensus_view_changes_total 41
/// cano_consensus_leader_changes_total 41
/// ```
#[derive(Debug, Default)]
...
impl ConsensusProgressMetrics {
    ...
    pub fn format_metrics(&self) -> String {
        ...
        // QC formation
        output.push_str(&format!(
            "cano_consensus_qcs_formed_total {}\n",
            self.qcs_formed_total()
        ));

        // Vote observation
        output.push_str(&format!(
            "cano_consensus_votes_observed_total {}\n",
            self.votes_observed_total()
        ));
        output.push_str(&format!(
            "cano_consensus_votes_observed_current_view {}\n",
            self.votes_observed_current_view()
        ));

        // View/leader changes
        output.push_str(&format!(
            "cano_consensus_view_changes_total {}\n",
            self.view_changes_total()
        ));
        output.push_str(&format!(
            "cano_consensus_leader_changes_total {}\n",
        ...
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_count {}\n",
            count
        ));
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_sum {}\n",
            sum
        ));
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_bucket{{le=\"100\"}} {}\n",
            b100
        ));
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_bucket{{le=\"500\"}} {}\n",
            b500
        ));
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_bucket{{le=\"2000\"}} {}\n",
            b2s
        ));
        output.push_str(&format!(
            "cano_consensus_qc_formation_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
            binf
        ));
...
impl cano_consensus::ConsensusProgressRecorder for ConsensusProgressMetrics {
...
/// Output format in `format_metrics()`:
/// ```text
/// cano_consensus_validator_votes_total{validator="1"} 123
/// cano_consensus_validator_last_vote_view{validator="1"} 456
/// ```
#[derive(Debug)]
...
impl ValidatorVoteMetrics {
    ...
    pub fn format_metrics(&self) -> String {
        ...
        for validator_id in validator_ids {
            if let Some(counters) = validators.get(&validator_id) {
                ...
                if votes_total > 0 {
                    output.push_str(&format!(
                        "cano_consensus_validator_votes_total{{validator=\"{}\"}} {}\n",
                        validator_id.0, votes_total
                    ));
                    output.push_str(&format!(
                        "cano_consensus_validator_last_vote_view{{validator=\"{}\"}} {}\n",
        ...
        if overflow > 0 {
            output.push_str(&format!(
                "cano_consensus_validator_overflow_count {}\n",
                overflow
            ));
        }
    ...
    /// cano_consensus_validator_votes_total{validator="1"} 123
    /// cano_consensus_validator_last_vote_view{validator="1"} 456
    /// cano_consensus_validator_view_lag{validator="1"} 10
    /// ```
    ///
        ...
        for validator_id in validator_ids {
            if let Some(counters) = validators.get(&validator_id) {
                ...
                if votes_total > 0 {
                    ...
                    output.push_str(&format!(
                        "cano_consensus_validator_view_lag{{validator=\"{}\"}} {}\n",
...
impl cano_consensus::ValidatorVoteRecorder for ValidatorVoteMetrics {
...
/// Output format in `format_metrics()`:
/// ```text
/// cano_consensus_validator_equivocations_total{validator="1"} 2
/// cano_consensus_validator_equivocating{validator="1"} 1
/// ```
#[derive(Debug)]
...
impl ValidatorEquivocationMetrics {
    ...
    pub fn format_metrics(&self) -> String {
        ...
        for validator_id in validator_ids {
            if let Some(counters) = validators.get(&validator_id) {
                ...
                if equivocations_total > 0 {
                    output.push_str(&format!(
                        "cano_consensus_validator_equivocations_total{{validator=\"{}\"}} {}\n",
                        validator_id.0, equivocations_total
                    ));
                    output.push_str(&format!(
                        "cano_consensus_validator_equivocating{{validator=\"{}\"}} {}\n",
        ...
        if overflow > 0 {
            output.push_str(&format!(
                "cano_consensus_validator_equivocation_overflow_count {}\n",
                overflow
            ));
        }
...
impl cano_consensus::ValidatorEquivocationRecorder for ValidatorEquivocationMetrics {
...
impl ViewLagMetrics {
    ...
    pub fn format_metrics(&self) -> String {
        ...
        output.push_str(&format!(
            "cano_consensus_current_view {}\n",
            self.current_view()
        ));
        output.push_str(&format!(
            "cano_consensus_highest_seen_view {}\n",
            self.highest_seen_view()
        ));
        output.push_str(&format!("cano_consensus_view_lag {}\n", self.view_lag()));
        output
    }
}
...
///
/// ```ignore
/// use cano_node::metrics::NodeMetrics;
...
impl NodeMetrics {
    ...
    fn format_kem_metrics(&self) -> String {
        ...
        output.push_str(&format!(
            "cano_net_kem_encaps_total {}\n",
            kem.encaps_total()
        ));
        output.push_str(&format!(
            "cano_net_kem_decaps_total {}\n",
            kem.decaps_total()
        ));
        ...
        output.push_str(&format!(
            "cano_net_kem_encaps_latency_ms_bucket{{le=\"0.1\"}} {}\n",
            encaps_0_1
        ));
        output.push_str(&format!(
            "cano_net_kem_encaps_latency_ms_bucket{{le=\"1\"}} {}\n",
            encaps_1
        ));
        output.push_str(&format!(
            "cano_net_kem_encaps_latency_ms_bucket{{le=\"10\"}} {}\n",
            encaps_10
        ));
        output.push_str(&format!(
            "cano_net_kem_encaps_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
            encaps_inf
        ));
        output.push_str(&format!(
            "cano_net_kem_encaps_latency_ms_count {}\n",
            encaps_inf
        ));
        ...
        output.push_str(&format!(
            "cano_net_kem_decaps_latency_ms_bucket{{le=\"0.1\"}} {}\n",
            decaps_0_1
        ));
        output.push_str(&format!(
            "cano_net_kem_decaps_latency_ms_bucket{{le=\"1\"}} {}\n",
            decaps_1
        ));
        output.push_str(&format!(
            "cano_net_kem_decaps_latency_ms_bucket{{le=\"10\"}} {}\n",
            decaps_10
        ));
        output.push_str(&format!(
            "cano_net_kem_decaps_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
            decaps_inf
        ));
        output.push_str(&format!(
            "cano_net_kem_decaps_latency_ms_count {}\n",
            decaps_inf
        ));
    ...
    pub fn format_metrics_with_crypto(
        &self,
        consensus_sig_metrics: Option<&cano_consensus::ConsensusSigMetrics>,
...
mod tests {
    ...
    fn kem_metrics_formatting() {
        ...
        assert!(formatted.contains("cano_net_kem_encaps_total"));
        assert!(formatted.contains("cano_net_kem_decaps_total"));
        assert!(formatted.contains("cano_net_kem_encaps_latency_ms_bucket"));
        assert!(formatted.contains("cano_net_kem_decaps_latency_ms_bucket"));
        assert!(formatted.contains("cano_net_kem_encaps_latency_ms_count"));
        assert!(formatted.contains("cano_net_kem_decaps_latency_ms_count"));

        // Check that values are non-zero
        assert!(formatted.contains("cano_net_kem_encaps_total 2"));
        assert!(formatted.contains("cano_net_kem_decaps_total 1"));
    }
    ...
    fn consensus_progress_metrics_format_output() {
        ...
        assert!(output.contains("cano_consensus_qcs_formed_total 2"));
        assert!(output.contains("cano_consensus_votes_observed_total 3"));
        assert!(output.contains("cano_consensus_votes_observed_current_view 1"));
        assert!(output.contains("cano_consensus_view_changes_total 1"));
        assert!(output.contains("cano_consensus_leader_changes_total 1"));
        assert!(output.contains("cano_consensus_qc_formation_latency_ms_count 1"));
        assert!(output.contains("cano_consensus_qc_formation_latency_ms_sum 100"));
    }
    ...
    fn node_metrics_includes_consensus_progress_metrics() {
        ...
        let output = metrics.format_metrics();
        assert!(output.contains("# Consensus progress metrics (T127)"));
        assert!(output.contains("cano_consensus_qcs_formed_total 1"));
        assert!(output.contains("cano_consensus_votes_observed_total 1"));
        assert!(output.contains("cano_consensus_view_changes_total 1"));
    }
