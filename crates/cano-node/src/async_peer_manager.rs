...
//!
//! The existing KEMTLS implementation (`cano-net`) is blocking. Handshakes are
...
//! variable `CANO_TRANSPORT_SECURITY_MODE=plain|kemtls`.
//!
...
use cano_consensus::network::{ConsensusNetworkEvent, NetworkError};
...
///
/// The mode can be set via the `CANO_TRANSPORT_SECURITY_MODE` environment variable:
...
impl TransportSecurityMode {
    ...
    /// Load from the `CANO_TRANSPORT_SECURITY_MODE` environment variable.
    ///
    /// Returns the configured mode, or `None` if the variable is not set
    /// or has an unrecognized value.
    pub fn from_env() -> Option<Self> {
        std::env::var("CANO_TRANSPORT_SECURITY_MODE")
            .ok()
...
impl Default for AsyncPeerManagerConfig {
    fn default() -> Self {
        ...
        //
        // For production use, set CANO_TRANSPORT_SECURITY_MODE=kemtls or use
                    ...
                    Err(e) => {
                        ...
                        let reason = match &e {
                            ...
                            crate::secure_channel::ChannelError::Net(net_err) => {
                                ...
                                match net_err {
                                    cano_net::NetError::KeySchedule(_)
                                    | cano_net::NetError::Aead(_)
                                    | cano_net::NetError::UnsupportedSuite(_) => {
                                        KemtlsHandshakeFailureReason::Crypto
                                    }
                                    cano_net::NetError::Protocol(_)
                                    | cano_net::NetError::NonceOverflow => {
                                        KemtlsHandshakeFailureReason::Protocol
                                    }
                                }
                            }
