...
use cano_consensus::verify_job::{ConsensusMsgKind, ConsensusVerifyJob, ConsensusVerifyResult};
...
impl SuiteAwareValidatorKeyProvider for MockKeyProvider {
    fn get_suite_and_key(
        &self,
        _id: cano_consensus::ids::ValidatorId,
...
impl ConsensusSigBackendRegistry for MockBackendRegistry {
    ...
    ) -> Option<std::sync::Arc<dyn cano_crypto::consensus_sig::ConsensusSigVerifier>> {
...
impl cano_crypto::consensus_sig::ConsensusSigVerifier for MockVerifier {
    fn verify_vote(
        &self,
        _validator_id: u64,
        _pk: &[u8],
