//! Run 281 — release-built helper for the Run 280 durable-completion
//! external-publication confirmation consumer / external-publication-receipt
//! sink boundary.
//!
//! Example-only release evidence harness. It drives the actual Run 256 backend ->
//! Run 258 receipt -> Run 260 acknowledgement -> Run 262 consumer -> Run 264
...
//! external-publication confirmation chain and then evaluates the Run 278
//! external-publication receipt. Projection is intentionally from
//! `input.external_publication_receipt_binding` through
...
use qbind_node::pqc_governance_durable_completion_external_publication_confirmation::{
    ...
    ExternalExternalPublicationConfirmationSink,
    FixtureDurableCompletionExternalPublicationConfirmationSink,
    GovernanceDurableCompletionExternalPublicationConfirmationSink,
    MainNetExternalPublicationConfirmationSink, ProductionExternalPublicationConfirmationSink,
};
use qbind_node::pqc_governance_durable_completion_external_publication_receipt::{
    ...
    GovernanceDurableCompletionExternalPublicationReceiptSink,
    MainNetExternalPublicationReceiptSink, ProductionExternalPublicationReceiptSink,
};
...
// Run 281 — Run 276 external-publication confirmation stage symbols used by the
// release symbol reachability probe (imported here so the release helper source
// grep-proves it links the real Run 276 boundary symbols in release mode).
use qbind_node::pqc_governance_durable_completion_external_publication_confirmation::{
    durable_completion_external_publication_confirmation_consumer_required,
    ...
    DurableCompletionExternalPublicationConfirmationBinding,
    DurableCompletionExternalPublicationConfirmationDigest,
    DurableCompletionExternalPublicationConfirmationEnvironment,
    ...
    DurableCompletionExternalPublicationConfirmationLedgerStatus,
    DurableCompletionExternalPublicationConfirmationRecord,
    DurableCompletionExternalPublicationConfirmationResponse,
    DurableCompletionExternalPublicationConfirmationSurface,
    DurableCompletionExternalPublicationConfirmationTranscriptDigest,
};
use qbind_node::pqc_governance_durable_completion_external_publication_receipt::{
    durable_completion_external_publication_receipt_confirmation_required,
    durable_completion_external_publication_receipt_consumer_required,
    ...
    external_publication_receipt_record_digest, external_publication_receipt_request_digest,
    external_publication_receipt_response_digest, external_publication_receipt_transcript_digest,
    DurableCompletionExternalPublicationReceiptBinding,
    DurableCompletionExternalPublicationReceiptDigest,
    DurableCompletionExternalPublicationReceiptEnvironment,
    DurableCompletionExternalPublicationReceiptLedgerRecord,
    DurableCompletionExternalPublicationReceiptLedgerSnapshot,
    DurableCompletionExternalPublicationReceiptLedgerStatus,
    DurableCompletionExternalPublicationReceiptRecord,
    DurableCompletionExternalPublicationReceiptResponse,
    DurableCompletionExternalPublicationReceiptSurface,
...
use qbind_node::pqc_governance_durable_completion_external_publication_acknowledgement::{
    ...
    DurableCompletionExternalPublicationAcknowledgementWindow,
    ExternalExternalPublicationAcknowledgementSink,
    FixtureDurableCompletionExternalPublicationAcknowledgementSink,
    GovernanceDurableCompletionExternalPublicationAcknowledgementSink,
    MainNetExternalPublicationAcknowledgementSink,
    ProductionExternalPublicationAcknowledgementSink,
};
// Run 281 — additional Run 278 production library symbols exercised by the
// release symbol reachability probe (imported here so the release helper source
// grep-proves it links the real Run 276 boundary symbols in release mode).
use qbind_node::pqc_governance_durable_completion_external_publication_acknowledgement::{
    durable_completion_external_publication_acknowledgement_confirmation_required,
    durable_completion_external_publication_acknowledgement_consumer_required,
    ...
    durable_completion_external_publication_acknowledgement_no_real_settlement,
    durable_completion_external_publication_acknowledgement_no_real_settlement_finality,
    durable_completion_external_publication_acknowledgement_no_real_settlement_finality_projection,
    durable_completion_external_publication_acknowledgement_no_real_settlement_receipt,
    ...
    DurableCompletionExternalPublicationAcknowledgementBinding,
    DurableCompletionExternalPublicationAcknowledgementDigest,
    DurableCompletionExternalPublicationAcknowledgementEnvironment,
    ...
    DurableCompletionExternalPublicationAcknowledgementLedgerStatus,
    DurableCompletionExternalPublicationAcknowledgementRecord,
    DurableCompletionExternalPublicationAcknowledgementResponse,
    DurableCompletionExternalPublicationAcknowledgementSurface,
...
const ACK280_ID: &str = "fixture-external-publication-acknowledgement-0001";
const ACK280_DOMAIN_TAG: &str = "QBIND:run280:domain-separation:v1";
const ACK280_RECORD_ID: &str = "durable-completion-external-publication-acknowledgement-0001";
...
fn default_action() -> ActionLabel {
    ActionLabel {
        ...
        external_publication_acknowledgement_record_id: ACK280_RECORD_ID.to_string(),
...
// ===========================================================================
// Run 276 external-publication-confirmation attachment (new projection source)
// Builds a REAL Run 276 recorded confirmation from the Run 274 prior, so the
// Run 278 cur-layer projects from a genuine attached confirmation outcome.
// ===========================================================================
// ===========================================================================
...
fn run276_external_publication_confirmation_identity(
) -> DurableCompletionExternalPublicationConfirmationIdentity {
    DurableCompletionExternalPublicationConfirmationIdentity {
...
fn attach_run274_external_publication_confirmation(
    environment: TrustBundleEnvironment,
    action: &ActionLabel,
    external_publication_confirmation_duplicate: bool,
    ...
    let id = run276_external_publication_confirmation_identity();
    ...
    let outcome = if external_publication_confirmation_duplicate {
    ...
    let record = ledger
        ...
        .expect("recorded Run 276 external publication confirmation");
    AttachedExternalPublicationConfirmation {
        outcome,
        confirmation_record_id: action.confirmation_record_id.clone(),
...
// ===========================================================================
// Run 278 external-publication-receipt attachment (new projection source)
// Builds a REAL Run 278 recorded receipt from the Run 276 prior, so the
// Run 280 cur-layer projects from a genuine attached receipt outcome.
// ===========================================================================
#[allow(dead_code)]
...
fn run278_external_publication_receipt_identity(
) -> DurableCompletionExternalPublicationReceiptIdentity {
    DurableCompletionExternalPublicationReceiptIdentity {
...
fn attach_run274_external_publication_receipt(
    environment: TrustBundleEnvironment,
    action: &ActionLabel,
    external_publication_receipt_duplicate: bool,
) -> AttachedExternalPublicationReceipt {
    let external_publication_confirmation =
        attach_run274_external_publication_confirmation(environment, action, false);
    ...
    let id = run278_external_publication_receipt_identity();
    ...
    let outcome = if external_publication_receipt_duplicate {
    ...
    let record = ledger
        ...
        .expect("recorded Run 278 external publication receipt");
    AttachedExternalPublicationReceipt {
        outcome,
        external_publication_receipt_record_id: action
            .external_publication_receipt_record_id
...
fn ack_identity(
    policy: DurableCompletionExternalPublicationAcknowledgementPolicy,
    kind: DurableCompletionExternalPublicationAcknowledgementKind,
) -> DurableCompletionExternalPublicationAcknowledgementIdentity {
    DurableCompletionExternalPublicationAcknowledgementIdentity {
        confirmation_id: ACK280_ID.to_string(),
        kind,
        policy,
        domain_separation_tag: ACK280_DOMAIN_TAG.to_string(),
    }
}
...
fn ctx_action(
    ...
    let external_publication_receipt =
        attach_run274_external_publication_receipt(environment, action, consumer_duplicate);
    ...
    let expectations = DurableCompletionExternalPublicationAcknowledgementExpectations {
        ...
        expected_external_publication_acknowledgement_kind: kind,
        expected_external_publication_acknowledgement_policy: policy,
        expected_domain_separation_tag: ACK280_DOMAIN_TAG.to_string(),
    };
    Ctx {
        env,
        runtime,
...
impl Ctx {
    #[allow(clippy::too_many_arguments)]
    fn input_with_outcome_publication(
        &self,
        policy: DurableCompletionExternalPublicationAcknowledgementPolicy,
        ...
        confirmation: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        external_publication_receipt: DurableCompletionExternalPublicationReceiptOutcome,
    ) -> DurableCompletionExternalPublicationAcknowledgementInput {
        DurableCompletionExternalPublicationAcknowledgementInput {
            policy,
            ...
            external_publication_receipt_binding: external_publication_receipt,
    ...
    fn input(
        ...
        self.input_with_outcome_publication(
            policy,
            replay,
            pipeline,
            sink,
            ...
            self.external_publication_receipt.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionExternalPublicationAcknowledgementInput {
        self.input_with_outcome_publication(
...
fn fixture_sink() -> FixtureDurableCompletionExternalPublicationAcknowledgementSink {
    FixtureDurableCompletionExternalPublicationAcknowledgementSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionExternalPublicationAcknowledgementPolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::LegacyBypassNoExternalPublicationAcknowledgement
    );
...
fn disabled_backend_policy_never_invokes_receipt_sink() {
    let c = devnet_ctx();
    let input = c.input_with_outcome_publication(
        DurableCompletionExternalPublicationAcknowledgementPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        DurableCompletionExternalPublicationReceiptOutcome::LegacyBypassNoExternalPublicationReceipt,
    );
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = fixture_sink();
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(external_publication_acknowledgement_outcome_authorizes_record(&outcome));
    assert!(external_publication_acknowledgement_outcome_projects_to_recorded(&outcome));
    assert_eq!(sink.invocations(), 1);
    assert_eq!(ledger.len(), 1);
    assert!(ledger.contains(ACK280_RECORD_ID));
    assert_eq!(
        external_publication_acknowledgement_identity_digest(&c.request.identity),
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.external_publication_acknowledgement_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!external_publication_acknowledgement_outcome_authorizes_record(&second));
    assert!(external_publication_acknowledgement_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run268_duplicate_idempotent_outcome_publication_only_matches_existing_never_creates() {
    // A real Run 274 ExternalPublicationReceiptDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.external_publication_receipt.outcome,
        DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_external_publication_acknowledgement(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_external_publication_acknowledgement(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAcknowledgementPolicy::ProductionExternalPublicationAcknowledgementRequired,
        DurableCompletionExternalPublicationAcknowledgementKind::ProductionExternalPublicationAcknowledgementUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = ProductionExternalPublicationAcknowledgementSink::default();
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAcknowledgementPolicy::MainNetExternalPublicationAcknowledgementRequired,
        DurableCompletionExternalPublicationAcknowledgementKind::MainNetExternalPublicationAcknowledgementUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = MainNetExternalPublicationAcknowledgementSink::default();
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAcknowledgementPolicy::ExternalExternalPublicationAcknowledgementRequired,
        DurableCompletionExternalPublicationAcknowledgementKind::ExternalExternalPublicationAcknowledgementUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = ExternalExternalPublicationAcknowledgementSink::default();
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn validator_set_rotation_and_policy_change_unsupported() {
    let c = devnet_ctx();
    let rotation = c.input_with_outcome_publication(
        DurableCompletionExternalPublicationAcknowledgementPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        DurableCompletionExternalPublicationReceiptOutcome::ValidatorSetRotationUnsupportedNoReceipt,
    );
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = fixture_sink();
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::ValidatorSetRotationUnsupportedNoAcknowledgement
    );
    assert_eq!(sink.invocations(), 0);

    let policy_change = c.input_with_outcome_publication(
        DurableCompletionExternalPublicationAcknowledgementPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        c.consumer.ack.outcome.clone(),
        c.consumer.outcome.clone(),
        DurableCompletionExternalPublicationReceiptOutcome::PolicyChangeUnsupportedNoReceipt,
    );
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &policy_change,
        &c.expectations,
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::PolicyChangeUnsupportedNoAcknowledgement
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_non_recording_outcome_publication(
    external_publication_receipt: DurableCompletionExternalPublicationReceiptOutcome,
    expected: DurableCompletionExternalPublicationAcknowledgementOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_outcome_publication(
        DurableCompletionExternalPublicationAcknowledgementPolicy::FixtureAllowed,
...
fn non_recording_outcome_publication_outcomes_never_record_confirmation() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Confirmation;
    use DurableCompletionExternalPublicationReceiptOutcome as Finalization;
    assert_non_recording_outcome_publication(
        Finalization::LegacyBypassNoExternalPublicationReceipt,
        Confirmation::LegacyBypassNoExternalPublicationAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::RejectedBeforeExternalPublicationConfirmationNoReceipt,
        Confirmation::RejectedBeforeExternalPublicationReceiptNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationConfirmationDidNotRecordNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationReceiptRejectedBeforeRecord,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationReceiptRecordFailedNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationReceiptRolledBackNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationReceiptRollbackFailedFatalNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ProductionExternalPublicationReceiptUnavailableNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::MainNetExternalPublicationReceiptUnavailableNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ExternalExternalPublicationReceiptUnavailableNoReceipt,
        Confirmation::ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::MainNetPeerDrivenApplyRefusedNoReceipt,
        Confirmation::MainNetPeerDrivenApplyRefusedNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::ValidatorSetRotationUnsupportedNoReceipt,
        Confirmation::ValidatorSetRotationUnsupportedNoAcknowledgement,
    );
    assert_non_recording_outcome_publication(
        Finalization::PolicyChangeUnsupportedNoReceipt,
        Confirmation::PolicyChangeUnsupportedNoAcknowledgement,
    );
}
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::RejectedBeforeExternalPublicationReceiptNoAcknowledgement
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionExternalPublicationAcknowledgementRequest),
) {
    let c = devnet_ctx();
...
fn wrong_external_publication_acknowledgement_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.external_publication_acknowledgement_record_id = "other-outcome_publication".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_external_publication_acknowledgement(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_receipt(
    fault: DurableCompletionExternalPublicationAcknowledgementFault,
    expected: DurableCompletionExternalPublicationAcknowledgementOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink =
        FixtureDurableCompletionExternalPublicationAcknowledgementSink::with_fault(fault);
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRecordFailedNoAcknowledgement,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRolledBackNoAcknowledgement,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRollbackFailedFatalNoAcknowledgement,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_receipt(
        ...
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement,
    );
}
...
fn only_recorded_outcome_publication_outcome_creates_external_publication_acknowledgement_request_intent(
) {
    use DurableCompletionExternalPublicationAcknowledgementRequestIntent as Intent;
    use DurableCompletionExternalPublicationReceiptOutcome as Finalization;
    assert_eq!(
        project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request(
            &Finalization::ExternalPublicationReceiptRecorded
        ),
    ...
    assert_eq!(
        project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request(
            &Finalization::ExternalPublicationReceiptDuplicateIdempotent
        ),
...
fn non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request(
) {
    use DurableCompletionExternalPublicationAcknowledgementRequestIntent as Intent;
    use DurableCompletionExternalPublicationReceiptOutcome as Finalization;
    for outcome_publication in [
        Finalization::LegacyBypassNoExternalPublicationReceipt,
        Finalization::RejectedBeforeExternalPublicationConfirmationNoReceipt,
        Finalization::ExternalPublicationConfirmationDidNotRecordNoReceipt,
        Finalization::ExternalPublicationReceiptRejectedBeforeRecord,
        ...
        assert!(matches!(
            project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request(&outcome_publication),
            Intent::NoReceipt(_)
        ));
    }
}
...
fn recovered_ledger() -> DurableCompletionExternalPublicationAcknowledgementLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_external_publication_acknowledgement(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let record = if with_record {
        ledger.find(ACK280_RECORD_ID)
    } else {
        None
    };
    let outcome = recover_durable_completion_external_publication_acknowledgement_window(
        &input,
        window,
        DurableCompletionExternalPublicationAcknowledgementKind::FixtureInMemory,
        record,
...
fn pre_external_publication_receipt_windows_fail_closed_no_commitment() {
...
fn after_external_publication_receipt_request_before_record_rejects_before_record() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Finalization;
    use DurableCompletionExternalPublicationAcknowledgementWindow as Window;
    assert_window(
...
fn after_external_publication_receipt_record_before_success_requires_explicit_matching_record() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Finalization;
    use DurableCompletionExternalPublicationAcknowledgementWindow as Window;
    assert_window(
    ...
    assert_window(
        Window::AfterExternalPublicationAcknowledgementRecordBeforeExternalPublicationAcknowledgementSuccess,
        true,
        Finalization::ExternalPublicationAcknowledgementRecorded,
    );
}

fn after_external_publication_receipt_success_recovers_as_recorded() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Finalization;
    use DurableCompletionExternalPublicationAcknowledgementWindow as Window;
    assert_window(
        Window::AfterExternalPublicationAcknowledgementSuccess,
        true,
        Finalization::ExternalPublicationAcknowledgementRecorded,
    );
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_external_publication_acknowledgement_window(
        &input,
        DurableCompletionExternalPublicationAcknowledgementWindow::AfterExternalPublicationAcknowledgementSuccess,
        DurableCompletionExternalPublicationAcknowledgementKind::ProductionExternalPublicationAcknowledgementUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionExternalPublicationAcknowledgementSink::with_fault(
        DurableCompletionExternalPublicationAcknowledgementFault::RolledBackNoReceipt,
    );
    let outcome = evaluate_durable_completion_external_publication_acknowledgement(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_external_publication_acknowledgement_rejection_is_non_mutating());
    assert!(durable_completion_external_publication_acknowledgement_never_calls_run_070());
    assert!(
        durable_completion_external_publication_acknowledgement_never_mutates_live_pqc_trust_state(
        )
    );
    ...
    assert!(durable_completion_external_publication_acknowledgement_no_external_publication());
    assert!(durable_completion_external_publication_acknowledgement_no_real_audit_ledger());
    assert!(durable_completion_external_publication_acknowledgement_pipeline_success_required());
    assert!(durable_completion_external_publication_acknowledgement_sink_receipt_required());
    assert!(durable_completion_external_publication_acknowledgement_completion_report_required());
    assert!(
        durable_completion_external_publication_acknowledgement_finalization_projection_required()
    );
    assert!(durable_completion_external_publication_acknowledgement_attestation_required());
    assert!(durable_completion_external_publication_acknowledgement_backend_submission_required());
    assert!(durable_completion_external_publication_acknowledgement_receipt_required());
    assert!(
        durable_completion_external_publication_acknowledgement_record_required_before_reported()
    );
    assert!(durable_completion_external_publication_acknowledgement_failed_record_never_records());
    assert!(durable_completion_external_publication_acknowledgement_rollback_never_records());
    assert!(
        durable_completion_external_publication_acknowledgement_ambiguous_window_fails_closed()
    );
    assert!(
    ...
    assert!(durable_completion_external_publication_acknowledgement_external_unavailable());
    assert!(
        durable_completion_external_publication_acknowledgement_validator_set_rotation_unsupported(
        )
    );
    assert!(durable_completion_external_publication_acknowledgement_policy_change_unsupported());
    assert!(
        durable_completion_external_publication_acknowledgement_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionExternalPublicationAcknowledgementInput = c.recorded();
    ...
    assert!(matches!(
        intent,
        DurableCompletionExternalPublicationAcknowledgementRequestIntent::CreateRequest
    ));
    ...
    let identity_digest: DurableCompletionExternalPublicationAcknowledgementDigest =
        external_publication_acknowledgement_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionExternalPublicationAcknowledgementDigest =
        external_publication_acknowledgement_request_digest(&c.request);
    let response = DurableCompletionExternalPublicationAcknowledgementResponse {
        external_publication_acknowledgement_record_id: c
            .request
            .external_publication_acknowledgement_record_id
            .clone(),
    ...
    let response_digest: DurableCompletionExternalPublicationAcknowledgementDigest =
        external_publication_acknowledgement_response_digest(&response);
    let record: DurableCompletionExternalPublicationAcknowledgementRecord = c.request.to_record();
    let record_digest: DurableCompletionExternalPublicationAcknowledgementDigest =
        external_publication_acknowledgement_record_digest(&record);
    let transcript_digest: DurableCompletionExternalPublicationAcknowledgementTranscriptDigest =
        external_publication_acknowledgement_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
    ...
    let ledger_record: &DurableCompletionExternalPublicationAcknowledgementLedgerRecord = ledger
        .find(ACK280_RECORD_ID)
        .expect("external-publication-acknowledgement ledger record");
    assert_eq!(
        ledger_record.status,
        DurableCompletionExternalPublicationAcknowledgementLedgerStatus::Recorded
    );
    let snapshot: DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot =
        ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionExternalPublicationAcknowledgementSurface = input.surface();
    let _environment: DurableCompletionExternalPublicationAcknowledgementEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionExternalPublicationAcknowledgementBinding =
        input.runtime_binding.clone();

    assert!(DurableCompletionExternalPublicationAcknowledgementPolicy::Disabled.is_disabled());
    assert!(
        DurableCompletionExternalPublicationAcknowledgementPolicy::FixtureAllowed.allows_fixture()
    );
    assert_eq!(
        DurableCompletionExternalPublicationAcknowledgementPolicy::ProductionExternalPublicationAcknowledgementRequired.tag(),
        "production-external-publication-acknowledgement-required"
    );
    assert!(DurableCompletionExternalPublicationAcknowledgementKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionExternalPublicationAcknowledgementKind::ProductionExternalPublicationAcknowledgementUnavailable.is_unavailable());
    assert_eq!(
        DurableCompletionExternalPublicationAcknowledgementKind::Disabled.tag(),
        "disabled"
    );
    let no_confirmation =
        DurableCompletionExternalPublicationAcknowledgementOutcome::LegacyBypassNoExternalPublicationAcknowledgement;
    assert!(no_confirmation.is_legacy_bypass());
    assert!(no_confirmation.no_commitment());
    assert!(
        DurableCompletionExternalPublicationAcknowledgementOutcome::MainNetPeerDrivenApplyRefusedNoAcknowledgement
    ...
    assert!(durable_completion_external_publication_acknowledgement_consumer_required());
    assert!(durable_completion_external_publication_acknowledgement_confirmation_required());
    assert!(durable_completion_external_publication_acknowledgement_no_real_settlement());
    assert!(durable_completion_external_publication_acknowledgement_no_real_settlement_finality());
    assert!(durable_completion_external_publication_acknowledgement_no_real_settlement_receipt());
    assert!(
        durable_completion_external_publication_acknowledgement_no_real_settlement_finality_projection()
    );
    assert!(
    ...
    assert!(durable_completion_external_publication_acknowledgement_no_real_external_publication_acknowledgement());

    let mut prod = ProductionExternalPublicationAcknowledgementSink::default();
    let mut mainnet = MainNetExternalPublicationAcknowledgementSink::default();
    let mut external = ExternalExternalPublicationAcknowledgementSink::default();
    let mut prod_ledger = DurableCompletionExternalPublicationAcknowledgementLedger::new();
    assert_eq!(
        prod.project_durable_completion_external_publication_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_external_publication_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_external_publication_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_281_durable_completion_external_publication_acknowledgement_release_binary/helper_evidence/run_281")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "run268_duplicate_idempotent_outcome_publication_only_matches_existing_never_creates", run268_duplicate_idempotent_outcome_publication_only_matches_existing_never_creates as fn()),
        ...
        ("rejection_fail_closed", "non_recording_outcome_publication_outcomes_never_record_confirmation", non_recording_outcome_publication_outcomes_never_record_confirmation as fn()),
        ...
        ("rejection_fail_closed", "wrong_external_publication_acknowledgement_record_id_rejected_before_record", wrong_external_publication_acknowledgement_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request", non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request as fn()),
        ...
        ("recovery_crash_window", "pre_external_publication_receipt_windows_fail_closed_no_commitment", pre_external_publication_receipt_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_external_publication_receipt_request_before_record_rejects_before_record", after_external_publication_receipt_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_external_publication_receipt_record_before_success_requires_explicit_matching_record", after_external_publication_receipt_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_external_publication_receipt_success_recovers_as_recorded", after_external_publication_receipt_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_outcome_publication_outcome_creates_external_publication_acknowledgement_request_intent", only_recorded_outcome_publication_outcome_creates_external_publication_acknowledgement_request_intent as fn()),
        ("projection", "non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request", non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request as fn()),
        ("projection", "run268_duplicate_idempotent_outcome_publication_only_matches_existing_never_creates", run268_duplicate_idempotent_outcome_publication_only_matches_existing_never_creates as fn()),
        ("external_publication_acknowledgement_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("external_publication_acknowledgement_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("external_publication_acknowledgement_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_outcome_publication_outcomes_never_record_confirmation", non_recording_outcome_publication_outcomes_never_record_confirmation as fn()),
        ("non_mutation", "non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request", non_recording_outcome_publication_outcomes_create_no_external_publication_acknowledgement_request as fn()),
    ...
    summary.push_str(
        "Run 281 durable-completion external-publication-acknowledgement release helper\n",
    );
    summary.push_str(&format!(
        "verdict: {}\n",
        if total_fail == 0 { "PASS" } else { "FAIL" }
    ));
    summary.push_str("projection_rule: input.external_publication_receipt_binding -> project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request\n");
    summary.push_str("attached_chain: Run256 backend -> Run258 receipt -> Run260 acknowledgement -> Run262 consumer -> Run264 settlement projection -> Run266 settlement commitment -> Run268 settlement finalization -> Run270 settlement-receipt acknowledgement -> Run272 settlement-outcome report -> Run274 settlement-outcome publication -> Run276 external-publication confirmation -> Run278 external-publication receipt -> Run280 external-publication acknowledgement\n");
    ...
    fs::write(
        outdir.join("fixtures/run_281_projection_rule.txt"),
        "input.external_publication_receipt_binding\nproject_external_publication_receipt_outcome_to_external_publication_acknowledgement_request\nExternalPublicationReceiptRecorded -> CreateRequest\n",
