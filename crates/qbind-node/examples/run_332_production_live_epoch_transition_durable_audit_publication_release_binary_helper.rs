//! Run 332 — release-binary helper for the Run 331 **live epoch-transition
//! durable-audit publication authorization / external-publication preparation
//! boundary**.
//!
//! Release-binary evidence for the Run 331 live epoch-transition durable-audit
//! publication authorization / external-publication preparation boundary
...
//! release-built production library symbols. It exercises the **real** Run 331
//! `ProductionLiveEpochTransitionDurableAuditPublicationExecutor` and proves,
...
//! 325 -> 327 -> 329 chain to produce a verified, accepted Run 329/330 live
//! epoch-transition audit-ledger-commitment decision (`is_accept()` with
//! `Some(audit_ledger_commitment_artifact)`), then feeds that decision into
//! the Run 331 live epoch-transition durable-audit-publication executor, which
//! emits only a typed, deterministic, non-mutating durable-audit-publication /
//! external-publication-preparation artifact.
//!
...
//! enablement, durable-audit-publication default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! durable-audit-publication artifacts; it never applies a live validator-set
...
//! `LiveEpochTransitionDurableAuditPublicationFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_332.md`.
#![allow(dead_code)]
...
fn u_exec_with_policy(
    policy: ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy,
) -> ProductionLiveEpochTransitionDurableAuditPublicationExecutor {
    ProductionLiveEpochTransitionDurableAuditPublicationExecutor::new(
...
/// assert the resulting outcome (fail-closed, no artifact).
fn u_reject_inputs(
    mutate: impl FnOnce(&mut ProductionLiveEpochTransitionDurableAuditPublicationInputs),
    expected: UO,
) {
    let mut c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    mutate(&mut c.inputs);
    let d = u_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
    assert!(d.durable_audit_publication_artifact.is_none());
    assert!(!d.is_accept());
    assert!(d.outcome.is_non_mutating());
}
...
/// assert the resulting outcome.
fn u_reject_source(
    source: LiveEpochTransitionDurableAuditPublicationAuthoritySource,
    expected: UO,
) {
    let mut c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
...
// ===========================================================================
// A. Accepted / compatible source-test audit-ledger-commitment artifacts
// ===========================================================================

fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        let c = u_case(TrustBundleEnvironment::Devnet, sc);
        let d = u_eval(&c);
        assert!(d.is_accept(), "scenario must accept");
        assert!(d.authorizes_future_mutation_only());
        let art = d.durable_audit_publication_artifact.as_ref().unwrap();
        assert_eq!(art.staged_kind, expected_crc4_kind(sc));
        assert_eq!(art.durable_audit_publication_nonce, CRC4_NONCE);
...
fn accept_all_scenarios_testnet() {
    for sc in ALL_SC {
        let c = u_case(TrustBundleEnvironment::Testnet, sc);
        let d = u_eval(&c);
        assert!(d.is_accept());
        let art = d.durable_audit_publication_artifact.as_ref().unwrap();
...
fn accept_outcome_carries_kind_env_target_nonce() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = u_eval(&c);
    match d.outcome {
        UO::AcceptedSourceTestLiveEpochTransitionDurableAuditPublication {
            execution_kind,
            environment,
            epoch_transition_target,
            durable_audit_publication_nonce,
        } => {
            assert_eq!(execution_kind, UK::StageApplyValidatorAdd);
            assert_eq!(environment, TrustBundleEnvironment::Devnet);
            assert_eq!(
                epoch_transition_target,
                c.request.proposed_epoch_transition_target
            );
            assert_eq!(durable_audit_publication_nonce, CRC4_NONCE);
        }
        other => panic!("unexpected outcome: {other:?}"),
    }
}

fn accept_artifact_reexposes_consumed_audit_ledger_commitment_transcript() {
    let dec = ct_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec
        .audit_ledger_commitment_artifact
        .as_ref()
        .unwrap()
        .epoch_transition_target;
    let inputs = u_inputs(TrustBundleEnvironment::Devnet, &dec);
    let handoff_id = dec.audit_ledger_commitment_id.clone();
    let handoff_req = dec.request_id.clone();
    let handoff_digest = dec.audit_ledger_commitment_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionDurableAuditPublicationExecutor::source_test();
    let d = exec.evaluate_live_epoch_transition_durable_audit_publication(
        &request,
        &inputs,
        &empty_replay331(),
    );
    let art = d.durable_audit_publication_artifact.as_ref().unwrap();
    assert_eq!(art.audit_ledger_commitment_decision_id, handoff_id);
    assert_eq!(art.audit_ledger_commitment_request_id, handoff_req);
    assert_eq!(art.audit_ledger_commitment_intent_digest, handoff_digest);
    assert_eq!(
        art.audit_ledger_commitment_transcript_digest,
        handoff_transcript
    );
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let art = d.durable_audit_publication_artifact.as_ref().unwrap();
    assert_eq!(
        d.durable_audit_publication_id,
        art.durable_audit_publication_id
    );
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(
        d.durable_audit_publication_digest,
        art.durable_audit_publication_digest
    );
    assert_eq!(d.transcript_digest, art.transcript_digest);
    assert!(!d.durable_audit_publication_id.is_empty());
    assert!(!d.request_id.is_empty());
    assert!(!d.durable_audit_publication_digest.is_empty());
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d2 = u_eval(&c);
        assert_eq!(
            d1.durable_audit_publication_id,
            d2.durable_audit_publication_id
        );
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(
            d1.durable_audit_publication_digest,
            d2.durable_audit_publication_digest
        );
...
fn artifact_content_digest_is_stable() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
    let d = u_eval(&c);
    let art = d.durable_audit_publication_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.durable_audit_publication_digest);
...
fn wrong_audit_ledger_commitment_decision_id() {
    u_reject_inputs(
        |i| i.expected_audit_ledger_commitment_decision_id = "bad".to_string(),
        UO::AuditLedgerCommitmentDecisionIdMismatch,
    );
}

fn wrong_audit_ledger_commitment_request_id() {
    u_reject_inputs(
        |i| i.expected_audit_ledger_commitment_request_id = "bad".to_string(),
        UO::AuditLedgerCommitmentDecisionRequestIdMismatch,
    );
}

fn wrong_audit_ledger_commitment_intent_digest() {
    u_reject_inputs(
        |i| i.expected_audit_ledger_commitment_intent_digest = "bad".to_string(),
        UO::AuditLedgerCommitmentDecisionIntentDigestMismatch,
    );
}

fn wrong_audit_ledger_commitment_transcript_digest() {
    u_reject_inputs(
        |i| i.expected_audit_ledger_commitment_transcript_digest = "bad".to_string(),
        UO::AuditLedgerCommitmentDecisionTranscriptMismatch,
    );
}

fn wrong_audit_ledger_commitment_nonce() {
    u_reject_inputs(
        |i| i.expected_audit_ledger_commitment_nonce = 9999,
        UO::WrongAuditLedgerCommitmentNonce,
    );
}

fn tampered_audit_ledger_commitment_package_integrity_mismatch() {
    // Mutate the consumed package so its content digest no longer matches the
    // bound handoff decision digest.
    let mut dec = ct_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec
        .audit_ledger_commitment_artifact
        .as_ref()
        .unwrap()
        .epoch_transition_target;
    let inputs = u_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.audit_ledger_commitment_artifact
        .as_mut()
        .unwrap()
    ...
    assert_eq!(
        d.outcome,
        UO::AuditLedgerCommitmentDecisionIntegrityMismatch
    );
    assert!(d.durable_audit_publication_artifact.is_none());
}
...
fn wrong_guarded_mutation_decision_id() {
    u_reject_inputs(
        ...
        UO::GuardedMutationDecisionIdMismatch,
    );
}
...
fn wrong_guarded_mutation_request_id() {
    u_reject_inputs(
        ...
        UO::GuardedMutationDecisionRequestIdMismatch,
    );
}
...
fn wrong_guarded_mutation_intent_digest() {
    u_reject_inputs(
        ...
        UO::GuardedMutationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_guarded_mutation_transcript_digest() {
    u_reject_inputs(
        ...
        UO::GuardedMutationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_decision_id() {
    u_reject_inputs(
        ...
        UO::StagedApplicationDecisionIdMismatch,
    );
}
...
fn wrong_staged_application_request_id() {
    u_reject_inputs(
        ...
        UO::StagedApplicationDecisionRequestIdMismatch,
    );
}
...
fn wrong_staged_application_intent_digest() {
    u_reject_inputs(
        ...
        UO::StagedApplicationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_staged_application_transcript_digest() {
    u_reject_inputs(
        ...
        UO::StagedApplicationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_nonce() {
    u_reject_inputs(
        ...
        UO::WrongStagedApplicationNonce,
    );
}
...
fn wrong_authorization_decision_id() {
    u_reject_inputs(
        ...
        UO::AuthorizationDecisionIdMismatch,
    );
}
...
fn wrong_authorization_request_id() {
    u_reject_inputs(
        ...
        UO::AuthorizationDecisionRequestIdMismatch,
    );
}
...
fn wrong_authorization_intent_digest() {
    u_reject_inputs(
        ...
        UO::AuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_authorization_transcript_digest() {
    u_reject_inputs(
        ...
        UO::AuthorizationDecisionTranscriptMismatch,
    );
}
...
fn wrong_application_intent_digest() {
    u_reject_inputs(
        ...
        UO::WrongApplicationIntentDigest,
    );
}
...
fn wrong_application_transcript_digest() {
    u_reject_inputs(
        ...
        UO::WrongApplicationTranscriptDigest,
    );
}
...
fn wrong_governance_execution_intent_digest() {
    u_reject_inputs(
        ...
        UO::WrongGovernanceExecutionIntentDigest,
    );
}
...
fn wrong_rotation_transcript_digest() {
    u_reject_inputs(
        ...
        UO::WrongRotationTranscriptDigest,
    );
}
...
fn wrong_current_validator_set_digest() {
    u_reject_inputs(
        ...
        UO::WrongCurrentValidatorSetDigest,
    );
}
...
fn wrong_proposed_validator_set_digest() {
    u_reject_inputs(
        ...
        UO::WrongProposedValidatorSetDigest,
    );
}
...
fn wrong_validator_set_delta_digest() {
    u_reject_inputs(
        ...
        UO::WrongValidatorSetDeltaDigest,
    );
}
...
fn wrong_current_validator_set_epoch() {
    u_reject_inputs(
        ...
        UO::WrongCurrentValidatorSetEpoch,
    );
}
...
fn wrong_current_validator_set_version() {
    u_reject_inputs(
        ...
        UO::WrongCurrentValidatorSetVersion,
    );
}
...
fn wrong_proposed_validator_count() {
    u_reject_inputs(
        ...
        UO::WrongProposedValidatorCount,
    );
}

fn wrong_rotation_nonce() {
    u_reject_inputs(|i| i.expected_rotation_nonce = 999, UO::WrongRotationNonce);
}
...
fn wrong_epoch_transition_target_request() {
    ...
    let d = u_eval(&c);
    assert_eq!(d.outcome, UO::WrongEpochTransitionTarget);
    assert!(d.durable_audit_publication_artifact.is_none());
}
...
fn reject_missing_audit_ledger_commitment_decision() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::MissingAuditLedgerCommitmentDecision,
        UO::VerifiedAuditLedgerCommitmentDecisionRequired,
    );
}

fn reject_unverified_audit_ledger_commitment_decision() {
    let dec = ct_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::UnverifiedAuditLedgerCommitmentDecision {
            decision: dec,
        },
        UO::UnverifiedAuditLedgerCommitmentDecisionRejected,
    );
}
...
fn reject_accepted_audit_ledger_commitment_without_package_via_verified_source() {
    let dec = ct_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::VerifiedAuditLedgerCommitmentDecision {
            decision: dec,
        },
...
fn reject_accepted_audit_ledger_commitment_without_package_variant() {
    let dec = ct_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
...
fn reject_guarded_mutation_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::GuardedMutationDecisionWithoutAuditLedgerCommitment,
        UO::GuardedMutationDecisionAloneRejected,
    );
}

fn reject_staged_application_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::StagedApplicationDecisionWithoutAuditLedgerCommitment,
        UO::StagedApplicationDecisionAloneRejected,
    );
}

fn reject_live_application_authorization_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::LiveApplicationAuthorizationWithoutAuditLedgerCommitment,
        UO::LiveApplicationAuthorizationAloneRejected,
    );
}

fn reject_application_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::ApplicationDecisionWithoutAuditLedgerCommitment,
        UO::ApplicationDecisionAloneRejected,
    );
}

fn reject_rotation_plan_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::RotationPlanWithoutAuditLedgerCommitment,
        UO::RotationPlanAloneRejected,
    );
}

fn reject_governance_execution_intent_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::GovernanceExecutionIntentWithoutAuditLedgerCommitment,
        UO::GovernanceExecutionIntentAloneRejected,
    );
}

fn reject_governance_proof_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::GovernanceProofWithoutAuditLedgerCommitment,
        UO::GovernanceProofAloneRejected,
    );
}

fn reject_local_operator_assertion() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::LocalOperatorAssertion,
        UO::LocalOperatorProofRejected,
    );
}

fn reject_peer_majority_assertion() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::PeerMajorityAssertion,
        UO::PeerMajorityProofRejected,
    );
}

fn reject_custody_only_evidence() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::CustodyOnlyEvidence,
        UO::CustodyOnlyProofRejected,
    );
}

fn reject_remote_signer_only_evidence() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::RemoteSignerOnlyEvidence,
        UO::RemoteSignerOnlyProofRejected,
    );
}

fn reject_custody_attestation_only_evidence() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::CustodyAttestationOnlyEvidence,
        UO::CustodyAttestationOnlyProofRejected,
    );
}

fn reject_fixture_only_audit_ledger_commitment() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::FixtureOnlyAuditLedgerCommitment,
        UO::FixtureStagedApplicationRejectedAsProductionAuthority,
    );
}

fn reject_arbitrary_validator_set_bytes() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::ArbitraryValidatorSetBytes,
        UO::ArbitraryValidatorSetBytesRejected,
    );
}
...
fn mainnet_domain_refused_under_source_test_policy() {
    ...
    let d = u_eval(&c);
    assert_eq!(d.outcome, UO::MainNetRefused);
    assert!(d.durable_audit_publication_artifact.is_none());
}

fn mainnet_policy_unavailable() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = u_exec_with_policy(
    ...
    assert_eq!(
        d.outcome,
        UO::MainNetProductionLiveEpochTransitionDurableAuditPublicationUnavailable
    );
    assert!(d.durable_audit_publication_artifact.is_none());
}
...
fn production_policy_unavailable() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = u_exec_with_policy(
        ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::RequireProductionLiveEpochTransitionDurableAuditPublication,
    );
    let d = exec.evaluate_live_epoch_transition_durable_audit_publication(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        UO::ProductionLiveEpochTransitionDurableAuditPublicationUnavailable
    );
    assert!(d.durable_audit_publication_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    ...
    assert_eq!(d.outcome, UO::Disabled);
    assert!(d.durable_audit_publication_artifact.is_none());
    assert!(!d.is_accept());
}
...
fn reserved_production_kind_fails_closed() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionDurableAuditPublicationExecutor::new(
        ProductionLiveEpochTransitionDurableAuditPublicationConfig::new(
    ...
    assert_eq!(
        d.outcome,
        UO::LiveEpochTransitionDurableAuditPublicationBoundaryUnavailable
    );
}
...
fn replay_rejected_when_id_present() {
    ...
    let id = d
        ...
        .clone();
    let replay = vec![id];
    let d2 = u_eval_replay(&c, &replay);
    match d2.outcome {
        UO::StagedApplicationReplayRejected { .. } => {}
        other => panic!("expected replay rejection, got {other:?}"),
    }
    assert!(d2.durable_audit_publication_artifact.is_none());
}

fn no_replay_when_id_absent() {
    let c = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let replay: Vec<String> = vec!["some-other-id".to_string()];
    let d = u_eval_replay(&c, &replay);
    assert!(d.is_accept());
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let d = u_eval(&c);
    let art = d.durable_audit_publication_artifact.as_ref().unwrap();
    let mut state = LiveEpochTransitionDurableAuditPublicationFixtureState::new(
        CUR_EPOCH,
        CUR_VERSION,
        "start",
    );
    assert!(state.apply_prepared_execution(art, &d.durable_audit_publication_id));
    ...
    assert!(!state.apply_prepared_execution(art, &d.durable_audit_publication_id));
    assert!(state.has_applied(&d.durable_audit_publication_id));
}
...
fn every_outcome_is_non_mutating() {
    ...
    let d = u_eval(&c);
    assert!(d.outcome.is_non_mutating());
    let bad = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let dbad = u_eval_replay(&bad, &[u_eval(&bad).durable_audit_publication_id]);
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionDurableAuditPublication.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionDurableAuditPublication.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionDurableAuditPublicationRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}

fn kind_predicates() {
    use ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionDurableAuditPublication.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionDurableAuditPublication.is_source_test());
    assert_eq!(K::default(), K::Disabled);
}

fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        let hk = expected_crc3_kind(sc);
        let pk = UK::from_staged_application_kind(hk);
        assert_eq!(pk, expected_crc4_kind(sc));
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = UK::from_staged_application_kind(
        LiveEpochTransitionAuditLedgerCommitmentKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = UO::AuditLedgerCommitmentDecisionIdMismatch;
    let b = UO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), UO::AuditLedgerCommitmentDecisionIdMismatch.tag());
    assert!(!UO::MainNetRefused.tag().is_empty());
}
...
fn config_and_inputs_well_formed() {
    ...
    assert!(
        ProductionLiveEpochTransitionDurableAuditPublicationConfig::source_test().is_well_formed()
    );
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionDurableAuditPublicationConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.durable_audit_publication_artifact
                    .as_ref()
                    .unwrap()
                    .environment,
                TrustBundleEnvironment::Testnet
...
macro_rules! per_scenario_audit_ledger_commitment_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            let mut c = u_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_audit_ledger_commitment_decision_id = "bad".to_string();
            let d = u_eval(&c);
            assert_eq!(d.outcome, UO::AuditLedgerCommitmentDecisionIdMismatch);
            assert!(d.durable_audit_publication_artifact.is_none());
        }
    };
}

per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_audit_ledger_commitment_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
macro_rules! per_scenario_guarded_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d = u_eval(&c);
            assert_eq!(d.outcome, UO::GuardedMutationDecisionIdMismatch);
            assert!(d.durable_audit_publication_artifact.is_none());
        }
    };
}
...
//    (additive layer introduced by Run 329 on top of the rotated Run 327
//    coverage above). All non-mutating, fail-closed.
// ===========================================================================

fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    let parent = ct_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let ppkg = parent
        .audit_ledger_commitment_artifact
        .as_ref()
        .unwrap()
...
fn reject_runtime_handoff_decision_id_mismatch() {
    u_reject_inputs(
        ...
        UO::RuntimeHandoffDecisionIdMismatch,
    );
}
...
fn reject_runtime_handoff_request_id_mismatch() {
    u_reject_inputs(
        ...
        UO::RuntimeHandoffDecisionRequestIdMismatch,
    );
}
...
fn reject_runtime_handoff_intent_digest_mismatch() {
    u_reject_inputs(
        ...
        UO::RuntimeHandoffDecisionIntentDigestMismatch,
    );
}
...
fn reject_runtime_handoff_transcript_mismatch() {
    u_reject_inputs(
        ...
        UO::RuntimeHandoffDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_runtime_handoff_nonce() {
    u_reject_inputs(
        ...
        UO::WrongRuntimeHandoffNonce,
    );
}

fn reject_runtime_handoff_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::RuntimeHandoffDecisionWithoutAuditLedgerCommitment,
        UO::RuntimeHandoffDecisionAloneRejected,
    );
}

fn runtime_handoff_alone_tag_is_stable() {
    assert_eq!(
        UO::RuntimeHandoffDecisionAloneRejected.tag(),
        "runtime-handoff-decision-alone-rejected"
    );
    assert_eq!(
        UO::RuntimeHandoffDecisionIdMismatch.tag(),
        "runtime-handoff-decision-id-mismatch"
    );
    assert_eq!(
        UO::WrongRuntimeHandoffNonce.tag(),
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = u_eval(&c);
    let d2 = u_eval(&c);
    let a1 = d1.durable_audit_publication_artifact.as_ref().unwrap();
    let a2 = d2.durable_audit_publication_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 331 — re-exposed Run 327/328 execution-preparation ancestor binding
// (carried through the consumed Run 329/330 audit-ledger-commitment artifact) and
...
fn reject_mutation_execution_decision_id_mismatch() {
    u_reject_inputs(
        ...
        UO::MutationExecutionDecisionIdMismatch,
    );
}
...
fn reject_mutation_execution_request_id_mismatch() {
    u_reject_inputs(
        ...
        UO::MutationExecutionDecisionRequestIdMismatch,
    );
}
...
fn reject_mutation_execution_intent_digest_mismatch() {
    u_reject_inputs(
        ...
        UO::MutationExecutionDecisionIntentDigestMismatch,
    );
}
...
fn reject_mutation_execution_transcript_mismatch() {
    u_reject_inputs(
        ...
        UO::MutationExecutionDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_mutation_execution_nonce() {
    u_reject_inputs(
        ...
        UO::WrongMutationExecutionNonce,
    );
}

fn reject_mutation_execution_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::MutationExecutionDecisionWithoutAuditLedgerCommitment,
        UO::MutationExecutionDecisionAloneRejected,
    );
}
...
fn reject_commit_authorization_decision_id_mismatch() {
    u_reject_inputs(
        ...
        UO::CommitAuthorizationDecisionIdMismatch,
    );
}
...
fn reject_commit_authorization_request_id_mismatch() {
    u_reject_inputs(
        ...
        UO::CommitAuthorizationDecisionRequestIdMismatch,
    );
}
...
fn reject_commit_authorization_intent_digest_mismatch() {
    u_reject_inputs(
        ...
        UO::CommitAuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn reject_commit_authorization_transcript_mismatch() {
    u_reject_inputs(
        ...
        UO::CommitAuthorizationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_commit_authorization_nonce() {
    u_reject_inputs(
        ...
        UO::WrongCommitAuthorizationNonce,
    );
}

fn reject_commit_authorization_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::CommitAuthorizationDecisionWithoutAuditLedgerCommitment,
        UO::CommitAuthorizationDecisionAloneRejected,
    );
}
...
fn mutation_execution_alone_tag_is_stable() {
    assert_eq!(
        UO::MutationExecutionDecisionAloneRejected.tag(),
        "mutation-execution-decision-alone-rejected"
    );
    assert_eq!(
        UO::MutationExecutionDecisionIdMismatch.tag(),
        "mutation-execution-decision-id-mismatch"
    );
    assert_eq!(
        UO::WrongMutationExecutionNonce.tag(),
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    // Parent Run 329/330 audit-ledger-commitment nonce re-exposed intact.
    assert_eq!(art.audit_ledger_commitment_nonce, CRC3_NONCE);
    // Self Run 331 durable-audit-publication proposed nonce.
    assert_eq!(art.durable_audit_publication_nonce, CRC4_NONCE);
}
...
fn reject_execution_preparation_decision_id_mismatch() {
    u_reject_inputs(
        ...
        UO::ExecutionPreparationDecisionIdMismatch,
    );
}
...
fn reject_execution_preparation_request_id_mismatch() {
    u_reject_inputs(
        ...
        UO::ExecutionPreparationDecisionRequestIdMismatch,
    );
}
...
fn reject_execution_preparation_intent_digest_mismatch() {
    u_reject_inputs(
        ...
        UO::ExecutionPreparationDecisionIntentDigestMismatch,
    );
}
...
fn reject_execution_preparation_transcript_mismatch() {
    u_reject_inputs(
        ...
        UO::ExecutionPreparationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_execution_preparation_nonce() {
    u_reject_inputs(
        ...
        UO::WrongExecutionPreparationNonce,
    );
}

fn reject_execution_preparation_decision_alone() {
    u_reject_source(
        LiveEpochTransitionDurableAuditPublicationAuthoritySource::ExecutionPreparationDecisionWithoutAuditLedgerCommitment,
        UO::ExecutionPreparationDecisionAloneRejected,
    );
}

fn execution_preparation_alone_tag_is_stable() {
    assert_eq!(
        UO::ExecutionPreparationDecisionAloneRejected.tag(),
        "execution-preparation-decision-alone-rejected"
    );
    assert_eq!(
        UO::ExecutionPreparationDecisionIdMismatch.tag(),
        "execution-preparation-decision-id-mismatch"
    );
    assert_eq!(
        UO::WrongExecutionPreparationNonce.tag(),
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_332_production_live_epoch_transition_durable_audit_publication_release_binary/helper_evidence/run_332",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_audit_ledger_commitment_transcript",
            accept_artifact_reexposes_consumed_audit_ledger_commitment_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_audit_ledger_commitment_decision_id",
            wrong_audit_ledger_commitment_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_audit_ledger_commitment_request_id",
            wrong_audit_ledger_commitment_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_audit_ledger_commitment_intent_digest",
            wrong_audit_ledger_commitment_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_audit_ledger_commitment_transcript_digest",
            wrong_audit_ledger_commitment_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_audit_ledger_commitment_nonce",
            wrong_audit_ledger_commitment_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_audit_ledger_commitment_package_integrity_mismatch",
            tampered_audit_ledger_commitment_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_audit_ledger_commitment_decision",
            reject_missing_audit_ledger_commitment_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_audit_ledger_commitment_decision",
            reject_unverified_audit_ledger_commitment_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_audit_ledger_commitment_without_package_via_verified_source",
            reject_accepted_audit_ledger_commitment_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_audit_ledger_commitment_without_package_variant",
            reject_accepted_audit_ledger_commitment_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_audit_ledger_commitment",
            reject_fixture_only_audit_ledger_commitment as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 331 domain-separated digest free functions.
    let case = u_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = u_eval(&case);
    ...
    // 327/328 durable-audit-finalization decision's durable-audit-finalization (content)
    // digest.
    let cs_dec = ct_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let audit_ledger_commitment_intent_digest = cs_dec.audit_ledger_commitment_digest.clone();
    let content_digest =
        production_live_epoch_transition_durable_audit_publication_content_digest(artifact);
    let durable_audit_publication_id =
        production_live_epoch_transition_durable_audit_publication_id(
            ...
            &case.inputs.durable_audit_publication_policy_id,
            case.request.proposed_epoch_transition_target,
            case.request.durable_audit_publication_nonce,
        );
    ...
    digest_body.push_str(&format!(
        "audit_ledger_commitment_intent_digest={audit_ledger_commitment_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = u_case(TrustBundleEnvironment::Devnet, sc);
        let decision = u_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "durable_audit_publication_id={}\n",
            decision.durable_audit_publication_id
        ));
    ...
    digest_body.push_str(&format!(
        "named_durable_audit_publication_id={durable_audit_publication_id}\n"
    ));
    ...
    fs::write(
        outdir.join("fixtures/run_332_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 331 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionDurableAuditPublicationArtifact` and its
    // `ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionDurableAuditPublicationExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionDurableAuditPublicationArtifact = artifact;
    let clean: ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_durable_audit_publication_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_durable_audit_publication_window(
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_332_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("run_332_production_live_epoch_transition_durable_audit_publication_release_binary_helper\n");
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_durable_audit_publication.rs :: ProductionLiveEpochTransitionDurableAuditPublicationExecutor (Run 331 source/test boundary)\n");
