//! Run 340 — release-binary helper for the Run 339 **live epoch-transition
//! settlement / finalization execution boundary**.
//!
//! Release-binary evidence for the Run 339 live epoch-transition settlement /
...
//! release-built production library symbols. It exercises the **real** Run 339
//! `ProductionLiveEpochTransitionSettlementExecutionExecutor` and proves, per
...
//! 333 -> 335 -> 337 chain to produce a verified, accepted Run 337 live
//! epoch-transition settlement / finalization execution-preparation decision
//! (`is_accept()` with `Some(settlement_execution_preparation_artifact)`), then
//! feeds that decision into the Run 339 live epoch-transition settlement /
//! finalization execution executor, which emits only a typed, deterministic,
//! non-mutating settlement / finalization execution artifact.
//!
...
//! enablement, settlement-execution default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live settlement /
//! finalization execution artifacts; it never applies a live validator-set
...
//! audit seal, audit-finalization record, audit-ledger record, audit-ledger
//! commitment record, durable-audit-publication record, durable replay record,
//! settlement record, settlement-finalization record, settlement-execution
//! record, publication record, external-publication record, or final-execution
...
//! `LiveEpochTransitionSettlementExecutionFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_340.md`.
#![allow(dead_code)]
...
mod run_339_settlement_execution {
    ...
    pub fn helper_main() {
        let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
            PathBuf::from(
                "docs/devnet/run_340_production_live_epoch_transition_settlement_execution_release_binary/helper_evidence/run_340",
            )
        });
        ...
        ("accepted_compatible", "accept_artifact_reexposes_consumed_settlement_execution_preparation_transcript", accept_artifact_reexposes_consumed_settlement_execution_preparation_transcript as fn()),
        ...
        ("rejection_fail_closed", "wrong_settlement_execution_preparation_decision_id", wrong_settlement_execution_preparation_decision_id as fn()),
        ("rejection_fail_closed", "wrong_settlement_execution_preparation_request_id", wrong_settlement_execution_preparation_request_id as fn()),
        ("rejection_fail_closed", "wrong_settlement_execution_preparation_intent_digest", wrong_settlement_execution_preparation_intent_digest as fn()),
        ("rejection_fail_closed", "wrong_settlement_execution_preparation_transcript_digest", wrong_settlement_execution_preparation_transcript_digest as fn()),
        ("rejection_fail_closed", "wrong_settlement_execution_preparation_nonce", wrong_settlement_execution_preparation_nonce as fn()),
        ("rejection_fail_closed", "tampered_settlement_execution_preparation_package_integrity_mismatch", tampered_settlement_execution_preparation_package_integrity_mismatch as fn()),
        ...
        ("rejection_fail_closed", "reject_missing_settlement_execution_preparation_decision", reject_missing_settlement_execution_preparation_decision as fn()),
        ("rejection_fail_closed", "reject_unverified_settlement_execution_preparation_decision", reject_unverified_settlement_execution_preparation_decision as fn()),
        ...
        ("rejection_fail_closed", "reject_accepted_settlement_execution_preparation_without_package_via_verified_source", reject_accepted_settlement_execution_preparation_without_package_via_verified_source as fn()),
        ("rejection_fail_closed", "reject_accepted_settlement_execution_preparation_without_package_variant", reject_accepted_settlement_execution_preparation_without_package_variant as fn()),
        ...
        ("rejection_fail_closed", "reject_fixture_only_settlement_execution_preparation", reject_fixture_only_settlement_execution_preparation as fn()),
        ...
        let accepted_case = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let accepted_decision = z_eval(&accepted_case);
        ...
        let accepted_artifact = accepted_decision
            ...
            .as_ref()
            .expect("accepted Devnet Add case must carry a settlement-execution artifact");
        let mut accepted_body = String::new();
        accepted_body.push_str("scenario=Devnet::Add\n");
        accepted_body.push_str(&format!("settlement_execution_id={}\n", accepted_decision.settlement_execution_id));
        ...
        accepted_body.push_str(&format!("settlement_execution_digest={}\n", accepted_decision.settlement_execution_digest));
        ...
        fs::write(
            outdir.join("fixtures/run_340_accepted_artifact.txt"),
            &accepted_body,
        )
        .expect("write accepted-artifact fixture");

        // Deterministic-digest fixture across two independent evaluations.
        let digest_case_1 = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let digest_case_2 = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let digest_decision_1 = z_eval(&digest_case_1);
        let digest_decision_2 = z_eval(&digest_case_2);
        ...
        assert_eq!(digest_decision_1.settlement_execution_id, digest_decision_2.settlement_execution_id);
        ...
        assert_eq!(digest_decision_1.settlement_execution_digest, digest_decision_2.settlement_execution_digest);
        ...
        digest_body.push_str(&format!("settlement_execution_id={}\n", digest_decision_1.settlement_execution_id));
        ...
        digest_body.push_str(&format!("settlement_execution_digest={}\n", digest_decision_1.settlement_execution_digest));
        digest_body.push_str(&format!("transcript_digest={}\n", digest_decision_1.transcript_digest));
        digest_body.push_str(&format!("second_settlement_execution_id={}\n", digest_decision_2.settlement_execution_id));
        ...
        digest_body.push_str(&format!("second_settlement_execution_digest={}\n", digest_decision_2.settlement_execution_digest));
        ...
        fs::write(
            outdir.join("fixtures/run_340_deterministic_digests.txt"),
            &digest_body,
        )
        .expect("write deterministic-digest fixture");

        // Replay-recovery window fixture (non-mutating, idempotent).
        let recover_exec = ProductionLiveEpochTransitionSettlementExecutionExecutor::source_test();
        let current_artifact: &ProductionLiveEpochTransitionSettlementExecutionArtifact = accepted_artifact;
        let clean: ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome =
            recover_exec.recover_live_epoch_transition_settlement_execution_window(None, current_artifact);
        let idempotent: ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome =
            recover_exec.recover_live_epoch_transition_settlement_execution_window(
        ...
        let staged_application_id = match &idempotent {
            ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome::IdempotentReplayObserved { staged_application_id } => staged_application_id.clone(),
        ...
        fs::write(
            outdir.join("fixtures/run_340_recovery_window.txt"),
            &recovery_body,
        )
        ...
        summary.push_str("run_340_production_live_epoch_transition_settlement_execution_release_binary_helper\n");
        summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_settlement_execution.rs :: ProductionLiveEpochTransitionSettlementExecutionExecutor (Run 339 source/test boundary)\n");
...
fn main() {
    run_339_settlement_execution::helper_main();
}
