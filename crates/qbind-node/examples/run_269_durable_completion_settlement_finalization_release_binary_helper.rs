//! Run 269 — release-built helper for the Run 268 durable-completion settlement-finalization sink boundary.
//!
...
//! settlement-projection -> Run 266 settlement-commitment chain and then evaluates
//! the Run 268 settlement finalization. Projection is intentionally from
//! `input.settlement_commitment_binding` through
...
use qbind_node::pqc_governance_durable_completion_settlement_finalization::{
    settlement_finalization_identity_digest, settlement_finalization_request_digest,
    settlement_finalization_response_digest, settlement_finalization_record_digest,
    settlement_finalization_transcript_digest,
    durable_completion_settlement_finalization_ambiguous_window_fails_closed,
    durable_completion_settlement_finalization_attestation_required,
    durable_completion_settlement_finalization_backend_submission_required,
    durable_completion_settlement_finalization_commitment_required,
    durable_completion_settlement_finalization_completion_report_required,
    durable_completion_settlement_finalization_consumer_required,
    durable_completion_settlement_finalization_external_unavailable,
    durable_completion_settlement_finalization_failed_record_never_records,
    durable_completion_settlement_finalization_finalization_required,
    durable_completion_settlement_finalization_local_operator_cannot_satisfy_mainnet_authority,
    durable_completion_settlement_finalization_mainnet_peer_driven_apply_refused_first,
    ...
    durable_completion_settlement_finalization_never_writes_sequence_or_marker,
    durable_completion_settlement_finalization_no_external_publication,
    durable_completion_settlement_finalization_no_real_audit_ledger,
    ...
    durable_completion_settlement_finalization_no_rocksdb_file_schema_migration_change,
    durable_completion_settlement_finalization_peer_majority_cannot_satisfy_mainnet_authority,
    durable_completion_settlement_finalization_pipeline_success_required,
    durable_completion_settlement_finalization_policy_change_unsupported,
    durable_completion_settlement_finalization_production_mainnet_unavailable,
    durable_completion_settlement_finalization_receipt_required,
    durable_completion_settlement_finalization_record_required_before_finalized,
    durable_completion_settlement_finalization_rejection_is_non_mutating,
    durable_completion_settlement_finalization_rollback_never_records,
    durable_completion_settlement_finalization_sink_receipt_required,
    durable_completion_settlement_finalization_validator_set_rotation_unsupported,
    ...
    DurableCompletionSettlementFinalizationBinding, DurableCompletionSettlementFinalizationDigest,
    DurableCompletionSettlementFinalizationEnvironment,
    DurableCompletionSettlementFinalizationExpectations,
    DurableCompletionSettlementFinalizationFault,
    ...
    DurableCompletionSettlementFinalizationSurface,
    DurableCompletionSettlementFinalizationTranscriptDigest,
    DurableCompletionSettlementFinalizationWindow, ExternalSettlementFinalizationSink,
    FixtureDurableCompletionSettlementFinalizationSink,
    GovernanceDurableCompletionSettlementFinalizationSink, MainNetSettlementFinalizationSink,
    ProductionSettlementFinalizationSink,
};
...
fn ack_identity(
    policy: DurableCompletionSettlementFinalizationPolicy,
...
fn ctx_action(
    ...
    let settlement_commitment =
        attach_run266_settlement_commitment(environment, action, consumer_duplicate);
    ...
    let expectations = DurableCompletionSettlementFinalizationExpectations {
        ...
        expected_consumer_transcript_digest: consumer.transcript_digest.clone(),
        expected_consumer_record_id: consumer.consumer_record_id.clone(),
        expected_settlement_commitment_identity_digest: settlement_commitment.identity_digest.clone(),
        ...
        expected_settlement_commitment_transcript_digest: settlement_commitment
            .transcript_digest
            .clone(),
        expected_settlement_commitment_record_id: settlement_commitment.commitment_record_id.clone(),
        expected_identity: id,
        expected_finalization_kind: kind,
        expected_finalization_policy: policy,
...
impl Ctx {
    ...
    fn input_with_commitment(
        ...
        settlement_commitment: DurableCompletionSettlementCommitmentOutcome,
    ) -> DurableCompletionSettlementFinalizationInput {
        DurableCompletionSettlementFinalizationInput {
            policy,
            ...
            settlement_commitment_binding: settlement_commitment,
    ...
    fn input(
        ...
        self.input_with_commitment(
            ...
            self.settlement_commitment.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionSettlementFinalizationInput {
        self.input_with_commitment(
...
fn fixture_sink() -> FixtureDurableCompletionSettlementFinalizationSink {
    FixtureDurableCompletionSettlementFinalizationSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionSettlementFinalizationPolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_settlement_finalization(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn disabled_backend_policy_never_invokes_receipt_sink() {
    let c = devnet_ctx();
    let input = c.input_with_commitment(
        DurableCompletionSettlementFinalizationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(settlement_finalization_outcome_authorizes_record(&outcome));
    assert!(settlement_finalization_outcome_projects_to_recorded(&outcome));
    assert_eq!(sink.invocations(), 1);
    assert_eq!(ledger.len(), 1);
    assert!(ledger.contains(FINALIZATION_RECORD_ID));
    assert_eq!(
        settlement_finalization_identity_digest(&c.request.identity),
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.finalization_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!settlement_finalization_outcome_authorizes_record(&second));
    assert!(settlement_finalization_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run266_duplicate_idempotent_commitment_only_matches_existing_never_creates() {
    // A real Run 266 SettlementCommitmentDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.settlement_commitment.outcome,
        DurableCompletionSettlementCommitmentOutcome::SettlementCommitmentDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_settlement_finalization(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_settlement_finalization(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementFinalizationPolicy::ProductionSettlementFinalizationRequired,
        DurableCompletionSettlementFinalizationKind::ProductionSettlementFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementFinalizationLedger::new();
    let mut sink = ProductionSettlementFinalizationSink::default();
    let outcome = evaluate_durable_completion_settlement_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementFinalizationPolicy::MainNetSettlementFinalizationRequired,
        DurableCompletionSettlementFinalizationKind::MainNetSettlementFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementFinalizationLedger::new();
    let mut sink = MainNetSettlementFinalizationSink::default();
    let outcome = evaluate_durable_completion_settlement_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementFinalizationPolicy::ExternalSettlementFinalizationRequired,
        DurableCompletionSettlementFinalizationKind::ExternalSettlementFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementFinalizationLedger::new();
    let mut sink = ExternalSettlementFinalizationSink::default();
    let outcome = evaluate_durable_completion_settlement_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn validator_set_rotation_and_policy_change_unsupported() {
    let c = devnet_ctx();
    let rotation = c.input_with_commitment(
        DurableCompletionSettlementFinalizationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
    ...
    let policy_change = c.input_with_commitment(
        DurableCompletionSettlementFinalizationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
...
fn assert_non_recording_commitment(
    settlement_commitment: DurableCompletionSettlementCommitmentOutcome,
    expected: DurableCompletionSettlementFinalizationOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_commitment(
        DurableCompletionSettlementFinalizationPolicy::FixtureAllowed,
...
fn non_recording_commitment_outcomes_never_record_finalization() {
    ...
    assert_non_recording_commitment(
    ...
    assert_non_recording_commitment(
        Commitment::SettlementCommitmentRejectedBeforeRecord,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::SettlementCommitmentRecordFailedNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::SettlementCommitmentRolledBackNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::SettlementCommitmentRollbackFailedFatalNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::SettlementCommitmentAmbiguousFailClosedNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::ProductionSettlementCommitmentUnavailableNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::MainNetSettlementCommitmentUnavailableNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
        Commitment::ExternalSettlementCommitmentUnavailableNoCommitment,
        Finalization::SettlementCommitmentDidNotRecordNoFinalization,
    );
    assert_non_recording_commitment(
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementFinalizationOutcome::RejectedBeforeSettlementCommitmentNoFinalization
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionSettlementFinalizationRequest),
) {
    let c = devnet_ctx();
...
fn wrong_finalization_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.finalization_record_id = "other-commitment".to_string();
    });
}
...
fn wrong_acknowledgement_identity_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.identity.commitment_id = "other-receipt-id".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_settlement_finalization(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_receipt(
    fault: DurableCompletionSettlementFinalizationFault,
    expected: DurableCompletionSettlementFinalizationOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionSettlementFinalizationLedger::new();
    let mut sink = FixtureDurableCompletionSettlementFinalizationSink::with_fault(fault);
    let outcome = evaluate_durable_completion_settlement_finalization(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRecordFailedNoFinalization,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRolledBackNoFinalization,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRollbackFailedFatalNoFinalization,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationAmbiguousFailClosedNoFinalization,
    );
}
...
fn only_recorded_commitment_outcome_creates_settlement_finalization_request_intent() {
    use DurableCompletionSettlementCommitmentOutcome as Commitment;
    use DurableCompletionSettlementFinalizationRequestIntent as Intent;
    assert_eq!(
        project_settlement_commitment_outcome_to_finalization_request(
            &Commitment::SettlementCommitmentRecorded
        ),
        Intent::CreateRequest
    );
    assert!(project_settlement_commitment_outcome_to_finalization_request(
        &Commitment::SettlementCommitmentRecorded
    )
    .creates_request());
    assert_eq!(
        project_settlement_commitment_outcome_to_finalization_request(
            &Commitment::SettlementCommitmentDuplicateIdempotent
        ),
...
fn non_recording_commitment_outcomes_create_no_settlement_finalization_request() {
    use DurableCompletionSettlementCommitmentOutcome as Commitment;
    use DurableCompletionSettlementFinalizationRequestIntent as Intent;
    for commitment in [
        Commitment::LegacyBypassNoSettlementCommitment,
        ...
        assert!(matches!(
            project_settlement_commitment_outcome_to_finalization_request(&commitment),
            Intent::NoCommitment(_)
        ));
    }
}
...
fn recovered_ledger() -> DurableCompletionSettlementFinalizationLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionSettlementFinalizationLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_settlement_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let record = if with_record {
        ledger.find(FINALIZATION_RECORD_ID)
    } else {
        None
    };
    let outcome = recover_durable_completion_settlement_finalization_window(
        &input,
        window,
        DurableCompletionSettlementFinalizationKind::FixtureInMemory,
        record,
...
fn pre_settlement_commitment_windows_fail_closed_no_commitment() {
    ...
    for w in [
        ...
        assert_window(
            w,
            false,
            Commitment::SettlementCommitmentDidNotRecordNoFinalization,
        );
    }
}

fn after_settlement_commitment_request_before_record_rejects_before_record() {
    use DurableCompletionSettlementFinalizationOutcome as Commitment;
    use DurableCompletionSettlementFinalizationWindow as Window;
    assert_window(
        Window::AfterSettlementFinalizationRequestBeforeSettlementFinalizationRecord,
        false,
        Commitment::SettlementFinalizationRejectedBeforeRecord,
    );
}

fn after_settlement_commitment_record_before_success_requires_explicit_matching_record() {
    use DurableCompletionSettlementFinalizationOutcome as Commitment;
    use DurableCompletionSettlementFinalizationWindow as Window;
    assert_window(
    ...
    assert_window(
        Window::AfterSettlementFinalizationRecordBeforeSettlementFinalizationSuccess,
        true,
        Commitment::SettlementFinalizationRecorded,
    );
}

fn after_settlement_commitment_success_recovers_as_recorded() {
    use DurableCompletionSettlementFinalizationOutcome as Commitment;
    use DurableCompletionSettlementFinalizationWindow as Window;
    assert_window(
        Window::AfterSettlementFinalizationSuccess,
        true,
        Commitment::SettlementFinalizationRecorded,
    );
...
fn ambiguous_record_failed_rollback_and_unknown_windows_fail_closed() {
    ...
    assert_window(
        ...
        false,
        Commitment::SettlementFinalizationAmbiguousFailClosedNoFinalization,
    );
    assert_window(
        Window::Unknown,
        false,
    ...
    assert_window(
        ...
        false,
        Commitment::SettlementFinalizationRecordFailedNoFinalization,
    );
    assert_window(
        Window::SettlementFinalizationRollbackCompleted,
        false,
        Commitment::SettlementFinalizationRolledBackNoFinalization,
    );
    assert_window(
        Window::SettlementFinalizationRollbackFailed,
        false,
        Commitment::SettlementFinalizationRollbackFailedFatalNoFinalization,
    );
}
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_settlement_finalization_window(
        &input,
        DurableCompletionSettlementFinalizationWindow::AfterSettlementFinalizationSuccess,
        DurableCompletionSettlementFinalizationKind::ProductionSettlementFinalizationUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionSettlementFinalizationSink::with_fault(
        DurableCompletionSettlementFinalizationFault::RolledBackNoCommitment,
    );
    let outcome = evaluate_durable_completion_settlement_finalization(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_settlement_finalization_rejection_is_non_mutating());
    assert!(durable_completion_settlement_finalization_never_calls_run_070());
    assert!(durable_completion_settlement_finalization_never_mutates_live_pqc_trust_state());
    assert!(durable_completion_settlement_finalization_never_writes_sequence_or_marker());
    assert!(durable_completion_settlement_finalization_no_rocksdb_file_schema_migration_change());
    assert!(durable_completion_settlement_finalization_no_external_publication());
    assert!(durable_completion_settlement_finalization_no_real_audit_ledger());
    assert!(durable_completion_settlement_finalization_pipeline_success_required());
    assert!(durable_completion_settlement_finalization_sink_receipt_required());
    assert!(durable_completion_settlement_finalization_completion_report_required());
    assert!(durable_completion_settlement_finalization_finalization_required());
    assert!(durable_completion_settlement_finalization_attestation_required());
    assert!(durable_completion_settlement_finalization_backend_submission_required());
    assert!(durable_completion_settlement_finalization_receipt_required());
    assert!(durable_completion_settlement_finalization_record_required_before_finalized());
    assert!(durable_completion_settlement_finalization_failed_record_never_records());
    assert!(durable_completion_settlement_finalization_rollback_never_records());
    assert!(durable_completion_settlement_finalization_ambiguous_window_fails_closed());
    assert!(
        durable_completion_settlement_finalization_mainnet_peer_driven_apply_refused_first(
            TrustBundleEnvironment::Mainnet
        )
    ...
    assert!(durable_completion_settlement_finalization_production_mainnet_unavailable());
    assert!(durable_completion_settlement_finalization_external_unavailable());
    assert!(durable_completion_settlement_finalization_validator_set_rotation_unsupported());
    assert!(durable_completion_settlement_finalization_policy_change_unsupported());
    assert!(
        durable_completion_settlement_finalization_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
// ===========================================================================
// Release symbol reachability probe — exercises the Run 268 production library
// symbols in release mode, driving projection from the Run 266 settlement-
// commitment outcome via input.settlement_commitment_binding.
// ===========================================================================

fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionSettlementFinalizationInput = c.recorded();

    let intent = project_settlement_commitment_outcome_to_finalization_request(
        &input.settlement_commitment_binding,
    );
    assert!(intent.creates_request());
    assert!(matches!(
        intent,
        DurableCompletionSettlementFinalizationRequestIntent::CreateRequest
    ));
    ...
    assert!(!outcome.no_commitment());
    assert_eq!(outcome.tag(), "settlement-finalization-recorded");
    assert_eq!(sink.kind(), DurableCompletionSettlementFinalizationKind::FixtureInMemory);
    assert_eq!(sink.invocations(), 1);

    let identity_digest: DurableCompletionSettlementFinalizationDigest =
        settlement_finalization_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionSettlementFinalizationDigest =
        settlement_finalization_request_digest(&c.request);
    let response = DurableCompletionSettlementFinalizationResponse {
        finalization_record_id: c.request.finalization_record_id.clone(),
        request_digest: request_digest.clone(),
        accepted: true,
        finalization_kind: DurableCompletionSettlementFinalizationKind::FixtureInMemory,
    };
    assert!(response.is_well_formed());
    let response_digest: DurableCompletionSettlementFinalizationDigest =
        settlement_finalization_response_digest(&response);
    let record: DurableCompletionSettlementFinalizationRecord = c.request.to_record();
    let record_digest: DurableCompletionSettlementFinalizationDigest =
        settlement_finalization_record_digest(&record);
    let transcript_digest: DurableCompletionSettlementFinalizationTranscriptDigest =
        settlement_finalization_transcript_digest(&request_digest, &response_digest, &record_digest);
    assert!(!identity_digest.as_hex().is_empty());
    assert!(!transcript_digest.as_hex().is_empty());

    let ledger_record: &DurableCompletionSettlementFinalizationLedgerRecord = ledger
        .find(FINALIZATION_RECORD_ID)
        .expect("settlement-finalization ledger record");
    assert_eq!(ledger_record.status, DurableCompletionSettlementFinalizationLedgerStatus::Recorded);
    let snapshot: DurableCompletionSettlementFinalizationLedgerSnapshot = ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionSettlementFinalizationSurface = input.surface();
    let _environment: DurableCompletionSettlementFinalizationEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionSettlementFinalizationBinding = input.runtime_binding.clone();

    assert!(DurableCompletionSettlementFinalizationPolicy::Disabled.is_disabled());
    assert!(DurableCompletionSettlementFinalizationPolicy::FixtureAllowed.allows_fixture());
    assert_eq!(
        DurableCompletionSettlementFinalizationPolicy::ProductionSettlementFinalizationRequired.tag(),
        "production-settlement-finalization-required"
    );
    assert!(DurableCompletionSettlementFinalizationKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionSettlementFinalizationKind::ProductionSettlementFinalizationUnavailable.is_unavailable());
    assert_eq!(DurableCompletionSettlementFinalizationKind::Disabled.tag(), "disabled");
    let no_finalization = DurableCompletionSettlementFinalizationOutcome::LegacyBypassNoSettlementFinalization;
    assert!(no_finalization.is_legacy_bypass());
    assert!(no_finalization.no_commitment());
    assert!(DurableCompletionSettlementFinalizationOutcome::MainNetPeerDrivenApplyRefusedNoFinalization
        .is_mainnet_peer_driven_apply_refused());

    let mut prod = ProductionSettlementFinalizationSink::default();
    let mut mainnet = MainNetSettlementFinalizationSink::default();
    let mut external = ExternalSettlementFinalizationSink::default();
    let mut prod_ledger = DurableCompletionSettlementFinalizationLedger::new();
    assert_eq!(
        prod.project_durable_completion_settlement_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_settlement_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_settlement_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_269_durable_completion_settlement_finalization_release_binary/helper_evidence/run_269")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "run266_duplicate_idempotent_commitment_only_matches_existing_never_creates", run266_duplicate_idempotent_commitment_only_matches_existing_never_creates as fn()),
        ...
        ("rejection_fail_closed", "non_recording_commitment_outcomes_never_record_finalization", non_recording_commitment_outcomes_never_record_finalization as fn()),
        ...
        ("rejection_fail_closed", "wrong_finalization_record_id_rejected_before_record", wrong_finalization_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_commitment_outcomes_create_no_settlement_finalization_request", non_recording_commitment_outcomes_create_no_settlement_finalization_request as fn()),
        ...
        ("recovery_crash_window", "pre_settlement_commitment_windows_fail_closed_no_commitment", pre_settlement_commitment_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_settlement_commitment_request_before_record_rejects_before_record", after_settlement_commitment_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_settlement_commitment_record_before_success_requires_explicit_matching_record", after_settlement_commitment_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_settlement_commitment_success_recovers_as_recorded", after_settlement_commitment_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_commitment_outcome_creates_settlement_finalization_request_intent", only_recorded_commitment_outcome_creates_settlement_finalization_request_intent as fn()),
        ("projection", "non_recording_commitment_outcomes_create_no_settlement_finalization_request", non_recording_commitment_outcomes_create_no_settlement_finalization_request as fn()),
        ("projection", "run266_duplicate_idempotent_commitment_only_matches_existing_never_creates", run266_duplicate_idempotent_commitment_only_matches_existing_never_creates as fn()),
        ("settlement_finalization_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("settlement_finalization_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("settlement_finalization_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_commitment_outcomes_never_record_finalization", non_recording_commitment_outcomes_never_record_finalization as fn()),
        ("non_mutation", "non_recording_commitment_outcomes_create_no_settlement_finalization_request", non_recording_commitment_outcomes_create_no_settlement_finalization_request as fn()),
    ...
    summary.push_str("Run 269 durable-completion settlement-finalization release helper\n");
    summary.push_str(&format!("verdict: {}\n", if total_fail == 0 { "PASS" } else { "FAIL" }));
    summary.push_str("projection_rule: input.settlement_commitment_binding -> project_settlement_commitment_outcome_to_finalization_request\n");
    summary.push_str("attached_chain: Run256 backend -> Run258 receipt -> Run260 acknowledgement -> Run262 consumer -> Run264 settlement projection -> Run266 settlement commitment -> Run268 settlement finalization\n");
    ...
    fs::write(
        outdir.join("fixtures/run_269_projection_rule.txt"),
        "input.settlement_commitment_binding\nproject_settlement_commitment_outcome_to_finalization_request\nSettlementCommitmentRecorded -> CreateRequest\n",
