//! Run 273 — release-built helper for the Run 272 durable-completion
//! settlement-outcome report / settlement-finality projection sink boundary.
//!
...
//! finalization -> Run 270 settlement-receipt acknowledgement chain and then
//! evaluates the Run 272 settlement-outcome report. Projection is intentionally
//! from `input.settlement_receipt_acknowledgement_binding` through
...
use qbind_node::pqc_governance_durable_completion_settlement_outcome_report::{
    ...
    durable_completion_settlement_outcome_report_ambiguous_window_fails_closed,
    durable_completion_settlement_outcome_report_attestation_required,
    durable_completion_settlement_outcome_report_backend_submission_required,
    durable_completion_settlement_outcome_report_completion_report_required,
    durable_completion_settlement_outcome_report_external_unavailable,
    durable_completion_settlement_outcome_report_failed_record_never_records,
    durable_completion_settlement_outcome_report_finalization_projection_required,
    durable_completion_settlement_outcome_report_local_operator_cannot_satisfy_mainnet_authority,
    durable_completion_settlement_outcome_report_mainnet_peer_driven_apply_refused_first,
    ...
    durable_completion_settlement_outcome_report_never_writes_sequence_or_marker,
    durable_completion_settlement_outcome_report_no_external_publication,
    durable_completion_settlement_outcome_report_no_real_audit_ledger,
    durable_completion_settlement_outcome_report_no_rocksdb_file_schema_migration_change,
    durable_completion_settlement_outcome_report_peer_majority_cannot_satisfy_mainnet_authority,
    durable_completion_settlement_outcome_report_pipeline_success_required,
    durable_completion_settlement_outcome_report_policy_change_unsupported,
    durable_completion_settlement_outcome_report_production_mainnet_unavailable,
    durable_completion_settlement_outcome_report_receipt_required,
    durable_completion_settlement_outcome_report_record_required_before_reported,
    durable_completion_settlement_outcome_report_rejection_is_non_mutating,
    durable_completion_settlement_outcome_report_rollback_never_records,
    durable_completion_settlement_outcome_report_sink_receipt_required,
    durable_completion_settlement_outcome_report_validator_set_rotation_unsupported,
    ...
    DurableCompletionSettlementOutcomeReportFault,
    ...
    DurableCompletionSettlementOutcomeReportWindow, ExternalSettlementOutcomeReportSink,
    FixtureDurableCompletionSettlementOutcomeReportSink,
    GovernanceDurableCompletionSettlementOutcomeReportSink, MainNetSettlementOutcomeReportSink,
    ProductionSettlementOutcomeReportSink,
};
...
// Run 273 — additional Run 272 production library symbols exercised by the
// release symbol reachability probe (imported here so the release helper source
// grep-proves it links the real Run 272 boundary symbols in release mode).
use qbind_node::pqc_governance_durable_completion_settlement_outcome_report::{
    durable_completion_settlement_outcome_report_consumer_required,
    ...
    settlement_outcome_report_record_digest,
    settlement_outcome_report_request_digest,
    settlement_outcome_report_response_digest,
    settlement_outcome_report_transcript_digest,
    DurableCompletionSettlementOutcomeReportBinding,
    DurableCompletionSettlementOutcomeReportDigest,
    DurableCompletionSettlementOutcomeReportEnvironment,
    DurableCompletionSettlementOutcomeReportLedgerRecord,
    DurableCompletionSettlementOutcomeReportLedgerSnapshot,
    DurableCompletionSettlementOutcomeReportLedgerStatus,
    DurableCompletionSettlementOutcomeReportRecord,
    DurableCompletionSettlementOutcomeReportResponse,
    DurableCompletionSettlementOutcomeReportSurface,
...
fn ack_identity(
    policy: DurableCompletionSettlementOutcomeReportPolicy,
...
fn ctx_action(
    ...
    let settlement_receipt_acknowledgement =
        attach_run270_settlement_receipt_acknowledgement(environment, action, consumer_duplicate);
    ...
    let expectations = DurableCompletionSettlementOutcomeReportExpectations {
        ...
        expected_outcome_report_kind: kind,
        expected_outcome_report_policy: policy,
...
impl Ctx {
    #[allow(clippy::too_many_arguments)]
    fn input_with_receipt_acknowledgement(
        &self,
        policy: DurableCompletionSettlementOutcomeReportPolicy,
        ...
        outcome_report: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        settlement_receipt_acknowledgement: DurableCompletionSettlementReceiptAcknowledgementOutcome,
    ) -> DurableCompletionSettlementOutcomeReportInput {
        DurableCompletionSettlementOutcomeReportInput {
            policy,
            ...
            receipt_acknowledgement_binding: outcome_report,
            ...
            settlement_receipt_acknowledgement_binding: settlement_receipt_acknowledgement,
    ...
    fn input(
        ...
        self.input_with_receipt_acknowledgement(
            policy,
            replay,
            pipeline,
            sink,
            ...
            self.settlement_receipt_acknowledgement.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionSettlementOutcomeReportInput {
        self.input_with_receipt_acknowledgement(
...
fn fixture_sink() -> FixtureDurableCompletionSettlementOutcomeReportSink {
    FixtureDurableCompletionSettlementOutcomeReportSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionSettlementOutcomeReportPolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn disabled_backend_policy_never_invokes_receipt_sink() {
    let c = devnet_ctx();
    let input = c.input_with_receipt_acknowledgement(
        DurableCompletionSettlementOutcomeReportPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(settlement_outcome_report_outcome_authorizes_record(&outcome));
    assert!(settlement_outcome_report_outcome_projects_to_recorded(&outcome));
    assert_eq!(sink.invocations(), 1);
    assert_eq!(ledger.len(), 1);
    assert!(ledger.contains(OUTCOME_REPORT_RECORD_ID));
    assert_eq!(
        settlement_outcome_report_identity_digest(&c.request.identity),
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.outcome_report_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!settlement_outcome_report_outcome_authorizes_record(&second));
    assert!(settlement_outcome_report_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run268_duplicate_idempotent_receipt_acknowledgement_only_matches_existing_never_creates() {
    // A real Run 270 SettlementReceiptAcknowledgementDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.settlement_receipt_acknowledgement.outcome,
        DurableCompletionSettlementReceiptAcknowledgementOutcome::SettlementReceiptAcknowledgementDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_settlement_outcome_report(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_settlement_outcome_report(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomeReportPolicy::ProductionSettlementOutcomeReportRequired,
        DurableCompletionSettlementOutcomeReportKind::ProductionSettlementOutcomeReportUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    let mut sink = ProductionSettlementOutcomeReportSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomeReportPolicy::MainNetSettlementOutcomeReportRequired,
        DurableCompletionSettlementOutcomeReportKind::MainNetSettlementOutcomeReportUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    let mut sink = MainNetSettlementOutcomeReportSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomeReportPolicy::ExternalSettlementOutcomeReportRequired,
        DurableCompletionSettlementOutcomeReportKind::ExternalSettlementOutcomeReportUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    let mut sink = ExternalSettlementOutcomeReportSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn validator_set_rotation_and_policy_change_unsupported() {
    let c = devnet_ctx();
    let rotation = c.input_with_receipt_acknowledgement(
        DurableCompletionSettlementOutcomeReportPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
    ...
    let policy_change = c.input_with_receipt_acknowledgement(
        DurableCompletionSettlementOutcomeReportPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
...
fn assert_non_recording_receipt_acknowledgement(
    settlement_receipt_acknowledgement: DurableCompletionSettlementReceiptAcknowledgementOutcome,
    expected: DurableCompletionSettlementOutcomeReportOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_receipt_acknowledgement(
        DurableCompletionSettlementOutcomeReportPolicy::FixtureAllowed,
...
fn non_recording_receipt_acknowledgement_outcomes_never_record_outcome_report() {
    ...
    assert_non_recording_receipt_acknowledgement(
        Finalization::LegacyBypassNoSettlementReceiptAcknowledgement,
        OutcomeReport::LegacyBypassNoSettlementOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::RejectedBeforeSettlementFinalizationNoReceiptAcknowledgement,
        OutcomeReport::RejectedBeforeSettlementReceiptAcknowledgementNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::SettlementFinalizationDidNotRecordNoReceiptAcknowledgement,
        OutcomeReport::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
    ...
    assert_non_recording_receipt_acknowledgement(
        Finalization::ProductionSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
        OutcomeReport::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::MainNetSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
        OutcomeReport::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::ExternalSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
        OutcomeReport::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement,
        OutcomeReport::MainNetPeerDrivenApplyRefusedNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::ValidatorSetRotationUnsupportedNoReceiptAcknowledgement,
        OutcomeReport::ValidatorSetRotationUnsupportedNoOutcomeReport,
    );
    assert_non_recording_receipt_acknowledgement(
        Finalization::PolicyChangeUnsupportedNoReceiptAcknowledgement,
        OutcomeReport::PolicyChangeUnsupportedNoOutcomeReport,
    );
}
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomeReportOutcome::RejectedBeforeSettlementReceiptAcknowledgementNoOutcomeReport
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionSettlementOutcomeReportRequest),
) {
    let c = devnet_ctx();
...
fn wrong_outcome_report_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.outcome_report_record_id = "other-receipt_acknowledgement".to_string();
    });
}
...
fn wrong_acknowledgement_identity_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.identity.receipt_acknowledgement_id = "other-receipt-id".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_settlement_outcome_report(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_receipt(
    fault: DurableCompletionSettlementOutcomeReportFault,
    expected: DurableCompletionSettlementOutcomeReportOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    let mut sink = FixtureDurableCompletionSettlementOutcomeReportSink::with_fault(fault);
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportRecordFailedNoOutcomeReport,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportRolledBackNoOutcomeReport,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportRollbackFailedFatalNoOutcomeReport,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportAmbiguousFailClosedNoOutcomeReport,
    );
}
...
fn only_recorded_receipt_acknowledgement_outcome_creates_settlement_outcome_report_request_intent() {
    use DurableCompletionSettlementReceiptAcknowledgementOutcome as Finalization;
    ...
    assert!(project_settlement_receipt_acknowledgement_outcome_to_outcome_report_request(
        &Finalization::SettlementReceiptAcknowledgementRecorded
    )
    .creates_request());
    assert_eq!(
        project_settlement_receipt_acknowledgement_outcome_to_outcome_report_request(
            &Finalization::SettlementReceiptAcknowledgementDuplicateIdempotent
        ),
...
fn non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request() {
    use DurableCompletionSettlementReceiptAcknowledgementOutcome as Finalization;
    use DurableCompletionSettlementOutcomeReportRequestIntent as Intent;
    for receipt_acknowledgement in [
        Finalization::LegacyBypassNoSettlementReceiptAcknowledgement,
        Finalization::RejectedBeforeSettlementFinalizationNoReceiptAcknowledgement,
        Finalization::SettlementFinalizationDidNotRecordNoReceiptAcknowledgement,
        Finalization::SettlementReceiptAcknowledgementRejectedBeforeRecord,
        ...
        assert!(matches!(
            project_settlement_receipt_acknowledgement_outcome_to_outcome_report_request(&receipt_acknowledgement),
            Intent::NoReceiptAcknowledgement(_)
        ));
    }
}
...
fn recovered_ledger() -> DurableCompletionSettlementOutcomeReportLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_settlement_outcome_report(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let record = if with_record {
        ledger.find(OUTCOME_REPORT_RECORD_ID)
    } else {
        None
    };
    let outcome = recover_durable_completion_settlement_outcome_report_window(
        &input,
        window,
        DurableCompletionSettlementOutcomeReportKind::FixtureInMemory,
        record,
...
fn pre_settlement_receipt_acknowledgement_windows_fail_closed_no_commitment() {
...
fn after_settlement_receipt_acknowledgement_request_before_record_rejects_before_record() {
    use DurableCompletionSettlementOutcomeReportOutcome as Finalization;
    use DurableCompletionSettlementOutcomeReportWindow as Window;
    assert_window(
...
fn after_settlement_receipt_acknowledgement_record_before_success_requires_explicit_matching_record() {
    use DurableCompletionSettlementOutcomeReportOutcome as Finalization;
    use DurableCompletionSettlementOutcomeReportWindow as Window;
    assert_window(
...
fn after_settlement_receipt_acknowledgement_success_recovers_as_recorded() {
    use DurableCompletionSettlementOutcomeReportOutcome as Finalization;
    use DurableCompletionSettlementOutcomeReportWindow as Window;
    assert_window(
        Window::AfterSettlementOutcomeReportSuccess,
        true,
        Finalization::SettlementOutcomeReportRecorded,
    );
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_settlement_outcome_report_window(
        &input,
        DurableCompletionSettlementOutcomeReportWindow::AfterSettlementOutcomeReportSuccess,
        DurableCompletionSettlementOutcomeReportKind::ProductionSettlementOutcomeReportUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionSettlementOutcomeReportSink::with_fault(
        DurableCompletionSettlementOutcomeReportFault::RolledBackNoReceiptAcknowledgement,
    );
    let outcome = evaluate_durable_completion_settlement_outcome_report(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_settlement_outcome_report_rejection_is_non_mutating());
    assert!(durable_completion_settlement_outcome_report_never_calls_run_070());
    assert!(durable_completion_settlement_outcome_report_never_mutates_live_pqc_trust_state());
    assert!(durable_completion_settlement_outcome_report_never_writes_sequence_or_marker());
    assert!(durable_completion_settlement_outcome_report_no_rocksdb_file_schema_migration_change());
    assert!(durable_completion_settlement_outcome_report_no_external_publication());
    assert!(durable_completion_settlement_outcome_report_no_real_audit_ledger());
    assert!(durable_completion_settlement_outcome_report_pipeline_success_required());
    assert!(durable_completion_settlement_outcome_report_sink_receipt_required());
    assert!(durable_completion_settlement_outcome_report_completion_report_required());
    assert!(durable_completion_settlement_outcome_report_finalization_projection_required());
    assert!(durable_completion_settlement_outcome_report_attestation_required());
    assert!(durable_completion_settlement_outcome_report_backend_submission_required());
    assert!(durable_completion_settlement_outcome_report_receipt_required());
    assert!(durable_completion_settlement_outcome_report_record_required_before_reported());
    assert!(durable_completion_settlement_outcome_report_failed_record_never_records());
    assert!(durable_completion_settlement_outcome_report_rollback_never_records());
    assert!(durable_completion_settlement_outcome_report_ambiguous_window_fails_closed());
    assert!(
        durable_completion_settlement_outcome_report_mainnet_peer_driven_apply_refused_first(
            TrustBundleEnvironment::Mainnet
        )
    ...
    assert!(durable_completion_settlement_outcome_report_production_mainnet_unavailable());
    assert!(durable_completion_settlement_outcome_report_external_unavailable());
    assert!(durable_completion_settlement_outcome_report_validator_set_rotation_unsupported());
    assert!(durable_completion_settlement_outcome_report_policy_change_unsupported());
    assert!(
        durable_completion_settlement_outcome_report_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
// ===========================================================================
// Release symbol reachability probe — exercises the Run 272 production library
// symbols in release mode, driving projection from the Run 270 settlement-
// receipt-acknowledgement outcome via input.settlement_receipt_acknowledgement_binding.
// ===========================================================================

fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionSettlementOutcomeReportInput = c.recorded();

    let intent = project_settlement_receipt_acknowledgement_outcome_to_outcome_report_request(
        &input.settlement_receipt_acknowledgement_binding,
    );
    assert!(intent.creates_request());
    assert!(matches!(
        intent,
        DurableCompletionSettlementOutcomeReportRequestIntent::CreateRequest
    ));
    ...
    assert!(!outcome.no_commitment());
    assert_eq!(outcome.tag(), "settlement-outcome-report-recorded");
    assert_eq!(
        sink.kind(),
        DurableCompletionSettlementOutcomeReportKind::FixtureInMemory
    );
    ...
    let identity_digest: DurableCompletionSettlementOutcomeReportDigest =
        settlement_outcome_report_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionSettlementOutcomeReportDigest =
        settlement_outcome_report_request_digest(&c.request);
    let response = DurableCompletionSettlementOutcomeReportResponse {
        outcome_report_record_id: c.request.outcome_report_record_id.clone(),
        request_digest: request_digest.clone(),
        accepted: true,
        outcome_report_kind: DurableCompletionSettlementOutcomeReportKind::FixtureInMemory,
    };
    assert!(response.is_well_formed());
    let response_digest: DurableCompletionSettlementOutcomeReportDigest =
        settlement_outcome_report_response_digest(&response);
    let record: DurableCompletionSettlementOutcomeReportRecord = c.request.to_record();
    let record_digest: DurableCompletionSettlementOutcomeReportDigest =
        settlement_outcome_report_record_digest(&record);
    let transcript_digest: DurableCompletionSettlementOutcomeReportTranscriptDigest =
        settlement_outcome_report_transcript_digest(&request_digest, &response_digest, &record_digest);
    assert!(!identity_digest.as_hex().is_empty());
    assert!(!transcript_digest.as_hex().is_empty());

    let ledger_record: &DurableCompletionSettlementOutcomeReportLedgerRecord = ledger
        .find(OUTCOME_REPORT_RECORD_ID)
        .expect("settlement-outcome-report ledger record");
    assert_eq!(
        ledger_record.status,
        DurableCompletionSettlementOutcomeReportLedgerStatus::Recorded
    );
    let snapshot: DurableCompletionSettlementOutcomeReportLedgerSnapshot = ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionSettlementOutcomeReportSurface = input.surface();
    let _environment: DurableCompletionSettlementOutcomeReportEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionSettlementOutcomeReportBinding = input.runtime_binding.clone();

    assert!(DurableCompletionSettlementOutcomeReportPolicy::Disabled.is_disabled());
    assert!(DurableCompletionSettlementOutcomeReportPolicy::FixtureAllowed.allows_fixture());
    assert_eq!(
        DurableCompletionSettlementOutcomeReportPolicy::ProductionSettlementOutcomeReportRequired.tag(),
        "production-settlement-outcome-report-required"
    );
    assert!(DurableCompletionSettlementOutcomeReportKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionSettlementOutcomeReportKind::ProductionSettlementOutcomeReportUnavailable.is_unavailable());
    assert_eq!(
        DurableCompletionSettlementOutcomeReportKind::Disabled.tag(),
        "disabled"
    );
    let no_outcome_report =
        DurableCompletionSettlementOutcomeReportOutcome::LegacyBypassNoSettlementOutcomeReport;
    assert!(no_outcome_report.is_legacy_bypass());
    assert!(no_outcome_report.no_commitment());
    assert!(
        DurableCompletionSettlementOutcomeReportOutcome::MainNetPeerDrivenApplyRefusedNoOutcomeReport
    ...
    assert!(durable_completion_settlement_outcome_report_consumer_required());
    assert!(durable_completion_settlement_outcome_report_receipt_acknowledgement_required());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement_finality());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement_receipt());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement_receipt_acknowledgement());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement_finality_projection());
    assert!(durable_completion_settlement_outcome_report_no_real_settlement_outcome_publication());

    let mut prod = ProductionSettlementOutcomeReportSink::default();
    let mut mainnet = MainNetSettlementOutcomeReportSink::default();
    let mut external = ExternalSettlementOutcomeReportSink::default();
    let mut prod_ledger = DurableCompletionSettlementOutcomeReportLedger::new();
    assert_eq!(
        prod.project_durable_completion_settlement_outcome_report(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_settlement_outcome_report(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_settlement_outcome_report(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_273_durable_completion_settlement_outcome_report_release_binary/helper_evidence/run_273")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "run268_duplicate_idempotent_receipt_acknowledgement_only_matches_existing_never_creates", run268_duplicate_idempotent_receipt_acknowledgement_only_matches_existing_never_creates as fn()),
        ...
        ("rejection_fail_closed", "non_recording_receipt_acknowledgement_outcomes_never_record_outcome_report", non_recording_receipt_acknowledgement_outcomes_never_record_outcome_report as fn()),
        ...
        ("rejection_fail_closed", "wrong_outcome_report_record_id_rejected_before_record", wrong_outcome_report_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request", non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request as fn()),
        ...
        ("recovery_crash_window", "pre_settlement_receipt_acknowledgement_windows_fail_closed_no_commitment", pre_settlement_receipt_acknowledgement_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_settlement_receipt_acknowledgement_request_before_record_rejects_before_record", after_settlement_receipt_acknowledgement_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_settlement_receipt_acknowledgement_record_before_success_requires_explicit_matching_record", after_settlement_receipt_acknowledgement_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_settlement_receipt_acknowledgement_success_recovers_as_recorded", after_settlement_receipt_acknowledgement_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_receipt_acknowledgement_outcome_creates_settlement_outcome_report_request_intent", only_recorded_receipt_acknowledgement_outcome_creates_settlement_outcome_report_request_intent as fn()),
        ("projection", "non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request", non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request as fn()),
        ("projection", "run268_duplicate_idempotent_receipt_acknowledgement_only_matches_existing_never_creates", run268_duplicate_idempotent_receipt_acknowledgement_only_matches_existing_never_creates as fn()),
        ("settlement_outcome_report_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("settlement_outcome_report_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("settlement_outcome_report_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_receipt_acknowledgement_outcomes_never_record_outcome_report", non_recording_receipt_acknowledgement_outcomes_never_record_outcome_report as fn()),
        ("non_mutation", "non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request", non_recording_receipt_acknowledgement_outcomes_create_no_settlement_outcome_report_request as fn()),
    ...
    summary.push_str("Run 273 durable-completion settlement-outcome-report release helper\n");
    summary.push_str(&format!("verdict: {}\n", if total_fail == 0 { "PASS" } else { "FAIL" }));
    summary.push_str("projection_rule: input.settlement_receipt_acknowledgement_binding -> project_settlement_receipt_acknowledgement_outcome_to_outcome_report_request\n");
    summary.push_str("attached_chain: Run256 backend -> Run258 receipt -> Run260 acknowledgement -> Run262 consumer -> Run264 settlement projection -> Run266 settlement commitment -> Run268 settlement finalization -> Run270 settlement-receipt acknowledgement -> Run272 settlement-outcome report\n");
    ...
    fs::write(
        outdir.join("fixtures/run_273_projection_rule.txt"),
        "input.settlement_receipt_acknowledgement_binding\nproject_settlement_receipt_acknowledgement_outcome_to_outcome_report_request\nSettlementReceiptAcknowledgementRecorded -> CreateRequest\n",
