...
use qbind_node::pqc_governance_modeled_durable_completion_finalization_projection::{
    ...
    ModeledDurableCompletionFinalizationDigest, ModeledDurableCompletionFinalizationFault,
    ModeledDurableCompletionFinalizationLedger, ModeledDurableCompletionFinalizationRecord,
    ModeledDurableCompletionFinalizationSnapshot, ModeledDurableCompletionFinalizationStatus,
...
impl Table {
    ...
    fn check_outcome(
        ...
        id: &str,
        expected: &str,
        o: &GovernanceModeledDurableCompletionFinalizationOutcome,
    ) {
...
fn drive(
    ...
    expectations: &GovernanceModeledDurableCompletionFinalizationExpectations,
    finalizer: &mut FixtureModeledDurableCompletionFinalizer,
...
fn run_accepted_table(out: &Path) -> (u64, u64) {
    ...
    use GovernanceModeledDurableCompletionFinalizationPolicy as P;
    ...
    for (id, policy) in [
        ("A1.finalizer-disabled", P::finalization_disabled()),
        ("A2.reporter-disabled", P::reporter_disabled()),
        ("A3.sink-disabled", P::sink_disabled()),
        ("A4.pipeline-disabled", P::pipeline_disabled()),
        ("A5.evaluator-disabled", P::evaluator_disabled()),
    ] {
        let c = devnet_ctx();
        let input = c.input(
            policy,
        ...
        let o = drive(&input, &c.expectations, &mut finalizer, &mut ledger);
        t.check_outcome(
            &format!("{id}.outcome"),
            "legacy-bypass-no-finalization",
            &o,
        );
        t.assert_true(&format!("{id}.no-invocation"), finalizer.invocations() == 0);
    ...
    for (id, c, env, kind) in [
        ...
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut finalizer = FixtureModeledDurableCompletionFinalizer::new(env);
        let o = drive(&c.recorded(), &c.expectations, &mut finalizer, &mut ledger);
        t.check_outcome(&format!("{id}.outcome"), "durable-completion-finalized", &o);
        t.assert_true(
            &format!("{id}.authorizes"),
            o.authorizes_modeled_durable_completion(),
        );
        t.assert_true(
            &format!("{id}.projects"),
        ...
        t.check(&format!("{id}.kind"), kind, finalizer.kind().tag());
    }
    for action in [
        "add-root",
        "retire-root",
        ...
        let o = drive(&c.recorded(), &c.expectations, &mut finalizer, &mut ledger);
        t.check_outcome(
            &format!("A8.{action}.outcome"),
            "durable-completion-finalized",
            &o,
        );
        t.assert_true(&format!("A8.{action}.ledger-one"), ledger.len() == 1);
    }
    {
        let c = devnet_ctx();
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut finalizer = devnet_finalizer();
        let first = drive(&c.recorded(), &c.expectations, &mut finalizer, &mut ledger);
        let second = drive(&c.recorded(), &c.expectations, &mut finalizer, &mut ledger);
        t.check_outcome("A9.first", "durable-completion-finalized", &first);
        t.check_outcome(
            "A9.duplicate",
            "durable-completion-duplicate-idempotent",
            &second,
        );
        t.assert_true("A9.ledger-one", ledger.len() == 1);
    }
    {
        let c = devnet_ctx();
        ...
        let mut empty = ModeledDurableCompletionFinalizationLedger::new();
        let mut finalizer = devnet_finalizer();
        let o = drive(&dup_input, &c.expectations, &mut finalizer, &mut empty);
        t.check_outcome(
            "A10.duplicate-empty",
            "durable-completion-rejected-before-record",
            &o,
        );
        t.assert_true("A10.empty", empty.is_empty());
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f2 = devnet_finalizer();
        let _ = drive(&c.recorded(), &c.expectations, &mut f2, &mut ledger);
        let o2 = drive(&dup_input, &c.expectations, &mut f2, &mut ledger);
        t.check_outcome(
            "A10.duplicate-matches",
            "durable-completion-duplicate-idempotent",
            &o2,
        );
    }
    {
        let c = devnet_ctx();
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f = ProductionModeledDurableCompletionFinalizer::default();
        let o = evaluate_modeled_durable_completion_finalization_projection(
            &c.recorded(),
            &c.expectations,
            &mut f,
            &mut ledger,
        ...
        t.assert_true("A11.no-record", ledger.is_empty());
        t.check("A11.kind", "production-unavailable", f.kind().tag());
    }
    {
        let c = ctx(
        ...
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f = MainNetModeledDurableCompletionFinalizer::default();
        let o = evaluate_modeled_durable_completion_finalization_projection(
            &c.recorded(),
            &c.expectations,
            &mut f,
    ...
    }
    {
        ...
        t.assert_true("A13.no-invocation", finalizer.invocations() == 0);
    }
    for (id, pipe, sink, report, tag) in [
        (
            "A14.validator",
        ...
        let c = devnet_ctx();
        let input = c.input(P::wired(), R::MutationAuthorized, pipe, sink, report);
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f = devnet_finalizer();
        let o = drive(&input, &c.expectations, &mut f, &mut ledger);
        t.check_outcome(id, tag, &o);
        t.assert_true(&format!("{id}.no-invocation"), f.invocations() == 0);
    }
    t.finish(out)
}
...
fn run_rejection_table(out: &Path) -> (u64, u64) {
    ...
    let reporter_cases: [(&str, Report, &str); 13] = [
        (
            "legacy",
    ...
    for (label, report, tag) in reporter_cases {
        let c = devnet_ctx();
        let input = c.input(
            P::wired(),
            R::MutationAuthorized,
        ...
        let mut f = devnet_finalizer();
        let o = drive(&input, &c.expectations, &mut f, &mut ledger);
        t.check_outcome(&format!("B.reporter.{label}"), tag, &o);
        t.assert_true(&format!("B.reporter.{label}.no-record"), ledger.is_empty());
        if tag != "durable-completion-finalized" {
            t.assert_true(
                &format!("B.reporter.{label}.no-finalization"),
                o.no_finalization(),
            );
        }
    }
    for (label, pipe) in [
        ...
        ("stale", Pipe::ReplayStaleOrExpiredNoConsume),
        ("consumed", Pipe::ReplayConsumedNoConsume),
        ("superseded", Pipe::ReplaySupersededNoConsume),
        ("backend", Pipe::BackendUnavailableNoConsume),
        (
            "before-snapshot",
        ...
        let o = drive(&input, &c.expectations, &mut f, &mut ledger);
        t.check_outcome(
            &format!("B.pipeline.{label}"),
            "reporter-did-not-record-completion-no-finalization",
            &o,
        );
        t.assert_true(
            &format!("B.pipeline.{label}.no-invocation"),
            f.invocations() == 0,
        );
    }
    for (label, sink) in [
        ("did-not", Sink::PipelineDidNotAuthorizeConsumeNoReceipt),
        ("reject", Sink::ConsumeReceiptRejectedBeforeRecord),
        ("record-failure", Sink::ConsumeReceiptRecordFailedNoConsume),
        ("rollback", Sink::ConsumeReceiptRolledBackNoConsume),
        (
            "rollback-failed",
        ...
        t.assert_true(
            &format!("B.sink.{label}.no-invocation"),
            f.invocations() == 0,
        );
    }
    for (label, fault, tag) in [
        ...
        let c = devnet_ctx();
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f = FixtureModeledDurableCompletionFinalizer::with_fault(
            TrustBundleEnvironment::Devnet,
            fault,
        );
        let o = drive(&c.recorded(), &c.expectations, &mut f, &mut ledger);
        t.check_outcome(&format!("B.fault.{label}"), tag, &o);
        t.assert_true(&format!("B.fault.{label}.invoked"), f.invocations() == 1);
    ...
    let finalization_cases: [(&str, fn(&mut Ctx)); 11] = [
        (
            "wrong-finalization-digest",
            (|c: &mut Ctx| {
                c.finalization.finalization_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-report-digest",
            (|c: &mut Ctx| {
                c.finalization.report_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-receipt-digest",
            (|c: &mut Ctx| {
                c.finalization.receipt_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-sink-decision-digest",
            (|c: &mut Ctx| {
                c.finalization.sink_decision_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-reporter-decision-digest",
            (|c: &mut Ctx| {
                c.finalization.reporter_decision_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-pipeline-decision-digest",
            (|c: &mut Ctx| {
                c.finalization.pipeline_decision_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-proposal",
            (|c: &mut Ctx| {
                c.finalization.proposal_id = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-decision",
            (|c: &mut Ctx| {
                c.finalization.decision_id = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-candidate",
            (|c: &mut Ctx| {
                c.finalization.candidate_digest = "wrong".to_string();
            }) as fn(&mut Ctx),
        ),
        (
            "wrong-sequence",
    ...
    for (label, mutate) in finalization_cases {
        let mut c = devnet_ctx();
        mutate(&mut c);
        let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
        let mut f = devnet_finalizer();
        let o = drive(&c.recorded(), &c.expectations, &mut f, &mut ledger);
        t.check_outcome(
            &format!("B.finalization.{label}"),
            "durable-completion-rejected-before-record",
            &o,
        );
        t.assert_true(
            &format!("B.finalization.{label}.invoked"),
            f.invocations() == 1,
        );
        t.assert_true(&format!("B.finalization.{label}.empty"), ledger.is_empty());
    }
    t.assert_true(
...
fn run_recovery_table(out: &Path) -> (u64, u64) {
    use ModeledDurableCompletionFinalizationWindow as W;
    ...
    for (id, w, tag) in [
        ...
        (
            "C.after-record-before-success-no-finalization",
            W::AfterFinalizationRecordBeforeFinalizationSuccess,
            "durable-completion-rejected-before-record",
        ),
        (
            "C.after-ambiguous",
            W::AfterFinalizationAmbiguous,
            "durable-completion-ambiguous-fail-closed-no-finalization",
        ),
        (
            "C.record-failed",
            W::FinalizationRecordFailed,
            "durable-completion-record-failed-no-finalization",
        ),
        (
            "C.rollback",
    ...
    let c = devnet_ctx();
    let fin = c.finalization.clone();
    t.check_outcome(
        "C.after-record-before-success-with-finalization",
        "durable-completion-finalized",
        &recover_devnet(
            W::AfterFinalizationRecordBeforeFinalizationSuccess,
            Some(&fin),
    ...
    t.check_outcome(
        "C.after-success",
        "durable-completion-finalized",
        &recover_devnet(W::AfterFinalizationSuccess, Some(&fin)),
    );
    let o = recover_modeled_durable_completion_finalization_window(
        &c.recorded(),
        W::AfterFinalizationSuccess,
    ...
    let o2 = recover_modeled_durable_completion_finalization_window(
        &c2.recorded(),
        W::AfterFinalizationSuccess,
    ...
    let o3 = recover_modeled_durable_completion_finalization_window(
        &c3.recorded(),
        W::AfterFinalizationSuccess,
        ModeledDurableCompletionFinalizerKind::FixtureDevNet,
        Some(&c3.finalization),
        &c3.expectations,
    );
    t.check_outcome(
...
fn run_projection_table(out: &Path) -> (u64, u64) {
    ...
    t.assert_true(
        "D.only-completion-report-recorded-creates-intent",
        project_completion_reporter_outcome_to_finalization_intent(
            &Report::CompletionReportRecorded,
        )
        .creates_intent(),
    ...
    t.assert_true(
        "D.pipeline-success-alone-no-intent",
        !project_completion_reporter_outcome_to_finalization_intent(
            &Report::SinkDidNotRecordReceiptNoCompletionReport,
        )
        .creates_intent(),
    ...
    t.assert_true(
        "D.finalized-authorizes",
        finalization_outcome_authorizes_modeled_durable_completion(
            &Final::DurableCompletionFinalized,
        ),
    );
    t.assert_true(
        "D.finalized-projects",
        finalization_outcome_projects_to_durable_completion(&Final::DurableCompletionFinalized),
    );
    t.assert_true(
...
fn run_stage_ordering_table(out: &Path) -> (u64, u64) {
    ...
    t.assert_true(
        ...
        o.is_mainnet_peer_driven_apply_refused() && f.invocations() == 0 && ledger.is_empty(),
    );
    let mut c2 = devnet_ctx();
    c2.env.genesis_hash = "wrong".to_string();
    let mut ledger2 = ModeledDurableCompletionFinalizationLedger::new();
    let mut f2 = devnet_finalizer();
    let _ = drive(&c2.recorded(), &c2.expectations, &mut f2, &mut ledger2);
    t.assert_true("E.binding-before-finalizer", f2.invocations() == 0);
    let c3 = devnet_ctx();
    let mut ledger3 = ModeledDurableCompletionFinalizationLedger::new();
    let mut f3 = FixtureModeledDurableCompletionFinalizer::with_fault(
        TrustBundleEnvironment::Devnet,
        ModeledDurableCompletionFinalizationFault::RecordFailedNoFinalization,
    );
    let o3 = drive(&c3.recorded(), &c3.expectations, &mut f3, &mut ledger3);
    t.assert_true(
        "E.record-failure-no-finality",
        o3.no_finalization() && f3.invocations() == 1 && ledger3.is_empty(),
    );
    let mut f4 = FixtureModeledDurableCompletionFinalizer::with_fault(
        TrustBundleEnvironment::Devnet,
        ModeledDurableCompletionFinalizationFault::RollbackFailedFatal,
    );
    let mut ledger4 = ModeledDurableCompletionFinalizationLedger::new();
    let o4 = drive(&c3.recorded(), &c3.expectations, &mut f4, &mut ledger4);
    t.check_outcome(
...
fn run_finalization_ledger_table(out: &Path) -> (u64, u64) {
    let mut t = Table::new("finalization_ledger");
    let c = devnet_ctx();
    let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
    let mut f = devnet_finalizer();
    let o = drive(&c.recorded(), &c.expectations, &mut f, &mut ledger);
    t.check_outcome("F.one-valid", "durable-completion-finalized", &o);
    t.assert_true(
        "F.len-one",
        ledger.len() == 1 && ledger.records().len() == 1 && ledger.contains(FINALIZATION_ID),
    );
    t.assert_true(
        "F.status",
        ledger.find(FINALIZATION_ID).map(|r| r.status)
            == Some(ModeledDurableCompletionFinalizationStatus::Finalized),
    );
    let snap = ledger.snapshot();
    t.assert_true("F.snapshot", snap.len() == 1 && !snap.is_empty());
    let mut restored = ModeledDurableCompletionFinalizationLedger::new();
    restored.restore(&snap);
    t.assert_true(
        "F.restore",
        restored.len() == 1 && restored.contains(FINALIZATION_ID),
    );
    let digest = c.finalization.digest();
    t.assert_true(
        "F.digest",
        ledger.find(FINALIZATION_ID).map(|r| r.digest.clone()) == Some(digest),
    );
    let second = drive(&c.recorded(), &c.expectations, &mut f, &mut ledger);
    t.check_outcome(
        "F.duplicate",
        "durable-completion-duplicate-idempotent",
        &second,
    ...
    c2.expectations.expected_finalization_digest = "different-digest".to_string();
    let o2 = drive(&c2.recorded(), &c2.expectations, &mut f, &mut ledger);
    t.check_outcome(
        "F.equivocation",
        "durable-completion-rejected-before-record",
        &o2,
...
fn run_non_mutation_table(out: &Path) -> (u64, u64) {
    ...
    t.assert_true(
        "G.failed-record-never-finalizes",
        modeled_finalization_failed_record_never_finalizes(),
    );
    t.assert_true(
        "G.rollback-never-finalizes",
    ...
    let mut ledger = ModeledDurableCompletionFinalizationLedger::new();
    let mut f = devnet_finalizer();
    let _ = drive(&c.recorded(), &c.expectations, &mut f, &mut ledger);
    t.assert_true(
        "G.rejected-ledger-empty",
        ledger.is_empty() && f.invocations() == 0,
    );
    t.finish(out)
}
...
fn run_reachability_table(out: &Path) -> (u64, u64) {
    ...
    t.check(
        "H.kind-devnet",
        "fixture-devnet",
        ModeledDurableCompletionFinalizerKind::FixtureDevNet.tag(),
    );
    t.check(
        "H.kind-testnet",
        "fixture-testnet",
        ModeledDurableCompletionFinalizerKind::FixtureTestNet.tag(),
    );
    t.check(
        "H.kind-production",
        "production-unavailable",
        ModeledDurableCompletionFinalizerKind::ProductionUnavailable.tag(),
    );
    t.check(
        "H.kind-mainnet",
        "mainnet-unavailable",
        ModeledDurableCompletionFinalizerKind::MainNetUnavailable.tag(),
    );
    t.assert_true(
        "H.kind-fixture",
        ModeledDurableCompletionFinalizerKind::FixtureDevNet.is_fixture(),
    );
    t.assert_true(
        "H.kind-unavailable",
        ModeledDurableCompletionFinalizerKind::ProductionUnavailable.is_unavailable(),
    );
    t.assert_true(
        "H.policy-wired",
        GovernanceModeledDurableCompletionFinalizationPolicy::wired().is_wired(),
    );
    t.assert_true(
        "H.policy-disabled",
        !GovernanceModeledDurableCompletionFinalizationPolicy::disabled().is_wired(),
    );
...
fn run_fixture_dump(out: &Path) {
    ...
    write_file(
        ...
        &format!(
            ...
            f.invocations(),
            ledger.len(),
            ledger.contains(FINALIZATION_ID)
        ),
    );
    let mut c2 = devnet_ctx();
    ...
    let mut ledger2 = ModeledDurableCompletionFinalizationLedger::new();
    let mut f2 = devnet_finalizer();
    let o2 = drive(&c2.recorded(), &c2.expectations, &mut f2, &mut ledger2);
    write_file(
        &dir.join("rejected_lifecycle.txt"),
        &format!(
            "outcome={} invocations={} no_finalization={} ledger_len={}\n",
            o2.tag(),
            f2.invocations(),
            o2.no_finalization(),
            ledger2.len()
