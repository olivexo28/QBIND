//! Run 348 — release-binary evidence helper for the Run 347 live epoch-transition
...
//! release-built production library symbols. It exercises the **real** Run 347
//! `ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor`
...
//! behavior of the real executor. Each accepted case composes the real
//! Run 303 → 305 → 307 → 309 → 311 → 313 → 315 → 317 → 319 → 321 → 323 → 325 →
//! 327 → 329 → 331 → 333 → 335 → 337 → 339 → 341 → 343 → 345 chain to produce a
//! verified, accepted Run 345/346 live epoch-transition authority-activation
//! authorization / final-execution readiness decision (`is_accept()` with
//! `Some(authority_activation_authorization_artifact)`), then feeds that
//! decision into the Run 347 live epoch-transition authority-activation
//! execution-preparation / final-execution preflight executor, which emits only
//! a typed, deterministic, non-mutating authority-activation execution-preparation
//! / final-execution preflight artifact.
//!
...
//! enablement, no default runtime wiring, and no live validator-set mutation. The
//! executor only ever produces typed non-mutating live authority-activation
//! execution-preparation / final-execution preflight artifacts; it never applies
...
//! receipt / audit / audit-seal / audit-finalization / audit-ledger /
//! audit-ledger-commitment / durable-audit-publication / durable-replay /
...
//! authority-activation-execution-preparation / final-execution-preflight /
//! final-execution-readiness / authority-activation / publication /
//! external-publication / final-execution record, never calls Run 070, never
//! mutates `LivePqcTrustState`, never calls
//! `BasicHotStuffEngine::transition_to_epoch` on production runtime state, never
//! writes `meta:current_epoch`, and never injects a `PAYLOAD_KIND_RECONFIG`
//! block. The only mutation any positive path performs is against a caller-owned
//! in-memory `LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState`
//! used exclusively as source/test evidence.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_348.md`.
...
fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        let c = y_case(TrustBundleEnvironment::Devnet, sc);
        let d = y_eval(&c);
        assert!(d.is_accept(), "scenario must accept");
        assert!(d.authorizes_future_mutation_only());
        let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
        assert_eq!(art.staged_kind, expected_crc7_kind(sc));
        assert_eq!(art.settlement_execution_preparation_nonce, CRC7_NONCE);
        assert_eq!(art.settlement_preparation_nonce, CRC4_NONCE);
...
fn accept_all_scenarios_testnet() {
    for sc in ALL_SC {
        let c = y_case(TrustBundleEnvironment::Testnet, sc);
        let d = y_eval(&c);
        assert!(d.is_accept());
        let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
...
fn accept_outcome_carries_kind_env_target_nonce() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = y_eval(&c);
    match d.outcome {
        YO::AcceptedSourceTestLiveEpochTransitionSettlementExecutionPreparation {
            execution_kind,
            environment,
            epoch_transition_target,
            settlement_execution_preparation_nonce,
        } => {
            assert_eq!(execution_kind, YK::StageApplyValidatorAdd);
            ...
            assert_eq!(settlement_execution_preparation_nonce, CRC7_NONCE);
        }
        other => panic!("unexpected outcome: {other:?}"),
    }
}

fn accept_artifact_reexposes_consumed_settlement_preparation_transcript() {
    let dec = cw_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec.settlement_preparation_artifact.as_ref().unwrap().epoch_transition_target;
    let inputs = y_inputs(TrustBundleEnvironment::Devnet, &dec);
    let handoff_id = dec.settlement_preparation_id.clone();
    let handoff_req = dec.request_id.clone();
    let handoff_digest = dec.settlement_preparation_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor::source_test();
    let d = exec.evaluate_live_epoch_transition_settlement_execution_preparation(
        &request,
        &inputs,
        &empty_replay337(),
    );
    let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
    assert_eq!(art.settlement_preparation_decision_id, handoff_id);
    assert_eq!(art.settlement_preparation_request_id, handoff_req);
    assert_eq!(art.settlement_preparation_intent_digest, handoff_digest);
    assert_eq!(art.settlement_preparation_transcript_digest, handoff_transcript);
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let d = y_eval(&c);
    let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
    assert_eq!(d.settlement_execution_preparation_id, art.settlement_execution_preparation_id);
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(d.settlement_execution_preparation_digest, art.settlement_execution_preparation_digest);
    assert_eq!(d.transcript_digest, art.transcript_digest);
    assert!(!d.settlement_execution_preparation_id.is_empty());
    assert!(!d.request_id.is_empty());
    assert!(!d.settlement_execution_preparation_digest.is_empty());
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d1 = y_eval(&c);
        let d2 = y_eval(&c);
        assert_eq!(d1.settlement_execution_preparation_id, d2.settlement_execution_preparation_id);
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(d1.settlement_execution_preparation_digest, d2.settlement_execution_preparation_digest);
        assert_eq!(d1.transcript_digest, d2.transcript_digest);
    }
}

fn artifact_content_digest_is_stable() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
    let d = y_eval(&c);
    let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.settlement_execution_preparation_digest);
...
fn wrong_settlement_preparation_decision_id() {
    y_reject_inputs(
        |i| i.expected_settlement_preparation_decision_id = "bad".to_string(),
        YO::SettlementPreparationDecisionIdMismatch,
    );
}

fn wrong_settlement_preparation_request_id() {
    y_reject_inputs(
        |i| i.expected_settlement_preparation_request_id = "bad".to_string(),
        YO::SettlementPreparationDecisionRequestIdMismatch,
    );
}

fn wrong_settlement_preparation_intent_digest() {
    y_reject_inputs(
        |i| i.expected_settlement_preparation_intent_digest = "bad".to_string(),
        YO::SettlementPreparationDecisionIntentDigestMismatch,
    );
}

fn wrong_settlement_preparation_transcript_digest() {
    y_reject_inputs(
        |i| i.expected_settlement_preparation_transcript_digest = "bad".to_string(),
        YO::SettlementPreparationDecisionTranscriptMismatch,
    );
}

fn wrong_settlement_preparation_nonce() {
    y_reject_inputs(
        |i| i.expected_settlement_preparation_nonce = 9999,
        YO::WrongSettlementPreparationNonce,
    );
}

fn tampered_settlement_preparation_package_integrity_mismatch() {
    // Mutate the consumed package so its content digest no longer matches the
    // bound handoff decision digest.
    let mut dec = cw_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec.settlement_preparation_artifact.as_ref().unwrap().epoch_transition_target;
    let inputs = y_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.settlement_preparation_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
    let request = ProductionLiveEpochTransitionSettlementExecutionPreparationRequest::new(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::VerifiedSettlementPreparationDecision {
            decision: dec,
        },
    ...
    assert_eq!(d.outcome, YO::SettlementPreparationDecisionIntegrityMismatch);
    assert!(d.settlement_execution_preparation_artifact.is_none());
}
...
fn wrong_guarded_mutation_decision_id() {
    y_reject_inputs(
        ...
        YO::GuardedMutationDecisionIdMismatch,
    );
}
...
fn wrong_guarded_mutation_request_id() {
    y_reject_inputs(
        ...
        YO::GuardedMutationDecisionRequestIdMismatch,
    );
}
...
fn wrong_guarded_mutation_intent_digest() {
    y_reject_inputs(
        ...
        YO::GuardedMutationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_guarded_mutation_transcript_digest() {
    y_reject_inputs(
        ...
        YO::GuardedMutationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_decision_id() {
    y_reject_inputs(
        ...
        YO::StagedApplicationDecisionIdMismatch,
    );
}
...
fn wrong_staged_application_request_id() {
    y_reject_inputs(
        ...
        YO::StagedApplicationDecisionRequestIdMismatch,
    );
}
...
fn wrong_staged_application_intent_digest() {
    y_reject_inputs(
        ...
        YO::StagedApplicationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_staged_application_transcript_digest() {
    y_reject_inputs(
        ...
        YO::StagedApplicationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_nonce() {
    y_reject_inputs(
        ...
        YO::WrongStagedApplicationNonce,
    );
}
...
fn wrong_authorization_decision_id() {
    y_reject_inputs(
        ...
        YO::AuthorizationDecisionIdMismatch,
    );
}
...
fn wrong_authorization_request_id() {
    y_reject_inputs(
        ...
        YO::AuthorizationDecisionRequestIdMismatch,
    );
}
...
fn wrong_authorization_intent_digest() {
    y_reject_inputs(
        ...
        YO::AuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_authorization_transcript_digest() {
    y_reject_inputs(
        ...
        YO::AuthorizationDecisionTranscriptMismatch,
    );
}
...
fn wrong_application_intent_digest() {
    y_reject_inputs(
        ...
        YO::WrongApplicationIntentDigest,
    );
}
...
fn wrong_application_transcript_digest() {
    y_reject_inputs(
        ...
        YO::WrongApplicationTranscriptDigest,
    );
}
...
fn wrong_governance_domain() {
    y_reject_inputs(
        ...
        YO::WrongGovernanceDomain,
    );
}

fn wrong_governance_epoch() {
    y_reject_inputs(|i| i.expected_governance_epoch = 999, YO::WrongGovernanceEpoch);
}

fn wrong_proposal_id() {
    y_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), YO::WrongProposalId);
}

fn wrong_governance_execution_intent_digest() {
    y_reject_inputs(
        |i| i.expected_governance_intent_digest = "bad".to_string(),
        YO::WrongGovernanceExecutionIntentDigest,
    );
}
...
fn wrong_rotation_transcript_digest() {
    y_reject_inputs(
        ...
        YO::WrongRotationTranscriptDigest,
    );
}
...
fn wrong_rotation_action() {
    y_reject_inputs(
        ...
        YO::WrongRotationAction,
    );
}

fn wrong_authority_sequence() {
    y_reject_inputs(|i| i.expected_authority_domain_sequence = 999, YO::WrongAuthoritySequence);
}
...
fn wrong_current_validator_set_digest() {
    y_reject_inputs(
        ...
        YO::WrongCurrentValidatorSetDigest,
    );
}
...
fn wrong_proposed_validator_set_digest() {
    y_reject_inputs(
        ...
        YO::WrongProposedValidatorSetDigest,
    );
}
...
fn wrong_validator_set_delta_digest() {
    y_reject_inputs(
        ...
        YO::WrongValidatorSetDeltaDigest,
    );
}

fn wrong_validator_set_epoch() {
    y_reject_inputs(|i| i.expected_validator_set_epoch = 999, YO::WrongValidatorSetEpoch);
}

fn wrong_validator_set_version() {
    y_reject_inputs(|i| i.expected_validator_set_version = 999, YO::WrongValidatorSetVersion);
}

fn wrong_current_validator_set_epoch() {
    y_reject_inputs(
        |i| i.expected_current_validator_set_epoch = 999,
        YO::WrongCurrentValidatorSetEpoch,
    );
}
...
fn wrong_current_validator_set_version() {
    y_reject_inputs(
        ...
        YO::WrongCurrentValidatorSetVersion,
    );
}

fn wrong_proposed_validator_count() {
    y_reject_inputs(|i| i.expected_proposed_validator_count = 999, YO::WrongProposedValidatorCount);
}

fn wrong_rotation_nonce() {
    y_reject_inputs(|i| i.expected_rotation_nonce = 999, YO::WrongRotationNonce);
}
...
fn wrong_epoch_transition_target_inputs() {
    y_reject_inputs(|i| i.expected_epoch_transition_target = 999, YO::WrongEpochTransitionTarget);
}
...
fn wrong_epoch_transition_target_request() {
    ...
    let d = y_eval(&c);
    assert_eq!(d.outcome, YO::WrongEpochTransitionTarget);
    assert!(d.settlement_execution_preparation_artifact.is_none());
}

fn wrong_application_nonce() {
    y_reject_inputs(|i| i.expected_application_nonce = 999, YO::WrongApplicationNonce);
}

fn wrong_live_application_nonce() {
    y_reject_inputs(|i| i.expected_live_application_nonce = 999, YO::WrongLiveApplicationNonce);
}
...
fn reject_missing_settlement_preparation_decision() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::MissingSettlementPreparationDecision,
        YO::VerifiedSettlementPreparationDecisionRequired,
    );
}

fn reject_unverified_settlement_preparation_decision() {
    let dec = cw_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::UnverifiedSettlementPreparationDecision {
            decision: dec,
        },
        YO::UnverifiedSettlementPreparationDecisionRejected,
    );
}
...
fn reject_accepted_settlement_preparation_without_package_via_verified_source() {
    let dec = cw_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::VerifiedSettlementPreparationDecision {
            decision: dec,
        },
...
fn reject_accepted_settlement_preparation_without_package_variant() {
    let dec = cw_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::AcceptedSettlementPreparationWithoutPackage {
            decision: dec,
        },
        YO::VerifiedSettlementPreparationDecisionRequired,
    );
...
fn reject_guarded_mutation_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::GuardedMutationDecisionWithoutSettlementPreparation,
        YO::GuardedMutationDecisionAloneRejected,
    );
}

fn reject_staged_application_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::StagedApplicationDecisionWithoutSettlementPreparation,
        YO::StagedApplicationDecisionAloneRejected,
    );
}

fn reject_live_application_authorization_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::LiveApplicationAuthorizationWithoutSettlementPreparation,
        YO::LiveApplicationAuthorizationAloneRejected,
    );
}

fn reject_application_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::ApplicationDecisionWithoutSettlementPreparation,
        YO::ApplicationDecisionAloneRejected,
    );
}

fn reject_rotation_plan_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::RotationPlanWithoutSettlementPreparation,
        YO::RotationPlanAloneRejected,
    );
}

fn reject_governance_execution_intent_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::GovernanceExecutionIntentWithoutSettlementPreparation,
        YO::GovernanceExecutionIntentAloneRejected,
    );
}

fn reject_governance_proof_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::GovernanceProofWithoutSettlementPreparation,
        YO::GovernanceProofAloneRejected,
    );
}

fn reject_local_operator_assertion() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::LocalOperatorAssertion,
        YO::LocalOperatorProofRejected,
    );
}

fn reject_peer_majority_assertion() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::PeerMajorityAssertion,
        YO::PeerMajorityProofRejected,
    );
}

fn reject_custody_only_evidence() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::CustodyOnlyEvidence,
        YO::CustodyOnlyProofRejected,
    );
}

fn reject_remote_signer_only_evidence() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::RemoteSignerOnlyEvidence,
        YO::RemoteSignerOnlyProofRejected,
    );
}

fn reject_custody_attestation_only_evidence() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::CustodyAttestationOnlyEvidence,
        YO::CustodyAttestationOnlyProofRejected,
    );
}

fn reject_fixture_only_settlement_preparation() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::FixtureOnlySettlementPreparation,
        YO::FixtureStagedApplicationRejectedAsProductionAuthority,
    );
}

fn reject_arbitrary_validator_set_bytes() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::ArbitraryValidatorSetBytes,
        YO::ArbitraryValidatorSetBytesRejected,
    );
}
...
fn mainnet_domain_refused_under_source_test_policy() {
    ...
    let d = y_eval(&c);
    assert_eq!(d.outcome, YO::MainNetRefused);
    assert!(d.settlement_execution_preparation_artifact.is_none());
}

fn mainnet_policy_unavailable() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = y_exec_with_policy(
        ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionSettlementExecutionPreparationRequired,
    );
    let d = exec.evaluate_live_epoch_transition_settlement_execution_preparation(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        YO::MainNetProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable
    );
    assert!(d.settlement_execution_preparation_artifact.is_none());
}
...
fn production_policy_unavailable() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = y_exec_with_policy(
        ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::RequireProductionLiveEpochTransitionSettlementExecutionPreparation,
    );
    let d = exec.evaluate_live_epoch_transition_settlement_execution_preparation(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        YO::ProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable
    );
    assert!(d.settlement_execution_preparation_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = y_exec_with_policy(
        ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::Disabled,
    );
    let d = exec.evaluate_live_epoch_transition_settlement_execution_preparation(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(d.outcome, YO::Disabled);
    assert!(d.settlement_execution_preparation_artifact.is_none());
    assert!(!d.is_accept());
}
...
fn reserved_production_kind_fails_closed() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor::new(
        ProductionLiveEpochTransitionSettlementExecutionPreparationConfig::new(
            ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::ProductionLiveEpochTransitionSettlementExecutionPreparation,
        ),
        ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::AllowSourceTestLiveEpochTransitionSettlementExecutionPreparation,
    );
    let d = exec.evaluate_live_epoch_transition_settlement_execution_preparation(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        YO::LiveEpochTransitionSettlementExecutionPreparationBoundaryUnavailable
    );
}
...
fn replay_rejected_when_id_present() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = y_eval(&c);
    let id = d.settlement_execution_preparation_artifact.as_ref().unwrap().request_id.clone();
    let replay = vec![id];
    let d2 = y_eval_replay(&c, &replay);
    match d2.outcome {
        YO::StagedApplicationReplayRejected { .. } => {}
        other => panic!("expected replay rejection, got {other:?}"),
    }
    assert!(d2.settlement_execution_preparation_artifact.is_none());
}

fn no_replay_when_id_absent() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let replay: Vec<String> = vec!["some-other-id".to_string()];
    let d = y_eval_replay(&c, &replay);
    assert!(d.is_accept());
}

fn stale_governance_epoch() {
    y_reject_inputs(|i| i.min_governance_epoch = u64::MAX, YO::StaleGovernanceEpoch);
}

fn stale_authority_sequence() {
    y_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), YO::StaleAuthoritySequence);
}

fn stale_validator_set_epoch() {
    y_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, YO::StaleValidatorSetEpoch);
}

fn stale_validator_set_version() {
    y_reject_inputs(|i| i.min_validator_set_version = u64::MAX, YO::StaleValidatorSetVersion);
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let d = y_eval(&c);
    let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
    let mut state = LiveEpochTransitionSettlementExecutionPreparationFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
    assert!(state.apply_prepared_execution(art, &d.settlement_execution_preparation_id));
    ...
    assert!(!state.apply_prepared_execution(art, &d.settlement_execution_preparation_id));
    assert!(state.has_applied(&d.settlement_execution_preparation_id));
}
...
fn every_outcome_is_non_mutating() {
    ...
    let d = y_eval(&c);
    assert!(d.outcome.is_non_mutating());
    let bad = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let dbad = y_eval_replay(&bad, &[y_eval(&bad).settlement_execution_preparation_id]);
...
fn accept_authorizes_future_mutation_only() {
    ...
    assert!(d.authorizes_future_mutation_only());
    assert!(d.settlement_execution_preparation_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
}
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionSettlementExecutionPreparation.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionSettlementExecutionPreparation.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionSettlementExecutionPreparationRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}

fn kind_predicates() {
    use ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionSettlementExecutionPreparation.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionSettlementExecutionPreparation.is_source_test());
    assert_eq!(K::default(), K::Disabled);
}

fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        let hk = expected_crc6_kind(sc);
        let pk = YK::from_staged_application_kind(hk);
        assert_eq!(pk, expected_crc7_kind(sc));
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = YK::from_staged_application_kind(LiveEpochTransitionSettlementPreparationKind::UnsupportedStagedApplication);
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = YO::SettlementPreparationDecisionIdMismatch;
    let b = YO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), YO::SettlementPreparationDecisionIdMismatch.tag());
    assert!(!YO::MainNetRefused.tag().is_empty());
}

fn config_and_inputs_well_formed() {
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    assert!(c.inputs.is_well_formed());
    assert!(ProductionLiveEpochTransitionSettlementExecutionPreparationConfig::source_test().is_well_formed());
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionSettlementExecutionPreparationConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d2 = y_eval(&c);
            assert_eq!(d.settlement_execution_preparation_digest, d2.settlement_execution_preparation_digest);
            assert_eq!(d.transcript_digest, d2.transcript_digest);
            let mut state = LiveEpochTransitionSettlementExecutionPreparationFixtureState::new(
                CUR_EPOCH,
                CUR_VERSION,
                "start",
            );
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.settlement_execution_preparation_artifact.as_ref().unwrap().environment,
...
macro_rules! per_scenario_settlement_preparation_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            let mut c = y_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_settlement_preparation_decision_id = "bad".to_string();
            let d = y_eval(&c);
            assert_eq!(d.outcome, YO::SettlementPreparationDecisionIdMismatch);
            assert!(d.settlement_execution_preparation_artifact.is_none());
        }
    };
}

per_scenario_settlement_preparation_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_settlement_preparation_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
macro_rules! per_scenario_guarded_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d = y_eval(&c);
            assert_eq!(d.outcome, YO::GuardedMutationDecisionIdMismatch);
            assert!(d.settlement_execution_preparation_artifact.is_none());
        }
    };
}
...
fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    let parent = cw_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let ppkg = parent.settlement_preparation_artifact.as_ref().unwrap().clone();
    let c = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = y_eval(&c);
    assert!(d.is_accept());
    let art = d.settlement_execution_preparation_artifact.as_ref().unwrap();
...
fn reject_runtime_handoff_decision_id_mismatch() {
    y_reject_inputs(
        ...
        YO::RuntimeHandoffDecisionIdMismatch,
    );
}
...
fn reject_runtime_handoff_request_id_mismatch() {
    y_reject_inputs(
        ...
        YO::RuntimeHandoffDecisionRequestIdMismatch,
    );
}
...
fn reject_runtime_handoff_intent_digest_mismatch() {
    y_reject_inputs(
        ...
        YO::RuntimeHandoffDecisionIntentDigestMismatch,
    );
}
...
fn reject_runtime_handoff_transcript_mismatch() {
    y_reject_inputs(
        ...
        YO::RuntimeHandoffDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_runtime_handoff_nonce() {
    y_reject_inputs(
        ...
        YO::WrongRuntimeHandoffNonce,
    );
}

fn reject_runtime_handoff_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::RuntimeHandoffDecisionWithoutSettlementPreparation,
        YO::RuntimeHandoffDecisionAloneRejected,
    );
}

fn runtime_handoff_alone_tag_is_stable() {
    assert_eq!(
        YO::RuntimeHandoffDecisionAloneRejected.tag(),
        "runtime-handoff-decision-alone-rejected"
    );
    assert_eq!(
        YO::RuntimeHandoffDecisionIdMismatch.tag(),
        "runtime-handoff-decision-id-mismatch"
    );
    assert_eq!(
        YO::WrongRuntimeHandoffNonce.tag(),
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = y_eval(&c);
    let d2 = y_eval(&c);
    let a1 = d1.settlement_execution_preparation_artifact.as_ref().unwrap();
    let a2 = d2.settlement_execution_preparation_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 335 — re-exposed Run 327/328 execution-preparation ancestor binding
// (carried through the consumed Run 333/334 settlement-preparation artifact) and
...
fn reject_mutation_execution_decision_id_mismatch() {
    y_reject_inputs(
        ...
        YO::MutationExecutionDecisionIdMismatch,
    );
}
...
fn reject_mutation_execution_request_id_mismatch() {
    y_reject_inputs(
        ...
        YO::MutationExecutionDecisionRequestIdMismatch,
    );
}
...
fn reject_mutation_execution_intent_digest_mismatch() {
    y_reject_inputs(
        ...
        YO::MutationExecutionDecisionIntentDigestMismatch,
    );
}
...
fn reject_mutation_execution_transcript_mismatch() {
    y_reject_inputs(
        ...
        YO::MutationExecutionDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_mutation_execution_nonce() {
    y_reject_inputs(
        ...
        YO::WrongMutationExecutionNonce,
    );
}

fn reject_mutation_execution_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::MutationExecutionDecisionWithoutSettlementPreparation,
        YO::MutationExecutionDecisionAloneRejected,
    );
}
...
fn reject_commit_authorization_decision_id_mismatch() {
    y_reject_inputs(
        ...
        YO::CommitAuthorizationDecisionIdMismatch,
    );
}
...
fn reject_commit_authorization_request_id_mismatch() {
    y_reject_inputs(
        ...
        YO::CommitAuthorizationDecisionRequestIdMismatch,
    );
}
...
fn reject_commit_authorization_intent_digest_mismatch() {
    y_reject_inputs(
        ...
        YO::CommitAuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn reject_commit_authorization_transcript_mismatch() {
    y_reject_inputs(
        ...
        YO::CommitAuthorizationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_commit_authorization_nonce() {
    y_reject_inputs(
        ...
        YO::WrongCommitAuthorizationNonce,
    );
}

fn reject_commit_authorization_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::CommitAuthorizationDecisionWithoutSettlementPreparation,
        YO::CommitAuthorizationDecisionAloneRejected,
    );
}
...
fn mutation_execution_alone_tag_is_stable() {
    assert_eq!(
        YO::MutationExecutionDecisionAloneRejected.tag(),
        "mutation-execution-decision-alone-rejected"
    );
    assert_eq!(
        YO::MutationExecutionDecisionIdMismatch.tag(),
        "mutation-execution-decision-id-mismatch"
    );
    assert_eq!(
        YO::WrongMutationExecutionNonce.tag(),
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    // Parent Run 333/334 settlement-preparation nonce re-exposed intact.
    assert_eq!(art.settlement_preparation_nonce, CRC4_NONCE);
    // Self Run 335 settlement-execution-preparation proposed nonce.
    assert_eq!(art.settlement_execution_preparation_nonce, CRC7_NONCE);
}
...
fn reject_execution_preparation_decision_id_mismatch() {
    y_reject_inputs(
        ...
        YO::ExecutionPreparationDecisionIdMismatch,
    );
}
...
fn reject_execution_preparation_request_id_mismatch() {
    y_reject_inputs(
        ...
        YO::ExecutionPreparationDecisionRequestIdMismatch,
    );
}
...
fn reject_execution_preparation_intent_digest_mismatch() {
    y_reject_inputs(
        ...
        YO::ExecutionPreparationDecisionIntentDigestMismatch,
    );
}
...
fn reject_execution_preparation_transcript_mismatch() {
    y_reject_inputs(
        ...
        YO::ExecutionPreparationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_execution_preparation_nonce() {
    y_reject_inputs(
        ...
        YO::WrongExecutionPreparationNonce,
    );
}

fn reject_execution_preparation_decision_alone() {
    y_reject_source(
        LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::ExecutionPreparationDecisionWithoutSettlementPreparation,
        YO::ExecutionPreparationDecisionAloneRejected,
    );
}

fn execution_preparation_alone_tag_is_stable() {
    assert_eq!(
        YO::ExecutionPreparationDecisionAloneRejected.tag(),
        "execution-preparation-decision-alone-rejected"
    );
    assert_eq!(
        YO::ExecutionPreparationDecisionIdMismatch.tag(),
        "execution-preparation-decision-id-mismatch"
    );
    assert_eq!(
        YO::WrongExecutionPreparationNonce.tag(),
...
// Run 339 — live epoch-transition settlement / finalization EXECUTION
// boundary (under test). Consumes the verified Run 337/338 settlement /
// finalization execution-preparation decision built above and produces a
// typed, non-mutating settlement / finalization execution artifact.
// ===========================================================================
mod run_339_settlement_execution {
    #![allow(unused_imports)]
    use super::*;
    ...
    // ===========================================================================
    // A. Accepted / compatible source-test settlement-execution-preparation artifacts
    // ===========================================================================

    fn accept_all_scenarios_devnet() {
        for sc in ALL_SC {
            let c = z_case(TrustBundleEnvironment::Devnet, sc);
            let d = z_eval(&c);
            assert!(d.is_accept(), "scenario must accept");
            assert!(d.authorizes_future_mutation_only());
            let art = d.settlement_execution_artifact.as_ref().unwrap();
            assert_eq!(art.staged_kind, expected_crc8_kind(sc));
            assert_eq!(art.settlement_execution_nonce, CRC8_NONCE);
            assert_eq!(art.settlement_execution_preparation_nonce, CRC4_NONCE);
            assert_eq!(art.guarded_mutation_nonce, GUARDED_NONCE);
            assert_eq!(art.staged_application_nonce, STAGED_NONCE);
        }
    }
    ...
    fn accept_all_scenarios_testnet() {
        for sc in ALL_SC {
            let c = z_case(TrustBundleEnvironment::Testnet, sc);
            let d = z_eval(&c);
            assert!(d.is_accept());
            let art = d.settlement_execution_artifact.as_ref().unwrap();
            assert_eq!(art.environment, TrustBundleEnvironment::Testnet);
    ...
    fn accept_outcome_carries_kind_env_target_nonce() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = z_eval(&c);
        match d.outcome {
            ZO::AcceptedSourceTestLiveEpochTransitionSettlementExecution {
                execution_kind,
                environment,
                epoch_transition_target,
                settlement_execution_nonce,
            } => {
                assert_eq!(execution_kind, ZK::StageApplyValidatorAdd);
                assert_eq!(environment, TrustBundleEnvironment::Devnet);
                assert_eq!(epoch_transition_target, c.request.proposed_epoch_transition_target);
                assert_eq!(settlement_execution_nonce, CRC8_NONCE);
            }
            other => panic!("unexpected outcome: {other:?}"),
        }
    }

    fn accept_artifact_reexposes_consumed_settlement_execution_preparation_transcript() {
        let dec = cz_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.settlement_execution_preparation_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = z_inputs(TrustBundleEnvironment::Devnet, &dec);
        let handoff_id = dec.settlement_execution_preparation_id.clone();
        let handoff_req = dec.request_id.clone();
        let handoff_digest = dec.settlement_execution_preparation_digest.clone();
        let handoff_transcript = dec.transcript_digest.clone();
        let request = ProductionLiveEpochTransitionSettlementExecutionRequest::new(
            LiveEpochTransitionSettlementExecutionAuthoritySource::VerifiedSettlementExecutionPreparationDecision {
                decision: dec,
        ...
        let exec = ProductionLiveEpochTransitionSettlementExecutionExecutor::source_test();
        let d = exec.evaluate_live_epoch_transition_settlement_execution(
            &request,
            &inputs,
            &empty_replay339(),
        );
        let art = d.settlement_execution_artifact.as_ref().unwrap();
        assert_eq!(art.settlement_execution_preparation_decision_id, handoff_id);
        assert_eq!(art.settlement_execution_preparation_request_id, handoff_req);
        assert_eq!(art.settlement_execution_preparation_intent_digest, handoff_digest);
        assert_eq!(art.settlement_execution_preparation_transcript_digest, handoff_transcript);
    }
    ...
    fn accept_decision_ids_match_artifact_ids() {
        ...
        let d = z_eval(&c);
        let art = d.settlement_execution_artifact.as_ref().unwrap();
        assert_eq!(d.settlement_execution_id, art.settlement_execution_id);
        assert_eq!(d.request_id, art.request_id);
        assert_eq!(d.settlement_execution_digest, art.settlement_execution_digest);
        assert_eq!(d.transcript_digest, art.transcript_digest);
        assert!(!d.settlement_execution_id.is_empty());
        assert!(!d.request_id.is_empty());
        assert!(!d.settlement_execution_digest.is_empty());
        assert!(!d.transcript_digest.is_empty());
    }
    ...
    fn deterministic_digests_under_reevaluation() {
        for sc in ALL_SC {
            ...
            let d1 = z_eval(&c);
            let d2 = z_eval(&c);
            assert_eq!(d1.settlement_execution_id, d2.settlement_execution_id);
            assert_eq!(d1.request_id, d2.request_id);
            assert_eq!(d1.settlement_execution_digest, d2.settlement_execution_digest);
            assert_eq!(d1.transcript_digest, d2.transcript_digest);
        }
    }

    fn artifact_content_digest_is_stable() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
        let d = z_eval(&c);
        let art = d.settlement_execution_artifact.as_ref().unwrap();
        assert_eq!(art.content_digest(), art.settlement_execution_digest);
        assert_eq!(art.content_digest(), art.content_digest());
    }
    ...
    fn wrong_settlement_execution_preparation_decision_id() {
        z_reject_inputs(
            |i| i.expected_settlement_execution_preparation_decision_id = "bad".to_string(),
            ZO::SettlementExecutionPreparationDecisionIdMismatch,
        );
    }

    fn wrong_settlement_execution_preparation_request_id() {
        z_reject_inputs(
            |i| i.expected_settlement_execution_preparation_request_id = "bad".to_string(),
            ZO::SettlementExecutionPreparationDecisionRequestIdMismatch,
        );
    }

    fn wrong_settlement_execution_preparation_intent_digest() {
        z_reject_inputs(
            |i| i.expected_settlement_execution_preparation_intent_digest = "bad".to_string(),
            ZO::SettlementExecutionPreparationDecisionIntentDigestMismatch,
        );
    }

    fn wrong_settlement_execution_preparation_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_settlement_execution_preparation_transcript_digest = "bad".to_string(),
            ZO::SettlementExecutionPreparationDecisionTranscriptMismatch,
        );
    }

    fn wrong_settlement_execution_preparation_nonce() {
        z_reject_inputs(
            |i| i.expected_settlement_execution_preparation_nonce = 9999,
            ZO::WrongSettlementExecutionPreparationNonce,
        );
    }

    fn tampered_settlement_execution_preparation_package_integrity_mismatch() {
        // Mutate the consumed package so its content digest no longer matches the
        // bound handoff decision digest.
        let mut dec = cz_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.settlement_execution_preparation_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = z_inputs(TrustBundleEnvironment::Devnet, &dec);
        dec.settlement_execution_preparation_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
        let request = ProductionLiveEpochTransitionSettlementExecutionRequest::new(
            LiveEpochTransitionSettlementExecutionAuthoritySource::VerifiedSettlementExecutionPreparationDecision {
                decision: dec,
            },
        ...
        assert_eq!(d.outcome, ZO::SettlementExecutionPreparationDecisionIntegrityMismatch);
        assert!(d.settlement_execution_artifact.is_none());
    }
    ...
    fn wrong_guarded_mutation_decision_id() {
        z_reject_inputs(
            |i| i.expected_guarded_mutation_decision_id = "bad".to_string(),
            ZO::GuardedMutationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_request_id() {
        z_reject_inputs(
            |i| i.expected_guarded_mutation_request_id = "bad".to_string(),
            ZO::GuardedMutationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_intent_digest() {
        z_reject_inputs(
            |i| i.expected_guarded_mutation_intent_digest = "bad".to_string(),
            ZO::GuardedMutationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_guarded_mutation_transcript_digest = "bad".to_string(),
            ZO::GuardedMutationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_decision_id() {
        z_reject_inputs(
            |i| i.expected_staged_application_decision_id = "bad".to_string(),
            ZO::StagedApplicationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_request_id() {
        z_reject_inputs(
            |i| i.expected_staged_application_request_id = "bad".to_string(),
            ZO::StagedApplicationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_intent_digest() {
        z_reject_inputs(
            |i| i.expected_staged_application_intent_digest = "bad".to_string(),
            ZO::StagedApplicationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_staged_application_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_staged_application_transcript_digest = "bad".to_string(),
            ZO::StagedApplicationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_nonce() {
        z_reject_inputs(
            |i| i.expected_staged_application_nonce = 9999,
            ZO::WrongStagedApplicationNonce,
        );
    }
    ...
    fn wrong_authorization_decision_id() {
        z_reject_inputs(
            |i| i.expected_authorization_decision_id = "bad".to_string(),
            ZO::AuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_authorization_request_id() {
        z_reject_inputs(
            |i| i.expected_authorization_request_id = "bad".to_string(),
            ZO::AuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_authorization_intent_digest() {
        z_reject_inputs(
            |i| i.expected_authorization_intent_digest = "bad".to_string(),
            ZO::AuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_authorization_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_authorization_transcript_digest = "bad".to_string(),
            ZO::AuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_application_intent_digest() {
        z_reject_inputs(
            |i| i.expected_application_intent_digest = "bad".to_string(),
            ZO::WrongApplicationIntentDigest,
        );
    }
    ...
    fn wrong_application_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_application_transcript_digest = "bad".to_string(),
            ZO::WrongApplicationTranscriptDigest,
        );
    }
    ...
    fn wrong_governance_domain() {
        z_reject_inputs(
            |i| i.expected_governance_domain_id = "bad".to_string(),
            ZO::WrongGovernanceDomain,
        );
    }

    fn wrong_governance_epoch() {
        z_reject_inputs(|i| i.expected_governance_epoch = 999, ZO::WrongGovernanceEpoch);
    }

    fn wrong_proposal_id() {
        z_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), ZO::WrongProposalId);
    }

    fn wrong_governance_execution_intent_digest() {
        z_reject_inputs(
            |i| i.expected_governance_intent_digest = "bad".to_string(),
            ZO::WrongGovernanceExecutionIntentDigest,
        );
    }
    ...
    fn wrong_rotation_transcript_digest() {
        z_reject_inputs(
            |i| i.expected_rotation_transcript_digest = "bad".to_string(),
            ZO::WrongRotationTranscriptDigest,
        );
    }
    ...
    fn wrong_rotation_action() {
        z_reject_inputs(
            |i| i.expected_rotation_action = ValidatorSetRotationAction::EmergencyValidatorRemoval,
            ZO::WrongRotationAction,
        );
    }

    fn wrong_authority_sequence() {
        z_reject_inputs(|i| i.expected_authority_domain_sequence = 999, ZO::WrongAuthoritySequence);
    }
    ...
    fn wrong_current_validator_set_digest() {
        z_reject_inputs(
            |i| i.expected_current_set_digest = "bad".to_string(),
            ZO::WrongCurrentValidatorSetDigest,
        );
    }
    ...
    fn wrong_proposed_validator_set_digest() {
        z_reject_inputs(
            |i| i.expected_proposed_set_digest = "bad".to_string(),
            ZO::WrongProposedValidatorSetDigest,
        );
    }
    ...
    fn wrong_validator_set_delta_digest() {
        z_reject_inputs(
            |i| i.expected_delta_digest = "bad".to_string(),
            ZO::WrongValidatorSetDeltaDigest,
        );
    }

    fn wrong_validator_set_epoch() {
        z_reject_inputs(|i| i.expected_validator_set_epoch = 999, ZO::WrongValidatorSetEpoch);
    }

    fn wrong_validator_set_version() {
        z_reject_inputs(|i| i.expected_validator_set_version = 999, ZO::WrongValidatorSetVersion);
    }

    fn wrong_current_validator_set_epoch() {
        z_reject_inputs(
            |i| i.expected_current_validator_set_epoch = 999,
            ZO::WrongCurrentValidatorSetEpoch,
        );
    }
    ...
    fn wrong_current_validator_set_version() {
        z_reject_inputs(
            |i| i.expected_current_validator_set_version = 999,
            ZO::WrongCurrentValidatorSetVersion,
        );
    }

    fn wrong_proposed_validator_count() {
        z_reject_inputs(|i| i.expected_proposed_validator_count = 999, ZO::WrongProposedValidatorCount);
    }

    fn wrong_rotation_nonce() {
        z_reject_inputs(|i| i.expected_rotation_nonce = 999, ZO::WrongRotationNonce);
    }
    ...
    fn wrong_epoch_transition_target_inputs() {
        z_reject_inputs(|i| i.expected_epoch_transition_target = 999, ZO::WrongEpochTransitionTarget);
    }
    ...
    fn wrong_epoch_transition_target_request() {
        ...
        c.request.proposed_epoch_transition_target = 4242;
        let d = z_eval(&c);
        assert_eq!(d.outcome, ZO::WrongEpochTransitionTarget);
        assert!(d.settlement_execution_artifact.is_none());
    }

    fn wrong_application_nonce() {
        z_reject_inputs(|i| i.expected_application_nonce = 999, ZO::WrongApplicationNonce);
    }

    fn wrong_live_application_nonce() {
        z_reject_inputs(|i| i.expected_live_application_nonce = 999, ZO::WrongLiveApplicationNonce);
    }
    ...
    fn reject_missing_settlement_execution_preparation_decision() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::MissingSettlementExecutionPreparationDecision,
            ZO::VerifiedSettlementExecutionPreparationDecisionRequired,
        );
    }

    fn reject_unverified_settlement_execution_preparation_decision() {
        let dec = cz_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::UnverifiedSettlementExecutionPreparationDecision {
                decision: dec,
            },
            ZO::UnverifiedSettlementExecutionPreparationDecisionRejected,
        );
    }
    ...
    fn reject_accepted_settlement_execution_preparation_without_package_via_verified_source() {
        let dec = cz_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::VerifiedSettlementExecutionPreparationDecision {
                decision: dec,
            },
    ...
    fn reject_accepted_settlement_execution_preparation_without_package_variant() {
        let dec = cz_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::AcceptedSettlementExecutionPreparationWithoutPackage {
                decision: dec,
            },
            ZO::VerifiedSettlementExecutionPreparationDecisionRequired,
        );
    ...
    fn reject_guarded_mutation_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::GuardedMutationDecisionWithoutSettlementExecutionPreparation,
            ZO::GuardedMutationDecisionAloneRejected,
        );
    }

    fn reject_staged_application_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::StagedApplicationDecisionWithoutSettlementExecutionPreparation,
            ZO::StagedApplicationDecisionAloneRejected,
        );
    }

    fn reject_live_application_authorization_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::LiveApplicationAuthorizationWithoutSettlementExecutionPreparation,
            ZO::LiveApplicationAuthorizationAloneRejected,
        );
    }

    fn reject_application_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::ApplicationDecisionWithoutSettlementExecutionPreparation,
            ZO::ApplicationDecisionAloneRejected,
        );
    }

    fn reject_rotation_plan_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::RotationPlanWithoutSettlementExecutionPreparation,
            ZO::RotationPlanAloneRejected,
        );
    }

    fn reject_governance_execution_intent_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::GovernanceExecutionIntentWithoutSettlementExecutionPreparation,
            ZO::GovernanceExecutionIntentAloneRejected,
        );
    }

    fn reject_governance_proof_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::GovernanceProofWithoutSettlementExecutionPreparation,
            ZO::GovernanceProofAloneRejected,
        );
    }

    fn reject_local_operator_assertion() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::LocalOperatorAssertion,
            ZO::LocalOperatorProofRejected,
        );
    }

    fn reject_peer_majority_assertion() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::PeerMajorityAssertion,
            ZO::PeerMajorityProofRejected,
        );
    }

    fn reject_custody_only_evidence() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::CustodyOnlyEvidence,
            ZO::CustodyOnlyProofRejected,
        );
    }

    fn reject_remote_signer_only_evidence() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::RemoteSignerOnlyEvidence,
            ZO::RemoteSignerOnlyProofRejected,
        );
    }

    fn reject_custody_attestation_only_evidence() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::CustodyAttestationOnlyEvidence,
            ZO::CustodyAttestationOnlyProofRejected,
        );
    }

    fn reject_fixture_only_settlement_execution_preparation() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::FixtureOnlySettlementExecutionPreparation,
            ZO::FixtureStagedApplicationRejectedAsProductionAuthority,
        );
    }

    fn reject_arbitrary_validator_set_bytes() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::ArbitraryValidatorSetBytes,
            ZO::ArbitraryValidatorSetBytesRejected,
        );
    }
    ...
    fn mainnet_domain_refused_under_source_test_policy() {
        ...
        c.inputs.trust_domain = trust_domain(TrustBundleEnvironment::Mainnet);
        let d = z_eval(&c);
        assert_eq!(d.outcome, ZO::MainNetRefused);
        assert!(d.settlement_execution_artifact.is_none());
    }

    fn mainnet_policy_unavailable() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = z_exec_with_policy(
            ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::MainnetProductionLiveEpochTransitionSettlementExecutionRequired,
        );
        let d = exec.evaluate_live_epoch_transition_settlement_execution(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            ZO::MainNetProductionLiveEpochTransitionSettlementExecutionUnavailable
        );
        assert!(d.settlement_execution_artifact.is_none());
    }
    ...
    fn production_policy_unavailable() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = z_exec_with_policy(
            ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::RequireProductionLiveEpochTransitionSettlementExecution,
        );
        let d = exec.evaluate_live_epoch_transition_settlement_execution(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            ZO::ProductionLiveEpochTransitionSettlementExecutionUnavailable
        );
        assert!(d.settlement_execution_artifact.is_none());
    }
    ...
    fn disabled_policy_fails_closed() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = z_exec_with_policy(
            ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::Disabled,
        );
        let d = exec.evaluate_live_epoch_transition_settlement_execution(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(d.outcome, ZO::Disabled);
        assert!(d.settlement_execution_artifact.is_none());
        assert!(!d.is_accept());
    }
    ...
    fn reserved_production_kind_fails_closed() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = ProductionLiveEpochTransitionSettlementExecutionExecutor::new(
            ProductionLiveEpochTransitionSettlementExecutionConfig::new(
                ProductionLiveEpochTransitionSettlementExecutionExecutorKind::ProductionLiveEpochTransitionSettlementExecution,
            ),
            ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::AllowSourceTestLiveEpochTransitionSettlementExecution,
        );
        let d = exec.evaluate_live_epoch_transition_settlement_execution(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            ZO::LiveEpochTransitionSettlementExecutionBoundaryUnavailable
        );
    }
    ...
    fn replay_rejected_when_id_present() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = z_eval(&c);
        let id = d.settlement_execution_artifact.as_ref().unwrap().request_id.clone();
        let replay = vec![id];
        let d2 = z_eval_replay(&c, &replay);
        match d2.outcome {
            ZO::StagedApplicationReplayRejected { .. } => {}
            other => panic!("expected replay rejection, got {other:?}"),
        }
        assert!(d2.settlement_execution_artifact.is_none());
    }

    fn no_replay_when_id_absent() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let replay: Vec<String> = vec!["some-other-id".to_string()];
        let d = z_eval_replay(&c, &replay);
        assert!(d.is_accept());
    }

    fn stale_governance_epoch() {
        z_reject_inputs(|i| i.min_governance_epoch = u64::MAX, ZO::StaleGovernanceEpoch);
    }

    fn stale_authority_sequence() {
        z_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), ZO::StaleAuthoritySequence);
    }

    fn stale_validator_set_epoch() {
        z_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, ZO::StaleValidatorSetEpoch);
    }

    fn stale_validator_set_version() {
        z_reject_inputs(|i| i.min_validator_set_version = u64::MAX, ZO::StaleValidatorSetVersion);
    }
    ...
    fn fixture_state_apply_is_idempotent() {
        ...
        let d = z_eval(&c);
        let art = d.settlement_execution_artifact.as_ref().unwrap();
        let mut state = LiveEpochTransitionSettlementExecutionFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
        assert!(state.apply_prepared_execution(art, &d.settlement_execution_id));
        assert_eq!(state.current_epoch, art.epoch_transition_target);
        assert_eq!(state.validator_set_version, art.validator_set_version);
        assert_eq!(state.current_set_digest, art.proposed_set_digest);
        // Re-applying the same id is a no-op.
        assert!(!state.apply_prepared_execution(art, &d.settlement_execution_id));
        assert!(state.has_applied(&d.settlement_execution_id));
    }
    ...
    fn every_outcome_is_non_mutating() {
        ...
        let d = z_eval(&c);
        assert!(d.outcome.is_non_mutating());
        let bad = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let dbad = z_eval_replay(&bad, &[z_eval(&bad).settlement_execution_id]);
        assert!(dbad.outcome.is_non_mutating());
    }
    ...
    fn accept_authorizes_future_mutation_only() {
        ...
        assert!(d.outcome.authorizes_future_mutation_only());
        assert!(d.authorizes_future_mutation_only());
        assert!(d.settlement_execution_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
    }
    ...
    fn policy_predicates() {
        use ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy as Pol;
        assert!(Pol::Disabled.is_disabled());
        assert!(Pol::AllowSourceTestLiveEpochTransitionSettlementExecution.allows_source_test());
        assert!(Pol::RequireProductionLiveEpochTransitionSettlementExecution.is_production());
        assert!(Pol::MainnetProductionLiveEpochTransitionSettlementExecutionRequired.is_mainnet());
        assert_eq!(Pol::default(), Pol::Disabled);
    }

    fn kind_predicates() {
        use ProductionLiveEpochTransitionSettlementExecutionExecutorKind as K;
        assert!(K::SourceTestLiveEpochTransitionSettlementExecution.is_source_test());
        assert!(!K::Disabled.is_source_test());
        assert!(!K::ProductionLiveEpochTransitionSettlementExecution.is_source_test());
        assert_eq!(K::default(), K::Disabled);
    }

    fn execution_kind_mapping_matches_handoff_kind() {
        for sc in ALL_SC {
            let hk = expected_crc7_kind(sc);
            let pk = ZK::from_staged_application_kind(hk);
            assert_eq!(pk, expected_crc8_kind(sc));
            assert!(pk.is_non_mutating());
            assert!(!pk.is_unsupported());
        }
    }
    ...
    fn unsupported_staged_application_kind_is_unsupported() {
        let pk = ZK::from_staged_application_kind(LiveEpochTransitionSettlementExecutionPreparationKind::UnsupportedStagedApplication);
        assert!(pk.is_unsupported());
    }

    fn outcome_tags_are_stable_and_distinct() {
        let a = ZO::SettlementExecutionPreparationDecisionIdMismatch;
        let b = ZO::GuardedMutationDecisionIdMismatch;
        assert_ne!(a.tag(), b.tag());
        assert_eq!(a.tag(), ZO::SettlementExecutionPreparationDecisionIdMismatch.tag());
        assert!(!ZO::MainNetRefused.tag().is_empty());
    }

    fn config_and_inputs_well_formed() {
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        assert!(c.inputs.is_well_formed());
        assert!(ProductionLiveEpochTransitionSettlementExecutionConfig::source_test().is_well_formed());
        // The default config still pins the supported protocol version.
        assert!(ProductionLiveEpochTransitionSettlementExecutionConfig::default().is_well_formed());
    }
    ...
    macro_rules! per_scenario_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(art.staged_kind.is_non_mutating());
                let d2 = z_eval(&c);
                assert_eq!(d.settlement_execution_digest, d2.settlement_execution_digest);
                assert_eq!(d.transcript_digest, d2.transcript_digest);
                let mut state = LiveEpochTransitionSettlementExecutionFixtureState::new(
                    CUR_EPOCH,
                    CUR_VERSION,
                    "start",
                );
    ...
    macro_rules! per_scenario_testnet_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(d.is_accept());
                assert_eq!(
                    d.settlement_execution_artifact.as_ref().unwrap().environment,
                    TrustBundleEnvironment::Testnet
                );
            }
        };
    ...
    macro_rules! per_scenario_settlement_execution_preparation_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                let mut c = z_case(TrustBundleEnvironment::Devnet, $sc);
                c.inputs.expected_settlement_execution_preparation_decision_id = "bad".to_string();
                let d = z_eval(&c);
                assert_eq!(d.outcome, ZO::SettlementExecutionPreparationDecisionIdMismatch);
                assert!(d.settlement_execution_artifact.is_none());
            }
        };
    }

    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_add, Sc::Add);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_remove, Sc::Remove);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_update, Sc::Update);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_noop, Sc::NoOp);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_identity, Sc::Identity);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_retire, Sc::Retire);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
    per_scenario_settlement_execution_preparation_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
    ...
    macro_rules! per_scenario_guarded_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                c.inputs.expected_guarded_mutation_decision_id = "bad".to_string();
                let d = z_eval(&c);
                assert_eq!(d.outcome, ZO::GuardedMutationDecisionIdMismatch);
                assert!(d.settlement_execution_artifact.is_none());
            }
        };
    }
    ...
    fn accept_reexposes_runtime_handoff_tuple_from_parent() {
        let parent = cz_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let ppkg = parent.settlement_execution_preparation_artifact.as_ref().unwrap().clone();
        let c = z_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = z_eval(&c);
        assert!(d.is_accept());
        let art = d.settlement_execution_artifact.as_ref().unwrap();
    ...
    fn reject_runtime_handoff_decision_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_runtime_handoff_decision_id = "wrong-runtime-handoff-id".to_string(),
            ZO::RuntimeHandoffDecisionIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_request_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_runtime_handoff_request_id = "wrong-runtime-handoff-req".to_string(),
            ZO::RuntimeHandoffDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_intent_digest_mismatch() {
        z_reject_inputs(
            |i| i.expected_runtime_handoff_intent_digest = "wrong-runtime-handoff-digest".to_string(),
            ZO::RuntimeHandoffDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_transcript_mismatch() {
        z_reject_inputs(
            ...
            ZO::RuntimeHandoffDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_runtime_handoff_nonce() {
        z_reject_inputs(
            |i| i.expected_runtime_handoff_nonce = HANDOFF_NONCE + 100,
            ZO::WrongRuntimeHandoffNonce,
        );
    }

    fn reject_runtime_handoff_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::RuntimeHandoffDecisionWithoutSettlementExecutionPreparation,
            ZO::RuntimeHandoffDecisionAloneRejected,
        );
    }

    fn runtime_handoff_alone_tag_is_stable() {
        assert_eq!(
            ZO::RuntimeHandoffDecisionAloneRejected.tag(),
            "runtime-handoff-decision-alone-rejected"
        );
        assert_eq!(
            ZO::RuntimeHandoffDecisionIdMismatch.tag(),
            "runtime-handoff-decision-id-mismatch"
        );
        assert_eq!(
            ZO::WrongRuntimeHandoffNonce.tag(),
            "wrong-runtime-handoff-nonce"
        );
    }
    ...
    fn accept_content_digest_binds_runtime_handoff_tuple() {
        ...
        let d1 = z_eval(&c);
        let d2 = z_eval(&c);
        let a1 = d1.settlement_execution_artifact.as_ref().unwrap();
        let a2 = d2.settlement_execution_artifact.as_ref().unwrap();
        assert_eq!(a1.content_digest(), a2.content_digest());
    }

    // ===========================================================================
    // Run 337 — re-exposed Run 329/330 execution-preparation ancestor binding
    // (carried through the consumed Run 335/336 settlement-execution-preparation artifact) and
    // its alone-rejected authority variant.
    // ===========================================================================
    ...
    fn reject_mutation_execution_decision_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_mutation_execution_decision_id = "wrong-mut-exec-id".to_string(),
            ZO::MutationExecutionDecisionIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_request_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_mutation_execution_request_id = "wrong-mut-exec-req".to_string(),
            ZO::MutationExecutionDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_intent_digest_mismatch() {
        z_reject_inputs(
            |i| i.expected_mutation_execution_intent_digest = "wrong-mut-exec-digest".to_string(),
            ZO::MutationExecutionDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_mutation_execution_transcript_mismatch() {
        z_reject_inputs(
            ...
            ZO::MutationExecutionDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_mutation_execution_nonce() {
        z_reject_inputs(
            |i| i.expected_mutation_execution_nonce = MUT_NONCE + 100,
            ZO::WrongMutationExecutionNonce,
        );
    }

    fn reject_mutation_execution_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::MutationExecutionDecisionWithoutSettlementExecutionPreparation,
            ZO::MutationExecutionDecisionAloneRejected,
        );
    }
    ...
    fn reject_commit_authorization_decision_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_commit_authorization_decision_id = "wrong-commit-auth-id".to_string(),
            ZO::CommitAuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_request_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_commit_authorization_request_id = "wrong-commit-auth-req".to_string(),
            ZO::CommitAuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_intent_digest_mismatch() {
        z_reject_inputs(
            ...
            ZO::CommitAuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_commit_authorization_transcript_mismatch() {
        z_reject_inputs(
            ...
            ZO::CommitAuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_commit_authorization_nonce() {
        z_reject_inputs(
            |i| i.expected_commit_authorization_nonce = CMT_NONCE + 100,
            ZO::WrongCommitAuthorizationNonce,
        );
    }

    fn reject_commit_authorization_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::CommitAuthorizationDecisionWithoutSettlementExecutionPreparation,
            ZO::CommitAuthorizationDecisionAloneRejected,
        );
    }
    ...
    fn mutation_execution_alone_tag_is_stable() {
        assert_eq!(
            ZO::MutationExecutionDecisionAloneRejected.tag(),
            "mutation-execution-decision-alone-rejected"
        );
        assert_eq!(
            ZO::MutationExecutionDecisionIdMismatch.tag(),
            "mutation-execution-decision-id-mismatch"
        );
        assert_eq!(
            ZO::WrongMutationExecutionNonce.tag(),
            "wrong-mutation-execution-nonce"
        );
    }
    ...
    fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
        ...
        assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
        // Parent Run 335/336 settlement-execution-preparation nonce re-exposed intact.
        assert_eq!(art.settlement_execution_preparation_nonce, CRC4_NONCE);
        // Self Run 337 settlement-execution proposed nonce.
        assert_eq!(art.settlement_execution_nonce, CRC8_NONCE);
    }
    ...
    fn reject_execution_preparation_decision_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_execution_preparation_decision_id = "wrong-exec-prep-id".to_string(),
            ZO::ExecutionPreparationDecisionIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_request_id_mismatch() {
        z_reject_inputs(
            |i| i.expected_execution_preparation_request_id = "wrong-exec-prep-req".to_string(),
            ZO::ExecutionPreparationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_intent_digest_mismatch() {
        z_reject_inputs(
            |i| i.expected_execution_preparation_intent_digest = "wrong-exec-prep-digest".to_string(),
            ZO::ExecutionPreparationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_execution_preparation_transcript_mismatch() {
        z_reject_inputs(
            ...
            ZO::ExecutionPreparationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_execution_preparation_nonce() {
        z_reject_inputs(
            |i| i.expected_execution_preparation_nonce = PREP_NONCE + 100,
            ZO::WrongExecutionPreparationNonce,
        );
    }

    fn reject_execution_preparation_decision_alone() {
        z_reject_source(
            LiveEpochTransitionSettlementExecutionAuthoritySource::ExecutionPreparationDecisionWithoutSettlementExecutionPreparation,
            ZO::ExecutionPreparationDecisionAloneRejected,
        );
    }

    fn execution_preparation_alone_tag_is_stable() {
        assert_eq!(
            ZO::ExecutionPreparationDecisionAloneRejected.tag(),
            "execution-preparation-decision-alone-rejected"
        );
        assert_eq!(
            ZO::ExecutionPreparationDecisionIdMismatch.tag(),
            "execution-preparation-decision-id-mismatch"
        );
        assert_eq!(
            ZO::WrongExecutionPreparationNonce.tag(),
            "wrong-execution-preparation-nonce"
        );
    }
...
// ===========================================================================
// Run 341 — live epoch-transition final settlement / authority-lifecycle
// completion boundary integration tests.
//
// The full Run 301 -> ... -> 339 chain above produces a verified, accepted
// Run 339 live epoch-transition settlement / finalization execution decision
// (the `a_`/`z_` builder layer below). That decision is fed into the Run 341
// final-settlement / authority-lifecycle completion executor. Run 341 produces
// only a typed, non-mutating final-settlement / authority-lifecycle completion
// artifact and never applies a live validator-set change, transitions an epoch,
// or writes any production record.
// ===========================================================================
mod run_341_final_settlement_completion {
    #![allow(unused_imports)]
    #![allow(dead_code)]
    use super::*;
    ...
    // ===========================================================================
    // A. Accepted / compatible source-test settlement-execution artifacts
    // ===========================================================================

    fn accept_all_scenarios_devnet() {
        for sc in ALL_SC {
            let c = a_case(TrustBundleEnvironment::Devnet, sc);
            let d = a_eval(&c);
            assert!(d.is_accept(), "scenario must accept");
            assert!(d.authorizes_future_mutation_only());
            let art = d.final_settlement_completion_artifact.as_ref().unwrap();
            assert_eq!(art.staged_kind, expected_crc9_kind(sc));
            assert_eq!(art.final_settlement_completion_nonce, CRC9_NONCE);
            assert_eq!(art.settlement_execution_nonce, CRC4_NONCE);
            assert_eq!(art.guarded_mutation_nonce, GUARDED_NONCE);
            assert_eq!(art.staged_application_nonce, STAGED_NONCE);
        }
    }
    ...
    fn accept_all_scenarios_testnet() {
        for sc in ALL_SC {
            let c = a_case(TrustBundleEnvironment::Testnet, sc);
            let d = a_eval(&c);
            assert!(d.is_accept());
            let art = d.final_settlement_completion_artifact.as_ref().unwrap();
            assert_eq!(art.environment, TrustBundleEnvironment::Testnet);
    ...
    fn accept_outcome_carries_kind_env_target_nonce() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = a_eval(&c);
        match d.outcome {
            AO::AcceptedSourceTestLiveEpochTransitionFinalSettlementCompletion {
                execution_kind,
                environment,
                epoch_transition_target,
                final_settlement_completion_nonce,
            } => {
                assert_eq!(execution_kind, AK::StageApplyValidatorAdd);
                assert_eq!(environment, TrustBundleEnvironment::Devnet);
                assert_eq!(epoch_transition_target, c.request.proposed_epoch_transition_target);
                assert_eq!(final_settlement_completion_nonce, CRC9_NONCE);
            }
            other => panic!("unexpected outcome: {other:?}"),
        }
    }

    fn accept_artifact_reexposes_consumed_settlement_execution_transcript() {
        let dec = ca_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.settlement_execution_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = a_inputs(TrustBundleEnvironment::Devnet, &dec);
        let handoff_id = dec.settlement_execution_id.clone();
        let handoff_req = dec.request_id.clone();
        let handoff_digest = dec.settlement_execution_digest.clone();
        let handoff_transcript = dec.transcript_digest.clone();
        let request = ProductionLiveEpochTransitionFinalSettlementCompletionRequest::new(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::VerifiedSettlementExecutionDecision {
                decision: dec,
        ...
        let exec = ProductionLiveEpochTransitionFinalSettlementCompletionExecutor::source_test();
        let d = exec.evaluate_live_epoch_transition_final_settlement_completion(
            &request,
            &inputs,
            &empty_replay341(),
        );
        let art = d.final_settlement_completion_artifact.as_ref().unwrap();
        assert_eq!(art.settlement_execution_decision_id, handoff_id);
        assert_eq!(art.settlement_execution_request_id, handoff_req);
        assert_eq!(art.settlement_execution_intent_digest, handoff_digest);
        assert_eq!(art.settlement_execution_transcript_digest, handoff_transcript);
    }
    ...
    fn accept_decision_ids_match_artifact_ids() {
        ...
        let d = a_eval(&c);
        let art = d.final_settlement_completion_artifact.as_ref().unwrap();
        assert_eq!(d.final_settlement_completion_id, art.final_settlement_completion_id);
        assert_eq!(d.request_id, art.request_id);
        assert_eq!(d.final_settlement_completion_digest, art.final_settlement_completion_digest);
        assert_eq!(d.transcript_digest, art.transcript_digest);
        assert!(!d.final_settlement_completion_id.is_empty());
        assert!(!d.request_id.is_empty());
        assert!(!d.final_settlement_completion_digest.is_empty());
        assert!(!d.transcript_digest.is_empty());
    }
    ...
    fn deterministic_digests_under_reevaluation() {
        for sc in ALL_SC {
            ...
            let d1 = a_eval(&c);
            let d2 = a_eval(&c);
            assert_eq!(d1.final_settlement_completion_id, d2.final_settlement_completion_id);
            assert_eq!(d1.request_id, d2.request_id);
            assert_eq!(d1.final_settlement_completion_digest, d2.final_settlement_completion_digest);
            assert_eq!(d1.transcript_digest, d2.transcript_digest);
        }
    }

    fn artifact_content_digest_is_stable() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
        let d = a_eval(&c);
        let art = d.final_settlement_completion_artifact.as_ref().unwrap();
        assert_eq!(art.content_digest(), art.final_settlement_completion_digest);
        assert_eq!(art.content_digest(), art.content_digest());
    }
    ...
    fn wrong_settlement_execution_decision_id() {
        a_reject_inputs(
            |i| i.expected_settlement_execution_decision_id = "bad".to_string(),
            AO::SettlementExecutionDecisionIdMismatch,
        );
    }

    fn wrong_settlement_execution_request_id() {
        a_reject_inputs(
            |i| i.expected_settlement_execution_request_id = "bad".to_string(),
            AO::SettlementExecutionDecisionRequestIdMismatch,
        );
    }

    fn wrong_settlement_execution_intent_digest() {
        a_reject_inputs(
            |i| i.expected_settlement_execution_intent_digest = "bad".to_string(),
            AO::SettlementExecutionDecisionIntentDigestMismatch,
        );
    }

    fn wrong_settlement_execution_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_settlement_execution_transcript_digest = "bad".to_string(),
            AO::SettlementExecutionDecisionTranscriptMismatch,
        );
    }

    fn wrong_settlement_execution_nonce() {
        a_reject_inputs(
            |i| i.expected_settlement_execution_nonce = 9999,
            AO::WrongSettlementExecutionNonce,
        );
    }

    fn tampered_settlement_execution_package_integrity_mismatch() {
        // Mutate the consumed package so its content digest no longer matches the
        // bound handoff decision digest.
        let mut dec = ca_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.settlement_execution_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = a_inputs(TrustBundleEnvironment::Devnet, &dec);
        dec.settlement_execution_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
        let request = ProductionLiveEpochTransitionFinalSettlementCompletionRequest::new(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::VerifiedSettlementExecutionDecision {
                decision: dec,
            },
        ...
        assert_eq!(d.outcome, AO::SettlementExecutionDecisionIntegrityMismatch);
        assert!(d.final_settlement_completion_artifact.is_none());
    }
    ...
    fn wrong_guarded_mutation_decision_id() {
        a_reject_inputs(
            |i| i.expected_guarded_mutation_decision_id = "bad".to_string(),
            AO::GuardedMutationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_request_id() {
        a_reject_inputs(
            |i| i.expected_guarded_mutation_request_id = "bad".to_string(),
            AO::GuardedMutationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_intent_digest() {
        a_reject_inputs(
            |i| i.expected_guarded_mutation_intent_digest = "bad".to_string(),
            AO::GuardedMutationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_guarded_mutation_transcript_digest = "bad".to_string(),
            AO::GuardedMutationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_decision_id() {
        a_reject_inputs(
            |i| i.expected_staged_application_decision_id = "bad".to_string(),
            AO::StagedApplicationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_request_id() {
        a_reject_inputs(
            |i| i.expected_staged_application_request_id = "bad".to_string(),
            AO::StagedApplicationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_intent_digest() {
        a_reject_inputs(
            |i| i.expected_staged_application_intent_digest = "bad".to_string(),
            AO::StagedApplicationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_staged_application_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_staged_application_transcript_digest = "bad".to_string(),
            AO::StagedApplicationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_nonce() {
        a_reject_inputs(
            |i| i.expected_staged_application_nonce = 9999,
            AO::WrongStagedApplicationNonce,
        );
    }
    ...
    fn wrong_authorization_decision_id() {
        a_reject_inputs(
            |i| i.expected_authorization_decision_id = "bad".to_string(),
            AO::AuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_authorization_request_id() {
        a_reject_inputs(
            |i| i.expected_authorization_request_id = "bad".to_string(),
            AO::AuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_authorization_intent_digest() {
        a_reject_inputs(
            |i| i.expected_authorization_intent_digest = "bad".to_string(),
            AO::AuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_authorization_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_authorization_transcript_digest = "bad".to_string(),
            AO::AuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_application_intent_digest() {
        a_reject_inputs(
            |i| i.expected_application_intent_digest = "bad".to_string(),
            AO::WrongApplicationIntentDigest,
        );
    }
    ...
    fn wrong_application_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_application_transcript_digest = "bad".to_string(),
            AO::WrongApplicationTranscriptDigest,
        );
    }
    ...
    fn wrong_governance_domain() {
        a_reject_inputs(
            |i| i.expected_governance_domain_id = "bad".to_string(),
            AO::WrongGovernanceDomain,
        );
    }

    fn wrong_governance_epoch() {
        a_reject_inputs(|i| i.expected_governance_epoch = 999, AO::WrongGovernanceEpoch);
    }

    fn wrong_proposal_id() {
        a_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), AO::WrongProposalId);
    }

    fn wrong_governance_execution_intent_digest() {
        a_reject_inputs(
            |i| i.expected_governance_intent_digest = "bad".to_string(),
            AO::WrongGovernanceExecutionIntentDigest,
        );
    }
    ...
    fn wrong_rotation_transcript_digest() {
        a_reject_inputs(
            |i| i.expected_rotation_transcript_digest = "bad".to_string(),
            AO::WrongRotationTranscriptDigest,
        );
    }
    ...
    fn wrong_rotation_action() {
        a_reject_inputs(
            |i| i.expected_rotation_action = ValidatorSetRotationAction::EmergencyValidatorRemoval,
            AO::WrongRotationAction,
        );
    }

    fn wrong_authority_sequence() {
        a_reject_inputs(|i| i.expected_authority_domain_sequence = 999, AO::WrongAuthoritySequence);
    }
    ...
    fn wrong_current_validator_set_digest() {
        a_reject_inputs(
            |i| i.expected_current_set_digest = "bad".to_string(),
            AO::WrongCurrentValidatorSetDigest,
        );
    }
    ...
    fn wrong_proposed_validator_set_digest() {
        a_reject_inputs(
            |i| i.expected_proposed_set_digest = "bad".to_string(),
            AO::WrongProposedValidatorSetDigest,
        );
    }
    ...
    fn wrong_validator_set_delta_digest() {
        a_reject_inputs(
            |i| i.expected_delta_digest = "bad".to_string(),
            AO::WrongValidatorSetDeltaDigest,
        );
    }

    fn wrong_validator_set_epoch() {
        a_reject_inputs(|i| i.expected_validator_set_epoch = 999, AO::WrongValidatorSetEpoch);
    }

    fn wrong_validator_set_version() {
        a_reject_inputs(|i| i.expected_validator_set_version = 999, AO::WrongValidatorSetVersion);
    }

    fn wrong_current_validator_set_epoch() {
        a_reject_inputs(
            |i| i.expected_current_validator_set_epoch = 999,
            AO::WrongCurrentValidatorSetEpoch,
        );
    }
    ...
    fn wrong_current_validator_set_version() {
        a_reject_inputs(
            |i| i.expected_current_validator_set_version = 999,
            AO::WrongCurrentValidatorSetVersion,
        );
    }

    fn wrong_proposed_validator_count() {
        a_reject_inputs(|i| i.expected_proposed_validator_count = 999, AO::WrongProposedValidatorCount);
    }

    fn wrong_rotation_nonce() {
        a_reject_inputs(|i| i.expected_rotation_nonce = 999, AO::WrongRotationNonce);
    }
    ...
    fn wrong_epoch_transition_target_inputs() {
        a_reject_inputs(|i| i.expected_epoch_transition_target = 999, AO::WrongEpochTransitionTarget);
    }
    ...
    fn wrong_epoch_transition_target_request() {
        ...
        c.request.proposed_epoch_transition_target = 4242;
        let d = a_eval(&c);
        assert_eq!(d.outcome, AO::WrongEpochTransitionTarget);
        assert!(d.final_settlement_completion_artifact.is_none());
    }

    fn wrong_application_nonce() {
        a_reject_inputs(|i| i.expected_application_nonce = 999, AO::WrongApplicationNonce);
    }

    fn wrong_live_application_nonce() {
        a_reject_inputs(|i| i.expected_live_application_nonce = 999, AO::WrongLiveApplicationNonce);
    }
    ...
    fn reject_missing_settlement_execution_decision() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::MissingSettlementExecutionDecision,
            AO::VerifiedSettlementExecutionDecisionRequired,
        );
    }

    fn reject_unverified_settlement_execution_decision() {
        let dec = ca_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::UnverifiedSettlementExecutionDecision {
                decision: dec,
            },
            AO::UnverifiedSettlementExecutionDecisionRejected,
        );
    }
    ...
    fn reject_accepted_settlement_execution_without_package_via_verified_source() {
        let dec = ca_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::VerifiedSettlementExecutionDecision {
                decision: dec,
            },
    ...
    fn reject_accepted_settlement_execution_without_package_variant() {
        let dec = ca_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::AcceptedSettlementExecutionWithoutPackage {
                decision: dec,
            },
            AO::VerifiedSettlementExecutionDecisionRequired,
        );
    ...
    fn reject_guarded_mutation_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::GuardedMutationDecisionWithoutSettlementExecution,
            AO::GuardedMutationDecisionAloneRejected,
        );
    }

    fn reject_staged_application_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::StagedApplicationDecisionWithoutSettlementExecution,
            AO::StagedApplicationDecisionAloneRejected,
        );
    }

    fn reject_live_application_authorization_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::LiveApplicationAuthorizationWithoutSettlementExecution,
            AO::LiveApplicationAuthorizationAloneRejected,
        );
    }

    fn reject_application_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::ApplicationDecisionWithoutSettlementExecution,
            AO::ApplicationDecisionAloneRejected,
        );
    }

    fn reject_rotation_plan_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::RotationPlanWithoutSettlementExecution,
            AO::RotationPlanAloneRejected,
        );
    }

    fn reject_governance_execution_intent_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::GovernanceExecutionIntentWithoutSettlementExecution,
            AO::GovernanceExecutionIntentAloneRejected,
        );
    }

    fn reject_governance_proof_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::GovernanceProofWithoutSettlementExecution,
            AO::GovernanceProofAloneRejected,
        );
    }

    fn reject_local_operator_assertion() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::LocalOperatorAssertion,
            AO::LocalOperatorProofRejected,
        );
    }

    fn reject_peer_majority_assertion() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::PeerMajorityAssertion,
            AO::PeerMajorityProofRejected,
        );
    }

    fn reject_custody_only_evidence() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::CustodyOnlyEvidence,
            AO::CustodyOnlyProofRejected,
        );
    }

    fn reject_remote_signer_only_evidence() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::RemoteSignerOnlyEvidence,
            AO::RemoteSignerOnlyProofRejected,
        );
    }

    fn reject_custody_attestation_only_evidence() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::CustodyAttestationOnlyEvidence,
            AO::CustodyAttestationOnlyProofRejected,
        );
    }

    fn reject_fixture_only_settlement_execution() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::FixtureOnlySettlementExecution,
            AO::FixtureStagedApplicationRejectedAsProductionAuthority,
        );
    }

    fn reject_arbitrary_validator_set_bytes() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::ArbitraryValidatorSetBytes,
            AO::ArbitraryValidatorSetBytesRejected,
        );
    }
    ...
    fn mainnet_domain_refused_under_source_test_policy() {
        ...
        c.inputs.trust_domain = trust_domain(TrustBundleEnvironment::Mainnet);
        let d = a_eval(&c);
        assert_eq!(d.outcome, AO::MainNetRefused);
        assert!(d.final_settlement_completion_artifact.is_none());
    }

    fn mainnet_policy_unavailable() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = a_exec_with_policy(
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::MainnetProductionLiveEpochTransitionFinalSettlementCompletionRequired,
        );
        let d = exec.evaluate_live_epoch_transition_final_settlement_completion(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            AO::MainNetProductionLiveEpochTransitionFinalSettlementCompletionUnavailable
        );
        assert!(d.final_settlement_completion_artifact.is_none());
    }
    ...
    fn production_policy_unavailable() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = a_exec_with_policy(
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::RequireProductionLiveEpochTransitionFinalSettlementCompletion,
        );
        let d = exec.evaluate_live_epoch_transition_final_settlement_completion(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            AO::ProductionLiveEpochTransitionFinalSettlementCompletionUnavailable
        );
        assert!(d.final_settlement_completion_artifact.is_none());
    }
    ...
    fn disabled_policy_fails_closed() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = a_exec_with_policy(
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::Disabled,
        );
        let d = exec.evaluate_live_epoch_transition_final_settlement_completion(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(d.outcome, AO::Disabled);
        assert!(d.final_settlement_completion_artifact.is_none());
        assert!(!d.is_accept());
    }
    ...
    fn reserved_production_kind_fails_closed() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = ProductionLiveEpochTransitionFinalSettlementCompletionExecutor::new(
            ProductionLiveEpochTransitionFinalSettlementCompletionConfig::new(
                ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::ProductionLiveEpochTransitionFinalSettlementCompletion,
            ),
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::AllowSourceTestLiveEpochTransitionFinalSettlementCompletion,
        );
        let d = exec.evaluate_live_epoch_transition_final_settlement_completion(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            AO::LiveEpochTransitionFinalSettlementCompletionBoundaryUnavailable
        );
    }
    ...
    fn replay_rejected_when_id_present() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = a_eval(&c);
        let id = d.final_settlement_completion_artifact.as_ref().unwrap().request_id.clone();
        let replay = vec![id];
        let d2 = a_eval_replay(&c, &replay);
        match d2.outcome {
            AO::StagedApplicationReplayRejected { .. } => {}
            other => panic!("expected replay rejection, got {other:?}"),
        }
        assert!(d2.final_settlement_completion_artifact.is_none());
    }

    fn no_replay_when_id_absent() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let replay: Vec<String> = vec!["some-other-id".to_string()];
        let d = a_eval_replay(&c, &replay);
        assert!(d.is_accept());
    }

    fn stale_governance_epoch() {
        a_reject_inputs(|i| i.min_governance_epoch = u64::MAX, AO::StaleGovernanceEpoch);
    }

    fn stale_authority_sequence() {
        a_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), AO::StaleAuthoritySequence);
    }

    fn stale_validator_set_epoch() {
        a_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, AO::StaleValidatorSetEpoch);
    }

    fn stale_validator_set_version() {
        a_reject_inputs(|i| i.min_validator_set_version = u64::MAX, AO::StaleValidatorSetVersion);
    }
    ...
    fn fixture_state_apply_is_idempotent() {
        ...
        let d = a_eval(&c);
        let art = d.final_settlement_completion_artifact.as_ref().unwrap();
        let mut state = LiveEpochTransitionFinalSettlementCompletionFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
        assert!(state.apply_prepared_execution(art, &d.final_settlement_completion_id));
        assert_eq!(state.current_epoch, art.epoch_transition_target);
        assert_eq!(state.validator_set_version, art.validator_set_version);
        assert_eq!(state.current_set_digest, art.proposed_set_digest);
        // Re-applying the same id is a no-op.
        assert!(!state.apply_prepared_execution(art, &d.final_settlement_completion_id));
        assert!(state.has_applied(&d.final_settlement_completion_id));
    }
    ...
    fn every_outcome_is_non_mutating() {
        ...
        let d = a_eval(&c);
        assert!(d.outcome.is_non_mutating());
        let bad = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let dbad = a_eval_replay(&bad, &[a_eval(&bad).final_settlement_completion_id]);
        assert!(dbad.outcome.is_non_mutating());
    }
    ...
    fn accept_authorizes_future_mutation_only() {
        ...
        assert!(d.outcome.authorizes_future_mutation_only());
        assert!(d.authorizes_future_mutation_only());
        assert!(d.final_settlement_completion_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
    }
    ...
    fn policy_predicates() {
        use ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy as Pol;
        assert!(Pol::Disabled.is_disabled());
        assert!(Pol::AllowSourceTestLiveEpochTransitionFinalSettlementCompletion.allows_source_test());
        assert!(Pol::RequireProductionLiveEpochTransitionFinalSettlementCompletion.is_production());
        assert!(Pol::MainnetProductionLiveEpochTransitionFinalSettlementCompletionRequired.is_mainnet());
        assert_eq!(Pol::default(), Pol::Disabled);
    }

    fn kind_predicates() {
        use ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind as K;
        assert!(K::SourceTestLiveEpochTransitionFinalSettlementCompletion.is_source_test());
        assert!(!K::Disabled.is_source_test());
        assert!(!K::ProductionLiveEpochTransitionFinalSettlementCompletion.is_source_test());
        assert_eq!(K::default(), K::Disabled);
    }

    fn execution_kind_mapping_matches_handoff_kind() {
        for sc in ALL_SC {
            let hk = expected_crc8_kind(sc);
            let pk = AK::from_staged_application_kind(hk);
            assert_eq!(pk, expected_crc9_kind(sc));
            assert!(pk.is_non_mutating());
            assert!(!pk.is_unsupported());
        }
    }
    ...
    fn unsupported_staged_application_kind_is_unsupported() {
        let pk = AK::from_staged_application_kind(LiveEpochTransitionSettlementExecutionKind::UnsupportedStagedApplication);
        assert!(pk.is_unsupported());
    }

    fn outcome_tags_are_stable_and_distinct() {
        let a = AO::SettlementExecutionDecisionIdMismatch;
        let b = AO::GuardedMutationDecisionIdMismatch;
        assert_ne!(a.tag(), b.tag());
        assert_eq!(a.tag(), AO::SettlementExecutionDecisionIdMismatch.tag());
        assert!(!AO::MainNetRefused.tag().is_empty());
    }

    fn config_and_inputs_well_formed() {
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        assert!(c.inputs.is_well_formed());
        assert!(ProductionLiveEpochTransitionFinalSettlementCompletionConfig::source_test().is_well_formed());
        // The default config still pins the supported protocol version.
        assert!(ProductionLiveEpochTransitionFinalSettlementCompletionConfig::default().is_well_formed());
    }
    ...
    macro_rules! per_scenario_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(art.staged_kind.is_non_mutating());
                let d2 = a_eval(&c);
                assert_eq!(d.final_settlement_completion_digest, d2.final_settlement_completion_digest);
                assert_eq!(d.transcript_digest, d2.transcript_digest);
                let mut state = LiveEpochTransitionFinalSettlementCompletionFixtureState::new(
                    CUR_EPOCH,
                    CUR_VERSION,
                    "start",
                );
    ...
    macro_rules! per_scenario_testnet_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(d.is_accept());
                assert_eq!(
                    d.final_settlement_completion_artifact.as_ref().unwrap().environment,
                    TrustBundleEnvironment::Testnet
                );
            }
        };
    ...
    macro_rules! per_scenario_settlement_execution_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                let mut c = a_case(TrustBundleEnvironment::Devnet, $sc);
                c.inputs.expected_settlement_execution_decision_id = "bad".to_string();
                let d = a_eval(&c);
                assert_eq!(d.outcome, AO::SettlementExecutionDecisionIdMismatch);
                assert!(d.final_settlement_completion_artifact.is_none());
            }
        };
    }

    per_scenario_settlement_execution_binding!(scenario_handoff_binding_add, Sc::Add);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_remove, Sc::Remove);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_update, Sc::Update);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_noop, Sc::NoOp);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_identity, Sc::Identity);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_retire, Sc::Retire);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
    per_scenario_settlement_execution_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
    ...
    macro_rules! per_scenario_guarded_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                c.inputs.expected_guarded_mutation_decision_id = "bad".to_string();
                let d = a_eval(&c);
                assert_eq!(d.outcome, AO::GuardedMutationDecisionIdMismatch);
                assert!(d.final_settlement_completion_artifact.is_none());
            }
        };
    }
    ...
    fn accept_reexposes_runtime_handoff_tuple_from_parent() {
        let parent = ca_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let ppkg = parent.settlement_execution_artifact.as_ref().unwrap().clone();
        let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = a_eval(&c);
        assert!(d.is_accept());
        let art = d.final_settlement_completion_artifact.as_ref().unwrap();
    ...
    fn reject_runtime_handoff_decision_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_runtime_handoff_decision_id = "wrong-runtime-handoff-id".to_string(),
            AO::RuntimeHandoffDecisionIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_request_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_runtime_handoff_request_id = "wrong-runtime-handoff-req".to_string(),
            AO::RuntimeHandoffDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_intent_digest_mismatch() {
        a_reject_inputs(
            |i| i.expected_runtime_handoff_intent_digest = "wrong-runtime-handoff-digest".to_string(),
            AO::RuntimeHandoffDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_transcript_mismatch() {
        a_reject_inputs(
            ...
            AO::RuntimeHandoffDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_runtime_handoff_nonce() {
        a_reject_inputs(
            |i| i.expected_runtime_handoff_nonce = HANDOFF_NONCE + 100,
            AO::WrongRuntimeHandoffNonce,
        );
    }

    fn reject_runtime_handoff_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::RuntimeHandoffDecisionWithoutSettlementExecution,
            AO::RuntimeHandoffDecisionAloneRejected,
        );
    }

    fn runtime_handoff_alone_tag_is_stable() {
        assert_eq!(
            AO::RuntimeHandoffDecisionAloneRejected.tag(),
            "runtime-handoff-decision-alone-rejected"
        );
        assert_eq!(
            AO::RuntimeHandoffDecisionIdMismatch.tag(),
            "runtime-handoff-decision-id-mismatch"
        );
        assert_eq!(
            AO::WrongRuntimeHandoffNonce.tag(),
            "wrong-runtime-handoff-nonce"
        );
    }
    ...
    fn accept_content_digest_binds_runtime_handoff_tuple() {
        ...
        let d1 = a_eval(&c);
        let d2 = a_eval(&c);
        let a1 = d1.final_settlement_completion_artifact.as_ref().unwrap();
        let a2 = d2.final_settlement_completion_artifact.as_ref().unwrap();
        assert_eq!(a1.content_digest(), a2.content_digest());
    }

    // ===========================================================================
    // Run 337 — re-exposed Run 329/330 execution-preparation ancestor binding
    // (carried through the consumed Run 335/336 settlement-execution artifact) and
    // its alone-rejected authority variant.
    // ===========================================================================
    ...
    fn reject_mutation_execution_decision_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_mutation_execution_decision_id = "wrong-mut-exec-id".to_string(),
            AO::MutationExecutionDecisionIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_request_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_mutation_execution_request_id = "wrong-mut-exec-req".to_string(),
            AO::MutationExecutionDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_intent_digest_mismatch() {
        a_reject_inputs(
            |i| i.expected_mutation_execution_intent_digest = "wrong-mut-exec-digest".to_string(),
            AO::MutationExecutionDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_mutation_execution_transcript_mismatch() {
        a_reject_inputs(
            ...
            AO::MutationExecutionDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_mutation_execution_nonce() {
        a_reject_inputs(
            |i| i.expected_mutation_execution_nonce = MUT_NONCE + 100,
            AO::WrongMutationExecutionNonce,
        );
    }

    fn reject_mutation_execution_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::MutationExecutionDecisionWithoutSettlementExecution,
            AO::MutationExecutionDecisionAloneRejected,
        );
    }
    ...
    fn reject_commit_authorization_decision_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_commit_authorization_decision_id = "wrong-commit-auth-id".to_string(),
            AO::CommitAuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_request_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_commit_authorization_request_id = "wrong-commit-auth-req".to_string(),
            AO::CommitAuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_intent_digest_mismatch() {
        a_reject_inputs(
            ...
            AO::CommitAuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_commit_authorization_transcript_mismatch() {
        a_reject_inputs(
            ...
            AO::CommitAuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_commit_authorization_nonce() {
        a_reject_inputs(
            |i| i.expected_commit_authorization_nonce = CMT_NONCE + 100,
            AO::WrongCommitAuthorizationNonce,
        );
    }

    fn reject_commit_authorization_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::CommitAuthorizationDecisionWithoutSettlementExecution,
            AO::CommitAuthorizationDecisionAloneRejected,
        );
    }
    ...
    fn mutation_execution_alone_tag_is_stable() {
        assert_eq!(
            AO::MutationExecutionDecisionAloneRejected.tag(),
            "mutation-execution-decision-alone-rejected"
        );
        assert_eq!(
            AO::MutationExecutionDecisionIdMismatch.tag(),
            "mutation-execution-decision-id-mismatch"
        );
        assert_eq!(
            AO::WrongMutationExecutionNonce.tag(),
            "wrong-mutation-execution-nonce"
        );
    }
    ...
    fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
        ...
        // Grandparent Run 329/330 mutation-execution nonce re-exposed intact.
        assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
        // Parent Run 335/336 settlement-execution nonce re-exposed intact.
        assert_eq!(art.settlement_execution_nonce, CRC4_NONCE);
        // Self Run 337 final-settlement-completion proposed nonce.
        assert_eq!(art.final_settlement_completion_nonce, CRC9_NONCE);
    }
    ...
    fn reject_execution_preparation_decision_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_execution_preparation_decision_id = "wrong-exec-prep-id".to_string(),
            AO::ExecutionPreparationDecisionIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_request_id_mismatch() {
        a_reject_inputs(
            |i| i.expected_execution_preparation_request_id = "wrong-exec-prep-req".to_string(),
            AO::ExecutionPreparationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_intent_digest_mismatch() {
        a_reject_inputs(
            |i| i.expected_execution_preparation_intent_digest = "wrong-exec-prep-digest".to_string(),
            AO::ExecutionPreparationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_execution_preparation_transcript_mismatch() {
        a_reject_inputs(
            ...
            AO::ExecutionPreparationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_execution_preparation_nonce() {
        a_reject_inputs(
            |i| i.expected_execution_preparation_nonce = PREP_NONCE + 100,
            AO::WrongExecutionPreparationNonce,
        );
    }

    fn reject_execution_preparation_decision_alone() {
        a_reject_source(
            LiveEpochTransitionFinalSettlementCompletionAuthoritySource::ExecutionPreparationDecisionWithoutSettlementExecution,
            AO::ExecutionPreparationDecisionAloneRejected,
        );
    }

    fn execution_preparation_alone_tag_is_stable() {
        assert_eq!(
            AO::ExecutionPreparationDecisionAloneRejected.tag(),
            "execution-preparation-decision-alone-rejected"
        );
        assert_eq!(
            AO::ExecutionPreparationDecisionIdMismatch.tag(),
            "execution-preparation-decision-id-mismatch"
        );
        assert_eq!(
            AO::WrongExecutionPreparationNonce.tag(),
            "wrong-execution-preparation-nonce"
        );
    }
...
// ===========================================================================
// Run 343 — live epoch-transition authority-lifecycle activation-readiness /
// post-completion attestation boundary integration tests.
//
// The full base chain above produces a verified, accepted Run 341 live
// epoch-transition final-settlement / authority-lifecycle completion decision
// (the z_/a_ builder layers below). That decision is fed into the Run 343
// post-completion attestation executor. Run 343 produces only a typed,
// non-mutating post-completion attestation artifact and never applies a live
// validator-set change, activates authority state, transitions an epoch, or
// writes any production record.
// ===========================================================================
mod run_347_authority_activation_execution_preparation {
    #![allow(unused_imports)]
    #![allow(dead_code)]
    #![allow(unused_variables)]
    use super::*;
    ...
    // ---- Run 345 (self) post-completion attestation layer ----
    ...
    use ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome as CO;
    use LiveEpochTransitionAuthorityActivationAuthorizationKind as CK;

    const CRC11_POLICY_ID: &str = "authority-activation-authorization-policy-1";
    ...
    fn c_inputs(
        ...
        ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs {
            trust_domain: trust_domain(env),
            authority_activation_authorization_policy_id: CRC11_POLICY_ID.to_string(),
            expected_authorization_policy_id: AUTH_POLICY_ID.to_string(),
            expected_application_policy_id: APP_POLICY_ID.to_string(),
            expected_governance_domain_id: GOV_DOMAIN.to_string(),
            expected_governance_epoch: GOV_EPOCH,
            ...
            expected_guarded_mutation_transcript_digest: pkg
                ...
                .clone(),
            expected_guarded_mutation_nonce: pkg.guarded_mutation_nonce,
            expected_post_completion_attestation_decision_id: dec.post_completion_attestation_id.clone(),
            expected_post_completion_attestation_request_id: dec.request_id.clone(),
            expected_post_completion_attestation_intent_digest: dec.post_completion_attestation_digest.clone(),
            expected_post_completion_attestation_transcript_digest: dec.transcript_digest.clone(),
            expected_post_completion_attestation_nonce: pkg.post_completion_attestation_nonce,
            expected_commit_authorization_decision_id: pkg.commit_authorization_decision_id.clone(),
            expected_commit_authorization_request_id: pkg.commit_authorization_request_id.clone(),
            expected_commit_authorization_intent_digest: pkg
                .commit_authorization_intent_digest
    ...
    fn c_case(env: TrustBundleEnvironment, sc: Sc) -> C345 {
    ...
    fn c_eval(case: &C345) -> ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
    ...
    // ===========================================================================
    // A. Accepted / compatible source-test post-completion-attestation artifacts
    // ===========================================================================


    // =======================================================================
    // Run 347 layer (module under test): consumes the verified Run 345/346
    // authority-activation-authorization decision and produces the typed,
    ...
    use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome as DO;
    use LiveEpochTransitionAuthorityActivationExecutionPreparationKind as DK;

    const CRC12_POLICY_ID: &str = "authority-activation-execution-preparation-policy-1";
    const CRC12_NONCE: u64 = 49;

    fn expected_crc12_kind(sc: Sc) -> DK {
        match sc {
            Sc::Add => DK::StageApplyValidatorAdd,
            Sc::Remove => DK::StageApplyValidatorRemove,
            Sc::Update => DK::StageApplyValidatorMetadataUpdate,
            Sc::NoOp => DK::StageApplyNoOpAlreadySynchronized,
            Sc::Identity => DK::StageApplyValidatorIdentityRotation,
            Sc::Retire => DK::StageApplyValidatorRetirement,
            Sc::Emergency => DK::StageApplyEmergencyValidatorRemoval,
            Sc::AuthSync => DK::StageApplyAuthoritySetSynchronization,
            Sc::Bulk => DK::StageApplyBulkValidatorSetRotation,
        }
    }

    /// Build an accepted Run 335/336 authority-activation-authorization decision — the sole
    ...
    fn dc_decision(
        ...
        assert!(
            d.is_accept(),
            "run 335 authority-activation-authorization decision must accept for fixture"
        );
        d
    }

    fn dc_decision_rejected(
        env: TrustBundleEnvironment,
        sc: Sc,
    ) -> ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
        let mut c = c_case(env, sc);
    ...
    fn dc_decision_no_artifact(
        env: TrustBundleEnvironment,
        sc: Sc,
    ) -> ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
        let mut d = dc_decision(env, sc);
    ...
    fn d_inputs(
        ...
        ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs {
            trust_domain: trust_domain(env),
            authority_activation_execution_preparation_policy_id: CRC12_POLICY_ID.to_string(),
            expected_authorization_policy_id: AUTH_POLICY_ID.to_string(),
            expected_application_policy_id: APP_POLICY_ID.to_string(),
            expected_governance_domain_id: GOV_DOMAIN.to_string(),
            expected_governance_epoch: GOV_EPOCH,
    ...
    fn d_case(env: TrustBundleEnvironment, sc: Sc) -> D347 {
        ...
        D347 {
            executor: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor::source_test(),
            request,
            inputs,
        }
    }

    fn empty_replay347() -> EmptyLiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet {
        EmptyLiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet
    }

    fn d_eval(case: &D347) -> ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision {
        case.executor.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &case.request,
            &case.inputs,
            &empty_replay347(),
        )
    ...
    fn d_eval_replay(
        ...
        case.executor
            .evaluate_live_epoch_transition_authority_activation_execution_preparation(&case.request, &case.inputs, &replay)
    }

    fn d_exec_with_policy(
        policy: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy,
    ) -> ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor {
        ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor::new(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig::source_test(),
            policy,
        )
    }
    ...
    /// assert the resulting outcome (fail-closed, no artifact).
    fn d_reject_inputs(
        mutate: impl FnOnce(&mut ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs),
        expected: DO,
    ) {
        ...
        let d = d_eval(&c);
        assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
        assert!(d.authority_activation_execution_preparation_artifact.is_none());
        assert!(!d.is_accept());
        assert!(d.outcome.is_non_mutating());
    }
    ...
    /// assert the resulting outcome.
    fn d_reject_source(
        source: LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource,
        expected: DO,
    ) {
        let mut c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        c.request.authority_source = source;
    ...
    // ===========================================================================
    // A. Accepted / compatible source-test authority-activation-authorization artifacts
    // ===========================================================================
    ...
    fn accept_all_scenarios_devnet() {
        for sc in ALL_SC {
            ...
            assert!(d.is_accept(), "scenario must accept");
            assert!(d.authorizes_future_mutation_only());
            let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
            assert_eq!(art.staged_kind, expected_crc12_kind(sc));
            assert_eq!(art.authority_activation_execution_preparation_nonce, CRC12_NONCE);
            assert_eq!(art.authority_activation_authorization_nonce, CRC4_NONCE);
            assert_eq!(art.guarded_mutation_nonce, GUARDED_NONCE);
            assert_eq!(art.staged_application_nonce, STAGED_NONCE);
        }
    }
    ...
    fn accept_outcome_carries_kind_env_target_nonce() {
        ...
        let d = d_eval(&c);
        match d.outcome {
            DO::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation {
                execution_kind,
                environment,
                epoch_transition_target,
                authority_activation_execution_preparation_nonce,
            } => {
                assert_eq!(execution_kind, DK::StageApplyValidatorAdd);
                assert_eq!(environment, TrustBundleEnvironment::Devnet);
                assert_eq!(epoch_transition_target, c.request.proposed_epoch_transition_target);
                assert_eq!(authority_activation_execution_preparation_nonce, CRC12_NONCE);
            }
            other => panic!("unexpected outcome: {other:?}"),
        }
    }

    fn accept_artifact_reexposes_consumed_authority_activation_authorization_transcript() {
        let dec = dc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.authority_activation_authorization_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = d_inputs(TrustBundleEnvironment::Devnet, &dec);
        let handoff_id = dec.authority_activation_authorization_id.clone();
        let handoff_req = dec.request_id.clone();
        let handoff_digest = dec.authority_activation_authorization_digest.clone();
        let handoff_transcript = dec.transcript_digest.clone();
        let request = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest::new(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::VerifiedAuthorityActivationAuthorizationDecision {
                decision: dec,
        ...
        let exec = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor::source_test();
        let d = exec.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &request,
            &inputs,
            &empty_replay347(),
        );
        let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        assert_eq!(art.authority_activation_authorization_decision_id, handoff_id);
        assert_eq!(art.authority_activation_authorization_request_id, handoff_req);
        assert_eq!(art.authority_activation_authorization_intent_digest, handoff_digest);
        assert_eq!(art.authority_activation_authorization_transcript_digest, handoff_transcript);
    }
    ...
    fn accept_decision_ids_match_artifact_ids() {
        ...
        let d = d_eval(&c);
        let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        assert_eq!(d.authority_activation_execution_preparation_id, art.authority_activation_execution_preparation_id);
        assert_eq!(d.request_id, art.request_id);
        assert_eq!(d.authority_activation_execution_preparation_digest, art.authority_activation_execution_preparation_digest);
        assert_eq!(d.transcript_digest, art.transcript_digest);
        assert!(!d.authority_activation_execution_preparation_id.is_empty());
        assert!(!d.request_id.is_empty());
        assert!(!d.authority_activation_execution_preparation_digest.is_empty());
        assert!(!d.transcript_digest.is_empty());
    }
    ...
    fn deterministic_digests_under_reevaluation() {
        for sc in ALL_SC {
            ...
            let d1 = d_eval(&c);
            let d2 = d_eval(&c);
            assert_eq!(d1.authority_activation_execution_preparation_id, d2.authority_activation_execution_preparation_id);
            assert_eq!(d1.request_id, d2.request_id);
            assert_eq!(d1.authority_activation_execution_preparation_digest, d2.authority_activation_execution_preparation_digest);
            assert_eq!(d1.transcript_digest, d2.transcript_digest);
        }
    }
    ...
    fn artifact_content_digest_is_stable() {
        ...
        let d = d_eval(&c);
        let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        assert_eq!(art.content_digest(), art.authority_activation_execution_preparation_digest);
        assert_eq!(art.content_digest(), art.content_digest());
    }
    ...
    fn wrong_authority_activation_authorization_decision_id() {
        d_reject_inputs(
            |i| i.expected_authority_activation_authorization_decision_id = "bad".to_string(),
            DO::AuthorityActivationAuthorizationDecisionIdMismatch,
        );
    }

    fn wrong_authority_activation_authorization_request_id() {
        d_reject_inputs(
            |i| i.expected_authority_activation_authorization_request_id = "bad".to_string(),
            DO::AuthorityActivationAuthorizationDecisionRequestIdMismatch,
        );
    }

    fn wrong_authority_activation_authorization_intent_digest() {
        d_reject_inputs(
            |i| i.expected_authority_activation_authorization_intent_digest = "bad".to_string(),
            DO::AuthorityActivationAuthorizationDecisionIntentDigestMismatch,
        );
    }

    fn wrong_authority_activation_authorization_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_authority_activation_authorization_transcript_digest = "bad".to_string(),
            DO::AuthorityActivationAuthorizationDecisionTranscriptMismatch,
        );
    }

    fn wrong_authority_activation_authorization_nonce() {
        d_reject_inputs(
            |i| i.expected_authority_activation_authorization_nonce = 9999,
            DO::WrongAuthorityActivationAuthorizationNonce,
        );
    }

    fn tampered_authority_activation_authorization_package_integrity_mismatch() {
        // Mutate the consumed package so its content digest no longer matches the
        // bound handoff decision digest.
        let mut dec = dc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec.authority_activation_authorization_artifact.as_ref().unwrap().epoch_transition_target;
        let inputs = d_inputs(TrustBundleEnvironment::Devnet, &dec);
        dec.authority_activation_authorization_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
        let request = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest::new(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::VerifiedAuthorityActivationAuthorizationDecision {
                decision: dec,
            },
        ...
        assert_eq!(d.outcome, DO::AuthorityActivationAuthorizationDecisionIntegrityMismatch);
        assert!(d.authority_activation_execution_preparation_artifact.is_none());
    }
    ...
    fn wrong_guarded_mutation_decision_id() {
        d_reject_inputs(
            |i| i.expected_guarded_mutation_decision_id = "bad".to_string(),
            DO::GuardedMutationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_request_id() {
        d_reject_inputs(
            |i| i.expected_guarded_mutation_request_id = "bad".to_string(),
            DO::GuardedMutationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_intent_digest() {
        d_reject_inputs(
            |i| i.expected_guarded_mutation_intent_digest = "bad".to_string(),
            DO::GuardedMutationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_guarded_mutation_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_guarded_mutation_transcript_digest = "bad".to_string(),
            DO::GuardedMutationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_decision_id() {
        d_reject_inputs(
            |i| i.expected_staged_application_decision_id = "bad".to_string(),
            DO::StagedApplicationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_request_id() {
        d_reject_inputs(
            |i| i.expected_staged_application_request_id = "bad".to_string(),
            DO::StagedApplicationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_staged_application_intent_digest() {
        d_reject_inputs(
            |i| i.expected_staged_application_intent_digest = "bad".to_string(),
            DO::StagedApplicationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_staged_application_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_staged_application_transcript_digest = "bad".to_string(),
            DO::StagedApplicationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_staged_application_nonce() {
        d_reject_inputs(
            |i| i.expected_staged_application_nonce = 9999,
            DO::WrongStagedApplicationNonce,
        );
    }
    ...
    fn wrong_authorization_decision_id() {
        d_reject_inputs(
            |i| i.expected_authorization_decision_id = "bad".to_string(),
            DO::AuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn wrong_authorization_request_id() {
        d_reject_inputs(
            |i| i.expected_authorization_request_id = "bad".to_string(),
            DO::AuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn wrong_authorization_intent_digest() {
        d_reject_inputs(
            |i| i.expected_authorization_intent_digest = "bad".to_string(),
            DO::AuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn wrong_authorization_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_authorization_transcript_digest = "bad".to_string(),
            DO::AuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn wrong_application_intent_digest() {
        d_reject_inputs(
            |i| i.expected_application_intent_digest = "bad".to_string(),
            DO::WrongApplicationIntentDigest,
        );
    }
    ...
    fn wrong_application_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_application_transcript_digest = "bad".to_string(),
            DO::WrongApplicationTranscriptDigest,
        );
    }
    ...
    fn wrong_governance_domain() {
        d_reject_inputs(
            |i| i.expected_governance_domain_id = "bad".to_string(),
            DO::WrongGovernanceDomain,
        );
    }

    fn wrong_governance_epoch() {
        d_reject_inputs(|i| i.expected_governance_epoch = 999, DO::WrongGovernanceEpoch);
    }

    fn wrong_proposal_id() {
        d_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), DO::WrongProposalId);
    }

    fn wrong_governance_execution_intent_digest() {
        d_reject_inputs(
            |i| i.expected_governance_intent_digest = "bad".to_string(),
            DO::WrongGovernanceExecutionIntentDigest,
        );
    }
    ...
    fn wrong_rotation_transcript_digest() {
        d_reject_inputs(
            |i| i.expected_rotation_transcript_digest = "bad".to_string(),
            DO::WrongRotationTranscriptDigest,
        );
    }
    ...
    fn wrong_rotation_action() {
        d_reject_inputs(
            |i| i.expected_rotation_action = ValidatorSetRotationAction::EmergencyValidatorRemoval,
            DO::WrongRotationAction,
        );
    }

    fn wrong_authority_sequence() {
        d_reject_inputs(|i| i.expected_authority_domain_sequence = 999, DO::WrongAuthoritySequence);
    }
    ...
    fn wrong_current_validator_set_digest() {
        d_reject_inputs(
            |i| i.expected_current_set_digest = "bad".to_string(),
            DO::WrongCurrentValidatorSetDigest,
        );
    }
    ...
    fn wrong_proposed_validator_set_digest() {
        d_reject_inputs(
            |i| i.expected_proposed_set_digest = "bad".to_string(),
            DO::WrongProposedValidatorSetDigest,
        );
    }
    ...
    fn wrong_validator_set_delta_digest() {
        d_reject_inputs(
            |i| i.expected_delta_digest = "bad".to_string(),
            DO::WrongValidatorSetDeltaDigest,
        );
    }

    fn wrong_validator_set_epoch() {
        d_reject_inputs(|i| i.expected_validator_set_epoch = 999, DO::WrongValidatorSetEpoch);
    }

    fn wrong_validator_set_version() {
        d_reject_inputs(|i| i.expected_validator_set_version = 999, DO::WrongValidatorSetVersion);
    }

    fn wrong_current_validator_set_epoch() {
        d_reject_inputs(
            |i| i.expected_current_validator_set_epoch = 999,
            DO::WrongCurrentValidatorSetEpoch,
        );
    }
    ...
    fn wrong_current_validator_set_version() {
        d_reject_inputs(
            |i| i.expected_current_validator_set_version = 999,
            DO::WrongCurrentValidatorSetVersion,
        );
    }

    fn wrong_proposed_validator_count() {
        d_reject_inputs(|i| i.expected_proposed_validator_count = 999, DO::WrongProposedValidatorCount);
    }

    fn wrong_rotation_nonce() {
        d_reject_inputs(|i| i.expected_rotation_nonce = 999, DO::WrongRotationNonce);
    }
    ...
    fn wrong_epoch_transition_target_inputs() {
        d_reject_inputs(|i| i.expected_epoch_transition_target = 999, DO::WrongEpochTransitionTarget);
    }
    ...
    fn wrong_application_nonce() {
        d_reject_inputs(|i| i.expected_application_nonce = 999, DO::WrongApplicationNonce);
    }

    fn wrong_live_application_nonce() {
        d_reject_inputs(|i| i.expected_live_application_nonce = 999, DO::WrongLiveApplicationNonce);
    }
    ...
    fn reject_missing_authority_activation_authorization_decision() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::MissingAuthorityActivationAuthorizationDecision,
            DO::VerifiedAuthorityActivationAuthorizationDecisionRequired,
        );
    }

    fn reject_unverified_authority_activation_authorization_decision() {
        let dec = dc_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::UnverifiedAuthorityActivationAuthorizationDecision {
                decision: dec,
            },
            DO::UnverifiedAuthorityActivationAuthorizationDecisionRejected,
        );
    }
    ...
    fn reject_accepted_authority_activation_authorization_without_package_via_verified_source() {
    ...
    fn reject_accepted_authority_activation_authorization_without_package_variant() {
        let dec = dc_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::AcceptedAuthorityActivationAuthorizationWithoutPackage {
                decision: dec,
            },
            DO::VerifiedAuthorityActivationAuthorizationDecisionRequired,
        );
    ...
    fn reject_guarded_mutation_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::GuardedMutationDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn reject_staged_application_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::StagedApplicationDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn reject_live_application_authorization_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::LiveApplicationAuthorizationWithoutAuthorityActivationAuthorization,
    ...
    fn reject_application_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::ApplicationDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn reject_rotation_plan_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::RotationPlanWithoutAuthorityActivationAuthorization,
    ...
    fn reject_governance_execution_intent_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::GovernanceExecutionIntentWithoutAuthorityActivationAuthorization,
    ...
    fn reject_governance_proof_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::GovernanceProofWithoutAuthorityActivationAuthorization,
    ...
    fn reject_local_operator_assertion() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::LocalOperatorAssertion,
    ...
    fn reject_peer_majority_assertion() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::PeerMajorityAssertion,
    ...
    fn reject_custody_only_evidence() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::CustodyOnlyEvidence,
    ...
    fn reject_remote_signer_only_evidence() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::RemoteSignerOnlyEvidence,
    ...
    fn reject_custody_attestation_only_evidence() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::CustodyAttestationOnlyEvidence,
            DO::CustodyAttestationOnlyProofRejected,
        );
    }

    fn reject_fixture_only_authority_activation_authorization() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::FixtureOnlyAuthorityActivationAuthorization,
    ...
    fn reject_arbitrary_validator_set_bytes() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::ArbitraryValidatorSetBytes,
    ...
    fn mainnet_policy_unavailable() {
        ...
        let exec = d_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            DO::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable
        );
        assert!(d.authority_activation_execution_preparation_artifact.is_none());
    }
    ...
    fn production_policy_unavailable() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = d_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::RequireProductionLiveEpochTransitionAuthorityActivationExecutionPreparation,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            DO::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable
        );
        assert!(d.authority_activation_execution_preparation_artifact.is_none());
    }
    ...
    fn disabled_policy_fails_closed() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = d_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::Disabled,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &c.request,
            &c.inputs,
    ...
    fn reserved_production_kind_fails_closed() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor::new(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig::new(
                ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparation,
            ),
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_execution_preparation(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            DO::LiveEpochTransitionAuthorityActivationExecutionPreparationBoundaryUnavailable
        );
    }
    ...
    fn replay_rejected_when_id_present() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = d_eval(&c);
        let id = d.authority_activation_execution_preparation_artifact.as_ref().unwrap().request_id.clone();
        let replay = vec![id];
        ...
        assert!(d2.authority_activation_execution_preparation_artifact.is_none());
    }

    fn no_replay_when_id_absent() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let replay: Vec<String> = vec!["some-other-id".to_string()];
        let d = d_eval_replay(&c, &replay);
        assert!(d.is_accept());
    }

    fn stale_governance_epoch() {
        d_reject_inputs(|i| i.min_governance_epoch = u64::MAX, DO::StaleGovernanceEpoch);
    }

    fn stale_authority_sequence() {
        d_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), DO::StaleAuthoritySequence);
    }

    fn stale_validator_set_epoch() {
        d_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, DO::StaleValidatorSetEpoch);
    }

    fn stale_validator_set_version() {
        d_reject_inputs(|i| i.min_validator_set_version = u64::MAX, DO::StaleValidatorSetVersion);
    }
    ...
    fn fixture_state_apply_is_idempotent() {
        ...
        let d = d_eval(&c);
        let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        let mut state = LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
        assert!(state.apply_prepared_execution(art, &d.authority_activation_execution_preparation_id));
        assert_eq!(state.current_epoch, art.epoch_transition_target);
        assert_eq!(state.validator_set_version, art.validator_set_version);
        assert_eq!(state.current_set_digest, art.proposed_set_digest);
        // Re-applying the same id is a no-op.
        assert!(!state.apply_prepared_execution(art, &d.authority_activation_execution_preparation_id));
        assert!(state.has_applied(&d.authority_activation_execution_preparation_id));
    }
    ...
    fn every_outcome_is_non_mutating() {
        ...
        let dbad = d_eval_replay(&bad, &[d_eval(&bad).authority_activation_execution_preparation_id]);
        assert!(dbad.outcome.is_non_mutating());
    }
    ...
    fn accept_authorizes_future_mutation_only() {
        ...
        assert!(d.outcome.authorizes_future_mutation_only());
        assert!(d.authorizes_future_mutation_only());
        assert!(d.authority_activation_execution_preparation_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
    }
    ...
    fn policy_predicates() {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy as Pol;
        assert!(Pol::Disabled.is_disabled());
        assert!(Pol::AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation.allows_source_test());
        assert!(Pol::RequireProductionLiveEpochTransitionAuthorityActivationExecutionPreparation.is_production());
        assert!(Pol::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired.is_mainnet());
        assert_eq!(Pol::default(), Pol::Disabled);
    }

    fn kind_predicates() {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind as K;
        assert!(K::SourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation.is_source_test());
        assert!(!K::Disabled.is_source_test());
        assert!(!K::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparation.is_source_test());
        assert_eq!(K::default(), K::Disabled);
    }

    fn execution_kind_mapping_matches_handoff_kind() {
        for sc in ALL_SC {
            let hk = expected_crc11_kind(sc);
            let pk = DK::from_staged_application_kind(hk);
            assert_eq!(pk, expected_crc12_kind(sc));
            assert!(pk.is_non_mutating());
            assert!(!pk.is_unsupported());
        }
    }
    ...
    fn unsupported_staged_application_kind_is_unsupported() {
        let pk = DK::from_staged_application_kind(LiveEpochTransitionAuthorityActivationAuthorizationKind::UnsupportedStagedApplication);
        assert!(pk.is_unsupported());
    }

    fn outcome_tags_are_stable_and_distinct() {
        let a = DO::AuthorityActivationAuthorizationDecisionIdMismatch;
        let b = DO::GuardedMutationDecisionIdMismatch;
        assert_ne!(a.tag(), b.tag());
        assert_eq!(a.tag(), DO::AuthorityActivationAuthorizationDecisionIdMismatch.tag());
        assert!(!DO::MainNetRefused.tag().is_empty());
    }

    fn config_and_inputs_well_formed() {
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        assert!(c.inputs.is_well_formed());
        assert!(ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig::source_test().is_well_formed());
        // The default config still pins the supported protocol version.
        assert!(ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig::default().is_well_formed());
    }
    ...
    macro_rules! per_scenario_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(art.staged_kind.is_non_mutating());
                let d2 = d_eval(&c);
                assert_eq!(d.authority_activation_execution_preparation_digest, d2.authority_activation_execution_preparation_digest);
                assert_eq!(d.transcript_digest, d2.transcript_digest);
                let mut state = LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState::new(
                    CUR_EPOCH,
                    CUR_VERSION,
                    "start",
                );
    ...
    macro_rules! per_scenario_testnet_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(d.is_accept());
                assert_eq!(
                    d.authority_activation_execution_preparation_artifact.as_ref().unwrap().environment,
                    TrustBundleEnvironment::Testnet
                );
            }
        };
    ...
    macro_rules! per_scenario_authority_activation_authorization_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                let mut c = d_case(TrustBundleEnvironment::Devnet, $sc);
                c.inputs.expected_authority_activation_authorization_decision_id = "bad".to_string();
                let d = d_eval(&c);
                assert_eq!(d.outcome, DO::AuthorityActivationAuthorizationDecisionIdMismatch);
                assert!(d.authority_activation_execution_preparation_artifact.is_none());
            }
        };
    }

    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_add, Sc::Add);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_remove, Sc::Remove);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_update, Sc::Update);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_noop, Sc::NoOp);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_identity, Sc::Identity);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_retire, Sc::Retire);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
    per_scenario_authority_activation_authorization_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
    ...
    fn accept_reexposes_runtime_handoff_tuple_from_parent() {
        let parent = dc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let ppkg = parent.authority_activation_authorization_artifact.as_ref().unwrap().clone();
        let c = d_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let d = d_eval(&c);
        assert!(d.is_accept());
        let art = d.authority_activation_execution_preparation_artifact.as_ref().unwrap();
    ...
    fn reject_runtime_handoff_decision_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_runtime_handoff_decision_id = "wrong-runtime-handoff-id".to_string(),
            DO::RuntimeHandoffDecisionIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_request_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_runtime_handoff_request_id = "wrong-runtime-handoff-req".to_string(),
            DO::RuntimeHandoffDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_intent_digest_mismatch() {
        d_reject_inputs(
            |i| i.expected_runtime_handoff_intent_digest = "wrong-runtime-handoff-digest".to_string(),
            DO::RuntimeHandoffDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_runtime_handoff_transcript_mismatch() {
        d_reject_inputs(
            ...
            DO::RuntimeHandoffDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_runtime_handoff_nonce() {
        d_reject_inputs(
            |i| i.expected_runtime_handoff_nonce = HANDOFF_NONCE + 100,
            DO::WrongRuntimeHandoffNonce,
        );
    }

    fn reject_runtime_handoff_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::RuntimeHandoffDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn runtime_handoff_alone_tag_is_stable() {
        assert_eq!(
            DO::RuntimeHandoffDecisionAloneRejected.tag(),
            "runtime-handoff-decision-alone-rejected"
        );
        assert_eq!(
            DO::RuntimeHandoffDecisionIdMismatch.tag(),
            "runtime-handoff-decision-id-mismatch"
        );
        assert_eq!(
            DO::WrongRuntimeHandoffNonce.tag(),
            "wrong-runtime-handoff-nonce"
        );
    }
    ...
    fn accept_content_digest_binds_runtime_handoff_tuple() {
        ...
        let d1 = d_eval(&c);
        let d2 = d_eval(&c);
        let a1 = d1.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        let a2 = d2.authority_activation_execution_preparation_artifact.as_ref().unwrap();
        assert_eq!(a1.content_digest(), a2.content_digest());
    }

    // ===========================================================================
    // Run 337 — re-exposed Run 329/330 execution-preparation ancestor binding
    // (carried through the consumed Run 335/336 authority-activation-authorization artifact) and
    // its alone-rejected authority variant.
    // ===========================================================================
    ...
    fn reject_mutation_execution_decision_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_mutation_execution_decision_id = "wrong-mut-exec-id".to_string(),
            DO::MutationExecutionDecisionIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_request_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_mutation_execution_request_id = "wrong-mut-exec-req".to_string(),
            DO::MutationExecutionDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_mutation_execution_intent_digest_mismatch() {
        d_reject_inputs(
            |i| i.expected_mutation_execution_intent_digest = "wrong-mut-exec-digest".to_string(),
            DO::MutationExecutionDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_mutation_execution_transcript_mismatch() {
        d_reject_inputs(
            ...
            DO::MutationExecutionDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_mutation_execution_nonce() {
        d_reject_inputs(
            |i| i.expected_mutation_execution_nonce = MUT_NONCE + 100,
            DO::WrongMutationExecutionNonce,
        );
    }

    fn reject_mutation_execution_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::MutationExecutionDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn reject_commit_authorization_decision_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_commit_authorization_decision_id = "wrong-commit-auth-id".to_string(),
            DO::CommitAuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_request_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_commit_authorization_request_id = "wrong-commit-auth-req".to_string(),
            DO::CommitAuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_intent_digest_mismatch() {
        d_reject_inputs(
            ...
            DO::CommitAuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_commit_authorization_transcript_mismatch() {
        d_reject_inputs(
            ...
            DO::CommitAuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_commit_authorization_nonce() {
        d_reject_inputs(
            |i| i.expected_commit_authorization_nonce = CMT_NONCE + 100,
            DO::WrongCommitAuthorizationNonce,
        );
    }

    fn reject_commit_authorization_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::CommitAuthorizationDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn mutation_execution_alone_tag_is_stable() {
        assert_eq!(
            DO::MutationExecutionDecisionAloneRejected.tag(),
            "mutation-execution-decision-alone-rejected"
        );
        assert_eq!(
            DO::MutationExecutionDecisionIdMismatch.tag(),
            "mutation-execution-decision-id-mismatch"
        );
        assert_eq!(
            DO::WrongMutationExecutionNonce.tag(),
            "wrong-mutation-execution-nonce"
        );
    }
    ...
    fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
        ...
        // Grandparent Run 329/330 mutation-execution nonce re-exposed intact.
        assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
        // Parent Run 335/336 authority-activation-authorization nonce re-exposed intact.
        assert_eq!(art.authority_activation_authorization_nonce, CRC4_NONCE);
        // Self Run 337 authority-activation-execution-preparation proposed nonce.
        assert_eq!(art.authority_activation_execution_preparation_nonce, CRC12_NONCE);
    }
    ...
    fn reject_execution_preparation_decision_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_execution_preparation_decision_id = "wrong-exec-prep-id".to_string(),
            DO::ExecutionPreparationDecisionIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_request_id_mismatch() {
        d_reject_inputs(
            |i| i.expected_execution_preparation_request_id = "wrong-exec-prep-req".to_string(),
            DO::ExecutionPreparationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_execution_preparation_intent_digest_mismatch() {
        d_reject_inputs(
            |i| i.expected_execution_preparation_intent_digest = "wrong-exec-prep-digest".to_string(),
            DO::ExecutionPreparationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_execution_preparation_transcript_mismatch() {
        d_reject_inputs(
            ...
            DO::ExecutionPreparationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_execution_preparation_nonce() {
        d_reject_inputs(
            |i| i.expected_execution_preparation_nonce = PREP_NONCE + 100,
            DO::WrongExecutionPreparationNonce,
        );
    }

    fn reject_execution_preparation_decision_alone() {
        d_reject_source(
            LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::ExecutionPreparationDecisionWithoutAuthorityActivationAuthorization,
    ...
    fn execution_preparation_alone_tag_is_stable() {
        assert_eq!(
            DO::ExecutionPreparationDecisionAloneRejected.tag(),
            "execution-preparation-decision-alone-rejected"
        );
        assert_eq!(
            DO::ExecutionPreparationDecisionIdMismatch.tag(),
            "execution-preparation-decision-id-mismatch"
        );
        assert_eq!(
            DO::WrongExecutionPreparationNonce.tag(),
            "wrong-execution-preparation-nonce"
        );
    }
    ...
    mod run_347_supplemental {
        ...
        fn supp_determinism_devnet_add() {
            ...
            assert_eq!(a, b);
            assert_eq!(
                a.authority_activation_execution_preparation_digest,
                b.authority_activation_execution_preparation_digest
            );
            assert_eq!(a.transcript_digest, b.transcript_digest);
        }

        fn supp_determinism_devnet_remove() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::Remove);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_update() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::Update);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_noop() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::NoOp);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_identity() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::Identity);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_retire() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::Retire);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_emergency() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::Emergency);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_devnet_authsync() {
            let c = d_case(TrustBundleEnvironment::Devnet, Sc::AuthSync);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_add() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Add);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_remove() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Remove);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_update() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Update);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_noop() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::NoOp);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_identity() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Identity);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_retire() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Retire);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_emergency() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Emergency);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_authsync() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::AuthSync);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_determinism_testnet_bulk() {
            let c = d_case(TrustBundleEnvironment::Testnet, Sc::Bulk);
            let a = d_eval(&c);
            let b = d_eval(&c);
            assert!(a.is_accept());
            assert_eq!(a, b);
        ...
        fn supp_artifact_present_devnet_add() {
            ...
            assert!(d.is_accept());
            let art = d
                .authority_activation_execution_preparation_artifact
                .as_ref()
                .expect("accepted preflight must carry an artifact");
            assert_eq!(art.content_digest(), d.authority_activation_execution_preparation_digest);
        ...
        fn supp_policy_default_is_disabled() {
            type P = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy;
        ...
        fn supp_policy_tags_nonempty() {
            ...
            for p in [
                P::Disabled,
                P::AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation,
                P::RequireProductionLiveEpochTransitionAuthorityActivationExecutionPreparation,
                P::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired,
            ] {
        ...
        fn supp_protocol_version_supported() {
            assert_eq!(PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_PROTOCOL_VERSION, 1);
        }

        fn supp_domain_tags_nonempty_and_distinct() {
            let tags = [
                PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_INTENT_DOMAIN_TAG,
                PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_ID_DOMAIN_TAG,
                PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_REQUEST_DOMAIN_TAG,
                PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_TRANSCRIPT_DOMAIN_TAG,
            ];
            for t in tags {
                assert!(t.contains("347"));
                assert!(!t.is_empty());
            }
            for i in 0..tags.len() {
    ...
    fn run_case(table: &str, name: &str, f: fn(), rows: &mut Vec<(String, String, bool)>) {
        ...
        println!(
            "case table={table} name={name} verdict={}",
    ...
    pub fn helper_main() {
        let outdir = std::env::args()
            ...
            .unwrap_or_else(|| {
                std::path::PathBuf::from(
                    "docs/devnet/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary/helper_evidence/run_348",
                )
            });
        std::fs::create_dir_all(outdir.join("fixtures")).expect("create helper output directory");
        ...
        let cases: &[(&str, &str, fn())] = &[
            ...
            ("accepted_compatible", "accept_artifact_reexposes_consumed_authority_activation_authorization_transcript", accept_artifact_reexposes_consumed_authority_activation_authorization_transcript as fn()),
            ...
            ("determinism", "deterministic_digests_under_reevaluation", deterministic_digests_under_reevaluation as fn()),
            ("accepted_compatible", "artifact_content_digest_is_stable", artifact_content_digest_is_stable as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_authorization_decision_id", wrong_authority_activation_authorization_decision_id as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_authorization_request_id", wrong_authority_activation_authorization_request_id as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_authorization_intent_digest", wrong_authority_activation_authorization_intent_digest as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_authorization_transcript_digest", wrong_authority_activation_authorization_transcript_digest as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_authorization_nonce", wrong_authority_activation_authorization_nonce as fn()),
            ("rejection_fail_closed", "tampered_authority_activation_authorization_package_integrity_mismatch", tampered_authority_activation_authorization_package_integrity_mismatch as fn()),
            ...
            ("rejection_fail_closed", "reject_missing_authority_activation_authorization_decision", reject_missing_authority_activation_authorization_decision as fn()),
            ("rejection_fail_closed", "reject_unverified_authority_activation_authorization_decision", reject_unverified_authority_activation_authorization_decision as fn()),
            ...
            ("rejection_fail_closed", "reject_accepted_authority_activation_authorization_without_package_via_verified_source", reject_accepted_authority_activation_authorization_without_package_via_verified_source as fn()),
            ("rejection_fail_closed", "reject_accepted_authority_activation_authorization_without_package_variant", reject_accepted_authority_activation_authorization_without_package_variant as fn()),
            ...
            ("rejection_fail_closed", "reject_fixture_only_authority_activation_authorization", reject_fixture_only_authority_activation_authorization as fn()),
            ("rejection_fail_closed", "reject_arbitrary_validator_set_bytes", reject_arbitrary_validator_set_bytes as fn()),
            ("mainnet_refusal", "mainnet_domain_refused_under_source_test_policy", mainnet_domain_refused_under_source_test_policy as fn()),
            ("mainnet_refusal", "mainnet_policy_unavailable", mainnet_policy_unavailable as fn()),
            ("rejection_fail_closed", "production_policy_unavailable", production_policy_unavailable as fn()),
            ("rejection_fail_closed", "disabled_policy_fails_closed", disabled_policy_fails_closed as fn()),
            ("rejection_fail_closed", "reserved_production_kind_fails_closed", reserved_production_kind_fails_closed as fn()),
            ...
            ("replay_recovery_idempotency", "fixture_state_apply_is_idempotent", fixture_state_apply_is_idempotent as fn()),
            ("rejection_fail_closed", "fixture_state_apply_all_scenarios", fixture_state_apply_all_scenarios as fn()),
            ("rejection_fail_closed", "every_outcome_is_non_mutating", every_outcome_is_non_mutating as fn()),
            ("accepted_compatible", "accept_authorizes_future_mutation_only", accept_authorizes_future_mutation_only as fn()),
            ("rejection_fail_closed", "reject_never_authorizes_future_mutation", reject_never_authorizes_future_mutation as fn()),
            ("rejection_fail_closed", "policy_predicates", policy_predicates as fn()),
            ("rejection_fail_closed", "kind_predicates", kind_predicates as fn()),
            ("rejection_fail_closed", "execution_kind_mapping_matches_handoff_kind", execution_kind_mapping_matches_handoff_kind as fn()),
            ("rejection_fail_closed", "unsupported_staged_application_kind_is_unsupported", unsupported_staged_application_kind_is_unsupported as fn()),
            ("rejection_fail_closed", "outcome_tags_are_stable_and_distinct", outcome_tags_are_stable_and_distinct as fn()),
            ("rejection_fail_closed", "config_and_inputs_well_formed", config_and_inputs_well_formed as fn()),
            ...
            ("rejection_fail_closed", "runtime_handoff_alone_tag_is_stable", runtime_handoff_alone_tag_is_stable as fn()),
            ("rejection_fail_closed", "runtime_handoff_binding_rejects_are_non_mutating", runtime_handoff_binding_rejects_are_non_mutating as fn()),
            ...
            ("rejection_fail_closed", "commit_authorization_binding_rejects_are_non_mutating", commit_authorization_binding_rejects_are_non_mutating as fn()),
            ...
            ("rejection_fail_closed", "mutation_execution_alone_tag_is_stable", mutation_execution_alone_tag_is_stable as fn()),
            ...
            ("rejection_fail_closed", "mutation_execution_binding_rejects_are_non_mutating", mutation_execution_binding_rejects_are_non_mutating as fn()),
            ...
            ("rejection_fail_closed", "execution_preparation_alone_tag_is_stable", execution_preparation_alone_tag_is_stable as fn()),
            ...
            ("rejection_fail_closed", "execution_preparation_binding_rejects_are_non_mutating", execution_preparation_binding_rejects_are_non_mutating as fn()),
        ];
        ...
        let d1 = d_eval(&c);
        let d2 = d_eval(&c);
        assert!(d1.is_accept(), "fixture accepted case must accept");
        assert!(d2.is_accept(), "second fixture accepted case must accept");
        let art1 = d1
            .authority_activation_execution_preparation_artifact
            .as_ref()
        ...
        let art2 = d2
            ...
            .expect("accepted artifact");
        assert_eq!(
            art1.authority_activation_execution_preparation_id,
            art2.authority_activation_execution_preparation_id
        );
        assert_eq!(art1.request_id, art2.request_id);
        assert_eq!(art1.content_digest(), art2.content_digest());
        assert_eq!(
            art1.authority_activation_execution_preparation_digest,
            art2.authority_activation_execution_preparation_digest
        );
        assert_eq!(art1.transcript_digest, art2.transcript_digest);

        std::fs::write(
            outdir.join("fixtures/run_348_accepted_artifact.txt"),
            format!(
                "authority_activation_execution_preparation_id={}\nrequest_id={}\ncontent_digest={}\nauthority_activation_execution_preparation_digest={}\ntranscript_digest={}\nenvironment={:?}\nstaged_kind={}\nepoch_transition_target={}\n",
                art1.authority_activation_execution_preparation_id,
                art1.request_id,
                art1.content_digest(),
                art1.authority_activation_execution_preparation_digest,
                art1.transcript_digest,
                art1.environment,
                art1.staged_kind.tag(),
                art1.epoch_transition_target,
            ),
        )
        .expect("write accepted artifact fixture");

        std::fs::write(
            outdir.join("fixtures/run_348_deterministic_digests.txt"),
            format!(
                "authority_activation_execution_preparation_id={}\nrequest_id={}\ncontent_digest={}\nauthority_activation_execution_preparation_digest={}\ntranscript_digest={}\nsecond_authority_activation_execution_preparation_id={}\nsecond_request_id={}\nsecond_content_digest={}\nsecond_authority_activation_execution_preparation_digest={}\nsecond_transcript_digest={}\n",
                art1.authority_activation_execution_preparation_id,
                art1.request_id,
                art1.content_digest(),
                art1.authority_activation_execution_preparation_digest,
        ...
        let recovery = c
            .executor
            .recover_live_epoch_transition_authority_activation_execution_preparation_window(
                Some(art1),
                art2,
            );
        std::fs::write(
            outdir.join("fixtures/run_348_replay_recovery.txt"),
            format!("recovery={:?}\n", recovery),
        )
        .expect("write replay recovery fixture");

        let mut tables: Vec<String> = rows.iter().map(|(t, _, _)| t.clone()).collect();
        tables.sort();
        tables.dedup();
        let mut summary = String::new();
        summary.push_str("run=348\n");
        summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_authority_activation_execution_preparation.rs::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor\n");
        summary.push_str(&format!("registered_cases={}\n", cases.len()));
        let mut total_pass = 0u64;
        let mut total_fail = 0u64;
        for t in &tables {
            let pass = rows.iter().filter(|(rt, _, ok)| rt == t && *ok).count() as u64;
            let fail = rows.iter().filter(|(rt, _, ok)| rt == t && !*ok).count() as u64;
            total_pass += pass;
            total_fail += fail;
            summary.push_str(&format!("table {t} pass={pass} fail={fail}\n"));
        }
        ...
        std::fs::write(outdir.join("helper_summary.txt"), &summary).expect("write helper summary");
        print!("{summary}");
        if total_fail != 0 {
            std::process::exit(1);
        }
...
fn main() {
    run_347_authority_activation_execution_preparation::helper_main();
}
