//! Run 172 — release-built fixture helper that mints a complete
//! proof-carrying / no-proof / malformed / invalid-binding v2
//! ratification corpus consumable by the **real `target/release/qbind-node`**
//! production v2 marker-decision surfaces under
...
//!
//! Per `task/RUN_172_TASK.txt`, Run 172 is **release-binary EVIDENCE
...
//!   * `signing-key.ratified.spec`                   - `--p2p-trust-bundle-signing-key`
//!   * `baseline.bundle`                             - signed seq=1 trust bundle
//!   * `candidate.bundle`                            - signed seq=2 trust bundle
...
//!   * `ratification.lifecycle_invalid_proof_valid.json`
//!     - lifecycle invariant broken (lower-sequence Rotate) but proof is well-formed
//!   * `ratification.no_proof.ratify.seq1.json`      - no-proof Ratify@seq=1 (A1/A2 default)
//!
//! Plus, under `<OUT_DIR>/mainnet/`:
//!
//!   * `genesis.json`                                - MainNet genesis
//!   * `baseline.bundle`                             - MainNet seq=1 bundle
//!   * `candidate.bundle`                            - MainNet seq=2 bundle
//!   * `signing-key.ratified.spec`
//!   * `ratification.valid_proof.rotate.seq2.json`   - valid proof (binding-correct)
//!   * `seed-marker.v2.seq1.json`                    - MainNet v2@seq=1 seed
//!     so the harness can prove R23: MainNet peer-driven apply remains
//!     refused even with `Required` policy and a valid governance
//!     proof; the surface refusal is owned by the Run 130 environment
//!     policy and is unchanged by Run 165 / Run 167 / Run 169 / Run 171.
//!
//! Usage:
//! ```text
//! run_172_governance_required_policy_release_binary_helper <OUT_DIR>
//! ```
//!
//! This helper exists alongside (and does NOT replace) the Run 168
//! release-built helper. The Run 168 helper exercises the
//! production loader + Run 165 gate symbols **in-process** through 13
//! `H1`..`H13` scenarios that stay below the binary-CLI boundary.
//! Run 172 carries those in-process boundary results forward via
//! harness replay while adding **real `target/release/qbind-node`**
//! evidence on the Run 169-wired production preflight call sites,
//! activated through the Run 171 hidden Required-policy selector.
...
fn harness(env: NetworkEnvironment) -> Harness {
    ...
    let authority_root = GenesisAuthorityRoot::with_public_key_bytes(
        GENESIS_AUTHORITY_SUITE_ML_DSA_44,
        &authority_pk,
        "run172-bundle-signing-authority",
    );
...
fn write_env_fixtures(base: &Path, env: NetworkEnvironment) {
    ...
    let other_active = mint_signing();
    let _ = other_active.spec; // keep alive
    ...
    // Rotate scenarios require the candidate trust bundle to be signed
    // by the **new** active key (`rotated`) because Run 069 / Run 105
    // ratification enforcement requires the candidate's signing key to
    // be the one ratified by the v2 sidecar. The Rotate v2 sidecar
    // ratifies `rotated` as the new active.
    ...
    // Operator must also pass the `rotated` signing-key spec on the CLI
    // for Rotate scenarios — write it alongside the `active` spec.
    ...
    // v2 ratify@seq=1 (no-proof Ratify, used to seed the v2@seq=1 marker
    // and as the A1/A2 default-NotRequired no-proof sidecar).
    ...
    // Seed v2@seq=1 marker (operator copies this to data-dir/pqc_authority_state.json).
    ...
    // No-proof Rotate sidecar (A1/A2 mutating-equivalent regression as well as
    // R1/R2/R3/R4 fail-closed under Required).
    write_bytes(
    ...
    // Idempotent: identical bytes (same authority sequence / digest /
    // proof) — replayed and accepted as no-op per existing marker rules.
    write_bytes(
        &base.join("ratification.idempotent.rotate.seq2.json"),
        &build_sidecar_json(&v2_rotate_seq2, Some(&good_proof)),
    );

    // Malformed proof: schema_version=0 (rejected by Run 167 wire decoder
    // as `Malformed`, mapped by Run 169 shim to `Unavailable`).
    ...
    // Re-sign with the same expected-bytes formula but for the wrong root —
    // we deliberately leave the issuer_signature aligned with the *true*
    // candidate digest under the true root so that the verifier's
    // authority-root binding check (which fires on the wire field) trips
    // before the signature-bytes check would.
    write_bytes(
        &base.join("ratification.wrong_root.rotate.seq2.json"),
        &build_sidecar_json(&v2_rotate_seq2, Some(&wrong_root)),
    );

    // Wrong lifecycle action — proof claims `ActivateInitial` for what the
    // candidate ratification declares as `Rotate`.
    ...
    // Unsupported issuer suite (e.g. classical / non-PQC suite id 1).
    ...
    // OnChainGovernance class — verifier rejects as
    // `UnsupportedOnChainGovernance` (fail-closed).
    ...
    // Lifecycle invariant invalid + proof valid: replay the seq=2 Rotate
    // ratification under a (caller-installed) v2@seq=2 seed marker so the
    // sequence-monotonic guard fires before the gate produces FirstV2Write
    // / UpgradeV2. Same proof binding as the good_proof. We also seed a
    // matching v2@seq=2 marker for the harness.
    let v2_marker_seq2 = PersistentAuthorityStateRecordV2::new(
        ...
        v2_rotate_seq2_digest_hex.clone(),
        None,
        AuthorityStateUpdateSource::TestOrFixture,
        FIXED_TS,
    );
    ...
    write_bytes(
        &base.join("ratification.lifecycle_invalid_proof_valid.rotate.seq2.json"),
        &build_sidecar_json(&v2_rotate_seq2, Some(&good_proof)),
    );
...
fn main() {
    let outdir = env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .expect("usage: run_172_governance_required_policy_release_binary_helper <OUTDIR>");
    ...
    eprintln!(
        "[run-172] release-built Required-policy fixtures written under {}",
        outdir.display()
    );
}