...
use qbind_node::pqc_governance_durable_completion_settlement_receipt_acknowledgement::{
    ...
    DurableCompletionSettlementReceiptAcknowledgementWindow,
    ExternalSettlementReceiptAcknowledgementSink,
    ...
    MainNetSettlementReceiptAcknowledgementSink, ProductionSettlementReceiptAcknowledgementSink,
};
use std::env;
use std::fs;
...
fn release_symbol_reachability_probe() {
    ...
    let snapshot: DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot =
        ledger.snapshot();
    assert_eq!(snapshot.len(), 1);
    ...
    let _binding: DurableCompletionSettlementReceiptAcknowledgementBinding =
        input.runtime_binding.clone();

    assert!(DurableCompletionSettlementReceiptAcknowledgementPolicy::Disabled.is_disabled());
    assert!(
        DurableCompletionSettlementReceiptAcknowledgementPolicy::FixtureAllowed.allows_fixture()
    );
    assert_eq!(
    ...
    assert_eq!(
        DurableCompletionSettlementReceiptAcknowledgementKind::Disabled.tag(),
        "disabled"
    );
...
fn main() {
    ...
    summary
        .push_str("Run 271 durable-completion settlement-receipt-acknowledgement release helper\n");
