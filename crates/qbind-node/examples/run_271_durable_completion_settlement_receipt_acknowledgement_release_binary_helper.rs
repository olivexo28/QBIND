//! Run 271 — release-built helper for the Run 270 durable-completion
//! settlement-receipt-acknowledgement / settlement-finality projection sink boundary.
//!
...
//! finalization chain and then evaluates the Run 270 settlement-receipt
//! acknowledgement. Projection is intentionally from
//! `input.settlement_finalization_binding` through
...
// Run 271 — additional Run 270 production library symbols exercised by the
// release symbol reachability probe (imported here so the release helper source
// grep-proves it links the real Run 270 boundary symbols in release mode).
use qbind_node::pqc_governance_durable_completion_settlement_receipt_acknowledgement::{
    durable_completion_settlement_receipt_acknowledgement_consumer_required,
    ...
    settlement_receipt_acknowledgement_record_digest,
    settlement_receipt_acknowledgement_request_digest,
    settlement_receipt_acknowledgement_response_digest,
    settlement_receipt_acknowledgement_transcript_digest,
    DurableCompletionSettlementReceiptAcknowledgementBinding,
    DurableCompletionSettlementReceiptAcknowledgementDigest,
    DurableCompletionSettlementReceiptAcknowledgementEnvironment,
    ...
    DurableCompletionSettlementReceiptAcknowledgementSurface,
...
// ===========================================================================
// Release symbol reachability probe — exercises the Run 270 production library
// symbols in release mode, driving projection from the Run 268 settlement-
// finalization outcome via input.settlement_finalization_binding.
// ===========================================================================

fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionSettlementReceiptAcknowledgementInput = c.recorded();

    let intent = project_settlement_finalization_outcome_to_receipt_acknowledgement_request(
        &input.settlement_finalization_binding,
    );
    assert!(intent.creates_request());
    assert!(matches!(
        intent,
        DurableCompletionSettlementReceiptAcknowledgementRequestIntent::CreateRequest
    ));
    ...
    assert!(!outcome.no_commitment());
    assert_eq!(outcome.tag(), "settlement-receipt-acknowledgement-recorded");
    assert_eq!(sink.kind(), DurableCompletionSettlementReceiptAcknowledgementKind::FixtureInMemory);
    assert_eq!(sink.invocations(), 1);

    let identity_digest: DurableCompletionSettlementReceiptAcknowledgementDigest =
        settlement_receipt_acknowledgement_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionSettlementReceiptAcknowledgementDigest =
        settlement_receipt_acknowledgement_request_digest(&c.request);
    let response = DurableCompletionSettlementReceiptAcknowledgementResponse {
        receipt_acknowledgement_record_id: c.request.receipt_acknowledgement_record_id.clone(),
    ...
    let response_digest: DurableCompletionSettlementReceiptAcknowledgementDigest =
        settlement_receipt_acknowledgement_response_digest(&response);
    let record: DurableCompletionSettlementReceiptAcknowledgementRecord = c.request.to_record();
    let record_digest: DurableCompletionSettlementReceiptAcknowledgementDigest =
        settlement_receipt_acknowledgement_record_digest(&record);
    let transcript_digest: DurableCompletionSettlementReceiptAcknowledgementTranscriptDigest =
        settlement_receipt_acknowledgement_transcript_digest(&request_digest, &response_digest, &record_digest);
    assert!(!identity_digest.as_hex().is_empty());
    assert!(!transcript_digest.as_hex().is_empty());

    let ledger_record: &DurableCompletionSettlementReceiptAcknowledgementLedgerRecord = ledger
        .find(RECEIPT_ACKNOWLEDGEMENT_RECORD_ID)
        .expect("settlement-receipt-acknowledgement ledger record");
    assert_eq!(ledger_record.status, DurableCompletionSettlementReceiptAcknowledgementLedgerStatus::Recorded);
    let snapshot: DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot = ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionSettlementReceiptAcknowledgementSurface = input.surface();
    let _environment: DurableCompletionSettlementReceiptAcknowledgementEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionSettlementReceiptAcknowledgementBinding = input.runtime_binding.clone();

    assert!(DurableCompletionSettlementReceiptAcknowledgementPolicy::Disabled.is_disabled());
    assert!(DurableCompletionSettlementReceiptAcknowledgementPolicy::FixtureAllowed.allows_fixture());
    assert_eq!(
        DurableCompletionSettlementReceiptAcknowledgementPolicy::ProductionSettlementReceiptAcknowledgementRequired.tag(),
        "production-settlement-receipt-acknowledgement-required"
    );
    assert!(DurableCompletionSettlementReceiptAcknowledgementKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionSettlementReceiptAcknowledgementKind::ProductionSettlementReceiptAcknowledgementUnavailable.is_unavailable());
    assert_eq!(DurableCompletionSettlementReceiptAcknowledgementKind::Disabled.tag(), "disabled");
    let no_receipt_acknowledgement = DurableCompletionSettlementReceiptAcknowledgementOutcome::LegacyBypassNoSettlementReceiptAcknowledgement;
    assert!(no_receipt_acknowledgement.is_legacy_bypass());
    assert!(no_receipt_acknowledgement.no_commitment());
    assert!(DurableCompletionSettlementReceiptAcknowledgementOutcome::MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement
        .is_mainnet_peer_driven_apply_refused());

    // Invariant helpers — fail-closed contract asserted by the release helper.
    assert!(durable_completion_settlement_receipt_acknowledgement_consumer_required());
    assert!(durable_completion_settlement_receipt_acknowledgement_finalization_required());
    assert!(durable_completion_settlement_receipt_acknowledgement_no_real_settlement());
    assert!(durable_completion_settlement_receipt_acknowledgement_no_real_settlement_finality());
    assert!(durable_completion_settlement_receipt_acknowledgement_no_real_settlement_receipt());
    assert!(durable_completion_settlement_receipt_acknowledgement_no_real_settlement_receipt_acknowledgement());
    assert!(durable_completion_settlement_receipt_acknowledgement_no_real_settlement_finality_projection());

    let mut prod = ProductionSettlementReceiptAcknowledgementSink::default();
    let mut mainnet = MainNetSettlementReceiptAcknowledgementSink::default();
    let mut external = ExternalSettlementReceiptAcknowledgementSink::default();
    let mut prod_ledger = DurableCompletionSettlementReceiptAcknowledgementLedger::new();
    assert_eq!(
        prod.project_durable_completion_settlement_receipt_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_settlement_receipt_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_settlement_receipt_acknowledgement(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_271_durable_completion_settlement_receipt_acknowledgement_release_binary/helper_evidence/run_271")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "run268_duplicate_idempotent_finalization_only_matches_existing_never_creates", run268_duplicate_idempotent_finalization_only_matches_existing_never_creates as fn()),
        ...
        ("rejection_fail_closed", "non_recording_finalization_outcomes_never_record_receipt_acknowledgement", non_recording_finalization_outcomes_never_record_receipt_acknowledgement as fn()),
        ...
        ("rejection_fail_closed", "wrong_receipt_acknowledgement_record_id_rejected_before_record", wrong_receipt_acknowledgement_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request", non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request as fn()),
        ...
        ("recovery_crash_window", "pre_settlement_finalization_windows_fail_closed_no_commitment", pre_settlement_finalization_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_settlement_finalization_request_before_record_rejects_before_record", after_settlement_finalization_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_settlement_finalization_record_before_success_requires_explicit_matching_record", after_settlement_finalization_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_settlement_finalization_success_recovers_as_recorded", after_settlement_finalization_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_finalization_outcome_creates_settlement_receipt_acknowledgement_request_intent", only_recorded_finalization_outcome_creates_settlement_receipt_acknowledgement_request_intent as fn()),
        ("projection", "non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request", non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request as fn()),
        ("projection", "run268_duplicate_idempotent_finalization_only_matches_existing_never_creates", run268_duplicate_idempotent_finalization_only_matches_existing_never_creates as fn()),
        ("settlement_receipt_acknowledgement_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("settlement_receipt_acknowledgement_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("settlement_receipt_acknowledgement_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_finalization_outcomes_never_record_receipt_acknowledgement", non_recording_finalization_outcomes_never_record_receipt_acknowledgement as fn()),
        ("non_mutation", "non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request", non_recording_finalization_outcomes_create_no_settlement_receipt_acknowledgement_request as fn()),
    ...
    summary.push_str("Run 271 durable-completion settlement-receipt-acknowledgement release helper\n");
    summary.push_str(&format!("verdict: {}\n", if total_fail == 0 { "PASS" } else { "FAIL" }));
    summary.push_str("projection_rule: input.settlement_finalization_binding -> project_settlement_finalization_outcome_to_receipt_acknowledgement_request\n");
    summary.push_str("attached_chain: Run256 backend -> Run258 receipt -> Run260 acknowledgement -> Run262 consumer -> Run264 settlement projection -> Run266 settlement commitment -> Run268 settlement finalization -> Run270 settlement-receipt acknowledgement\n");
    ...
    fs::write(
        outdir.join("fixtures/run_271_projection_rule.txt"),
        "input.settlement_finalization_binding\nproject_settlement_finalization_outcome_to_receipt_acknowledgement_request\nSettlementFinalizationRecorded -> CreateRequest\n",
