//! Run 338 — release-binary helper for the Run 337 **live epoch-transition
//! settlement / finalization execution-preparation boundary**.
//!
//! Release-binary evidence for the Run 337 live epoch-transition settlement /
//! finalization execution-preparation boundary
//! (`crates/qbind-node/src/pqc_production_live_epoch_transition_settlement_execution_preparation.rs`).
//! This helper is compiled as a release example and linked against the
//! release-built production library symbols. It exercises the **real** Run 337
//! `ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor` and
...
//! -> 327 -> 329 -> 331 -> 333 -> 335 chain to produce a verified, accepted Run
//! 335 live epoch-transition settlement / finalization-preparation decision
//! (`is_accept()` with `Some(settlement_preparation_artifact)`), then feeds that
//! decision into the Run 337 live epoch-transition settlement / finalization
//! execution-preparation executor, which emits only a typed, deterministic,
//! non-mutating settlement / finalization execution-preparation artifact.
//!
...
//! enablement, settlement-execution-preparation default wiring, or live
//! validator-set mutation. The executor only ever produces typed non-mutating
//! live settlement / finalization execution-preparation artifacts; it never
//! applies a live validator-set change, never transitions a consensus epoch,
//! never commits or finalizes production runtime state, never writes a
//! production receipt, audit record, audit seal, audit-finalization record,
//! audit-ledger record, audit-ledger commitment record, durable-audit-publication
//! record, durable replay record, settlement record, settlement-finalization
//! record, settlement-execution record, publication record, external-publication
...
//! exclusively as source/test evidence and clearly distinct from production
//! runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_338.md`.
#![allow(dead_code)]
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary/helper_evidence/run_338",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_settlement_preparation_transcript",
            accept_artifact_reexposes_consumed_settlement_preparation_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_settlement_preparation_decision_id",
            wrong_settlement_preparation_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_settlement_preparation_request_id",
            wrong_settlement_preparation_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_settlement_preparation_intent_digest",
            wrong_settlement_preparation_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_settlement_preparation_transcript_digest",
            wrong_settlement_preparation_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_settlement_preparation_nonce",
            wrong_settlement_preparation_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_settlement_preparation_package_integrity_mismatch",
            tampered_settlement_preparation_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_settlement_preparation_decision",
            reject_missing_settlement_preparation_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_settlement_preparation_decision",
            reject_unverified_settlement_preparation_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_settlement_preparation_without_package_via_verified_source",
            reject_accepted_settlement_preparation_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_settlement_preparation_without_package_variant",
            reject_accepted_settlement_preparation_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_settlement_preparation",
            reject_fixture_only_settlement_preparation as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    let accepted_case = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let accepted_decision = y_eval(&accepted_case);
    assert!(
    ...
    let accepted_artifact = accepted_decision
        .settlement_execution_preparation_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a settlement-execution-preparation artifact");
    ...
    accepted_body.push_str(&format!(
        ...
        accepted_decision.settlement_execution_preparation_id
    ));
    ...
    accepted_body.push_str(&format!(
        ...
        accepted_decision.settlement_execution_preparation_digest
    ));
    accepted_body.push_str(&format!(
        "transcript_digest={}\n",
    ...
    fs::write(
        outdir.join("fixtures/run_338_accepted_artifact.txt"),
        &accepted_body,
    )
    .expect("write accepted-artifact fixture");

    let digest_case_1 = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let digest_case_2 = y_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let digest_decision_1 = y_eval(&digest_case_1);
    let digest_decision_2 = y_eval(&digest_case_2);
    ...
    assert_eq!(
        digest_decision_1.settlement_execution_preparation_id,
        digest_decision_2.settlement_execution_preparation_id
    );
    ...
    assert_eq!(
        digest_decision_1.settlement_execution_preparation_digest,
        digest_decision_2.settlement_execution_preparation_digest
    );
    assert_eq!(
    ...
    digest_body.push_str(&format!(
        "second_settlement_execution_preparation_id={}\n",
        digest_decision_2.settlement_execution_preparation_id
    ));
    digest_body.push_str(&format!(
        "second_request_id={}\n",
    ...
    digest_body.push_str(&format!(
        "second_settlement_execution_preparation_digest={}\n",
        digest_decision_2.settlement_execution_preparation_digest
    ));
    digest_body.push_str(&format!(
    ...
    fs::write(
        outdir.join("fixtures/run_338_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    let recover_exec =
        ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact =
        accepted_artifact;
    let clean: ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_settlement_execution_preparation_window(
            None,
            current_artifact,
        );
    let idempotent: ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_settlement_execution_preparation_window(
    ...
    let staged_application_id = match &idempotent {
        ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome::IdempotentReplayObserved { staged_application_id } => staged_application_id.clone(),
    ...
    fs::write(
        outdir.join("fixtures/run_338_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper\n");
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_settlement_execution_preparation.rs :: ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor (Run 337 source/test boundary)\n");
