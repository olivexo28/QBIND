//! Run 283 — release-binary durable-completion external-publication-acknowledgement
//! consumer / external-publication-audit-finalization boundary helper.
//!
//! Release-binary evidence for the Run 282 durable-completion external-publication
//! acknowledgement consumer / external-publication-audit-finalization boundary.
...
//! receipt -> Run 280 external-publication acknowledgement, and then evaluates the
//! Run 282 external-publication-audit-finalization boundary. Projection is
//! intentionally from `input.external_publication_acknowledgement_binding` through
...
//! audit-finalization path stays fixture/in-memory/dev-test only. No production
...
fn attach_run280_external_publication_acknowledgement(
    ...
    let record = ledger
        ...
        .expect("recorded Run 280 settlement outcome publication");
    AttachedExternalPublicationAcknowledgement {
        outcome,
        external_publication_acknowledgement_record_id: action.external_publication_acknowledgement_record_id.clone(),
...
struct Ctx {
    ...
    consumer: AttachedConsumer,
    external_publication_acknowledgement: AttachedExternalPublicationAcknowledgement,
}

fn ack_identity(
    policy: DurableCompletionExternalPublicationAuditFinalizationPolicy,
...
impl Ctx {
    ...
    fn input_with_confirmation(
        ...
        external_publication_audit_finalization: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        external_publication_acknowledgement: DurableCompletionExternalPublicationAcknowledgementOutcome,
    ) -> DurableCompletionExternalPublicationAuditFinalizationInput {
        DurableCompletionExternalPublicationAuditFinalizationInput {
            policy,
            ...
            confirmation_binding: external_publication_audit_finalization,
            ...
            external_publication_acknowledgement_binding: external_publication_acknowledgement,
    ...
    fn input(
        ...
        self.input_with_confirmation(
            ...
            self.external_publication_acknowledgement.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionExternalPublicationAuditFinalizationInput {
        self.input_with_confirmation(
...
fn fixture_sink() -> FixtureDurableCompletionExternalPublicationAuditFinalizationSink {
    FixtureDurableCompletionExternalPublicationAuditFinalizationSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionExternalPublicationAuditFinalizationPolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(external_publication_audit_finalization_outcome_authorizes_record(&outcome));
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.external_publication_audit_finalization_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!external_publication_audit_finalization_outcome_authorizes_record(&second));
    assert!(external_publication_audit_finalization_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run268_duplicate_idempotent_confirmation_only_matches_existing_never_creates() {
    // A real Run 280 ExternalPublicationAcknowledgementDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.external_publication_acknowledgement.outcome,
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAuditFinalizationOutcome::ExternalPublicationAuditFinalizationRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_external_publication_audit_finalization(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_external_publication_audit_finalization(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditFinalizationPolicy::ProductionExternalPublicationAuditFinalizationRequired,
        DurableCompletionExternalPublicationAuditFinalizationKind::ProductionExternalPublicationAuditFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    let mut sink = ProductionExternalPublicationAuditFinalizationSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditFinalizationPolicy::MainNetExternalPublicationAuditFinalizationRequired,
        DurableCompletionExternalPublicationAuditFinalizationKind::MainNetExternalPublicationAuditFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    let mut sink = MainNetExternalPublicationAuditFinalizationSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditFinalizationPolicy::ExternalExternalPublicationAuditFinalizationRequired,
        DurableCompletionExternalPublicationAuditFinalizationKind::ExternalExternalPublicationAuditFinalizationUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    let mut sink = ExternalExternalPublicationAuditFinalizationSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn assert_non_recording_confirmation(
    external_publication_acknowledgement: DurableCompletionExternalPublicationAcknowledgementOutcome,
    expected: DurableCompletionExternalPublicationAuditFinalizationOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_confirmation(
        DurableCompletionExternalPublicationAuditFinalizationPolicy::FixtureAllowed,
...
fn non_recording_confirmation_outcomes_never_record_external_publication_audit_finalization() {
    ...
    assert_non_recording_confirmation(
        ...
        Receipt::LegacyBypassNoExternalPublicationAuditFinalization,
    );
    assert_non_recording_confirmation(
    ...
    assert_non_recording_confirmation(
        ...
        Receipt::MainNetPeerDrivenApplyRefusedNoAuditFinalization,
    );
    assert_non_recording_confirmation(
        Finalization::ValidatorSetRotationUnsupportedNoAcknowledgement,
        Receipt::ValidatorSetRotationUnsupportedNoAuditFinalization,
    );
    assert_non_recording_confirmation(
        Finalization::PolicyChangeUnsupportedNoAcknowledgement,
        Receipt::PolicyChangeUnsupportedNoAuditFinalization,
    );
}
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAuditFinalizationOutcome::RejectedBeforeExternalPublicationAcknowledgementNoAuditFinalization
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionExternalPublicationAuditFinalizationRequest),
) {
    let c = devnet_ctx();
...
fn wrong_external_publication_audit_finalization_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.external_publication_audit_finalization_record_id = "other-confirmation".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_external_publication_audit_finalization(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_acknowledgement(
    fault: DurableCompletionExternalPublicationAuditFinalizationFault,
    expected: DurableCompletionExternalPublicationAuditFinalizationOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    let mut sink = FixtureDurableCompletionExternalPublicationAuditFinalizationSink::with_fault(fault);
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditFinalizationOutcome::ExternalPublicationAuditFinalizationRecordFailedNoAuditFinalization,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditFinalizationOutcome::ExternalPublicationAuditFinalizationRolledBackNoAuditFinalization,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditFinalizationOutcome::ExternalPublicationAuditFinalizationRollbackFailedFatalNoAuditFinalization,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditFinalizationOutcome::ExternalPublicationAuditFinalizationAmbiguousFailClosedNoAuditFinalization,
    );
}
...
fn only_recorded_confirmation_outcome_creates_external_publication_audit_finalization_request_intent() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Finalization;
    ...
    assert!(project_external_publication_acknowledgement_outcome_to_external_publication_audit_finalization_request(
        &Finalization::ExternalPublicationAcknowledgementRecorded
    )
    .creates_request());
    assert_eq!(
...
fn non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request() {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Finalization;
    use DurableCompletionExternalPublicationAuditFinalizationRequestIntent as Intent;
    for confirmation in [
        Finalization::LegacyBypassNoExternalPublicationAcknowledgement,
...
fn recovered_ledger() -> DurableCompletionExternalPublicationAuditFinalizationLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_external_publication_audit_finalization(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let outcome = recover_durable_completion_external_publication_audit_finalization_window(
        &input,
        window,
        DurableCompletionExternalPublicationAuditFinalizationKind::FixtureInMemory,
        record,
...
fn pre_external_publication_acknowledgement_windows_fail_closed_no_commitment() {
...
fn after_external_publication_acknowledgement_request_before_record_rejects_before_record() {
    use DurableCompletionExternalPublicationAuditFinalizationOutcome as Finalization;
    use DurableCompletionExternalPublicationAuditFinalizationWindow as Window;
    assert_window(
...
fn after_external_publication_acknowledgement_record_before_success_requires_explicit_matching_record() {
    use DurableCompletionExternalPublicationAuditFinalizationOutcome as Finalization;
    use DurableCompletionExternalPublicationAuditFinalizationWindow as Window;
    assert_window(
...
fn after_external_publication_acknowledgement_success_recovers_as_recorded() {
    use DurableCompletionExternalPublicationAuditFinalizationOutcome as Finalization;
    use DurableCompletionExternalPublicationAuditFinalizationWindow as Window;
    assert_window(
        Window::AfterExternalPublicationAuditFinalizationSuccess,
        true,
        Finalization::ExternalPublicationAuditFinalizationRecorded,
    );
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_external_publication_audit_finalization_window(
        &input,
        DurableCompletionExternalPublicationAuditFinalizationWindow::AfterExternalPublicationAuditFinalizationSuccess,
        DurableCompletionExternalPublicationAuditFinalizationKind::ProductionExternalPublicationAuditFinalizationUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionExternalPublicationAuditFinalizationSink::with_fault(
        DurableCompletionExternalPublicationAuditFinalizationFault::RolledBackNoReceipt,
    );
    let outcome = evaluate_durable_completion_external_publication_audit_finalization(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_external_publication_audit_finalization_rejection_is_non_mutating());
    assert!(durable_completion_external_publication_audit_finalization_never_calls_run_070());
    assert!(durable_completion_external_publication_audit_finalization_never_mutates_live_pqc_trust_state());
    assert!(durable_completion_external_publication_audit_finalization_never_writes_sequence_or_marker());
    assert!(durable_completion_external_publication_audit_finalization_no_rocksdb_file_schema_migration_change());
    assert!(durable_completion_external_publication_audit_finalization_no_external_publication());
    assert!(durable_completion_external_publication_audit_finalization_no_real_audit_ledger());
    assert!(durable_completion_external_publication_audit_finalization_pipeline_success_required());
    assert!(durable_completion_external_publication_audit_finalization_sink_receipt_required());
    assert!(durable_completion_external_publication_audit_finalization_completion_report_required());
    assert!(durable_completion_external_publication_audit_finalization_finalization_projection_required());
    assert!(durable_completion_external_publication_audit_finalization_attestation_required());
    assert!(durable_completion_external_publication_audit_finalization_backend_submission_required());
    assert!(durable_completion_external_publication_audit_finalization_receipt_required());
    assert!(durable_completion_external_publication_audit_finalization_record_required_before_reported());
    assert!(durable_completion_external_publication_audit_finalization_failed_record_never_records());
    assert!(durable_completion_external_publication_audit_finalization_rollback_never_records());
    assert!(durable_completion_external_publication_audit_finalization_ambiguous_window_fails_closed());
    assert!(
        durable_completion_external_publication_audit_finalization_mainnet_peer_driven_apply_refused_first(
            TrustBundleEnvironment::Mainnet
        )
    ...
    assert!(durable_completion_external_publication_audit_finalization_production_mainnet_unavailable());
    assert!(durable_completion_external_publication_audit_finalization_external_unavailable());
    assert!(durable_completion_external_publication_audit_finalization_validator_set_rotation_unsupported());
    assert!(durable_completion_external_publication_audit_finalization_policy_change_unsupported());
    assert!(
        durable_completion_external_publication_audit_finalization_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
// Supplementary imports used only by the release-mode reachability probe below.
use qbind_node::pqc_governance_durable_completion_external_publication_audit_finalization::{
    durable_completion_external_publication_audit_finalization_confirmation_required,
    durable_completion_external_publication_audit_finalization_consumer_required,
    ...
    DurableCompletionExternalPublicationAuditFinalizationBinding,
    DurableCompletionExternalPublicationAuditFinalizationDigest,
    DurableCompletionExternalPublicationAuditFinalizationEnvironment,
    ...
    DurableCompletionExternalPublicationAuditFinalizationLedgerStatus,
    DurableCompletionExternalPublicationAuditFinalizationRecord,
    DurableCompletionExternalPublicationAuditFinalizationResponse,
    DurableCompletionExternalPublicationAuditFinalizationSurface,
    DurableCompletionExternalPublicationAuditFinalizationTranscriptDigest,
};

// Release-mode reachability probe: exercises the actual Run 282 production
// external-publication-audit-finalization API symbols in release mode, proving
// they are present and linked, while the fixture path stays in-memory only.
fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionExternalPublicationAuditFinalizationInput = c.recorded();
    ...
    assert!(matches!(
        intent,
        DurableCompletionExternalPublicationAuditFinalizationRequestIntent::CreateRequest
    ));
    ...
    assert_eq!(
        outcome.tag(),
        "external-publication-audit-finalization-recorded"
    );
    assert_eq!(
        sink.kind(),
        DurableCompletionExternalPublicationAuditFinalizationKind::FixtureInMemory
    ...
    let identity_digest: DurableCompletionExternalPublicationAuditFinalizationDigest =
        external_publication_audit_finalization_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionExternalPublicationAuditFinalizationDigest =
        external_publication_audit_finalization_request_digest(&c.request);
    let response = DurableCompletionExternalPublicationAuditFinalizationResponse {
        external_publication_audit_finalization_record_id: c
            .request
    ...
    let response_digest: DurableCompletionExternalPublicationAuditFinalizationDigest =
        external_publication_audit_finalization_response_digest(&response);
    let record: DurableCompletionExternalPublicationAuditFinalizationRecord = c.request.to_record();
    let record_digest: DurableCompletionExternalPublicationAuditFinalizationDigest =
        external_publication_audit_finalization_record_digest(&record);
    let transcript_digest: DurableCompletionExternalPublicationAuditFinalizationTranscriptDigest =
        external_publication_audit_finalization_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
    ...
    let ledger_record: &DurableCompletionExternalPublicationAuditFinalizationLedgerRecord = ledger
        .find(AUDIT_FINALIZATION_RECORD_ID)
        .expect("external-publication-audit-finalization ledger record");
    assert_eq!(
        ledger_record.status,
        DurableCompletionExternalPublicationAuditFinalizationLedgerStatus::Recorded
    );
    let snapshot: DurableCompletionExternalPublicationAuditFinalizationLedgerSnapshot =
        ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionExternalPublicationAuditFinalizationSurface = input.surface();
    let _environment: DurableCompletionExternalPublicationAuditFinalizationEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionExternalPublicationAuditFinalizationBinding =
        input.runtime_binding.clone();

    assert!(DurableCompletionExternalPublicationAuditFinalizationPolicy::Disabled.is_disabled());
    assert!(
        DurableCompletionExternalPublicationAuditFinalizationPolicy::FixtureAllowed.allows_fixture()
    );
    assert_eq!(
        DurableCompletionExternalPublicationAuditFinalizationPolicy::ProductionExternalPublicationAuditFinalizationRequired.tag(),
        "production-external-publication-audit-finalization-required"
    );
    assert!(DurableCompletionExternalPublicationAuditFinalizationKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionExternalPublicationAuditFinalizationKind::ProductionExternalPublicationAuditFinalizationUnavailable.is_unavailable());
    assert_eq!(
        DurableCompletionExternalPublicationAuditFinalizationKind::Disabled.tag(),
        "disabled"
    );
    let no_finalization =
        DurableCompletionExternalPublicationAuditFinalizationOutcome::LegacyBypassNoExternalPublicationAuditFinalization;
    ...
    assert!(durable_completion_external_publication_audit_finalization_consumer_required());
    assert!(durable_completion_external_publication_audit_finalization_confirmation_required());
    assert!(durable_completion_external_publication_audit_finalization_no_real_settlement());
    assert!(durable_completion_external_publication_audit_finalization_no_real_settlement_finality());
    assert!(durable_completion_external_publication_audit_finalization_no_real_settlement_receipt());
    assert!(
        durable_completion_external_publication_audit_finalization_no_real_settlement_finality_projection()
    );
    assert!(
    ...
    assert!(durable_completion_external_publication_audit_finalization_no_real_external_publication_audit_finalization());

    let mut prod = ProductionExternalPublicationAuditFinalizationSink::default();
    let mut mainnet = MainNetExternalPublicationAuditFinalizationSink::default();
    let mut external = ExternalExternalPublicationAuditFinalizationSink::default();
    let mut prod_ledger = DurableCompletionExternalPublicationAuditFinalizationLedger::new();
    assert_eq!(
        prod.project_durable_completion_external_publication_audit_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_external_publication_audit_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_external_publication_audit_finalization(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_283_durable_completion_external_publication_acknowledgement_release_binary/helper_evidence/run_283")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("rejection_fail_closed", "non_recording_confirmation_outcomes_never_record_external_publication_audit_finalization", non_recording_confirmation_outcomes_never_record_external_publication_audit_finalization as fn()),
        ...
        ("rejection_fail_closed", "wrong_external_publication_audit_finalization_record_id_rejected_before_record", wrong_external_publication_audit_finalization_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request as fn()),
        ...
        ("recovery_crash_window", "pre_external_publication_acknowledgement_windows_fail_closed_no_commitment", pre_external_publication_acknowledgement_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_external_publication_acknowledgement_request_before_record_rejects_before_record", after_external_publication_acknowledgement_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_external_publication_acknowledgement_record_before_success_requires_explicit_matching_record", after_external_publication_acknowledgement_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_external_publication_acknowledgement_success_recovers_as_recorded", after_external_publication_acknowledgement_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_confirmation_outcome_creates_external_publication_audit_finalization_request_intent", only_recorded_confirmation_outcome_creates_external_publication_audit_finalization_request_intent as fn()),
        ("projection", "non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request as fn()),
        ...
        ("external_publication_audit_finalization_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("external_publication_audit_finalization_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("external_publication_audit_finalization_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_confirmation_outcomes_never_record_external_publication_audit_finalization", non_recording_confirmation_outcomes_never_record_external_publication_audit_finalization as fn()),
        ("non_mutation", "non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_finalization_request as fn()),
    ...
    summary.push_str(
        "Run 283 durable-completion external-publication-audit-finalization release helper\n",
    );
    summary.push_str(&format!(
        "verdict: {}\n",
        if total_fail == 0 { "PASS" } else { "FAIL" }
    ));
    summary.push_str("projection_rule: input.external_publication_acknowledgement_binding -> project_external_publication_acknowledgement_outcome_to_external_publication_audit_finalization_request\n");
    ...
    fs::write(
        outdir.join("fixtures/run_283_projection_rule.txt"),
        "input.external_publication_acknowledgement_binding\nproject_external_publication_acknowledgement_outcome_to_external_publication_audit_finalization_request\nExternalPublicationAcknowledgementRecorded -> CreateRequest\n",
