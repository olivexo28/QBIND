...
/// assert the resulting outcome.
fn x_reject_source(
    source: LiveEpochTransitionCommitExecutionAuthoritySource,
    expected: XO,
) {
    let mut c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
...
fn accept_artifact_reexposes_consumed_commit_authorization_transcript() {
    ...
    assert_eq!(art.commit_authorization_transcript_digest, handoff_transcript);
}
...
fn tampered_commit_authorization_package_integrity_mismatch() {
    ...
    let target = dec.commit_authorization_artifact.as_ref().unwrap().epoch_transition_target;
    let inputs = x_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.commit_authorization_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
    let request = ProductionLiveEpochTransitionCommitExecutionRequest::new(
        LiveEpochTransitionCommitExecutionAuthoritySource::VerifiedCommitAuthorizationDecision {
            decision: dec,
        },
...
fn wrong_governance_epoch() {
    x_reject_inputs(|i| i.expected_governance_epoch = 999, XO::WrongGovernanceEpoch);
}

fn wrong_proposal_id() {
    x_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), XO::WrongProposalId);
}
...
fn wrong_authority_sequence() {
    x_reject_inputs(|i| i.expected_authority_domain_sequence = 999, XO::WrongAuthoritySequence);
}
...
fn wrong_validator_set_epoch() {
    x_reject_inputs(|i| i.expected_validator_set_epoch = 999, XO::WrongValidatorSetEpoch);
}

fn wrong_validator_set_version() {
    x_reject_inputs(|i| i.expected_validator_set_version = 999, XO::WrongValidatorSetVersion);
}
...
fn wrong_proposed_validator_count() {
    x_reject_inputs(|i| i.expected_proposed_validator_count = 999, XO::WrongProposedValidatorCount);
}
...
fn wrong_epoch_transition_target_inputs() {
    x_reject_inputs(|i| i.expected_epoch_transition_target = 999, XO::WrongEpochTransitionTarget);
}
...
fn wrong_application_nonce() {
    x_reject_inputs(|i| i.expected_application_nonce = 999, XO::WrongApplicationNonce);
}

fn wrong_live_application_nonce() {
    x_reject_inputs(|i| i.expected_live_application_nonce = 999, XO::WrongLiveApplicationNonce);
}
...
fn replay_rejected_when_id_present() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = x_eval(&c);
    let id = d.commit_execution_artifact.as_ref().unwrap().request_id.clone();
    let replay = vec![id];
...
fn stale_governance_epoch() {
    x_reject_inputs(|i| i.min_governance_epoch = u64::MAX, XO::StaleGovernanceEpoch);
}

fn stale_authority_sequence() {
    x_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), XO::StaleAuthoritySequence);
}

fn stale_validator_set_epoch() {
    x_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, XO::StaleValidatorSetEpoch);
}

fn stale_validator_set_version() {
    x_reject_inputs(|i| i.min_validator_set_version = u64::MAX, XO::StaleValidatorSetVersion);
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let d = x_eval(&c);
    let art = d.commit_execution_artifact.as_ref().unwrap();
    let mut state = LiveEpochTransitionCommitExecutionFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn accept_authorizes_future_mutation_only() {
    ...
    assert!(d.authorizes_future_mutation_only());
    assert!(d.commit_execution_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
}
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = XK::from_staged_application_kind(LiveEpochTransitionCommitAuthorizationKind::UnsupportedStagedApplication);
    assert!(pk.is_unsupported());
}
...
fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    ...
    let ppkg = parent.commit_authorization_artifact.as_ref().unwrap().clone();
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = x_eval(&c);
    assert!(d.is_accept());
    let art = d.commit_execution_artifact.as_ref().unwrap();
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "accept_artifact_reexposes_consumed_commit_authorization_transcript", accept_artifact_reexposes_consumed_commit_authorization_transcript as fn()),
        ...
        ("rejection_fail_closed", "wrong_commit_authorization_decision_id", wrong_commit_authorization_decision_id as fn()),
        ("rejection_fail_closed", "wrong_commit_authorization_request_id", wrong_commit_authorization_request_id as fn()),
        ("rejection_fail_closed", "wrong_commit_authorization_intent_digest", wrong_commit_authorization_intent_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_authorization_transcript_digest", wrong_commit_authorization_transcript_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_authorization_nonce", wrong_commit_authorization_nonce as fn()),
        ("rejection_fail_closed", "tampered_commit_authorization_package_integrity_mismatch", tampered_commit_authorization_package_integrity_mismatch as fn()),
        ...
        ("rejection_fail_closed", "reject_missing_commit_authorization_decision", reject_missing_commit_authorization_decision as fn()),
        ("rejection_fail_closed", "reject_unverified_commit_authorization_decision", reject_unverified_commit_authorization_decision as fn()),
        ...
        ("rejection_fail_closed", "reject_accepted_commit_authorization_without_package_via_verified_source", reject_accepted_commit_authorization_without_package_via_verified_source as fn()),
        ("rejection_fail_closed", "reject_accepted_commit_authorization_without_package_variant", reject_accepted_commit_authorization_without_package_variant as fn()),
        ...
        ("rejection_fail_closed", "reject_fixture_only_commit_authorization", reject_fixture_only_commit_authorization as fn()),
    ...
    let content_digest =
        production_live_epoch_transition_commit_execution_content_digest(artifact);
    ...
    for sc in ALL_SC {
        ...
        digest_body.push_str(&format!("commit_execution_id={}\n", decision.commit_execution_id));
        digest_body.push_str(&format!("request_id={}\n", decision.request_id));
        digest_body.push_str(&format!("commit_execution_digest={}\n", decision.commit_execution_digest));
        digest_body.push_str(&format!(
            "content_digest={}\n",
            decision
                .commit_execution_artifact
    ...
    digest_body.push_str(&format!("named_commit_execution_id={commit_execution_id}\n"));
    ...
    let clean: ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_execution_window(
            None,
            current_artifact,
        );
    let idempotent: ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_execution_window(
    ...
    summary.push_str("run_322_production_live_epoch_transition_commit_execution_release_binary_helper\n");
