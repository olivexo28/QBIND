//! Run 322 — release-binary helper for the Run 321 **live epoch-transition
//! commit execution / finalization boundary**.
//!
//! Release-binary evidence for the Run 321 live epoch-transition commit
//! execution / finalization boundary
...
//! release-built production library symbols. It exercises the **real** Run 321
//! `ProductionLiveEpochTransitionCommitExecutionExecutor` and proves, per
...
//! -> 311 -> 313 -> 315 -> 317 -> 319 chain to produce a verified, accepted Run
//! 319/320 live epoch-transition commit-authorization decision (`is_accept()`
//! with `Some(commit_authorization_artifact)`), then feeds that decision into
//! the Run 321 live epoch-transition commit-execution executor, which emits
//! only a typed, deterministic, non-mutating live commit-execution/finalization
//! artifact.
//!
...
//! enablement, commit-execution default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! commit-execution/finalization artifacts; it never applies a live
//! validator-set change, never transitions a consensus epoch, never commits or
//! finalizes production runtime state, never calls Run 070, never mutates
...
//! `LiveEpochTransitionCommitExecutionFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_322.md`.
#![allow(dead_code)]
...
fn handoff_decision(
    ...
    assert!(
        d.is_accept(),
        "run 313 handoff decision must accept for fixture"
    );
    d
}
...
use LiveEpochTransitionCommitExecutionKind as XK;
use ProductionLiveEpochTransitionCommitExecutionOutcome as XO;

const CXE_POLICY_ID: &str = "commit-execution-policy-1";
...
/// Build an accepted Run 319/320 commit-authorization decision — the sole
/// accepted Run 321 commit-execution authority source.
fn ca_decision(
    env: TrustBundleEnvironment,
    sc: Sc,
) -> ProductionLiveEpochTransitionCommitAuthorizationDecision {
    ...
    assert!(
        d.is_accept(),
        "run 319 commit-authorization decision must accept for fixture"
    );
    d
}
...
fn x_inputs(
    ...
    ProductionLiveEpochTransitionCommitExecutionInputs {
        trust_domain: trust_domain(env),
        commit_execution_policy_id: CXE_POLICY_ID.to_string(),
        expected_authorization_policy_id: AUTH_POLICY_ID.to_string(),
        expected_application_policy_id: APP_POLICY_ID.to_string(),
        expected_governance_domain_id: GOV_DOMAIN.to_string(),
        expected_governance_epoch: GOV_EPOCH,
...
fn x_case(env: TrustBundleEnvironment, sc: Sc) -> X321 {
...
fn empty_replay321() -> EmptyLiveEpochTransitionCommitExecutionReplaySet {
    EmptyLiveEpochTransitionCommitExecutionReplaySet
}

fn x_eval(case: &X321) -> ProductionLiveEpochTransitionCommitExecutionDecision {
    case.executor
...
fn x_eval_replay(
    ...
    case.executor
        .evaluate_live_epoch_transition_commit_execution(&case.request, &case.inputs, &replay)
}

fn x_exec_with_policy(
    policy: ProductionLiveEpochTransitionCommitExecutionExecutorPolicy,
) -> ProductionLiveEpochTransitionCommitExecutionExecutor {
    ProductionLiveEpochTransitionCommitExecutionExecutor::new(
        ProductionLiveEpochTransitionCommitExecutionConfig::source_test(),
        policy,
    )
}
...
/// assert the resulting outcome (fail-closed, no artifact).
fn x_reject_inputs(
    mutate: impl FnOnce(&mut ProductionLiveEpochTransitionCommitExecutionInputs),
    expected: XO,
) {
    let mut c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    mutate(&mut c.inputs);
    let d = x_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
    assert!(d.commit_execution_artifact.is_none());
    assert!(!d.is_accept());
    assert!(d.outcome.is_non_mutating());
}
...
/// Common helper: build a Devnet/Add case, replace its authority source, and
/// assert the resulting outcome.
fn x_reject_source(source: LiveEpochTransitionCommitExecutionAuthoritySource, expected: XO) {
    let mut c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
    let d = x_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
...
fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        let c = x_case(TrustBundleEnvironment::Devnet, sc);
        let d = x_eval(&c);
        ...
        assert_eq!(art.staged_kind, expected_cxe_kind(sc));
...
fn accept_all_scenarios_testnet() {
    for sc in ALL_SC {
        let c = x_case(TrustBundleEnvironment::Testnet, sc);
        let d = x_eval(&c);
        assert!(d.is_accept());
        let art = d.commit_execution_artifact.as_ref().unwrap();
...
fn accept_outcome_carries_kind_env_target_nonce() {
    ...
    let d = x_eval(&c);
    match d.outcome {
        XO::AcceptedSourceTestLiveEpochTransitionCommitExecution {
            execution_kind,
            environment,
            epoch_transition_target,
            commit_execution_nonce,
        } => {
            assert_eq!(execution_kind, XK::StageApplyValidatorAdd);
            assert_eq!(environment, TrustBundleEnvironment::Devnet);
            assert_eq!(
                epoch_transition_target,
                c.request.proposed_epoch_transition_target
            );
            assert_eq!(commit_execution_nonce, CXE_NONCE);
        }
        other => panic!("unexpected outcome: {other:?}"),
    }
}

fn accept_artifact_reexposes_consumed_commit_authorization_transcript() {
    ...
    let target = dec
        ...
        .unwrap()
        .epoch_transition_target;
    let inputs = x_inputs(TrustBundleEnvironment::Devnet, &dec);
    let handoff_id = dec.commit_authorization_id.clone();
    let handoff_req = dec.request_id.clone();
    let handoff_digest = dec.commit_authorization_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionCommitExecutionExecutor::source_test();
    let d =
        exec.evaluate_live_epoch_transition_commit_execution(&request, &inputs, &empty_replay321());
    let art = d.commit_execution_artifact.as_ref().unwrap();
    assert_eq!(art.commit_authorization_decision_id, handoff_id);
    assert_eq!(art.commit_authorization_request_id, handoff_req);
    assert_eq!(art.commit_authorization_intent_digest, handoff_digest);
    assert_eq!(
        art.commit_authorization_transcript_digest,
        handoff_transcript
    );
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let d = x_eval(&c);
    let art = d.commit_execution_artifact.as_ref().unwrap();
    assert_eq!(d.commit_execution_id, art.commit_execution_id);
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(d.commit_execution_digest, art.commit_execution_digest);
    assert_eq!(d.transcript_digest, art.transcript_digest);
    assert!(!d.commit_execution_id.is_empty());
    assert!(!d.request_id.is_empty());
    assert!(!d.commit_execution_digest.is_empty());
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d1 = x_eval(&c);
        let d2 = x_eval(&c);
        assert_eq!(d1.commit_execution_id, d2.commit_execution_id);
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(d1.commit_execution_digest, d2.commit_execution_digest);
        assert_eq!(d1.transcript_digest, d2.transcript_digest);
    }
}

fn artifact_content_digest_is_stable() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
    let d = x_eval(&c);
    let art = d.commit_execution_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.commit_execution_digest);
...
fn wrong_commit_authorization_decision_id() {
    x_reject_inputs(
        |i| i.expected_commit_authorization_decision_id = "bad".to_string(),
        XO::CommitAuthorizationDecisionIdMismatch,
    );
}

fn wrong_commit_authorization_request_id() {
    x_reject_inputs(
        |i| i.expected_commit_authorization_request_id = "bad".to_string(),
        XO::CommitAuthorizationDecisionRequestIdMismatch,
    );
}

fn wrong_commit_authorization_intent_digest() {
    x_reject_inputs(
        |i| i.expected_commit_authorization_intent_digest = "bad".to_string(),
        XO::CommitAuthorizationDecisionIntentDigestMismatch,
    );
}

fn wrong_commit_authorization_transcript_digest() {
    x_reject_inputs(
        |i| i.expected_commit_authorization_transcript_digest = "bad".to_string(),
        XO::CommitAuthorizationDecisionTranscriptMismatch,
    );
}

fn wrong_commit_authorization_nonce() {
    x_reject_inputs(
        |i| i.expected_commit_authorization_nonce = 9999,
        XO::WrongCommitAuthorizationNonce,
    );
}

fn tampered_commit_authorization_package_integrity_mismatch() {
    ...
    let target = dec
        ...
        .epoch_transition_target;
    let inputs = x_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.commit_authorization_artifact
        .as_mut()
        .unwrap()
    ...
    let d =
        exec.evaluate_live_epoch_transition_commit_execution(&request, &inputs, &empty_replay321());
    assert_eq!(d.outcome, XO::CommitAuthorizationDecisionIntegrityMismatch);
    assert!(d.commit_execution_artifact.is_none());
}
...
fn wrong_guarded_mutation_decision_id() {
    x_reject_inputs(
        ...
        XO::GuardedMutationDecisionIdMismatch,
    );
}
...
fn wrong_guarded_mutation_request_id() {
    x_reject_inputs(
        ...
        XO::GuardedMutationDecisionRequestIdMismatch,
    );
}
...
fn wrong_guarded_mutation_intent_digest() {
    x_reject_inputs(
        ...
        XO::GuardedMutationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_guarded_mutation_transcript_digest() {
    x_reject_inputs(
        ...
        XO::GuardedMutationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_decision_id() {
    x_reject_inputs(
        ...
        XO::StagedApplicationDecisionIdMismatch,
    );
}
...
fn wrong_staged_application_request_id() {
    x_reject_inputs(
        ...
        XO::StagedApplicationDecisionRequestIdMismatch,
    );
}
...
fn wrong_staged_application_intent_digest() {
    x_reject_inputs(
        ...
        XO::StagedApplicationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_staged_application_transcript_digest() {
    x_reject_inputs(
        ...
        XO::StagedApplicationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_nonce() {
    x_reject_inputs(
        ...
        XO::WrongStagedApplicationNonce,
    );
}
...
fn wrong_authorization_decision_id() {
    x_reject_inputs(
        ...
        XO::AuthorizationDecisionIdMismatch,
    );
}
...
fn wrong_authorization_request_id() {
    x_reject_inputs(
        ...
        XO::AuthorizationDecisionRequestIdMismatch,
    );
}
...
fn wrong_authorization_intent_digest() {
    x_reject_inputs(
        ...
        XO::AuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_authorization_transcript_digest() {
    x_reject_inputs(
        ...
        XO::AuthorizationDecisionTranscriptMismatch,
    );
}
...
fn wrong_application_intent_digest() {
    x_reject_inputs(
        ...
        XO::WrongApplicationIntentDigest,
    );
}
...
fn wrong_application_transcript_digest() {
    x_reject_inputs(
        ...
        XO::WrongApplicationTranscriptDigest,
    );
}
...
fn wrong_governance_execution_intent_digest() {
    x_reject_inputs(
        ...
        XO::WrongGovernanceExecutionIntentDigest,
    );
}
...
fn wrong_rotation_transcript_digest() {
    x_reject_inputs(
        ...
        XO::WrongRotationTranscriptDigest,
    );
}
...
fn wrong_current_validator_set_digest() {
    x_reject_inputs(
        ...
        XO::WrongCurrentValidatorSetDigest,
    );
}
...
fn wrong_proposed_validator_set_digest() {
    x_reject_inputs(
        ...
        XO::WrongProposedValidatorSetDigest,
    );
}
...
fn wrong_validator_set_delta_digest() {
    x_reject_inputs(
        ...
        XO::WrongValidatorSetDeltaDigest,
    );
}
...
fn wrong_current_validator_set_epoch() {
    x_reject_inputs(
        ...
        XO::WrongCurrentValidatorSetEpoch,
    );
}
...
fn wrong_current_validator_set_version() {
    x_reject_inputs(
        ...
        XO::WrongCurrentValidatorSetVersion,
    );
}
...
fn wrong_proposed_validator_count() {
    x_reject_inputs(
        ...
        XO::WrongProposedValidatorCount,
    );
}

fn wrong_rotation_nonce() {
    x_reject_inputs(|i| i.expected_rotation_nonce = 999, XO::WrongRotationNonce);
}
...
fn wrong_epoch_transition_target_request() {
    ...
    let d = x_eval(&c);
    assert_eq!(d.outcome, XO::WrongEpochTransitionTarget);
    assert!(d.commit_execution_artifact.is_none());
}
...
fn reject_missing_commit_authorization_decision() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::MissingCommitAuthorizationDecision,
        XO::VerifiedCommitAuthorizationDecisionRequired,
    );
}

fn reject_unverified_commit_authorization_decision() {
    let dec = ca_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::UnverifiedCommitAuthorizationDecision {
            decision: dec,
        },
        XO::UnverifiedCommitAuthorizationDecisionRejected,
    );
}
...
fn reject_accepted_commit_authorization_without_package_via_verified_source() {
...
fn reject_accepted_commit_authorization_without_package_variant() {
    let dec = ca_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::AcceptedCommitAuthorizationWithoutPackage {
            decision: dec,
        },
        XO::VerifiedCommitAuthorizationDecisionRequired,
    );
...
fn reject_guarded_mutation_decision_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::GuardedMutationDecisionWithoutCommitAuthorization,
        XO::GuardedMutationDecisionAloneRejected,
    );
}

fn reject_staged_application_decision_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::StagedApplicationDecisionWithoutCommitAuthorization,
        XO::StagedApplicationDecisionAloneRejected,
    );
}

fn reject_live_application_authorization_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::LiveApplicationAuthorizationWithoutCommitAuthorization,
        XO::LiveApplicationAuthorizationAloneRejected,
    );
}

fn reject_application_decision_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::ApplicationDecisionWithoutCommitAuthorization,
        XO::ApplicationDecisionAloneRejected,
    );
}

fn reject_rotation_plan_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::RotationPlanWithoutCommitAuthorization,
        XO::RotationPlanAloneRejected,
    );
}

fn reject_governance_execution_intent_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::GovernanceExecutionIntentWithoutCommitAuthorization,
        XO::GovernanceExecutionIntentAloneRejected,
    );
}

fn reject_governance_proof_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::GovernanceProofWithoutCommitAuthorization,
        XO::GovernanceProofAloneRejected,
    );
}

fn reject_local_operator_assertion() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::LocalOperatorAssertion,
        XO::LocalOperatorProofRejected,
    );
}

fn reject_peer_majority_assertion() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::PeerMajorityAssertion,
        XO::PeerMajorityProofRejected,
    );
}

fn reject_custody_only_evidence() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::CustodyOnlyEvidence,
        XO::CustodyOnlyProofRejected,
    );
}

fn reject_remote_signer_only_evidence() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::RemoteSignerOnlyEvidence,
        XO::RemoteSignerOnlyProofRejected,
    );
}

fn reject_custody_attestation_only_evidence() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::CustodyAttestationOnlyEvidence,
        XO::CustodyAttestationOnlyProofRejected,
    );
}

fn reject_fixture_only_commit_authorization() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::FixtureOnlyCommitAuthorization,
        XO::FixtureStagedApplicationRejectedAsProductionAuthority,
    );
}

fn reject_arbitrary_validator_set_bytes() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::ArbitraryValidatorSetBytes,
        XO::ArbitraryValidatorSetBytesRejected,
    );
}
...
fn mainnet_domain_refused_under_source_test_policy() {
    ...
    let d = x_eval(&c);
    assert_eq!(d.outcome, XO::MainNetRefused);
    assert!(d.commit_execution_artifact.is_none());
}

fn mainnet_policy_unavailable() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = x_exec_with_policy(
        ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::MainnetProductionLiveEpochTransitionCommitExecutionRequired,
    );
    let d = exec.evaluate_live_epoch_transition_commit_execution(
        &c.request,
        &c.inputs,
        &empty_replay321(),
    );
    assert_eq!(
        d.outcome,
        XO::MainNetProductionLiveEpochTransitionCommitExecutionUnavailable
    );
    assert!(d.commit_execution_artifact.is_none());
}
...
fn production_policy_unavailable() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = x_exec_with_policy(
        ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::RequireProductionLiveEpochTransitionCommitExecution,
    );
    let d = exec.evaluate_live_epoch_transition_commit_execution(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        XO::ProductionLiveEpochTransitionCommitExecutionUnavailable
    );
    assert!(d.commit_execution_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec =
        x_exec_with_policy(ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::Disabled);
    let d = exec.evaluate_live_epoch_transition_commit_execution(
        &c.request,
        &c.inputs,
        &empty_replay321(),
    );
    assert_eq!(d.outcome, XO::Disabled);
    assert!(d.commit_execution_artifact.is_none());
    assert!(!d.is_accept());
}
...
fn reserved_production_kind_fails_closed() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionCommitExecutionExecutor::new(
        ProductionLiveEpochTransitionCommitExecutionConfig::new(
            ProductionLiveEpochTransitionCommitExecutionExecutorKind::ProductionLiveEpochTransitionCommitExecution,
        ),
        ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::AllowSourceTestLiveEpochTransitionCommitExecution,
    );
    let d = exec.evaluate_live_epoch_transition_commit_execution(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        XO::LiveEpochTransitionCommitExecutionBoundaryUnavailable
    );
}
...
fn replay_rejected_when_id_present() {
    ...
    let id = d
        ...
        .clone();
    let replay = vec![id];
    let d2 = x_eval_replay(&c, &replay);
    match d2.outcome {
        XO::StagedApplicationReplayRejected { .. } => {}
        other => panic!("expected replay rejection, got {other:?}"),
    }
    assert!(d2.commit_execution_artifact.is_none());
}

fn no_replay_when_id_absent() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let replay: Vec<String> = vec!["some-other-id".to_string()];
    let d = x_eval_replay(&c, &replay);
    assert!(d.is_accept());
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.commit_execution_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionCommitExecutionFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
    assert!(state.apply_prepared_execution(art, &d.commit_execution_id));
    ...
    assert!(!state.apply_prepared_execution(art, &d.commit_execution_id));
    assert!(state.has_applied(&d.commit_execution_id));
}
...
fn every_outcome_is_non_mutating() {
    ...
    let d = x_eval(&c);
    assert!(d.outcome.is_non_mutating());
    let bad = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let dbad = x_eval_replay(&bad, &[x_eval(&bad).commit_execution_id]);
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionCommitExecutionExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionCommitExecution.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionCommitExecution.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionCommitExecutionRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}

fn kind_predicates() {
    use ProductionLiveEpochTransitionCommitExecutionExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionCommitExecution.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionCommitExecution.is_source_test());
    assert_eq!(K::default(), K::Disabled);
}

fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        let hk = expected_cmt_kind(sc);
        let pk = XK::from_staged_application_kind(hk);
        assert_eq!(pk, expected_cxe_kind(sc));
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = XK::from_staged_application_kind(
        LiveEpochTransitionCommitAuthorizationKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = XO::CommitAuthorizationDecisionIdMismatch;
    let b = XO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), XO::CommitAuthorizationDecisionIdMismatch.tag());
    assert!(!XO::MainNetRefused.tag().is_empty());
}

fn config_and_inputs_well_formed() {
    let c = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    assert!(c.inputs.is_well_formed());
    assert!(ProductionLiveEpochTransitionCommitExecutionConfig::source_test().is_well_formed());
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionCommitExecutionConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d2 = x_eval(&c);
            assert_eq!(d.commit_execution_digest, d2.commit_execution_digest);
            assert_eq!(d.transcript_digest, d2.transcript_digest);
            let mut state = LiveEpochTransitionCommitExecutionFixtureState::new(
                CUR_EPOCH,
                CUR_VERSION,
                "start",
            );
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.commit_execution_artifact.as_ref().unwrap().environment,
                TrustBundleEnvironment::Testnet
            );
        }
    };
...
macro_rules! per_scenario_commit_authorization_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            let mut c = x_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_commit_authorization_decision_id = "bad".to_string();
            let d = x_eval(&c);
            assert_eq!(d.outcome, XO::CommitAuthorizationDecisionIdMismatch);
            assert!(d.commit_execution_artifact.is_none());
        }
    };
}

per_scenario_commit_authorization_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_commit_authorization_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
macro_rules! per_scenario_guarded_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d = x_eval(&c);
            assert_eq!(d.outcome, XO::GuardedMutationDecisionIdMismatch);
            assert!(d.commit_execution_artifact.is_none());
        }
    };
}
...
// ===========================================================================
// Q. Deeper re-exposed Run 315/316 runtime-handoff authority-tuple binding
//    (additive layer introduced by Run 319 on top of the rotated Run 317
...
fn reject_runtime_handoff_decision_id_mismatch() {
    x_reject_inputs(
        ...
        XO::RuntimeHandoffDecisionIdMismatch,
    );
}
...
fn reject_runtime_handoff_request_id_mismatch() {
    x_reject_inputs(
        ...
        XO::RuntimeHandoffDecisionRequestIdMismatch,
    );
}
...
fn reject_runtime_handoff_intent_digest_mismatch() {
    x_reject_inputs(
        ...
        XO::RuntimeHandoffDecisionIntentDigestMismatch,
    );
}
...
fn reject_runtime_handoff_transcript_mismatch() {
    x_reject_inputs(
        ...
        XO::RuntimeHandoffDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_runtime_handoff_nonce() {
    x_reject_inputs(
        ...
        XO::WrongRuntimeHandoffNonce,
    );
}

fn reject_runtime_handoff_decision_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::RuntimeHandoffDecisionWithoutCommitAuthorization,
        XO::RuntimeHandoffDecisionAloneRejected,
    );
}

fn runtime_handoff_alone_tag_is_stable() {
    assert_eq!(
        XO::RuntimeHandoffDecisionAloneRejected.tag(),
        "runtime-handoff-decision-alone-rejected"
    );
    assert_eq!(
        XO::RuntimeHandoffDecisionIdMismatch.tag(),
        "runtime-handoff-decision-id-mismatch"
    );
    assert_eq!(
        XO::WrongRuntimeHandoffNonce.tag(),
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = x_eval(&c);
    let d2 = x_eval(&c);
    let a1 = d1.commit_execution_artifact.as_ref().unwrap();
    let a2 = d2.commit_execution_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 321 — re-exposed Run 317/318 execution-preparation ancestor binding
...
fn reject_execution_preparation_decision_id_mismatch() {
    x_reject_inputs(
        ...
        XO::ExecutionPreparationDecisionIdMismatch,
    );
}
...
fn reject_execution_preparation_request_id_mismatch() {
    x_reject_inputs(
        ...
        XO::ExecutionPreparationDecisionRequestIdMismatch,
    );
}
...
fn reject_execution_preparation_intent_digest_mismatch() {
    x_reject_inputs(
        ...
        XO::ExecutionPreparationDecisionIntentDigestMismatch,
    );
}
...
fn reject_execution_preparation_transcript_mismatch() {
    x_reject_inputs(
        ...
        XO::ExecutionPreparationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_execution_preparation_nonce() {
    x_reject_inputs(
        ...
        XO::WrongExecutionPreparationNonce,
    );
}

fn reject_execution_preparation_decision_alone() {
    x_reject_source(
        LiveEpochTransitionCommitExecutionAuthoritySource::ExecutionPreparationDecisionWithoutCommitAuthorization,
        XO::ExecutionPreparationDecisionAloneRejected,
    );
}

fn execution_preparation_alone_tag_is_stable() {
    assert_eq!(
        XO::ExecutionPreparationDecisionAloneRejected.tag(),
        "execution-preparation-decision-alone-rejected"
    );
    assert_eq!(
        XO::ExecutionPreparationDecisionIdMismatch.tag(),
        "execution-preparation-decision-id-mismatch"
    );
    assert_eq!(
        XO::WrongExecutionPreparationNonce.tag(),
...
fn accepted_artifact_re_exposes_execution_preparation_ancestor() {
    ...
    assert!(d.is_accept());
    let art = d.commit_execution_artifact.as_ref().unwrap();
    // Grandparent Run 317/318 execution-preparation nonce re-exposed intact.
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_322_production_live_epoch_transition_commit_execution_release_binary/helper_evidence/run_322",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_commit_authorization_transcript",
            accept_artifact_reexposes_consumed_commit_authorization_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_commit_authorization_decision_id",
            wrong_commit_authorization_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_authorization_request_id",
            wrong_commit_authorization_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_authorization_intent_digest",
            wrong_commit_authorization_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_authorization_transcript_digest",
            wrong_commit_authorization_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_authorization_nonce",
            wrong_commit_authorization_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_commit_authorization_package_integrity_mismatch",
            tampered_commit_authorization_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_commit_authorization_decision",
            reject_missing_commit_authorization_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_commit_authorization_decision",
            reject_unverified_commit_authorization_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_commit_authorization_without_package_via_verified_source",
            reject_accepted_commit_authorization_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_commit_authorization_without_package_variant",
            reject_accepted_commit_authorization_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_commit_authorization",
            reject_fixture_only_commit_authorization as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 321 domain-separated digest free functions.
    let case = x_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = x_eval(&case);
    let artifact = decision
        .commit_execution_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live commit-execution artifact");
    // On accept, the executor derives the identifiers from the consumed Run
    // 319/320 commit-authorization decision's commit-authorization (content)
    // digest.
    let ca_dec = ca_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let commit_authorization_intent_digest = ca_dec.commit_authorization_digest.clone();
    let content_digest = production_live_epoch_transition_commit_execution_content_digest(artifact);
    let commit_execution_id = production_live_epoch_transition_commit_execution_id(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION,
        &commit_authorization_intent_digest,
        &case.inputs.commit_execution_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.commit_execution_nonce,
    );
    let request_id = production_live_epoch_transition_commit_execution_request_id(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION,
        &commit_authorization_intent_digest,
        &case.inputs.commit_execution_policy_id,
        case.request.proposed_epoch_transition_target,
    ...
    let transcript_digest = production_live_epoch_transition_commit_execution_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION,
        &request_id,
        &content_digest,
        outcome_tag,
    ...
    digest_body.push_str(&format!(
        "commit_authorization_intent_digest={commit_authorization_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = x_case(TrustBundleEnvironment::Devnet, sc);
        let decision = x_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "commit_execution_id={}\n",
            decision.commit_execution_id
        ));
    ...
    digest_body.push_str(&format!(
        "named_commit_execution_id={commit_execution_id}\n"
    ));
    ...
    fs::write(
        outdir.join("fixtures/run_322_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 321 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionCommitExecutionArtifact` and its
    // `ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionCommitExecutionExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionCommitExecutionArtifact = artifact;
    let clean: ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_execution_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome = recover_exec
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_322_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_commit_execution.rs :: ProductionLiveEpochTransitionCommitExecutionExecutor (Run 321 source/test boundary)\n");
