...
//! Run 346 — release-binary evidence helper for the Run 345 live epoch-transition
//! authority-activation authorization / final-execution readiness boundary.
//!
//! This example compiles as a release binary and executes the real
//! `ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor` from
//! `crates/qbind-node/src/pqc_production_live_epoch_transition_authority_activation_authorization.rs`.
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_346.md`.
...
// ===========================================================================
mod run_345_authority_activation_authorization {
    #![allow(unused_imports)]
    #![allow(dead_code)]
    #![allow(unused_variables)]
    use super::*;
    ...
    fn accept_all_scenarios_devnet() {
        for sc in ALL_SC {
            ...
            let art = d
                ...
                .as_ref()
                .unwrap();
            assert_eq!(art.staged_kind, expected_crc11_kind(sc));
            assert_eq!(art.authority_activation_authorization_nonce, CRC11_NONCE);
            assert_eq!(art.post_completion_attestation_nonce, CRC4_NONCE);
            assert_eq!(art.guarded_mutation_nonce, GUARDED_NONCE);
            assert_eq!(art.staged_application_nonce, STAGED_NONCE);
        }
    }
    ...
    fn accept_outcome_carries_kind_env_target_nonce() {
        ...
        let d = c_eval(&c);
        match d.outcome {
            CO::AcceptedSourceTestLiveEpochTransitionAuthorityActivationAuthorization {
                execution_kind,
                environment,
                epoch_transition_target,
                ...
                assert_eq!(authority_activation_authorization_nonce, CRC11_NONCE);
            }
            other => panic!("unexpected outcome: {other:?}"),
        }
    }

    fn accept_artifact_reexposes_consumed_post_completion_attestation_transcript() {
        let dec = cc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec
            .post_completion_attestation_artifact
            .as_ref()
            .unwrap()
        ...
        let handoff_id = dec.post_completion_attestation_id.clone();
        let handoff_req = dec.request_id.clone();
        let handoff_digest = dec.post_completion_attestation_digest.clone();
        let handoff_transcript = dec.transcript_digest.clone();
        let request = ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest::new(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::VerifiedPostCompletionAttestationDecision {
                decision: dec,
        ...
        let exec =
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor::source_test();
        let d = exec.evaluate_live_epoch_transition_authority_activation_authorization(
            &request,
            &inputs,
            &empty_replay345(),
        );
        let art = d
            ...
            .as_ref()
            .unwrap();
        assert_eq!(art.post_completion_attestation_decision_id, handoff_id);
        assert_eq!(art.post_completion_attestation_request_id, handoff_req);
        assert_eq!(
            art.post_completion_attestation_intent_digest,
            handoff_digest
        );
        assert_eq!(
            art.post_completion_attestation_transcript_digest,
            handoff_transcript
        );
    }
    ...
    fn accept_decision_ids_match_artifact_ids() {
        ...
        let art = d
            ...
            .unwrap();
        assert_eq!(
            d.authority_activation_authorization_id,
            art.authority_activation_authorization_id
        );
        assert_eq!(d.request_id, art.request_id);
        assert_eq!(
            d.authority_activation_authorization_digest,
            art.authority_activation_authorization_digest
        );
        assert_eq!(d.transcript_digest, art.transcript_digest);
        assert!(!d.authority_activation_authorization_id.is_empty());
        assert!(!d.request_id.is_empty());
        assert!(!d.authority_activation_authorization_digest.is_empty());
        assert!(!d.transcript_digest.is_empty());
    }
    ...
    fn deterministic_digests_under_reevaluation() {
        for sc in ALL_SC {
            ...
            let d2 = c_eval(&c);
            assert_eq!(
                d1.authority_activation_authorization_id,
                d2.authority_activation_authorization_id
            );
            assert_eq!(d1.request_id, d2.request_id);
            assert_eq!(
                d1.authority_activation_authorization_digest,
                d2.authority_activation_authorization_digest
            );
            assert_eq!(d1.transcript_digest, d2.transcript_digest);
        }
    }
    ...
    fn wrong_post_completion_attestation_decision_id() {
        c_reject_inputs(
            |i| i.expected_post_completion_attestation_decision_id = "bad".to_string(),
            CO::PostCompletionAttestationDecisionIdMismatch,
        );
    }

    fn wrong_post_completion_attestation_request_id() {
        c_reject_inputs(
            |i| i.expected_post_completion_attestation_request_id = "bad".to_string(),
            CO::PostCompletionAttestationDecisionRequestIdMismatch,
        );
    }

    fn wrong_post_completion_attestation_intent_digest() {
        c_reject_inputs(
            |i| i.expected_post_completion_attestation_intent_digest = "bad".to_string(),
            CO::PostCompletionAttestationDecisionIntentDigestMismatch,
        );
    }

    fn wrong_post_completion_attestation_transcript_digest() {
        c_reject_inputs(
            |i| i.expected_post_completion_attestation_transcript_digest = "bad".to_string(),
            CO::PostCompletionAttestationDecisionTranscriptMismatch,
        );
    }

    fn wrong_post_completion_attestation_nonce() {
        c_reject_inputs(
            |i| i.expected_post_completion_attestation_nonce = 9999,
            CO::WrongPostCompletionAttestationNonce,
        );
    }

    fn tampered_post_completion_attestation_package_integrity_mismatch() {
        // Mutate the consumed package so its content digest no longer matches the
        // bound handoff decision digest.
        let mut dec = cc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let target = dec
            .post_completion_attestation_artifact
            .as_ref()
            .unwrap()
            .epoch_transition_target;
        let inputs = c_inputs(TrustBundleEnvironment::Devnet, &dec);
        dec.post_completion_attestation_artifact
            .as_mut()
            .unwrap()
            .proposal_digest = "tampered".to_string();
        let request = ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest::new(
        ...
        assert_eq!(
            d.outcome,
            CO::PostCompletionAttestationDecisionIntegrityMismatch
        );
        assert!(d.authority_activation_authorization_artifact.is_none());
    }
    ...
    fn wrong_proposed_validator_count() {
        c_reject_inputs(
            |i| i.expected_proposed_validator_count = 999,
            CO::WrongProposedValidatorCount,
        );
    }
    ...
    fn reject_missing_post_completion_attestation_decision() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::MissingPostCompletionAttestationDecision,
            CO::VerifiedPostCompletionAttestationDecisionRequired,
        );
    }

    fn reject_unverified_post_completion_attestation_decision() {
        let dec = cc_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::UnverifiedPostCompletionAttestationDecision {
                decision: dec,
            },
            CO::UnverifiedPostCompletionAttestationDecisionRejected,
        );
    }
    ...
    fn reject_accepted_post_completion_attestation_without_package_via_verified_source() {
        let dec = cc_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::VerifiedPostCompletionAttestationDecision {
                decision: dec,
            },
    ...
    fn reject_accepted_post_completion_attestation_without_package_variant() {
        let dec = cc_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::AcceptedPostCompletionAttestationWithoutPackage {
                decision: dec,
            },
            CO::VerifiedPostCompletionAttestationDecisionRequired,
        );
    ...
    fn reject_guarded_mutation_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::GuardedMutationDecisionWithoutPostCompletionAttestation,
    ...
    fn reject_staged_application_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::StagedApplicationDecisionWithoutPostCompletionAttestation,
    ...
    fn reject_live_application_authorization_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::LiveApplicationAuthorizationWithoutPostCompletionAttestation,
    ...
    fn reject_application_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::ApplicationDecisionWithoutPostCompletionAttestation,
    ...
    fn reject_rotation_plan_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::RotationPlanWithoutPostCompletionAttestation,
    ...
    fn reject_governance_execution_intent_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::GovernanceExecutionIntentWithoutPostCompletionAttestation,
    ...
    fn reject_governance_proof_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::GovernanceProofWithoutPostCompletionAttestation,
    ...
    fn reject_local_operator_assertion() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::LocalOperatorAssertion,
    ...
    fn reject_peer_majority_assertion() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::PeerMajorityAssertion,
    ...
    fn reject_custody_only_evidence() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::CustodyOnlyEvidence,
    ...
    fn reject_remote_signer_only_evidence() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::RemoteSignerOnlyEvidence,
    ...
    fn reject_custody_attestation_only_evidence() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::CustodyAttestationOnlyEvidence,
            CO::CustodyAttestationOnlyProofRejected,
        );
    }

    fn reject_fixture_only_post_completion_attestation() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::FixtureOnlyPostCompletionAttestation,
    ...
    fn reject_arbitrary_validator_set_bytes() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::ArbitraryValidatorSetBytes,
    ...
    fn mainnet_policy_unavailable() {
        ...
        let exec = c_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_authorization(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            CO::MainNetProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable
        );
        assert!(d.authority_activation_authorization_artifact.is_none());
    }
    ...
    fn production_policy_unavailable() {
        let c = c_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = c_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::RequireProductionLiveEpochTransitionAuthorityActivationAuthorization,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_authorization(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            CO::ProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable
        );
        assert!(d.authority_activation_authorization_artifact.is_none());
    }
    ...
    fn disabled_policy_fails_closed() {
        let c = c_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = c_exec_with_policy(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::Disabled,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_authorization(
            &c.request,
            &c.inputs,
    ...
    fn reserved_production_kind_fails_closed() {
        let c = c_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let exec = ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor::new(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig::new(
                ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::ProductionLiveEpochTransitionAuthorityActivationAuthorization,
            ),
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::AllowSourceTestLiveEpochTransitionAuthorityActivationAuthorization,
        );
        let d = exec.evaluate_live_epoch_transition_authority_activation_authorization(
            &c.request,
            &c.inputs,
        ...
        assert_eq!(
            d.outcome,
            CO::LiveEpochTransitionAuthorityActivationAuthorizationBoundaryUnavailable
        );
    }
    ...
    fn replay_rejected_when_id_present() {
        ...
        assert!(d2.authority_activation_authorization_artifact.is_none());
    }
    ...
    fn fixture_state_apply_is_idempotent() {
        ...
        let art = d
            ...
            .as_ref()
            .unwrap();
        let mut state = LiveEpochTransitionAuthorityActivationAuthorizationFixtureState::new(
            CUR_EPOCH,
            CUR_VERSION,
            "start",
        );
        assert!(state.apply_prepared_execution(art, &d.authority_activation_authorization_id));
        assert_eq!(state.current_epoch, art.epoch_transition_target);
        assert_eq!(state.validator_set_version, art.validator_set_version);
        assert_eq!(state.current_set_digest, art.proposed_set_digest);
        // Re-applying the same id is a no-op.
        assert!(!state.apply_prepared_execution(art, &d.authority_activation_authorization_id));
        assert!(state.has_applied(&d.authority_activation_authorization_id));
    }
    ...
    fn every_outcome_is_non_mutating() {
        ...
        let dbad = c_eval_replay(&bad, &[c_eval(&bad).authority_activation_authorization_id]);
        assert!(dbad.outcome.is_non_mutating());
    }
    ...
    fn policy_predicates() {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy as Pol;
        assert!(Pol::Disabled.is_disabled());
        assert!(
            Pol::AllowSourceTestLiveEpochTransitionAuthorityActivationAuthorization
                .allows_source_test()
        );
        assert!(
            Pol::RequireProductionLiveEpochTransitionAuthorityActivationAuthorization
                .is_production()
        );
        assert!(
            Pol::MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired
                .is_mainnet()
        );
        assert_eq!(Pol::default(), Pol::Disabled);
    }
    ...
    fn kind_predicates() {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind as K;
        assert!(K::SourceTestLiveEpochTransitionAuthorityActivationAuthorization.is_source_test());
        assert!(!K::Disabled.is_source_test());
        assert!(!K::ProductionLiveEpochTransitionAuthorityActivationAuthorization.is_source_test());
        assert_eq!(K::default(), K::Disabled);
    }

    fn execution_kind_mapping_matches_handoff_kind() {
        for sc in ALL_SC {
            let hk = expected_crc10_kind(sc);
            let pk = CK::from_staged_application_kind(hk);
            assert_eq!(pk, expected_crc11_kind(sc));
            assert!(pk.is_non_mutating());
            assert!(!pk.is_unsupported());
        }
    }
    ...
    fn unsupported_staged_application_kind_is_unsupported() {
        let pk = CK::from_staged_application_kind(
            LiveEpochTransitionPostCompletionAttestationKind::UnsupportedStagedApplication,
        );
        assert!(pk.is_unsupported());
    }

    fn outcome_tags_are_stable_and_distinct() {
        let a = CO::PostCompletionAttestationDecisionIdMismatch;
        let b = CO::GuardedMutationDecisionIdMismatch;
        assert_ne!(a.tag(), b.tag());
        assert_eq!(
            a.tag(),
            CO::PostCompletionAttestationDecisionIdMismatch.tag()
        );
    ...
    fn config_and_inputs_well_formed() {
        ...
        // The default config still pins the supported protocol version.
        assert!(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig::default()
                .is_well_formed()
        );
    }
    ...
    macro_rules! per_scenario_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert_eq!(d.transcript_digest, d2.transcript_digest);
                let mut state =
                    LiveEpochTransitionAuthorityActivationAuthorizationFixtureState::new(
                        CUR_EPOCH,
                        CUR_VERSION,
                        "start",
                    );
                assert!(
                    state.apply_prepared_execution(art, &d.authority_activation_authorization_id)
                );
                assert_eq!(state.current_epoch, art.epoch_transition_target);
            }
        };
    ...
    macro_rules! per_scenario_testnet_accept {
        ($name:ident, $sc:expr) => {
            fn $name() {
                ...
                assert!(d.is_accept());
                assert_eq!(
                    d.authority_activation_authorization_artifact
                        .as_ref()
                        .unwrap()
                        .environment,
                    TrustBundleEnvironment::Testnet
    ...
    macro_rules! per_scenario_post_completion_attestation_binding {
        ($name:ident, $sc:expr) => {
            fn $name() {
                let mut c = c_case(TrustBundleEnvironment::Devnet, $sc);
                c.inputs.expected_post_completion_attestation_decision_id = "bad".to_string();
                let d = c_eval(&c);
                assert_eq!(d.outcome, CO::PostCompletionAttestationDecisionIdMismatch);
                assert!(d.authority_activation_authorization_artifact.is_none());
            }
        };
    }

    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_add, Sc::Add);
    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_remove, Sc::Remove);
    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_update, Sc::Update);
    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_noop, Sc::NoOp);
    per_scenario_post_completion_attestation_binding!(
        scenario_handoff_binding_identity,
        Sc::Identity
    );
    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_retire, Sc::Retire);
    per_scenario_post_completion_attestation_binding!(
        scenario_handoff_binding_emergency,
        Sc::Emergency
    );
    ...
    per_scenario_post_completion_attestation_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
    ...
    fn accept_reexposes_runtime_handoff_tuple_from_parent() {
        let parent = cc_decision(TrustBundleEnvironment::Devnet, Sc::Add);
        let ppkg = parent
            .post_completion_attestation_artifact
            .as_ref()
            .unwrap()
    ...
    fn reject_runtime_handoff_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::RuntimeHandoffDecisionWithoutPostCompletionAttestation,
    ...
    // ===========================================================================
    // Run 337 — re-exposed Run 329/330 execution-preparation ancestor binding
    // (carried through the consumed Run 335/336 post-completion-attestation artifact) and
    // its alone-rejected authority variant.
    // ===========================================================================
    ...
    fn reject_mutation_execution_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::MutationExecutionDecisionWithoutPostCompletionAttestation,
            CO::MutationExecutionDecisionAloneRejected,
        );
    }
    ...
    fn reject_commit_authorization_decision_id_mismatch() {
        c_reject_inputs(
            |i| i.expected_commit_authorization_decision_id = "wrong-commit-auth-id".to_string(),
            CO::CommitAuthorizationDecisionIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_request_id_mismatch() {
        c_reject_inputs(
            |i| i.expected_commit_authorization_request_id = "wrong-commit-auth-req".to_string(),
            CO::CommitAuthorizationDecisionRequestIdMismatch,
        );
    }
    ...
    fn reject_commit_authorization_intent_digest_mismatch() {
        c_reject_inputs(
            ...
            CO::CommitAuthorizationDecisionIntentDigestMismatch,
        );
    }
    ...
    fn reject_commit_authorization_transcript_mismatch() {
        c_reject_inputs(
            ...
            CO::CommitAuthorizationDecisionTranscriptMismatch,
        );
    }
    ...
    fn reject_wrong_commit_authorization_nonce() {
        c_reject_inputs(
            |i| i.expected_commit_authorization_nonce = CMT_NONCE + 100,
            CO::WrongCommitAuthorizationNonce,
        );
    }

    fn reject_commit_authorization_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::CommitAuthorizationDecisionWithoutPostCompletionAttestation,
            CO::CommitAuthorizationDecisionAloneRejected,
        );
    }
    ...
    fn mutation_execution_alone_tag_is_stable() {
        assert_eq!(
            CO::MutationExecutionDecisionAloneRejected.tag(),
            "mutation-execution-decision-alone-rejected"
        );
        assert_eq!(
            CO::MutationExecutionDecisionIdMismatch.tag(),
            "mutation-execution-decision-id-mismatch"
        );
        assert_eq!(
            CO::WrongMutationExecutionNonce.tag(),
            "wrong-mutation-execution-nonce"
        );
    }
    ...
    fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
        ...
        // Grandparent Run 329/330 mutation-execution nonce re-exposed intact.
        assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
        // Parent Run 335/336 post-completion-attestation nonce re-exposed intact.
        assert_eq!(art.post_completion_attestation_nonce, CRC4_NONCE);
        // Self Run 337 authority-activation-authorization proposed nonce.
        assert_eq!(art.authority_activation_authorization_nonce, CRC11_NONCE);
    }
    ...
    fn reject_execution_preparation_decision_alone() {
        c_reject_source(
            LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::ExecutionPreparationDecisionWithoutPostCompletionAttestation,
    ...
    pub fn helper_main() {
        let outdir = std::env::args().nth(1)
            ...
            .unwrap_or_else(|| std::path::PathBuf::from("docs/devnet/run_346_production_live_epoch_transition_authority_activation_authorization_release_binary/helper_evidence/run_346"));
        ...
        let cases: &[(&str, &str, fn())] = &[
            ...
            (
                "accepted_compatible",
                "accept_artifact_reexposes_consumed_post_completion_attestation_transcript",
                accept_artifact_reexposes_consumed_post_completion_attestation_transcript as fn(),
            ),
            (
                "accepted_compatible",
                "accept_artifact_encodes_future_executor_preconditions",
                accept_artifact_encodes_future_executor_preconditions as fn(),
            ),
            (
                "accepted_compatible",
                "accept_decision_ids_match_artifact_ids",
                accept_decision_ids_match_artifact_ids as fn(),
            ),
            (
                "rejection_fail_closed",
                "deterministic_digests_under_reevaluation",
                deterministic_digests_under_reevaluation as fn(),
            ),
            (
                "rejection_fail_closed",
                "artifact_content_digest_is_stable",
                artifact_content_digest_is_stable as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_post_completion_attestation_decision_id",
                wrong_post_completion_attestation_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_post_completion_attestation_request_id",
                wrong_post_completion_attestation_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_post_completion_attestation_intent_digest",
                wrong_post_completion_attestation_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_post_completion_attestation_transcript_digest",
                wrong_post_completion_attestation_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_post_completion_attestation_nonce",
                wrong_post_completion_attestation_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "tampered_post_completion_attestation_package_integrity_mismatch",
                tampered_post_completion_attestation_package_integrity_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_guarded_mutation_decision_id",
                wrong_guarded_mutation_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_guarded_mutation_request_id",
                wrong_guarded_mutation_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_guarded_mutation_intent_digest",
                wrong_guarded_mutation_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_guarded_mutation_transcript_digest",
                wrong_guarded_mutation_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_guarded_mutation_nonce",
                wrong_guarded_mutation_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_staged_application_decision_id",
                wrong_staged_application_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_staged_application_request_id",
                wrong_staged_application_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_staged_application_intent_digest",
                wrong_staged_application_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_staged_application_transcript_digest",
                wrong_staged_application_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_staged_application_nonce",
                wrong_staged_application_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authorization_decision_id",
                wrong_authorization_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authorization_request_id",
                wrong_authorization_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authorization_intent_digest",
                wrong_authorization_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authorization_transcript_digest",
                wrong_authorization_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_decision_id",
                wrong_application_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_request_id",
                wrong_application_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_intent_digest",
                wrong_application_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_transcript_digest",
                wrong_application_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_policy_id",
                wrong_application_policy_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authorization_policy_id",
                wrong_authorization_policy_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_environment",
                wrong_environment as fn(),
            ),
            ...
            (
                ...
                wrong_authority_root as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_governance_domain",
                wrong_governance_domain as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_governance_epoch",
                wrong_governance_epoch as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_proposal_id",
                wrong_proposal_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_governance_execution_intent_digest",
                wrong_governance_execution_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_decision_id",
                wrong_rotation_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_request_id",
                wrong_rotation_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_transcript_digest",
                wrong_rotation_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_plan_digest",
                wrong_rotation_plan_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_lifecycle_action",
                wrong_lifecycle_action as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_action",
                wrong_rotation_action as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_authority_sequence",
                wrong_authority_sequence as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_current_validator_set_digest",
                wrong_current_validator_set_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_proposed_validator_set_digest",
                wrong_proposed_validator_set_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_validator_set_delta_digest",
                wrong_validator_set_delta_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_validator_set_epoch",
                wrong_validator_set_epoch as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_validator_set_version",
                wrong_validator_set_version as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_current_validator_set_epoch",
                wrong_current_validator_set_epoch as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_current_validator_set_version",
                wrong_current_validator_set_version as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_proposed_validator_count",
                wrong_proposed_validator_count as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_rotation_nonce",
                wrong_rotation_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_epoch_transition_target_inputs",
                wrong_epoch_transition_target_inputs as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_epoch_transition_target_request",
                wrong_epoch_transition_target_request as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_application_nonce",
                wrong_application_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_live_application_nonce",
                wrong_live_application_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_missing_post_completion_attestation_decision",
                reject_missing_post_completion_attestation_decision as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_unverified_post_completion_attestation_decision",
                reject_unverified_post_completion_attestation_decision as fn(),
            ),
            (
                "accepted_compatible",
                "reject_verified_source_with_non_accept_decision",
                reject_verified_source_with_non_accept_decision as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_accepted_post_completion_attestation_without_package_via_verified_source",
                reject_accepted_post_completion_attestation_without_package_via_verified_source
                    as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_accepted_post_completion_attestation_without_package_variant",
                reject_accepted_post_completion_attestation_without_package_variant as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_guarded_mutation_decision_alone",
                reject_guarded_mutation_decision_alone as fn(),
            ),
            (
                "rejection_fail_closed",
            ...
            (
                "rejection_fail_closed",
                "reject_fixture_only_post_completion_attestation",
                reject_fixture_only_post_completion_attestation as fn(),
            ),
            (
                "rejection_fail_closed",
            ...
            (
                "mainnet_policy",
                "mainnet_domain_refused_under_source_test_policy",
                mainnet_domain_refused_under_source_test_policy as fn(),
            ),
            (
                "mainnet_policy",
                "mainnet_policy_unavailable",
                mainnet_policy_unavailable as fn(),
            ),
            (
                "mainnet_policy",
                "production_policy_unavailable",
                production_policy_unavailable as fn(),
            ),
            (
                "mainnet_policy",
                "disabled_policy_fails_closed",
                disabled_policy_fails_closed as fn(),
            ),
            (
                "mainnet_policy",
                "reserved_production_kind_fails_closed",
                reserved_production_kind_fails_closed as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "replay_rejected_when_id_present",
                replay_rejected_when_id_present as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "no_replay_when_id_absent",
                no_replay_when_id_absent as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "stale_governance_epoch",
                stale_governance_epoch as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "stale_authority_sequence",
                stale_authority_sequence as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "stale_validator_set_epoch",
                stale_validator_set_epoch as fn(),
            ),
            (
                "replay_idempotency_freshness",
                "stale_validator_set_version",
                stale_validator_set_version as fn(),
            ),
            (
                "fixture_state",
            ...
            (
                "fixture_state",
                "fixture_state_apply_all_scenarios",
                fixture_state_apply_all_scenarios as fn(),
            ),
            (
                "non_mutation",
                "every_outcome_is_non_mutating",
                every_outcome_is_non_mutating as fn(),
            ),
            (
                "accepted_compatible",
                "accept_authorizes_future_mutation_only",
                accept_authorizes_future_mutation_only as fn(),
            ),
            (
                "non_mutation",
                "reject_never_authorizes_future_mutation",
                reject_never_authorizes_future_mutation as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "policy_predicates",
                policy_predicates as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "kind_predicates",
                kind_predicates as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "execution_kind_mapping_matches_handoff_kind",
                execution_kind_mapping_matches_handoff_kind as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "unsupported_staged_application_kind_is_unsupported",
                unsupported_staged_application_kind_is_unsupported as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "outcome_tags_are_stable_and_distinct",
                outcome_tags_are_stable_and_distinct as fn(),
            ),
            (
                "taxonomy_policy_kind",
            ...
            (
                "accepted_compatible",
                "accept_reexposes_runtime_handoff_tuple_from_parent",
                accept_reexposes_runtime_handoff_tuple_from_parent as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_runtime_handoff_decision_id_mismatch",
                reject_runtime_handoff_decision_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_runtime_handoff_request_id_mismatch",
                reject_runtime_handoff_request_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_runtime_handoff_intent_digest_mismatch",
                reject_runtime_handoff_intent_digest_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_runtime_handoff_transcript_mismatch",
                reject_runtime_handoff_transcript_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_wrong_runtime_handoff_nonce",
                reject_wrong_runtime_handoff_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_runtime_handoff_decision_alone",
                reject_runtime_handoff_decision_alone as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "runtime_handoff_alone_tag_is_stable",
                runtime_handoff_alone_tag_is_stable as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "runtime_handoff_binding_rejects_are_non_mutating",
                runtime_handoff_binding_rejects_are_non_mutating as fn(),
            ),
            (
                "accepted_compatible",
                "accept_content_digest_binds_runtime_handoff_tuple",
                accept_content_digest_binds_runtime_handoff_tuple as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_mutation_execution_decision_id_mismatch",
                reject_mutation_execution_decision_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_mutation_execution_request_id_mismatch",
                reject_mutation_execution_request_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_mutation_execution_intent_digest_mismatch",
                reject_mutation_execution_intent_digest_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_mutation_execution_transcript_mismatch",
                reject_mutation_execution_transcript_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_wrong_mutation_execution_nonce",
                reject_wrong_mutation_execution_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_mutation_execution_decision_alone",
                reject_mutation_execution_decision_alone as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_commit_authorization_decision_id_mismatch",
                reject_commit_authorization_decision_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_commit_authorization_request_id_mismatch",
                reject_commit_authorization_request_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_commit_authorization_intent_digest_mismatch",
                reject_commit_authorization_intent_digest_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_commit_authorization_transcript_mismatch",
                reject_commit_authorization_transcript_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_wrong_commit_authorization_nonce",
                reject_wrong_commit_authorization_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_commit_authorization_decision_alone",
                reject_commit_authorization_decision_alone as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "commit_authorization_binding_rejects_are_non_mutating",
                commit_authorization_binding_rejects_are_non_mutating as fn(),
            ),
            (
                "accepted_compatible",
                "accept_artifact_reexposes_commit_authorization_tuple",
                accept_artifact_reexposes_commit_authorization_tuple as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "mutation_execution_alone_tag_is_stable",
                mutation_execution_alone_tag_is_stable as fn(),
            ),
            (
                "accepted_compatible",
                "accepted_artifact_re_exposes_mutation_execution_ancestor",
                accepted_artifact_re_exposes_mutation_execution_ancestor as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "mutation_execution_binding_rejects_are_non_mutating",
                mutation_execution_binding_rejects_are_non_mutating as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_execution_preparation_decision_id_mismatch",
                reject_execution_preparation_decision_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_execution_preparation_request_id_mismatch",
                reject_execution_preparation_request_id_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_execution_preparation_intent_digest_mismatch",
                reject_execution_preparation_intent_digest_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_execution_preparation_transcript_mismatch",
                reject_execution_preparation_transcript_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_wrong_execution_preparation_nonce",
                reject_wrong_execution_preparation_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_execution_preparation_decision_alone",
                reject_execution_preparation_decision_alone as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "execution_preparation_alone_tag_is_stable",
                execution_preparation_alone_tag_is_stable as fn(),
            ),
            (
                "accepted_compatible",
                "accepted_artifact_re_exposes_execution_preparation_ancestor",
                accepted_artifact_re_exposes_execution_preparation_ancestor as fn(),
            ),
            (
                "taxonomy_policy_kind",
                "execution_preparation_binding_rejects_are_non_mutating",
                execution_preparation_binding_rejects_are_non_mutating as fn(),
            ),
            (
                "accepted_compatible",
                "scenario_accept_add",
            ...
            (
                ...
                scenario_handoff_binding_add as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_remove",
                scenario_handoff_binding_remove as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_update",
                scenario_handoff_binding_update as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_noop",
                scenario_handoff_binding_noop as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_identity",
                scenario_handoff_binding_identity as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_retire",
                scenario_handoff_binding_retire as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_emergency",
                scenario_handoff_binding_emergency as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_authsync",
                scenario_handoff_binding_authsync as fn(),
            ),
            (
                "rejection_fail_closed",
                "scenario_handoff_binding_bulk",
                scenario_handoff_binding_bulk as fn(),
            ),
            (
                "rejection_fail_closed",
        ...
        let art2 = d2
            ...
            .expect("accepted artifact");
        assert_eq!(
            art1.authority_activation_authorization_id,
            art2.authority_activation_authorization_id
        );
        assert_eq!(art1.request_id, art2.request_id);
        assert_eq!(art1.content_digest(), art2.content_digest());
        assert_eq!(
            art1.authority_activation_authorization_digest,
            art2.authority_activation_authorization_digest
        );
        assert_eq!(art1.transcript_digest, art2.transcript_digest);

        std::fs::write(
            outdir.join("fixtures/run_346_accepted_artifact.txt"),
            format!(
        ...
        std::fs::write(
            outdir.join("fixtures/run_346_deterministic_digests.txt"),
            format!(
                "authority_activation_authorization_id={}\nrequest_id={}\ncontent_digest={}\nauthority_activation_authorization_digest={}\ntranscript_digest={}\nsecond_authority_activation_authorization_id={}\nsecond_request_id={}\nsecond_content_digest={}\nsecond_authority_activation_authorization_digest={}\nsecond_transcript_digest={}\n",
                art1.authority_activation_authorization_id,
                art1.request_id,
                art1.content_digest(),
                art1.authority_activation_authorization_digest,
        ...
        let recovery = c
            .executor
            .recover_live_epoch_transition_authority_activation_authorization_window(
                Some(art1),
                art2,
            );
        std::fs::write(
            outdir.join("fixtures/run_346_replay_recovery.txt"),
            format!("recovery={:?}\n", recovery),
        )
        ...
        summary.push_str("run=346\n");
        summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_authority_activation_authorization.rs::ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor\n");
...
fn main() {
    run_345_authority_activation_authorization::helper_main();
}