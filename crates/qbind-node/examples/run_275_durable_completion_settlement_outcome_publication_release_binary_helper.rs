//! Run 275 — release-built helper for the Run 274 durable-completion
//! settlement-outcome publication / settlement-outcome-report publication sink boundary.
//!
...
//! settlement-outcome report chain and then evaluates the Run 274
//! settlement-outcome publication. Projection is intentionally from
//! `input.settlement_outcome_report_binding` through
...
use qbind_node::pqc_governance_durable_completion_settlement_outcome_publication::{
    ...
    durable_completion_settlement_outcome_publication_ambiguous_window_fails_closed,
    durable_completion_settlement_outcome_publication_attestation_required,
    durable_completion_settlement_outcome_publication_backend_submission_required,
    durable_completion_settlement_outcome_publication_completion_report_required,
    durable_completion_settlement_outcome_publication_external_unavailable,
    ...
    durable_completion_settlement_outcome_publication_local_operator_cannot_satisfy_mainnet_authority,
    ...
    durable_completion_settlement_outcome_publication_no_external_publication,
    ...
    durable_completion_settlement_outcome_publication_pipeline_success_required,
    durable_completion_settlement_outcome_publication_policy_change_unsupported,
    durable_completion_settlement_outcome_publication_production_mainnet_unavailable,
    durable_completion_settlement_outcome_publication_receipt_required,
    durable_completion_settlement_outcome_publication_record_required_before_reported,
    durable_completion_settlement_outcome_publication_rejection_is_non_mutating,
    durable_completion_settlement_outcome_publication_rollback_never_records,
    durable_completion_settlement_outcome_publication_sink_receipt_required,
    durable_completion_settlement_outcome_publication_validator_set_rotation_unsupported,
    ...
    DurableCompletionSettlementOutcomePublicationFault,
    ...
    DurableCompletionSettlementOutcomePublicationWindow, ExternalSettlementOutcomePublicationSink,
    FixtureDurableCompletionSettlementOutcomePublicationSink,
    GovernanceDurableCompletionSettlementOutcomePublicationSink, MainNetSettlementOutcomePublicationSink,
    ProductionSettlementOutcomePublicationSink,
};
...
// Run 275 — additional Run 274 production library symbols exercised by the
// release symbol reachability probe (imported here so the release helper source
// grep-proves it links the real Run 274 boundary symbols in release mode).
use qbind_node::pqc_governance_durable_completion_settlement_outcome_publication::{
    durable_completion_settlement_outcome_publication_consumer_required,
    ...
    settlement_outcome_publication_record_digest,
    ...
    DurableCompletionSettlementOutcomePublicationBinding,
    DurableCompletionSettlementOutcomePublicationDigest,
    DurableCompletionSettlementOutcomePublicationEnvironment,
    DurableCompletionSettlementOutcomePublicationLedgerRecord,
    DurableCompletionSettlementOutcomePublicationLedgerSnapshot,
    DurableCompletionSettlementOutcomePublicationLedgerStatus,
    DurableCompletionSettlementOutcomePublicationRecord,
    DurableCompletionSettlementOutcomePublicationResponse,
    DurableCompletionSettlementOutcomePublicationSurface,
...
fn ack_identity(
    policy: DurableCompletionSettlementOutcomePublicationPolicy,
...
fn ctx_action(
    ...
    let settlement_outcome_report =
        attach_run272_settlement_outcome_report(environment, action, consumer_duplicate);
    ...
    let expectations = DurableCompletionSettlementOutcomePublicationExpectations {
        ...
        expected_outcome_publication_kind: kind,
        expected_outcome_publication_policy: policy,
...
impl Ctx {
    ...
    fn input_with_outcome_report(
        ...
        outcome_publication: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        settlement_outcome_report: DurableCompletionSettlementOutcomeReportOutcome,
    ) -> DurableCompletionSettlementOutcomePublicationInput {
        DurableCompletionSettlementOutcomePublicationInput {
            policy,
            ...
            outcome_report_binding: outcome_publication,
            ...
            settlement_outcome_report_binding: settlement_outcome_report,
    ...
    fn input(
        ...
        self.input_with_outcome_report(
            policy,
            replay,
            pipeline,
            sink,
            ...
            self.settlement_outcome_report.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionSettlementOutcomePublicationInput {
        self.input_with_outcome_report(
...
fn fixture_sink() -> FixtureDurableCompletionSettlementOutcomePublicationSink {
    FixtureDurableCompletionSettlementOutcomePublicationSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionSettlementOutcomePublicationPolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomePublicationOutcome::LegacyBypassNoSettlementOutcomePublication
    );
...
fn disabled_backend_policy_never_invokes_receipt_sink() {
    let c = devnet_ctx();
    let input = c.input_with_outcome_report(
        DurableCompletionSettlementOutcomePublicationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        DurableCompletionSettlementOutcomeReportOutcome::LegacyBypassNoSettlementOutcomeReport,
    );
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = fixture_sink();
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(settlement_outcome_publication_outcome_authorizes_record(&outcome));
    assert!(settlement_outcome_publication_outcome_projects_to_recorded(&outcome));
    assert_eq!(sink.invocations(), 1);
    assert_eq!(ledger.len(), 1);
    assert!(ledger.contains(OUTCOME_PUBLICATION_RECORD_ID));
    assert_eq!(
        settlement_outcome_publication_identity_digest(&c.request.identity),
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.outcome_publication_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!settlement_outcome_publication_outcome_authorizes_record(&second));
    assert!(settlement_outcome_publication_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run268_duplicate_idempotent_outcome_report_only_matches_existing_never_creates() {
    // A real Run 272 SettlementOutcomeReportDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.settlement_outcome_report.outcome,
        DurableCompletionSettlementOutcomeReportOutcome::SettlementOutcomeReportDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomePublicationOutcome::SettlementOutcomePublicationRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_settlement_outcome_publication(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_settlement_outcome_publication(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomePublicationPolicy::ProductionSettlementOutcomePublicationRequired,
        DurableCompletionSettlementOutcomePublicationKind::ProductionSettlementOutcomePublicationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = ProductionSettlementOutcomePublicationSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomePublicationPolicy::MainNetSettlementOutcomePublicationRequired,
        DurableCompletionSettlementOutcomePublicationKind::MainNetSettlementOutcomePublicationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = MainNetSettlementOutcomePublicationSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionSettlementOutcomePublicationPolicy::ExternalSettlementOutcomePublicationRequired,
        DurableCompletionSettlementOutcomePublicationKind::ExternalSettlementOutcomePublicationUnavailable,
    );
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = ExternalSettlementOutcomePublicationSink::default();
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn validator_set_rotation_and_policy_change_unsupported() {
    let c = devnet_ctx();
    let rotation = c.input_with_outcome_report(
        DurableCompletionSettlementOutcomePublicationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        DurableCompletionSettlementOutcomeReportOutcome::ValidatorSetRotationUnsupportedNoOutcomeReport,
    );
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = fixture_sink();
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomePublicationOutcome::ValidatorSetRotationUnsupportedNoOutcomePublication
    );
    assert_eq!(sink.invocations(), 0);

    let policy_change = c.input_with_outcome_report(
        DurableCompletionSettlementOutcomePublicationPolicy::FixtureAllowed,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        ...
        c.consumer.ack.outcome.clone(),
        c.consumer.outcome.clone(),
        DurableCompletionSettlementOutcomeReportOutcome::PolicyChangeUnsupportedNoOutcomeReport,
    );
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &policy_change,
        &c.expectations,
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomePublicationOutcome::PolicyChangeUnsupportedNoOutcomePublication
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_non_recording_outcome_report(
    settlement_outcome_report: DurableCompletionSettlementOutcomeReportOutcome,
    expected: DurableCompletionSettlementOutcomePublicationOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_outcome_report(
        DurableCompletionSettlementOutcomePublicationPolicy::FixtureAllowed,
...
fn non_recording_outcome_report_outcomes_never_record_outcome_publication() {
    ...
    assert_non_recording_outcome_report(
        Finalization::LegacyBypassNoSettlementOutcomeReport,
        OutcomePublication::LegacyBypassNoSettlementOutcomePublication,
    );
    assert_non_recording_outcome_report(
    ...
    assert_non_recording_outcome_report(
        Finalization::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::SettlementOutcomeReportRejectedBeforeRecord,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::SettlementOutcomeReportRecordFailedNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::SettlementOutcomeReportRolledBackNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::SettlementOutcomeReportRollbackFailedFatalNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::SettlementOutcomeReportAmbiguousFailClosedNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::ProductionSettlementOutcomeReportUnavailableNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::MainNetSettlementOutcomeReportUnavailableNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
        Finalization::ExternalSettlementOutcomeReportUnavailableNoOutcomeReport,
        OutcomePublication::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
    );
    assert_non_recording_outcome_report(
    ...
    assert_non_recording_outcome_report(
        ...
        OutcomePublication::PolicyChangeUnsupportedNoOutcomePublication,
    );
}
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionSettlementOutcomePublicationOutcome::RejectedBeforeSettlementOutcomeReportNoOutcomePublication
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionSettlementOutcomePublicationRequest),
) {
    let c = devnet_ctx();
...
fn wrong_outcome_publication_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.outcome_publication_record_id = "other-outcome_report".to_string();
    });
}
...
fn wrong_finalization_decision_digest_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.finalization_decision_digest = "other-outcome-report".to_string();
    });
}
...
fn wrong_acknowledgement_identity_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.identity.outcome_report_id = "other-receipt-id".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_settlement_outcome_publication(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_receipt(
    fault: DurableCompletionSettlementOutcomePublicationFault,
    expected: DurableCompletionSettlementOutcomePublicationOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = FixtureDurableCompletionSettlementOutcomePublicationSink::with_fault(fault);
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomePublicationOutcome::SettlementOutcomePublicationRecordFailedNoOutcomePublication,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomePublicationOutcome::SettlementOutcomePublicationRolledBackNoOutcomePublication,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomePublicationOutcome::SettlementOutcomePublicationRollbackFailedFatalNoOutcomePublication,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_receipt(
        ...
        DurableCompletionSettlementOutcomePublicationOutcome::SettlementOutcomePublicationAmbiguousFailClosedNoOutcomePublication,
    );
}
...
fn only_recorded_outcome_report_outcome_creates_settlement_outcome_publication_request_intent() {
    use DurableCompletionSettlementOutcomeReportOutcome as Finalization;
    use DurableCompletionSettlementOutcomePublicationRequestIntent as Intent;
    assert_eq!(
        project_settlement_outcome_report_outcome_to_outcome_publication_request(
            &Finalization::SettlementOutcomeReportRecorded
        ),
        Intent::CreateRequest
    );
    assert!(project_settlement_outcome_report_outcome_to_outcome_publication_request(
        &Finalization::SettlementOutcomeReportRecorded
    )
    .creates_request());
    assert_eq!(
        project_settlement_outcome_report_outcome_to_outcome_publication_request(
            &Finalization::SettlementOutcomeReportDuplicateIdempotent
        ),
...
fn non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request() {
    use DurableCompletionSettlementOutcomeReportOutcome as Finalization;
    use DurableCompletionSettlementOutcomePublicationRequestIntent as Intent;
    for outcome_report in [
        Finalization::LegacyBypassNoSettlementOutcomeReport,
        Finalization::RejectedBeforeSettlementReceiptAcknowledgementNoOutcomeReport,
        Finalization::SettlementReceiptAcknowledgementDidNotRecordNoOutcomeReport,
        Finalization::SettlementOutcomeReportRejectedBeforeRecord,
        ...
        assert!(matches!(
            project_settlement_outcome_report_outcome_to_outcome_publication_request(&outcome_report),
            Intent::NoOutcomeReport(_)
        ));
    }
}
...
fn recovered_ledger() -> DurableCompletionSettlementOutcomePublicationLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_settlement_outcome_publication(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let record = if with_record {
        ledger.find(OUTCOME_PUBLICATION_RECORD_ID)
    } else {
        None
    };
    let outcome = recover_durable_completion_settlement_outcome_publication_window(
        &input,
        window,
        DurableCompletionSettlementOutcomePublicationKind::FixtureInMemory,
        record,
...
fn pre_settlement_outcome_report_windows_fail_closed_no_commitment() {
    ...
    for w in [
        ...
        assert_window(
            w,
            false,
            Finalization::SettlementOutcomeReportDidNotRecordNoOutcomePublication,
        );
    }
}

fn after_settlement_outcome_report_request_before_record_rejects_before_record() {
    use DurableCompletionSettlementOutcomePublicationOutcome as Finalization;
    use DurableCompletionSettlementOutcomePublicationWindow as Window;
    assert_window(
        Window::AfterSettlementOutcomePublicationRequestBeforeSettlementOutcomePublicationRecord,
        false,
        Finalization::SettlementOutcomePublicationRejectedBeforeRecord,
    );
}

fn after_settlement_outcome_report_record_before_success_requires_explicit_matching_record() {
    use DurableCompletionSettlementOutcomePublicationOutcome as Finalization;
    use DurableCompletionSettlementOutcomePublicationWindow as Window;
    assert_window(
    ...
    assert_window(
        Window::AfterSettlementOutcomePublicationRecordBeforeSettlementOutcomePublicationSuccess,
        true,
        Finalization::SettlementOutcomePublicationRecorded,
    );
}

fn after_settlement_outcome_report_success_recovers_as_recorded() {
    use DurableCompletionSettlementOutcomePublicationOutcome as Finalization;
    use DurableCompletionSettlementOutcomePublicationWindow as Window;
    assert_window(
        Window::AfterSettlementOutcomePublicationSuccess,
        true,
        Finalization::SettlementOutcomePublicationRecorded,
    );
...
fn ambiguous_record_failed_rollback_and_unknown_windows_fail_closed() {
    ...
    assert_window(
        ...
        false,
        Finalization::SettlementOutcomePublicationAmbiguousFailClosedNoOutcomePublication,
    );
    assert_window(
        Window::Unknown,
        false,
    ...
    assert_window(
        ...
        false,
        Finalization::SettlementOutcomePublicationRecordFailedNoOutcomePublication,
    );
    assert_window(
        Window::SettlementOutcomePublicationRollbackCompleted,
        false,
        Finalization::SettlementOutcomePublicationRolledBackNoOutcomePublication,
    );
    assert_window(
        Window::SettlementOutcomePublicationRollbackFailed,
        false,
        Finalization::SettlementOutcomePublicationRollbackFailedFatalNoOutcomePublication,
    );
}
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_settlement_outcome_publication_window(
        &input,
        DurableCompletionSettlementOutcomePublicationWindow::AfterSettlementOutcomePublicationSuccess,
        DurableCompletionSettlementOutcomePublicationKind::ProductionSettlementOutcomePublicationUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionSettlementOutcomePublicationSink::with_fault(
        DurableCompletionSettlementOutcomePublicationFault::RolledBackNoOutcomeReport,
    );
    let outcome = evaluate_durable_completion_settlement_outcome_publication(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_settlement_outcome_publication_rejection_is_non_mutating());
    assert!(durable_completion_settlement_outcome_publication_never_calls_run_070());
    assert!(durable_completion_settlement_outcome_publication_never_mutates_live_pqc_trust_state());
    assert!(durable_completion_settlement_outcome_publication_never_writes_sequence_or_marker());
    assert!(durable_completion_settlement_outcome_publication_no_rocksdb_file_schema_migration_change());
    assert!(durable_completion_settlement_outcome_publication_no_external_publication());
    assert!(durable_completion_settlement_outcome_publication_no_real_audit_ledger());
    assert!(durable_completion_settlement_outcome_publication_pipeline_success_required());
    assert!(durable_completion_settlement_outcome_publication_sink_receipt_required());
    assert!(durable_completion_settlement_outcome_publication_completion_report_required());
    assert!(durable_completion_settlement_outcome_publication_finalization_projection_required());
    assert!(durable_completion_settlement_outcome_publication_attestation_required());
    assert!(durable_completion_settlement_outcome_publication_backend_submission_required());
    assert!(durable_completion_settlement_outcome_publication_receipt_required());
    assert!(durable_completion_settlement_outcome_publication_record_required_before_reported());
    assert!(durable_completion_settlement_outcome_publication_failed_record_never_records());
    assert!(durable_completion_settlement_outcome_publication_rollback_never_records());
    assert!(durable_completion_settlement_outcome_publication_ambiguous_window_fails_closed());
    assert!(
        durable_completion_settlement_outcome_publication_mainnet_peer_driven_apply_refused_first(
            TrustBundleEnvironment::Mainnet
        )
    ...
    assert!(durable_completion_settlement_outcome_publication_production_mainnet_unavailable());
    assert!(durable_completion_settlement_outcome_publication_external_unavailable());
    assert!(durable_completion_settlement_outcome_publication_validator_set_rotation_unsupported());
    assert!(durable_completion_settlement_outcome_publication_policy_change_unsupported());
    assert!(
        durable_completion_settlement_outcome_publication_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
// ===========================================================================
// Release symbol reachability probe — exercises the Run 274 production library
// symbols in release mode, driving projection from the Run 272 settlement-
// outcome-report outcome via input.settlement_outcome_report_binding.
// ===========================================================================

fn release_symbol_reachability_probe() {
    let c = devnet_ctx();
    let input: DurableCompletionSettlementOutcomePublicationInput = c.recorded();

    let intent = project_settlement_outcome_report_outcome_to_outcome_publication_request(
        &input.settlement_outcome_report_binding,
    );
    assert!(intent.creates_request());
    assert!(matches!(
        intent,
        DurableCompletionSettlementOutcomePublicationRequestIntent::CreateRequest
    ));
    ...
    assert!(!outcome.no_commitment());
    assert_eq!(outcome.tag(), "settlement-outcome-publication-recorded");
    assert_eq!(
        sink.kind(),
        DurableCompletionSettlementOutcomePublicationKind::FixtureInMemory
    );
    ...
    let identity_digest: DurableCompletionSettlementOutcomePublicationDigest =
        settlement_outcome_publication_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionSettlementOutcomePublicationDigest =
        settlement_outcome_publication_request_digest(&c.request);
    let response = DurableCompletionSettlementOutcomePublicationResponse {
        outcome_publication_record_id: c.request.outcome_publication_record_id.clone(),
    ...
    let response_digest: DurableCompletionSettlementOutcomePublicationDigest =
        settlement_outcome_publication_response_digest(&response);
    let record: DurableCompletionSettlementOutcomePublicationRecord = c.request.to_record();
    let record_digest: DurableCompletionSettlementOutcomePublicationDigest =
        settlement_outcome_publication_record_digest(&record);
    let transcript_digest: DurableCompletionSettlementOutcomePublicationTranscriptDigest =
        settlement_outcome_publication_transcript_digest(&request_digest, &response_digest, &record_digest);
    assert!(!identity_digest.as_hex().is_empty());
    assert!(!transcript_digest.as_hex().is_empty());

    let ledger_record: &DurableCompletionSettlementOutcomePublicationLedgerRecord = ledger
        .find(OUTCOME_PUBLICATION_RECORD_ID)
        .expect("settlement-outcome-publication ledger record");
    assert_eq!(
        ledger_record.status,
        DurableCompletionSettlementOutcomePublicationLedgerStatus::Recorded
    );
    let snapshot: DurableCompletionSettlementOutcomePublicationLedgerSnapshot = ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionSettlementOutcomePublicationSurface = input.surface();
    let _environment: DurableCompletionSettlementOutcomePublicationEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionSettlementOutcomePublicationBinding = input.runtime_binding.clone();

    assert!(DurableCompletionSettlementOutcomePublicationPolicy::Disabled.is_disabled());
    assert!(DurableCompletionSettlementOutcomePublicationPolicy::FixtureAllowed.allows_fixture());
    assert_eq!(
        DurableCompletionSettlementOutcomePublicationPolicy::ProductionSettlementOutcomePublicationRequired.tag(),
        "production-settlement-outcome-publication-required"
    );
    assert!(DurableCompletionSettlementOutcomePublicationKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionSettlementOutcomePublicationKind::ProductionSettlementOutcomePublicationUnavailable.is_unavailable());
    assert_eq!(
        DurableCompletionSettlementOutcomePublicationKind::Disabled.tag(),
        "disabled"
    );
    let no_outcome_publication =
        DurableCompletionSettlementOutcomePublicationOutcome::LegacyBypassNoSettlementOutcomePublication;
    assert!(no_outcome_publication.is_legacy_bypass());
    assert!(no_outcome_publication.no_commitment());
    assert!(
        DurableCompletionSettlementOutcomePublicationOutcome::MainNetPeerDrivenApplyRefusedNoOutcomePublication
    ...
    assert!(durable_completion_settlement_outcome_publication_consumer_required());
    assert!(durable_completion_settlement_outcome_publication_outcome_report_required());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement_finality());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement_receipt());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement_outcome_report());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement_finality_projection());
    assert!(durable_completion_settlement_outcome_publication_no_real_settlement_outcome_publication());

    let mut prod = ProductionSettlementOutcomePublicationSink::default();
    let mut mainnet = MainNetSettlementOutcomePublicationSink::default();
    let mut external = ExternalSettlementOutcomePublicationSink::default();
    let mut prod_ledger = DurableCompletionSettlementOutcomePublicationLedger::new();
    assert_eq!(
        prod.project_durable_completion_settlement_outcome_publication(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_settlement_outcome_publication(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_settlement_outcome_publication(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_275_durable_completion_settlement_outcome_publication_release_binary/helper_evidence/run_275")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "run268_duplicate_idempotent_outcome_report_only_matches_existing_never_creates", run268_duplicate_idempotent_outcome_report_only_matches_existing_never_creates as fn()),
        ...
        ("rejection_fail_closed", "non_recording_outcome_report_outcomes_never_record_outcome_publication", non_recording_outcome_report_outcomes_never_record_outcome_publication as fn()),
        ...
        ("rejection_fail_closed", "wrong_outcome_publication_record_id_rejected_before_record", wrong_outcome_publication_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request", non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request as fn()),
        ...
        ("recovery_crash_window", "pre_settlement_outcome_report_windows_fail_closed_no_commitment", pre_settlement_outcome_report_windows_fail_closed_no_commitment as fn()),
        ("recovery_crash_window", "after_settlement_outcome_report_request_before_record_rejects_before_record", after_settlement_outcome_report_request_before_record_rejects_before_record as fn()),
        ("recovery_crash_window", "after_settlement_outcome_report_record_before_success_requires_explicit_matching_record", after_settlement_outcome_report_record_before_success_requires_explicit_matching_record as fn()),
        ("recovery_crash_window", "after_settlement_outcome_report_success_recovers_as_recorded", after_settlement_outcome_report_success_recovers_as_recorded as fn()),
        ...
        ("projection", "only_recorded_outcome_report_outcome_creates_settlement_outcome_publication_request_intent", only_recorded_outcome_report_outcome_creates_settlement_outcome_publication_request_intent as fn()),
        ("projection", "non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request", non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request as fn()),
        ("projection", "run268_duplicate_idempotent_outcome_report_only_matches_existing_never_creates", run268_duplicate_idempotent_outcome_report_only_matches_existing_never_creates as fn()),
        ("settlement_outcome_publication_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("settlement_outcome_publication_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("settlement_outcome_publication_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_outcome_report_outcomes_never_record_outcome_publication", non_recording_outcome_report_outcomes_never_record_outcome_publication as fn()),
        ("non_mutation", "non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request", non_recording_outcome_report_outcomes_create_no_settlement_outcome_publication_request as fn()),
    ...
    summary.push_str("Run 275 durable-completion settlement-outcome-publication release helper\n");
    summary.push_str(&format!("verdict: {}\n", if total_fail == 0 { "PASS" } else { "FAIL" }));
    summary.push_str("projection_rule: input.settlement_outcome_report_binding -> project_settlement_outcome_report_outcome_to_outcome_publication_request\n");
    summary.push_str("attached_chain: Run256 backend -> Run258 receipt -> Run260 acknowledgement -> Run262 consumer -> Run264 settlement projection -> Run266 settlement commitment -> Run268 settlement finalization -> Run270 settlement-receipt acknowledgement -> Run272 settlement-outcome report -> Run274 settlement-outcome publication\n");
    ...
    fs::write(
        outdir.join("fixtures/run_275_projection_rule.txt"),
        "input.settlement_outcome_report_binding\nproject_settlement_outcome_report_outcome_to_outcome_publication_request\nSettlementOutcomeReportRecorded -> CreateRequest\n",
