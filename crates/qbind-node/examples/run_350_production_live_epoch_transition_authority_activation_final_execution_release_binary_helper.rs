//! Run 350 — release-binary evidence helper for the Run 349 live epoch-transition
//! **authority-activation final-execution** boundary
...
//! release-built production library symbols. It exercises the **real** Run 349
...
//! 327 → 329 → 331 → 333 → 335 → 337 → 339 → 341 → 343 → 345 → 347 chain to
//! produce a verified, accepted Run 347/348 live epoch-transition
//! authority-activation execution-preparation / final-execution preflight
//! decision (`is_accept()` with
//! `Some(authority_activation_execution_preparation_artifact)`), then feeds that
//! decision into the Run 349 live epoch-transition authority-activation
//! final-execution executor, which emits only a typed, deterministic,
//! non-mutating authority-activation final-execution artifact.
//!
...
//! final-execution artifacts; it never applies a live validator-set change, never
//! transitions a consensus epoch, never commits or finalizes production runtime
//! state, never writes a production receipt / audit / audit-seal /
//! audit-finalization / audit-ledger / audit-ledger-commitment /
//! durable-audit-publication / durable-replay / settlement /
...
//! authority-activation-execution-preparation / final-execution-preflight /
//! final-execution-readiness / authority-activation / authority-activation
//! final-execution / publication / external-publication / final-execution
//! record, never calls Run 070, never mutates `LivePqcTrustState`, never calls
...
//! in-memory `LiveEpochTransitionAuthorityActivationFinalExecutionFixtureState`
//! used exclusively as source/test evidence.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_350.md`.
#![allow(dead_code)]
...
mod run_349_authority_activation_final_execution {
    ...
    pub fn helper_main() {
        let outdir = std::env::args()
            ...
            .unwrap_or_else(|| {
                std::path::PathBuf::from(
                    "docs/devnet/run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary/helper_evidence/run_350",
                )
            });
        ...
        let cases: &[(&str, &str, fn())] = &[
            ...
            ("accepted_compatible", "accept_artifact_reexposes_consumed_authority_activation_execution_preparation_transcript", accept_artifact_reexposes_consumed_authority_activation_execution_preparation_transcript as fn()),
            ...
            ("rejection_fail_closed", "wrong_authority_activation_execution_preparation_decision_id", wrong_authority_activation_execution_preparation_decision_id as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_execution_preparation_request_id", wrong_authority_activation_execution_preparation_request_id as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_execution_preparation_intent_digest", wrong_authority_activation_execution_preparation_intent_digest as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_execution_preparation_transcript_digest", wrong_authority_activation_execution_preparation_transcript_digest as fn()),
            ("rejection_fail_closed", "wrong_authority_activation_execution_preparation_nonce", wrong_authority_activation_execution_preparation_nonce as fn()),
            ("rejection_fail_closed", "tampered_authority_activation_execution_preparation_package_integrity_mismatch", tampered_authority_activation_execution_preparation_package_integrity_mismatch as fn()),
            ...
            ("rejection_fail_closed", "reject_missing_authority_activation_execution_preparation_decision", reject_missing_authority_activation_execution_preparation_decision as fn()),
            ("rejection_fail_closed", "reject_unverified_authority_activation_execution_preparation_decision", reject_unverified_authority_activation_execution_preparation_decision as fn()),
            ...
            ("rejection_fail_closed", "reject_accepted_authority_activation_execution_preparation_without_package_via_verified_source", reject_accepted_authority_activation_execution_preparation_without_package_via_verified_source as fn()),
            ("rejection_fail_closed", "reject_accepted_authority_activation_execution_preparation_without_package_variant", reject_accepted_authority_activation_execution_preparation_without_package_variant as fn()),
            ...
            ("rejection_fail_closed", "reject_fixture_only_authority_activation_execution_preparation", reject_fixture_only_authority_activation_execution_preparation as fn()),
        ...
        let art2 = d2
            ...
            .expect("accepted artifact");
        assert_eq!(
            art1.authority_activation_final_execution_id,
            art2.authority_activation_final_execution_id
        );
        assert_eq!(art1.request_id, art2.request_id);
        assert_eq!(art1.content_digest(), art2.content_digest());
        assert_eq!(
            art1.authority_activation_final_execution_digest,
            art2.authority_activation_final_execution_digest
        );
        assert_eq!(art1.transcript_digest, art2.transcript_digest);

        std::fs::write(
            outdir.join("fixtures/run_350_accepted_artifact.txt"),
            format!(
        ...
        std::fs::write(
            outdir.join("fixtures/run_350_deterministic_digests.txt"),
            format!(
                "authority_activation_final_execution_id={}\nrequest_id={}\ncontent_digest={}\nauthority_activation_final_execution_digest={}\ntranscript_digest={}\nsecond_authority_activation_final_execution_id={}\nsecond_request_id={}\nsecond_content_digest={}\nsecond_authority_activation_final_execution_digest={}\nsecond_transcript_digest={}\n",
                art1.authority_activation_final_execution_id,
                art1.request_id,
                art1.content_digest(),
                art1.authority_activation_final_execution_digest,
        ...
        let recovery = c
            .executor
            .recover_live_epoch_transition_authority_activation_final_execution_window(
                Some(art1),
                art2,
            );
        std::fs::write(
            outdir.join("fixtures/run_350_replay_recovery.txt"),
            format!("recovery={:?}\n", recovery),
        )
        ...
        summary.push_str("run=350\n");
        summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_authority_activation_final_execution.rs::ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor\n");
...
fn main() {
    run_349_authority_activation_final_execution::helper_main();
}