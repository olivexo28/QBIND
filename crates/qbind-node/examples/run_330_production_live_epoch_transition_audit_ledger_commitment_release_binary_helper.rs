...
fn t_eval_replay(
    ...
    case.executor
        .evaluate_live_epoch_transition_audit_ledger_commitment(&case.request, &case.inputs, &replay)
}
...
/// assert the resulting outcome.
fn t_reject_source(
    source: LiveEpochTransitionAuditLedgerCommitmentAuthoritySource,
    expected: TO,
) {
    let mut c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
...
fn accept_artifact_reexposes_consumed_durable_audit_finalization_transcript() {
    ...
    assert_eq!(art.durable_audit_finalization_transcript_digest, handoff_transcript);
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    assert_eq!(d.audit_ledger_commitment_digest, art.audit_ledger_commitment_digest);
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        assert_eq!(d1.audit_ledger_commitment_digest, d2.audit_ledger_commitment_digest);
...
fn tampered_durable_audit_finalization_package_integrity_mismatch() {
    ...
    let target = dec.durable_audit_finalization_artifact.as_ref().unwrap().epoch_transition_target;
    let inputs = t_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.durable_audit_finalization_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
    let request = ProductionLiveEpochTransitionAuditLedgerCommitmentRequest::new(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::VerifiedDurableAuditFinalizationDecision {
            decision: dec,
        },
    ...
    assert_eq!(d.outcome, TO::DurableAuditFinalizationDecisionIntegrityMismatch);
    assert!(d.audit_ledger_commitment_artifact.is_none());
}
...
fn wrong_governance_epoch() {
    t_reject_inputs(|i| i.expected_governance_epoch = 999, TO::WrongGovernanceEpoch);
}

fn wrong_proposal_id() {
    t_reject_inputs(|i| i.expected_proposal_id = "bad".to_string(), TO::WrongProposalId);
}
...
fn wrong_authority_sequence() {
    t_reject_inputs(|i| i.expected_authority_domain_sequence = 999, TO::WrongAuthoritySequence);
}
...
fn wrong_validator_set_epoch() {
    t_reject_inputs(|i| i.expected_validator_set_epoch = 999, TO::WrongValidatorSetEpoch);
}

fn wrong_validator_set_version() {
    t_reject_inputs(|i| i.expected_validator_set_version = 999, TO::WrongValidatorSetVersion);
}
...
fn wrong_proposed_validator_count() {
    t_reject_inputs(|i| i.expected_proposed_validator_count = 999, TO::WrongProposedValidatorCount);
}
...
fn wrong_epoch_transition_target_inputs() {
    t_reject_inputs(|i| i.expected_epoch_transition_target = 999, TO::WrongEpochTransitionTarget);
}
...
fn wrong_application_nonce() {
    t_reject_inputs(|i| i.expected_application_nonce = 999, TO::WrongApplicationNonce);
}

fn wrong_live_application_nonce() {
    t_reject_inputs(|i| i.expected_live_application_nonce = 999, TO::WrongLiveApplicationNonce);
}
...
fn replay_rejected_when_id_present() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = t_eval(&c);
    let id = d.audit_ledger_commitment_artifact.as_ref().unwrap().request_id.clone();
    let replay = vec![id];
...
fn stale_governance_epoch() {
    t_reject_inputs(|i| i.min_governance_epoch = u64::MAX, TO::StaleGovernanceEpoch);
}

fn stale_authority_sequence() {
    t_reject_inputs(|i| i.persisted_sequence = Some(u64::MAX), TO::StaleAuthoritySequence);
}

fn stale_validator_set_epoch() {
    t_reject_inputs(|i| i.min_validator_set_epoch = u64::MAX, TO::StaleValidatorSetEpoch);
}

fn stale_validator_set_version() {
    t_reject_inputs(|i| i.min_validator_set_version = u64::MAX, TO::StaleValidatorSetVersion);
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let d = t_eval(&c);
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    let mut state = LiveEpochTransitionAuditLedgerCommitmentFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn accept_authorizes_future_mutation_only() {
    ...
    assert!(d.authorizes_future_mutation_only());
    assert!(d.audit_ledger_commitment_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
}
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = TK::from_staged_application_kind(LiveEpochTransitionDurableAuditFinalizationKind::UnsupportedStagedApplication);
    assert!(pk.is_unsupported());
}
...
fn outcome_tags_are_stable_and_distinct() {
    ...
    assert_eq!(a.tag(), TO::DurableAuditFinalizationDecisionIdMismatch.tag());
...
fn config_and_inputs_well_formed() {
    ...
    assert!(ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::source_test().is_well_formed());
...
macro_rules! per_scenario_accept {
    ($name:ident, $sc:expr) => {
        ...
        fn $name() {
            ...
            let d2 = t_eval(&c);
            assert_eq!(d.audit_ledger_commitment_digest, d2.audit_ledger_commitment_digest);
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        ...
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.audit_ledger_commitment_artifact.as_ref().unwrap().environment,
...
fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    ...
    let ppkg = parent.durable_audit_finalization_artifact.as_ref().unwrap().clone();
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = t_eval(&c);
    assert!(d.is_accept());
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "accept_artifact_reexposes_consumed_durable_audit_finalization_transcript", accept_artifact_reexposes_consumed_durable_audit_finalization_transcript as fn()),
        ...
        ("rejection_fail_closed", "wrong_durable_audit_finalization_decision_id", wrong_durable_audit_finalization_decision_id as fn()),
        ("rejection_fail_closed", "wrong_durable_audit_finalization_request_id", wrong_durable_audit_finalization_request_id as fn()),
        ("rejection_fail_closed", "wrong_durable_audit_finalization_intent_digest", wrong_durable_audit_finalization_intent_digest as fn()),
        ("rejection_fail_closed", "wrong_durable_audit_finalization_transcript_digest", wrong_durable_audit_finalization_transcript_digest as fn()),
        ("rejection_fail_closed", "wrong_durable_audit_finalization_nonce", wrong_durable_audit_finalization_nonce as fn()),
        ("rejection_fail_closed", "tampered_durable_audit_finalization_package_integrity_mismatch", tampered_durable_audit_finalization_package_integrity_mismatch as fn()),
        ...
        ("rejection_fail_closed", "reject_missing_durable_audit_finalization_decision", reject_missing_durable_audit_finalization_decision as fn()),
        ("rejection_fail_closed", "reject_unverified_durable_audit_finalization_decision", reject_unverified_durable_audit_finalization_decision as fn()),
        ...
        ("rejection_fail_closed", "reject_accepted_durable_audit_finalization_without_package_via_verified_source", reject_accepted_durable_audit_finalization_without_package_via_verified_source as fn()),
        ("rejection_fail_closed", "reject_accepted_durable_audit_finalization_without_package_variant", reject_accepted_durable_audit_finalization_without_package_variant as fn()),
        ...
        ("rejection_fail_closed", "reject_fixture_only_durable_audit_finalization", reject_fixture_only_durable_audit_finalization as fn()),
    ...
    for sc in ALL_SC {
        ...
        digest_body.push_str(&format!("audit_ledger_commitment_id={}\n", decision.audit_ledger_commitment_id));
        digest_body.push_str(&format!("request_id={}\n", decision.request_id));
        digest_body.push_str(&format!("audit_ledger_commitment_digest={}\n", decision.audit_ledger_commitment_digest));
        digest_body.push_str(&format!(
            "content_digest={}\n",
            decision
                .audit_ledger_commitment_artifact
    ...
    digest_body.push_str(&format!("named_audit_ledger_commitment_id={audit_ledger_commitment_id}\n"));
    ...
    let clean: ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome =
    ...
    summary.push_str("run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary_helper\n");
