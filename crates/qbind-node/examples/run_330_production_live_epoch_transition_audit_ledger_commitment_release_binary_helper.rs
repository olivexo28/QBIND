//! Run 330 — release-binary helper for the Run 329 **live epoch-transition
//! audit-ledger commitment / durable-audit publication-preparation boundary**.
//!
//! Release-binary evidence for the Run 329 live epoch-transition audit-ledger
//! commitment / durable-audit publication-preparation boundary
...
//! release-built production library symbols. It exercises the **real** Run 329
//! `ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor` and proves,
...
//! 325 -> 327 chain to produce a verified, accepted Run 327/328 live
//! epoch-transition durable-audit-finalization decision (`is_accept()` with
//! `Some(durable_audit_finalization_artifact)`), then feeds that decision into
//! the Run 329 live epoch-transition audit-ledger-commitment executor, which
//! emits only a typed, deterministic, non-mutating audit-ledger-commitment /
//! durable-audit-publication-preparation artifact.
//!
...
//! enablement, audit-ledger-commitment default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! audit-ledger-commitment artifacts; it never applies a live validator-set
...
//! commitment record, durable replay record, settlement record, publication
//! record, or external publication record, never calls Run 070, never mutates
...
//! `LiveEpochTransitionAuditLedgerCommitmentFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_330.md`.
#![allow(dead_code)]
...
fn t_exec_with_policy(
    policy: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy,
) -> ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor {
    ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor::new(
...
/// assert the resulting outcome (fail-closed, no artifact).
fn t_reject_inputs(
    mutate: impl FnOnce(&mut ProductionLiveEpochTransitionAuditLedgerCommitmentInputs),
    expected: TO,
) {
    let mut c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    mutate(&mut c.inputs);
    let d = t_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
    assert!(d.audit_ledger_commitment_artifact.is_none());
    assert!(!d.is_accept());
    assert!(d.outcome.is_non_mutating());
}
...
/// Common helper: build a Devnet/Add case, replace its authority source, and
/// assert the resulting outcome.
fn t_reject_source(source: LiveEpochTransitionAuditLedgerCommitmentAuthoritySource, expected: TO) {
    let mut c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
    let d = t_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
...
// ===========================================================================
// A. Accepted / compatible source-test durable-audit-finalization artifacts
// ===========================================================================

fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        let c = t_case(TrustBundleEnvironment::Devnet, sc);
        let d = t_eval(&c);
        assert!(d.is_accept(), "scenario must accept");
        assert!(d.authorizes_future_mutation_only());
        let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
        assert_eq!(art.staged_kind, expected_crc3_kind(sc));
        assert_eq!(art.audit_ledger_commitment_nonce, CRC3_NONCE);
...
fn accept_all_scenarios_testnet() {
    for sc in ALL_SC {
        let c = t_case(TrustBundleEnvironment::Testnet, sc);
        let d = t_eval(&c);
        assert!(d.is_accept());
        let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
...
fn accept_outcome_carries_kind_env_target_nonce() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = t_eval(&c);
    match d.outcome {
        TO::AcceptedSourceTestLiveEpochTransitionAuditLedgerCommitment {
            execution_kind,
            environment,
            epoch_transition_target,
            audit_ledger_commitment_nonce,
        } => {
            assert_eq!(execution_kind, TK::StageApplyValidatorAdd);
            assert_eq!(environment, TrustBundleEnvironment::Devnet);
            assert_eq!(
                epoch_transition_target,
                c.request.proposed_epoch_transition_target
            );
            assert_eq!(audit_ledger_commitment_nonce, CRC3_NONCE);
        }
        other => panic!("unexpected outcome: {other:?}"),
    }
}

fn accept_artifact_reexposes_consumed_durable_audit_finalization_transcript() {
    let dec = cs_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec
        .durable_audit_finalization_artifact
        .as_ref()
        .unwrap()
        .epoch_transition_target;
    let inputs = t_inputs(TrustBundleEnvironment::Devnet, &dec);
    let handoff_id = dec.durable_audit_finalization_id.clone();
    let handoff_req = dec.request_id.clone();
    let handoff_digest = dec.durable_audit_finalization_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor::source_test();
    let d = exec.evaluate_live_epoch_transition_audit_ledger_commitment(
        &request,
        &inputs,
        &empty_replay329(),
    );
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    assert_eq!(art.durable_audit_finalization_decision_id, handoff_id);
    assert_eq!(art.durable_audit_finalization_request_id, handoff_req);
    assert_eq!(art.durable_audit_finalization_intent_digest, handoff_digest);
    assert_eq!(
        art.durable_audit_finalization_transcript_digest,
        handoff_transcript
    );
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let d = t_eval(&c);
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    assert_eq!(d.audit_ledger_commitment_id, art.audit_ledger_commitment_id);
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(
        d.audit_ledger_commitment_digest,
        art.audit_ledger_commitment_digest
    );
    assert_eq!(d.transcript_digest, art.transcript_digest);
    assert!(!d.audit_ledger_commitment_id.is_empty());
    assert!(!d.request_id.is_empty());
    assert!(!d.audit_ledger_commitment_digest.is_empty());
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d1 = t_eval(&c);
        let d2 = t_eval(&c);
        assert_eq!(d1.audit_ledger_commitment_id, d2.audit_ledger_commitment_id);
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(
            d1.audit_ledger_commitment_digest,
            d2.audit_ledger_commitment_digest
        );
...
fn artifact_content_digest_is_stable() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
    let d = t_eval(&c);
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.audit_ledger_commitment_digest);
...
fn wrong_durable_audit_finalization_decision_id() {
    t_reject_inputs(
        |i| i.expected_durable_audit_finalization_decision_id = "bad".to_string(),
        TO::DurableAuditFinalizationDecisionIdMismatch,
    );
}

fn wrong_durable_audit_finalization_request_id() {
    t_reject_inputs(
        |i| i.expected_durable_audit_finalization_request_id = "bad".to_string(),
        TO::DurableAuditFinalizationDecisionRequestIdMismatch,
    );
}

fn wrong_durable_audit_finalization_intent_digest() {
    t_reject_inputs(
        |i| i.expected_durable_audit_finalization_intent_digest = "bad".to_string(),
        TO::DurableAuditFinalizationDecisionIntentDigestMismatch,
    );
}

fn wrong_durable_audit_finalization_transcript_digest() {
    t_reject_inputs(
        |i| i.expected_durable_audit_finalization_transcript_digest = "bad".to_string(),
        TO::DurableAuditFinalizationDecisionTranscriptMismatch,
    );
}

fn wrong_durable_audit_finalization_nonce() {
    t_reject_inputs(
        |i| i.expected_durable_audit_finalization_nonce = 9999,
        TO::WrongDurableAuditFinalizationNonce,
    );
}

fn tampered_durable_audit_finalization_package_integrity_mismatch() {
    // Mutate the consumed package so its content digest no longer matches the
    // bound handoff decision digest.
    let mut dec = cs_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let target = dec
        .durable_audit_finalization_artifact
        .as_ref()
        .unwrap()
        .epoch_transition_target;
    let inputs = t_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.durable_audit_finalization_artifact
        .as_mut()
        .unwrap()
    ...
    assert_eq!(
        d.outcome,
        TO::DurableAuditFinalizationDecisionIntegrityMismatch
    );
    assert!(d.audit_ledger_commitment_artifact.is_none());
}
...
fn wrong_guarded_mutation_decision_id() {
    t_reject_inputs(
        ...
        TO::GuardedMutationDecisionIdMismatch,
    );
}
...
fn wrong_guarded_mutation_request_id() {
    t_reject_inputs(
        ...
        TO::GuardedMutationDecisionRequestIdMismatch,
    );
}
...
fn wrong_guarded_mutation_intent_digest() {
    t_reject_inputs(
        ...
        TO::GuardedMutationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_guarded_mutation_transcript_digest() {
    t_reject_inputs(
        ...
        TO::GuardedMutationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_decision_id() {
    t_reject_inputs(
        ...
        TO::StagedApplicationDecisionIdMismatch,
    );
}
...
fn wrong_staged_application_request_id() {
    t_reject_inputs(
        ...
        TO::StagedApplicationDecisionRequestIdMismatch,
    );
}
...
fn wrong_staged_application_intent_digest() {
    t_reject_inputs(
        ...
        TO::StagedApplicationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_staged_application_transcript_digest() {
    t_reject_inputs(
        ...
        TO::StagedApplicationDecisionTranscriptMismatch,
    );
}
...
fn wrong_staged_application_nonce() {
    t_reject_inputs(
        ...
        TO::WrongStagedApplicationNonce,
    );
}
...
fn wrong_authorization_decision_id() {
    t_reject_inputs(
        ...
        TO::AuthorizationDecisionIdMismatch,
    );
}
...
fn wrong_authorization_request_id() {
    t_reject_inputs(
        ...
        TO::AuthorizationDecisionRequestIdMismatch,
    );
}
...
fn wrong_authorization_intent_digest() {
    t_reject_inputs(
        ...
        TO::AuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn wrong_authorization_transcript_digest() {
    t_reject_inputs(
        ...
        TO::AuthorizationDecisionTranscriptMismatch,
    );
}
...
fn wrong_application_intent_digest() {
    t_reject_inputs(
        ...
        TO::WrongApplicationIntentDigest,
    );
}
...
fn wrong_application_transcript_digest() {
    t_reject_inputs(
        ...
        TO::WrongApplicationTranscriptDigest,
    );
}
...
fn wrong_governance_execution_intent_digest() {
    t_reject_inputs(
        ...
        TO::WrongGovernanceExecutionIntentDigest,
    );
}
...
fn wrong_rotation_transcript_digest() {
    t_reject_inputs(
        ...
        TO::WrongRotationTranscriptDigest,
    );
}
...
fn wrong_current_validator_set_digest() {
    t_reject_inputs(
        ...
        TO::WrongCurrentValidatorSetDigest,
    );
}
...
fn wrong_proposed_validator_set_digest() {
    t_reject_inputs(
        ...
        TO::WrongProposedValidatorSetDigest,
    );
}
...
fn wrong_validator_set_delta_digest() {
    t_reject_inputs(
        ...
        TO::WrongValidatorSetDeltaDigest,
    );
}
...
fn wrong_current_validator_set_epoch() {
    t_reject_inputs(
        ...
        TO::WrongCurrentValidatorSetEpoch,
    );
}
...
fn wrong_current_validator_set_version() {
    t_reject_inputs(
        ...
        TO::WrongCurrentValidatorSetVersion,
    );
}
...
fn wrong_proposed_validator_count() {
    t_reject_inputs(
        ...
        TO::WrongProposedValidatorCount,
    );
}

fn wrong_rotation_nonce() {
    t_reject_inputs(|i| i.expected_rotation_nonce = 999, TO::WrongRotationNonce);
}
...
fn wrong_epoch_transition_target_request() {
    ...
    let d = t_eval(&c);
    assert_eq!(d.outcome, TO::WrongEpochTransitionTarget);
    assert!(d.audit_ledger_commitment_artifact.is_none());
}
...
fn reject_missing_durable_audit_finalization_decision() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::MissingDurableAuditFinalizationDecision,
        TO::VerifiedDurableAuditFinalizationDecisionRequired,
    );
}

fn reject_unverified_durable_audit_finalization_decision() {
    let dec = cs_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::UnverifiedDurableAuditFinalizationDecision {
            decision: dec,
        },
        TO::UnverifiedDurableAuditFinalizationDecisionRejected,
    );
}
...
fn reject_accepted_durable_audit_finalization_without_package_via_verified_source() {
    let dec = cs_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::VerifiedDurableAuditFinalizationDecision {
            decision: dec,
        },
...
fn reject_accepted_durable_audit_finalization_without_package_variant() {
    let dec = cs_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    t_reject_source(
...
fn reject_guarded_mutation_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::GuardedMutationDecisionWithoutDurableAuditFinalization,
        TO::GuardedMutationDecisionAloneRejected,
    );
}

fn reject_staged_application_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::StagedApplicationDecisionWithoutDurableAuditFinalization,
        TO::StagedApplicationDecisionAloneRejected,
    );
}

fn reject_live_application_authorization_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::LiveApplicationAuthorizationWithoutDurableAuditFinalization,
        TO::LiveApplicationAuthorizationAloneRejected,
    );
}

fn reject_application_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::ApplicationDecisionWithoutDurableAuditFinalization,
        TO::ApplicationDecisionAloneRejected,
    );
}

fn reject_rotation_plan_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::RotationPlanWithoutDurableAuditFinalization,
        TO::RotationPlanAloneRejected,
    );
}

fn reject_governance_execution_intent_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::GovernanceExecutionIntentWithoutDurableAuditFinalization,
        TO::GovernanceExecutionIntentAloneRejected,
    );
}

fn reject_governance_proof_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::GovernanceProofWithoutDurableAuditFinalization,
        TO::GovernanceProofAloneRejected,
    );
}

fn reject_local_operator_assertion() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::LocalOperatorAssertion,
        TO::LocalOperatorProofRejected,
    );
}

fn reject_peer_majority_assertion() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::PeerMajorityAssertion,
        TO::PeerMajorityProofRejected,
    );
}

fn reject_custody_only_evidence() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::CustodyOnlyEvidence,
        TO::CustodyOnlyProofRejected,
    );
}

fn reject_remote_signer_only_evidence() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::RemoteSignerOnlyEvidence,
        TO::RemoteSignerOnlyProofRejected,
    );
}

fn reject_custody_attestation_only_evidence() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::CustodyAttestationOnlyEvidence,
        TO::CustodyAttestationOnlyProofRejected,
    );
}

fn reject_fixture_only_durable_audit_finalization() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::FixtureOnlyDurableAuditFinalization,
        TO::FixtureStagedApplicationRejectedAsProductionAuthority,
    );
}

fn reject_arbitrary_validator_set_bytes() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::ArbitraryValidatorSetBytes,
        TO::ArbitraryValidatorSetBytesRejected,
    );
}
...
fn mainnet_domain_refused_under_source_test_policy() {
    ...
    let d = t_eval(&c);
    assert_eq!(d.outcome, TO::MainNetRefused);
    assert!(d.audit_ledger_commitment_artifact.is_none());
}

fn mainnet_policy_unavailable() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = t_exec_with_policy(
    ...
    assert_eq!(
        d.outcome,
        TO::MainNetProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable
    );
    assert!(d.audit_ledger_commitment_artifact.is_none());
}
...
fn production_policy_unavailable() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = t_exec_with_policy(
        ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::RequireProductionLiveEpochTransitionAuditLedgerCommitment,
    );
    let d = exec.evaluate_live_epoch_transition_audit_ledger_commitment(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        TO::ProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable
    );
    assert!(d.audit_ledger_commitment_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    ...
    assert_eq!(d.outcome, TO::Disabled);
    assert!(d.audit_ledger_commitment_artifact.is_none());
    assert!(!d.is_accept());
}
...
fn reserved_production_kind_fails_closed() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor::new(
        ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::new(
    ...
    assert_eq!(
        d.outcome,
        TO::LiveEpochTransitionAuditLedgerCommitmentBoundaryUnavailable
    );
}
...
fn replay_rejected_when_id_present() {
    ...
    let id = d
        ...
        .clone();
    let replay = vec![id];
    let d2 = t_eval_replay(&c, &replay);
    match d2.outcome {
        TO::StagedApplicationReplayRejected { .. } => {}
        other => panic!("expected replay rejection, got {other:?}"),
    }
    assert!(d2.audit_ledger_commitment_artifact.is_none());
}

fn no_replay_when_id_absent() {
    let c = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let replay: Vec<String> = vec!["some-other-id".to_string()];
    let d = t_eval_replay(&c, &replay);
    assert!(d.is_accept());
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionAuditLedgerCommitmentFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
    assert!(state.apply_prepared_execution(art, &d.audit_ledger_commitment_id));
    ...
    assert!(!state.apply_prepared_execution(art, &d.audit_ledger_commitment_id));
    assert!(state.has_applied(&d.audit_ledger_commitment_id));
}
...
fn fixture_state_apply_all_scenarios() {
    for sc in ALL_SC {
        ...
        let d = t_eval(&c);
        let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
        let mut state = LiveEpochTransitionAuditLedgerCommitmentFixtureState::new(
            CUR_EPOCH,
            CUR_VERSION,
            "start",
        );
...
fn every_outcome_is_non_mutating() {
    ...
    let d = t_eval(&c);
    assert!(d.outcome.is_non_mutating());
    let bad = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let dbad = t_eval_replay(&bad, &[t_eval(&bad).audit_ledger_commitment_id]);
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionAuditLedgerCommitment.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionAuditLedgerCommitment.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionAuditLedgerCommitmentRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}

fn kind_predicates() {
    use ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionAuditLedgerCommitment.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionAuditLedgerCommitment.is_source_test());
    assert_eq!(K::default(), K::Disabled);
}

fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        let hk = expected_crc2_kind(sc);
        let pk = TK::from_staged_application_kind(hk);
        assert_eq!(pk, expected_crc3_kind(sc));
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = TK::from_staged_application_kind(
        LiveEpochTransitionDurableAuditFinalizationKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = TO::DurableAuditFinalizationDecisionIdMismatch;
    let b = TO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(
        a.tag(),
        TO::DurableAuditFinalizationDecisionIdMismatch.tag()
    );
    assert!(!TO::MainNetRefused.tag().is_empty());
}
...
fn config_and_inputs_well_formed() {
    ...
    assert!(
        ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::source_test().is_well_formed()
    );
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        ...
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.audit_ledger_commitment_artifact
                    .as_ref()
                    .unwrap()
                    .environment,
                TrustBundleEnvironment::Testnet
...
macro_rules! per_scenario_durable_audit_finalization_binding {
    ($name:ident, $sc:expr) => {
        #[test]
        fn $name() {
            let mut c = t_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_durable_audit_finalization_decision_id = "bad".to_string();
            let d = t_eval(&c);
            assert_eq!(d.outcome, TO::DurableAuditFinalizationDecisionIdMismatch);
            assert!(d.audit_ledger_commitment_artifact.is_none());
        }
    };
}

per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_durable_audit_finalization_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
macro_rules! per_scenario_guarded_binding {
    ($name:ident, $sc:expr) => {
        ...
        fn $name() {
            ...
            let d = t_eval(&c);
            assert_eq!(d.outcome, TO::GuardedMutationDecisionIdMismatch);
            assert!(d.audit_ledger_commitment_artifact.is_none());
        }
    };
}
...
// ===========================================================================
// Q. Deeper re-exposed Run 323/324 runtime-handoff authority-tuple binding
//    (additive layer introduced by Run 327 on top of the rotated Run 325
//    coverage above). All non-mutating, fail-closed.
// ===========================================================================

fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    let parent = cs_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let ppkg = parent
        .durable_audit_finalization_artifact
        .as_ref()
        .unwrap()
...
fn reject_runtime_handoff_decision_id_mismatch() {
    t_reject_inputs(
        ...
        TO::RuntimeHandoffDecisionIdMismatch,
    );
}
...
fn reject_runtime_handoff_request_id_mismatch() {
    t_reject_inputs(
        ...
        TO::RuntimeHandoffDecisionRequestIdMismatch,
    );
}
...
fn reject_runtime_handoff_intent_digest_mismatch() {
    t_reject_inputs(
        ...
        TO::RuntimeHandoffDecisionIntentDigestMismatch,
    );
}
...
fn reject_runtime_handoff_transcript_mismatch() {
    t_reject_inputs(
        ...
        TO::RuntimeHandoffDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_runtime_handoff_nonce() {
    t_reject_inputs(
        ...
        TO::WrongRuntimeHandoffNonce,
    );
}

fn reject_runtime_handoff_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::RuntimeHandoffDecisionWithoutDurableAuditFinalization,
        TO::RuntimeHandoffDecisionAloneRejected,
    );
}

fn runtime_handoff_alone_tag_is_stable() {
    assert_eq!(
        TO::RuntimeHandoffDecisionAloneRejected.tag(),
        "runtime-handoff-decision-alone-rejected"
    );
    assert_eq!(
        TO::RuntimeHandoffDecisionIdMismatch.tag(),
        "runtime-handoff-decision-id-mismatch"
    );
    assert_eq!(
        TO::WrongRuntimeHandoffNonce.tag(),
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = t_eval(&c);
    let d2 = t_eval(&c);
    let a1 = d1.audit_ledger_commitment_artifact.as_ref().unwrap();
    let a2 = d2.audit_ledger_commitment_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 329 — re-exposed Run 325/326 execution-preparation ancestor binding
// (carried through the consumed Run 327/328 durable-audit-finalization artifact) and
...
fn reject_mutation_execution_decision_id_mismatch() {
    t_reject_inputs(
        ...
        TO::MutationExecutionDecisionIdMismatch,
    );
}
...
fn reject_mutation_execution_request_id_mismatch() {
    t_reject_inputs(
        ...
        TO::MutationExecutionDecisionRequestIdMismatch,
    );
}
...
fn reject_mutation_execution_intent_digest_mismatch() {
    t_reject_inputs(
        ...
        TO::MutationExecutionDecisionIntentDigestMismatch,
    );
}
...
fn reject_mutation_execution_transcript_mismatch() {
    t_reject_inputs(
        ...
        TO::MutationExecutionDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_mutation_execution_nonce() {
    t_reject_inputs(
        ...
        TO::WrongMutationExecutionNonce,
    );
}

fn reject_mutation_execution_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::MutationExecutionDecisionWithoutDurableAuditFinalization,
        TO::MutationExecutionDecisionAloneRejected,
    );
}
...
fn reject_commit_authorization_decision_id_mismatch() {
    t_reject_inputs(
        ...
        TO::CommitAuthorizationDecisionIdMismatch,
    );
}
...
fn reject_commit_authorization_request_id_mismatch() {
    t_reject_inputs(
        ...
        TO::CommitAuthorizationDecisionRequestIdMismatch,
    );
}
...
fn reject_commit_authorization_intent_digest_mismatch() {
    t_reject_inputs(
        ...
        TO::CommitAuthorizationDecisionIntentDigestMismatch,
    );
}
...
fn reject_commit_authorization_transcript_mismatch() {
    t_reject_inputs(
        ...
        TO::CommitAuthorizationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_commit_authorization_nonce() {
    t_reject_inputs(
        ...
        TO::WrongCommitAuthorizationNonce,
    );
}

fn reject_commit_authorization_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::CommitAuthorizationDecisionWithoutDurableAuditFinalization,
        TO::CommitAuthorizationDecisionAloneRejected,
    );
}
...
fn mutation_execution_alone_tag_is_stable() {
    assert_eq!(
        TO::MutationExecutionDecisionAloneRejected.tag(),
        "mutation-execution-decision-alone-rejected"
    );
    assert_eq!(
        TO::MutationExecutionDecisionIdMismatch.tag(),
        "mutation-execution-decision-id-mismatch"
    );
    assert_eq!(
        TO::WrongMutationExecutionNonce.tag(),
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    assert!(d.is_accept());
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    // Grandparent Run 325/326 mutation-execution nonce re-exposed intact.
    assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
    // Parent Run 327/328 durable-audit-finalization nonce re-exposed intact.
    assert_eq!(art.durable_audit_finalization_nonce, CRC2_NONCE);
    // Self Run 329 audit-ledger-commitment proposed nonce.
    assert_eq!(art.audit_ledger_commitment_nonce, CRC3_NONCE);
}
...
fn reject_execution_preparation_decision_id_mismatch() {
    t_reject_inputs(
        ...
        TO::ExecutionPreparationDecisionIdMismatch,
    );
}
...
fn reject_execution_preparation_request_id_mismatch() {
    t_reject_inputs(
        ...
        TO::ExecutionPreparationDecisionRequestIdMismatch,
    );
}
...
fn reject_execution_preparation_intent_digest_mismatch() {
    t_reject_inputs(
        ...
        TO::ExecutionPreparationDecisionIntentDigestMismatch,
    );
}
...
fn reject_execution_preparation_transcript_mismatch() {
    t_reject_inputs(
        ...
        TO::ExecutionPreparationDecisionTranscriptMismatch,
    );
}
...
fn reject_wrong_execution_preparation_nonce() {
    t_reject_inputs(
        ...
        TO::WrongExecutionPreparationNonce,
    );
}

fn reject_execution_preparation_decision_alone() {
    t_reject_source(
        LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::ExecutionPreparationDecisionWithoutDurableAuditFinalization,
        TO::ExecutionPreparationDecisionAloneRejected,
    );
}

fn execution_preparation_alone_tag_is_stable() {
    assert_eq!(
        TO::ExecutionPreparationDecisionAloneRejected.tag(),
        "execution-preparation-decision-alone-rejected"
    );
    assert_eq!(
        TO::ExecutionPreparationDecisionIdMismatch.tag(),
        "execution-preparation-decision-id-mismatch"
    );
    assert_eq!(
        TO::WrongExecutionPreparationNonce.tag(),
...
fn accepted_artifact_re_exposes_execution_preparation_ancestor() {
    ...
    assert!(d.is_accept());
    let art = d.audit_ledger_commitment_artifact.as_ref().unwrap();
    // Grandparent Run 325/326 execution-preparation nonce re-exposed intact.
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary/helper_evidence/run_330",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_durable_audit_finalization_transcript",
            accept_artifact_reexposes_consumed_durable_audit_finalization_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_durable_audit_finalization_decision_id",
            wrong_durable_audit_finalization_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_finalization_request_id",
            wrong_durable_audit_finalization_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_finalization_intent_digest",
            wrong_durable_audit_finalization_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_finalization_transcript_digest",
            wrong_durable_audit_finalization_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_finalization_nonce",
            wrong_durable_audit_finalization_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_durable_audit_finalization_package_integrity_mismatch",
            tampered_durable_audit_finalization_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_durable_audit_finalization_decision",
            reject_missing_durable_audit_finalization_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_durable_audit_finalization_decision",
            reject_unverified_durable_audit_finalization_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_durable_audit_finalization_without_package_via_verified_source",
            reject_accepted_durable_audit_finalization_without_package_via_verified_source
                as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_durable_audit_finalization_without_package_variant",
            reject_accepted_durable_audit_finalization_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_durable_audit_finalization",
            reject_fixture_only_durable_audit_finalization as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 329 domain-separated digest free functions.
    let case = t_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = t_eval(&case);
    let artifact = decision
        .audit_ledger_commitment_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live audit-ledger-commitment artifact");
    // On accept, the executor derives the identifiers from the consumed Run
    // 325/326 durable-audit-finalization decision's durable-audit-finalization (content)
    // digest.
    let cs_dec = cs_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let durable_audit_finalization_intent_digest = cs_dec.durable_audit_finalization_digest.clone();
    let content_digest =
        production_live_epoch_transition_audit_ledger_commitment_content_digest(artifact);
    let audit_ledger_commitment_id = production_live_epoch_transition_audit_ledger_commitment_id(
        ...
        &case.inputs.audit_ledger_commitment_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.audit_ledger_commitment_nonce,
    );
    ...
    digest_body.push_str(&format!(
        "durable_audit_finalization_intent_digest={durable_audit_finalization_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = t_case(TrustBundleEnvironment::Devnet, sc);
        let decision = t_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "audit_ledger_commitment_id={}\n",
            decision.audit_ledger_commitment_id
        ));
    ...
    digest_body.push_str(&format!(
        "named_audit_ledger_commitment_id={audit_ledger_commitment_id}\n"
    ));
    ...
    fs::write(
        outdir.join("fixtures/run_330_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 329 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact` and its
    // `ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact = artifact;
    let clean: ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_audit_ledger_commitment_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_audit_ledger_commitment_window(
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_330_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_audit_ledger_commitment.rs :: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor (Run 329 source/test boundary)\n");
