//! Run 287 — release-binary durable-completion external-publication-audit-finalization
//! consumer / external-publication-audit-completion boundary helper.
//!
//! Release-binary evidence for the Run 286 durable-completion external-publication
...
use qbind_node::pqc_governance_durable_completion_external_publication_audit_archive::{
    ...
    external_publication_audit_archive_request_digest,
    external_publication_audit_archive_response_digest,
    external_publication_audit_archive_record_digest,
    external_publication_audit_archive_transcript_digest,
    durable_completion_external_publication_audit_archive_consumer_required,
    durable_completion_external_publication_audit_archive_confirmation_required,
    durable_completion_external_publication_audit_archive_no_real_settlement,
    ...
    DurableCompletionExternalPublicationAuditArchiveEnvironment,
    ...
    DurableCompletionExternalPublicationAuditArchiveLedgerStatus,
    DurableCompletionExternalPublicationAuditArchiveRecord,
    DurableCompletionExternalPublicationAuditArchiveResponse,
    DurableCompletionExternalPublicationAuditArchiveSurface,
...
// ===========================================================================
// Run 284 external_publication_audit_completion attachment (prior boundary)
// ===========================================================================

/// The real Run 284 external-publication-audit-completion path, evaluated from
/// the real Run 282 `ExternalPublicationAuditFinalizationRecorded` path. This is
/// the carried attachment layer between the Run 282 audit-finalization boundary
/// and the Run 286 audit-archive boundary. Only a recorded Run 284
/// audit-completion outcome may create a Run 286 audit-archive request.
...
fn attach_run284_external_publication_audit_completion(
    ...
    let record = ledger
        ...
        .expect("recorded Run 284 external publication audit completion");
    AttachedExternalPublicationAuditCompletion {
        outcome,
        external_publication_audit_completion_record_id: action.external_publication_audit_completion_record_id.clone(),
...
struct Ctx {
    ...
    consumer: AttachedConsumer,
    external_publication_audit_completion: AttachedExternalPublicationAuditCompletion,
}

fn ack_identity(
    policy: DurableCompletionExternalPublicationAuditArchivePolicy,
    kind: DurableCompletionExternalPublicationAuditArchiveKind,
) -> DurableCompletionExternalPublicationAuditArchiveIdentity {
    DurableCompletionExternalPublicationAuditArchiveIdentity {
        confirmation_id: AUDIT_COMPLETION_ID.to_string(),
        kind,
        policy,
        domain_separation_tag: AUDIT_COMPLETION_DOMAIN_TAG.to_string(),
    }
}
...
fn ctx_action(
    ...
    let external_publication_audit_completion =
        attach_run284_external_publication_audit_completion(environment, action, consumer_duplicate);
    ...
    let expectations = DurableCompletionExternalPublicationAuditArchiveExpectations {
        ...
        expected_external_publication_audit_archive_kind: kind,
        expected_external_publication_audit_archive_policy: policy,
        expected_domain_separation_tag: AUDIT_COMPLETION_DOMAIN_TAG.to_string(),
    };
    Ctx {
        env,
        runtime,
...
impl Ctx {
    ...
    fn input_with_confirmation(
        ...
        external_publication_audit_archive: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        external_publication_audit_completion: DurableCompletionExternalPublicationAuditCompletionOutcome,
    ) -> DurableCompletionExternalPublicationAuditArchiveInput {
        DurableCompletionExternalPublicationAuditArchiveInput {
            policy,
            ...
            confirmation_binding: external_publication_audit_archive,
            ...
            external_publication_audit_completion_binding: external_publication_audit_completion,
    ...
    fn input(
        ...
        self.input_with_confirmation(
            ...
            self.external_publication_audit_completion.outcome.clone(),
        )
    }

    fn recorded(&self) -> DurableCompletionExternalPublicationAuditArchiveInput {
        self.input_with_confirmation(
...
fn fixture_sink() -> FixtureDurableCompletionExternalPublicationAuditArchiveSink {
    FixtureDurableCompletionExternalPublicationAuditArchiveSink::new()
}
fn disabled_receipt_policy_preserves_legacy_bypass_no_record_no_invocation() {
    let c = devnet_ctx();
    let input = c.input(
        DurableCompletionExternalPublicationAuditArchivePolicy::Disabled,
        DurableReplayObservation::MutationAuthorized,
        GovernanceModeledEndToEndPipelineOutcome::ModeledApplierAppliedAndDurableConsumeAuthorized,
        GovernanceModeledDurableConsumeSinkOutcome::ConsumeReceiptRecorded,
        GovernanceModeledDurableConsumeCompletionReporterOutcome::CompletionReportRecorded,
    ...
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &input,
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(external_publication_audit_archive_outcome_authorizes_record(&outcome));
    assert!(external_publication_audit_archive_outcome_projects_to_recorded(&outcome));
    assert_eq!(sink.invocations(), 1);
    assert_eq!(ledger.len(), 1);
    assert!(ledger.contains(AUDIT_COMPLETION_RECORD_ID));
    assert_eq!(
        external_publication_audit_archive_identity_digest(&c.request.identity),
...
fn governance_action_variants_record_only_after_backend_submission() {
    ...
    for label in [
        ...
        assert_eq!(ledger.len(), 1);
        assert!(ledger.contains(&action.external_publication_audit_archive_record_id));
    }
}
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!external_publication_audit_archive_outcome_authorizes_record(&second));
    assert!(external_publication_audit_archive_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

fn run268_duplicate_idempotent_confirmation_only_matches_existing_never_creates() {
    // A real Run 284 ExternalPublicationAuditCompletionDuplicateIdempotent outcome with
    // identical digests.
    ...
    assert_eq!(
        dup_ctx.external_publication_audit_completion.outcome,
        DurableCompletionExternalPublicationAuditCompletionOutcome::ExternalPublicationAuditCompletionDuplicateIdempotent
    );
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRejectedBeforeRecord
    );
    assert!(ledger.is_empty());

    // After a real receipt records, the duplicate-idempotent backend matches it.
    let rec_ctx = devnet_ctx();
    let recorded = evaluate_durable_completion_external_publication_audit_archive(
        &rec_ctx.recorded(),
    ...
    let matched = evaluate_durable_completion_external_publication_audit_archive(
        &dup_ctx.recorded(),
        &dup_ctx.expectations,
        &mut sink,
        &mut ledger,
...
fn production_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditArchivePolicy::ProductionExternalPublicationAuditArchiveRequired,
        DurableCompletionExternalPublicationAuditArchiveKind::ProductionExternalPublicationAuditArchiveUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    let mut sink = ProductionExternalPublicationAuditArchiveSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn mainnet_audit_ledger_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditArchivePolicy::MainNetExternalPublicationAuditArchiveRequired,
        DurableCompletionExternalPublicationAuditArchiveKind::MainNetExternalPublicationAuditArchiveUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    let mut sink = MainNetExternalPublicationAuditArchiveSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn external_publication_path_reachable_but_unavailable_records_nothing() {
    let c = ctx(
        ...
        GovernanceExecutionRuntimeSurface::ReloadApply,
        GovernanceExecutionRuntimeSurface::ReloadApply,
        DurableCompletionExternalPublicationAuditArchivePolicy::ExternalExternalPublicationAuditArchiveRequired,
        DurableCompletionExternalPublicationAuditArchiveKind::ExternalExternalPublicationAuditArchiveUnavailable,
    );
    let mut ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    let mut sink = ExternalExternalPublicationAuditArchiveSink::default();
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &c.recorded(),
        &c.expectations,
        &mut sink,
...
fn assert_non_recording_confirmation(
    external_publication_audit_completion: DurableCompletionExternalPublicationAuditCompletionOutcome,
    expected: DurableCompletionExternalPublicationAuditArchiveOutcome,
) {
    let c = devnet_ctx();
    let input = c.input_with_confirmation(
        DurableCompletionExternalPublicationAuditArchivePolicy::FixtureAllowed,
...
fn non_recording_confirmation_outcomes_never_record_external_publication_audit_archive() {
    ...
    assert_non_recording_confirmation(
        ...
        Receipt::LegacyBypassNoExternalPublicationAuditArchive,
    );
    assert_non_recording_confirmation(
    ...
    assert_non_recording_confirmation(
        Finalization::ProductionExternalPublicationAuditCompletionUnavailableNoAuditCompletion,
        Receipt::ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive,
    );
    assert_non_recording_confirmation(
        Finalization::MainNetExternalPublicationAuditCompletionUnavailableNoAuditCompletion,
        Receipt::ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive,
    );
    assert_non_recording_confirmation(
        Finalization::ExternalExternalPublicationAuditCompletionUnavailableNoAuditCompletion,
        Receipt::ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive,
    );
    assert_non_recording_confirmation(
        Finalization::MainNetPeerDrivenApplyRefusedNoAuditCompletion,
        Receipt::MainNetPeerDrivenApplyRefusedNoAuditArchive,
    );
    assert_non_recording_confirmation(
        Finalization::ValidatorSetRotationUnsupportedNoAuditCompletion,
        Receipt::ValidatorSetRotationUnsupportedNoAuditArchive,
    );
    assert_non_recording_confirmation(
        Finalization::PolicyChangeUnsupportedNoAuditCompletion,
        Receipt::PolicyChangeUnsupportedNoAuditArchive,
    );
}
...
fn assert_binding_mismatch_rejected(mut mutate: impl FnMut(&mut Ctx)) {
    ...
    assert_eq!(
        outcome,
        DurableCompletionExternalPublicationAuditArchiveOutcome::RejectedBeforeExternalPublicationAuditCompletionNoAuditArchive
    );
    assert_eq!(sink.invocations(), 0);
    assert!(ledger.is_empty());
}
...
fn assert_request_mismatch_rejected(
    mut mutate: impl FnMut(&mut DurableCompletionExternalPublicationAuditArchiveRequest),
) {
    let c = devnet_ctx();
...
fn wrong_external_publication_audit_archive_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.external_publication_audit_archive_record_id = "other-confirmation".to_string();
    });
}
...
fn same_ack_record_id_different_digest_is_equivocation_no_second_receipt() {
    ...
    let equivocation = evaluate_durable_completion_external_publication_audit_archive(
        &c2.recorded(),
        &c2.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_fault_no_acknowledgement(
    fault: DurableCompletionExternalPublicationAuditArchiveFault,
    expected: DurableCompletionExternalPublicationAuditArchiveOutcome,
) {
    let c = devnet_ctx();
    let input = c.recorded();
    let mut ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    let mut sink = FixtureDurableCompletionExternalPublicationAuditArchiveSink::with_fault(fault);
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &input,
        &c.expectations,
        &mut sink,
...
fn receipt_record_failed_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRecordFailedNoAuditArchive,
    );
}
...
fn receipt_rollback_completed_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRolledBackNoAuditArchive,
    );
}
...
fn receipt_rollback_failed_fatal_never_records() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRollbackFailedFatalNoAuditArchive,
    );
}
...
fn receipt_ambiguous_window_fails_closed() {
    assert_fault_no_acknowledgement(
        ...
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive,
    );
}
...
fn recovered_ledger() -> DurableCompletionExternalPublicationAuditArchiveLedger {
    let c = devnet_ctx();
    let mut ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    let mut sink = fixture_sink();
    let _ = evaluate_durable_completion_external_publication_audit_archive(
        &c.recorded(),
        &c.expectations,
        &mut sink,
        &mut ledger,
...
fn assert_window(
    ...
    let record = if with_record {
        ledger.find(AUDIT_COMPLETION_RECORD_ID)
    } else {
        None
    };
    let outcome = recover_durable_completion_external_publication_audit_archive_window(
        &input,
        window,
        DurableCompletionExternalPublicationAuditArchiveKind::FixtureInMemory,
        record,
...
fn after_external_publication_audit_finalization_success_recovers_as_recorded() {
    ...
    use DurableCompletionExternalPublicationAuditArchiveWindow as Window;
    assert_window(
        Window::AfterExternalPublicationAuditArchiveSuccess,
        true,
        Finalization::ExternalPublicationAuditArchiveRecorded,
    );
...
fn production_mainnet_external_recovery_classification_unavailable() {
    ...
    let outcome = recover_durable_completion_external_publication_audit_archive_window(
        &input,
        DurableCompletionExternalPublicationAuditArchiveWindow::AfterExternalPublicationAuditArchiveSuccess,
        DurableCompletionExternalPublicationAuditArchiveKind::ProductionExternalPublicationAuditArchiveUnavailable,
        None,
        &c.expectations,
    );
...
fn rollback_restores_receipt_ledger_snapshot() {
    ...
    let mut faulted = FixtureDurableCompletionExternalPublicationAuditArchiveSink::with_fault(
        DurableCompletionExternalPublicationAuditArchiveFault::RolledBackNoReceipt,
    );
    let outcome = evaluate_durable_completion_external_publication_audit_archive(
        &c2.recorded(),
...
fn invariant_helpers_assert_fail_closed_contract() {
    assert!(durable_completion_external_publication_audit_archive_rejection_is_non_mutating());
    assert!(durable_completion_external_publication_audit_archive_never_calls_run_070());
    assert!(durable_completion_external_publication_audit_archive_never_mutates_live_pqc_trust_state());
    assert!(durable_completion_external_publication_audit_archive_never_writes_sequence_or_marker());
    assert!(durable_completion_external_publication_audit_archive_no_rocksdb_file_schema_migration_change());
    assert!(durable_completion_external_publication_audit_archive_no_external_publication());
    assert!(durable_completion_external_publication_audit_archive_no_real_audit_ledger());
    assert!(durable_completion_external_publication_audit_archive_pipeline_success_required());
    assert!(durable_completion_external_publication_audit_archive_sink_receipt_required());
    assert!(durable_completion_external_publication_audit_archive_completion_report_required());
    assert!(durable_completion_external_publication_audit_archive_finalization_projection_required());
    assert!(durable_completion_external_publication_audit_archive_attestation_required());
    assert!(durable_completion_external_publication_audit_archive_backend_submission_required());
    assert!(durable_completion_external_publication_audit_archive_receipt_required());
    assert!(durable_completion_external_publication_audit_archive_record_required_before_reported());
    assert!(durable_completion_external_publication_audit_archive_failed_record_never_records());
    assert!(durable_completion_external_publication_audit_archive_rollback_never_records());
    assert!(durable_completion_external_publication_audit_archive_ambiguous_window_fails_closed());
    assert!(
        durable_completion_external_publication_audit_archive_mainnet_peer_driven_apply_refused_first(
            TrustBundleEnvironment::Mainnet
        )
    ...
    assert!(durable_completion_external_publication_audit_archive_production_mainnet_unavailable());
    assert!(durable_completion_external_publication_audit_archive_external_unavailable());
    assert!(durable_completion_external_publication_audit_archive_validator_set_rotation_unsupported());
    assert!(durable_completion_external_publication_audit_archive_policy_change_unsupported());
    assert!(
        durable_completion_external_publication_audit_archive_local_operator_cannot_satisfy_mainnet_authority()
    );
    assert!(
...
fn release_symbol_reachability_probe() {
    ...
    assert!(matches!(
        intent,
        DurableCompletionExternalPublicationAuditArchiveRequestIntent::CreateRequest
    ));
    ...
    let identity_digest: DurableCompletionExternalPublicationAuditArchiveDigest =
        external_publication_audit_archive_identity_digest(&c.request.identity);
    let request_digest: DurableCompletionExternalPublicationAuditArchiveDigest =
        external_publication_audit_archive_request_digest(&c.request);
    let response = DurableCompletionExternalPublicationAuditArchiveResponse {
        external_publication_audit_archive_record_id: c
            .request
    ...
    let response_digest: DurableCompletionExternalPublicationAuditArchiveDigest =
        external_publication_audit_archive_response_digest(&response);
    let record: DurableCompletionExternalPublicationAuditArchiveRecord = c.request.to_record();
    let record_digest: DurableCompletionExternalPublicationAuditArchiveDigest =
        external_publication_audit_archive_record_digest(&record);
    let transcript_digest: DurableCompletionExternalPublicationAuditArchiveTranscriptDigest =
        external_publication_audit_archive_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
    ...
    let ledger_record: &DurableCompletionExternalPublicationAuditArchiveLedgerRecord = ledger
        .find(AUDIT_COMPLETION_RECORD_ID)
    ...
    let snapshot: DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot =
        ledger.snapshot();
    assert_eq!(snapshot.len(), 1);

    let _surface: DurableCompletionExternalPublicationAuditArchiveSurface = input.surface();
    let _environment: DurableCompletionExternalPublicationAuditArchiveEnvironment =
        input.environment_binding.clone();
    let _binding: DurableCompletionExternalPublicationAuditArchiveBinding =
        input.runtime_binding.clone();

    assert!(DurableCompletionExternalPublicationAuditArchivePolicy::Disabled.is_disabled());
    assert!(
        DurableCompletionExternalPublicationAuditArchivePolicy::FixtureAllowed.allows_fixture()
    );
    assert_eq!(
        DurableCompletionExternalPublicationAuditArchivePolicy::ProductionExternalPublicationAuditArchiveRequired.tag(),
        "production-external-publication-audit-archive-required"
    );
    assert!(DurableCompletionExternalPublicationAuditArchiveKind::FixtureInMemory.is_fixture());
    assert!(DurableCompletionExternalPublicationAuditArchiveKind::ProductionExternalPublicationAuditArchiveUnavailable.is_unavailable());
    assert_eq!(
        DurableCompletionExternalPublicationAuditArchiveKind::Disabled.tag(),
        "disabled"
    );
    let no_finalization =
        DurableCompletionExternalPublicationAuditArchiveOutcome::LegacyBypassNoExternalPublicationAuditArchive;
    ...
    assert!(durable_completion_external_publication_audit_archive_consumer_required());
    assert!(durable_completion_external_publication_audit_archive_confirmation_required());
    assert!(durable_completion_external_publication_audit_archive_no_real_settlement());
    assert!(durable_completion_external_publication_audit_archive_no_real_settlement_finality());
    assert!(durable_completion_external_publication_audit_archive_no_real_settlement_receipt());
    assert!(
        durable_completion_external_publication_audit_archive_no_real_settlement_finality_projection()
    );
    assert!(
    ...
    let mut prod = ProductionExternalPublicationAuditArchiveSink::default();
    let mut mainnet = MainNetExternalPublicationAuditArchiveSink::default();
    let mut external = ExternalExternalPublicationAuditArchiveSink::default();
    let mut prod_ledger = DurableCompletionExternalPublicationAuditArchiveLedger::new();
    assert_eq!(
        prod.project_durable_completion_external_publication_audit_archive(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        mainnet.project_durable_completion_external_publication_audit_archive(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
    ...
    assert_eq!(
        external.project_durable_completion_external_publication_audit_archive(
            &c.request,
            &c.expectations,
            false,
            &mut prod_ledger,
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("docs/devnet/run_287_durable_completion_external_publication_audit_archive_release_binary/helper_evidence/run_287")
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("rejection_fail_closed", "non_recording_confirmation_outcomes_never_record_external_publication_audit_archive", non_recording_confirmation_outcomes_never_record_external_publication_audit_archive as fn()),
        ...
        ("rejection_fail_closed", "wrong_external_publication_audit_archive_record_id_rejected_before_record", wrong_external_publication_audit_archive_record_id_rejected_before_record as fn()),
        ...
        ("rejection_fail_closed", "non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request as fn()),
        ...
        ("projection", "only_recorded_confirmation_outcome_creates_external_publication_audit_archive_request_intent", only_recorded_confirmation_outcome_creates_external_publication_audit_archive_request_intent as fn()),
        ("projection", "non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request as fn()),
        ...
        ("external_publication_audit_archive_ledger", "duplicate_identical_receipt_is_idempotent", duplicate_identical_receipt_is_idempotent as fn()),
        ("external_publication_audit_archive_ledger", "same_ack_record_id_different_digest_is_equivocation_no_second_receipt", same_ack_record_id_different_digest_is_equivocation_no_second_receipt as fn()),
        ("external_publication_audit_archive_ledger", "rollback_restores_receipt_ledger_snapshot", rollback_restores_receipt_ledger_snapshot as fn()),
        ...
        ("non_mutation", "non_recording_confirmation_outcomes_never_record_external_publication_audit_archive", non_recording_confirmation_outcomes_never_record_external_publication_audit_archive as fn()),
        ("non_mutation", "non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request", non_recording_confirmation_outcomes_create_no_external_publication_audit_archive_request as fn()),
    ...
    summary.push_str(
        "Run 287 durable-completion external-publication-audit-completion release helper\n",
    );
    ...
    fs::write(
        ...
        "input.external_publication_audit_completion_binding\nproject_external_publication_audit_completion_outcome_to_external_publication_audit_archive_request\nExternalPublicationAuditCompletionRecorded -> CreateRequest\n",
