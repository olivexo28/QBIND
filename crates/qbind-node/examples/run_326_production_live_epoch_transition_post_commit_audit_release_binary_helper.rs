//! Run 326 — release-binary helper for the Run 325 **live epoch-transition
//! post-commit audit seal / durable-audit authorization boundary**.
//!
//! Release-binary evidence for the Run 325 live epoch-transition post-commit
//! audit seal / durable-audit authorization boundary
...
//! release-built production library symbols. It exercises the **real** Run 325
//! `ProductionLiveEpochTransitionPostCommitAuditExecutor` and proves, per
...
//! -> 311 -> 313 -> 315 -> 317 -> 319 -> 321 -> 323 chain to produce a verified,
//! accepted Run 323/324 live epoch-transition commit-receipt decision
//! (`is_accept()` with `Some(commit_receipt_artifact)`), then feeds that
//! decision into the Run 325 live epoch-transition post-commit audit executor,
//! which emits only a typed, deterministic, non-mutating live post-commit-audit
//! artifact.
//!
...
//! enablement, post-commit-audit default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! post-commit-audit artifacts; it never applies a live
//! validator-set change, never transitions a consensus epoch, never commits or
//! finalizes production runtime state, never writes a production receipt, audit
//! record, audit seal, durable replay record, settlement record, or external
//! publication record, never calls Run 070, never mutates
...
//! `LiveEpochTransitionPostCommitAuditFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_326.md`.
#![allow(dead_code)]
...
// ===========================================================================
// SELF BOUNDARY: Run 325 live epoch-transition post-commit audit executor
// ===========================================================================

use qbind_node::pqc_production_live_epoch_transition_post_commit_audit::*;

use LiveEpochTransitionPostCommitAuditKind as AK;
use ProductionLiveEpochTransitionPostCommitAuditOutcome as AO;

const PCA_POLICY_ID: &str = "post-commit-audit-policy-1";
const PCA_NONCE: u64 = 49;

fn expected_pca_kind(sc: Sc) -> AK {
    match sc {
...
/// Build an accepted Run 325/324 commit-receipt decision — the sole
...
fn cr_decision(
    ...
    assert!(
        d.is_accept(),
        "run 321 commit-receipt decision must accept for fixture"
    );
    d
}
...
fn a_inputs(
    ...
    ProductionLiveEpochTransitionPostCommitAuditInputs {
        trust_domain: trust_domain(env),
        post_commit_audit_policy_id: PCA_POLICY_ID.to_string(),
        expected_authorization_policy_id: AUTH_POLICY_ID.to_string(),
        expected_application_policy_id: APP_POLICY_ID.to_string(),
        expected_governance_domain_id: GOV_DOMAIN.to_string(),
        expected_governance_epoch: GOV_EPOCH,
...
fn a_case(env: TrustBundleEnvironment, sc: Sc) -> A325 {
...
fn a_eval(case: &A325) -> ProductionLiveEpochTransitionPostCommitAuditDecision {
    case.executor
...
/// Common helper: build a Devnet/Add case, replace its authority source, and
/// assert the resulting outcome.
fn a_reject_source(source: LiveEpochTransitionPostCommitAuditAuthoritySource, expected: AO) {
    let mut c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
    let d = a_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
...
fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        ...
        assert_eq!(art.staged_kind, expected_pca_kind(sc));
        assert_eq!(art.post_commit_audit_nonce, PCA_NONCE);
...
fn accept_outcome_carries_kind_env_target_nonce() {
    ...
    let d = a_eval(&c);
    match d.outcome {
        AO::AcceptedSourceTestLiveEpochTransitionPostCommitAudit {
            execution_kind,
            environment,
            epoch_transition_target,
            ...
            assert_eq!(post_commit_audit_nonce, PCA_NONCE);
        }
...
fn wrong_commit_receipt_decision_id() {
    a_reject_inputs(
        ...
        AO::CommitReceiptDecisionIdMismatch,
    );
}
...
fn wrong_commit_receipt_request_id() {
    a_reject_inputs(
        ...
        AO::CommitReceiptDecisionRequestIdMismatch,
    );
}
...
fn wrong_commit_receipt_intent_digest() {
    a_reject_inputs(
        ...
        AO::CommitReceiptDecisionIntentDigestMismatch,
    );
}
...
fn wrong_commit_receipt_transcript_digest() {
    a_reject_inputs(
        ...
        AO::CommitReceiptDecisionTranscriptMismatch,
    );
}
...
fn tampered_commit_receipt_package_integrity_mismatch() {
    ...
    assert_eq!(d.outcome, AO::CommitReceiptDecisionIntegrityMismatch);
    assert!(d.post_commit_audit_artifact.is_none());
}
...
fn reject_missing_commit_receipt_decision() {
    a_reject_source(
        ...
        AO::VerifiedCommitReceiptDecisionRequired,
    );
}
...
fn reject_unverified_commit_receipt_decision() {
    ...
    a_reject_source(
        ...
        AO::UnverifiedCommitReceiptDecisionRejected,
    );
}
...
fn mainnet_policy_unavailable() {
    ...
    assert_eq!(
        d.outcome,
        AO::MainNetProductionLiveEpochTransitionPostCommitAuditUnavailable
    );
    assert!(d.post_commit_audit_artifact.is_none());
}
...
fn production_policy_unavailable() {
    ...
    assert_eq!(
        d.outcome,
        AO::ProductionLiveEpochTransitionPostCommitAuditUnavailable
    );
    assert!(d.post_commit_audit_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    let c = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec =
        a_exec_with_policy(ProductionLiveEpochTransitionPostCommitAuditExecutorPolicy::Disabled);
    let d = exec.evaluate_live_epoch_transition_post_commit_audit(
        &c.request,
        &c.inputs,
        &empty_replay325(),
...
fn reserved_production_kind_fails_closed() {
    ...
    assert_eq!(
        d.outcome,
        AO::LiveEpochTransitionPostCommitAuditBoundaryUnavailable
    );
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.post_commit_audit_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionPostCommitAuditFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn every_outcome_is_non_mutating() {
    ...
    let dbad = a_eval_replay(&bad, &[a_eval(&bad).post_commit_audit_id]);
...
fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        ...
        assert_eq!(pk, expected_pca_kind(sc));
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = AK::from_staged_application_kind(
        LiveEpochTransitionCommitReceiptKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = AO::CommitReceiptDecisionIdMismatch;
    let b = AO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), AO::CommitReceiptDecisionIdMismatch.tag());
...
macro_rules! per_scenario_commit_receipt_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d = a_eval(&c);
            assert_eq!(d.outcome, AO::CommitReceiptDecisionIdMismatch);
            assert!(d.post_commit_audit_artifact.is_none());
        }
    };
}
...
//    (additive layer introduced by Run 323 on top of the rotated Run 319
...
// ===========================================================================
// Run 325 — re-exposed Run 319/320 execution-preparation ancestor binding
// (carried through the consumed Run 325/324 commit-receipt artifact) and
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    // Parent Run 325/324 commit-receipt nonce re-exposed intact.
    assert_eq!(art.commit_receipt_nonce, CRC_NONCE);
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_326_production_live_epoch_transition_post_commit_audit_release_binary/helper_evidence/run_326",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_commit_receipt_transcript",
            accept_artifact_reexposes_consumed_commit_receipt_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_commit_receipt_decision_id",
            wrong_commit_receipt_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_receipt_request_id",
            wrong_commit_receipt_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_receipt_intent_digest",
            wrong_commit_receipt_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_receipt_transcript_digest",
            wrong_commit_receipt_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_receipt_nonce",
            wrong_commit_receipt_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_commit_receipt_package_integrity_mismatch",
            tampered_commit_receipt_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_commit_receipt_decision",
            reject_missing_commit_receipt_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_commit_receipt_decision",
            reject_unverified_commit_receipt_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_commit_receipt_without_package_via_verified_source",
            reject_accepted_commit_receipt_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_commit_receipt_without_package_variant",
            reject_accepted_commit_receipt_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_commit_receipt",
            reject_fixture_only_commit_receipt as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 325 domain-separated digest free functions.
    let case = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = a_eval(&case);
    let artifact = decision
        .post_commit_audit_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live post-commit-audit artifact");
    // On accept, the executor derives the identifiers from the consumed Run
    // 319/320 commit-receipt decision's commit-receipt (content)
    // digest.
    let cr_dec = cr_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let commit_receipt_intent_digest = cr_dec.commit_receipt_digest.clone();
    let content_digest =
        production_live_epoch_transition_post_commit_audit_content_digest(artifact);
    let post_commit_audit_id = production_live_epoch_transition_post_commit_audit_id(
        ...
        &case.inputs.post_commit_audit_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.post_commit_audit_nonce,
    );
    ...
    let transcript_digest = production_live_epoch_transition_post_commit_audit_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMMIT_AUDIT_PROTOCOL_VERSION,
        &request_id,
        &content_digest,
        outcome_tag,
    ...
    digest_body.push_str(&format!(
        "commit_receipt_intent_digest={commit_receipt_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = a_case(TrustBundleEnvironment::Devnet, sc);
        let decision = a_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "post_commit_audit_id={}\n",
            decision.post_commit_audit_id
        ));
    ...
    digest_body.push_str(&format!(
        "named_post_commit_audit_id={post_commit_audit_id}\n"
    ));
    ...
    fs::write(
        outdir.join("fixtures/run_326_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 325 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionPostCommitAuditArtifact` and its
    // `ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionPostCommitAuditExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionPostCommitAuditArtifact = artifact;
    let clean: ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_post_commit_audit_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome = recover_exec
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_326_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_post_commit_audit.rs :: ProductionLiveEpochTransitionPostCommitAuditExecutor (Run 325 source/test boundary)\n");
