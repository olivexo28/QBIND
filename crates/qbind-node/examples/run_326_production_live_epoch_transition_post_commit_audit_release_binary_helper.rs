...
fn r_eval_replay(
    ...
    case.executor
        .evaluate_live_epoch_transition_commit_receipt(&case.request, &case.inputs, &replay)
}
...
/// assert the resulting outcome.
fn r_reject_source(
    source: LiveEpochTransitionCommitReceiptAuthoritySource,
    expected: RO,
) {
    let mut c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = AK::from_staged_application_kind(LiveEpochTransitionCommitReceiptKind::UnsupportedStagedApplication);
    assert!(pk.is_unsupported());
}
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "accept_artifact_reexposes_consumed_commit_receipt_transcript", accept_artifact_reexposes_consumed_commit_receipt_transcript as fn()),
        ...
        ("rejection_fail_closed", "wrong_commit_receipt_decision_id", wrong_commit_receipt_decision_id as fn()),
        ("rejection_fail_closed", "wrong_commit_receipt_request_id", wrong_commit_receipt_request_id as fn()),
        ("rejection_fail_closed", "wrong_commit_receipt_intent_digest", wrong_commit_receipt_intent_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_receipt_transcript_digest", wrong_commit_receipt_transcript_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_receipt_nonce", wrong_commit_receipt_nonce as fn()),
        ("rejection_fail_closed", "tampered_commit_receipt_package_integrity_mismatch", tampered_commit_receipt_package_integrity_mismatch as fn()),
        ...
        ("rejection_fail_closed", "reject_missing_commit_receipt_decision", reject_missing_commit_receipt_decision as fn()),
        ("rejection_fail_closed", "reject_unverified_commit_receipt_decision", reject_unverified_commit_receipt_decision as fn()),
        ...
        ("rejection_fail_closed", "reject_accepted_commit_receipt_without_package_via_verified_source", reject_accepted_commit_receipt_without_package_via_verified_source as fn()),
        ("rejection_fail_closed", "reject_accepted_commit_receipt_without_package_variant", reject_accepted_commit_receipt_without_package_variant as fn()),
        ...
        ("rejection_fail_closed", "reject_fixture_only_commit_receipt", reject_fixture_only_commit_receipt as fn()),
    ...
    for sc in ALL_SC {
        ...
        digest_body.push_str(&format!("post_commit_audit_id={}\n", decision.post_commit_audit_id));
        digest_body.push_str(&format!("request_id={}\n", decision.request_id));
        digest_body.push_str(&format!("post_commit_audit_digest={}\n", decision.post_commit_audit_digest));
        digest_body.push_str(&format!(
            "content_digest={}\n",
            decision
                .post_commit_audit_artifact
    ...
    digest_body.push_str(&format!("named_post_commit_audit_id={post_commit_audit_id}\n"));
    ...
    let clean: ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_post_commit_audit_window(
            None,
            current_artifact,
        );
    let idempotent: ProductionLiveEpochTransitionPostCommitAuditRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_post_commit_audit_window(
    ...
    summary.push_str("run_326_production_live_epoch_transition_post_commit_audit_release_binary_helper\n");
