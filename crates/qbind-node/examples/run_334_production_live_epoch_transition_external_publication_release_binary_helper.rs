//! Run 334 — release-binary helper for the Run 333 **live epoch-transition
//! external-publication / settlement-preparation boundary**.
//!
//! Release-binary evidence for the Run 333 live epoch-transition
//! external-publication / settlement-preparation boundary
...
//! release-built production library symbols. It exercises the **real** Run 333
//! `ProductionLiveEpochTransitionExternalPublicationExecutor` and proves, per
...
//! chain to produce a verified, accepted Run 331/332 live epoch-transition
//! durable-audit-publication decision (`is_accept()` with
//! `Some(durable_audit_publication_artifact)`), then feeds that decision into
//! the Run 333 live epoch-transition external-publication /
//! settlement-preparation executor, which emits only a typed, deterministic,
//! non-mutating external-publication / settlement-preparation artifact.
//!
...
//! enablement, external-publication default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! external-publication / settlement-preparation artifacts; it never applies a
//! live validator-set change, never transitions a consensus epoch, never
...
//! durable replay record, settlement record, publication record, or external
//! publication record, never calls Run 070, never mutates `LivePqcTrustState`,
//! never mutates a live validator set, consensus state, or epoch counter, never
//! calls `BasicHotStuffEngine::transition_to_epoch`, never writes
...
//! `LiveEpochTransitionExternalPublicationFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_334.md`.
#![allow(dead_code)]
...
fn v_exec_with_policy(
    policy: ProductionLiveEpochTransitionExternalPublicationExecutorPolicy,
) -> ProductionLiveEpochTransitionExternalPublicationExecutor {
    ProductionLiveEpochTransitionExternalPublicationExecutor::new(
...
/// assert the resulting outcome (fail-closed, no artifact).
fn v_reject_inputs(
    mutate: impl FnOnce(&mut ProductionLiveEpochTransitionExternalPublicationInputs),
    expected: VO,
) {
...
// ===========================================================================
// A. Accepted / compatible source-test durable-audit-publication artifacts
// ===========================================================================
fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        ...
        assert_eq!(art.external_publication_nonce, CRC5_NONCE);
...
fn accept_all_scenarios_testnet() {
    for sc in ALL_SC {
        let c = v_case(TrustBundleEnvironment::Testnet, sc);
        let d = v_eval(&c);
        assert!(d.is_accept());
        let art = d.external_publication_artifact.as_ref().unwrap();
...
fn accept_outcome_carries_kind_env_target_nonce() {
    ...
    let d = v_eval(&c);
    match d.outcome {
        VO::AcceptedSourceTestLiveEpochTransitionExternalPublication {
            execution_kind,
            environment,
            epoch_transition_target,
...
fn accept_artifact_reexposes_consumed_durable_audit_publication_transcript() {
    ...
    let handoff_digest = dec.durable_audit_publication_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionExternalPublicationExecutor::source_test();
    let d = exec.evaluate_live_epoch_transition_external_publication(
        &request,
        &inputs,
        &empty_replay333(),
    );
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    assert!(!d.external_publication_digest.is_empty());
...
fn artifact_content_digest_is_stable() {
    let c = v_case(TrustBundleEnvironment::Devnet, Sc::Bulk);
    let d = v_eval(&c);
    let art = d.external_publication_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.external_publication_digest);
...
// ===========================================================================
fn wrong_durable_audit_publication_decision_id() {
    v_reject_inputs(
        |i| i.expected_durable_audit_publication_decision_id = "bad".to_string(),
        VO::DurableAuditPublicationDecisionIdMismatch,
    );
}
fn wrong_durable_audit_publication_request_id() {
    v_reject_inputs(
        |i| i.expected_durable_audit_publication_request_id = "bad".to_string(),
        VO::DurableAuditPublicationDecisionRequestIdMismatch,
    );
}
fn wrong_durable_audit_publication_intent_digest() {
    v_reject_inputs(
        |i| i.expected_durable_audit_publication_intent_digest = "bad".to_string(),
        VO::DurableAuditPublicationDecisionIntentDigestMismatch,
    );
}
fn wrong_durable_audit_publication_transcript_digest() {
    v_reject_inputs(
        |i| i.expected_durable_audit_publication_transcript_digest = "bad".to_string(),
        VO::DurableAuditPublicationDecisionTranscriptMismatch,
    );
}
fn wrong_durable_audit_publication_nonce() {
    v_reject_inputs(
        |i| i.expected_durable_audit_publication_nonce = 9999,
        VO::WrongDurableAuditPublicationNonce,
    );
}
fn tampered_durable_audit_publication_package_integrity_mismatch() {
...
fn wrong_rotation_nonce() {
    v_reject_inputs(|i| i.expected_rotation_nonce = 999, VO::WrongRotationNonce);
}
...
fn wrong_epoch_transition_target_request() {
    ...
    let d = v_eval(&c);
    assert_eq!(d.outcome, VO::WrongEpochTransitionTarget);
    assert!(d.external_publication_artifact.is_none());
}
...
// ===========================================================================
fn reject_missing_durable_audit_publication_decision() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::MissingDurableAuditPublicationDecision,
        VO::VerifiedDurableAuditPublicationDecisionRequired,
    );
}
fn reject_unverified_durable_audit_publication_decision() {
    let dec = cu_decision_rejected(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::UnverifiedDurableAuditPublicationDecision {
            decision: dec,
        },
        VO::UnverifiedDurableAuditPublicationDecisionRejected,
    );
}
...
fn reject_accepted_durable_audit_publication_without_package_via_verified_source() {
    let dec = cu_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::VerifiedDurableAuditPublicationDecision {
            decision: dec,
        },
...
fn reject_accepted_durable_audit_publication_without_package_variant() {
    let dec = cu_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    u_reject_source(
...
fn reject_guarded_mutation_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::GuardedMutationDecisionWithoutDurableAuditPublication,
        VO::GuardedMutationDecisionAloneRejected,
    );
}
fn reject_staged_application_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::StagedApplicationDecisionWithoutDurableAuditPublication,
        VO::StagedApplicationDecisionAloneRejected,
    );
}
fn reject_live_application_authorization_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::LiveApplicationAuthorizationWithoutDurableAuditPublication,
        VO::LiveApplicationAuthorizationAloneRejected,
    );
}
fn reject_application_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::ApplicationDecisionWithoutDurableAuditPublication,
        VO::ApplicationDecisionAloneRejected,
    );
}
fn reject_rotation_plan_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::RotationPlanWithoutDurableAuditPublication,
        VO::RotationPlanAloneRejected,
    );
}
fn reject_governance_execution_intent_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::GovernanceExecutionIntentWithoutDurableAuditPublication,
        VO::GovernanceExecutionIntentAloneRejected,
    );
}
fn reject_governance_proof_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::GovernanceProofWithoutDurableAuditPublication,
        VO::GovernanceProofAloneRejected,
    );
}
fn reject_local_operator_assertion() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::LocalOperatorAssertion,
        VO::LocalOperatorProofRejected,
    );
}
fn reject_peer_majority_assertion() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::PeerMajorityAssertion,
        VO::PeerMajorityProofRejected,
    );
}
fn reject_custody_only_evidence() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::CustodyOnlyEvidence,
        VO::CustodyOnlyProofRejected,
    );
}
fn reject_remote_signer_only_evidence() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::RemoteSignerOnlyEvidence,
        VO::RemoteSignerOnlyProofRejected,
    );
}
fn reject_custody_attestation_only_evidence() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::CustodyAttestationOnlyEvidence,
        VO::CustodyAttestationOnlyProofRejected,
    );
}
fn reject_fixture_only_durable_audit_publication() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::FixtureOnlyDurableAuditPublication,
        VO::FixtureStagedApplicationRejectedAsProductionAuthority,
    );
}
fn reject_arbitrary_validator_set_bytes() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::ArbitraryValidatorSetBytes,
        VO::ArbitraryValidatorSetBytesRejected,
    );
}
...
fn mainnet_domain_refused_under_source_test_policy() {
    ...
    let d = v_eval(&c);
    assert_eq!(d.outcome, VO::MainNetRefused);
    assert!(d.external_publication_artifact.is_none());
}
fn mainnet_policy_unavailable() {
    let c = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = v_exec_with_policy(
    ...
    assert_eq!(
        d.outcome,
        VO::MainNetProductionLiveEpochTransitionExternalPublicationUnavailable
    );
    assert!(d.external_publication_artifact.is_none());
}
fn production_policy_unavailable() {
    let c = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = v_exec_with_policy(
        ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::RequireProductionLiveEpochTransitionExternalPublication,
    );
    let d = exec.evaluate_live_epoch_transition_external_publication(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        VO::ProductionLiveEpochTransitionExternalPublicationUnavailable
    );
    assert!(d.external_publication_artifact.is_none());
}
fn disabled_policy_fails_closed() {
    ...
    assert_eq!(d.outcome, VO::Disabled);
    assert!(d.external_publication_artifact.is_none());
    assert!(!d.is_accept());
}
fn reserved_production_kind_fails_closed() {
    let c = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionExternalPublicationExecutor::new(
        ProductionLiveEpochTransitionExternalPublicationConfig::new(
    ...
    assert_eq!(
        d.outcome,
        VO::LiveEpochTransitionExternalPublicationBoundaryUnavailable
    );
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    assert!(!state.apply_prepared_execution(art, &d.external_publication_id));
    assert!(state.has_applied(&d.external_publication_id));
}
...
fn every_outcome_is_non_mutating() {
    ...
    let d = v_eval(&c);
    assert!(d.outcome.is_non_mutating());
    let bad = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let dbad = v_eval_replay(&bad, &[v_eval(&bad).external_publication_id]);
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionExternalPublicationExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionExternalPublication.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionExternalPublication.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionExternalPublicationRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}
fn kind_predicates() {
    use ProductionLiveEpochTransitionExternalPublicationExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionExternalPublication.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionExternalPublication.is_source_test());
    assert_eq!(K::default(), K::Disabled);
}
fn execution_kind_mapping_matches_handoff_kind() {
    for sc in ALL_SC {
        let hk = expected_crc4_kind(sc);
        let pk = VK::from_staged_application_kind(hk);
        assert_eq!(pk, expected_crc5_kind(sc));
...
fn outcome_tags_are_stable_and_distinct() {
    let a = VO::DurableAuditPublicationDecisionIdMismatch;
    let b = VO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), VO::DurableAuditPublicationDecisionIdMismatch.tag());
    assert!(!VO::MainNetRefused.tag().is_empty());
}
fn config_and_inputs_well_formed() {
    let c = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    assert!(c.inputs.is_well_formed());
    assert!(ProductionLiveEpochTransitionExternalPublicationConfig::source_test().is_well_formed());
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionExternalPublicationConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_durable_audit_publication_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            let mut c = v_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_durable_audit_publication_decision_id = "bad".to_string();
            let d = v_eval(&c);
            assert_eq!(d.outcome, VO::DurableAuditPublicationDecisionIdMismatch);
            assert!(d.external_publication_artifact.is_none());
        }
    };
}

per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_durable_audit_publication_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
macro_rules! per_scenario_guarded_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d = v_eval(&c);
            assert_eq!(d.outcome, VO::GuardedMutationDecisionIdMismatch);
            assert!(d.external_publication_artifact.is_none());
        }
    };
}
...
//    (additive layer introduced by Run 331 on top of the rotated Run 327
...
fn reject_runtime_handoff_intent_digest_mismatch() {
    v_reject_inputs(
        ...
        VO::RuntimeHandoffDecisionIntentDigestMismatch,
    );
}
fn reject_runtime_handoff_transcript_mismatch() {
    v_reject_inputs(
        ...
        VO::RuntimeHandoffDecisionTranscriptMismatch,
    );
}
fn reject_wrong_runtime_handoff_nonce() {
    v_reject_inputs(
        |i| i.expected_runtime_handoff_nonce = HANDOFF_NONCE + 100,
        VO::WrongRuntimeHandoffNonce,
    );
}
fn reject_runtime_handoff_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::RuntimeHandoffDecisionWithoutDurableAuditPublication,
        VO::RuntimeHandoffDecisionAloneRejected,
    );
}
fn runtime_handoff_alone_tag_is_stable() {
    assert_eq!(
        VO::RuntimeHandoffDecisionAloneRejected.tag(),
        "runtime-handoff-decision-alone-rejected"
    );
    assert_eq!(
        VO::RuntimeHandoffDecisionIdMismatch.tag(),
        "runtime-handoff-decision-id-mismatch"
    );
    assert_eq!(
        VO::WrongRuntimeHandoffNonce.tag(),
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = v_eval(&c);
    let d2 = v_eval(&c);
    let a1 = d1.external_publication_artifact.as_ref().unwrap();
    let a2 = d2.external_publication_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 333 — re-exposed Run 327/328 execution-preparation ancestor binding
// (carried through the consumed Run 331/332 durable-audit-publication artifact) and
...
fn reject_mutation_execution_intent_digest_mismatch() {
    v_reject_inputs(
        ...
        VO::MutationExecutionDecisionIntentDigestMismatch,
    );
}
fn reject_mutation_execution_transcript_mismatch() {
    v_reject_inputs(
        ...
        VO::MutationExecutionDecisionTranscriptMismatch,
    );
}
fn reject_wrong_mutation_execution_nonce() {
    v_reject_inputs(
        |i| i.expected_mutation_execution_nonce = MUT_NONCE + 100,
        VO::WrongMutationExecutionNonce,
    );
}
fn reject_mutation_execution_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::MutationExecutionDecisionWithoutDurableAuditPublication,
        VO::MutationExecutionDecisionAloneRejected,
    );
}
fn reject_commit_authorization_decision_id_mismatch() {
    v_reject_inputs(
        ...
        VO::CommitAuthorizationDecisionIdMismatch,
    );
}
fn reject_commit_authorization_request_id_mismatch() {
    v_reject_inputs(
        ...
        VO::CommitAuthorizationDecisionRequestIdMismatch,
    );
}
...
fn reject_commit_authorization_transcript_mismatch() {
    v_reject_inputs(
        ...
        VO::CommitAuthorizationDecisionTranscriptMismatch,
    );
}
fn reject_wrong_commit_authorization_nonce() {
    v_reject_inputs(
        |i| i.expected_commit_authorization_nonce = CMT_NONCE + 100,
        VO::WrongCommitAuthorizationNonce,
    );
}
fn reject_commit_authorization_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::CommitAuthorizationDecisionWithoutDurableAuditPublication,
        VO::CommitAuthorizationDecisionAloneRejected,
    );
}
...
fn mutation_execution_alone_tag_is_stable() {
    assert_eq!(
        VO::MutationExecutionDecisionAloneRejected.tag(),
        "mutation-execution-decision-alone-rejected"
    );
    assert_eq!(
        VO::MutationExecutionDecisionIdMismatch.tag(),
        "mutation-execution-decision-id-mismatch"
    );
    assert_eq!(
        VO::WrongMutationExecutionNonce.tag(),
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    // Parent Run 331/332 durable-audit-publication nonce re-exposed intact.
    assert_eq!(art.durable_audit_publication_nonce, CRC4_NONCE);
    // Self Run 333 external-publication proposed nonce.
    assert_eq!(art.external_publication_nonce, CRC5_NONCE);
}
...
fn reject_execution_preparation_decision_id_mismatch() {
    v_reject_inputs(
        ...
        VO::ExecutionPreparationDecisionIdMismatch,
    );
}
fn reject_execution_preparation_request_id_mismatch() {
    v_reject_inputs(
        ...
        VO::ExecutionPreparationDecisionRequestIdMismatch,
    );
}
fn reject_execution_preparation_intent_digest_mismatch() {
    v_reject_inputs(
        ...
        VO::ExecutionPreparationDecisionIntentDigestMismatch,
    );
}
fn reject_execution_preparation_transcript_mismatch() {
    v_reject_inputs(
        ...
        VO::ExecutionPreparationDecisionTranscriptMismatch,
    );
}
fn reject_wrong_execution_preparation_nonce() {
    v_reject_inputs(
        |i| i.expected_execution_preparation_nonce = PREP_NONCE + 100,
        VO::WrongExecutionPreparationNonce,
    );
}
fn reject_execution_preparation_decision_alone() {
    u_reject_source(
        LiveEpochTransitionExternalPublicationAuthoritySource::ExecutionPreparationDecisionWithoutDurableAuditPublication,
        VO::ExecutionPreparationDecisionAloneRejected,
    );
}
fn execution_preparation_alone_tag_is_stable() {
    assert_eq!(
        VO::ExecutionPreparationDecisionAloneRejected.tag(),
        "execution-preparation-decision-alone-rejected"
    );
    assert_eq!(
        VO::ExecutionPreparationDecisionIdMismatch.tag(),
        "execution-preparation-decision-id-mismatch"
    );
    assert_eq!(
        VO::WrongExecutionPreparationNonce.tag(),
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_334_production_live_epoch_transition_external_publication_release_binary/helper_evidence/run_334",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_durable_audit_publication_transcript",
            accept_artifact_reexposes_consumed_durable_audit_publication_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_durable_audit_publication_decision_id",
            wrong_durable_audit_publication_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_publication_request_id",
            wrong_durable_audit_publication_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_publication_intent_digest",
            wrong_durable_audit_publication_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_publication_transcript_digest",
            wrong_durable_audit_publication_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_durable_audit_publication_nonce",
            wrong_durable_audit_publication_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_durable_audit_publication_package_integrity_mismatch",
            tampered_durable_audit_publication_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_durable_audit_publication_decision",
            reject_missing_durable_audit_publication_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_durable_audit_publication_decision",
            reject_unverified_durable_audit_publication_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_durable_audit_publication_without_package_via_verified_source",
            reject_accepted_durable_audit_publication_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_durable_audit_publication_without_package_variant",
            reject_accepted_durable_audit_publication_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_durable_audit_publication",
            reject_fixture_only_durable_audit_publication as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 333 domain-separated digest free functions over the real accepted
    // external-publication artifact for every scenario.
    let case = v_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = v_eval(&case);
    let artifact = decision
        .external_publication_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live external-publication artifact");
    // On accept, the executor derives the identifiers from the consumed Run
    // 331/332 durable-audit-publication decision's durable-audit-publication
    // (content) digest.
    let parent_dec = cu_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let durable_audit_publication_intent_digest =
        parent_dec.durable_audit_publication_digest.clone();
    let content_digest =
        production_live_epoch_transition_external_publication_content_digest(artifact);
    let external_publication_id = production_live_epoch_transition_external_publication_id(
        ...
        &case.inputs.external_publication_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.external_publication_nonce,
    );
    ...
    let transcript_digest = production_live_epoch_transition_external_publication_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_PROTOCOL_VERSION,
        &request_id,
    ...
    digest_body.push_str(&format!(
        "durable_audit_publication_intent_digest={durable_audit_publication_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = v_case(TrustBundleEnvironment::Devnet, sc);
        let decision = v_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "external_publication_id={}\n",
            decision.external_publication_id
        ));
    ...
    digest_body.push_str(&format!(
        "named_external_publication_id={external_publication_id}\n"
    ));
    ...
    fs::write(
        outdir.join("fixtures/run_334_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 333 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionExternalPublicationArtifact` and its
    // `ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionExternalPublicationExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionExternalPublicationArtifact = artifact;
    let clean: ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_external_publication_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_external_publication_window(
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_334_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str(
        "run_334_production_live_epoch_transition_external_publication_release_binary_helper\n",
    );
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_external_publication.rs :: ProductionLiveEpochTransitionExternalPublicationExecutor (Run 333 source/test boundary)\n");
