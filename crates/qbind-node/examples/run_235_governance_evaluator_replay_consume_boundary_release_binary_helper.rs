...
fn run_accepted_table(out: &Path) -> (u64, u64) {
    ...
    // A2 — ProceedDeferred does not consume.
    {
        ...
        t.assert_true(
            "A2.deferred-never-consumed",
            deferred_is_never_consumed(),
            "",
        );
    }
    ...
    // A9 — DevNet fixture consume records consumed only after AppliedSuccessfully.
    {
        ...
        t.assert_true(
            "A9.not-consumed-before",
            !store.is_consumed(&input.replay_state_key_digest),
            "",
        );
        store.record_for(&freshness_input(
            Env::Devnet,
            S::ReloadApply,
            PreviouslySeenState::FirstSeen,
        ));
        let performed = perform_post_mutation_consume(
    ...
    // A12 — consume authorization digest is deterministic in release mode.
    {
        ...
        t.assert_true(
            "A12.stable",
            consume_authorization_digest(&input) == consume_authorization_digest(&input),
            "",
        );
        t.assert_true(
            "A12.twin-equal",
            consume_authorization_digest(&input) == consume_authorization_digest(&twin),
            "",
        );
    }
    ...
    // A13 — consume transcript digest is deterministic in release mode.
    {
        ...
        t.assert_true(
            "A13.stable",
            consume_transcript_digest(&input, &oc) == consume_transcript_digest(&input, &oc),
            "",
        );
        t.assert_true(
            "A13.outcome-bound",
    ...
    // A14 — post-mutation consume record digest is deterministic in release mode.
    {
        ...
        t.assert_true(
            "A14.epoch-bound",
            post_mutation_consume_record_digest(&input, CANONICAL)
                != post_mutation_consume_record_digest(&input, CANONICAL + 1),
            "",
        );
    }
    ...
    // field changes the consume authorization digest.
    {
        ...
        let mutators: Vec<(&str, Box<dyn Fn(&mut PostMutationConsumeInput)>)> = vec![
            (
                ...
                Box::new(|i: &mut PostMutationConsumeInput| {
                    i.replay_state_key_digest = "x".to_string()
                }),
            ),
            (
                "evaluator_request_digest",
                Box::new(|i| i.evaluator_request_digest = "x".to_string()),
            ),
            (
                "evaluator_response_digest",
                Box::new(|i| i.evaluator_response_digest = "x".to_string()),
            ),
            (
                "governance_execution_decision_digest",
                Box::new(|i| i.governance_execution_decision_digest = "x".to_string()),
            ),
            (
                "lifecycle_action",
            ...
            (
                ...
                Box::new(|i| i.authority_domain_sequence += 1),
            ),
            (
                "replay_nonce",
            ...
            (
                "validation_surface",
                Box::new(|i| i.validation_surface = S::Sighup),
            ),
            (
                "mutation_surface",
                Box::new(|i| i.mutation_surface = S::Sighup),
            ),
            (
                "mutation_completion_status",
                Box::new(|i| i.mutation_completion_status = MC::ApplyFailed),
            ),
        ];
    ...
    // A16 — production consume writer is callable and fails closed unavailable.
    {
        ...
        t.assert_true(
            "A16.writer-fails-closed",
            !writer.mark_consumed(&input.replay_state_key_digest),
            "",
        );
        let performed =
    ...
    // A17 — MainNet consume writer is callable and fails closed unavailable/refused.
    {
        ...
        t.check_outcome(
            "A17.evaluate",
            "fail-closed-mainnet-consume-unavailable",
            &o,
        );
        let mut writer = MainnetReplayStateReader;
        t.assert_true(
            "A17.writer-fails-closed",
            !writer.mark_consumed(&input.replay_state_key_digest),
            "",
        );
        let performed =
    ...
    // decision ProceedFresh.
    {
        let fresh = freshness_input(Env::Devnet, S::ReloadApply, PreviouslySeenState::FirstSeen);
        let exp230 = EvaluatorReplayFreshnessExpectations::from_evaluator_material(
            &ev_identity(Env::Devnet),
            &ev_request(&ev_identity(Env::Devnet)),
            &ev_response(&ev_request(&ev_identity(Env::Devnet))),
            TRANSCRIPT_DIGEST,
            DECISION_DIGEST,
            Env::Devnet,
            CHAIN,
    ...
    // consumes through the consume boundary.
    {
        ...
        let o = evaluate_post_mutation_consume(ReplayStatePolicy::FixtureDevNet, &input, &exp);
        t.check_outcome(
            "A21.live-inbound-validation-only",
            "do-not-consume-validation-only",
            &o,
        );
        t.assert_no_consume("A21", &o);
        t.assert_true(
            "A21.surface-validation-only",
            surface_is_validation_only(S::LiveInbound0x05),
            "",
        );
    }
...
fn assert_wrong_binding(
    t: &mut Table,
    id: &str,
    mutate: impl FnOnce(&mut PostMutationConsumeInput),
) {
...
fn run_rejection_table(out: &Path) -> (u64, u64) {
    ...
    assert_wrong_binding(&mut t, "R1.wrong-replay-state-key", |i| {
        i.replay_state_key_digest = "wrong-key".to_string()
    });
    assert_wrong_binding(&mut t, "R2.wrong-source-identity", |i| {
        i.evaluator_source_identity_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R3.wrong-request", |i| {
        i.evaluator_request_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R4.wrong-response", |i| {
        i.evaluator_response_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R5.wrong-transcript", |i| {
        i.evaluator_transcript_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R6.wrong-decision-digest", |i| {
        i.governance_execution_decision_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R7.wrong-proposal", |i| {
        i.proposal_id = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R8.wrong-decision-id", |i| {
        i.decision_id = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R9.wrong-lifecycle", |i| {
        i.lifecycle_action = LocalLifecycleAction::Revoke
    });
    assert_wrong_binding(&mut t, "R10.wrong-candidate", |i| {
        i.candidate_digest = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R11.wrong-sequence", |i| {
        i.authority_domain_sequence = SEQUENCE + 1
    });
    assert_wrong_binding(&mut t, "R12.wrong-effective-epoch", |i| {
        i.effective_epoch = EFFECTIVE + 1
    });
    assert_wrong_binding(&mut t, "R13.wrong-expiry-epoch", |i| {
        i.expiry_epoch = EXPIRY + 1
    });
    assert_wrong_binding(&mut t, "R14.wrong-replay-nonce", |i| {
        i.replay_nonce = "wrong".to_string()
    });
    assert_wrong_binding(&mut t, "R15.wrong-environment", |i| {
        i.environment = TrustBundleEnvironment::Testnet
    });
    assert_wrong_binding(&mut t, "R16.wrong-chain", |i| {
        i.chain_id = "wrong-chain".to_string()
    });
    assert_wrong_binding(&mut t, "R17.wrong-genesis", |i| {
    ...
    assert_wrong_binding(&mut t, "R18.wrong-validation-surface", |i| {
        i.validation_surface = S::Sighup
    });
    assert_wrong_binding(&mut t, "R19.wrong-mutation-surface", |i| {
    ...
    // marker write. Performed against a fixture store that records nothing.
    {
        ...
        let o = perform_post_mutation_consume(
            ReplayStatePolicy::FixtureDevNet,
            &input,
            &devnet_exp(),
            &mut store,
...
fn run_reachability_table(out: &Path) -> (u64, u64) {
    ...
    // Consume-after-success-only across every completion status.
    {
        for completion in [
            ...
            let o = evaluate_post_mutation_consume(
                ReplayStatePolicy::FixtureDevNet,
                &input,
                &devnet_exp(),
            );
    ...
    // Validation-only surfaces never consume.
    {
        for surface in [
            ...
            t.assert_true(
                &format!("VO.is-validation-only.{}", surface.tag()),
                surface_is_validation_only(surface),
                "",
            );
    ...
    // Disabled-policy consume fails closed unavailable.
    {
        ...
        t.check_outcome(
            "DP.disabled-fails-closed",
            "fail-closed-consume-unavailable",
            &o,
        );
    }
    ...
    // Outcome / status tags reachable / stable.
    {
        t.check(
            "T.consume",
            "consume-fixture-after-success",
            ConsumeBoundaryOutcome::ConsumeFixtureAfterSuccess.tag(),
        );
        t.check(
            "T.do-not-consume-deferred",
            "do-not-consume-deferred",
            ConsumeBoundaryOutcome::DoNotConsumeDeferred.tag(),
        );
        t.check(
            "T.fail-closed-production",
            "fail-closed-production-consume-unavailable",
            ConsumeBoundaryOutcome::FailClosedProductionConsumeUnavailable.tag(),
        );
        t.check(
            "T.fail-closed-mainnet",
            "fail-closed-mainnet-consume-unavailable",
            ConsumeBoundaryOutcome::FailClosedMainNetConsumeUnavailable.tag(),
        );
        t.check(
            "T.auth-authorized-fresh",
            "authorized-fresh",
            MA::AuthorizedFresh.tag(),
        );
        t.check(
            "T.completion-applied",
            "applied-successfully",
            MC::AppliedSuccessfully.tag(),
        );
        t.assert_true(
            "T.authorizes-mutation",
            MA::AuthorizedFresh.authorizes_mutation(),
            "",
        );
        t.assert_true(
            "T.applied-is-success",
            MC::AppliedSuccessfully.is_applied_successfully(),
            "",
        );
    }
    ...
    // Grep-verifiable refusal / fail-closed helper invariants.
    {
        ...
        t.assert_true(
            ...
            !mainnet_peer_driven_apply_remains_refused_under_consume_boundary(Env::Devnet),
            "",
        );
        t.assert_true(
            "G.consume-after-success-only",
            consume_only_after_successful_mutation(),
            "",
        );
        t.assert_true(
            "G.deferred-never-consumed",
            deferred_is_never_consumed(),
            "",
        );
        t.assert_true(
            "G.validation-only-never-consumed",
