...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let art = d.durable_audit_finalization_artifact.as_ref().unwrap();
    assert_eq!(
        d.durable_audit_finalization_id,
        art.durable_audit_finalization_id
    );
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(
        d.durable_audit_finalization_digest,
        art.durable_audit_finalization_digest
    );
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d2 = s_eval(&c);
        assert_eq!(
            d1.durable_audit_finalization_id,
            d2.durable_audit_finalization_id
        );
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(
            d1.durable_audit_finalization_digest,
            d2.durable_audit_finalization_digest
        );
...
fn tampered_post_commit_audit_package_integrity_mismatch() {
    ...
    let target = dec
        ...
        .epoch_transition_target;
    let inputs = s_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.post_commit_audit_artifact
        .as_mut()
        .unwrap()
...
fn wrong_proposed_validator_count() {
    s_reject_inputs(
        ...
        SO::WrongProposedValidatorCount,
    );
}
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = SK::from_staged_application_kind(
        LiveEpochTransitionPostCommitAuditKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}
...
fn config_and_inputs_well_formed() {
    ...
    assert!(
        ProductionLiveEpochTransitionDurableAuditFinalizationConfig::source_test().is_well_formed()
    );
    // The default config still pins the supported protocol version.
    assert!(
        ProductionLiveEpochTransitionDurableAuditFinalizationConfig::default().is_well_formed()
    );
}
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.durable_audit_finalization_artifact
                    .as_ref()
                    .unwrap()
                    .environment,
                TrustBundleEnvironment::Testnet
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_post_commit_audit_transcript",
            accept_artifact_reexposes_consumed_post_commit_audit_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_post_commit_audit_decision_id",
            wrong_post_commit_audit_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_post_commit_audit_request_id",
            wrong_post_commit_audit_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_post_commit_audit_intent_digest",
            wrong_post_commit_audit_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_post_commit_audit_transcript_digest",
            wrong_post_commit_audit_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_post_commit_audit_nonce",
            wrong_post_commit_audit_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_post_commit_audit_package_integrity_mismatch",
            tampered_post_commit_audit_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_post_commit_audit_decision",
            reject_missing_post_commit_audit_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_post_commit_audit_decision",
            reject_unverified_post_commit_audit_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_post_commit_audit_without_package_via_verified_source",
            reject_accepted_post_commit_audit_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_post_commit_audit_without_package_variant",
            reject_accepted_post_commit_audit_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_post_commit_audit",
            reject_fixture_only_post_commit_audit as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    digest_body.push_str(&format!(
        "named_durable_audit_finalization_id={durable_audit_finalization_id}\n"
    ));
    ...
    let clean: ProductionLiveEpochTransitionDurableAuditFinalizationRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_durable_audit_finalization_window(None, current_artifact);
