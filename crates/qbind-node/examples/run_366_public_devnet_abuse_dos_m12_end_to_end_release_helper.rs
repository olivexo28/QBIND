//! Run 366 — release-built helper that exercises **both** public DevNet
//! abuse/DoS runtime controls **in release mode** through the *deployed*
//! `qbind-node` builder path landed in Run 365:
//!
...
//! 2. the runtime **per-peer message-rate override** threaded through the
//!    **deployed** builder path in Run 365
//!    ([`qbind_node::cli::CliArgs::abuse_dos_runtime_config`] →
//!    [`qbind_node::p2p_node_builder::P2pNodeBuilder::with_abuse_dos_runtime_config`]
//!    → [`qbind_node::p2p_node_builder::P2pNodeBuilder::deployed_peer_rate_limiter_config`]
//!    → [`qbind_node::p2p_node_builder::P2pNodeBuilder::deployed_async_peer_manager_config`]
//!    → [`qbind_node::p2p_node_builder::P2pNodeBuilder::build_deployed_peer_manager`]
//!    → [`qbind_node::async_peer_manager::AsyncPeerManagerImpl`] →
//!    [`qbind_node::peer_rate_limiter::PeerRateLimiter`]).
//!
//! Unlike Run 364 (which reached the peer-manager limiter construction path via
//! [`qbind_node::async_peer_manager::AsyncPeerManagerConfig::with_peer_rate_limiter_config`]
//! directly), Run 366 drives the per-peer scenarios through the exact deployed
//! `P2pNodeBuilder` methods `main.rs` uses, so the evidence exercises the
//! Run 365 **deployed-builder threading** on real release artifacts.
//!
//! Per `task/RUN_366_TASK.txt`, this helper is fixture-tooling and:
//!
...
//! * exists alongside (and does NOT replace) the Run 361/362/363/365
//!   source/test targets and the Run 364 release helper.
//!
//! It links the real Run 361/362/363/365 runtime symbols and proves, in release
...
//! the **deployed builder** peer-manager limiter construction path and allows
//! under-budget / drops over-budget messages without touching the connection-
//! rate metric; both limiters are independent; invalid configs fail closed;
//! MainNet is refused; and the hidden/devnet-only CLI surface is hidden from
//! `--help` while the real hidden flags parse and invented flags are rejected by
//! clap.
//!
...
//! Usage:
//! ```text
//! run_366_public_devnet_abuse_dos_m12_end_to_end_release_helper <OUT_DIR>
//! ```
...
/// method `build_deployed_peer_manager` (NOT the direct
/// `AsyncPeerManagerConfig::with_peer_rate_limiter_config` seam Run 364 used).
...
fn main() {
    let out_dir = env::args()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("run_366_out"));
    ...
    // drop metric remains zero. The per-peer default is proven through the
    // deployed builder path (build_deployed_peer_manager).
    {
        ...
        // The DEPLOYED builder with no override must derive None and build the
        // peer-manager with PeerRateLimiter::with_defaults() exactly.
        ...
        scenarios.push(Scenario {
            ...
            detail: "No abuse/DoS flags: the connection limiter stays disabled \
                     ...
                     100 burst) via build_deployed_peer_manager()."
                .to_string(),
        });
    }

    // Scenario 2: connection_rate_end_to_end — configure a small connection-rate
    // bucket, prove first N allowed, N+1 refused, and the metric increments
    // exactly once; no peer admitted on refusal.
    {
        let metrics = Arc::new(P2pMetrics::new());
        ...
        let ok = admitted == 4
            ...
            && state.drop_count() == 2;
        scenarios.push(Scenario {
            id: "02_connection_rate_end_to_end",
            ...
            detail: "An enabled connection-rate limiter bounds inbound connections, \
                     refuses over-budget attempts without admitting a peer, and \
                     increments qbind_p2p_connection_rate_drop_total on each refusal."
                .to_string(),
        });
    }

    // Scenario 3: per_peer_message_rate_end_to_end — configure a small per-peer
    // message bucket via the DEPLOYED builder path, prove the builder installs
    // the custom config, the deployed peer manager honors the custom values,
    ...
    // NOT increment for the message-rate drop.
    {
        ...
        // Deployed builder must derive Some(custom) with the custom thresholds.
        ...
        let ok = derived_ok
            ...
            && metrics.connection_rate_drop_total() == 0;
        scenarios.push(Scenario {
            id: "03_per_peer_message_rate_end_to_end",
            ...
            detail: "The hidden --p2p-max-messages-per-second/--p2p-burst-allowance \
                     ...
                     messages drop without touching the separate connection-rate metric."
                .to_string(),
        });
    }

    // Scenario 4: combined_independence — configure both; prove a connection-rate
    // refusal does not admit a peer, a message-rate drop occurs only on the
    // admitted message path (via the deployed builder), and the counters remain
    // distinct.
    {
        let metrics = Arc::new(P2pMetrics::new());
        ...
        // Per-peer limiter derived through the DEPLOYED builder from the same
        // posture (capacity 3).
    ...
    // unbounded unsafe values / inconsistent per-address config are rejected
    // before any runtime state or deployed peer manager exists.
    {
    ...
    let summary = format!(
        "Run 366 — public DevNet abuse/DoS M12 end-to-end release-binary helper\n\
         scenarios_total: {}\n\
         scenarios_passed: {}\n\
         ...
         message-rate override reaches the DEPLOYED P2pNodeBuilder peer-manager \
         construction path (build_deployed_peer_manager, Run 365 threading); both \
         operator-configurable + default-off/defaults-preserved; no P2P socket \
         opened; no launch claim; MainNet refused; no trust/validator/epoch \
         mutation.\n",
        scenarios.len(),
    ...
    if !all_ok {
        eprintln!("[run_366_helper] FAIL: one or more scenarios did not match");
        std::process::exit(1);
    }
}