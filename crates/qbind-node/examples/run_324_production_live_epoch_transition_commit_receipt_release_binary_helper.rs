...
fn tampered_commit_execution_package_integrity_mismatch() {
    ...
    let target = dec.commit_execution_artifact.as_ref().unwrap().epoch_transition_target;
    let inputs = r_inputs(TrustBundleEnvironment::Devnet, &dec);
    dec.commit_execution_artifact.as_mut().unwrap().proposal_digest = "tampered".to_string();
    let request = ProductionLiveEpochTransitionCommitReceiptRequest::new(
        LiveEpochTransitionCommitReceiptAuthoritySource::VerifiedCommitExecutionDecision {
            decision: dec,
        },
...
fn replay_rejected_when_id_present() {
    let c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = r_eval(&c);
    let id = d.commit_receipt_artifact.as_ref().unwrap().request_id.clone();
    let replay = vec![id];
...
fn fixture_state_apply_is_idempotent() {
    ...
    let d = r_eval(&c);
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    let mut state = LiveEpochTransitionCommitReceiptFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn accept_authorizes_future_mutation_only() {
    ...
    assert!(d.authorizes_future_mutation_only());
    assert!(d.commit_receipt_artifact.as_ref().unwrap().staged_kind.is_non_mutating());
}
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = RK::from_staged_application_kind(LiveEpochTransitionCommitExecutionKind::UnsupportedStagedApplication);
    assert!(pk.is_unsupported());
}
...
macro_rules! per_scenario_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let mut state = LiveEpochTransitionCommitReceiptFixtureState::new(
                CUR_EPOCH,
                CUR_VERSION,
                "start",
            );
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        ("accepted_compatible", "accept_artifact_reexposes_consumed_commit_execution_transcript", accept_artifact_reexposes_consumed_commit_execution_transcript as fn()),
        ...
        ("rejection_fail_closed", "wrong_commit_execution_decision_id", wrong_commit_execution_decision_id as fn()),
        ("rejection_fail_closed", "wrong_commit_execution_request_id", wrong_commit_execution_request_id as fn()),
        ("rejection_fail_closed", "wrong_commit_execution_intent_digest", wrong_commit_execution_intent_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_execution_transcript_digest", wrong_commit_execution_transcript_digest as fn()),
        ("rejection_fail_closed", "wrong_commit_execution_nonce", wrong_commit_execution_nonce as fn()),
        ("rejection_fail_closed", "tampered_commit_execution_package_integrity_mismatch", tampered_commit_execution_package_integrity_mismatch as fn()),
        ...
        ("rejection_fail_closed", "reject_missing_commit_execution_decision", reject_missing_commit_execution_decision as fn()),
        ("rejection_fail_closed", "reject_unverified_commit_execution_decision", reject_unverified_commit_execution_decision as fn()),
        ...
        ("rejection_fail_closed", "reject_accepted_commit_execution_without_package_via_verified_source", reject_accepted_commit_execution_without_package_via_verified_source as fn()),
        ("rejection_fail_closed", "reject_accepted_commit_execution_without_package_variant", reject_accepted_commit_execution_without_package_variant as fn()),
        ...
        ("rejection_fail_closed", "reject_fixture_only_commit_execution", reject_fixture_only_commit_execution as fn()),
    ...
    let content_digest =
        production_live_epoch_transition_commit_receipt_content_digest(artifact);
    ...
    for sc in ALL_SC {
        ...
        digest_body.push_str(&format!("commit_receipt_id={}\n", decision.commit_receipt_id));
        digest_body.push_str(&format!("request_id={}\n", decision.request_id));
        digest_body.push_str(&format!("commit_receipt_digest={}\n", decision.commit_receipt_digest));
        digest_body.push_str(&format!(
            "content_digest={}\n",
            decision
                .commit_receipt_artifact
    ...
    let clean: ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_receipt_window(
            None,
            current_artifact,
        );
    let idempotent: ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_receipt_window(
    ...
    summary.push_str("run_324_production_live_epoch_transition_commit_receipt_release_binary_helper\n");
