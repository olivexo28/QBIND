//! Run 324 — release-binary helper for the Run 323 **live epoch-transition
//! commit receipt boundary**.
//!
//! Release-binary evidence for the Run 323 live epoch-transition commit
//! receipt boundary
...
//! release-built production library symbols. It exercises the **real** Run 323
//! `ProductionLiveEpochTransitionCommitReceiptExecutor` and proves, per
...
//! -> 311 -> 313 -> 315 -> 317 -> 319 -> 321 chain to produce a verified,
//! accepted Run 321/322 live epoch-transition commit-execution decision
//! (`is_accept()` with `Some(commit_execution_artifact)`), then feeds that
//! decision into the Run 323 live epoch-transition commit-receipt executor,
//! which emits only a typed, deterministic, non-mutating live commit-receipt
//! artifact.
//!
...
//! enablement, commit-receipt default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! commit-receipt artifacts; it never applies a live
...
//! `LiveEpochTransitionCommitReceiptFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_324.md`.
#![allow(dead_code)]
...
use LiveEpochTransitionCommitReceiptKind as RK;
use ProductionLiveEpochTransitionCommitReceiptOutcome as RO;

const CRC_POLICY_ID: &str = "commit-receipt-policy-1";
...
/// Build an accepted Run 321/322 commit-execution decision — the sole
...
fn cx_decision_no_artifact(
    ...
    d.commit_execution_artifact = None;
    d
}
...
fn r_inputs(
    ...
    ProductionLiveEpochTransitionCommitReceiptInputs {
        trust_domain: trust_domain(env),
        commit_receipt_policy_id: CRC_POLICY_ID.to_string(),
        expected_authorization_policy_id: AUTH_POLICY_ID.to_string(),
        expected_application_policy_id: APP_POLICY_ID.to_string(),
        expected_governance_domain_id: GOV_DOMAIN.to_string(),
        expected_governance_epoch: GOV_EPOCH,
...
fn r_case(env: TrustBundleEnvironment, sc: Sc) -> R323 {
...
fn r_eval(case: &R323) -> ProductionLiveEpochTransitionCommitReceiptDecision {
...
) -> ProductionLiveEpochTransitionCommitReceiptExecutor {
    ProductionLiveEpochTransitionCommitReceiptExecutor::new(
...
/// assert the resulting outcome (fail-closed, no artifact).
fn r_reject_inputs(
    mutate: impl FnOnce(&mut ProductionLiveEpochTransitionCommitReceiptInputs),
    expected: RO,
) {
    ...
    let d = r_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
    assert!(d.commit_receipt_artifact.is_none());
    assert!(!d.is_accept());
    assert!(d.outcome.is_non_mutating());
}
...
/// Common helper: build a Devnet/Add case, replace its authority source, and
/// assert the resulting outcome.
fn r_reject_source(source: LiveEpochTransitionCommitReceiptAuthoritySource, expected: RO) {
    let mut c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    c.request.authority_source = source;
    let d = r_eval(&c);
    assert_eq!(d.outcome, expected, "outcome tag: {}", d.outcome.tag());
...
// ===========================================================================
// A. Accepted / compatible source-test commit-execution artifacts
// ===========================================================================
...
fn accept_all_scenarios_devnet() {
    for sc in ALL_SC {
        ...
        assert_eq!(art.commit_receipt_nonce, CRC_NONCE);
...
fn accept_outcome_carries_kind_env_target_nonce() {
    ...
    let d = r_eval(&c);
    match d.outcome {
        RO::AcceptedSourceTestLiveEpochTransitionCommitReceipt {
            execution_kind,
            environment,
            epoch_transition_target,
            ...
            assert_eq!(commit_receipt_nonce, CRC_NONCE);
        }
        other => panic!("unexpected outcome: {other:?}"),
    }
}

fn accept_artifact_reexposes_consumed_commit_execution_transcript() {
    ...
    let handoff_id = dec.commit_execution_id.clone();
    let handoff_req = dec.request_id.clone();
    let handoff_digest = dec.commit_execution_digest.clone();
    ...
    let exec = ProductionLiveEpochTransitionCommitReceiptExecutor::source_test();
    let d =
        exec.evaluate_live_epoch_transition_commit_receipt(&request, &inputs, &empty_replay323());
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    assert_eq!(art.commit_execution_decision_id, handoff_id);
    assert_eq!(art.commit_execution_request_id, handoff_req);
    assert_eq!(art.commit_execution_intent_digest, handoff_digest);
    assert_eq!(art.commit_execution_transcript_digest, handoff_transcript);
}
...
fn accept_decision_ids_match_artifact_ids() {
    ...
    let d = r_eval(&c);
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    assert_eq!(d.commit_receipt_id, art.commit_receipt_id);
    assert_eq!(d.request_id, art.request_id);
    assert_eq!(d.commit_receipt_digest, art.commit_receipt_digest);
    assert_eq!(d.transcript_digest, art.transcript_digest);
    assert!(!d.commit_receipt_id.is_empty());
    assert!(!d.request_id.is_empty());
    assert!(!d.commit_receipt_digest.is_empty());
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        let d1 = r_eval(&c);
        let d2 = r_eval(&c);
        assert_eq!(d1.commit_receipt_id, d2.commit_receipt_id);
        assert_eq!(d1.request_id, d2.request_id);
        assert_eq!(d1.commit_receipt_digest, d2.commit_receipt_digest);
...
fn artifact_content_digest_is_stable() {
    ...
    let d = r_eval(&c);
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    assert_eq!(art.content_digest(), art.commit_receipt_digest);
...
fn wrong_commit_execution_decision_id() {
    r_reject_inputs(
        |i| i.expected_commit_execution_decision_id = "bad".to_string(),
        RO::CommitExecutionDecisionIdMismatch,
    );
}

fn wrong_commit_execution_request_id() {
    r_reject_inputs(
        |i| i.expected_commit_execution_request_id = "bad".to_string(),
        RO::CommitExecutionDecisionRequestIdMismatch,
    );
}

fn wrong_commit_execution_intent_digest() {
    r_reject_inputs(
        |i| i.expected_commit_execution_intent_digest = "bad".to_string(),
        RO::CommitExecutionDecisionIntentDigestMismatch,
    );
}

fn wrong_commit_execution_transcript_digest() {
    r_reject_inputs(
        |i| i.expected_commit_execution_transcript_digest = "bad".to_string(),
        RO::CommitExecutionDecisionTranscriptMismatch,
    );
}

fn wrong_commit_execution_nonce() {
    r_reject_inputs(
        |i| i.expected_commit_execution_nonce = 9999,
        RO::WrongCommitExecutionNonce,
    );
}

fn tampered_commit_execution_package_integrity_mismatch() {
    ...
    let d =
        exec.evaluate_live_epoch_transition_commit_receipt(&request, &inputs, &empty_replay323());
    assert_eq!(d.outcome, RO::CommitExecutionDecisionIntegrityMismatch);
    assert!(d.commit_receipt_artifact.is_none());
}
...
fn wrong_proposed_validator_count() {
    r_reject_inputs(
        ...
        RO::WrongProposedValidatorCount,
    );
}
...
fn reject_missing_commit_execution_decision() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::MissingCommitExecutionDecision,
        RO::VerifiedCommitExecutionDecisionRequired,
    );
}

fn reject_unverified_commit_execution_decision() {
    ...
    r_reject_source(
        ...
        RO::UnverifiedCommitExecutionDecisionRejected,
    );
}
...
fn reject_accepted_commit_execution_without_package_via_verified_source() {
...
fn reject_accepted_commit_execution_without_package_variant() {
    let dec = cx_decision_no_artifact(TrustBundleEnvironment::Devnet, Sc::Add);
    r_reject_source(
...
fn reject_guarded_mutation_decision_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::GuardedMutationDecisionWithoutCommitExecution,
...
fn reject_staged_application_decision_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::StagedApplicationDecisionWithoutCommitExecution,
...
fn reject_live_application_authorization_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::LiveApplicationAuthorizationWithoutCommitExecution,
...
fn reject_application_decision_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::ApplicationDecisionWithoutCommitExecution,
...
fn reject_rotation_plan_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::RotationPlanWithoutCommitExecution,
...
fn reject_governance_execution_intent_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::GovernanceExecutionIntentWithoutCommitExecution,
...
fn reject_governance_proof_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::GovernanceProofWithoutCommitExecution,
...
fn reject_local_operator_assertion() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::LocalOperatorAssertion,
...
fn reject_peer_majority_assertion() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::PeerMajorityAssertion,
...
fn reject_custody_only_evidence() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::CustodyOnlyEvidence,
...
fn reject_remote_signer_only_evidence() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::RemoteSignerOnlyEvidence,
...
fn reject_custody_attestation_only_evidence() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::CustodyAttestationOnlyEvidence,
        RO::CustodyAttestationOnlyProofRejected,
    );
}

fn reject_fixture_only_commit_execution() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::FixtureOnlyCommitExecution,
...
fn reject_arbitrary_validator_set_bytes() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::ArbitraryValidatorSetBytes,
...
fn mainnet_policy_unavailable() {
    ...
    let d = exec.evaluate_live_epoch_transition_commit_receipt(
        &c.request,
        &c.inputs,
        &empty_replay323(),
    );
    assert_eq!(
        d.outcome,
        RO::MainNetProductionLiveEpochTransitionCommitReceiptUnavailable
    );
    assert!(d.commit_receipt_artifact.is_none());
}
...
fn production_policy_unavailable() {
    let c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = r_exec_with_policy(
        ProductionLiveEpochTransitionCommitReceiptExecutorPolicy::RequireProductionLiveEpochTransitionCommitReceipt,
    );
    let d = exec.evaluate_live_epoch_transition_commit_receipt(
        &c.request,
        &c.inputs,
    ...
    assert_eq!(
        d.outcome,
        RO::ProductionLiveEpochTransitionCommitReceiptUnavailable
    );
    assert!(d.commit_receipt_artifact.is_none());
}
...
fn disabled_policy_fails_closed() {
    let c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec =
        r_exec_with_policy(ProductionLiveEpochTransitionCommitReceiptExecutorPolicy::Disabled);
    let d = exec.evaluate_live_epoch_transition_commit_receipt(
        &c.request,
        &c.inputs,
        &empty_replay323(),
...
fn reserved_production_kind_fails_closed() {
    let c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec = ProductionLiveEpochTransitionCommitReceiptExecutor::new(
        ProductionLiveEpochTransitionCommitReceiptConfig::new(
    ...
    assert_eq!(
        d.outcome,
        RO::LiveEpochTransitionCommitReceiptBoundaryUnavailable
    );
}
...
fn replay_rejected_when_id_present() {
    ...
    assert!(d2.commit_receipt_artifact.is_none());
}
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionCommitReceiptFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
    assert!(state.apply_prepared_execution(art, &d.commit_receipt_id));
    ...
    assert!(!state.apply_prepared_execution(art, &d.commit_receipt_id));
    assert!(state.has_applied(&d.commit_receipt_id));
}
...
fn every_outcome_is_non_mutating() {
    ...
    let dbad = r_eval_replay(&bad, &[r_eval(&bad).commit_receipt_id]);
...
fn policy_predicates() {
    use ProductionLiveEpochTransitionCommitReceiptExecutorPolicy as Pol;
    assert!(Pol::Disabled.is_disabled());
    assert!(Pol::AllowSourceTestLiveEpochTransitionCommitReceipt.allows_source_test());
    assert!(Pol::RequireProductionLiveEpochTransitionCommitReceipt.is_production());
    assert!(Pol::MainnetProductionLiveEpochTransitionCommitReceiptRequired.is_mainnet());
    assert_eq!(Pol::default(), Pol::Disabled);
}

fn kind_predicates() {
    use ProductionLiveEpochTransitionCommitReceiptExecutorKind as K;
    assert!(K::SourceTestLiveEpochTransitionCommitReceipt.is_source_test());
    assert!(!K::Disabled.is_source_test());
    assert!(!K::ProductionLiveEpochTransitionCommitReceipt.is_source_test());
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = RK::from_staged_application_kind(
        LiveEpochTransitionCommitExecutionKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}

fn outcome_tags_are_stable_and_distinct() {
    let a = RO::CommitExecutionDecisionIdMismatch;
    let b = RO::GuardedMutationDecisionIdMismatch;
    assert_ne!(a.tag(), b.tag());
    assert_eq!(a.tag(), RO::CommitExecutionDecisionIdMismatch.tag());
...
fn config_and_inputs_well_formed() {
    ...
    assert!(ProductionLiveEpochTransitionCommitReceiptConfig::source_test().is_well_formed());
    // The default config still pins the supported protocol version.
    assert!(ProductionLiveEpochTransitionCommitReceiptConfig::default().is_well_formed());
}
...
macro_rules! per_scenario_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            let d2 = r_eval(&c);
            assert_eq!(d.commit_receipt_digest, d2.commit_receipt_digest);
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.commit_receipt_artifact.as_ref().unwrap().environment,
                TrustBundleEnvironment::Testnet
            );
        }
    };
...
macro_rules! per_scenario_commit_execution_binding {
    ($name:ident, $sc:expr) => {
        fn $name() {
            let mut c = r_case(TrustBundleEnvironment::Devnet, $sc);
            c.inputs.expected_commit_execution_decision_id = "bad".to_string();
            let d = r_eval(&c);
            assert_eq!(d.outcome, RO::CommitExecutionDecisionIdMismatch);
            assert!(d.commit_receipt_artifact.is_none());
        }
    };
}

per_scenario_commit_execution_binding!(scenario_handoff_binding_add, Sc::Add);
per_scenario_commit_execution_binding!(scenario_handoff_binding_remove, Sc::Remove);
per_scenario_commit_execution_binding!(scenario_handoff_binding_update, Sc::Update);
per_scenario_commit_execution_binding!(scenario_handoff_binding_noop, Sc::NoOp);
per_scenario_commit_execution_binding!(scenario_handoff_binding_identity, Sc::Identity);
per_scenario_commit_execution_binding!(scenario_handoff_binding_retire, Sc::Retire);
per_scenario_commit_execution_binding!(scenario_handoff_binding_emergency, Sc::Emergency);
per_scenario_commit_execution_binding!(scenario_handoff_binding_authsync, Sc::AuthSync);
per_scenario_commit_execution_binding!(scenario_handoff_binding_bulk, Sc::Bulk);
...
// ===========================================================================
// Q. Deeper re-exposed Run 317/318 runtime-handoff authority-tuple binding
//    (additive layer introduced by Run 321 on top of the rotated Run 319
...
fn accept_reexposes_runtime_handoff_tuple_from_parent() {
    ...
    let ppkg = parent.commit_execution_artifact.as_ref().unwrap().clone();
    let c = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let d = r_eval(&c);
    assert!(d.is_accept());
    let art = d.commit_receipt_artifact.as_ref().unwrap();
...
fn reject_runtime_handoff_decision_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::RuntimeHandoffDecisionWithoutCommitExecution,
...
fn accept_content_digest_binds_runtime_handoff_tuple() {
    ...
    let d1 = r_eval(&c);
    let d2 = r_eval(&c);
    let a1 = d1.commit_receipt_artifact.as_ref().unwrap();
    let a2 = d2.commit_receipt_artifact.as_ref().unwrap();
    assert_eq!(a1.content_digest(), a2.content_digest());
}

// ===========================================================================
// Run 323 — re-exposed Run 319/320 execution-preparation ancestor binding
// (carried through the consumed Run 321/322 commit-execution artifact) and
...
fn accepted_artifact_re_exposes_mutation_execution_ancestor() {
    ...
    assert!(d.is_accept());
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    // Grandparent Run 319/320 mutation-execution nonce re-exposed intact.
    assert_eq!(art.mutation_execution_nonce, MUT_NONCE);
    // Parent Run 321/322 commit-execution nonce re-exposed intact.
    assert_eq!(art.commit_execution_nonce, CXE_NONCE);
    // Self Run 323 commit-receipt proposed nonce.
    assert_eq!(art.commit_receipt_nonce, CRC_NONCE);
}
...
fn reject_execution_preparation_decision_alone() {
    r_reject_source(
        LiveEpochTransitionCommitReceiptAuthoritySource::ExecutionPreparationDecisionWithoutCommitExecution,
...
fn accepted_artifact_re_exposes_execution_preparation_ancestor() {
    ...
    assert!(d.is_accept());
    let art = d.commit_receipt_artifact.as_ref().unwrap();
    // Grandparent Run 319/320 execution-preparation nonce re-exposed intact.
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_324_production_live_epoch_transition_commit_receipt_release_binary/helper_evidence/run_324",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_commit_execution_transcript",
            accept_artifact_reexposes_consumed_commit_execution_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_commit_execution_decision_id",
            wrong_commit_execution_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_execution_request_id",
            wrong_commit_execution_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_execution_intent_digest",
            wrong_commit_execution_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_execution_transcript_digest",
            wrong_commit_execution_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_commit_execution_nonce",
            wrong_commit_execution_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_commit_execution_package_integrity_mismatch",
            tampered_commit_execution_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_commit_execution_decision",
            reject_missing_commit_execution_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_commit_execution_decision",
            reject_unverified_commit_execution_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_commit_execution_without_package_via_verified_source",
            reject_accepted_commit_execution_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_commit_execution_without_package_variant",
            reject_accepted_commit_execution_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_commit_execution",
            reject_fixture_only_commit_execution as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 323 domain-separated digest free functions.
    let case = r_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = r_eval(&case);
    let artifact = decision
        .commit_receipt_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live commit-receipt artifact");
    // On accept, the executor derives the identifiers from the consumed Run
    // 319/320 commit-execution decision's commit-execution (content)
    // digest.
    let cx_dec = cx_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let commit_execution_intent_digest = cx_dec.commit_execution_digest.clone();
    let content_digest = production_live_epoch_transition_commit_receipt_content_digest(artifact);
    let commit_receipt_id = production_live_epoch_transition_commit_receipt_id(
        ...
        &case.inputs.commit_receipt_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.commit_receipt_nonce,
    );
    ...
    let transcript_digest = production_live_epoch_transition_commit_receipt_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_RECEIPT_PROTOCOL_VERSION,
        &request_id,
        &content_digest,
        outcome_tag,
    ...
    digest_body.push_str(&format!(
        "commit_execution_intent_digest={commit_execution_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = r_case(TrustBundleEnvironment::Devnet, sc);
        let decision = r_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!(
            "commit_receipt_id={}\n",
            decision.commit_receipt_id
        ));
    ...
    digest_body.push_str(&format!("named_commit_receipt_id={commit_receipt_id}\n"));
    ...
    fs::write(
        outdir.join("fixtures/run_324_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 323 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionCommitReceiptArtifact` and its
    // `ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionCommitReceiptExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionCommitReceiptArtifact = artifact;
    let clean: ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome =
        recover_exec.recover_live_epoch_transition_commit_receipt_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_commit_receipt_window(
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_324_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_commit_receipt.rs :: ProductionLiveEpochTransitionCommitReceiptExecutor (Run 323 source/test boundary)\n");
