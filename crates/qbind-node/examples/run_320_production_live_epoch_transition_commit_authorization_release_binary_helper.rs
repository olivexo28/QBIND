...
fn accept_decision_ids_match_artifact_ids() {
    ...
    assert_eq!(
        d.commit_authorization_digest,
        art.commit_authorization_digest
    );
...
fn deterministic_digests_under_reevaluation() {
    for sc in ALL_SC {
        ...
        assert_eq!(
            d1.commit_authorization_digest,
            d2.commit_authorization_digest
        );
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.commit_authorization_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionCommitAuthorizationFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = CK::from_staged_application_kind(
        LiveEpochTransitionMutationExecutionKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}
...
macro_rules! per_scenario_testnet_accept {
    ($name:ident, $sc:expr) => {
        fn $name() {
            ...
            assert!(d.is_accept());
            assert_eq!(
                d.commit_authorization_artifact
                    .as_ref()
                    .unwrap()
                    .environment,
                TrustBundleEnvironment::Testnet
...
fn main() {
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_mutation_execution_transcript",
            accept_artifact_reexposes_consumed_mutation_execution_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_mutation_execution_decision_id",
            wrong_mutation_execution_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_mutation_execution_request_id",
            wrong_mutation_execution_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_mutation_execution_intent_digest",
            wrong_mutation_execution_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_mutation_execution_transcript_digest",
            wrong_mutation_execution_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_mutation_execution_nonce",
            wrong_mutation_execution_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_mutation_execution_package_integrity_mismatch",
            tampered_mutation_execution_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_mutation_execution_decision",
            reject_missing_mutation_execution_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_mutation_execution_decision",
            reject_unverified_mutation_execution_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_mutation_execution_without_package_via_verified_source",
            reject_accepted_mutation_execution_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_mutation_execution_without_package_variant",
            reject_accepted_mutation_execution_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_mutation_execution",
            reject_fixture_only_mutation_execution as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    let transcript_digest = production_live_epoch_transition_commit_authorization_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_PROTOCOL_VERSION,
        &request_id,
        &content_digest,
        outcome_tag,
    ...
    digest_body.push_str(&format!(
        "named_commit_authorization_id={commit_authorization_id}\n"
    ));
    ...
    let clean: ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_commit_authorization_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome = recover_exec
