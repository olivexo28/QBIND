//! Run 314 — release-binary helper for the Run 313 **epoch-transition runtime
...
//! release-built production library symbols. It exercises the **real** Run 313
//! `ProductionEpochTransitionRuntimeHandoffExecutor` and proves, per check with
...
//! -> 311 chain to produce a verified, accepted Run 311/312 guarded
//! epoch-transition mutation-execution decision (`is_accept()` with
//! `Some(staged_application_record)`), then feeds that decision into the Run 313
//! epoch-transition runtime handoff executor, which emits only a typed,
//! deterministic, non-mutating runtime handoff / live-mutation preflight
//! package for a future live executor.
//!
...
//! enablement, runtime-handoff default wiring, or live validator-set mutation.
//! The executor only ever produces typed non-mutating runtime handoff packages;
//! it never applies a live validator-set change, never transitions a consensus
//! epoch, never calls Run 070, never mutates `LivePqcTrustState`, never mutates
...
//! `EpochTransitionRuntimeHandoffFixtureState` used exclusively as source/test
//! evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_314.md`.
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_314_production_epoch_transition_runtime_handoff_release_binary/helper_evidence/run_314",
        )
    });
    ...
    // Run 313 domain-separated digest free functions.
    let case = h_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = h_eval(&case);
    ...
    let handoff_id = production_epoch_transition_runtime_handoff_id(
        ...
        &case.inputs.handoff_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.runtime_handoff_nonce,
    );
    ...
    summary.push_str("run_314_production_epoch_transition_runtime_handoff_release_binary_helper\n");
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_epoch_transition_runtime_handoff.rs :: ProductionEpochTransitionRuntimeHandoffExecutor (Run 313 source/test boundary)\n");
