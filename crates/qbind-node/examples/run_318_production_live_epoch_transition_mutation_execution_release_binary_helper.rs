//! Run 318 — release-binary helper for the Run 317 **live epoch-transition
//! mutation execution boundary**.
//!
//! Release-binary evidence for the Run 317 live epoch-transition mutation
//! execution boundary
...
//! release-built production library symbols. It exercises the **real** Run 317
//! `ProductionLiveEpochTransitionMutationExecutionExecutor` and proves, per
...
//! -> 311 -> 313 -> 315 chain to produce a verified, accepted Run 315/316
//! epoch-transition execution-preparation decision (`is_accept()` with
//! `Some(preparation_artifact)`), then feeds that decision into the Run 317
//! live epoch-transition mutation-execution executor, which emits only a typed,
//! deterministic, non-mutating live-mutation execution artifact.
//!
...
//! enablement, mutation-execution default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live-mutation
//! execution artifacts; it never applies a live validator-set change, never
...
//! `LiveEpochTransitionMutationExecutionFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_318.md`.
#![allow(dead_code)]
...
fn accept_artifact_reexposes_consumed_execution_preparation_transcript() {
    ...
    assert_eq!(
        art.execution_preparation_transcript_digest,
        handoff_transcript
    );
}
...
fn wrong_proposed_validator_count() {
    m_reject_inputs(
        ...
        MO::WrongProposedValidatorCount,
    );
}
...
fn disabled_policy_fails_closed() {
    let c = m_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let exec =
        m_exec_with_policy(ProductionLiveEpochTransitionMutationExecutionExecutorPolicy::Disabled);
    let d = exec.evaluate_live_epoch_transition_mutation_execution(
        &c.request,
        &c.inputs,
        &empty_replay317(),
...
fn fixture_state_apply_is_idempotent() {
    ...
    let art = d.execution_artifact.as_ref().unwrap();
    let mut state =
        LiveEpochTransitionMutationExecutionFixtureState::new(CUR_EPOCH, CUR_VERSION, "start");
...
fn unsupported_staged_application_kind_is_unsupported() {
    let pk = MK::from_staged_application_kind(
        LiveEpochTransitionExecutionPreparationKind::UnsupportedStagedApplication,
    );
    assert!(pk.is_unsupported());
}
...
per_scenario_execution_preparation_binding!(scenario_execution_preparation_binding_add, Sc::Add);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_remove,
    Sc::Remove
);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_update,
    Sc::Update
);
per_scenario_execution_preparation_binding!(scenario_execution_preparation_binding_noop, Sc::NoOp);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_identity,
    Sc::Identity
);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_retire,
    Sc::Retire
);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_emergency,
    Sc::Emergency
);
per_scenario_execution_preparation_binding!(
    scenario_execution_preparation_binding_authsync,
    Sc::AuthSync
);
per_scenario_execution_preparation_binding!(scenario_execution_preparation_binding_bulk, Sc::Bulk);
...
fn main() {
    let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            "docs/devnet/run_318_production_live_epoch_transition_mutation_execution_release_binary/helper_evidence/run_318",
        )
    });
    ...
    let cases: &[(&str, &str, fn())] = &[
        ...
        (
            "accepted_compatible",
            "accept_artifact_reexposes_consumed_execution_preparation_transcript",
            accept_artifact_reexposes_consumed_execution_preparation_transcript as fn(),
        ),
        (
            "accepted_compatible",
        ...
        (
            ...
            "wrong_execution_preparation_decision_id",
            wrong_execution_preparation_decision_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_execution_preparation_request_id",
            wrong_execution_preparation_request_id as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_execution_preparation_intent_digest",
            wrong_execution_preparation_intent_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_execution_preparation_transcript_digest",
            wrong_execution_preparation_transcript_digest as fn(),
        ),
        (
            "rejection_fail_closed",
            "wrong_execution_preparation_nonce",
            wrong_execution_preparation_nonce as fn(),
        ),
        (
            "rejection_fail_closed",
            "tampered_execution_preparation_package_integrity_mismatch",
            tampered_execution_preparation_package_integrity_mismatch as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_missing_execution_preparation_decision",
            reject_missing_execution_preparation_decision as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_unverified_execution_preparation_decision",
            reject_unverified_execution_preparation_decision as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_accepted_execution_preparation_without_package_via_verified_source",
            reject_accepted_execution_preparation_without_package_via_verified_source as fn(),
        ),
        (
            "rejection_fail_closed",
            "reject_accepted_execution_preparation_without_package_variant",
            reject_accepted_execution_preparation_without_package_variant as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "reject_fixture_only_execution_preparation",
            reject_fixture_only_execution_preparation as fn(),
        ),
        (
            "rejection_fail_closed",
        ...
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_add",
            scenario_execution_preparation_binding_add as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_remove",
            scenario_execution_preparation_binding_remove as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_update",
            scenario_execution_preparation_binding_update as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_noop",
            scenario_execution_preparation_binding_noop as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_identity",
            scenario_execution_preparation_binding_identity as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_retire",
            scenario_execution_preparation_binding_retire as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_emergency",
            scenario_execution_preparation_binding_emergency as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_authsync",
            scenario_execution_preparation_binding_authsync as fn(),
        ),
        (
            "rejection_fail_closed",
            "scenario_execution_preparation_binding_bulk",
            scenario_execution_preparation_binding_bulk as fn(),
        ),
        (
            "rejection_fail_closed",
    ...
    // Run 317 domain-separated digest free functions.
    let case = m_case(TrustBundleEnvironment::Devnet, Sc::Add);
    let decision = m_eval(&case);
    let artifact = decision
        .execution_artifact
        .as_ref()
        .expect("accepted Devnet Add case must carry a live-mutation execution artifact");
    // On accept, the executor derives the identifiers from the consumed Run 315
    // execution-preparation decision's preparation (content) digest.
    let prep = prep_decision(TrustBundleEnvironment::Devnet, Sc::Add);
    let execution_preparation_intent_digest = prep.preparation_digest.clone();
    let content_digest =
        production_live_epoch_transition_mutation_execution_content_digest(artifact);
    let execution_id = production_live_epoch_transition_mutation_execution_id(
        ...
        &case.inputs.mutation_execution_policy_id,
        case.request.proposed_epoch_transition_target,
        case.request.mutation_execution_nonce,
    );
    ...
    let transcript_digest = production_live_epoch_transition_mutation_execution_transcript_digest(
        PRODUCTION_LIVE_EPOCH_TRANSITION_MUTATION_EXECUTION_PROTOCOL_VERSION,
        &request_id,
        &content_digest,
        outcome_tag,
    ...
    digest_body.push_str(&format!(
        "execution_preparation_intent_digest={execution_preparation_intent_digest}\n"
    ));
    for sc in ALL_SC {
        let case = m_case(TrustBundleEnvironment::Devnet, sc);
        let decision = m_eval(&case);
        digest_body.push_str(&format!("scenario={:?}\n", sc));
        digest_body.push_str(&format!("execution_id={}\n", decision.execution_id));
        digest_body.push_str(&format!("request_id={}\n", decision.request_id));
        digest_body.push_str(&format!("execution_digest={}\n", decision.execution_digest));
        digest_body.push_str(&format!(
            "content_digest={}\n",
            decision
                .execution_artifact
    ...
    digest_body.push_str(&format!("named_execution_id={execution_id}\n"));
    ...
    fs::write(
        outdir.join("fixtures/run_318_deterministic_digests.txt"),
        &digest_body,
    )
    .expect("write deterministic-digest fixture");

    // Recovery / idempotency fixture — forces release-symbol linkage of the
    // Run 317 non-mutating recovery window over the typed
    // `ProductionLiveEpochTransitionMutationExecutionArtifact` and its
    // `ProductionLiveEpochTransitionMutationExecutionRecoveryOutcome`.
    let recover_exec = ProductionLiveEpochTransitionMutationExecutionExecutor::source_test();
    let current_artifact: &ProductionLiveEpochTransitionMutationExecutionArtifact = artifact;
    let clean: ProductionLiveEpochTransitionMutationExecutionRecoveryOutcome = recover_exec
        .recover_live_epoch_transition_mutation_execution_window(None, current_artifact);
    let idempotent: ProductionLiveEpochTransitionMutationExecutionRecoveryOutcome = recover_exec
    ...
    assert!(
        matches!(
            idempotent,
            ProductionLiveEpochTransitionMutationExecutionRecoveryOutcome::IdempotentReplayObserved { .. }
        ),
    ...
    fs::write(
        outdir.join("fixtures/run_318_recovery_window.txt"),
        &recovery_body,
    )
    ...
    summary.push_str(
        "run_318_production_live_epoch_transition_mutation_execution_release_binary_helper\n",
    );
    summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_mutation_execution.rs :: ProductionLiveEpochTransitionMutationExecutionExecutor (Run 317 source/test boundary)\n");
