//! Run 342 - release-binary helper for the Run 341 **live epoch-transition
//! final settlement / authority-lifecycle completion boundary**.
//!
//! Release-binary evidence for the Run 341 live epoch-transition final
//! settlement / authority-lifecycle completion boundary
...
//! release-built production library symbols. It exercises the **real** Run 341
//! `ProductionLiveEpochTransitionFinalSettlementCompletionExecutor` and proves,
...
//! -> 327 -> 329 -> 331 -> 333 -> 335 -> 337 -> 339 chain to produce a verified,
//! accepted Run 339 live epoch-transition settlement / finalization execution
//! decision (`is_accept()` with `Some(settlement_execution_artifact)`), then
//! feeds that decision into the Run 341 live epoch-transition final settlement /
//! authority-lifecycle completion executor, which emits only a typed,
//! deterministic, non-mutating final-settlement / authority-lifecycle completion
//! artifact.
//!
...
//! enablement, final-settlement-completion default wiring, or live validator-set
//! mutation. The executor only ever produces typed non-mutating live
//! final-settlement / authority-lifecycle completion artifacts; it never applies
...
//! durable replay record, settlement record, settlement-finalization record,
//! settlement-execution record, final-settlement record,
//! authority-lifecycle-completion record, publication record,
//! external-publication record, or final-execution record, never calls Run 070,
...
//! sequence or authority marker files. The only mutation any positive path
//! performs is against a caller-owned in-memory
//! `LiveEpochTransitionFinalSettlementCompletionFixtureState` used exclusively as
//! source/test evidence and clearly distinct from production runtime state.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_342.md`.
#![allow(dead_code)]
...
mod run_341_final_settlement_completion {
    ...
    pub fn helper_main() {
        let outdir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
            PathBuf::from(
                "docs/devnet/run_342_production_live_epoch_transition_final_settlement_completion_release_binary/helper_evidence/run_342",
            )
        });
        ...
        let cases: &[(&str, &str, fn())] = &[
            ...
            (
                "accepted_compatible",
                "accept_artifact_reexposes_consumed_settlement_execution_transcript",
                accept_artifact_reexposes_consumed_settlement_execution_transcript as fn(),
            ),
            (
                "accepted_compatible",
            ...
            (
                ...
                "wrong_settlement_execution_decision_id",
                wrong_settlement_execution_decision_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_settlement_execution_request_id",
                wrong_settlement_execution_request_id as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_settlement_execution_intent_digest",
                wrong_settlement_execution_intent_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_settlement_execution_transcript_digest",
                wrong_settlement_execution_transcript_digest as fn(),
            ),
            (
                "rejection_fail_closed",
                "wrong_settlement_execution_nonce",
                wrong_settlement_execution_nonce as fn(),
            ),
            (
                "rejection_fail_closed",
                "tampered_settlement_execution_package_integrity_mismatch",
                tampered_settlement_execution_package_integrity_mismatch as fn(),
            ),
            (
                "rejection_fail_closed",
            ...
            (
                "rejection_fail_closed",
                "reject_missing_settlement_execution_decision",
                reject_missing_settlement_execution_decision as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_unverified_settlement_execution_decision",
                reject_unverified_settlement_execution_decision as fn(),
            ),
            (
                "rejection_fail_closed",
            ...
            (
                "rejection_fail_closed",
                "reject_accepted_settlement_execution_without_package_via_verified_source",
                reject_accepted_settlement_execution_without_package_via_verified_source as fn(),
            ),
            (
                "rejection_fail_closed",
                "reject_accepted_settlement_execution_without_package_variant",
                reject_accepted_settlement_execution_without_package_variant as fn(),
            ),
            (
                "rejection_fail_closed",
            ...
            (
                "rejection_fail_closed",
                "reject_fixture_only_settlement_execution",
                reject_fixture_only_settlement_execution as fn(),
            ),
            (
                "rejection_fail_closed",
        ...
        let accepted_case = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let accepted_decision = a_eval(&accepted_case);
        assert!(
            accepted_decision.is_accept(),
            "accepted fixture case must accept"
        );
        let accepted_artifact = accepted_decision
            .final_settlement_completion_artifact
            .as_ref()
            .expect("accepted Devnet Add case must carry a final-settlement-completion artifact");
        let mut accepted_body = String::new();
        accepted_body.push_str("scenario=Devnet::Add\n");
        accepted_body.push_str(&format!(
            "final_settlement_completion_id={}\n",
            accepted_decision.final_settlement_completion_id
        ));
        ...
        accepted_body.push_str(&format!(
            ...
            accepted_decision.final_settlement_completion_digest
        ));
        accepted_body.push_str(&format!(
            "transcript_digest={}\n",
        ...
        fs::write(
            outdir.join("fixtures/run_342_accepted_artifact.txt"),
            &accepted_body,
        )
        .expect("write accepted-artifact fixture");

        // Deterministic-digest fixture across two independent evaluations.
        let digest_case_1 = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let digest_case_2 = a_case(TrustBundleEnvironment::Devnet, Sc::Add);
        let digest_decision_1 = a_eval(&digest_case_1);
        let digest_decision_2 = a_eval(&digest_case_2);
        ...
        assert_eq!(
            digest_decision_1.final_settlement_completion_id,
            digest_decision_2.final_settlement_completion_id
        );
        ...
        assert_eq!(
            digest_decision_1.final_settlement_completion_digest,
            digest_decision_2.final_settlement_completion_digest
        );
        assert_eq!(
        ...
        digest_body.push_str(&format!(
            "second_final_settlement_completion_id={}\n",
            digest_decision_2.final_settlement_completion_id
        ));
        digest_body.push_str(&format!(
            "second_request_id={}\n",
        ...
        digest_body.push_str(&format!(
            "second_final_settlement_completion_digest={}\n",
            digest_decision_2.final_settlement_completion_digest
        ));
        digest_body.push_str(&format!(
        ...
        fs::write(
            outdir.join("fixtures/run_342_deterministic_digests.txt"),
            &digest_body,
        )
        ...
        let recover_exec =
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutor::source_test();
        let current_artifact: &ProductionLiveEpochTransitionFinalSettlementCompletionArtifact =
            accepted_artifact;
        let clean: ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome =
            recover_exec.recover_live_epoch_transition_final_settlement_completion_window(
                None,
                current_artifact,
            );
        let idempotent: ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome =
            recover_exec.recover_live_epoch_transition_final_settlement_completion_window(
        ...
        let staged_application_id = match &idempotent {
            ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome::IdempotentReplayObserved { staged_application_id } => staged_application_id.clone(),
        ...
        fs::write(
            outdir.join("fixtures/run_342_recovery_window.txt"),
            &recovery_body,
        )
        ...
        summary.push_str("run_342_production_live_epoch_transition_final_settlement_completion_release_binary_helper\n");
        summary.push_str("boundary=crates/qbind-node/src/pqc_production_live_epoch_transition_final_settlement_completion.rs :: ProductionLiveEpochTransitionFinalSettlementCompletionExecutor (Run 341 source/test boundary)\n");
...
fn main() {
    run_341_final_settlement_completion::helper_main();
}
