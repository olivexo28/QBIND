//! Run 337 — source/test **live epoch-transition settlement /
//! finalization execution-preparation** boundary integration tests.
//!
//! Source/test only. Run 337 does **not** capture release-binary evidence;
//! release-binary evidence for the live epoch-transition settlement /
//! finalization execution-preparation boundary is deferred to **Run 338**.
//!
//! Each accepted case composes the real Run 303 -> 305 -> 307 -> 309 -> 311 ->
//! 313 -> 315 -> 317 -> 319 -> 321 -> 323 -> 325 -> 327 -> 329 -> 331 -> 333 ->
//! 335 chain to produce a verified, accepted Run 335 live epoch-transition
//! settlement / finalization-preparation decision, then feeds that decision
//! into the Run 337 live epoch-transition settlement / finalization
//! execution-preparation executor.
//!
//! Run 337 produces **only** a typed, non-mutating settlement /
//! finalization execution-preparation artifact. It never applies a live
...
//! production settlement / settlement-finalization / external-publication
//! record, and never mutates any durable trust state. The only mutation a
//! positive path performs is against a caller-owned in-memory
//! `LiveEpochTransitionSettlementExecutionPreparationFixtureState` used
