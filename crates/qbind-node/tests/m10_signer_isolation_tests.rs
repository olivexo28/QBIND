...
fn m10_domain_tag_constant_correct() {
    ...
    assert_eq!(
        REMOTE_SIGNER_DOMAIN_TAG.len(),
        "QBIND:remote-signer:v1".len()
    );
}
...
fn m10_request_kind_message_type_conversion() {
    ...
    // Round-trip
    assert_eq!(
        RemoteSignRequestKind::from_message_type(0x01),
        Some(RemoteSignRequestKind::Proposal)
    );
    assert_eq!(
        RemoteSignRequestKind::from_message_type(0x02),
        Some(RemoteSignRequestKind::Vote)
    );
    assert_eq!(
        RemoteSignRequestKind::from_message_type(0x03),
        Some(RemoteSignRequestKind::Timeout)
    );
...
fn m10_replay_protection_rejects_duplicate_request_id() {
    ...
    let resp2 = transport
        .send_sign_request(req2)
        .expect("request 2 should return error response");
...
fn m10_replay_protection_rejects_lower_request_id() {
    ...
    let resp2 = transport
        ...
        .expect("request 2 should return error response");
    assert!(
        resp2.signature.is_none(),
        "lower request_id should be rejected"
    );
    assert_eq!(resp2.error, Some(RemoteSignError::ReplayDetected));
}
...
fn m10_error_display_strings() {
    assert_eq!(
        format!("{}", RemoteSignError::ReplayDetected),
        "replay detected: request_id not monotonic"
    );
    assert_eq!(
        format!("{}", RemoteSignError::SignerUnavailable),
        "signer unavailable (fail-closed)"
    );
    assert_eq!(
        format!("{}", RemoteSignError::MalformedResponse),
        "malformed response from signer"
    );
}
...
fn m10_loopback_round_trip_proposal() {
    ...
    assert!(
        signature.len() > 2000,
        "ML-DSA-44 signatures should be ~2420 bytes"
    );
}
