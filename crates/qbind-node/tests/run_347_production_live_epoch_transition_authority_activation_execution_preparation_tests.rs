//! Run 347 — source/test **live epoch-transition authority-activation
//! execution-preparation / final-execution preflight** boundary integration
//! tests.
//!
//! Source/test only. Run 347 does **not** capture release-binary evidence;
...
//! **Run 348**.
//!
//! Each accepted `run_347_authority_activation_execution_preparation` case
//! composes the real deep chain up through the Run 343/344
//! post-completion-attestation and Run 345/346 authority-activation-authorization
//! layers to produce a verified, accepted Run 345/346 live epoch-transition
//! authority-activation-authorization decision, then feeds that decision into
//! the Run 347 live epoch-transition authority-activation execution-preparation
//! / final-execution preflight executor.
//!
//! Run 347 produces **only** a typed, non-mutating authority-activation
...
//! validator-set change, never transitions a consensus epoch, never writes
//! `meta:current_epoch`, never injects a reconfig block, never writes any
//! production settlement / settlement-finalization / post-completion-attestation
//! / authority-activation-authorization / authority-activation
//! execution-preparation / final-execution record, and never mutates any
//! durable trust state. The only mutation a positive path performs is against a
//! caller-owned in-memory fixture state used exclusively by these tests.
