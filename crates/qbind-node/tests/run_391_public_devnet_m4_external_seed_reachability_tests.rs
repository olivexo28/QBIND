//! Run 391 — public DevNet M4 external seed reachability: live-admission tests.
//!
//! Run 391 lands **no production source change**: the
//! `qbind-node identity register-check --status live --reachability-evidence <ref>`
//! admission gate already exists (Run 376). These tests pin the Run 391 M4
//! evidence contract in CI against the Run 391 reachability-evidence reference:
...
//! infrastructure, which is unavailable in this environment — the Run 391 harness
...
fn reach_ref() -> String {
    "docs/release/public-devnet/network/reachability/RUN_391_qbind-devnet-seed-1.md".to_string()
}
...
// ---------------------------------------------------------------------------
// 1. --status live WITH the Run 391 reachability-evidence reference is admitted,
...
fn live_status_with_reachability_evidence_is_admitted() {
    ...
    // The live candidate carries the supplied Run 391 reachability reference.
    assert_eq!(
