...
//!
//! ```bash
//! cargo test -p qbind-node --test async_runner_tests -- --test-threads=1
//! ```
//!
...
use qbind_node::async_runner::{AsyncNodeError, AsyncNodeRunner, ConsensusEvent};
...
fn async_node_error_display_variants() {
    ...
    let harness_err = qbind_node::NodeHotstuffHarnessError::Config("test error".to_string());
