//! Run 377 — public DevNet M4 live seed reachability: live-admission tests.
//!
//! Run 377 lands **no production source change**: the
//! `qbind-node identity register-check --status live --reachability-evidence <ref>`
//! admission gate already exists (Run 376). These tests pin the Run 377 M4
//! evidence contract in CI: the live admission gate accepts a live candidate ONLY
//! when a reachability-evidence reference is supplied, fails closed without it,
//! and fails closed for a `planned` candidate that falsely carries reachability
//! evidence. They also assert the command makes NO live/reachability/M4/C4/C5
//! claim even when it admits a `live` candidate — the gate is a structural
//! admission decision, not a reachability proof.
//!
...
//! admission. External reachability is proven only by the Run 377 harness'
//! evidence document, which records a Partial-positive (loopback-only) verdict —
//! M4 stays Yellow.
...
fn reach_ref() -> String {
    "docs/release/public-devnet/network/reachability/RUN_377_qbind-devnet-seed-1.md".to_string()
}
...
// ---------------------------------------------------------------------------
// 1. --status live WITH a reachability-evidence reference is admitted, and the
//    command still makes NO live/reachability/M4/C4/C5 claim.
// ---------------------------------------------------------------------------
...
fn live_status_with_reachability_evidence_is_admitted() {
    ...
    // The live candidate carries the supplied reachability reference.
    assert_eq!(
...
// ---------------------------------------------------------------------------
// 4. The committed Run 377 live-candidate document is a schema-honest preflight:
//    its single seed entry is NOT `live` and carries null reachability evidence
//    (external reachability was not proven; M4 stays Yellow).
// ---------------------------------------------------------------------------
