//! Tests for per-validator vote metrics (T128) in qbind-node.
//!
...
use qbind_node::{NodeMetrics, ValidatorVoteMetrics, MAX_TRACKED_VALIDATORS};
...
fn format_metrics_empty_when_no_votes() {
    ...
    // Should not contain any qbind_consensus_validator_votes_total lines
    assert!(!output.contains("qbind_consensus_validator_votes_total{"));
}
...
fn format_metrics_includes_validators_with_votes() {
    ...
    assert!(output.contains("qbind_consensus_validator_votes_total{validator=\"1\"} 2"));
    assert!(output.contains("qbind_consensus_validator_last_vote_view{validator=\"1\"} 10"));
    assert!(output.contains("qbind_consensus_validator_votes_total{validator=\"42\"} 1"));
    assert!(output.contains("qbind_consensus_validator_last_vote_view{validator=\"42\"} 3"));
}
...
fn overflow_shown_in_format_metrics() {
    ...
    let output = metrics.format_metrics();
    assert!(output.contains("qbind_consensus_validator_overflow_count 1"));
}
...
fn node_metrics_format_includes_validator_votes() {
    ...
    assert!(output.contains("qbind_consensus_validator_votes_total{validator=\"1\"} 1"));
    assert!(output.contains("qbind_consensus_validator_last_vote_view{validator=\"1\"} 5"));
}
...
fn format_metrics_with_view_lag_includes_lag() {
    ...
    assert!(output.contains("qbind_consensus_validator_view_lag{validator=\"1\"} 5"));
    assert!(output.contains("qbind_consensus_validator_view_lag{validator=\"2\"} 2"));
}
