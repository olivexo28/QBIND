// Run 278 — durable-completion external-publication-confirmation consumer /
// external-publication-receipt boundary tests.
//
// Source/test-only modeled boundary that sits one rung above the Run 276
// external-publication-confirmation boundary. The chain attaches the real Run 256
...
// settlement-outcome report, Run 274 settlement-outcome publication, and Run 276
// external-publication-confirmation records, then evaluates the Run 278
// external-publication-receipt boundary. Only a Run 278
// ExternalPublicationReceiptRecorded outcome authorizes new state; every
...
fn ctx_action(
    ...
    let external_publication_confirmation =
        attach_run276_external_publication_confirmation(environment, action, consumer_duplicate);
...
impl Ctx {
    ...
    fn input_with_confirmation(
        ...
        external_publication_receipt: GovernanceModeledDurableCompletionFinalizationOutcome,
        ...
        DurableCompletionExternalPublicationReceiptInput {
            ...
            confirmation_binding: external_publication_receipt,
...
fn devnet_fixture_chain_records_exactly_one_receipt_only_after_backend_submission() {
    ...
    assert!(external_publication_receipt_outcome_authorizes_record(&outcome));
    assert!(external_publication_receipt_outcome_projects_to_recorded(&outcome));
...
fn duplicate_identical_receipt_is_idempotent() {
    ...
    assert!(!external_publication_receipt_outcome_authorizes_record(&second));
    assert!(external_publication_receipt_outcome_projects_to_recorded(&second));
    assert_eq!(ledger.len(), 1);
}

#[test]
fn run268_duplicate_idempotent_confirmation_only_matches_existing_never_creates() {
    // A real Run 276 ExternalPublicationConfirmationDuplicateIdempotent outcome with
    // identical digests.
...
#[test]
fn non_recording_confirmation_outcomes_never_record_external_publication_receipt() {
    ...
    assert_non_recording_confirmation(
        ...
        Receipt::LegacyBypassNoExternalPublicationReceipt,
    );
    assert_non_recording_confirmation(
        Finalization::RejectedBeforeSettlementOutcomePublicationNoConfirmation,
        Receipt::RejectedBeforeExternalPublicationConfirmationNoReceipt,
    );
    assert_non_recording_confirmation(
...
fn wrong_external_publication_receipt_record_id_rejected_before_record() {
    assert_request_mismatch_rejected(|r| {
        r.external_publication_receipt_record_id = "other-confirmation".to_string();
    });
}
...
#[test]
fn only_recorded_confirmation_outcome_creates_external_publication_receipt_request_intent() {
    use DurableCompletionExternalPublicationConfirmationOutcome as Finalization;
    ...
    assert!(project_external_publication_confirmation_outcome_to_external_publication_receipt_request(
        &Finalization::ExternalPublicationConfirmationRecorded
    )
    .creates_request());
    assert_eq!(
...
#[test]
fn non_recording_confirmation_outcomes_create_no_external_publication_receipt_request() {
    use DurableCompletionExternalPublicationConfirmationOutcome as Finalization;
    use DurableCompletionExternalPublicationReceiptRequestIntent as Intent;
    for confirmation in [
        Finalization::LegacyBypassNoExternalPublicationConfirmation,
        ...
        assert!(matches!(
            project_external_publication_confirmation_outcome_to_external_publication_receipt_request(&confirmation),
...
fn invariant_helpers_assert_fail_closed_contract() {
    ...
    assert!(durable_completion_external_publication_receipt_no_rocksdb_file_schema_migration_change());
