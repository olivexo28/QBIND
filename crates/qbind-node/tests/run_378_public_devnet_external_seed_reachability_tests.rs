//! Run 378 — public DevNet M4 external seed reachability: live-admission tests.
//!
//! Run 378 lands **no production source change**: the
//! `qbind-node identity register-check --status live --reachability-evidence <ref>`
//! admission gate already exists (Run 376). These tests pin the Run 378 M4
//! evidence contract in CI against the Run 378 reachability-evidence reference:
...
//! infrastructure, which is unavailable in this environment — the Run 378 harness
...
fn reach_ref() -> String {
    "docs/release/public-devnet/network/reachability/RUN_378_qbind-devnet-seed-1.md".to_string()
}
...
// ---------------------------------------------------------------------------
// 1. --status live WITH the Run 378 reachability-evidence reference is admitted,
...
fn live_status_with_reachability_evidence_is_admitted() {
    ...
    // The live candidate carries the supplied Run 378 reachability reference.
    assert_eq!(
