...
fn test_nodeid_derivation_from_pubkey_is_deterministic() {
    ...
    assert_eq!(
        node_id_1, node_id_2,
        "Same pubkey should produce same NodeId"
    );
}
...
fn test_different_certs_produce_different_nodeids() {
    ...
    assert_ne!(
        node_id_a, node_id_b,
        "Different certs should produce different NodeIds"
    );
}
...
fn test_signature_affects_nodeid() {
    ...
    assert_ne!(
        node_id_a, node_id_b,
        "Different signatures should produce different NodeIds"
    );
}
...
fn test_different_leaf_kem_pk_produces_different_nodeid() {
    ...
    assert_ne!(
        node_id_a, node_id_b,
        "Different leaf_kem_pk should produce different NodeIds"
    );
}
...
fn test_empty_pubkey_produces_nonzero_nodeid() {
    ...
    assert_ne!(
        node_id, zero,
        "Even empty pubkey should produce non-zero NodeId due to domain tag"
    );
}
...
fn test_large_pubkey_works() {
    ...
    assert_eq!(
        node_id, node_id_2,
        "Large pubkey derivation should be deterministic"
    );
}
...
fn test_regression_nodeid_not_zero() {
    ...
    for (i, pubkey) in pubkey_scenarios.iter().enumerate() {
        ...
        assert_ne!(
            node_id, zero,
            "Scenario {}: NodeId should not be zero for any valid pubkey",
            i
        );
    }
}
...
fn test_nodeid_stable_across_calls() {
    ...
    let node_ids: Vec<_> = (0..10).map(|_| derive_node_id_from_cert(&cert)).collect();

    // All should be identical
    for (i, node_id) in node_ids.iter().enumerate() {
        assert_eq!(
            *node_id, node_ids[0],
            "NodeId at iteration {} should match first derivation",
            i
        );
    }
}
