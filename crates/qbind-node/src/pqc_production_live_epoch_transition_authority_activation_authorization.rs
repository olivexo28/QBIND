//! Run 345 — source/test live epoch-transition **authority-lifecycle
//! activation-readiness / post-completion attestation** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 343/344 live epoch-transition final-settlement / authority-lifecycle
//! completion boundary: the boundary that consumes a **verified** Run 343/344
//! non-mutating live epoch-transition final-settlement / authority-lifecycle
//! completion *decision* (the accepted
//! [`ProductionLiveEpochTransitionPostCompletionAttestationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionPostCompletionAttestationArtifact`], as
//! source-test-evidenced by Run 343) and translates it into a typed,
//! deterministic, policy-gated **post-completion attestation artifact** for a
//! *future* production authority-lifecycle activation / completion-recognition
//! / final-execution sink — *without* ever activating anything, completing
//! anything in production, applying a live validator-set change, transitioning
//! a consensus epoch, or mutating any trust state.
//!
//! The name is deliberate: this is **activation-readiness / post-completion
//! attestation**, *not* production activation. The artifact may attest that
//! prior non-mutating prerequisites have been satisfied in source/test, but it
//! never applies a validator set, transitions an epoch, activates authority
//! state, writes completion state, or performs final execution. Where the Run
//! 343 boundary answers "given a verified settlement-execution decision, what
//! typed non-mutating final-settlement / authority-lifecycle completion
//! artifact does it authorize?", Run 345 answers the next question: "given a
//! verified final-settlement / authority-lifecycle completion decision, what
//! typed, non-mutating **post-completion attestation artifact** carrying the
//! exact preconditions a *future* production activation-readiness /
//! authority-activation-authorization sink must re-verify does it authorize, under an
//! explicit authority-activation-authorization policy, bound to the full
//! completion-decision / settlement / commit / audit / publication / governance
...
//! authority-activation-authorization nonce?".
//!
//! ## Scope and honesty constraints (Run 345)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 346**.
...
//!   attestation-artifact construction.
//! * Only a **verified** Run 343/344 live epoch-transition final-settlement /
//!   authority-lifecycle completion decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionPostCompletionAttestationArtifact`] can authorize a
//!   post-completion attestation artifact. Unverified decisions, accepted
//!   decisions without a prepared artifact, a settlement-execution decision
...
//!   [`ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact`]; it
//!   **never** applies the decision, never activates authority state, never
//!   mutates a live validator set, never writes durable validator-set state,
...
//!   activation-readiness / authority-activation-authorization sink.
//! * MainNet remains **refused**: even a fully valid source/test
//!   DevNet/TestNet attestation artifact does not enable MainNet runtime
//!   behavior.
...
//!   activation, no external publication, and no default runtime wiring.
//! * No CLI flag and no default runtime wiring is added. Full C4 remains
//!   OPEN; C5 remains OPEN.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_343.md`.
...
use crate::pqc_production_live_epoch_transition_authority_lifecycle_post_completion_attestation::{
...
/// Run 345 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_PROTOCOL_VERSION: u16 = 1;

/// Run 345 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:345-epoch-transition-authority-activation-authorization-intent:v1";

/// Run 345 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_ID_DOMAIN_TAG: &str =
    "QBIND:345-epoch-transition-authority-activation-authorization-id:v1";

/// Run 345 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:345-epoch-transition-authority-activation-authorization-request:v1";

/// Run 345 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:345-epoch-transition-authority-activation-authorization-transcript:v1";
...
/// Run 345 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuthorityActivationAuthorization` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 343/344 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuthorityActivationAuthorization` and
/// `MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired` are
...
pub enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionAuthorityActivationAuthorization,
    ...
    RequireProductionLiveEpochTransitionAuthorityActivationAuthorization,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuthorityActivationAuthorization => {
                "allow-source-test-epoch-transition-post-completion-attestation"
            }
            Self::RequireProductionLiveEpochTransitionAuthorityActivationAuthorization => {
                "require-production-epoch-transition-post-completion-attestation"
            }
            Self::MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired => {
                "mainnet-production-epoch-transition-post-completion-attestation-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuthorityActivationAuthorization` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionAuthorityActivationAuthorization` kind is fail-closed as
/// unavailable in Run 345 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionAuthorityActivationAuthorization,
    /// Reserved production authorization kind. Fail-closed in Run 345.
    ProductionLiveEpochTransitionAuthorityActivationAuthorization,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuthorityActivationAuthorization => {
                "source-test-epoch-transition-post-completion-attestation"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationAuthorization => {
                "production-epoch-transition-post-completion-attestation"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::SourceTestLiveEpochTransitionAuthorityActivationAuthorization,
        )
    }
...
impl Default for ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::Disabled)
    }
}
...
/// Run 345 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 343/344
/// [`LiveEpochTransitionPostCompletionAttestationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuthorityActivationAuthorizationKind {
...
impl LiveEpochTransitionAuthorityActivationAuthorizationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 345 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 343/344 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionPostCompletionAttestationKind,
    ) -> Self {
        use LiveEpochTransitionPostCompletionAttestationKind as A;
        match kind {
...
/// Run 345 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedPostCompletionAttestationDecision`] carrying a Run 343/344
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource {
    /// A verified Run 343/344 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedPostCompletionAttestationDecision {
        decision: ProductionLiveEpochTransitionPostCompletionAttestationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingPostCompletionAttestationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedPostCompletionAttestationDecision {
        decision: ProductionLiveEpochTransitionPostCompletionAttestationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedPostCompletionAttestationWithoutPackage {
        decision: ProductionLiveEpochTransitionPostCompletionAttestationDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 343/344 post-completion-attestation decision. Rejected.
    CommitAuthorizationDecisionWithoutPostCompletionAttestation,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 343/344 post-completion-attestation decision. Rejected.
    MutationExecutionDecisionWithoutPostCompletionAttestation,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 343/344 post-completion-attestation decision. Rejected.
    ExecutionPreparationDecisionWithoutPostCompletionAttestation,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 343/344 post-completion-attestation decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutPostCompletionAttestation,
    /// A Run 331/332 guarded epoch-transition authority-activation-authorization decision
    /// presented directly, without a Run 343/344 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutPostCompletionAttestation,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 343/344 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutPostCompletionAttestation,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 343/344 guarded authority-activation-authorization decision. Rejected.
    LiveApplicationAuthorizationWithoutPostCompletionAttestation,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 343/344 guarded authority-activation-authorization decision.
    /// Rejected.
    ApplicationDecisionWithoutPostCompletionAttestation,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 343/344 live-application authorization. Rejected.
    RotationPlanWithoutPostCompletionAttestation,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 343/344 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutPostCompletionAttestation,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutPostCompletionAttestation,
    ...
    FixtureOnlyPostCompletionAttestation,
...
/// Run 345 — the explicit trusted inputs the executor binds a verified
/// Run 343/344 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs {
    ...
    pub authority_activation_authorization_policy_id: String,
    /// Expected Run 343/344 live-application authorization policy id (bound
    ...
    /// Expected Run 343/344 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 343/344 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 343/344 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 343/344 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 343/344 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 343/344 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 343/344 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 343/344 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 343/344 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 343/344 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded authority-activation-authorization decision id (re-exposed
    /// by the consumed Run 343/344 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded authority-activation-authorization request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded authority-activation-authorization intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded authority-activation-authorization transcript digest.
    ...
    /// Expected Run 343/344 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_post_completion_attestation_decision_id: String,
    /// Expected Run 343/344 runtime handoff request id.
    pub expected_post_completion_attestation_request_id: String,
    /// Expected Run 343/344 runtime handoff package/content digest (the
    /// consumed decision's `authority_activation_authorization_digest`, i.e. the handoff content
    /// digest).
    pub expected_post_completion_attestation_intent_digest: String,
    /// Expected Run 343/344 runtime handoff transcript digest.
    pub expected_post_completion_attestation_transcript_digest: String,
    /// Expected Run 343/344 post-completion-attestation nonce (re-exposed by the
    /// consumed Run 343/344 post-completion-attestation artifact).
    pub expected_post_completion_attestation_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 343/344 post-completion-attestation artifact).
    ...
    /// Run 343/344 post-completion-attestation artifact).
    ...
    /// wall-clock value and never a reject path in Run 345.
...
impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.authority_activation_authorization_policy_id.is_empty()
            ...
            && !self.expected_post_completion_attestation_decision_id.is_empty()
            && !self.expected_post_completion_attestation_request_id.is_empty()
            && !self.expected_post_completion_attestation_intent_digest.is_empty()
            && !self.expected_post_completion_attestation_transcript_digest.is_empty()
...
/// Run 345 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition authority-activation-authorization decision), the explicit
/// epoch-transition target, a post-completion-attestation nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest {
    pub authority_source: LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The post-completion-attestation nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and post-completion-attestation nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource,
        proposed_epoch_transition_target: u64,
        authority_activation_authorization_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 345 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuthorityActivationAuthorizationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuthorityActivationAuthorizationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuthorityActivationAuthorizationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuthorityActivationAuthorizationReplaySet;

impl LiveEpochTransitionAuthorityActivationAuthorizationReplaySet
    for EmptyLiveEpochTransitionAuthorityActivationAuthorizationReplaySet
{
...
/// Run 345 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 346+); Run 345 never
...
/// preconditions, and carries deterministic `authority_activation_authorization_id`, `request_id`,
/// `authority_activation_authorization_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact {
    pub staged_kind: LiveEpochTransitionAuthorityActivationAuthorizationKind,
    pub protocol_version: u16,
    pub authority_activation_authorization_policy_id: String,

    // ---- Re-exposed Run 343/344 authorization intent tuple ------------
    ...
    // ---- Bound Run 343/344 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 343/344 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 343/344 guarded authority-activation-authorization authority tuple -
    // (the consumed guarded authority-activation-authorization decision transcript)
    ...
    // ---- Guarded authority-activation-authorization binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded authority-activation-authorization nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 343/344 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub post_completion_attestation_decision_id: String,
    pub post_completion_attestation_request_id: String,
    pub post_completion_attestation_intent_digest: String,
    pub post_completion_attestation_transcript_digest: String,
    /// The re-exposed Run 343/344 post-completion-attestation nonce (consumed).
    pub post_completion_attestation_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 343/344 post-completion-attestation artifact)
    ...
    /// The newly proposed authority-activation-authorization nonce.
    pub authority_activation_authorization_nonce: u64,
    ...
    pub authority_activation_authorization_id: String,
    pub request_id: String,
    pub authority_activation_authorization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`authority_activation_authorization_id`,
    /// `request_id`, `authority_activation_authorization_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_authorization_policy_id",
            self.authority_activation_authorization_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"post_completion_attestation_decision_id",
            self.post_completion_attestation_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_completion_attestation_request_id",
            self.post_completion_attestation_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_completion_attestation_intent_digest",
            self.post_completion_attestation_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_completion_attestation_transcript_digest",
            self.post_completion_attestation_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_completion_attestation_nonce",
            &self.post_completion_attestation_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 345 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_authority_activation_authorization_content_digest(
    package: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
) -> String {
    package.content_digest()
}

/// Run 345 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and post-completion-attestation nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_authority_activation_authorization_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_authorization_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"authority_activation_authorization_policy_id",
        authority_activation_authorization_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"authority_activation_authorization_nonce",
        &authority_activation_authorization_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 345 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and post-completion-attestation nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_authority_activation_authorization_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_authorization_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"authority_activation_authorization_policy_id", authority_activation_authorization_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 345 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_authority_activation_authorization_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_POST_COMPLETION_ATTESTATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 345 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationAuthorization`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuthorityActivationAuthorizationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition authority-activation-authorization
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuthorityActivationAuthorization {
        execution_kind: LiveEpochTransitionAuthorityActivationAuthorizationKind,
    ...
    VerifiedPostCompletionAttestationDecisionRequired,
    UnverifiedPostCompletionAttestationDecisionRejected,
    ...
    PostCompletionAttestationDecisionIdMismatch,
    PostCompletionAttestationDecisionRequestIdMismatch,
    PostCompletionAttestationDecisionIntentDigestMismatch,
    PostCompletionAttestationDecisionTranscriptMismatch,
    PostCompletionAttestationDecisionIntegrityMismatch,
    WrongPostCompletionAttestationNonce,
    ...
    LiveEpochTransitionAuthorityActivationAuthorizationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuthorityActivationAuthorizationBoundaryUnavailable => {
                "live-epoch-transition-authority-activation-authorization-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable => {
                "production-live-epoch-transition-authority-activation-authorization-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable => {
                "mainnet-production-live-epoch-transition-authority-activation-authorization-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationAuthorization { .. } => {
                "accepted-source-test-live-epoch-transition-authority-activation-authorization"
            }
            Self::VerifiedPostCompletionAttestationDecisionRequired => {
                "verified-post-completion-attestation-decision-required"
            }
            Self::UnverifiedPostCompletionAttestationDecisionRejected => {
                "unverified-post-completion-attestation-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-post-completion-attestation-decision-rejected-as-production-authority"
            }
            ...
            Self::PostCompletionAttestationDecisionIdMismatch => "post-completion-attestation-decision-id-mismatch",
            Self::PostCompletionAttestationDecisionRequestIdMismatch => {
                "post-completion-attestation-decision-request-id-mismatch"
            }
            Self::PostCompletionAttestationDecisionIntentDigestMismatch => {
                "post-completion-attestation-decision-intent-digest-mismatch"
            }
            Self::PostCompletionAttestationDecisionTranscriptMismatch => {
                "post-completion-attestation-decision-transcript-mismatch"
            }
            Self::PostCompletionAttestationDecisionIntegrityMismatch => {
                "post-completion-attestation-decision-integrity-mismatch"
            }
            Self::WrongPostCompletionAttestationNonce => "wrong-post-completion-attestation-nonce",
            ...
            Self::LiveEpochTransitionAuthorityActivationAuthorizationAmbiguous { .. } => {
                "epoch-transition-post-completion-attestation-ambiguous"
            }
...
/// Run 345 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
    pub outcome: ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome,
    pub authority_activation_authorization_id: String,
    pub request_id: String,
    pub authority_activation_authorization_artifact:
        Option<ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact>,
    pub authority_activation_authorization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.authority_activation_authorization_artifact.is_some()
    }
}
...
/// Run 345 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome {
...
/// Run 345 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor {
    pub config: ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig,
    pub policy: ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionPostCompletionAttestationDecision,
            &'a ProductionLiveEpochTransitionPostCompletionAttestationArtifact,
        ),
        ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome,
    > {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome as O;
        use LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource as S;
        match source {
            S::VerifiedPostCompletionAttestationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedPostCompletionAttestationDecisionRejected);
                }
                match &decision.post_completion_attestation_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedPostCompletionAttestationDecisionRequired),
                }
            }
            S::MissingPostCompletionAttestationDecision => {
                Err(O::VerifiedPostCompletionAttestationDecisionRequired)
            }
            S::UnverifiedPostCompletionAttestationDecision { .. } => {
                Err(O::UnverifiedPostCompletionAttestationDecisionRejected)
            }
            S::AcceptedPostCompletionAttestationWithoutPackage { .. } => {
                Err(O::VerifiedPostCompletionAttestationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutPostCompletionAttestation => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutPostCompletionAttestation => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutPostCompletionAttestation => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutPostCompletionAttestation => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutPostCompletionAttestation => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutPostCompletionAttestation => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutPostCompletionAttestation => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutPostCompletionAttestation => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutPostCompletionAttestation => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutPostCompletionAttestation => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutPostCompletionAttestation => {
            ...
            S::FixtureOnlyPostCompletionAttestation => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationAuthorizationRequired => {
                    O::MainNetProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuthorityActivationAuthorizationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 345.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionAuthorityActivationAuthorizationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 343/344 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionPostCompletionAttestationDecision,
        intent: &ProductionLiveEpochTransitionPostCompletionAttestationArtifact,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs,
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 343/344 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.post_completion_attestation_id != inputs.expected_post_completion_attestation_decision_id {
            return Some(O::PostCompletionAttestationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_post_completion_attestation_request_id {
            return Some(O::PostCompletionAttestationDecisionRequestIdMismatch);
        }
        if decision.post_completion_attestation_digest != inputs.expected_post_completion_attestation_intent_digest {
            return Some(O::PostCompletionAttestationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_post_completion_attestation_transcript_digest {
            return Some(O::PostCompletionAttestationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.post_completion_attestation_digest {
            return Some(O::PostCompletionAttestationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed post-completion-attestation
        // nonce binding.
        if intent.post_completion_attestation_nonce != inputs.expected_post_completion_attestation_nonce {
            return Some(O::WrongPostCompletionAttestationNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 343/344
        // post-completion-attestation artifact).
        ...
        // authority tuple binding (carried through the consumed Run 343/344
        // post-completion-attestation artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition authority-activation-authorization
        ...
        // Re-exposed Run 343/344 staged-application decision authority tuple
        ...
        // Re-exposed Run 343/344 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationAuthorizationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_authority_activation_authorization_request_id(
            self.config.protocol_version.0,
            &decision.post_completion_attestation_digest,
            &inputs.authority_activation_authorization_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_authorization_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionAuthorityActivationAuthorizationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            authority_activation_authorization_policy_id: inputs.authority_activation_authorization_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded authority-activation-authorization decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded authority-activation-authorization nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 343/344 runtime handoff decision transcript (the
            // authority source this run consumes).
            post_completion_attestation_decision_id: decision.post_completion_attestation_id.clone(),
            post_completion_attestation_request_id: decision.request_id.clone(),
            post_completion_attestation_intent_digest: decision.post_completion_attestation_digest.clone(),
            post_completion_attestation_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed post-completion-attestation nonce.
            post_completion_attestation_nonce: application_intent.post_completion_attestation_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed post-completion-attestation artifact).
            ...
            // (carried through the consumed post-completion-attestation artifact).
            ...
            // Newly proposed authority-activation-authorization nonce.
            authority_activation_authorization_nonce: request.authority_activation_authorization_nonce,
            // Exact future-executor preconditions.
            ...
            authority_activation_authorization_id: String::new(),
            request_id: String::new(),
            authority_activation_authorization_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuthorityActivationAuthorization {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            authority_activation_authorization_nonce: request.authority_activation_authorization_nonce,
        };
        let authority_activation_authorization_id = production_live_epoch_transition_authority_activation_authorization_id(
            self.config.protocol_version.0,
            &decision.post_completion_attestation_digest,
            &inputs.authority_activation_authorization_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_authority_activation_authorization_request_id(
            self.config.protocol_version.0,
            &decision.post_completion_attestation_digest,
            &inputs.authority_activation_authorization_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.authority_activation_authorization_digest = record.content_digest();
        record.authority_activation_authorization_id = authority_activation_authorization_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_authority_activation_authorization_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.authority_activation_authorization_digest,
                outcome.tag(),
            );
    ...
    /// Run 345 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_authority_activation_authorization<
        R: LiveEpochTransitionAuthorityActivationAuthorizationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
                outcome,
                authority_activation_authorization_id: pkg.authority_activation_authorization_id.clone(),
                request_id: pkg.request_id.clone(),
                authority_activation_authorization_artifact: record.clone(),
                authority_activation_authorization_digest: pkg.authority_activation_authorization_digest.clone(),
        ...
        let post_completion_attestation_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource::AcceptedPostCompletionAttestationWithoutPackage {
                decision,
            } => decision.post_completion_attestation_digest.clone(),
            _ => String::new(),
        };

        let authority_activation_authorization_id = production_live_epoch_transition_authority_activation_authorization_id(
            self.config.protocol_version.0,
            &post_completion_attestation_intent_digest,
            &inputs.authority_activation_authorization_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_authorization_nonce,
        );
        ...
        let authority_activation_authorization_digest = String::new();
        ...
        ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision {
            ...
            request_id,
            authority_activation_authorization_artifact: None,
            authority_activation_authorization_digest,
            transcript_digest,
        }
    }

    /// Run 345 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_authority_activation_authorization_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact>,
        current: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
    ) -> ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome {
        use ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::Disabled
        {
...
/// Run 345 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_authority_activation_authorization_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::default()
        == ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig::default().kind
            == ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind::Disabled
}

/// Run 345 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 346).
pub fn production_live_epoch_transition_authority_activation_authorization_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 345 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_authority_activation_authorization_executor_mainnet_refused() -> bool {
    true
}

/// Run 345 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_authority_activation_authorization_executor_is_non_mutating() -> bool {
    true
}

/// Run 345 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_authority_activation_authorization_executor_never_falls_back() -> bool {
    true
}

/// Run 345 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_authority_activation_authorization_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 345 — the executor only requires a verified Run 343/344 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_authority_activation_authorization_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 345 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 345. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuthorityActivationAuthorizationFixtureState {
...
impl LiveEpochTransitionAuthorityActivationAuthorizationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, authority_activation_authorization_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == authority_activation_authorization_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `authority_activation_authorization_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
        authority_activation_authorization_id: &str,
    ) -> bool {
        if self.has_applied(authority_activation_authorization_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(authority_activation_authorization_id.to_string());
        true
    }
}