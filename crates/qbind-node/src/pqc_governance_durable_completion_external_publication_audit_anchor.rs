//! Run 290 — source/test durable-completion **external-publication-audit-seal
//! consumer / external-publication-audit-anchor interface boundary**.
//!
//! Source/test only. Run 290 captures **no** release-binary evidence and enables
...
//! external-publication-audit-seal backend, a real settlement-finality projection
//! backend, a real audit-ledger acknowledgement, a real external-publication
//! external_publication_audit_anchor, a real external-publication system, a real production attestation /
...
//!
//! Run 288
//! ([`crate::pqc_governance_durable_completion_external_publication_audit_seal`])
...
//!
//! Run 290 defines the **first typed interface** a future production settlement-receipt
...
//! **after** the Run 288 settlement-confirmation stage produced
//! `ExternalPublicationAuditSealRecorded`. It is an **interface / projection boundary only**:
//! production / MainNet / external external-publication-audit-seal implementations
...
//!
//! The external-publication-audit-seal layer is a **model only**. It does not
//! implement a real settlement, a real settlement finality, a real settlement receipt, a
//! real external-publication-audit-seal, a real settlement-finality projection, a
...
//! fixture external-publication-audit-seal sink mutates only the in-memory
...
//! 2. **legacy bypass** — a
//!    [`DurableCompletionExternalPublicationAuditAnchorPolicy::Disabled`] policy preserves
//!    the legacy no-external-publication-audit-anchor bypass and never invokes the
//!    external-publication-audit-seal sink;
//! 3. **settlement-confirmation-outcome projection** — only
//!    [`DurableCompletionExternalPublicationAuditSealOutcome::ExternalPublicationAuditSealRecorded`]
//!    creates a external-publication-audit-seal request; every other Run 288 outcome
...
//!    and Run 288 settlement-confirmation digest sets) must match expectations *before*
//!    the external-publication-audit-seal sink is invoked; a mismatch fails closed
//!    and leaves the sink invocation count at zero;
//! 5. **external-publication-audit-seal record** — only after every prior gate
//!    passes is the sink invoked; the record-identity fields must match exactly before
//!    any modeled external-publication-audit-seal record is recorded;
//! 6. **external-publication-audit-seal authorization** — only
//!    [`DurableCompletionExternalPublicationAuditAnchorOutcome::ExternalPublicationAuditAnchorRecorded`]
//!    authorizes a new modeled external-publication-audit-seal / settlement-finality
//!    projection state.
//!
//! A external-publication-audit-seal record failure, rollback, rollback failure, or
//! ambiguous window never retroactively claims a durable settlement-receipt
//! acknowledgement record. A duplicate identical external-publication-audit-seal
//! record is idempotent; the same record id with a different digest fails closed as
//! equivocation and records no second record. A Run 288
//! [`DurableCompletionExternalPublicationAuditSealOutcome::ExternalPublicationAuditSealDuplicateIdempotent`]
//! never creates a new external-publication-audit-seal record by itself — it can only
//! match an already-recorded external-publication-audit-seal record.

use crate::pqc_governance_durable_completion_acknowledgement_consumer::DurableCompletionAcknowledgementConsumerOutcome;
use crate::pqc_governance_durable_completion_external_publication_audit_seal::DurableCompletionExternalPublicationAuditSealOutcome;
...
/// Run 290 — the validation / mutation surface pair the receipt binds to.
pub type DurableCompletionExternalPublicationAuditAnchorSurface =
    ModeledGovernanceTrustMutationSurface;

/// Run 290 — the trust-domain environment binding the receipt is bound to.
pub type DurableCompletionExternalPublicationAuditAnchorEnvironment =
    ModeledGovernanceTrustMutationEnvironmentBinding;

/// Run 290 — the runtime binding (governance + mutation surface + sequence) the
/// receipt is bound to.
pub type DurableCompletionExternalPublicationAuditAnchorBinding =
    ModeledGovernanceTrustMutationRuntimeBinding;

/// Run 290 — the Run 240/246 durable replay observation carried as freshness
/// context.
pub type DurableCompletionExternalPublicationAuditAnchorReplayBinding = DurableReplayObservation;

/// Run 290 — the Run 246 pipeline outcome carried as consume authorization context.
pub type DurableCompletionExternalPublicationAuditAnchorPipelineBinding =
    GovernanceModeledEndToEndPipelineOutcome;

/// Run 290 — the Run 248 sink outcome carried as receipt-record context.
pub type DurableCompletionExternalPublicationAuditAnchorSinkBinding =
    GovernanceModeledDurableConsumeSinkOutcome;

/// Run 290 — the Run 250 reporter outcome carried as completion-report context.
pub type DurableCompletionExternalPublicationAuditAnchorReporterBinding =
    GovernanceModeledDurableConsumeCompletionReporterOutcome;

/// Run 290 — the Run 252 external_publication_audit_anchor outcome carried as external_publication_audit_anchor context.
pub type DurableCompletionExternalPublicationAuditAnchorFinalizationBinding =
    GovernanceModeledDurableCompletionFinalizationOutcome;

/// Run 290 — the Run 254 attestation outcome carried as attestation context.
pub type DurableCompletionExternalPublicationAuditAnchorAttestationBinding =
    GovernanceModeledDurableCompletionAttestationOutcome;

/// Run 290 — the Run 256 backend outcome carried as backend-record context. The
/// acknowledgement boundary never reimplements the backend; it only carries its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAuditAnchorBackendBinding =
    DurableCompletionAttestationBackendOutcome;

/// Run 290 — the Run 258 audit/publication receipt outcome carried as
/// receipt-record context. The consumer boundary never reimplements the receipt; it
/// only carries its terminal outcome.
pub type DurableCompletionExternalPublicationAuditAnchorReceiptBinding =
    DurableCompletionAuditPublicationReceiptOutcome;

/// Run 290 — the Run 260 audit-receipt acknowledgement outcome carried as
...
pub type DurableCompletionExternalPublicationAuditAnchorAcknowledgementBinding =
    DurableCompletionAuditReceiptAcknowledgementOutcome;

/// Run 290 — the Run 262 acknowledgement consumer outcome the settlement-confirmation
...
pub type DurableCompletionExternalPublicationAuditAnchorConsumerBinding =
    DurableCompletionAcknowledgementConsumerOutcome;

/// Run 290 — the Run 288 settlement-confirmation outcome the external-publication-audit-anchor
/// boundary projects to a external-publication-audit-anchor request. The external-publication-audit-anchor
/// boundary never reimplements the settlement confirmation; it only projects its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAuditAnchorExternalPublicationAuditSealBinding =
...
/// Run 290 — the typed durable-completion audit/publication receipt kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditAnchorKind {
    ...
    /// fail-closed).
    ProductionExternalPublicationAuditAnchorUnavailable,
    /// MainNet audit-ledger receipt sink (reachable-but-unavailable / fail-closed).
    MainNetExternalPublicationAuditAnchorUnavailable,
    /// External-publication receipt sink (reachable-but-unavailable / fail-closed).
    ExternalExternalPublicationAuditAnchorUnavailable,
    /// An unknown receipt kind — fails closed.
    Unknown,
}

impl DurableCompletionExternalPublicationAuditAnchorKind {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureInMemory => "fixture-in-memory",
            Self::ProductionExternalPublicationAuditAnchorUnavailable => {
                "production-external-publication-audit-anchor-unavailable"
            }
            Self::MainNetExternalPublicationAuditAnchorUnavailable => {
                "mainnet-external-publication-audit-anchor-unavailable"
            }
            Self::ExternalExternalPublicationAuditAnchorUnavailable => {
                "external-external-publication-audit-anchor-unavailable"
            }
...
/// Run 290 — the typed durable-completion audit/publication receipt policy
/// selector.
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditAnchorPolicy {
    /// The receipt boundary is disabled (legacy bypass).
    Disabled,
    ...
    /// unavailable.
    ProductionExternalPublicationAuditAnchorRequired,
    /// A real MainNet audit-ledger receipt sink is required — reachable but
    /// unavailable.
    MainNetExternalPublicationAuditAnchorRequired,
    /// A real external-publication receipt sink is required — reachable but
    /// unavailable.
    ExternalExternalPublicationAuditAnchorRequired,
}

impl DurableCompletionExternalPublicationAuditAnchorPolicy {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureAllowed => "fixture-allowed",
            Self::ProductionExternalPublicationAuditAnchorRequired => {
                "production-external-publication-audit-anchor-required"
            }
            Self::MainNetExternalPublicationAuditAnchorRequired => "mainnet-external-publication-audit-anchor-required",
            Self::ExternalExternalPublicationAuditAnchorRequired => "external-external-publication-audit-anchor-required",
        }
    }
...
/// Run 290 — the typed receipt identity a receipt request is bound to.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditAnchorIdentity {
    /// Stable receipt id.
    pub confirmation_id: String,
    /// The receipt kind.
    pub kind: DurableCompletionExternalPublicationAuditAnchorKind,
    /// The receipt policy.
    pub policy: DurableCompletionExternalPublicationAuditAnchorPolicy,
    /// The domain separation tag the receipt operates under.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAuditAnchorIdentity {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.confirmation_id.is_empty()
            ...
            && self.kind != DurableCompletionExternalPublicationAuditAnchorKind::Unknown
    }

    /// The deterministic, domain-separated receipt identity digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditAnchorDigest {
        external_publication_audit_anchor_identity_digest(self)
    }
}
...
/// Run 290 — domain separator for the receipt identity digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_IDENTITY_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-anchor-identity:v1";
/// Run 290 — domain separator for the receipt request digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-anchor-request:v1";
/// Run 290 — domain separator for the receipt response digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-anchor-response:v1";
/// Run 290 — domain separator for the receipt record digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-anchor-record:v1";
/// Run 290 — domain separator for the receipt transcript digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_TRANSCRIPT_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-anchor-transcript:v1";

/// Run 290 — a deterministic, domain-separated receipt digest (lowercase hex of a
/// SHA3-256 over length-prefixed, field-bound canonical material). It contains no
/// unstable display text.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditAnchorDigest(String);

impl DurableCompletionExternalPublicationAuditAnchorDigest {
...
/// Run 290 — a deterministic, domain-separated receipt transcript digest.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditAnchorTranscriptDigest(String);

impl DurableCompletionExternalPublicationAuditAnchorTranscriptDigest {
    /// The lowercase hex representation.
    pub fn as_hex(&self) -> &str {
        &self.0
    }
...
/// Run 290 — deterministic, domain-separated receipt identity digest.
pub fn external_publication_audit_anchor_identity_digest(
    identity: &DurableCompletionExternalPublicationAuditAnchorIdentity,
) -> DurableCompletionExternalPublicationAuditAnchorDigest {
    ...
    DurableCompletionExternalPublicationAuditAnchorDigest(w.finish())
}

/// Run 290 — deterministic, domain-separated receipt request digest. Binds every
/// request field including the full Run 256 backend digest binding.
pub fn external_publication_audit_anchor_request_digest(
    request: &DurableCompletionExternalPublicationAuditAnchorRequest,
) -> DurableCompletionExternalPublicationAuditAnchorDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN);
    w.str_field(&request.external_publication_audit_anchor_record_id)
        ...
        .str_field(&request.external_publication_audit_seal_identity_digest)
        .str_field(&request.external_publication_audit_seal_request_digest)
        .str_field(&request.external_publication_audit_seal_response_digest)
        .str_field(&request.external_publication_audit_seal_record_digest)
        .str_field(&request.external_publication_audit_seal_transcript_digest)
        .str_field(&request.external_publication_audit_seal_record_id)
        .str_field(&request.domain_separation_tag)
        .str_field(external_publication_audit_anchor_identity_digest(&request.identity).as_hex());
    DurableCompletionExternalPublicationAuditAnchorDigest(w.finish())
}

/// Run 290 — deterministic, domain-separated receipt response digest. Binds the
/// receipt record id, the request digest it answers, the acceptance flag, and the
/// responding receipt kind.
pub fn external_publication_audit_anchor_response_digest(
    response: &DurableCompletionExternalPublicationAuditAnchorResponse,
) -> DurableCompletionExternalPublicationAuditAnchorDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN);
    w.str_field(&response.external_publication_audit_anchor_record_id)
        ...
        .str_field(response.external_publication_audit_anchor_kind.tag());
    DurableCompletionExternalPublicationAuditAnchorDigest(w.finish())
}

/// Run 290 — deterministic, domain-separated receipt record digest. Binds the
/// receipt record id, the request digest, and the receipt identity digest.
pub fn external_publication_audit_anchor_record_digest(
    record: &DurableCompletionExternalPublicationAuditAnchorRecord,
) -> DurableCompletionExternalPublicationAuditAnchorDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN);
    w.str_field(&record.external_publication_audit_anchor_record_id)
        .str_field(record.request_digest.as_hex())
        .str_field(record.identity_digest.as_hex());
    DurableCompletionExternalPublicationAuditAnchorDigest(w.finish())
}

/// Run 290 — deterministic, domain-separated receipt transcript digest. Binds the
/// request, response, and record digests into a single transcript binding.
pub fn external_publication_audit_anchor_transcript_digest(
    request_digest: &DurableCompletionExternalPublicationAuditAnchorDigest,
    response_digest: &DurableCompletionExternalPublicationAuditAnchorDigest,
    record_digest: &DurableCompletionExternalPublicationAuditAnchorDigest,
) -> DurableCompletionExternalPublicationAuditAnchorTranscriptDigest {
    ...
    DurableCompletionExternalPublicationAuditAnchorTranscriptDigest(w.finish())
}
...
/// Run 290 — the typed receipt request a future production audit ledger or
...
/// attestation / Run 252 external_publication_audit_anchor / Run 250 completion report / Run 248
/// receipt / Run 246 decision material — never a copy of any wire payload and never
/// a production durable record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorRequest {
    /// Stable receipt record id (the idempotency key of the receipt).
    pub external_publication_audit_anchor_record_id: String,
    ...
    /// Run 252 external_publication_audit_anchor decision digest.
    ...
    /// Run 288 settlement-confirmation identity digest.
    pub external_publication_audit_seal_identity_digest: String,
    /// Run 288 settlement-confirmation request digest.
    pub external_publication_audit_seal_request_digest: String,
    /// Run 288 settlement-confirmation response digest.
    pub external_publication_audit_seal_response_digest: String,
    /// Run 288 settlement-confirmation record digest.
    pub external_publication_audit_seal_record_digest: String,
    /// Run 288 settlement-confirmation transcript digest.
    pub external_publication_audit_seal_transcript_digest: String,
    /// Run 288 settlement-confirmation record id.
    pub external_publication_audit_seal_record_id: String,
    /// External-publication-confirmation identity.
    pub identity: DurableCompletionExternalPublicationAuditAnchorIdentity,
    /// Domain separation tag.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAuditAnchorRequest {
    /// `true` iff every mandatory field is structurally present (non-empty) and the
    /// identity is well-formed.
    pub fn is_well_formed(&self) -> bool {
        !self.external_publication_audit_anchor_record_id.is_empty()
            ...
            && !self.external_publication_audit_seal_identity_digest.is_empty()
            && !self.external_publication_audit_seal_request_digest.is_empty()
            && !self.external_publication_audit_seal_response_digest.is_empty()
            && !self.external_publication_audit_seal_record_digest.is_empty()
            && !self.external_publication_audit_seal_transcript_digest.is_empty()
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditAnchorDigest {
        external_publication_audit_anchor_request_digest(self)
    }

    /// The canonical immutable record derived from this request.
    pub fn to_record(&self) -> DurableCompletionExternalPublicationAuditAnchorRecord {
        DurableCompletionExternalPublicationAuditAnchorRecord {
            external_publication_audit_anchor_record_id: self.external_publication_audit_anchor_record_id.clone(),
...
/// Run 290 — the typed receipt response a receipt sink returns for a request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorResponse {
    /// The receipt record id the response answers.
    pub external_publication_audit_anchor_record_id: String,
    /// The request digest the response answers.
    pub request_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
    /// `true` iff the receipt sink accepted the request.
    pub accepted: bool,
    /// The responding receipt kind.
    pub external_publication_audit_anchor_kind: DurableCompletionExternalPublicationAuditAnchorKind,
}

impl DurableCompletionExternalPublicationAuditAnchorResponse {
    /// `true` iff the response is structurally well-formed.
    pub fn is_well_formed(&self) -> bool {
        !self.external_publication_audit_anchor_record_id.is_empty()
            && self.external_publication_audit_anchor_kind != DurableCompletionExternalPublicationAuditAnchorKind::Unknown
    }

    /// The deterministic receipt response digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditAnchorDigest {
        external_publication_audit_anchor_response_digest(self)
    }
}

/// Run 290 — the canonical immutable receipt record derived from a request. Two
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditAnchorRecord {
    /// The receipt record id.
    pub external_publication_audit_anchor_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
    /// The receipt identity digest.
    pub identity_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
}

impl DurableCompletionExternalPublicationAuditAnchorRecord {
    /// The deterministic receipt record digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditAnchorDigest {
        external_publication_audit_anchor_record_digest(self)
    }
}
...
/// Run 290 — the recorded status of a modeled audit/publication receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditAnchorLedgerStatus {
    /// The modeled receipt is recorded in the in-memory fixture ledger.
    Recorded,
}

/// Run 290 — a single modeled receipt record held in the in-memory fixture ledger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorLedgerRecord {
    /// The receipt record id (stable identity of the receipt).
    pub external_publication_audit_anchor_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
    /// The response digest.
    pub response_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
    /// The record digest.
    pub record_digest: DurableCompletionExternalPublicationAuditAnchorDigest,
    /// The transcript digest.
    pub transcript_digest: DurableCompletionExternalPublicationAuditAnchorTranscriptDigest,
    /// The recorded status.
    pub status: DurableCompletionExternalPublicationAuditAnchorLedgerStatus,
}

/// Run 290 — an immutable snapshot of the modeled receipt ledger used to model a
/// fixture rollback.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorLedgerSnapshot {
    records: Vec<DurableCompletionExternalPublicationAuditAnchorLedgerRecord>,
}

impl DurableCompletionExternalPublicationAuditAnchorLedgerSnapshot {
...
/// Run 290 — the modeled in-memory receipt ledger.
///
...
pub struct DurableCompletionExternalPublicationAuditAnchorLedger {
    records: Vec<DurableCompletionExternalPublicationAuditAnchorLedgerRecord>,
}

impl DurableCompletionExternalPublicationAuditAnchorLedger {
    ...
    pub fn records(&self) -> &[DurableCompletionExternalPublicationAuditAnchorLedgerRecord] {
        &self.records
    }

    /// The record for `external_publication_audit_anchor_record_id`, if present.
    pub fn find(
        &self,
        external_publication_audit_anchor_record_id: &str,
    ) -> Option<&DurableCompletionExternalPublicationAuditAnchorLedgerRecord> {
        self.records
            .iter()
            .find(|r| r.external_publication_audit_anchor_record_id == external_publication_audit_anchor_record_id)
    }

    /// `true` iff a receipt with `external_publication_audit_anchor_record_id` is recorded.
    pub fn contains(&self, external_publication_audit_anchor_record_id: &str) -> bool {
        self.find(external_publication_audit_anchor_record_id).is_some()
    }

    /// Capture an immutable snapshot for a modeled rollback.
    pub fn snapshot(&self) -> DurableCompletionExternalPublicationAuditAnchorLedgerSnapshot {
        DurableCompletionExternalPublicationAuditAnchorLedgerSnapshot {
    ...
    pub fn restore(
        &mut self,
        snapshot: &DurableCompletionExternalPublicationAuditAnchorLedgerSnapshot,
    ) {
    ...
    fn insert(&mut self, record: DurableCompletionExternalPublicationAuditAnchorLedgerRecord) {
...
/// Run 290 — the canonical binding a [`DurableCompletionExternalPublicationAuditAnchorInput`]
/// is checked against.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorExpectations {
    /// Expected receipt record id.
    pub expected_external_publication_audit_anchor_record_id: String,
    ...
    /// Expected Run 252 external_publication_audit_anchor decision digest.
    ...
    /// Expected Run 288 settlement-confirmation identity digest.
    pub expected_external_publication_audit_seal_identity_digest: String,
    /// Expected Run 288 settlement-confirmation request digest.
    pub expected_external_publication_audit_seal_request_digest: String,
    /// Expected Run 288 settlement-confirmation response digest.
    pub expected_external_publication_audit_seal_response_digest: String,
    /// Expected Run 288 settlement-confirmation record digest.
    pub expected_external_publication_audit_seal_record_digest: String,
    /// Expected Run 288 settlement-confirmation transcript digest.
    pub expected_external_publication_audit_seal_transcript_digest: String,
    /// Expected Run 288 settlement-confirmation record id.
    pub expected_external_publication_audit_seal_record_id: String,
    /// Expected settlement-confirmation identity.
    pub expected_identity: DurableCompletionExternalPublicationAuditAnchorIdentity,
    /// Expected settlement-confirmation kind.
    pub expected_external_publication_audit_anchor_kind: DurableCompletionExternalPublicationAuditAnchorKind,
    /// Expected settlement-confirmation policy.
    pub expected_external_publication_audit_anchor_policy: DurableCompletionExternalPublicationAuditAnchorPolicy,
    /// Expected domain separation tag.
    pub expected_domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAuditAnchorExpectations {
    /// Internal: the first environment / surface binding mismatch reason, if any.
    fn binding_mismatch_reason(
        &self,
        input: &DurableCompletionExternalPublicationAuditAnchorInput,
    ) -> Option<&'static str> {
    ...
    fn request_mismatch_reason(
        ...
        if request.external_publication_audit_anchor_record_id != self.expected_external_publication_audit_anchor_record_id {
        ...
        if request.finalization_decision_digest != self.expected_finalization_decision_digest {
            return Some("wrong external_publication_audit_anchor decision digest");
        }
        ...
        if request.external_publication_audit_seal_identity_digest
            != self.expected_external_publication_audit_seal_identity_digest
        {
            return Some("wrong settlement-confirmation identity digest");
        }
        if request.external_publication_audit_seal_request_digest
            != self.expected_external_publication_audit_seal_request_digest
        {
            return Some("wrong settlement-confirmation request digest");
        }
        if request.external_publication_audit_seal_response_digest
            != self.expected_external_publication_audit_seal_response_digest
        {
            return Some("wrong settlement-confirmation response digest");
        }
        if request.external_publication_audit_seal_record_digest
            != self.expected_external_publication_audit_seal_record_digest
        {
            return Some("wrong settlement-confirmation record digest");
        }
        if request.external_publication_audit_seal_transcript_digest
            != self.expected_external_publication_audit_seal_transcript_digest
        {
        ...
        if request.identity != self.expected_identity {
            return Some("wrong external-publication-audit-anchor identity");
        }
        if request.identity.kind != self.expected_external_publication_audit_anchor_kind {
            return Some("wrong external-publication-audit-anchor kind");
        }
        if request.identity.policy != self.expected_external_publication_audit_anchor_policy {
            return Some("wrong external-publication-audit-anchor policy");
        }
        None
    }
...
/// Run 290 — typed inputs for one modeled durable-completion audit/publication
/// receipt round-trip.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditAnchorInput {
    /// The receipt policy selector.
    pub policy: DurableCompletionExternalPublicationAuditAnchorPolicy,
    /// The trust-domain environment binding.
    pub environment_binding: DurableCompletionExternalPublicationAuditAnchorEnvironment,
    /// The runtime binding (governance + mutation surface + sequence).
    pub runtime_binding: DurableCompletionExternalPublicationAuditAnchorBinding,
    /// The Run 240/246 durable replay observation the pipeline authorized consume
    /// under.
    pub replay_binding: DurableCompletionExternalPublicationAuditAnchorReplayBinding,
    /// The Run 246 pipeline outcome.
    pub pipeline_binding: DurableCompletionExternalPublicationAuditAnchorPipelineBinding,
    /// The Run 248 sink outcome.
    pub sink_binding: DurableCompletionExternalPublicationAuditAnchorSinkBinding,
    /// The Run 250 reporter outcome.
    pub reporter_binding: DurableCompletionExternalPublicationAuditAnchorReporterBinding,
    /// The Run 252 external_publication_audit_anchor outcome.
    pub confirmation_binding: DurableCompletionExternalPublicationAuditAnchorFinalizationBinding,
    /// The Run 254 attestation outcome.
    pub attestation_binding: DurableCompletionExternalPublicationAuditAnchorAttestationBinding,
    /// The Run 256 backend outcome carried as backend-record context.
    pub backend_binding: DurableCompletionExternalPublicationAuditAnchorBackendBinding,
    /// The Run 258 audit/publication receipt outcome carried as receipt-record
    /// context.
    pub receipt_binding: DurableCompletionExternalPublicationAuditAnchorReceiptBinding,
    /// The Run 260 audit-receipt acknowledgement outcome carried as
    /// acknowledgement-record context.
    pub acknowledgement_binding:
        DurableCompletionExternalPublicationAuditAnchorAcknowledgementBinding,
    ...
    pub consumer_binding: DurableCompletionExternalPublicationAuditAnchorConsumerBinding,
    /// The Run 288 settlement-confirmation outcome the external-publication-audit-anchor boundary
    /// projects to a external-publication-audit-anchor request.
    pub external_publication_audit_seal_binding:
        DurableCompletionExternalPublicationAuditAnchorExternalPublicationAuditSealBinding,
    /// The external-publication-audit-anchor request the call site would submit.
    pub request: DurableCompletionExternalPublicationAuditAnchorRequest,
}

impl DurableCompletionExternalPublicationAuditAnchorInput {
    ...
    pub fn surface(&self) -> DurableCompletionExternalPublicationAuditAnchorSurface {
    ...
    pub fn is_mainnet_peer_driven(&self) -> bool {
        (self.environment() == TrustBundleEnvironment::Mainnet && self.surface().is_peer_driven())
            ...
            || matches!(
                self.external_publication_audit_seal_binding,
                DurableCompletionExternalPublicationAuditSealOutcome::MainNetPeerDrivenApplyRefusedNoAuditSeal
            )
    }
}
...
/// Run 290 — the typed outcome of one modeled durable-completion audit/publication
/// receipt round-trip.
///
/// Only [`Self::ExternalPublicationAuditAnchorRecorded`] authorizes a **new** modeled
/// audit/publication-receipt state. A [`Self::ExternalPublicationAuditAnchorDuplicateIdempotent`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAuditAnchorOutcome {
    /// Legacy bypass — a disabled receipt policy preserved the legacy
    /// no-audit-receipt path. No receipt invocation.
    LegacyBypassNoExternalPublicationAuditAnchor,
    ...
    RejectedBeforeExternalPublicationAuditSealNoAuditAnchor,
    ...
    /// invocation.
    ExternalPublicationAuditSealDidNotRecordNoAuditAnchor,
    ...
    ExternalPublicationAuditAnchorRecorded,
    /// A duplicate identical receipt — idempotent; no second receipt recorded.
    ExternalPublicationAuditAnchorDuplicateIdempotent,
    ...
    /// receipt.
    ExternalPublicationAuditAnchorRejectedBeforeRecord,
    /// The receipt record failed. No receipt.
    ExternalPublicationAuditAnchorRecordFailedNoAuditAnchor,
    /// The receipt record was rolled back. No receipt.
    ExternalPublicationAuditAnchorRolledBackNoAuditAnchor,
    /// The receipt rollback itself failed — fatal / fail-closed. No receipt.
    ExternalPublicationAuditAnchorRollbackFailedFatalNoAuditAnchor,
    /// The after-record receipt window was ambiguous — fails closed. No receipt.
    ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor,
    /// The production audit-ledger path was reached but is unavailable. No receipt.
    ProductionExternalPublicationAuditAnchorUnavailableNoAuditAnchor,
    /// The MainNet audit-ledger path was reached but is unavailable. No receipt.
    MainNetExternalPublicationAuditAnchorUnavailableNoAuditAnchor,
    /// The external-publication path was reached but is unavailable. No receipt.
    ExternalExternalPublicationAuditAnchorUnavailableNoAuditAnchor,
    ...
    MainNetPeerDrivenApplyRefusedNoAuditAnchor,
    /// Validator-set rotation is unsupported. No receipt.
    ValidatorSetRotationUnsupportedNoAuditAnchor,
    /// Policy-change actions are unsupported. No receipt.
    PolicyChangeUnsupportedNoAuditAnchor,
}

impl DurableCompletionExternalPublicationAuditAnchorOutcome {
    /// `true` iff this outcome authorizes a **new** modeled audit/publication
    /// receipt (only [`Self::ExternalPublicationAuditAnchorRecorded`]).
    pub fn authorizes_record(&self) -> bool {
        matches!(self, Self::ExternalPublicationAuditAnchorRecorded)
    }
    ...
    pub fn projects_to_recorded(&self) -> bool {
        matches!(
            self,
            Self::ExternalPublicationAuditAnchorRecorded | Self::ExternalPublicationAuditAnchorDuplicateIdempotent
        )
    }
    ...
    pub fn is_legacy_bypass(&self) -> bool {
        matches!(self, Self::LegacyBypassNoExternalPublicationAuditAnchor)
    }

    /// `true` iff this is the MainNet peer-driven-apply refusal.
    pub fn is_mainnet_peer_driven_apply_refused(&self) -> bool {
        matches!(self, Self::MainNetPeerDrivenApplyRefusedNoAuditAnchor)
    }

    /// Stable operator-facing tag.
    pub fn tag(&self) -> &'static str {
        match self {
            Self::LegacyBypassNoExternalPublicationAuditAnchor => "legacy-bypass-no-external-publication-audit-anchor",
            Self::RejectedBeforeExternalPublicationAuditSealNoAuditAnchor => {
                "rejected-before-settlement-confirmation-no-outcome-publication"
            }
            Self::ExternalPublicationAuditSealDidNotRecordNoAuditAnchor => {
                "settlement-confirmation-did-not-record-no-outcome-publication"
            }
            Self::ExternalPublicationAuditAnchorRecorded => "external-publication-audit-anchor-recorded",
            Self::ExternalPublicationAuditAnchorDuplicateIdempotent => {
                "external-publication-audit-anchor-duplicate-idempotent"
            }
            Self::ExternalPublicationAuditAnchorRejectedBeforeRecord => {
                "external-publication-audit-anchor-rejected-before-record"
            }
            Self::ExternalPublicationAuditAnchorRecordFailedNoAuditAnchor => {
                "external-publication-audit-anchor-record-failed-no-outcome-publication"
            }
            Self::ExternalPublicationAuditAnchorRolledBackNoAuditAnchor => {
                "external-publication-audit-anchor-rolled-back-no-outcome-publication"
            }
            Self::ExternalPublicationAuditAnchorRollbackFailedFatalNoAuditAnchor => {
                "external-publication-audit-anchor-rollback-failed-fatal-no-outcome-publication"
            }
            Self::ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor => {
                "external-publication-audit-anchor-ambiguous-fail-closed-no-outcome-publication"
            }
            Self::ProductionExternalPublicationAuditAnchorUnavailableNoAuditAnchor => {
                "production-external-publication-audit-anchor-unavailable-no-outcome-publication"
            }
            Self::MainNetExternalPublicationAuditAnchorUnavailableNoAuditAnchor => {
                "mainnet-external-publication-audit-anchor-unavailable-no-outcome-publication"
            }
            Self::ExternalExternalPublicationAuditAnchorUnavailableNoAuditAnchor => {
                "external-external-publication-audit-anchor-unavailable-no-outcome-publication"
            }
            Self::MainNetPeerDrivenApplyRefusedNoAuditAnchor => {
                "mainnet-peer-driven-apply-refused-no-outcome-publication"
            }
            Self::ValidatorSetRotationUnsupportedNoAuditAnchor => {
                "validator-set-rotation-unsupported-no-outcome-publication"
            }
            Self::PolicyChangeUnsupportedNoAuditAnchor => "policy-change-unsupported-no-outcome-publication",
        }
    }
}
...
/// Run 290 — the typed confirmation of a Run 262 acknowledgement consumer outcome
/// onto a settlement-confirmation request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAuditAnchorRequestIntent {
    ...
    NoReceipt(DurableCompletionExternalPublicationAuditAnchorOutcome),
}

impl DurableCompletionExternalPublicationAuditAnchorRequestIntent {
...
/// Run 290 — project a Run 288 settlement-confirmation outcome onto a
/// external-publication-audit-anchor request.
///
/// Only
/// [`DurableCompletionExternalPublicationAuditSealOutcome::ExternalPublicationAuditSealRecorded`]
/// creates a external-publication-audit-anchor request.
/// [`DurableCompletionExternalPublicationAuditSealOutcome::ExternalPublicationAuditSealDuplicateIdempotent`]
/// may only match an already-recorded external-publication-audit-anchor record and never creates
...
/// [`DurableCompletionExternalPublicationAuditAnchorOutcome::ExternalPublicationAuditSealDidNotRecordNoAuditAnchor`]).
/// Pure: performs no work and never records.
pub fn project_external_publication_audit_seal_outcome_to_external_publication_audit_anchor_request(
    outcome: &DurableCompletionExternalPublicationAuditAnchorExternalPublicationAuditSealBinding,
    ...
    match outcome {
        Finalization::ExternalPublicationAuditSealRecorded => Intent::CreateRequest,
        Finalization::ExternalPublicationAuditSealDuplicateIdempotent => Intent::IdempotentOnly,
        Finalization::LegacyBypassNoExternalPublicationAuditSeal => {
            Intent::NoReceipt(Receipt::LegacyBypassNoExternalPublicationAuditAnchor)
        }
        Finalization::RejectedBeforeExternalPublicationAuditArchiveNoAuditSeal => {
            Intent::NoReceipt(Receipt::RejectedBeforeExternalPublicationAuditSealNoAuditAnchor)
        }
        Finalization::MainNetPeerDrivenApplyRefusedNoAuditSeal => {
            Intent::NoReceipt(Receipt::MainNetPeerDrivenApplyRefusedNoAuditAnchor)
        }
        Finalization::ValidatorSetRotationUnsupportedNoAuditSeal => {
            Intent::NoReceipt(Receipt::ValidatorSetRotationUnsupportedNoAuditAnchor)
        }
        Finalization::PolicyChangeUnsupportedNoAuditSeal => {
            Intent::NoReceipt(Receipt::PolicyChangeUnsupportedNoAuditAnchor)
        }
        ...
        // record, so no external-publication-audit-anchor record may exist.
        _ => Intent::NoReceipt(Receipt::ExternalPublicationAuditSealDidNotRecordNoAuditAnchor),
    }
}
...
/// Run 290 — a modeled fault the fixture receipt sink injects to exercise the
/// fail-closed record / rollback / ambiguous paths. Source/test only.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditAnchorFault {
...
/// Run 290 — the pure/mockable modeled durable-completion audit/publication receipt
/// boundary.
///
...
/// [`DurableCompletionExternalPublicationAuditAnchorLedger`].
pub trait GovernanceDurableCompletionExternalPublicationAuditAnchorSink {
    /// The receipt kind (used for typed recovery classification).
    fn kind(&self) -> DurableCompletionExternalPublicationAuditAnchorKind;
    ...
    fn project_durable_completion_external_publication_audit_anchor(
        &mut self,
        request: &DurableCompletionExternalPublicationAuditAnchorRequest,
        expectations: &DurableCompletionExternalPublicationAuditAnchorExpectations,
        idempotent_only: bool,
        ledger: &mut DurableCompletionExternalPublicationAuditAnchorLedger,
    ...
    fn recover_durable_completion_external_publication_audit_anchor_window(
        &self,
        input: &DurableCompletionExternalPublicationAuditAnchorInput,
        window: DurableCompletionExternalPublicationAuditAnchorWindow,
        recovered_record: Option<&DurableCompletionExternalPublicationAuditAnchorLedgerRecord>,
        expectations: &DurableCompletionExternalPublicationAuditAnchorExpectations,
    ) -> DurableCompletionExternalPublicationAuditAnchorOutcome {
...
/// Run 290 — the DevNet/TestNet in-memory fixture receipt sink.
///
/// Source-test only. It mutates only the in-memory
/// [`DurableCompletionExternalPublicationAuditAnchorLedger`] and exposes an invocation
/// counter so tests can prove non-submitting backend paths and pre-receipt
/// rejections never invoke it.
#[derive(Debug, Clone)]
pub struct FixtureDurableCompletionExternalPublicationAuditAnchorSink {
    fault: Option<DurableCompletionExternalPublicationAuditAnchorFault>,
    invocations: u32,
}

impl Default for FixtureDurableCompletionExternalPublicationAuditAnchorSink {
    fn default() -> Self {
        Self::new()
    }
}

impl FixtureDurableCompletionExternalPublicationAuditAnchorSink {
    ...
    pub fn with_fault(fault: DurableCompletionExternalPublicationAuditAnchorFault) -> Self {
        Self {
            fault: Some(fault),
            invocations: 0,
        }
...
impl GovernanceDurableCompletionExternalPublicationAuditAnchorSink
    for FixtureDurableCompletionExternalPublicationAuditAnchorSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditAnchorKind {
        DurableCompletionExternalPublicationAuditAnchorKind::FixtureInMemory
    }
    ...
    fn project_durable_completion_external_publication_audit_anchor(
        ...
        if let Some(fault) = self.fault {
            ...
            return match fault {
                DurableCompletionExternalPublicationAuditAnchorFault::RecordFailedNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditAnchorRecordFailedNoAuditAnchor
                }
                DurableCompletionExternalPublicationAuditAnchorFault::RolledBackNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditAnchorRolledBackNoAuditAnchor
                }
                DurableCompletionExternalPublicationAuditAnchorFault::RollbackFailedFatal => {
                    Receipt::ExternalPublicationAuditAnchorRollbackFailedFatalNoAuditAnchor
                }
                DurableCompletionExternalPublicationAuditAnchorFault::AmbiguousAfterRecord => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor
                }
            };
        }
        ...
        if !matches!(
            ...
            return Receipt::ExternalPublicationAuditAnchorRejectedBeforeRecord;
        }
        ...
        let response = DurableCompletionExternalPublicationAuditAnchorResponse {
            external_publication_audit_anchor_record_id: request.external_publication_audit_anchor_record_id.clone(),
            request_digest: request_digest.clone(),
            accepted: true,
            external_publication_audit_anchor_kind: DurableCompletionExternalPublicationAuditAnchorKind::FixtureInMemory,
        };
        let response_digest = response.digest();
        let record = request.to_record();
        let record_digest = record.digest();
        let transcript_digest = external_publication_audit_anchor_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
        );
        ...
        if let Some(existing) = ledger.find(&request.external_publication_audit_anchor_record_id) {
            ...
            {
                return Receipt::ExternalPublicationAuditAnchorDuplicateIdempotent;
            }
        ...
        ledger.insert(DurableCompletionExternalPublicationAuditAnchorLedgerRecord {
            external_publication_audit_anchor_record_id: request.external_publication_audit_anchor_record_id.clone(),
            request_digest,
            response_digest,
            record_digest,
            transcript_digest,
            status: DurableCompletionExternalPublicationAuditAnchorLedgerStatus::Recorded,
        });
        Receipt::ExternalPublicationAuditAnchorRecorded
    }
}
...
/// Run 290 — the production audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ProductionExternalPublicationAuditAnchorSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditAnchorSink
    for ProductionExternalPublicationAuditAnchorSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditAnchorKind {
        DurableCompletionExternalPublicationAuditAnchorKind::ProductionExternalPublicationAuditAnchorUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_anchor(
        &mut self,
        _request: &DurableCompletionExternalPublicationAuditAnchorRequest,
        _expectations: &DurableCompletionExternalPublicationAuditAnchorExpectations,
        _idempotent_only: bool,
        _ledger: &mut DurableCompletionExternalPublicationAuditAnchorLedger,
    ) -> DurableCompletionExternalPublicationAuditAnchorOutcome {
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditAnchorOutcome::ProductionExternalPublicationAuditAnchorUnavailableNoAuditAnchor
    }
}

/// Run 290 — the MainNet audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct MainNetExternalPublicationAuditAnchorSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditAnchorSink
    for MainNetExternalPublicationAuditAnchorSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditAnchorKind {
        DurableCompletionExternalPublicationAuditAnchorKind::MainNetExternalPublicationAuditAnchorUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_anchor(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditAnchorOutcome::MainNetExternalPublicationAuditAnchorUnavailableNoAuditAnchor
    }
}

/// Run 290 — the external-publication receipt sink. Reachable but unavailable /
/// fail-closed. It performs no external publication, records no receipt, and
/// mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ExternalExternalPublicationAuditAnchorSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditAnchorSink
    for ExternalExternalPublicationAuditAnchorSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditAnchorKind {
        DurableCompletionExternalPublicationAuditAnchorKind::ExternalExternalPublicationAuditAnchorUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_anchor(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditAnchorOutcome::ExternalExternalPublicationAuditAnchorUnavailableNoAuditAnchor
    }
}
...
/// Run 290 — evaluate one modeled durable-completion audit/publication receipt
/// round-trip.
///
...
/// 2. legacy bypass — a
///    [`DurableCompletionExternalPublicationAuditAnchorPolicy::Disabled`] policy;
...
pub fn evaluate_durable_completion_external_publication_audit_anchor<S>(
    input: &DurableCompletionExternalPublicationAuditAnchorInput,
    expectations: &DurableCompletionExternalPublicationAuditAnchorExpectations,
    sink: &mut S,
    ledger: &mut DurableCompletionExternalPublicationAuditAnchorLedger,
...
where
    S: GovernanceDurableCompletionExternalPublicationAuditAnchorSink,
{
    use DurableCompletionExternalPublicationAuditAnchorOutcome as Finalization;
    use DurableCompletionExternalPublicationAuditAnchorRequestIntent as Intent;
    ...
    if input.is_mainnet_peer_driven() {
        return Finalization::MainNetPeerDrivenApplyRefusedNoAuditAnchor;
    }
    ...
    if input.policy.is_disabled() {
        return Finalization::LegacyBypassNoExternalPublicationAuditAnchor;
    }
    ...
    let idempotent_only =
        match project_external_publication_audit_seal_outcome_to_external_publication_audit_anchor_request(
            &input.external_publication_audit_seal_binding,
        ) {
    ...
    if !expectations.binding_matches(input) {
        return Finalization::RejectedBeforeExternalPublicationAuditSealNoAuditAnchor;
    }

    // Step 5: invoke the settlement-confirmation sink to record the modeled confirmation.
    sink.project_durable_completion_external_publication_audit_anchor(
        &input.request,
        expectations,
        idempotent_only,
        ledger,
...
/// Run 290 — the modeled durable-completion audit/publication receipt
/// crash/recovery window.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditAnchorWindow {
    ...
    /// Crashed after the report record but before a external_publication_audit_anchor intent.
    AfterPublicationRecordBeforeReceiptIntent,
    /// Crashed after a external_publication_audit_anchor intent but before any external_publication_audit_anchor record.
    AfterReceiptIntentBeforeReceiptRecord,
    /// Crashed after the external_publication_audit_anchor record but before an attestation intent.
    ...
    AfterConsumerSuccessBeforeExternalPublicationAuditSealRequest,
    ...
    /// Crashed after settlement-confirmation success but before a external-publication-audit-anchor
    /// request.
    AfterExternalPublicationAuditSealSuccessBeforeExternalPublicationAuditAnchorRequest,
    /// Crashed after a external-publication-audit-anchor request but before any
    /// external-publication-audit-anchor record.
    AfterExternalPublicationAuditAnchorRequestBeforeExternalPublicationAuditAnchorRecord,
    /// Crashed after a external-publication-audit-anchor record but before external-publication-audit-anchor
    /// success — fails closed unless an explicit matching external-publication-audit-anchor
    /// record exists.
    AfterExternalPublicationAuditAnchorRecordBeforeExternalPublicationAuditAnchorSuccess,
    /// Recovered after a successful external-publication-audit-anchor record.
    AfterExternalPublicationAuditAnchorSuccess,
    /// Recovered after an ambiguous external-publication-audit-anchor window.
    AfterExternalPublicationAuditAnchorAmbiguous,
    /// The external-publication-audit-anchor record itself failed.
    ExternalPublicationAuditAnchorRecordFailed,
    /// The external-publication-audit-anchor record was rolled back.
    ExternalPublicationAuditAnchorRollbackCompleted,
    /// The external-publication-audit-anchor rollback itself failed — fatal.
    ExternalPublicationAuditAnchorRollbackFailed,
    /// An unknown window — fails closed.
    Unknown,
}

/// Run 290 — classify a modeled durable-completion audit/publication receipt
/// crash/recovery window.
///
...
/// [`DurableCompletionExternalPublicationAuditAnchorOutcome::ExternalPublicationAuditAnchorRecorded`]. Pure:
/// performs no modeled mutation and never invokes Run 070.
pub fn recover_durable_completion_external_publication_audit_anchor_window(
    input: &DurableCompletionExternalPublicationAuditAnchorInput,
    window: DurableCompletionExternalPublicationAuditAnchorWindow,
    kind: DurableCompletionExternalPublicationAuditAnchorKind,
    recovered_record: Option<&DurableCompletionExternalPublicationAuditAnchorLedgerRecord>,
    expectations: &DurableCompletionExternalPublicationAuditAnchorExpectations,
) -> DurableCompletionExternalPublicationAuditAnchorOutcome {
    use DurableCompletionExternalPublicationAuditAnchorOutcome as Receipt;
    use DurableCompletionExternalPublicationAuditAnchorWindow as Window;

    // MainNet peer-driven apply refusal precedes recovery classification.
    if input.is_mainnet_peer_driven() {
        return Receipt::MainNetPeerDrivenApplyRefusedNoAuditAnchor;
    }
    ...
    match kind {
        DurableCompletionExternalPublicationAuditAnchorKind::ProductionExternalPublicationAuditAnchorUnavailable => {
            return Receipt::ProductionExternalPublicationAuditAnchorUnavailableNoAuditAnchor;
        }
        DurableCompletionExternalPublicationAuditAnchorKind::MainNetExternalPublicationAuditAnchorUnavailable => {
            return Receipt::MainNetExternalPublicationAuditAnchorUnavailableNoAuditAnchor;
        }
        DurableCompletionExternalPublicationAuditAnchorKind::ExternalExternalPublicationAuditAnchorUnavailable => {
            return Receipt::ExternalExternalPublicationAuditAnchorUnavailableNoAuditAnchor;
        }
        DurableCompletionExternalPublicationAuditAnchorKind::Disabled => {
            return Receipt::LegacyBypassNoExternalPublicationAuditAnchor;
        }
        DurableCompletionExternalPublicationAuditAnchorKind::Unknown => {
            return Receipt::ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor;
        }
        DurableCompletionExternalPublicationAuditAnchorKind::FixtureInMemory => {}
    }
    ...
    let recovered_matches =
        |record: &DurableCompletionExternalPublicationAuditAnchorLedgerRecord| -> bool {
            record.external_publication_audit_anchor_record_id == expectations.expected_external_publication_audit_anchor_record_id
                && record.request_digest == input.request.digest()
                && record.status
                    == DurableCompletionExternalPublicationAuditAnchorLedgerStatus::Recorded
        };

    match window {
        // Through settlement-confirmation success but before a external-publication-audit-anchor
        // request there is nothing to record a settlement external_publication_audit_anchor for.
        ...
        | Window::AfterConsumerSuccessBeforeExternalPublicationAuditSealRequest
        ...
        | Window::AfterExternalPublicationAuditSealSuccessBeforeExternalPublicationAuditAnchorRequest => {
            Receipt::ExternalPublicationAuditSealDidNotRecordNoAuditAnchor
        }
        // A external-publication-audit-anchor request without a record never records a
        // settlement external_publication_audit_anchor.
        Window::AfterExternalPublicationAuditAnchorRequestBeforeExternalPublicationAuditAnchorRecord => {
            Receipt::ExternalPublicationAuditAnchorRejectedBeforeRecord
        }
        // After a external-publication-audit-anchor record but before success: fails closed
        // unless an explicit matching, well-formed external-publication-audit-anchor record
        // exists.
        Window::AfterExternalPublicationAuditAnchorRecordBeforeExternalPublicationAuditAnchorSuccess => {
            match recovered_record {
                Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAuditAnchorRecorded,
                _ => Receipt::ExternalPublicationAuditAnchorRejectedBeforeRecord,
            }
        }
        // An explicit successful settlement external_publication_audit_anchor recovers as recorded only if
        // it matches.
        Window::AfterExternalPublicationAuditAnchorSuccess => match recovered_record {
            Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAuditAnchorRecorded,
            _ => Receipt::ExternalPublicationAuditAnchorRejectedBeforeRecord,
        },
        Window::AfterExternalPublicationAuditAnchorAmbiguous => {
            Receipt::ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor
        }
        Window::ExternalPublicationAuditAnchorRecordFailed => {
            Receipt::ExternalPublicationAuditAnchorRecordFailedNoAuditAnchor
        }
        Window::ExternalPublicationAuditAnchorRollbackCompleted => {
            Receipt::ExternalPublicationAuditAnchorRolledBackNoAuditAnchor
        }
        Window::ExternalPublicationAuditAnchorRollbackFailed => {
            Receipt::ExternalPublicationAuditAnchorRollbackFailedFatalNoAuditAnchor
        }
        // Any unknown window fails closed.
        Window::Unknown => Receipt::ExternalPublicationAuditAnchorAmbiguousFailClosedNoAuditAnchor,
    }
}

/// Run 290 — `true` iff a receipt outcome authorizes a **new** modeled
/// audit/publication receipt (only
/// [`DurableCompletionExternalPublicationAuditAnchorOutcome::ExternalPublicationAuditAnchorRecorded`]).
pub fn external_publication_audit_anchor_outcome_authorizes_record(
    outcome: &DurableCompletionExternalPublicationAuditAnchorOutcome,
) -> bool {
    outcome.authorizes_record()
}

/// Run 290 — `true` iff a receipt outcome projects to a recorded audit/publication
/// receipt (a newly recorded receipt or an idempotent duplicate of an
/// already-recorded receipt).
pub fn external_publication_audit_anchor_outcome_projects_to_recorded(
    outcome: &DurableCompletionExternalPublicationAuditAnchorOutcome,
) -> bool {
...
/// Run 290 — a receipt rejection is non-mutating: it records no receipt, mutates no
/// `LivePqcTrustState`, and writes no durable state.
pub fn durable_completion_external_publication_audit_anchor_rejection_is_non_mutating() -> bool {
    true
}

/// Run 290 — the receipt boundary never calls Run 070.
pub fn durable_completion_external_publication_audit_anchor_never_calls_run_070() -> bool {
    true
}

/// Run 290 — the receipt boundary never mutates live PQC trust state.
pub fn durable_completion_external_publication_audit_anchor_never_mutates_live_pqc_trust_state() -> bool {
    true
}

/// Run 290 — the receipt boundary never writes a sequence or a marker.
pub fn durable_completion_external_publication_audit_anchor_never_writes_sequence_or_marker() -> bool {
    true
}

/// Run 290 — the receipt boundary changes no RocksDB file schema / migration.
pub fn durable_completion_external_publication_audit_anchor_no_rocksdb_file_schema_migration_change() -> bool {
    true
}

/// Run 290 — the receipt boundary performs no external publication.
pub fn durable_completion_external_publication_audit_anchor_no_external_publication() -> bool {
    true
}

/// Run 290 — the receipt boundary performs no real audit-ledger persistence.
pub fn durable_completion_external_publication_audit_anchor_no_real_audit_ledger() -> bool {
    true
}

/// Run 290 — a receipt requires a successful Run 246 pipeline outcome upstream.
pub fn durable_completion_external_publication_audit_anchor_pipeline_success_required() -> bool {
    true
}

/// Run 290 — a receipt requires a Run 248 sink receipt record upstream.
pub fn durable_completion_external_publication_audit_anchor_sink_receipt_required() -> bool {
    true
}

/// Run 290 — a receipt requires a Run 250 completion report upstream.
pub fn durable_completion_external_publication_audit_anchor_completion_report_required() -> bool {
    true
}

/// Run 290 — a receipt requires a Run 252 external_publication_audit_anchor upstream.
pub fn durable_completion_external_publication_audit_anchor_finalization_projection_required() -> bool {
    true
}

/// Run 290 — a receipt requires a Run 254 attestation upstream.
pub fn durable_completion_external_publication_audit_anchor_attestation_required() -> bool {
    true
}

/// Run 290 — an acknowledgement requires a Run 256 backend submission upstream.
pub fn durable_completion_external_publication_audit_anchor_backend_submission_required() -> bool {
    true
}

/// Run 290 — an acknowledgement requires a Run 258 audit receipt upstream.
pub fn durable_completion_external_publication_audit_anchor_receipt_required() -> bool {
    true
}

/// Run 290 — a consumer requires a Run 260 audit-receipt acknowledgement upstream.
pub fn durable_completion_external_publication_audit_anchor_consumer_required() -> bool {
    true
}

/// Run 290 — a settlement external_publication_audit_anchor requires a Run 288 settlement confirmation
/// upstream; no settlement external_publication_audit_anchor is authorized without a recorded
/// settlement confirmation.
pub fn durable_completion_external_publication_audit_anchor_confirmation_required() -> bool {
    true
}

/// Run 290 — the consumer boundary never performs a real settlement; production /
/// MainNet / external settlement consumers are reachable but fail closed.
pub fn durable_completion_external_publication_audit_anchor_no_real_settlement() -> bool {
    true
}

/// Run 290 — the external-publication-audit-anchor boundary never confers real settlement
/// finality; the only external-publication-audit-anchor record is a modeled in-memory fixture
/// record. Production / MainNet / external external-publication-audit-anchor sinks are reachable
/// but unavailable / fail closed and never confer any real finality.
pub fn durable_completion_external_publication_audit_anchor_no_real_settlement_finality() -> bool {
    true
}

/// Run 290 — the external-publication-audit-anchor boundary never emits a real settlement
/// receipt; the only external-publication-audit-anchor record is a modeled in-memory fixture
/// record with no external publication, network I/O, or persistent backend.
pub fn durable_completion_external_publication_audit_anchor_no_real_settlement_receipt() -> bool {
    true
}

/// Run 290 — the external-publication-audit-anchor boundary never confers a real
/// external-publication-audit-seal; the only external-publication-audit-seal
/// record is a modeled in-memory fixture record. Production / MainNet / external
/// external-publication-audit-seal sinks are reachable but unavailable / fail closed
/// and never confer any real acknowledgement.
pub fn durable_completion_external_publication_audit_anchor_no_real_external_publication_audit_seal() -> bool {
    true
}
...
/// backend.
pub fn durable_completion_external_publication_audit_anchor_no_real_settlement_finality_projection() -> bool {
    true
}

/// Run 290 — the external-publication-audit-anchor boundary never confers a real
/// external-publication-audit-seal; the only external-publication-audit-seal is a modeled
/// in-memory fixture record with no external publication, network I/O, or persistent
/// backend.
pub fn durable_completion_external_publication_audit_anchor_no_real_external_publication_audit_anchor() -> bool {
    true
}

/// Run 290 — a receipt record is required before a receipt is acknowledged.
pub fn durable_completion_external_publication_audit_anchor_record_required_before_reported() -> bool {
    true
}

/// Run 290 — a failed receipt record never records a receipt.
pub fn durable_completion_external_publication_audit_anchor_failed_record_never_records() -> bool {
    true
}

/// Run 290 — a rolled-back receipt record never records a receipt.
pub fn durable_completion_external_publication_audit_anchor_rollback_never_records() -> bool {
    true
}

/// Run 290 — an ambiguous after-record receipt window fails closed.
pub fn durable_completion_external_publication_audit_anchor_ambiguous_window_fails_closed() -> bool {
    true
}

/// Run 290 — MainNet peer-driven apply refusal precedes every receipt stage.
pub fn durable_completion_external_publication_audit_anchor_mainnet_peer_driven_apply_refused_first(
    environment: TrustBundleEnvironment,
) -> bool {
    environment == TrustBundleEnvironment::Mainnet
}

/// Run 290 — production / MainNet audit-ledger sinks are reachable but unavailable.
pub fn durable_completion_external_publication_audit_anchor_production_mainnet_unavailable() -> bool {
    true
}

/// Run 290 — the external-publication sink is reachable but unavailable.
pub fn durable_completion_external_publication_audit_anchor_external_unavailable() -> bool {
    true
}

/// Run 290 — validator-set rotation remains unsupported at the receipt boundary.
pub fn durable_completion_external_publication_audit_anchor_validator_set_rotation_unsupported() -> bool {
    true
}

/// Run 290 — policy-change actions remain unsupported at the receipt boundary.
pub fn durable_completion_external_publication_audit_anchor_policy_change_unsupported() -> bool {
    true
}

/// Run 290 — a local operator cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_audit_anchor_local_operator_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}

/// Run 290 — a peer majority cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_audit_anchor_peer_majority_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}