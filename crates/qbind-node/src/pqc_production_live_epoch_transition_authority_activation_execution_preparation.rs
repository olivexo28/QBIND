...
//! execution-preparation / final-execution preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 345/346 live epoch-transition authority-activation-authorization /
//! final-execution-readiness boundary: the boundary that consumes a
//! **verified** Run 345/346 non-mutating live epoch-transition
//! authority-activation-authorization *decision* (the accepted
//! [`ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact`], as
//! source-test-evidenced by Run 345/346) and translates it into a typed,
//! deterministic, policy-gated **authority-activation execution-preparation /
//! final-execution preflight artifact** for a *future* production
//! authority-activation / final-execution executor — *without* ever activating
//! authority state, executing final settlement, executing final-execution,
//! transitioning a consensus epoch, applying a live validator-set change, or
//! mutating any trust state.
//!
//! The name is deliberate: this is **authority-activation execution-preparation
//! / final-execution preflight artifact generation**, *not* authority
//! activation, *not* final execution, *not* final settlement, and *not*
//! runtime mutation. The artifact only *describes* exactly what a future
//! production authority-activation / final-execution executor would need to
//! re-verify before any actual mutation; it never applies a validator set,
//! transitions an epoch, activates authority state, executes final settlement,
//! executes final-execution, publishes anything, or mutates runtime state.
//! Where the Run 345/346 boundary answers "given a verified
//! post-completion-attestation decision, what typed non-mutating
//! authority-activation-authorization / final-execution-readiness artifact does
//! it authorize?", Run 347 answers the next question: "given a verified
//! authority-activation-authorization / final-execution-readiness decision,
//! what typed, non-mutating **authority-activation execution-preparation /
//! final-execution preflight artifact** carrying the exact preconditions a
//! *future* production authority-activation / final-execution executor must
//! re-verify does it authorize, under an explicit authority-activation
//! execution-preparation policy, bound to the full
//! authorization-decision / settlement / commit / audit / publication /
//! governance / validator-set / custody / attestation / durable-replay evidence
//! tuple, an explicit epoch-transition target, current/proposed validator-set
//! epoch/version preconditions, a required replay window, and a
//! authority-activation-execution-preparation nonce?".
//!
//! ## Scope and honesty constraints (Run 347)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 348**.
...
//! * Only a **verified** Run 345/346 live epoch-transition
//!   authority-activation-authorization / final-execution-readiness decision
//!   that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact`] can authorize an
...
//!   [`ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_347.md`.
...
use crate::pqc_production_live_epoch_transition_authority_activation_authorization::{
    ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
    ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
    LiveEpochTransitionAuthorityActivationAuthorizationKind,
};
...
/// Run 347 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_PROTOCOL_VERSION: u16 = 1;

/// Run 347 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:347-epoch-transition-authority-activation-execution-preparation-intent:v1";

/// Run 347 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_ID_DOMAIN_TAG: &str =
    "QBIND:347-epoch-transition-authority-activation-execution-preparation-id:v1";

/// Run 347 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:347-epoch-transition-authority-activation-execution-preparation-request:v1";

/// Run 347 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:347-epoch-transition-authority-activation-execution-preparation-transcript:v1";
...
/// Run 347 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 345/346 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuthorityActivationExecutionPreparation` and
/// `MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired` are
...
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy {
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation => {
                "allow-source-test-epoch-transition-authority-activation-authorization"
            }
            Self::RequireProductionLiveEpochTransitionAuthorityActivationExecutionPreparation => {
                "require-production-epoch-transition-authority-activation-authorization"
            }
            Self::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired => {
                "mainnet-production-epoch-transition-authority-activation-authorization-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionAuthorityActivationExecutionPreparation` kind is fail-closed as
/// unavailable in Run 347 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation,
    /// Reserved production authorization kind. Fail-closed in Run 347.
    ProductionLiveEpochTransitionAuthorityActivationExecutionPreparation,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation => {
                "source-test-epoch-transition-authority-activation-authorization"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparation => {
                "production-epoch-transition-authority-activation-authorization"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::SourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation,
        )
    }
...
impl Default for ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::Disabled)
    }
}
...
/// Run 347 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 345/346
/// [`LiveEpochTransitionAuthorityActivationAuthorizationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuthorityActivationExecutionPreparationKind {
...
impl LiveEpochTransitionAuthorityActivationExecutionPreparationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 347 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 345/346 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionAuthorityActivationAuthorizationKind,
    ) -> Self {
        use LiveEpochTransitionAuthorityActivationAuthorizationKind as A;
        match kind {
...
/// Run 347 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedAuthorityActivationAuthorizationDecision`] carrying a Run 345/346
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource {
    /// A verified Run 345/346 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedAuthorityActivationAuthorizationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingAuthorityActivationAuthorizationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedAuthorityActivationAuthorizationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedAuthorityActivationAuthorizationWithoutPackage {
        decision: ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 345/346 authority-activation-authorization decision. Rejected.
    CommitAuthorizationDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 345/346 authority-activation-authorization decision. Rejected.
    MutationExecutionDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 345/346 authority-activation-authorization decision. Rejected.
    ExecutionPreparationDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 345/346 authority-activation-authorization decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 331/332 guarded epoch-transition authority-activation-execution-preparation decision
    /// presented directly, without a Run 345/346 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 345/346 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 345/346 guarded authority-activation-execution-preparation decision. Rejected.
    LiveApplicationAuthorizationWithoutAuthorityActivationAuthorization,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 345/346 guarded authority-activation-execution-preparation decision.
    /// Rejected.
    ApplicationDecisionWithoutAuthorityActivationAuthorization,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 345/346 live-application authorization. Rejected.
    RotationPlanWithoutAuthorityActivationAuthorization,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 345/346 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutAuthorityActivationAuthorization,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutAuthorityActivationAuthorization,
    ...
    FixtureOnlyAuthorityActivationAuthorization,
...
/// Run 347 — the explicit trusted inputs the executor binds a verified
/// Run 345/346 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs {
    ...
    pub authority_activation_execution_preparation_policy_id: String,
    /// Expected Run 345/346 live-application authorization policy id (bound
    ...
    /// Expected Run 345/346 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 345/346 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 345/346 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 345/346 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 345/346 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 345/346 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 345/346 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 345/346 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 345/346 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 345/346 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded authority-activation-execution-preparation decision id (re-exposed
    /// by the consumed Run 345/346 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded authority-activation-execution-preparation request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded authority-activation-execution-preparation intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded authority-activation-execution-preparation transcript digest.
    ...
    /// Expected Run 345/346 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_authority_activation_authorization_decision_id: String,
    /// Expected Run 345/346 runtime handoff request id.
    pub expected_authority_activation_authorization_request_id: String,
    /// Expected Run 345/346 runtime handoff package/content digest (the
    /// consumed decision's `authority_activation_execution_preparation_digest`, i.e. the handoff content
    /// digest).
    pub expected_authority_activation_authorization_intent_digest: String,
    /// Expected Run 345/346 runtime handoff transcript digest.
    pub expected_authority_activation_authorization_transcript_digest: String,
    /// Expected Run 345/346 authority-activation-authorization nonce (re-exposed by the
    /// consumed Run 345/346 authority-activation-authorization artifact).
    pub expected_authority_activation_authorization_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 345/346 authority-activation-authorization artifact).
    ...
    /// Run 345/346 authority-activation-authorization artifact).
    ...
    /// wall-clock value and never a reject path in Run 347.
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.authority_activation_execution_preparation_policy_id.is_empty()
            ...
            && !self.expected_authority_activation_authorization_decision_id.is_empty()
            && !self.expected_authority_activation_authorization_request_id.is_empty()
            && !self.expected_authority_activation_authorization_intent_digest.is_empty()
            && !self.expected_authority_activation_authorization_transcript_digest.is_empty()
...
/// Run 347 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition authority-activation-execution-preparation decision), the explicit
/// epoch-transition target, a authority-activation-authorization nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest {
    pub authority_source: LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The authority-activation-authorization nonce (idempotency / replay binding).
    pub authority_activation_execution_preparation_nonce: u64,
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and authority-activation-authorization nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource,
        proposed_epoch_transition_target: u64,
        authority_activation_execution_preparation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 347 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet;

impl LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet
    for EmptyLiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet
{
...
/// Run 347 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 348+); Run 347 never
...
/// preconditions, and carries deterministic `authority_activation_execution_preparation_id`, `request_id`,
/// `authority_activation_execution_preparation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact {
    pub staged_kind: LiveEpochTransitionAuthorityActivationExecutionPreparationKind,
    pub protocol_version: u16,
    pub authority_activation_execution_preparation_policy_id: String,

    // ---- Re-exposed Run 345/346 authorization intent tuple ------------
    ...
    // ---- Bound Run 345/346 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 345/346 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 345/346 guarded authority-activation-execution-preparation authority tuple -
    // (the consumed guarded authority-activation-execution-preparation decision transcript)
    ...
    // ---- Guarded authority-activation-execution-preparation binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded authority-activation-execution-preparation nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 345/346 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub authority_activation_authorization_decision_id: String,
    pub authority_activation_authorization_request_id: String,
    pub authority_activation_authorization_intent_digest: String,
    pub authority_activation_authorization_transcript_digest: String,
    /// The re-exposed Run 345/346 authority-activation-authorization nonce (consumed).
    pub authority_activation_authorization_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 345/346 authority-activation-authorization artifact)
    ...
    /// The newly proposed authority-activation-execution-preparation nonce.
    pub authority_activation_execution_preparation_nonce: u64,
    ...
    pub authority_activation_execution_preparation_id: String,
    pub request_id: String,
    pub authority_activation_execution_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`authority_activation_execution_preparation_id`,
    /// `request_id`, `authority_activation_execution_preparation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_policy_id",
            self.authority_activation_execution_preparation_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_authorization_decision_id",
            self.authority_activation_authorization_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_authorization_request_id",
            self.authority_activation_authorization_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_authorization_intent_digest",
            self.authority_activation_authorization_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_authorization_transcript_digest",
            self.authority_activation_authorization_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_authorization_nonce",
            &self.authority_activation_authorization_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_nonce",
            &self.authority_activation_execution_preparation_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 347 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_content_digest(
    package: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact,
) -> String {
    package.content_digest()
}

/// Run 347 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and authority-activation-authorization nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_execution_preparation_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"authority_activation_execution_preparation_policy_id",
        authority_activation_execution_preparation_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"authority_activation_execution_preparation_nonce",
        &authority_activation_execution_preparation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 347 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and authority-activation-authorization nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_execution_preparation_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"authority_activation_execution_preparation_policy_id", authority_activation_execution_preparation_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 347 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_AUTHORIZATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 347 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuthorityActivationExecutionPreparationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition authority-activation-execution-preparation
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation {
        execution_kind: LiveEpochTransitionAuthorityActivationExecutionPreparationKind,
    ...
    VerifiedAuthorityActivationAuthorizationDecisionRequired,
    UnverifiedAuthorityActivationAuthorizationDecisionRejected,
    ...
    AuthorityActivationAuthorizationDecisionIdMismatch,
    AuthorityActivationAuthorizationDecisionRequestIdMismatch,
    AuthorityActivationAuthorizationDecisionIntentDigestMismatch,
    AuthorityActivationAuthorizationDecisionTranscriptMismatch,
    AuthorityActivationAuthorizationDecisionIntegrityMismatch,
    WrongAuthorityActivationAuthorizationNonce,
    ...
    LiveEpochTransitionAuthorityActivationExecutionPreparationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuthorityActivationExecutionPreparationBoundaryUnavailable => {
                "live-epoch-transition-authority-activation-execution-preparation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable => {
                "production-live-epoch-transition-authority-activation-execution-preparation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable => {
                "mainnet-production-live-epoch-transition-authority-activation-execution-preparation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation { .. } => {
                "accepted-source-test-live-epoch-transition-authority-activation-execution-preparation"
            }
            Self::VerifiedAuthorityActivationAuthorizationDecisionRequired => {
                "verified-authority-activation-authorization-decision-required"
            }
            Self::UnverifiedAuthorityActivationAuthorizationDecisionRejected => {
                "unverified-authority-activation-authorization-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-authority-activation-authorization-decision-rejected-as-production-authority"
            }
            ...
            Self::AuthorityActivationAuthorizationDecisionIdMismatch => "authority-activation-authorization-decision-id-mismatch",
            Self::AuthorityActivationAuthorizationDecisionRequestIdMismatch => {
                "authority-activation-authorization-decision-request-id-mismatch"
            }
            Self::AuthorityActivationAuthorizationDecisionIntentDigestMismatch => {
                "authority-activation-authorization-decision-intent-digest-mismatch"
            }
            Self::AuthorityActivationAuthorizationDecisionTranscriptMismatch => {
                "authority-activation-authorization-decision-transcript-mismatch"
            }
            Self::AuthorityActivationAuthorizationDecisionIntegrityMismatch => {
                "authority-activation-authorization-decision-integrity-mismatch"
            }
            Self::WrongAuthorityActivationAuthorizationNonce => "wrong-authority-activation-authorization-nonce",
            ...
            Self::LiveEpochTransitionAuthorityActivationExecutionPreparationAmbiguous { .. } => {
                "epoch-transition-authority-activation-authorization-ambiguous"
            }
...
/// Run 347 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision {
    pub outcome: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome,
    pub authority_activation_execution_preparation_id: String,
    pub request_id: String,
    pub authority_activation_execution_preparation_artifact:
        Option<ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact>,
    pub authority_activation_execution_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.authority_activation_execution_preparation_artifact.is_some()
    }
}
...
/// Run 347 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRecoveryOutcome {
...
/// Run 347 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor {
    pub config: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig,
    pub policy: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
            &'a ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
        ),
        ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome,
    > {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome as O;
        use LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource as S;
        match source {
            S::VerifiedAuthorityActivationAuthorizationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedAuthorityActivationAuthorizationDecisionRejected);
                }
                match &decision.authority_activation_authorization_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedAuthorityActivationAuthorizationDecisionRequired),
                }
            }
            S::MissingAuthorityActivationAuthorizationDecision => {
                Err(O::VerifiedAuthorityActivationAuthorizationDecisionRequired)
            }
            S::UnverifiedAuthorityActivationAuthorizationDecision { .. } => {
                Err(O::UnverifiedAuthorityActivationAuthorizationDecisionRejected)
            }
            S::AcceptedAuthorityActivationAuthorizationWithoutPackage { .. } => {
                Err(O::VerifiedAuthorityActivationAuthorizationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutAuthorityActivationAuthorization => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutAuthorityActivationAuthorization => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutAuthorityActivationAuthorization => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutAuthorityActivationAuthorization => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutAuthorityActivationAuthorization => {
            ...
            S::FixtureOnlyAuthorityActivationAuthorization => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequired => {
                    O::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 347.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionAuthorityActivationExecutionPreparationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 345/346 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision,
        intent: &ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs,
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 345/346 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.authority_activation_authorization_id != inputs.expected_authority_activation_authorization_decision_id {
            return Some(O::AuthorityActivationAuthorizationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_authority_activation_authorization_request_id {
            return Some(O::AuthorityActivationAuthorizationDecisionRequestIdMismatch);
        }
        if decision.authority_activation_authorization_digest != inputs.expected_authority_activation_authorization_intent_digest {
            return Some(O::AuthorityActivationAuthorizationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_authority_activation_authorization_transcript_digest {
            return Some(O::AuthorityActivationAuthorizationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.authority_activation_authorization_digest {
            return Some(O::AuthorityActivationAuthorizationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed authority-activation-authorization
        // nonce binding.
        if intent.authority_activation_authorization_nonce != inputs.expected_authority_activation_authorization_nonce {
            return Some(O::WrongAuthorityActivationAuthorizationNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 345/346
        // authority-activation-authorization artifact).
        ...
        // authority tuple binding (carried through the consumed Run 345/346
        // authority-activation-authorization artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition authority-activation-execution-preparation
        ...
        // Re-exposed Run 345/346 staged-application decision authority tuple
        ...
        // Re-exposed Run 345/346 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_authority_activation_execution_preparation_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_authorization_digest,
            &inputs.authority_activation_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_execution_preparation_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionAuthorityActivationExecutionPreparationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            authority_activation_execution_preparation_policy_id: inputs.authority_activation_execution_preparation_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded authority-activation-execution-preparation decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded authority-activation-execution-preparation nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 345/346 runtime handoff decision transcript (the
            // authority source this run consumes).
            authority_activation_authorization_decision_id: decision.authority_activation_authorization_id.clone(),
            authority_activation_authorization_request_id: decision.request_id.clone(),
            authority_activation_authorization_intent_digest: decision.authority_activation_authorization_digest.clone(),
            authority_activation_authorization_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed authority-activation-authorization nonce.
            authority_activation_authorization_nonce: application_intent.authority_activation_authorization_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed authority-activation-authorization artifact).
            ...
            // (carried through the consumed authority-activation-authorization artifact).
            ...
            // Newly proposed authority-activation-execution-preparation nonce.
            authority_activation_execution_preparation_nonce: request.authority_activation_execution_preparation_nonce,
            // Exact future-executor preconditions.
            ...
            authority_activation_execution_preparation_id: String::new(),
            request_id: String::new(),
            authority_activation_execution_preparation_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionPreparation {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            authority_activation_execution_preparation_nonce: request.authority_activation_execution_preparation_nonce,
        };
        let authority_activation_execution_preparation_id = production_live_epoch_transition_authority_activation_execution_preparation_id(
            self.config.protocol_version.0,
            &decision.authority_activation_authorization_digest,
            &inputs.authority_activation_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_authority_activation_execution_preparation_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_authorization_digest,
            &inputs.authority_activation_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.authority_activation_execution_preparation_digest = record.content_digest();
        record.authority_activation_execution_preparation_id = authority_activation_execution_preparation_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_authority_activation_execution_preparation_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.authority_activation_execution_preparation_digest,
                outcome.tag(),
            );
    ...
    /// Run 347 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_authority_activation_execution_preparation<
        R: LiveEpochTransitionAuthorityActivationExecutionPreparationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision {
                outcome,
                authority_activation_execution_preparation_id: pkg.authority_activation_execution_preparation_id.clone(),
                request_id: pkg.request_id.clone(),
                authority_activation_execution_preparation_artifact: record.clone(),
                authority_activation_execution_preparation_digest: pkg.authority_activation_execution_preparation_digest.clone(),
        ...
        let authority_activation_authorization_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionAuthorityActivationExecutionPreparationAuthoritySource::AcceptedAuthorityActivationAuthorizationWithoutPackage {
                decision,
            } => decision.authority_activation_authorization_digest.clone(),
            _ => String::new(),
        };

        let authority_activation_execution_preparation_id = production_live_epoch_transition_authority_activation_execution_preparation_id(
            self.config.protocol_version.0,
            &authority_activation_authorization_intent_digest,
            &inputs.authority_activation_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_execution_preparation_nonce,
        );
        ...
        let authority_activation_execution_preparation_digest = String::new();
        ...
        ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision {
            ...
            request_id,
            authority_activation_execution_preparation_artifact: None,
            authority_activation_execution_preparation_digest,
            transcript_digest,
        }
    }

    /// Run 347 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_authority_activation_execution_preparation_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact>,
        current: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact,
    ) -> ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRecoveryOutcome {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::Disabled
        {
...
/// Run 347 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::default()
        == ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationConfig::default().kind
            == ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutorKind::Disabled
}

/// Run 347 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 348).
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 347 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_mainnet_refused() -> bool {
    true
}

/// Run 347 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_is_non_mutating() -> bool {
    true
}

/// Run 347 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_never_falls_back() -> bool {
    true
}

/// Run 347 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 347 — the executor only requires a verified Run 345/346 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_authority_activation_execution_preparation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 347 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 347. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState {
...
impl LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, authority_activation_execution_preparation_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == authority_activation_execution_preparation_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `authority_activation_execution_preparation_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact,
        authority_activation_execution_preparation_id: &str,
    ) -> bool {
        if self.has_applied(authority_activation_execution_preparation_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(authority_activation_execution_preparation_id.to_string());
        true
    }
}