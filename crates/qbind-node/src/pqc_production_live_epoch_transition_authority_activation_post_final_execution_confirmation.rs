...
//! 349/350 live epoch-transition authority-activation execution-preparation /
//! final-execution preflight boundary: the boundary that consumes a
//! **verified** Run 349/350 non-mutating live epoch-transition
//! authority-activation-final-execution *decision* (the accepted
//! [`ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact`], as
//! source-test-evidenced by Run 349/350) and translates it into a typed,
...
//! 349/350 boundary answers "given a verified authority-activation-authorization
//! decision, what typed non-mutating authority-activation execution-preparation
//! / final-execution preflight artifact does it authorize?", Run 351 answers the
...
//! authority-activation-post-final-execution-confirmation nonce?".
//!
//! ## Scope and honesty constraints (Run 351)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 352**.
...
//! * Only a **verified** Run 349/350 live epoch-transition
//!   authority-activation-final-execution / final-execution-readiness decision
//!   that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact`] can authorize an
...
//!   [`ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact`]; it
...
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:351-epoch-transition-authority-activation-post-final-execution-confirmation-request:v1";

/// Run 351 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:351-epoch-transition-authority-activation-post-final-execution-confirmation-transcript:v1";
...
/// Run 351 — typed live validator-set application authorization boundary
...
impl Default for ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 349/350 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation` and
/// `MainnetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRequired` are
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation => {
                "allow-source-test-epoch-transition-authority-activation-final-execution"
            }
            Self::RequireProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation => {
                "require-production-epoch-transition-authority-activation-final-execution"
            }
            Self::MainnetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRequired => {
                "mainnet-production-epoch-transition-authority-activation-final-execution-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation` performs real
...
pub enum ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 351.
    ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation,
}

impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation => {
                "source-test-epoch-transition-authority-activation-final-execution"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation => {
                "production-epoch-transition-authority-activation-final-execution"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorKind,
}
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationConfig {
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorKind::SourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation,
        )
    }
...
/// Run 351 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 349/350
/// [`LiveEpochTransitionAuthorityActivationFinalExecutionKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind {
...
impl LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind {
    ...
    /// Every Run 351 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 349/350 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionAuthorityActivationFinalExecutionKind,
    ) -> Self {
        use LiveEpochTransitionAuthorityActivationFinalExecutionKind as A;
        match kind {
...
/// Run 351 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedAuthorityActivationFinalExecutionDecision`] carrying a Run 349/350
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationAuthoritySource {
    /// A verified Run 349/350 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedAuthorityActivationFinalExecutionDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingAuthorityActivationFinalExecutionDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedAuthorityActivationFinalExecutionDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedAuthorityActivationFinalExecutionWithoutPackage {
        decision: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 349/350 authority-activation-final-execution decision. Rejected.
    CommitAuthorizationDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 349/350 authority-activation-final-execution decision. Rejected.
    MutationExecutionDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 349/350 authority-activation-final-execution decision. Rejected.
    ExecutionPreparationDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 349/350 authority-activation-final-execution decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 331/332 guarded epoch-transition authority-activation-post-final-execution-confirmation decision
    /// presented directly, without a Run 349/350 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 349/350 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 349/350 guarded authority-activation-post-final-execution-confirmation decision. Rejected.
    LiveApplicationAuthorizationWithoutAuthorityActivationFinalExecution,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 349/350 guarded authority-activation-post-final-execution-confirmation decision.
    /// Rejected.
    ApplicationDecisionWithoutAuthorityActivationFinalExecution,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 349/350 live-application authorization. Rejected.
    RotationPlanWithoutAuthorityActivationFinalExecution,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 349/350 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutAuthorityActivationFinalExecution,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutAuthorityActivationFinalExecution,
    ...
    FixtureOnlyAuthorityActivationFinalExecution,
...
/// Run 351 — the explicit trusted inputs the executor binds a verified
/// Run 349/350 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationInputs {
    ...
    pub authority_activation_post_final_execution_confirmation_policy_id: String,
    /// Expected Run 349/350 live-application authorization policy id (bound
    ...
    /// Expected Run 349/350 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 349/350 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 349/350 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 349/350 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 349/350 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 349/350 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 349/350 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 349/350 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 349/350 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 349/350 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded authority-activation-post-final-execution-confirmation decision id (re-exposed
    /// by the consumed Run 349/350 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded authority-activation-post-final-execution-confirmation request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded authority-activation-post-final-execution-confirmation intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded authority-activation-post-final-execution-confirmation transcript digest.
    ...
    /// Expected Run 349/350 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_authority_activation_final_execution_decision_id: String,
    /// Expected Run 349/350 runtime handoff request id.
    pub expected_authority_activation_final_execution_request_id: String,
    /// Expected Run 349/350 runtime handoff package/content digest (the
    /// consumed decision's `authority_activation_post_final_execution_confirmation_digest`, i.e. the handoff content
    /// digest).
    pub expected_authority_activation_final_execution_intent_digest: String,
    /// Expected Run 349/350 runtime handoff transcript digest.
    pub expected_authority_activation_final_execution_transcript_digest: String,
    /// Expected Run 349/350 authority-activation-final-execution nonce (re-exposed by the
    /// consumed Run 349/350 authority-activation-final-execution artifact).
    pub expected_authority_activation_final_execution_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 349/350 authority-activation-final-execution artifact).
    ...
    /// Run 349/350 authority-activation-final-execution artifact).
    ...
    /// wall-clock value and never a reject path in Run 351.
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationInputs {
...
/// Run 351 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition authority-activation-post-final-execution-confirmation decision), the explicit
/// epoch-transition target, a authority-activation-final-execution nonce, and any represented
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and authority-activation-final-execution nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationAuthoritySource,
        proposed_epoch_transition_target: u64,
        authority_activation_post_final_execution_confirmation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 351 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet;

impl LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet
    for EmptyLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet
{
...
/// Run 351 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 352+); Run 351 never
...
/// preconditions, and carries deterministic `authority_activation_post_final_execution_confirmation_id`, `request_id`,
/// `authority_activation_post_final_execution_confirmation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact {
    pub staged_kind: LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind,
    pub protocol_version: u16,
    pub authority_activation_post_final_execution_confirmation_policy_id: String,

    // ---- Re-exposed Run 349/350 authorization intent tuple ------------
    ...
    // ---- Bound Run 349/350 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 349/350 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 349/350 guarded authority-activation-post-final-execution-confirmation authority tuple -
    // (the consumed guarded authority-activation-post-final-execution-confirmation decision transcript)
    ...
    // ---- Guarded authority-activation-post-final-execution-confirmation binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded authority-activation-post-final-execution-confirmation nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 349/350 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub authority_activation_final_execution_decision_id: String,
    pub authority_activation_final_execution_request_id: String,
    pub authority_activation_final_execution_intent_digest: String,
    pub authority_activation_final_execution_transcript_digest: String,
    /// The re-exposed Run 349/350 authority-activation-final-execution nonce (consumed).
    pub authority_activation_final_execution_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 349/350 authority-activation-final-execution artifact)
    ...
    /// The newly proposed authority-activation-post-final-execution-confirmation nonce.
    pub authority_activation_post_final_execution_confirmation_nonce: u64,
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`authority_activation_post_final_execution_confirmation_id`,
    /// `request_id`, `authority_activation_post_final_execution_confirmation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
...
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_content_digest(
    package: &ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact,
) -> String {
    package.content_digest()
}

/// Run 351 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and authority-activation-final-execution nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_post_final_execution_confirmation_policy_id: &str,
    ...
    hash_field(
        &mut h,
        b"authority_activation_post_final_execution_confirmation_nonce",
        &authority_activation_post_final_execution_confirmation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 351 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and authority-activation-final-execution nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_post_final_execution_confirmation_policy_id: &str,
    epoch_transition_target: u64,
...
/// Run 351 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
...
/// Run 351 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition authority-activation-post-final-execution-confirmation
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation {
        execution_kind: LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind,
    ...
    VerifiedAuthorityActivationFinalExecutionDecisionRequired,
    UnverifiedAuthorityActivationFinalExecutionDecisionRejected,
    ...
    AuthorityActivationFinalExecutionDecisionIdMismatch,
    AuthorityActivationFinalExecutionDecisionRequestIdMismatch,
    AuthorityActivationFinalExecutionDecisionIntentDigestMismatch,
    AuthorityActivationFinalExecutionDecisionTranscriptMismatch,
    AuthorityActivationFinalExecutionDecisionIntegrityMismatch,
    WrongAuthorityActivationFinalExecutionNonce,
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationBoundaryUnavailable => {
                "live-epoch-transition-authority-activation-post-final-execution-confirmation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable => {
                "production-live-epoch-transition-authority-activation-post-final-execution-confirmation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable => {
                "mainnet-production-live-epoch-transition-authority-activation-post-final-execution-confirmation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation { .. } => {
                "accepted-source-test-live-epoch-transition-authority-activation-post-final-execution-confirmation"
            }
            Self::VerifiedAuthorityActivationFinalExecutionDecisionRequired => {
                "verified-authority-activation-final-execution-decision-required"
            }
            Self::UnverifiedAuthorityActivationFinalExecutionDecisionRejected => {
                "unverified-authority-activation-final-execution-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-authority-activation-final-execution-decision-rejected-as-production-authority"
            }
            ...
            Self::AuthorityActivationFinalExecutionDecisionIdMismatch => "authority-activation-final-execution-decision-id-mismatch",
            Self::AuthorityActivationFinalExecutionDecisionRequestIdMismatch => {
                "authority-activation-final-execution-decision-request-id-mismatch"
            }
            Self::AuthorityActivationFinalExecutionDecisionIntentDigestMismatch => {
                "authority-activation-final-execution-decision-intent-digest-mismatch"
            }
            Self::AuthorityActivationFinalExecutionDecisionTranscriptMismatch => {
                "authority-activation-final-execution-decision-transcript-mismatch"
            }
            Self::AuthorityActivationFinalExecutionDecisionIntegrityMismatch => {
                "authority-activation-final-execution-decision-integrity-mismatch"
            }
            Self::WrongAuthorityActivationFinalExecutionNonce => "wrong-authority-activation-final-execution-nonce",
            ...
            Self::LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationAmbiguous { .. } => {
                "epoch-transition-authority-activation-final-execution-ambiguous"
            }
...
/// Run 351 — the typed decision produced by the executor boundary: the
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision {
    pub fn is_accept(&self) -> bool {
...
/// Run 351 — typed idempotency / recovery outcome for a prepared staged
...
impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRecoveryOutcome {
...
/// Run 351 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor {
    pub config: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationConfig,
    pub policy: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision,
            &'a ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact,
        ),
        ...
        match source {
            S::VerifiedAuthorityActivationFinalExecutionDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedAuthorityActivationFinalExecutionDecisionRejected);
                }
                match &decision.authority_activation_final_execution_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedAuthorityActivationFinalExecutionDecisionRequired),
                }
            }
            S::MissingAuthorityActivationFinalExecutionDecision => {
                Err(O::VerifiedAuthorityActivationFinalExecutionDecisionRequired)
            }
            S::UnverifiedAuthorityActivationFinalExecutionDecision { .. } => {
                Err(O::UnverifiedAuthorityActivationFinalExecutionDecisionRejected)
            }
            S::AcceptedAuthorityActivationFinalExecutionWithoutPackage { .. } => {
                Err(O::VerifiedAuthorityActivationFinalExecutionDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutAuthorityActivationFinalExecution => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutAuthorityActivationFinalExecution => {
            ...
            S::GovernanceProofWithoutAuthorityActivationFinalExecution => {
            ...
            S::FixtureOnlyAuthorityActivationFinalExecution => {
    ...
    fn preflight_gate(
        ...
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRequired => {
                    O::MainNetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 351.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 349/350 live-application authorization
    ...
    fn check_application_binding(
        ...
        if intent.content_digest() != decision.authority_activation_final_execution_digest {
            return Some(O::AuthorityActivationFinalExecutionDecisionIntegrityMismatch);
        }
        ...
        // authority tuple binding (carried through the consumed Run 349/350
        // authority-activation-final-execution artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition authority-activation-post-final-execution-confirmation
        ...
        // Re-exposed Run 349/350 staged-application decision authority tuple
        ...
        // Re-exposed Run 349/350 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationReplaySet + ?Sized>(
        ...
        let staged_application_id = production_live_epoch_transition_authority_activation_post_final_execution_confirmation_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_final_execution_digest,
            &inputs.authority_activation_post_final_execution_confirmation_policy_id,
        ...
        let staged_kind =
            LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            authority_activation_post_final_execution_confirmation_policy_id: inputs.authority_activation_post_final_execution_confirmation_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded authority-activation-post-final-execution-confirmation decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded authority-activation-post-final-execution-confirmation nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 349/350 runtime handoff decision transcript (the
            // authority source this run consumes).
            authority_activation_final_execution_decision_id: decision.authority_activation_final_execution_id.clone(),
            authority_activation_final_execution_request_id: decision.request_id.clone(),
            authority_activation_final_execution_intent_digest: decision.authority_activation_final_execution_digest.clone(),
            authority_activation_final_execution_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed authority-activation-final-execution nonce.
            authority_activation_final_execution_nonce: application_intent.authority_activation_final_execution_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed authority-activation-final-execution artifact).
            ...
            // (carried through the consumed authority-activation-final-execution artifact).
            ...
            // Newly proposed authority-activation-post-final-execution-confirmation nonce.
            authority_activation_post_final_execution_confirmation_nonce: request.authority_activation_post_final_execution_confirmation_nonce,
            // Exact future-executor preconditions.
            ...
            authority_activation_post_final_execution_confirmation_id: String::new(),
            request_id: String::new(),
            authority_activation_post_final_execution_confirmation_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            authority_activation_post_final_execution_confirmation_nonce: request.authority_activation_post_final_execution_confirmation_nonce,
        };
        let authority_activation_post_final_execution_confirmation_id = production_live_epoch_transition_authority_activation_post_final_execution_confirmation_id(
            self.config.protocol_version.0,
            &decision.authority_activation_final_execution_digest,
            &inputs.authority_activation_post_final_execution_confirmation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_authority_activation_post_final_execution_confirmation_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_final_execution_digest,
            &inputs.authority_activation_post_final_execution_confirmation_policy_id,
            request.proposed_epoch_transition_target,
    ...
    /// Run 351 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_authority_activation_post_final_execution_confirmation<
    ...
    >(
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision {
                outcome,
                authority_activation_post_final_execution_confirmation_id: pkg.authority_activation_post_final_execution_confirmation_id.clone(),
                request_id: pkg.request_id.clone(),
                authority_activation_post_final_execution_confirmation_artifact: record.clone(),
                authority_activation_post_final_execution_confirmation_digest: pkg.authority_activation_post_final_execution_confirmation_digest.clone(),
        ...
        let authority_activation_final_execution_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationAuthoritySource::AcceptedAuthorityActivationFinalExecutionWithoutPackage {
                decision,
            } => decision.authority_activation_final_execution_digest.clone(),
            _ => String::new(),
        };

        let authority_activation_post_final_execution_confirmation_id = production_live_epoch_transition_authority_activation_post_final_execution_confirmation_id(
            self.config.protocol_version.0,
            &authority_activation_final_execution_intent_digest,
            &inputs.authority_activation_post_final_execution_confirmation_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_post_final_execution_confirmation_nonce,
        );
        ...
        let authority_activation_post_final_execution_confirmation_digest = String::new();
        ...
        ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision {
            ...
            request_id,
            authority_activation_post_final_execution_confirmation_artifact: None,
            authority_activation_post_final_execution_confirmation_digest,
            transcript_digest,
        }
    }

    /// Run 351 — idempotency / recovery over a prepared-authorization window.
...
/// Run 351 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 352).
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}
...
/// Run 351 — the executor never falls back to rotation-plan-alone /
...
/// Run 351 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 351 — the executor only requires a verified Run 349/350 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_authority_activation_post_final_execution_confirmation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 351 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 351. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationFixtureState {
...
impl LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationFixtureState {
    ...
    ///
    /// Idempotent: re-applying the same `authority_activation_post_final_execution_confirmation_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact,
        authority_activation_post_final_execution_confirmation_id: &str,
    ) -> bool {
        if self.has_applied(authority_activation_post_final_execution_confirmation_id) {
            return false;
        }
