//! Run 341 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 331/332 guarded epoch-transition final-settlement-completion boundary: the boundary
//! that consumes a **verified** Run 339/340 non-mutating guarded
//! epoch-transition final-settlement-completion *decision* (the accepted
//! [`ProductionLiveEpochTransitionSettlementExecutionDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionSettlementExecutionArtifact`], as
...
//! mutation record does it authorize?", Run 341 answers the next question:
//! "given a verified guarded final-settlement-completion decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a settlement-execution
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 341)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 342**.
...
//! * Only a **verified** Run 339/340 guarded epoch-transition
//!   final-settlement-completion decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionSettlementExecutionArtifact`] can authorize a runtime
...
//!   [`ProductionLiveEpochTransitionFinalSettlementCompletionArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_339.md`.
...
use crate::pqc_production_live_epoch_transition_settlement_execution::{
    ProductionLiveEpochTransitionSettlementExecutionDecision,
    ProductionLiveEpochTransitionSettlementExecutionArtifact,
    LiveEpochTransitionSettlementExecutionKind,
};
...
/// Run 341 — the only live validator-set application authorization boundary
...
/// Run 341 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_INTENT_DOMAIN_TAG: &str =
    "QBIND:341-epoch-transition-final-settlement-completion-intent:v1";

/// Run 341 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_ID_DOMAIN_TAG: &str =
    "QBIND:341-epoch-transition-final-settlement-completion-id:v1";

/// Run 341 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:341-epoch-transition-final-settlement-completion-request:v1";

/// Run 341 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:341-epoch-transition-final-settlement-completion-transcript:v1";
...
/// Run 341 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion {
...
impl Default for ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionFinalSettlementCompletion` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 339/340 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionFinalSettlementCompletion` and
/// `MainnetProductionLiveEpochTransitionFinalSettlementCompletionRequired` are
...
pub enum ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy {
...
impl ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionFinalSettlementCompletion => {
                "allow-source-test-epoch-transition-settlement-execution"
            }
            Self::RequireProductionLiveEpochTransitionFinalSettlementCompletion => {
                "require-production-epoch-transition-settlement-execution"
            }
            Self::MainnetProductionLiveEpochTransitionFinalSettlementCompletionRequired => {
                "mainnet-production-epoch-transition-settlement-execution-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionFinalSettlementCompletion` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionFinalSettlementCompletion` kind is fail-closed as
/// unavailable in Run 341 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 341.
    ProductionLiveEpochTransitionFinalSettlementCompletion,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionFinalSettlementCompletion => {
                "source-test-epoch-transition-settlement-execution"
            }
            Self::ProductionLiveEpochTransitionFinalSettlementCompletion => {
                "production-epoch-transition-settlement-execution"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionConfig {
    pub fn new(kind: ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::SourceTestLiveEpochTransitionFinalSettlementCompletion,
        )
    }
...
impl Default for ProductionLiveEpochTransitionFinalSettlementCompletionConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::Disabled)
    }
}
...
/// Run 341 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 339/340
/// [`LiveEpochTransitionSettlementExecutionKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionFinalSettlementCompletionKind {
...
impl LiveEpochTransitionFinalSettlementCompletionKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 341 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 339/340 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionSettlementExecutionKind,
    ) -> Self {
        use LiveEpochTransitionSettlementExecutionKind as A;
        match kind {
...
/// Run 341 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedSettlementExecutionDecision`] carrying a Run 339/340
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionFinalSettlementCompletionAuthoritySource {
    /// A verified Run 339/340 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedSettlementExecutionDecision {
        decision: ProductionLiveEpochTransitionSettlementExecutionDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingSettlementExecutionDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedSettlementExecutionDecision {
        decision: ProductionLiveEpochTransitionSettlementExecutionDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedSettlementExecutionWithoutPackage {
        decision: ProductionLiveEpochTransitionSettlementExecutionDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 339/340 settlement-execution decision. Rejected.
    CommitAuthorizationDecisionWithoutSettlementExecution,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 339/340 settlement-execution decision. Rejected.
    MutationExecutionDecisionWithoutSettlementExecution,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 339/340 settlement-execution decision. Rejected.
    ExecutionPreparationDecisionWithoutSettlementExecution,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 339/340 settlement-execution decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutSettlementExecution,
    /// A Run 331/332 guarded epoch-transition final-settlement-completion decision
    /// presented directly, without a Run 339/340 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutSettlementExecution,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 339/340 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutSettlementExecution,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 339/340 guarded final-settlement-completion decision. Rejected.
    LiveApplicationAuthorizationWithoutSettlementExecution,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 339/340 guarded final-settlement-completion decision.
    /// Rejected.
    ApplicationDecisionWithoutSettlementExecution,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 339/340 live-application authorization. Rejected.
    RotationPlanWithoutSettlementExecution,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 339/340 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutSettlementExecution,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutSettlementExecution,
    ...
    FixtureOnlySettlementExecution,
...
/// Run 341 — the explicit trusted inputs the executor binds a verified
/// Run 339/340 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionInputs {
    ...
    pub final_settlement_completion_policy_id: String,
    /// Expected Run 339/340 live-application authorization policy id (bound
    ...
    /// Expected Run 339/340 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 339/340 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 339/340 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 339/340 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 339/340 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 339/340 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 339/340 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 339/340 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 339/340 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 339/340 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded final-settlement-completion decision id (re-exposed
    /// by the consumed Run 339/340 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded final-settlement-completion request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded final-settlement-completion intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded final-settlement-completion transcript digest.
    ...
    /// Expected Run 339/340 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_settlement_execution_decision_id: String,
    /// Expected Run 339/340 runtime handoff request id.
    pub expected_settlement_execution_request_id: String,
    /// Expected Run 339/340 runtime handoff package/content digest (the
    /// consumed decision's `final_settlement_completion_digest`, i.e. the handoff content
    /// digest).
    pub expected_settlement_execution_intent_digest: String,
    /// Expected Run 339/340 runtime handoff transcript digest.
    pub expected_settlement_execution_transcript_digest: String,
    /// Expected Run 339/340 settlement-execution nonce (re-exposed by the
    /// consumed Run 339/340 settlement-execution artifact).
    pub expected_settlement_execution_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 339/340 settlement-execution artifact).
    ...
    /// Run 339/340 settlement-execution artifact).
    ...
    /// wall-clock value and never a reject path in Run 341.
...
impl ProductionLiveEpochTransitionFinalSettlementCompletionInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.final_settlement_completion_policy_id.is_empty()
            ...
            && !self.expected_settlement_execution_decision_id.is_empty()
            && !self.expected_settlement_execution_request_id.is_empty()
            && !self.expected_settlement_execution_intent_digest.is_empty()
            && !self.expected_settlement_execution_transcript_digest.is_empty()
...
/// Run 341 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition final-settlement-completion decision), the explicit
/// epoch-transition target, a settlement-execution nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionRequest {
    pub authority_source: LiveEpochTransitionFinalSettlementCompletionAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The settlement-execution nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionFinalSettlementCompletionRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and settlement-execution nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionFinalSettlementCompletionAuthoritySource,
        proposed_epoch_transition_target: u64,
        final_settlement_completion_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 341 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionFinalSettlementCompletionReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionFinalSettlementCompletionReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionFinalSettlementCompletionReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionFinalSettlementCompletionReplaySet;

impl LiveEpochTransitionFinalSettlementCompletionReplaySet
    for EmptyLiveEpochTransitionFinalSettlementCompletionReplaySet
{
...
/// Run 341 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 342+); Run 341 never
...
/// preconditions, and carries deterministic `final_settlement_completion_id`, `request_id`,
/// `final_settlement_completion_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionArtifact {
    pub staged_kind: LiveEpochTransitionFinalSettlementCompletionKind,
    pub protocol_version: u16,
    pub final_settlement_completion_policy_id: String,

    // ---- Re-exposed Run 339/340 authorization intent tuple ------------
    ...
    // ---- Bound Run 339/340 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 339/340 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 339/340 guarded final-settlement-completion authority tuple -
    // (the consumed guarded final-settlement-completion decision transcript)
    ...
    // ---- Guarded final-settlement-completion binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded final-settlement-completion nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 339/340 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub settlement_execution_decision_id: String,
    pub settlement_execution_request_id: String,
    pub settlement_execution_intent_digest: String,
    pub settlement_execution_transcript_digest: String,
    /// The re-exposed Run 339/340 settlement-execution nonce (consumed).
    pub settlement_execution_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 339/340 settlement-execution artifact)
    ...
    /// The newly proposed final-settlement-completion nonce.
    pub final_settlement_completion_nonce: u64,
    ...
    pub final_settlement_completion_id: String,
    pub request_id: String,
    pub final_settlement_completion_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`final_settlement_completion_id`,
    /// `request_id`, `final_settlement_completion_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        ...
        hash_field(
            &mut h,
            b"final_settlement_completion_policy_id",
            self.final_settlement_completion_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"settlement_execution_decision_id",
            self.settlement_execution_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_request_id",
            self.settlement_execution_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_intent_digest",
            self.settlement_execution_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_transcript_digest",
            self.settlement_execution_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 341 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_final_settlement_completion_content_digest(
    package: &ProductionLiveEpochTransitionFinalSettlementCompletionArtifact,
) -> String {
    package.content_digest()
}

/// Run 341 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and settlement-execution nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_final_settlement_completion_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    final_settlement_completion_policy_id: &str,
    ...
    hash_field(
        &mut h,
        b"final_settlement_completion_policy_id",
        final_settlement_completion_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"final_settlement_completion_nonce",
        &final_settlement_completion_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 341 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and settlement-execution nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_final_settlement_completion_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    final_settlement_completion_policy_id: &str,
    epoch_transition_target: u64,
    ...
    hash_field(&mut h, b"final_settlement_completion_policy_id", final_settlement_completion_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 341 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_final_settlement_completion_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
...
/// Run 341 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionFinalSettlementCompletion`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionFinalSettlementCompletionOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionFinalSettlementCompletionBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionFinalSettlementCompletionUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition final-settlement-completion
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionFinalSettlementCompletion {
        execution_kind: LiveEpochTransitionFinalSettlementCompletionKind,
    ...
    VerifiedSettlementExecutionDecisionRequired,
    UnverifiedSettlementExecutionDecisionRejected,
    ...
    SettlementExecutionDecisionIdMismatch,
    SettlementExecutionDecisionRequestIdMismatch,
    SettlementExecutionDecisionIntentDigestMismatch,
    SettlementExecutionDecisionTranscriptMismatch,
    SettlementExecutionDecisionIntegrityMismatch,
    WrongSettlementExecutionNonce,
    ...
    LiveEpochTransitionFinalSettlementCompletionAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionFinalSettlementCompletionBoundaryUnavailable => {
                "live-epoch-transition-final-settlement-completion-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionFinalSettlementCompletionUnavailable => {
                "production-live-epoch-transition-final-settlement-completion-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionFinalSettlementCompletionUnavailable => {
                "mainnet-production-live-epoch-transition-final-settlement-completion-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionFinalSettlementCompletion { .. } => {
                "accepted-source-test-live-epoch-transition-final-settlement-completion"
            }
            Self::VerifiedSettlementExecutionDecisionRequired => {
                "verified-settlement-execution-decision-required"
            }
            Self::UnverifiedSettlementExecutionDecisionRejected => {
                "unverified-settlement-execution-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-settlement-execution-decision-rejected-as-production-authority"
            }
            ...
            Self::SettlementExecutionDecisionIdMismatch => "settlement-execution-decision-id-mismatch",
            Self::SettlementExecutionDecisionRequestIdMismatch => {
                "settlement-execution-decision-request-id-mismatch"
            }
            Self::SettlementExecutionDecisionIntentDigestMismatch => {
                "settlement-execution-decision-intent-digest-mismatch"
            }
            Self::SettlementExecutionDecisionTranscriptMismatch => {
                "settlement-execution-decision-transcript-mismatch"
            }
            Self::SettlementExecutionDecisionIntegrityMismatch => {
                "settlement-execution-decision-integrity-mismatch"
            }
            Self::WrongSettlementExecutionNonce => "wrong-settlement-execution-nonce",
            ...
            Self::LiveEpochTransitionFinalSettlementCompletionAmbiguous { .. } => {
                "epoch-transition-settlement-execution-ambiguous"
            }
...
/// Run 341 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionDecision {
    pub outcome: ProductionLiveEpochTransitionFinalSettlementCompletionOutcome,
    pub final_settlement_completion_id: String,
    pub request_id: String,
    pub final_settlement_completion_artifact:
        Option<ProductionLiveEpochTransitionFinalSettlementCompletionArtifact>,
    pub final_settlement_completion_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.final_settlement_completion_artifact.is_some()
    }
}
...
/// Run 341 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome {
...
impl ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome {
...
/// Run 341 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionFinalSettlementCompletionExecutor {
    pub config: ProductionLiveEpochTransitionFinalSettlementCompletionConfig,
    pub policy: ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy,
}

impl ProductionLiveEpochTransitionFinalSettlementCompletionExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionFinalSettlementCompletionConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionFinalSettlementCompletionAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionSettlementExecutionDecision,
            &'a ProductionLiveEpochTransitionSettlementExecutionArtifact,
        ),
        ProductionLiveEpochTransitionFinalSettlementCompletionOutcome,
    > {
        use ProductionLiveEpochTransitionFinalSettlementCompletionOutcome as O;
        use LiveEpochTransitionFinalSettlementCompletionAuthoritySource as S;
        match source {
            S::VerifiedSettlementExecutionDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedSettlementExecutionDecisionRejected);
                }
                match &decision.settlement_execution_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedSettlementExecutionDecisionRequired),
                }
            }
            S::MissingSettlementExecutionDecision => {
                Err(O::VerifiedSettlementExecutionDecisionRequired)
            }
            S::UnverifiedSettlementExecutionDecision { .. } => {
                Err(O::UnverifiedSettlementExecutionDecisionRejected)
            }
            S::AcceptedSettlementExecutionWithoutPackage { .. } => {
                Err(O::VerifiedSettlementExecutionDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutSettlementExecution => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutSettlementExecution => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutSettlementExecution => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutSettlementExecution => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutSettlementExecution => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutSettlementExecution => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutSettlementExecution => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutSettlementExecution => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutSettlementExecution => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutSettlementExecution => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutSettlementExecution => {
            ...
            S::FixtureOnlySettlementExecution => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionFinalSettlementCompletionOutcome> {
        use ProductionLiveEpochTransitionFinalSettlementCompletionOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::MainnetProductionLiveEpochTransitionFinalSettlementCompletionRequired => {
                    O::MainNetProductionLiveEpochTransitionFinalSettlementCompletionUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionFinalSettlementCompletionUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionFinalSettlementCompletionUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 341.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionFinalSettlementCompletionBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 339/340 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionSettlementExecutionDecision,
        intent: &ProductionLiveEpochTransitionSettlementExecutionArtifact,
        inputs: &ProductionLiveEpochTransitionFinalSettlementCompletionInputs,
    ) -> Option<ProductionLiveEpochTransitionFinalSettlementCompletionOutcome> {
        use ProductionLiveEpochTransitionFinalSettlementCompletionOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 339/340 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.settlement_execution_id != inputs.expected_settlement_execution_decision_id {
            return Some(O::SettlementExecutionDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_settlement_execution_request_id {
            return Some(O::SettlementExecutionDecisionRequestIdMismatch);
        }
        if decision.settlement_execution_digest != inputs.expected_settlement_execution_intent_digest {
            return Some(O::SettlementExecutionDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_settlement_execution_transcript_digest {
            return Some(O::SettlementExecutionDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.settlement_execution_digest {
            return Some(O::SettlementExecutionDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed settlement-execution
        // nonce binding.
        if intent.settlement_execution_nonce != inputs.expected_settlement_execution_nonce {
            return Some(O::WrongSettlementExecutionNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 339/340
        // settlement-execution artifact).
        ...
        // authority tuple binding (carried through the consumed Run 339/340
        // settlement-execution artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition final-settlement-completion
        ...
        // Re-exposed Run 339/340 staged-application decision authority tuple
        ...
        // Re-exposed Run 339/340 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionFinalSettlementCompletionReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionFinalSettlementCompletionRequest,
        inputs: &ProductionLiveEpochTransitionFinalSettlementCompletionInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_final_settlement_completion_request_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_digest,
            &inputs.final_settlement_completion_policy_id,
            request.proposed_epoch_transition_target,
            request.final_settlement_completion_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionFinalSettlementCompletionKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionFinalSettlementCompletionArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            final_settlement_completion_policy_id: inputs.final_settlement_completion_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded final-settlement-completion decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded final-settlement-completion nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 339/340 runtime handoff decision transcript (the
            // authority source this run consumes).
            settlement_execution_decision_id: decision.settlement_execution_id.clone(),
            settlement_execution_request_id: decision.request_id.clone(),
            settlement_execution_intent_digest: decision.settlement_execution_digest.clone(),
            settlement_execution_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed settlement-execution nonce.
            settlement_execution_nonce: application_intent.settlement_execution_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed settlement-execution artifact).
            ...
            // (carried through the consumed settlement-execution artifact).
            ...
            // Newly proposed final-settlement-completion nonce.
            final_settlement_completion_nonce: request.final_settlement_completion_nonce,
            // Exact future-executor preconditions.
            ...
            final_settlement_completion_id: String::new(),
            request_id: String::new(),
            final_settlement_completion_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionFinalSettlementCompletion {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            final_settlement_completion_nonce: request.final_settlement_completion_nonce,
        };
        let final_settlement_completion_id = production_live_epoch_transition_final_settlement_completion_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_digest,
            &inputs.final_settlement_completion_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_final_settlement_completion_request_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_digest,
            &inputs.final_settlement_completion_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.final_settlement_completion_digest = record.content_digest();
        record.final_settlement_completion_id = final_settlement_completion_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_final_settlement_completion_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.final_settlement_completion_digest,
                outcome.tag(),
            );
    ...
    /// Run 341 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_final_settlement_completion<
        R: LiveEpochTransitionFinalSettlementCompletionReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionFinalSettlementCompletionRequest,
        inputs: &ProductionLiveEpochTransitionFinalSettlementCompletionInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionFinalSettlementCompletionDecision {
                outcome,
                final_settlement_completion_id: pkg.final_settlement_completion_id.clone(),
                request_id: pkg.request_id.clone(),
                final_settlement_completion_artifact: record.clone(),
                final_settlement_completion_digest: pkg.final_settlement_completion_digest.clone(),
        ...
        let settlement_execution_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionFinalSettlementCompletionAuthoritySource::AcceptedSettlementExecutionWithoutPackage {
                decision,
            } => decision.settlement_execution_digest.clone(),
            _ => String::new(),
        };

        let final_settlement_completion_id = production_live_epoch_transition_final_settlement_completion_id(
            self.config.protocol_version.0,
            &settlement_execution_intent_digest,
            &inputs.final_settlement_completion_policy_id,
            request.proposed_epoch_transition_target,
            request.final_settlement_completion_nonce,
        );
        ...
        let final_settlement_completion_digest = String::new();
        ...
        ProductionLiveEpochTransitionFinalSettlementCompletionDecision {
            ...
            request_id,
            final_settlement_completion_artifact: None,
            final_settlement_completion_digest,
            transcript_digest,
        }
    }

    /// Run 341 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_final_settlement_completion_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionFinalSettlementCompletionArtifact>,
        current: &ProductionLiveEpochTransitionFinalSettlementCompletionArtifact,
    ) -> ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome {
        use ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::Disabled
        {
...
/// Run 341 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_final_settlement_completion_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::default()
        == ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionFinalSettlementCompletionConfig::default().kind
            == ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind::Disabled
}

/// Run 341 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 342).
pub fn production_live_epoch_transition_final_settlement_completion_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 341 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_final_settlement_completion_executor_mainnet_refused() -> bool {
    true
}

/// Run 341 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_final_settlement_completion_executor_is_non_mutating() -> bool {
    true
}

/// Run 341 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_final_settlement_completion_executor_never_falls_back() -> bool {
    true
}

/// Run 341 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_final_settlement_completion_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 341 — the executor only requires a verified Run 339/340 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_final_settlement_completion_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 341 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 341. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionFinalSettlementCompletionFixtureState {
...
impl LiveEpochTransitionFinalSettlementCompletionFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, final_settlement_completion_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == final_settlement_completion_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `final_settlement_completion_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionFinalSettlementCompletionArtifact,
        final_settlement_completion_id: &str,
    ) -> bool {
        if self.has_applied(final_settlement_completion_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(final_settlement_completion_id.to_string());
        true
    }
}