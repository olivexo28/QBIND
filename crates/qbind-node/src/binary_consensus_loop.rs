...
pub struct ProposalVoteAuthority {
    ...
    pub signer: Option<Arc<dyn ValidatorSigner>>,
    /// Run 422 D6: **mandatory** versioned Proposal/Vote signing domain.
    ///
    ...
    /// attacker-controlled wire data. A `ProposalVoteAuthority` cannot exist
    /// without a validated v2 domain, so a present authority on the fail-closed
    /// [`ConsensusVerificationPolicy::Required`] node path can NEVER fall back
    /// to legacy v1 verification: there is no v2→v1 fallback and no way to
    /// construct an authority that "has no domain metadata". The boundary signs
    /// and verifies exclusively with this versioned v2 domain
    /// ([`qbind_wire::pv_signing_domain::ProposalVoteSigningDomainV2`]), binding
    /// the full runtime chain id, accepted genesis identity, and accepted
    /// authority commitment. Inbound and outbound messages whose wire
    /// `chain_id` disagrees with the domain's validated `expected_wire_chain_id`
    /// are rejected/suppressed before crypto or transmission.
    ///
    ...
    pub signing_domain: ProposalVoteSigningDomainV2,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProposalVoteAuthority")
            ...
            .field(
                ...
                &self.signing_domain.expected_wire_chain_id(),
            )
            .finish()
    }
}
...
impl ProposalVoteAuthority {
    /// Build the outbound signing preimage for a proposal under the mandatory
    /// v2 domain. The preimage is built from *trusted authority* (the domain),
    /// never from the message beyond its canonical body.
    fn proposal_preimage(&self, proposal: &BlockProposal) -> Vec<u8> {
        self.signing_domain.proposal_preimage(proposal)
    }

    /// Build the outbound signing preimage for a vote under the mandatory v2
    /// domain. See [`ProposalVoteAuthority::proposal_preimage`].
    fn vote_preimage(&self, vote: &Vote) -> Vec<u8> {
        self.signing_domain.vote_preimage(vote)
    }

    /// Whether an outbound/inbound message's wire `chain_id` matches the
    /// domain's *validated* expected wire-chain id. Returns `false` (reject)
    /// on mismatch. This never rewrites the message and never derives the
    /// expected identity from the message: the expected value comes only from
    /// trusted config.
...
pub struct BinaryConsensusLoopInboundStats {
    ...
    /// Run 422 D6: outbound Proposal frames suppressed BEFORE signing/transmission
    /// because the message's wire `chain_id` disagreed with the selected v2
    /// signing domain's validated `expected_wire_chain_id`. The message is never
    /// silently rewritten to make it pass; it is refused fail-closed.
    pub outbound_proposal_wire_chain_mismatch: u64,
    /// Run 422 D6: outbound Vote frames suppressed for wire-chain mismatch. See
    /// [`Self::outbound_proposal_wire_chain_mismatch`].
    pub outbound_vote_wire_chain_mismatch: u64,
...
fn sign_proposal_for_broadcast(
    ...
    // Run 422 D6: refuse to sign/transmit a proposal whose wire chain_id
    // disagrees with the selected v2 domain's validated expected wire-chain id.
    // The message is never rewritten to make it pass — it is suppressed
    // fail-closed BEFORE the signature is computed or attached.
    if !ctx.wire_chain_id_ok(proposal.header.chain_id) {
        inbound_stats.outbound_proposal_wire_chain_mismatch = inbound_stats
            ...
            .saturating_add(1);
        eprintln!(
            "[binary-consensus] Run 422 D6: outbound proposal NOT signed/broadcast \
             (wire chain_id {} != expected) — fail-closed",
            proposal.header.chain_id
        );
        return None;
    }
...
fn sign_vote_for_broadcast(
    ...
    // Run 422 D6: refuse to sign/transmit a vote whose wire chain_id disagrees
    // with the selected v2 domain's validated expected wire-chain id. Suppressed
    // fail-closed BEFORE signing; never rewritten.
    if !ctx.wire_chain_id_ok(vote.chain_id) {
        inbound_stats.outbound_vote_wire_chain_mismatch = inbound_stats
            ...
            .saturating_add(1);
        eprintln!(
            "[binary-consensus] Run 422 D6: outbound vote NOT signed/transmitted \
             (wire chain_id {} != expected) — fail-closed",
            vote.chain_id
        );
        return None;
                ...
                Ok(proposal) => {
                    ...
                    match effective_pv {
                        Some(ctx) => {
                            ...
                            // THIS message and the trusted domain (a caller can
                            // never substitute a stale/foreign preimage), and it
                            // enforces expected wire-chain consistency BEFORE any
                            // crypto — a wire chain_id disagreeing with the
                            // domain's validated `expected_wire_chain_id` is a
                            // typed `WireChainMismatch`, fail-closed, without ever
                            // rewriting the inbound message.
                            let t_start = std::time::Instant::now();
                            let res = verify_proposal_msg_with_domain(
                                &proposal,
                                from,
                            ...
                            // Wire-chain consistency is enforced BEFORE any
                            // crypto inside `_with_domain`; a mismatch is a typed
                            // rejection that never runs the signature backend, so
                            // it must NOT be recorded as a crypto-verify latency
                            // observation.
                            if let Err(
                                e @ ProposalVoteVerifyError::WireChainMismatch { .. },
                            ) = &res
                            {
                                ...
                                eprintln!(
                                    "[binary-consensus] Run 422 D6: inbound proposal REJECTED \
                                     ...
                                     wire_chain_id={} reason={} — fail-closed, not delivered",
                                    proposal.header.height,
                                    from,
                                    proposal.header.chain_id,
                                    e,
                            ...
                            match res {
                                ...
                                Err(ProposalVoteVerifyError::WireChainMismatch { .. }) => {
                                    // Handled above (before crypto latency
                                    // recording); unreachable here.
                                    return;
                                }
                                Err(e) => {
                                    stats.inbound_proposal_verify_rejected_total = stats
                ...
                Ok(vote) => {
                    ...
                    match effective_pv {
                        Some(ctx) => {
                            ...
                            // consistency before crypto).
                            let t_start = std::time::Instant::now();
                            let res = verify_vote_msg_with_domain(
                                &vote,
                                from,
                                ctx.validators.as_ref(),
                                ctx.key_provider.as_ref(),
                            ...
                            // Wire-chain mismatch is a typed rejection enforced
                            // before any crypto; do NOT record it as a
                            // crypto-verify latency observation.
                            if let Err(e @ ProposalVoteVerifyError::WireChainMismatch { .. }) =
                                &res
                            {
                                ...
                                eprintln!(
                                    "[binary-consensus] Run 422 D6: inbound vote REJECTED \
                                     ...
                                     wire_chain_id={} reason={} — fail-closed, not delivered",
                                    vote.height, from, vote.chain_id, e,
                                );
                                return;
                            }
                            let elapsed = t_start.elapsed().as_nanos() as u64;
...
fn update_binary_view_timeout_metrics(
    ...
    metrics
        .binary_view_timeout()
        .set_run420(&BinaryViewTimeoutRun420Snapshot {
            inbound_proposal_verify_accepted: stats.inbound_proposal_verify_accepted,
            inbound_proposal_verify_rejected_total: stats.inbound_proposal_verify_rejected_total,
            inbound_proposal_rejected_signer_mismatch: stats
                .inbound_proposal_rejected_signer_mismatch,
            inbound_proposal_rejected_unknown_validator: stats
                .inbound_proposal_rejected_unknown_validator,
            inbound_proposal_rejected_missing_signature: stats
                .inbound_proposal_rejected_missing_signature,
            inbound_proposal_rejected_missing_key: stats.inbound_proposal_rejected_missing_key,
            inbound_proposal_rejected_unsupported_suite: stats
                .inbound_proposal_rejected_unsupported_suite,
            inbound_proposal_rejected_wrong_suite: stats.inbound_proposal_rejected_wrong_suite,
            inbound_proposal_rejected_bad_signature: stats.inbound_proposal_rejected_bad_signature,
            inbound_proposal_rejected_internal_error: stats
                .inbound_proposal_rejected_internal_error,
            inbound_vote_verify_accepted: stats.inbound_vote_verify_accepted,
            inbound_vote_verify_rejected_total: stats.inbound_vote_verify_rejected_total,
            inbound_vote_rejected_signer_mismatch: stats.inbound_vote_rejected_signer_mismatch,
            inbound_vote_rejected_unknown_validator: stats.inbound_vote_rejected_unknown_validator,
            inbound_vote_rejected_missing_signature: stats.inbound_vote_rejected_missing_signature,
            inbound_vote_rejected_missing_key: stats.inbound_vote_rejected_missing_key,
            inbound_vote_rejected_unsupported_suite: stats.inbound_vote_rejected_unsupported_suite,
            inbound_vote_rejected_wrong_suite: stats.inbound_vote_rejected_wrong_suite,
            inbound_vote_rejected_bad_signature: stats.inbound_vote_rejected_bad_signature,
            inbound_vote_rejected_internal_error: stats.inbound_vote_rejected_internal_error,
            proposal_vote_crypto_verify_latency_ns_total: stats
                .proposal_vote_crypto_verify_latency_ns_total,
            proposal_vote_crypto_verify_latency_observations_total: stats
                .proposal_vote_crypto_verify_latency_observations_total,
            outbound_proposal_signing_success: stats.outbound_proposal_signing_success,
...
mod tests {
    ...
    /// Run 420 (F3/F4/F8): loop-level adversarial, non-mutation and bypass
    /// tests for the inbound Proposal/Vote signature + suite verification gate
    /// and the outbound Proposal/Vote signing boundary.
    mod run420 {
        use super::*;
        use crate::metrics::NodeMetrics;
        use crate::validator_signer::{LocalKeySigner, ValidatorSigner};
        ...
        /// The canonical Run 420/D5 control v2 signing domain used by the main
        /// module's `make_ctx` and signing helpers. Run 422 D6 makes the v2
        /// domain mandatory for any `ProposalVoteAuthority`, so these fixtures
        /// sign and verify under this exact domain (there is no v1-authority
        /// path any more). `expected_wire_chain_id = 0` matches `base_header`
        /// and `base_vote`, whose wire `chain_id` is 0.
        fn d5_control_domain() -> ProposalVoteSigningDomainV2 {
            ProposalVoteSigningDomainV2::try_new(
                ChainId(0xD5D5_0000_0000_0001),
                0,
                [0x5Du8; 32],
                [0xA5u8; 32],
            )
            .expect("valid d5 control domain")
        }
        ...
        fn make_ctx(
            ...
            // Run 422 D5/D6: this Proposal/Vote verifier test module constructs
            // the *Proposal/Vote* authority explicitly (a distinct type from the
            // Timeout/NewView `TimeoutVerificationContext`). D6 makes the v2
            // signing domain mandatory, so no authority can select legacy v1
            // verification. Building this in a test proves verifier coverage
            // only; it is not a production activation route.
            ProposalVoteAuthority {
                validators: fixture.validators.clone(),
                key_provider: fixture.kp.clone(),
                backend_registry: fixture.br.clone(),
        ...
        /// delegates to the REAL ML-DSA-44 backend.
        ///
        /// Signer invocation is observed DIRECTLY. The
        /// `outbound_*_signing_success` counters record only *successful
        /// completion* of a signature, so a signer that is invoked and then
        /// returns an error would leave them at `0` — i.e. `signing_success == 0`
        /// is NOT by itself proof that the signer was never called. The
        /// dedicated recording-signer tests below wrap the real ML-DSA-44 signer
        /// in a [`RecordingSigner`] that increments a per-authority counter AT
        /// METHOD ENTRY (before delegating). A positive control proves the
        /// counter is connected to the actual cached re-emission path, and every
        /// negative asserts a literal zero invocation for the rejected message.
        /// Emitted signatures are additionally corroborated by verification
        ...
        mod run422_d7b2 {
            ...
            fn assert_proposal_only_partial(out: &ReemitOutcome) {
                ...
                assert_eq!(
                    ...
                    "no successful Vote signature was produced for the rejected Vote \
                     (direct zero-invocation of the real signer is asserted in the \
                     recording-signer tests below)"
                );
            ...
            // =============================================================
            // Run 422 D7-B2 corrective continuation (task §2–§4) — DIRECT
            // signer-invocation observation, REAL cache creation, and the
            // reachable cached-Vote / authority-selection cases.
            // =============================================================

            /// An invocation-recording [`ValidatorSigner`] wrapper. It counts
            /// every `sign_proposal` / `sign_vote` call AT METHOD ENTRY (before
            /// delegating to the real ML-DSA-44 `LocalKeySigner`), so a signer
            /// that is invoked and then returns an error is still recorded as
            /// invoked. This is precisely what `outbound_*_signing_success == 0`
            /// (a *successful-completion* count) does not establish.
            ...
            /// Dedicated call counters for one recording signer, so a test can
            /// attribute invocations to a specific supplied/bound authority.
            #[derive(Clone)]
            ...
            impl SignerCounters {
                fn new() -> Self {
                    Self {
                        proposal_calls: Arc::new(AtomicU64::new(0)),
                        vote_calls: Arc::new(AtomicU64::new(0)),
                    }
                }
                fn proposals(&self) -> u64 {
                    self.proposal_calls.load(SeqCst)
                }
                fn votes(&self) -> u64 {
                    self.vote_calls.load(SeqCst)
                }
            }

            /// Build a `ProposalVoteAuthority` whose validator-0 local signer is
            /// an invocation-recording wrapper over the real ML-DSA-44 key,
            /// returning the authority together with its OWN dedicated counters.
            fn recording_ctx_v2(
                fixture: &Fixture,
                domain: ProposalVoteSigningDomainV2,
            ) -> (ProposalVoteAuthority, SignerCounters) {
                let counters = SignerCounters::new();
                ...
                let signer: Arc<dyn ValidatorSigner> = Arc::new(RecordingSigner {
                    inner,
                    proposal_calls: counters.proposal_calls.clone(),
                    vote_calls: counters.vote_calls.clone(),
                });
            ...
            // ---- DIRECT signer-invocation observation (task §2) --------------

            #[test]
            fn d7b2_recording_signer_positive_control_directly_counts_both_invocations() {
                // POSITIVE CONTROL: prove the recording counters are connected to
                // the actual cached re-emission path — a valid cache under a
                // wired, currently-authorized snapshot invokes the REAL signer
                // exactly once for the Proposal and once for the Vote.
                let fixture = make_fixture(4);
                let domain = d6_control_domain();
                let (pv, counters) = recording_ctx_v2(&fixture, domain.clone());
                let snap = coherent_snapshot_for(&pv);

                // No signer call before the re-emission path runs.
                ...
                // Directly observed invocation on the cached re-emission path.
                assert_eq!(counters.proposals(), 1, "cached Proposal re-emission invoked the real signer once");
                assert_eq!(counters.votes(), 1, "cached Vote re-emission invoked the real signer once");
                // Corroborated by the success counters + actual facade emission +
                ...
                assert_eq!(out.facade.nproposals(), 1);
                assert_eq!(out.facade.nbroadcast_votes(), 1);
                let ep = &out.facade.proposals.lock().unwrap()[0];
                let ev = &out.facade.broadcast_votes.lock().unwrap()[0];
                assert!(qbind_consensus::verify_proposal_msg_with_domain(
                    ep,
                    ValidatorId(0),
                    fixture.validators.as_ref(),
            ...
            #[test]
            fn d7b2_recording_signer_rejected_proposal_directly_shows_zero_invocations() {
                // NEGATIVE: an owner-level current-state failure rejects the
                // Proposal before signing. The real signer is DIRECTLY observed
                // to be un-invoked for BOTH families (counter == 0, not merely
                // "no signing success").
                let fixture = make_fixture(4);
                let (pv, counters) = recording_ctx_v2(&fixture, d6_control_domain());
                let snap = snap_unavailable(&pv, CurrentStateUnavailableReason::MissingStorage);
                let out = run_reemit(
                    Some(&snap),
                    None,
                ...
                assert_eq!(counters.proposals(), 0, "rejected Proposal never reached the signer");
                assert_eq!(counters.votes(), 0, "no Vote signing attempted after Proposal rejection");
                assert_eq!(out.stats.outbound_proposal_current_state_unavailable_total, 1);
                out.facade.assert_silent();
            }

            #[test]
            fn d7b2_recording_signer_rejected_vote_directly_shows_zero_vote_invocations() {
                // NEGATIVE: an admissible Proposal control is emitted (Proposal
                // signer invoked once) so the cached-Vote branch is genuinely
                // reached, then the Vote fails (missing provenance). The Vote
                // signer is DIRECTLY observed to be un-invoked — a signer that
                // errored would still register here, so this rules out the
                // "error mistaken for not-invoked" ambiguity.
                let fixture = make_fixture(4);
                let (pv, counters) = recording_ctx_v2(&fixture, d6_control_domain());
                let snap = coherent_snapshot_for(&pv);
                let out = run_reemit(
                ...
                assert_eq!(counters.proposals(), 1, "admissible Proposal invoked the signer once");
                assert_eq!(counters.votes(), 0, "the rejected Vote never reached the signer");
                assert_eq!(out.stats.outbound_vote_reemit_missing_provenance_total, 1);
                assert_proposal_only_partial(&out);
            }

            // ---- REAL cache creation via do_leader_tick (task §3) ------------

            #[test]
            fn d7b2_do_leader_tick_creates_caches_then_real_reemission_uses_them() {
                // Drive the ACTUAL cache writer (`do_leader_tick`) to produce the
                // typed Proposal/Vote caches from a leader-generated tick, then
                // drive the ACTUAL `maybe_reemit_on_late_peer_connect` over a
                // genuine new-peer transition. Initial forwarding and later
                // re-emission are distinguished by SEPARATE facades and EXPLICIT
                // signer/stat deltas.
                let fixture = make_fixture(4);
                // The engine hard-codes wire chain_id = 1 and epoch 0 on its
                // leader-emitted Proposal/Vote, so the bound authority's v2 domain
                // must expect wire chain id 1 (founding epoch 0 is already the
                // authorized epoch).
                ...
                // The REAL cache writer created BOTH entries with originating
                // authorization captured at cache-creation.
                let cached_p = last_leader_proposal.clone().expect("proposal cached");
                let cached_v = last_leader_vote.clone().expect("vote cached");
                assert!(cached_p.provenance.is_some(), "Proposal cache carries originating provenance");
                assert!(cached_v.provenance.is_some(), "Vote cache carries originating provenance");
                assert_eq!(cached_p.view, view);
                assert_eq!(cached_v.view, view);
                // The cached messages correspond to the leader-generated actions
                // (same view/epoch; cached bodies are the pre-sign engine output).
                assert_eq!(cached_p.proposal.header.height, view);
                assert_eq!(cached_p.proposal.header.epoch, 0);
                assert_eq!(cached_v.vote.height, view);
                assert_eq!(cached_v.vote.epoch, 0);

                // Initial forwarding invoked the signer once per family.
                assert_eq!(proposals_emitted, 1);
                assert_eq!(counters.proposals(), 1, "initial forwarding signed the Proposal once");
                assert_eq!(counters.votes(), 1, "initial forwarding signed the Vote once");
                let initial_p_success = stats.outbound_proposal_signing_success;
                let initial_v_success = stats.outbound_vote_signing_success;
                assert_eq!(initial_p_success, 1);
                assert_eq!(initial_v_success, 1);
                assert_eq!(initial_facade.nproposals(), 1);
                assert_eq!(initial_facade.nbroadcast_votes(), 1);

                let prov_p_gen_before =
                    cached_p.provenance.as_ref().unwrap().ticket.generation();
                let prov_v_gen_before =
                    cached_v.provenance.as_ref().unwrap().ticket.generation();

                // ---- Later re-emission across a genuine new-peer transition ----
                let mut reemitted_for_view: Option<u64> = None;
                let mut last_known_peers: HashSet<NodeId> = HashSet::new();
                let peer = pv_node_for(1);
                let connectivity = OnePeer(peer);
                ...
                // Re-emission used the SAME cached entries (single-shot latch set;
                // both families re-emitted once, to the SEPARATE facade).
                assert_eq!(reemitted_for_view, Some(view));
                assert_eq!(stats.outbound_proposal_late_peer_reemits, 1);
                ...
                // retained entries still carry the same-generation ticket.
                let after_p = last_leader_proposal.as_ref().expect("proposal cache retained");
                let after_v = last_leader_vote.as_ref().expect("vote cache retained");
                assert_eq!(
                    after_p.provenance.as_ref().unwrap().ticket.generation(),
                    prov_p_gen_before,
                    "Proposal provenance unchanged by re-emission",
                );
                assert_eq!(
                    after_v.provenance.as_ref().unwrap().ticket.generation(),
                    prov_v_gen_before,
                    "Vote provenance unchanged by re-emission",
                );

                // Emitted (re-emitted) signatures verify under the SELECTED domain.
                let ep = &reemit_facade.proposals.lock().unwrap()[0];
                let ev = &reemit_facade.broadcast_votes.lock().unwrap()[0];
                assert!(qbind_consensus::verify_proposal_msg_with_domain(
                    ep,
                    ValidatorId(0),
                    fixture.validators.as_ref(),
            ...
            #[test]
            fn d7b2_do_leader_tick_without_current_auth_then_replay_stays_rejected() {
                // Cache creation WITHOUT current authorization (production wiring:
                // no snapshot) leaves the entries with NO provenance. A later
                // replay under an otherwise-valid authorization must NOT
                // manufacture provenance: the unproven entries stay rejected.
                let fixture = make_fixture(4);
                let domain = d6_domain(
                    0xD6D6_0000_0000_00A2,
                    1,
                    d6_genesis_identity(0x52),
                    d6_authority_commitment(0x62),
                );
                let (pv, counters) = recording_ctx_v2(&fixture, domain);
                ...
                // Cache-creation with NO snapshot (LocalFixtureUnsigned so the
                // tick still emits + caches, exactly modelling a cache produced
                // without established current authorization).
                do_leader_tick(
                    &mut engine,
                    &mut proposals_emitted,
                    &metrics,
                ...
                let cached_p = last_leader_proposal.clone().expect("proposal cached");
                assert!(
                    cached_p.provenance.is_none(),
                    "a cache created without current authorization carries NO provenance",
                );

                // Replay under an otherwise-valid current authorization.
                let snap = coherent_snapshot_for(&pv);
                let mut reemitted_for_view: Option<u64> = None;
                let mut last_known_peers: HashSet<NodeId> = HashSet::new();
                let peer = pv_node_for(1);
                let connectivity = OnePeer(peer);
                let reemit_facade = RecordingFacade::default();
                let sign_before = counters.proposals();
                maybe_reemit_on_late_peer_connect(
                    &engine,
                    &mut last_leader_proposal,
                    &mut last_leader_vote,
                ...
                // provenance and the signer is never invoked for it.
                assert_eq!(stats.outbound_proposal_reemit_missing_provenance_total, 1);
                assert_eq!(stats.outbound_proposal_late_peer_reemits, 0);
                assert_eq!(reemitted_for_view, None);
                assert_eq!(counters.proposals() - sign_before, 0, "no signer invocation on the unproven replay");
                // Provenance is still None after the replay attempt.
                assert!(last_leader_proposal.as_ref().unwrap().provenance.is_none());
                reemit_facade.assert_silent();
            }

            // ---- Reachable cached-Vote / authority-selection cases (task §4) --

            #[test]
            fn d7b2_case_a_cached_vote_generation_invalidation_across_a_b_a() {
                // CASE A — a retained Vote from generation g is invalidated once
                // the SAME owner's generation advances. A real A→B→A configuration
                // sequence runs between completed calls; a Proposal from the
                // CURRENT generation is admissible so execution reaches the cached
                // Vote; the stale-generation Vote provenance is rejected, its
                // signer is never invoked, and no Vote is emitted. A fresh
                // admission after returning to A still succeeds.
                let fixture = make_fixture(4);
                let (pv, counters) = recording_ctx_v2(&fixture, d6_control_domain());
                let mut snap = coherent_snapshot_for(&pv);

                // Config A (founding). Capture a Vote provenance at generation g.
                let stale_vote_prov = prov(&snap);

                // Real A→B→A: advance the SAME owner to config B, then back to A.
                // Each replacement advances the in-process generation.
                let ident_a = snap.owner().candidate().config_identity();
                let cid = ident_a.chain_id.clone();
                let gh = ident_a.genesis_hash;
                let commit_a = ident_a.authority_commitment;
                let vc = ident_a.validator_count;
                let ep = ident_a.epoch;
                let config_b = ObservedConsensusConfiguration::new(cid.clone(), gh, COMMIT_B, vc, ep);
                let config_a_again =
                    ObservedConsensusConfiguration::new(cid, gh, commit_a, vc, ep);
                snap.owner_mut()
                    .replace_for_fixture(LocalAuthorizationState::Established(config_b));
                snap.owner_mut()
                    .replace_for_fixture(LocalAuthorizationState::Established(config_a_again));

                // Admissible Proposal provenance captured at the CURRENT (post
                // A→B→A) generation.
                let fresh_proposal_prov = prov(&snap);
                let out = run_reemit(
                    Some(&snap),
                    None,
                    ConsensusVerificationPolicy::Required,
                    {
                        let fresh_proposal_prov = fresh_proposal_prov.clone();
                        let stale_vote_prov = stale_vote_prov.clone();
                        move |view| {
                            (
                                Some(cached_proposal(0, view, fresh_proposal_prov)),
                                Some(cached_vote(0, view, stale_vote_prov)),
                            )
                        }
                    },
                );
                assert_eq!(
                    out.stats.outbound_vote_reemit_provenance_rejected_total, 1,
                    "stale-generation Vote provenance rejected",
                );
                assert_eq!(counters.proposals(), 1, "fresh-generation Proposal signed");
                assert_eq!(counters.votes(), 0, "stale-generation Vote never reached the signer");
                assert_proposal_only_partial(&out);

                // Fresh admission after returning to A: a provenance captured NOW
                // (current generation) is admissible for the Vote too, while the
                // retained gen-g provenance stays invalid.
                let fresh_now = prov(&snap);
                let out_fresh = run_reemit(
                    Some(&snap),
                    None,
                    ConsensusVerificationPolicy::Required,
                    {
                        let fresh_now = fresh_now.clone();
                        move |view| {
                            (
                                Some(cached_proposal(0, view, fresh_now.clone())),
                                Some(cached_vote(0, view, fresh_now)),
                            )
                        }
                    },
                );
                assert_eq!(
                    out_fresh.stats.outbound_vote_late_peer_reemits, 1,
                    "fresh-generation Vote admitted after returning to A",
                );
                assert_eq!(counters.votes(), 1, "only the fresh Vote invoked the signer; the stale one never did");
            }

            #[test]
            fn d7b2_case_b_cached_vote_wire_chain_rejection_partial_outcome() {
                // CASE B — an admissible Proposal control is emitted, then the
                // cached Vote reaches its branch with valid current authorization
                // + provenance but carries inconsistent wire-chain metadata
                // (wire chain_id 1 vs the domain's expected 0). The Vote is
                // refused for the wire-mismatch reason BEFORE signing; the signer
                // is never invoked; the Proposal-only partial outcome is exact.
                // This is distinct from the Proposal-first inconsistent-wire test.
                let fixture = make_fixture(4);
                let (pv, counters) = recording_ctx_v2(&fixture, d6_control_domain());
                let snap = snap_matching(&pv);
                let out = run_reemit(
                    ...
                    ConsensusVerificationPolicy::Required,
                    |view| {
                        let proposal = cached_proposal(0, view, prov(&snap));
                        let mut v = base_vote(0);
                        v.epoch = 0;
                        v.chain_id = 1; // inconsistent wire chain id vs domain wire 0
                        let vote = CachedLeaderVote {
                            view,
                            vote: v,
                            provenance: prov(&snap),
                        };
                        (Some(proposal), Some(vote))
                    },
                );
                assert_eq!(
                    out.stats.outbound_vote_wire_chain_mismatch, 1,
                    "cached Vote refused for wire-chain mismatch",
                );
                assert_eq!(counters.proposals(), 1);
                assert_eq!(counters.votes(), 0, "wire-mismatched Vote refused BEFORE signing");
                assert_proposal_only_partial(&out);
            }

            #[test]
            fn d7b2_case_c_bound_a_signs_supplied_b_never_invoked() {
                // CASE C — a valid bound snapshot A and a distinct, separately
                // instrumented supplied authority B are provided simultaneously.
                // Under Required, A signs both families; B is never invoked; the
                // emitted signatures verify under A's domain and FAIL under B's
                // foreign domain and the legacy v1 boundary.
                let fixture = make_fixture(4);
                let domain_a = d6_control_domain();
                let (pv_a, counters_a) = recording_ctx_v2(&fixture, domain_a.clone());
                let domain_b = d6_domain(
                    0xB0B0_0000_0000_0002,
                    0,
                    d6_genesis_identity(0x77),
                    d6_authority_commitment(0x88),
                );
                let (pv_b, counters_b) = recording_ctx_v2(&fixture, domain_b.clone());
                ...
                let out = run_reemit(
                    ...
                    |view| {
                        (
                            Some(cached_proposal(0, view, prov(&snap_a))),
                            Some(cached_vote(0, view, prov(&snap_a))),
                        )
                    },
                );

                assert_eq!(counters_a.proposals(), 1, "bound A signed the Proposal");
                assert_eq!(counters_a.votes(), 1, "bound A signed the Vote");
                assert_eq!(counters_b.proposals(), 0, "supplied B never signed the Proposal");
                assert_eq!(counters_b.votes(), 0, "supplied B never signed the Vote");
                assert_eq!(out.stats.outbound_proposal_late_peer_reemits, 1);
                assert_eq!(out.stats.outbound_vote_late_peer_reemits, 1);

                let ep = &out.facade.proposals.lock().unwrap()[0];
                let ev = &out.facade.broadcast_votes.lock().unwrap()[0];
                // Verify under A's domain ...
                assert!(qbind_consensus::verify_proposal_msg_with_domain(
                    ep, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
                    fixture.br.as_ref(), &domain_a,
                )
                .is_ok());
                assert!(qbind_consensus::verify_vote_msg_with_domain(
                    ev, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
                    fixture.br.as_ref(), &domain_a,
                )
                .is_ok());
                // ... FAIL under B's foreign domain ...
                assert!(qbind_consensus::verify_proposal_msg_with_domain(
                    ep, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
                    fixture.br.as_ref(), &domain_b,
                )
                .is_err());
                assert!(qbind_consensus::verify_vote_msg_with_domain(
                    ev, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
                ...
                .is_err());
                // ... and FAIL under the legacy v1 boundary.
                assert!(qbind_consensus::verify_proposal_msg(
                    ep, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
                    fixture.br.as_ref(), QBIND_DEVNET_CHAIN_ID,
                )
                .is_err());
                assert!(qbind_consensus::verify_vote_msg(
                    ev, ValidatorId(0), fixture.validators.as_ref(), fixture.kp.as_ref(),
            ...
            #[test]
            fn d7b2_case_d_shared_candidate_arc_still_isolates_by_issuer() {
                // CASE D — two owners built from clones of the EXACT SAME
                // candidate Arc (asserted via `Arc::ptr_eq`). Even with a shared
                // candidate handle, a cache minted under owner A offered while
                // owner B is current is refused (foreign issuer): shared candidate
                // structure does not launder foreign cached authorization.
                let fixture = make_fixture(4);
                let (pv, counters) = recording_ctx_v2(&fixture, d6_control_domain());

                let candidate = coherent_authority_for(&pv);
                let owner_a = CurrentAuthorizationOwner::establish_for_fixture(
                    candidate.clone(),
                    candidate.config_identity(),
                );
                let owner_b = CurrentAuthorizationOwner::establish_for_fixture(
                    candidate.clone(),
                    candidate.config_identity(),
                );
                // The two owners share the SAME candidate allocation.
                assert!(Arc::ptr_eq(owner_a.candidate(), owner_b.candidate()));
                assert!(Arc::ptr_eq(owner_a.candidate(), &candidate));

                let snap_a = AuthorizedProposalVoteSnapshot::try_bind(owner_a, Arc::new(pv.clone()))
                    .expect("bind A");
                let snap_b = AuthorizedProposalVoteSnapshot::try_bind(owner_b, Arc::new(pv.clone()))
                    .expect("bind B");
                ...
                let out = run_reemit(
                    ...
                    ConsensusVerificationPolicy::Required,
                    {
                        let prov_a = prov_a.clone();
                        move |view| {
                            (
                                Some(cached_proposal(0, view, prov_a)),
                                Some(cached_vote(0, view, None)),
                            )
                        }
                    },
                ...
                assert_eq!(
                    out.stats.outbound_proposal_reemit_provenance_rejected_total, 1,
                    "foreign-issuer provenance refused despite shared candidate Arc",
                );
                assert_eq!(counters.proposals(), 0, "foreign-issuer cache never reached the signer");
                assert_eq!(out.stats.outbound_proposal_late_peer_reemits, 0);
                out.facade.assert_silent();
            }
        }
        ...
        /// Build a proposal signed by `proposer`'s consensus key over the
        /// mandatory v2 control-domain preimage (Run 422 D6).
        fn signed_proposal(proposer: u16, fixture: &Fixture) -> BlockProposal {
        ...
        /// Build a proposal signed by `proposer` over the LEGACY v1 chain-aware
        /// preimage. Retained only for explicit legacy-consumer regression
        /// tests (e.g. cross-format v1→v2 rejection); the v2 authority path
        /// never accepts these.
        fn signed_proposal_v1_legacy(proposer: u16, fixture: &Fixture) -> BlockProposal {
            let mut p = BlockProposal {
                header: base_header(proposer),
                qc: None,
                txs: vec![],
        ...
        /// `proposer`'s consensus key over the mandatory v2 control-domain
        /// preimage. Used by the active-restore-mode case so the frame is shaped
        /// (height above committed+1) to reach restore deferral if admitted.
        ...
        /// Build a vote signed by `voter`'s consensus key over the mandatory v2
        /// control-domain preimage (Run 422 D6).
        fn signed_vote(voter: u16, fixture: &Fixture) -> Vote {
        ...
        /// Build a vote signed by `voter` over the LEGACY v1 chain-aware
        /// preimage. Retained only for explicit legacy-consumer regression
        /// tests; the v2 authority path never accepts these.
        fn signed_vote_v1_legacy(voter: u16, fixture: &Fixture) -> Vote {
            let mut v = base_vote(voter);
        ...
        #[test]
        fn run420_inbound_valid_signed_proposal_verify_accepted() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            deliver_proposal(&mut engine, &mut stats, Some(&ctx), &p, &metrics);
        ...
        #[test]
        fn run420_inbound_valid_signed_vote_verify_accepted() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            deliver_vote(&mut engine, &mut stats, Some(&ctx), &v, &metrics);
        ...
        #[test]
        fn run420_valid_messages_survive_encode_decode_and_verify_identically() {
            use qbind_wire::io::{WireDecode, WireEncode};
            let fixture = make_fixture(4);
            let p = signed_proposal(2, &fixture);
            let mut pb = Vec::new();
            p.encode(&mut pb);
            let mut ps: &[u8] = &pb;
            let p2 = BlockProposal::decode(&mut ps).expect("decode");
            assert_eq!(p, p2);
            assert!(qbind_consensus::verify_proposal_msg_with_domain(
                &p2,
                ValidatorId(2),
        ...
        // =================================================================
        // SIGNATURE / SUITE / AUTHORITY FAILURES (inbound gate)
        // =================================================================

        #[test]
        fn run420_inbound_unsigned_proposal_rejected_before_engine() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
        ...
        #[test]
        fn run420_inbound_bitflipped_vote_signature_rejected() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            assert_eq!(stats.inbound_vote_verify_rejected_total, 1);
            assert_eq!(stats.inbound_vote_rejected_bad_signature, 1);
            assert_eq!(stats.inbound_votes_delivered, 0);
        }

        #[test]
        fn run420_inbound_wrong_validator_signature_rejected() {
            // Valid signature, but signed by V2 while the message claims V1.
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            // sign with V2's key over the v2 control-domain preimage but keep
            // proposer_index = 1 (wrong-signer isolation under the v2 boundary).
            let preimage = d5_control_domain().proposal_preimage(&proposal);
            let sk2 = fixture.sks.get(&ValidatorId(2)).unwrap();
            proposal.signature = MlDsa44Backend::sign(sk2, &preimage).expect("sign");
            deliver_proposal(&mut engine, &mut stats, Some(&ctx), &proposal, &metrics);
        ...
        #[test]
        fn run420_inbound_unknown_validator_proposal_rejected() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            // proposer_index = 9 (not in the 4-validator set). Signature bytes
            // are irrelevant: membership is checked before crypto.
            let mut proposal = BlockProposal {
                header: base_header(9),
                qc: None,
                txs: vec![],
                signature: vec![1u8; 8],
            };
            proposal.header.proposer_index = 9;
            deliver_proposal(&mut engine, &mut stats, Some(&ctx), &proposal, &metrics);

            assert_eq!(stats.inbound_proposal_verify_rejected_total, 1);
            assert_eq!(stats.inbound_proposal_rejected_unknown_validator, 1);
            assert_eq!(stats.inbound_proposals_delivered, 0);
        }

        #[test]
        fn run420_inbound_wrong_suite_proposal_rejected() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            // Sign correctly, then alter the wire suite_id after signing.
            let mut p = signed_proposal(1, &fixture);
            p.header.suite_id = TEST_SUITE_U16 + 7;
            deliver_proposal(&mut engine, &mut stats, Some(&ctx), &p, &metrics);

            assert_eq!(stats.inbound_proposal_verify_rejected_total, 1);
            assert_eq!(stats.inbound_proposal_rejected_wrong_suite, 1);
            assert_eq!(stats.inbound_proposals_delivered, 0);
        }

        #[test]
        fn run420_inbound_wrong_suite_vote_rejected() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            let mut v = signed_vote(1, &fixture);
            v.suite_id = TEST_SUITE_U16 + 7;
            deliver_vote(&mut engine, &mut stats, Some(&ctx), &v, &metrics);

            assert_eq!(stats.inbound_vote_verify_rejected_total, 1);
            assert_eq!(stats.inbound_vote_rejected_wrong_suite, 1);
            assert_eq!(stats.inbound_votes_delivered, 0);
        }

        #[test]
        fn run420_inbound_proposal_relabeled_as_vote_rejected() {
            // A valid proposal signature applied to a vote preimage must not
            // verify (domain separation): sign the proposal preimage, then
            // attach it to a vote with the same fields.
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            v.signature = p.signature.clone(); // proposal signature on a vote
            deliver_vote(&mut engine, &mut stats, Some(&ctx), &v, &metrics);
        ...
        #[test]
        fn run420_inbound_changed_payload_proposal_rejected() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            let mut p = signed_proposal(1, &fixture);
            p.header.payload_hash = [0xAAu8; 32]; // mutate signed field post-sign
            deliver_proposal(&mut engine, &mut stats, Some(&ctx), &p, &metrics);
        ...
        // =================================================================
        // ORDERING / NON-MUTATION
        // =================================================================

        #[test]
        fn run420_rejected_proposal_does_not_advance_view() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
        ...
        #[test]
        fn run420_malformed_proposal_bytes_do_not_panic_or_advance() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            handle_inbound_consensus_msg(
                &mut engine,
                ConsensusNetMsg::Proposal(vec![0xffu8; 12]),
                &mut stats,
                None,
                &metrics,
                ValidatorId(0),
        ...
        // =================================================================
        // OUTBOUND SIGNING BOUNDARY
        // =================================================================

        #[test]
        fn run420_outbound_signs_proposal_and_vote_when_signer_present() {
            let fixture = make_fixture(4);
            ...
            // The signed proposal self-verifies against the same context's domain.
            assert!(qbind_consensus::verify_proposal_msg_with_domain(
                &signed_p,
                ValidatorId(0),
                ctx.validators.as_ref(),
        ...
        #[test]
        fn run420_outbound_fail_closed_when_signer_missing() {
            let fixture = make_fixture(4);
            // ctx with NO signer: a verifying node without a configured signer
            // must NOT emit unsigned proposals/votes.
            let ctx = make_ctx(&fixture, None);
            let mut stats = BinaryConsensusLoopInboundStats::default();
        ...
        #[test]
        fn run420_outbound_no_ctx_returns_unchanged() {
            // Legacy / single-validator path (ctx == None): no signing gate;
            // the message is returned unchanged and the inbound gate is also
            // disabled, so no unsigned message is admitted at a verifying peer.
            let mut stats = BinaryConsensusLoopInboundStats::default();
            let unsigned_p = BlockProposal {
                header: base_header(0),
                qc: None,
        ...
        // =================================================================
        // BYPASS: no-ctx inbound path does not run the gate (documents that
        // production activation is gated on a wired verification context).
        // =================================================================

        #[test]
        fn run420_inbound_no_ctx_does_not_run_verify_gate() {
            let fixture = make_fixture(4);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            let metrics = make_metrics();
            ...
            let mut p = signed_proposal(1, &fixture);
            p.signature.clear();
            // ctx = None → gate disabled (legacy path). The verify counters
            // stay at zero (the message is handled by the pre-Run-420 path).
            deliver_proposal(&mut engine, &mut stats, None, &p, &metrics);
            assert_eq!(stats.inbound_proposal_verify_accepted, 0);
            assert_eq!(stats.inbound_proposal_verify_rejected_total, 0);
        }
        ...
        #[test]
        fn run420_metrics_exposition_renders_all_counters() {
            let fixture = make_fixture(4);
            let ctx = make_ctx(&fixture, None);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
        ...
        // =================================================================
        // Run 422 D6 — versioned Proposal/Vote signing-domain isolation at
        // the REAL node inbound/outbound boundary.
        //
        // These reuse the real ML-DSA-44 backend, a real `PeerConsensusBinding
        // Gate` + matching `AuthenticatedConsensusOrigin`, the `Required`
        // policy, and an explicit Proposal/Vote authority whose
        // `signing_domain` selects the versioned v2 format. They remain
        // FIXTURE tests: production `main` never constructs a
        // `ProposalVoteAuthority`, so none of this is a production activation
        // route.
        // =================================================================

        /// A non-zero fixture genesis identity (never all-zero: the domain
        /// constructor rejects zeros, and zeros must never be a default).
        fn d6_genesis_identity(seed: u8) -> [u8; 32] {
            let mut g = [0u8; 32];
        ...
        /// A non-zero fixture authority commitment.
        fn d6_authority_commitment(seed: u8) -> [u8; 32] {
            let mut c = [0u8; 32];
            for (i, b) in c.iter_mut().enumerate() {
                *b = seed.wrapping_mul(3).wrapping_add(i as u8).wrapping_add(7);
            }
            c
        }

        /// Build a validated v2 signing domain fixture. `wire` is the expected
        /// wire `chain_id`; the base fixture headers/votes carry `chain_id = 0`,
        /// so the positive control uses `wire = 0`.
        fn d6_domain(
            runtime: u64,
            wire: u32,
            genesis: [u8; 32],
            ...
            ProposalVoteSigningDomainV2::try_new(ChainId(runtime), wire, genesis, commitment)
                .expect("valid v2 domain fixture")
        }

        /// The canonical positive-control v2 domain used by the D6 handler
        /// tests (runtime chain id, wire chain id 0 to match base fixtures).
        fn d6_control_domain() -> ProposalVoteSigningDomainV2 {
            d6_domain(
                0xD6D6_0000_0000_0001,
                0,
        ...
        /// A `ProposalVoteAuthority` whose Proposal/Vote boundary is bound to
        /// `domain` (v2 format), optionally with a local signer.
        fn make_ctx_v2(
            fixture: &Fixture,
            local_signer_for: Option<ValidatorId>,
            domain: ProposalVoteSigningDomainV2,
        ...
        /// A proposal signed by `proposer` over `domain`'s v2 preimage.
        fn signed_proposal_v2(
            proposer: u16,
            fixture: &Fixture,
            domain: &ProposalVoteSigningDomainV2,
        ) -> BlockProposal {
        ...
        /// A vote signed by `voter` over `domain`'s v2 preimage.
        fn signed_vote_v2(
            voter: u16,
            fixture: &Fixture,
            domain: &ProposalVoteSigningDomainV2,
        ...
        // engine mutation, or outbound effect.
        #[test]
        fn run422_d6_correct_domain_proposal_accepted_foreign_rejected() {
            let fixture = make_fixture(4);
            let domain = d6_control_domain();
            let ctx = make_ctx_v2(&fixture, None, domain.clone());
            let gate = pv_binding_gate(4);
            let origin = AuthenticatedConsensusOrigin::new(pv_node_for(0), ValidatorId(0));
            let timeout_ctx = make_timeout_ctx(&fixture, Some(ValidatorId(0)));

            // Positive control: signed under the SAME domain the authority uses.
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            let metrics = make_metrics();
            let p_ok = signed_proposal_v2(0, &fixture, &domain);
            deliver_proposal_combined(
                &mut engine,
                &mut stats,
                Some(&timeout_ctx),
            ...
            assert_eq!(stats.inbound_proposal_verify_accepted, 1);
            assert_eq!(stats.inbound_proposal_verify_rejected_total, 0);
            assert_eq!(stats.proposal_vote_crypto_verify_latency_observations_total, 1);

            // Negative: SAME key, SAME message, SAME wire chain_id — but signed
            // under a FOREIGN domain (different full runtime chain id). Rejected
            // before delivery / engine mutation / outbound action.
            let foreign = d6_domain(
                0xEEEE_0000_0000_0009, // different full runtime chain id
                0,
                d6_genesis_identity(0x11),
                d6_authority_commitment(0x22),
            );
            let p_foreign = signed_proposal_v2(0, &fixture, &foreign);
            let mut engine2 = make_engine(ValidatorId(0), 4);
            let view_before = engine2.current_view();
            ...
            // F6 still admits (real gate); crypto ran and rejected as a bad
            // signature (the foreign preimage does not match).
            assert_eq!(gate.metrics().accepted(), 2);
            assert_eq!(stats2.inbound_sender_binding_rejected_total, 0);
            assert_eq!(stats2.inbound_proposal_verify_accepted, 0);
            assert_eq!(stats2.inbound_proposal_verify_rejected_total, 1);
            assert_eq!(stats2.inbound_proposal_rejected_bad_signature, 1);
            // No delivery, engine acceptance, restore deferral, reconfig
            // observation, or outbound effect.
            assert_eq!(stats2.inbound_proposals_delivered, 0);
            assert_eq!(stats2.inbound_proposals_engine_accepted, 0);
            assert_eq!(stats2.restore_catchup_proposals_deferred, 0);
            assert_eq!(stats2.outbound_votes_sent, 0);
            assert!(detector.header_cache.is_empty());
            assert_eq!(engine2.current_view(), view_before);
        }

        // D6-2: The Vote analogue of D6-1.
        #[test]
        fn run422_d6_correct_domain_vote_accepted_foreign_rejected() {
            let fixture = make_fixture(4);
            let domain = d6_control_domain();
            let ctx = make_ctx_v2(&fixture, None, domain.clone());
            let gate = pv_binding_gate(4);
            ...
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            let v_ok = signed_vote_v2(0, &fixture, &domain);
            deliver_vote_combined(
                &mut engine,
                &mut stats,
                Some(&timeout_ctx),
            ...
            // Negative: same key/message, foreign genesis identity only.
            let foreign = d6_domain(
                0xD6D6_0000_0000_0001,
                0,
                d6_genesis_identity(0x99), // different accepted genesis identity
                d6_authority_commitment(0x22),
            );
            let v_foreign = signed_vote_v2(0, &fixture, &foreign);
            let mut engine2 = make_engine(ValidatorId(0), 4);
            let mut stats2 = BinaryConsensusLoopInboundStats::default();
            deliver_vote_combined(
                &mut engine2,
            ...
            assert_eq!(stats2.inbound_vote_verify_rejected_total, 1);
            assert_eq!(stats2.inbound_vote_rejected_bad_signature, 1);
            assert_eq!(stats2.inbound_votes_delivered, 0);
            assert_eq!(stats2.inbound_votes_engine_accepted, 0);
        }

        // D6-3: An inbound message whose wire chain_id disagrees with the
        // domain's validated expected wire-chain id is rejected BEFORE any
        // cryptographic verification, and the message is never rewritten.
        #[test]
        fn run422_d6_wire_chain_mismatch_rejected_before_crypto() {
            let fixture = make_fixture(4);
            // Domain expects wire chain_id == 7.
            let domain = d6_domain(
                0xD6D6_0000_0000_0001,
                7,
                d6_genesis_identity(0x11),
            ...
            // Proposal carries wire chain_id 0 (base header) != expected 7,
            // yet is otherwise correctly signed under the domain.
            let mut p = BlockProposal {
                header: base_header(0),
                qc: None,
                txs: vec![],
            ...
            assert_eq!(p.header.chain_id, 0);
            let preimage = domain.proposal_preimage(&p);
            let sk = fixture.sks.get(&ValidatorId(0)).expect("sk");
            p.signature = MlDsa44Backend::sign(sk, &preimage).expect("sign");
            ...
            // Rejected for wire-chain mismatch, and crypto never ran.
        ...
        // D6-4: Missing PV authority still rejects even with a valid Timeout
        // context (message-family separation), and F6 mismatch still wins
        // before any domain/crypto processing.
        #[test]
        fn run422_d6_f6_mismatch_precedes_domain_and_missing_authority_rejects() {
            let fixture = make_fixture(4);
            let domain = d6_control_domain();
            let ctx = make_ctx_v2(&fixture, None, domain.clone());
            let gate = pv_binding_gate(4);
            ...
            // F6 mismatch: origin authenticated as validator 1, but the message
            // is a proposal from validator 0. Rejected by the binding gate
            // before any domain/crypto work, even though a v2 authority exists.
            let origin_mismatch =
                AuthenticatedConsensusOrigin::new(pv_node_for(1), ValidatorId(1));
            let p = signed_proposal_v2(0, &fixture, &domain);
            let mut engine = make_engine(ValidatorId(0), 4);
            let mut stats = BinaryConsensusLoopInboundStats::default();
            deliver_proposal_combined(
                &mut engine,
            ...
            // Missing PV authority + valid Timeout context: still rejected
            // authority-unavailable under Required, even though the Timeout
            // context is fully valid.
            let origin_ok = AuthenticatedConsensusOrigin::new(pv_node_for(0), ValidatorId(0));
            let mut engine2 = make_engine(ValidatorId(0), 4);
            let mut stats2 = BinaryConsensusLoopInboundStats::default();
            deliver_proposal_combined(
                &mut engine2,
            ...
            assert_eq!(stats2.inbound_proposal_verification_context_unavailable_total, 1);
            assert_eq!(stats2.inbound_proposal_verify_accepted, 0);
        }

        // D6-5: Outbound Proposal/Vote signatures produced under the selected
        // v2 domain verify under that domain and FAIL under a foreign domain.
        // No legacy-preimage fallback is used.
        #[test]
        fn run422_d6_outbound_signs_under_selected_domain_only() {
            let fixture = make_fixture(4);
            let domain = d6_control_domain();
            let ctx = make_ctx_v2(&fixture, Some(ValidatorId(0)), domain.clone());
            let mut stats = BinaryConsensusLoopInboundStats::default();
            ...
            let signed_p = sign_proposal_for_broadcast(
                unsigned_p,
                Some(&ctx),
                ConsensusVerificationPolicy::Required,
                &mut stats,
            )
            .expect("signed under v2 domain");
            assert_eq!(stats.outbound_proposal_signing_success, 1);
            // Verifies under the SELECTED v2 domain (message-bound verifier
            // recomputes the preimage internally).
            assert!(qbind_consensus::verify_proposal_msg_with_domain(
                &signed_p,
                ValidatorId(0),
                ctx.validators.as_ref(),
            ...
            .is_ok());
            // FAILS under a foreign domain (different commitment, same wire).
            let foreign = d6_domain(
                0xD6D6_0000_0000_0001,
                0,
                d6_genesis_identity(0x11),
                d6_authority_commitment(0x77),
            );
            assert!(qbind_consensus::verify_proposal_msg_with_domain(
                &signed_p,
                ValidatorId(0),
            ...
            .is_err());
            // FAILS under the legacy v1 boundary: no fallback path exists.
            assert!(qbind_consensus::verify_proposal_msg(
                &signed_p,
                ValidatorId(0),
                ctx.validators.as_ref(),
            ...
            // Outbound vote: same properties.
            let signed_v = sign_vote_for_broadcast(
                base_vote(0),
                Some(&ctx),
                ConsensusVerificationPolicy::Required,
                &mut stats,
        ...
        // D6-6: A legacy v1 signature is NOT accepted by the v2 boundary, and
        // a v2 signature is NOT accepted by the legacy v1 boundary (cross-
        // format rejection, both directions).
        #[test]
        fn run422_d6_cross_format_rejection_both_directions() {
            let fixture = make_fixture(4);
            let domain = d6_control_domain();

            // Legacy v1-signed proposal presented to the v2 boundary.
            let p_v1 = signed_proposal_v1_legacy(0, &fixture); // signs v1 chain-aware preimage
            assert!(qbind_consensus::verify_proposal_msg_with_domain(
                &p_v1,
                ValidatorId(0),
                fixture.validators.as_ref(),
            ...
            // v2-signed proposal presented to the legacy v1 boundary.
            let p_v2 = signed_proposal_v2(0, &fixture, &domain);
            assert!(qbind_consensus::verify_proposal_msg(
                &p_v2,
                ValidatorId(0),
                fixture.validators.as_ref(),
