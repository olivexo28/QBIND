//! Run 315 — source/test epoch-transition **runtime handoff** /
...
//! that consumes a **verified** Run 313/314 non-mutating guarded
//! epoch-transition mutation-execution *decision* (the accepted
//! [`ProductionEpochTransitionRuntimeHandoffDecision`] carrying a prepared
//! [`ProductionEpochTransitionRuntimeHandoffPackage`], as
...
//! mutation record does it authorize?", Run 315 answers the next question:
...
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 315)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 316**.
...
//! * Only a **verified** Run 313/314 guarded epoch-transition
//!   mutation-execution decision that `is_accept()` and carries a
//!   [`ProductionEpochTransitionRuntimeHandoffPackage`] can authorize a runtime
...
//!   [`ProductionLiveEpochTransitionExecutionPreparationArtifact`]; it
...
use crate::pqc_production_epoch_transition_runtime_handoff::{
    EpochTransitionRuntimeHandoffKind, ProductionEpochTransitionRuntimeHandoffDecision,
...
/// Run 315 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_PROTOCOL_VERSION: u16 = 1;

/// Run 315 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:run315-epoch-transition-runtime-handoff-intent:v1";

/// Run 315 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_ID_DOMAIN_TAG: &str =
    "QBIND:run315-epoch-transition-runtime-handoff-id:v1";

/// Run 315 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:run315-epoch-transition-runtime-handoff-request:v1";

/// Run 315 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:run315-epoch-transition-runtime-handoff-transcript:v1";
...
/// Run 315 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionExecutionPreparationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionExecutionPreparationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionExecutionPreparationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionExecutionPreparation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 313/314 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionExecutionPreparation` and
/// `MainnetProductionLiveEpochTransitionExecutionPreparationRequired` are
...
pub enum ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionExecutionPreparation,
    ...
    RequireProductionLiveEpochTransitionExecutionPreparation,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionExecutionPreparationRequired,
}

impl ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionExecutionPreparation => {
                "allow-source-test-epoch-transition-runtime-handoff"
            }
            Self::RequireProductionLiveEpochTransitionExecutionPreparation => {
                "require-production-epoch-transition-runtime-handoff"
            }
            Self::MainnetProductionLiveEpochTransitionExecutionPreparationRequired => {
...
/// `SourceTestLiveEpochTransitionExecutionPreparation` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionExecutionPreparation` kind is fail-closed as
/// unavailable in Run 315 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionExecutionPreparationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionExecutionPreparation,
    /// Reserved production authorization kind. Fail-closed in Run 315.
    ProductionLiveEpochTransitionExecutionPreparation,
}

impl ProductionLiveEpochTransitionExecutionPreparationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionExecutionPreparation => {
                "source-test-epoch-transition-runtime-handoff"
            }
            Self::ProductionLiveEpochTransitionExecutionPreparation => {
...
pub struct ProductionLiveEpochTransitionExecutionPreparationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionExecutionPreparationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionExecutionPreparationExecutorKind,
}

impl ProductionLiveEpochTransitionExecutionPreparationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionExecutionPreparationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionExecutionPreparationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionExecutionPreparationExecutorKind::SourceTestLiveEpochTransitionExecutionPreparation,
        )
    }
...
impl Default for ProductionLiveEpochTransitionExecutionPreparationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionExecutionPreparationExecutorKind::Disabled)
    }
}
...
/// Run 315 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 313/314
/// [`EpochTransitionRuntimeHandoffKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionExecutionPreparationKind {
...
impl LiveEpochTransitionExecutionPreparationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 315 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 313/314 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(kind: EpochTransitionRuntimeHandoffKind) -> Self {
        use EpochTransitionRuntimeHandoffKind as A;
        match kind {
...
/// Run 315 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedRuntimeHandoffDecision`] carrying a Run 313/314
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionExecutionPreparationAuthoritySource {
    /// A verified Run 313/314 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedRuntimeHandoffDecision {
        decision: ProductionEpochTransitionRuntimeHandoffDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingRuntimeHandoffDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedRuntimeHandoffDecision {
        decision: ProductionEpochTransitionRuntimeHandoffDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedRuntimeHandoffWithoutPackage {
        decision: ProductionEpochTransitionRuntimeHandoffDecision,
    },
    /// A Run 311/312 guarded epoch-transition mutation-execution decision
    /// presented directly, without a Run 313/314 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutRuntimeHandoff,
    /// A Run 309/310 staged-application decision presented directly, without a
    /// Run 313/314 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutRuntimeHandoff,
    /// A Run 307/308 live-application authorization presented directly, without
    /// a Run 313/314 guarded mutation-execution decision. Rejected.
    LiveApplicationAuthorizationWithoutRuntimeHandoff,
    /// A Run 305/306 validator-set rotation *application decision* presented
    /// directly, without a Run 313/314 guarded mutation-execution decision.
    /// Rejected.
    ApplicationDecisionWithoutRuntimeHandoff,
    /// A Run 303/304 validator-set rotation plan presented directly, without a
    /// Run 313/314 live-application authorization. Rejected.
    RotationPlanWithoutRuntimeHandoff,
    /// A Run 301/302 governance execution intent presented directly, without a
    /// Run 313/314 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutRuntimeHandoff,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 307/308 live-application authorization. Rejected.
    GovernanceProofWithoutRuntimeHandoff,
...
/// Run 315 — the explicit trusted inputs the executor binds a verified
/// Run 313/314 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExecutionPreparationInputs {
    ...
    pub preparation_policy_id: String,
    /// Expected Run 313/314 live-application authorization policy id (bound
    ...
    /// Expected Run 313/314 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 313/314 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 313/314 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 313/314 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 313/314 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 313/314 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 313/314 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 313/314 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 313/314 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 313/314 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 311/312 guarded mutation-execution decision id (re-exposed
    /// by the consumed Run 313/314 runtime handoff package).
    ...
    /// Expected Run 313/314 runtime handoff decision id (bound into the
    ...
    /// Expected Run 313/314 runtime handoff package/content digest (the
    /// consumed decision's `preparation_digest`, i.e. the handoff content
    /// digest).
    ...
    /// wall-clock value and never a reject path in Run 315.
...
impl ProductionLiveEpochTransitionExecutionPreparationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.preparation_policy_id.is_empty()
...
/// Run 315 — a runtime handoff request: the authority source (a verified
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExecutionPreparationRequest {
    pub authority_source: LiveEpochTransitionExecutionPreparationAuthoritySource,
...
impl ProductionLiveEpochTransitionExecutionPreparationRequest {
    ...
    pub fn new(
        authority_source: LiveEpochTransitionExecutionPreparationAuthoritySource,
        proposed_epoch_transition_target: u64,
        execution_preparation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 315 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionExecutionPreparationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionExecutionPreparationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionExecutionPreparationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionExecutionPreparationReplaySet;

impl LiveEpochTransitionExecutionPreparationReplaySet
    for EmptyLiveEpochTransitionExecutionPreparationReplaySet
{
...
/// Run 315 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 316+); Run 315 never
...
/// preconditions, and carries deterministic `preparation_id`, `request_id`,
/// `preparation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExecutionPreparationArtifact {
    pub staged_kind: LiveEpochTransitionExecutionPreparationKind,
    pub protocol_version: u16,
    pub preparation_policy_id: String,

    // ---- Re-exposed Run 313/314 authorization intent tuple ------------
    ...
    // ---- Bound Run 313/314 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 313/314 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 313/314 guarded mutation-execution authority tuple -
    ...
    /// The re-exposed Run 311/312 guarded mutation-execution nonce.
    ...
    /// The newly proposed execution-preparation nonce.
    pub execution_preparation_nonce: u64,
...
impl ProductionLiveEpochTransitionExecutionPreparationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`preparation_id`,
    /// `request_id`, `preparation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"preparation_policy_id",
            self.preparation_policy_id.as_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 315 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_execution_preparation_content_digest(
    package: &ProductionLiveEpochTransitionExecutionPreparationArtifact,
) -> String {
    package.content_digest()
}

/// Run 315 — deterministic, domain-separated runtime handoff request id
...
pub fn production_live_epoch_transition_execution_preparation_request_id(
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG.as_bytes());
    ...
    hash_field(
        &mut h,
        b"preparation_policy_id",
        preparation_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
        b"epoch_transition_target",
    ...
    hash_field(
        &mut h,
        b"execution_preparation_nonce",
        &execution_preparation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 315 — deterministic, domain-separated runtime handoff id binding the
...
/// request id.
pub fn production_live_epoch_transition_execution_preparation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    preparation_policy_id: &str,
    epoch_transition_target: u64,
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_ID_DOMAIN_TAG.as_bytes());
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
        b"guarded_mutation_intent_digest",
...
/// Run 315 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_execution_preparation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 315 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionExecutionPreparation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionExecutionPreparationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionExecutionPreparationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionExecutionPreparationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionExecutionPreparationUnavailable,
    ...
    AcceptedSourceTestLiveEpochTransitionExecutionPreparation {
        preparation_kind: LiveEpochTransitionExecutionPreparationKind,
    ...
    VerifiedRuntimeHandoffDecisionRequired,
    UnverifiedRuntimeHandoffDecisionRejected,
    ...
    RuntimeHandoffDecisionIntegrityMismatch,
    ...
    LiveEpochTransitionExecutionPreparationAmbiguous {
        reason: String,
    },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionExecutionPreparationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionExecutionPreparationBoundaryUnavailable => {
                "live-epoch-transition-execution-preparation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionExecutionPreparationUnavailable => {
                "production-live-epoch-transition-execution-preparation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionExecutionPreparationUnavailable => {
                "mainnet-production-live-epoch-transition-execution-preparation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionExecutionPreparation { .. } => {
                "accepted-source-test-live-epoch-transition-execution-preparation"
            }
            Self::VerifiedRuntimeHandoffDecisionRequired => {
                "verified-runtime-handoff-decision-required"
            }
            Self::UnverifiedRuntimeHandoffDecisionRejected => {
                "unverified-runtime-handoff-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-runtime-handoff-decision-rejected-as-production-authority"
            }
            ...
            Self::RuntimeHandoffDecisionIntegrityMismatch => {
                "runtime-handoff-decision-integrity-mismatch"
            }
            ...
            Self::LiveEpochTransitionExecutionPreparationAmbiguous { .. } => {
...
/// Run 315 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExecutionPreparationDecision {
    pub outcome: ProductionLiveEpochTransitionExecutionPreparationOutcome,
    pub preparation_id: String,
    pub request_id: String,
    pub preparation_artifact: Option<ProductionLiveEpochTransitionExecutionPreparationArtifact>,
    pub preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionExecutionPreparationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.preparation_artifact.is_some()
    }
}
...
/// Run 315 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome {
...
/// Run 315 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExecutionPreparationExecutor {
    pub config: ProductionLiveEpochTransitionExecutionPreparationConfig,
    pub policy: ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy,
}

impl ProductionLiveEpochTransitionExecutionPreparationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionExecutionPreparationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionExecutionPreparationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionEpochTransitionRuntimeHandoffDecision,
            &'a ProductionEpochTransitionRuntimeHandoffPackage,
        ),
        ProductionLiveEpochTransitionExecutionPreparationOutcome,
    > {
        use LiveEpochTransitionExecutionPreparationAuthoritySource as S;
        use ProductionLiveEpochTransitionExecutionPreparationOutcome as O;
        match source {
            S::VerifiedRuntimeHandoffDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedRuntimeHandoffDecisionRejected);
                }
                match &decision.handoff_package {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedRuntimeHandoffDecisionRequired),
                }
            }
            S::MissingRuntimeHandoffDecision => Err(O::VerifiedRuntimeHandoffDecisionRequired),
            S::UnverifiedRuntimeHandoffDecision { .. } => {
                Err(O::UnverifiedRuntimeHandoffDecisionRejected)
            }
            S::AcceptedRuntimeHandoffWithoutPackage { .. } => {
                Err(O::VerifiedRuntimeHandoffDecisionRequired)
            }
            S::GuardedMutationDecisionWithoutRuntimeHandoff => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutRuntimeHandoff => Err(O::ApplicationDecisionAloneRejected),
            S::StagedApplicationDecisionWithoutRuntimeHandoff => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutRuntimeHandoff => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutRuntimeHandoff => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutRuntimeHandoff => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutRuntimeHandoff => Err(O::GovernanceProofAloneRejected),
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionExecutionPreparationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionExecutionPreparationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionExecutionPreparationRequired => {
                    O::MainNetProductionLiveEpochTransitionExecutionPreparationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionExecutionPreparationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionExecutionPreparationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 315.
        if self.config.kind
            == ProductionLiveEpochTransitionExecutionPreparationExecutorKind::ProductionLiveEpochTransitionExecutionPreparation
        {
            return Some(O::LiveEpochTransitionExecutionPreparationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 313/314 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionEpochTransitionRuntimeHandoffDecision,
        intent: &ProductionEpochTransitionRuntimeHandoffPackage,
        inputs: &ProductionLiveEpochTransitionExecutionPreparationInputs,
    ) -> Option<ProductionLiveEpochTransitionExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionExecutionPreparationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 313/314 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.handoff_id != inputs.expected_runtime_handoff_decision_id {
            return Some(O::RuntimeHandoffDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_runtime_handoff_request_id {
            return Some(O::RuntimeHandoffDecisionRequestIdMismatch);
        }
        if decision.handoff_digest != inputs.expected_runtime_handoff_intent_digest {
            return Some(O::RuntimeHandoffDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_runtime_handoff_transcript_digest {
        ...
        if intent.content_digest() != decision.handoff_digest {
            return Some(O::RuntimeHandoffDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed runtime-handoff
        // nonce binding.
        if intent.runtime_handoff_nonce != inputs.expected_runtime_handoff_nonce {
            return Some(O::WrongRuntimeHandoffNonce);
        }

        // Re-exposed Run 311/312 guarded epoch-transition mutation-execution
        ...
        // Re-exposed Run 313/314 staged-application decision authority tuple
        ...
        // Re-exposed Run 313/314 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionExecutionPreparationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionExecutionPreparationInputs,
        replay_set: &R,
        ...
        ProductionLiveEpochTransitionExecutionPreparationOutcome,
        Option<ProductionLiveEpochTransitionExecutionPreparationArtifact>,
    ) {
        use ProductionLiveEpochTransitionExecutionPreparationOutcome as O;
        ...
        let staged_application_id =
            production_live_epoch_transition_execution_preparation_request_id(
                self.config.protocol_version.0,
                &decision.handoff_digest,
                &inputs.preparation_policy_id,
                request.proposed_epoch_transition_target,
                request.execution_preparation_nonce,
            );
        ...
        let staged_kind = LiveEpochTransitionExecutionPreparationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionExecutionPreparationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            preparation_policy_id: inputs.preparation_policy_id.clone(),
            ...
            // Re-exposed Run 311/312 guarded mutation-execution decision
            ...
            // Consumed Run 313/314 runtime handoff decision transcript (the
            // authority source this run consumes).
            runtime_handoff_decision_id: decision.handoff_id.clone(),
            runtime_handoff_request_id: decision.request_id.clone(),
            runtime_handoff_intent_digest: decision.handoff_digest.clone(),
            runtime_handoff_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed runtime handoff nonce.
            runtime_handoff_nonce: application_intent.runtime_handoff_nonce,
            // Newly proposed execution-preparation nonce.
            execution_preparation_nonce: request.execution_preparation_nonce,
            // Exact future-executor preconditions.
        ...
        let outcome = O::AcceptedSourceTestLiveEpochTransitionExecutionPreparation {
            preparation_kind: staged_kind,
            environment: application_intent.environment,
        ...
        record.preparation_digest = record.content_digest();
    ...
    /// Run 315 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_execution_preparation<
        R: LiveEpochTransitionExecutionPreparationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionExecutionPreparationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionExecutionPreparationDecision {
                outcome,
                preparation_id: pkg.preparation_id.clone(),
                request_id: pkg.request_id.clone(),
                preparation_artifact: record.clone(),
                preparation_digest: pkg.preparation_digest.clone(),
        ...
        let runtime_handoff_intent_digest = match &request.authority_source {
        ...
        let preparation_digest = String::new();
        ...
        ProductionLiveEpochTransitionExecutionPreparationDecision {
            ...
            preparation_id,
            request_id,
            preparation_artifact: None,
            preparation_digest,
            transcript_digest,
        }
    }

    /// Run 315 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_execution_preparation_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionExecutionPreparationArtifact>,
        current: &ProductionLiveEpochTransitionExecutionPreparationArtifact,
    ) -> ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome {
        use ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionExecutionPreparationExecutorKind::Disabled
        {
...
/// Run 315 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_execution_preparation_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy::default()
        == ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionExecutionPreparationConfig::default().kind
            == ProductionLiveEpochTransitionExecutionPreparationExecutorKind::Disabled
}

/// Run 315 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 316).
pub fn production_live_epoch_transition_execution_preparation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 315 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_execution_preparation_executor_mainnet_refused() -> bool {
    true
}

/// Run 315 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_execution_preparation_executor_is_non_mutating() -> bool {
    true
}

/// Run 315 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_execution_preparation_executor_never_falls_back() -> bool {
    true
}

/// Run 315 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_execution_preparation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 315 — the executor only requires a verified Run 313/314 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_execution_preparation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 315 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 315. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionExecutionPreparationFixtureState {
...
impl LiveEpochTransitionExecutionPreparationFixtureState {
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionExecutionPreparationArtifact,
        execution_id: &str,
    ) -> bool {
