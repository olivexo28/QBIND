//! Channel capacity configuration for the QBIND async runtime and network.
//!
...
//!
//! - `QBIND_CONSENSUS_EVENT_CHANNEL_CAPACITY`: ConsensusEvent channel capacity
//! - `QBIND_OUTBOUND_COMMAND_CHANNEL_CAPACITY`: Outbound command channel capacity
//! - `QBIND_ASYNC_PEER_INBOUND_CAPACITY`: AsyncPeerManager inbound channel capacity
//! - `QBIND_ASYNC_PEER_OUTBOUND_CAPACITY`: AsyncPeerManager outbound channel capacity
//!
...
/// Configuration for async channel capacities in the QBIND node.
///
...
impl ChannelCapacityConfig {
    ...
    /// - `QBIND_CONSENSUS_EVENT_CHANNEL_CAPACITY`
    /// - `QBIND_OUTBOUND_COMMAND_CHANNEL_CAPACITY`
    /// - `QBIND_ASYNC_PEER_INBOUND_CAPACITY`
    /// - `QBIND_ASYNC_PEER_OUTBOUND_CAPACITY`
    ///
    ...
    /// QBIND_CONSENSUS_EVENT_CHANNEL_CAPACITY=2048 cargo test -p qbind-node
    /// ```
    pub fn from_env() -> Self {
        let mut config = Self::default();
        let mut overrides = Vec::new();

        if let Some(val) = parse_env_capacity("QBIND_CONSENSUS_EVENT_CHANNEL_CAPACITY") {
            config.consensus_event_capacity = val;
            overrides.push(format!("event={}", val));
        }

        if let Some(val) = parse_env_capacity("QBIND_OUTBOUND_COMMAND_CHANNEL_CAPACITY") {
            config.outbound_command_capacity = val;
            overrides.push(format!("outbound={}", val));
        }

        if let Some(val) = parse_env_capacity("QBIND_ASYNC_PEER_INBOUND_CAPACITY") {
            config.async_peer_inbound_capacity = val;
            overrides.push(format!("async_peer_in={}", val));
        }

        if let Some(val) = parse_env_capacity("QBIND_ASYNC_PEER_OUTBOUND_CAPACITY") {
    ...
    pub fn has_env_overrides() -> bool {
        env::var("QBIND_CONSENSUS_EVENT_CHANNEL_CAPACITY").is_ok()
            || env::var("QBIND_OUTBOUND_COMMAND_CHANNEL_CAPACITY").is_ok()
            || env::var("QBIND_ASYNC_PEER_INBOUND_CAPACITY").is_ok()
            || env::var("QBIND_ASYNC_PEER_OUTBOUND_CAPACITY").is_ok()
    }
}
