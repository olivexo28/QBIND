//! Run 280 — source/test durable-completion **external-publication-receipt
//! consumer / external-publication-acknowledgement interface boundary**.
//!
//! Source/test only. Run 280 captures **no** release-binary evidence and enables
...
//! external-publication-receipt backend, a real settlement-finality projection
//! backend, a real audit-ledger acknowledgement, a real external-publication
//! external_publication_acknowledgement, a real external-publication system, a real production attestation /
...
//!
//! Run 278
//! ([`crate::pqc_governance_durable_completion_external_publication_receipt`])
...
//!
//! Run 280 defines the **first typed interface** a future production settlement-receipt
...
//! **after** the Run 278 settlement-confirmation stage produced
//! `ExternalPublicationReceiptRecorded`. It is an **interface / projection boundary only**:
//! production / MainNet / external external-publication-receipt implementations
...
//!
//! The external-publication-receipt layer is a **model only**. It does not
//! implement a real settlement, a real settlement finality, a real settlement receipt, a
//! real external-publication-receipt, a real settlement-finality projection, a
...
//! fixture external-publication-receipt sink mutates only the in-memory
...
//! 2. **legacy bypass** — a
//!    [`DurableCompletionExternalPublicationAcknowledgementPolicy::Disabled`] policy preserves
//!    the legacy no-external-publication-acknowledgement bypass and never invokes the
//!    external-publication-receipt sink;
//! 3. **settlement-confirmation-outcome projection** — only
//!    [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptRecorded`]
//!    creates a external-publication-receipt request; every other Run 278 outcome
...
//!    and Run 278 settlement-confirmation digest sets) must match expectations *before*
//!    the external-publication-receipt sink is invoked; a mismatch fails closed
//!    and leaves the sink invocation count at zero;
//! 5. **external-publication-receipt record** — only after every prior gate
//!    passes is the sink invoked; the record-identity fields must match exactly before
//!    any modeled external-publication-receipt record is recorded;
//! 6. **external-publication-receipt authorization** — only
//!    [`DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRecorded`]
//!    authorizes a new modeled external-publication-receipt / settlement-finality
//!    projection state.
//!
//! A external-publication-receipt record failure, rollback, rollback failure, or
//! ambiguous window never retroactively claims a durable settlement-receipt
//! acknowledgement record. A duplicate identical external-publication-receipt
//! record is idempotent; the same record id with a different digest fails closed as
//! equivocation and records no second record. A Run 278
//! [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptDuplicateIdempotent`]
//! never creates a new external-publication-receipt record by itself — it can only
//! match an already-recorded external-publication-receipt record.

use crate::pqc_governance_durable_completion_acknowledgement_consumer::DurableCompletionAcknowledgementConsumerOutcome;
use crate::pqc_governance_durable_completion_external_publication_receipt::DurableCompletionExternalPublicationReceiptOutcome;
...
/// Run 280 — the validation / mutation surface pair the receipt binds to.
pub type DurableCompletionExternalPublicationAcknowledgementSurface =
    ModeledGovernanceTrustMutationSurface;

/// Run 280 — the trust-domain environment binding the receipt is bound to.
pub type DurableCompletionExternalPublicationAcknowledgementEnvironment =
    ModeledGovernanceTrustMutationEnvironmentBinding;

/// Run 280 — the runtime binding (governance + mutation surface + sequence) the
/// receipt is bound to.
pub type DurableCompletionExternalPublicationAcknowledgementBinding =
    ModeledGovernanceTrustMutationRuntimeBinding;

/// Run 280 — the Run 240/246 durable replay observation carried as freshness
/// context.
pub type DurableCompletionExternalPublicationAcknowledgementReplayBinding = DurableReplayObservation;

/// Run 280 — the Run 246 pipeline outcome carried as consume authorization context.
pub type DurableCompletionExternalPublicationAcknowledgementPipelineBinding =
    GovernanceModeledEndToEndPipelineOutcome;

/// Run 280 — the Run 248 sink outcome carried as receipt-record context.
pub type DurableCompletionExternalPublicationAcknowledgementSinkBinding =
    GovernanceModeledDurableConsumeSinkOutcome;

/// Run 280 — the Run 250 reporter outcome carried as completion-report context.
pub type DurableCompletionExternalPublicationAcknowledgementReporterBinding =
    GovernanceModeledDurableConsumeCompletionReporterOutcome;

/// Run 280 — the Run 252 external_publication_acknowledgement outcome carried as external_publication_acknowledgement context.
pub type DurableCompletionExternalPublicationAcknowledgementFinalizationBinding =
    GovernanceModeledDurableCompletionFinalizationOutcome;

/// Run 280 — the Run 254 attestation outcome carried as attestation context.
pub type DurableCompletionExternalPublicationAcknowledgementAttestationBinding =
    GovernanceModeledDurableCompletionAttestationOutcome;

/// Run 280 — the Run 256 backend outcome carried as backend-record context. The
/// acknowledgement boundary never reimplements the backend; it only carries its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAcknowledgementBackendBinding =
    DurableCompletionAttestationBackendOutcome;

/// Run 280 — the Run 258 audit/publication receipt outcome carried as
/// receipt-record context. The consumer boundary never reimplements the receipt; it
/// only carries its terminal outcome.
pub type DurableCompletionExternalPublicationAcknowledgementReceiptBinding =
    DurableCompletionAuditPublicationReceiptOutcome;

/// Run 280 — the Run 260 audit-receipt acknowledgement outcome carried as
...
pub type DurableCompletionExternalPublicationAcknowledgementAcknowledgementBinding =
    DurableCompletionAuditReceiptAcknowledgementOutcome;

/// Run 280 — the Run 262 acknowledgement consumer outcome the settlement-confirmation
...
pub type DurableCompletionExternalPublicationAcknowledgementConsumerBinding =
    DurableCompletionAcknowledgementConsumerOutcome;

/// Run 280 — the Run 278 settlement-confirmation outcome the external-publication-acknowledgement
/// boundary projects to a external-publication-acknowledgement request. The external-publication-acknowledgement
/// boundary never reimplements the settlement confirmation; it only projects its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAcknowledgementExternalPublicationReceiptBinding =
...
/// Run 280 — the typed durable-completion audit/publication receipt kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAcknowledgementKind {
    ...
    /// fail-closed).
    ProductionExternalPublicationAcknowledgementUnavailable,
    /// MainNet audit-ledger receipt sink (reachable-but-unavailable / fail-closed).
    MainNetExternalPublicationAcknowledgementUnavailable,
    /// External-publication receipt sink (reachable-but-unavailable / fail-closed).
    ExternalExternalPublicationAcknowledgementUnavailable,
    /// An unknown receipt kind — fails closed.
    Unknown,
}

impl DurableCompletionExternalPublicationAcknowledgementKind {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureInMemory => "fixture-in-memory",
            Self::ProductionExternalPublicationAcknowledgementUnavailable => {
                "production-external-publication-acknowledgement-unavailable"
            }
            Self::MainNetExternalPublicationAcknowledgementUnavailable => {
                "mainnet-external-publication-acknowledgement-unavailable"
            }
            Self::ExternalExternalPublicationAcknowledgementUnavailable => {
                "external-external-publication-acknowledgement-unavailable"
            }
...
/// Run 280 — the typed durable-completion audit/publication receipt policy
/// selector.
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAcknowledgementPolicy {
    /// The receipt boundary is disabled (legacy bypass).
    Disabled,
    ...
    /// unavailable.
    ProductionExternalPublicationAcknowledgementRequired,
    /// A real MainNet audit-ledger receipt sink is required — reachable but
    /// unavailable.
    MainNetExternalPublicationAcknowledgementRequired,
    /// A real external-publication receipt sink is required — reachable but
    /// unavailable.
    ExternalExternalPublicationAcknowledgementRequired,
}

impl DurableCompletionExternalPublicationAcknowledgementPolicy {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureAllowed => "fixture-allowed",
            Self::ProductionExternalPublicationAcknowledgementRequired => {
                "production-external-publication-acknowledgement-required"
            }
            Self::MainNetExternalPublicationAcknowledgementRequired => "mainnet-external-publication-acknowledgement-required",
            Self::ExternalExternalPublicationAcknowledgementRequired => "external-external-publication-acknowledgement-required",
        }
    }
...
/// Run 280 — the typed receipt identity a receipt request is bound to.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAcknowledgementIdentity {
    /// Stable receipt id.
    pub confirmation_id: String,
    /// The receipt kind.
    pub kind: DurableCompletionExternalPublicationAcknowledgementKind,
    /// The receipt policy.
    pub policy: DurableCompletionExternalPublicationAcknowledgementPolicy,
    /// The domain separation tag the receipt operates under.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAcknowledgementIdentity {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.confirmation_id.is_empty()
            ...
            && self.kind != DurableCompletionExternalPublicationAcknowledgementKind::Unknown
    }

    /// The deterministic, domain-separated receipt identity digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAcknowledgementDigest {
        external_publication_acknowledgement_identity_digest(self)
    }
}
...
/// Run 280 — domain separator for the receipt identity digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_IDENTITY_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-acknowledgement-identity:v1";
/// Run 280 — domain separator for the receipt request digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-acknowledgement-request:v1";
/// Run 280 — domain separator for the receipt response digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-acknowledgement-response:v1";
/// Run 280 — domain separator for the receipt record digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-acknowledgement-record:v1";
/// Run 280 — domain separator for the receipt transcript digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_TRANSCRIPT_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-acknowledgement-transcript:v1";

/// Run 280 — a deterministic, domain-separated receipt digest (lowercase hex of a
/// SHA3-256 over length-prefixed, field-bound canonical material). It contains no
/// unstable display text.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAcknowledgementDigest(String);

impl DurableCompletionExternalPublicationAcknowledgementDigest {
...
/// Run 280 — a deterministic, domain-separated receipt transcript digest.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAcknowledgementTranscriptDigest(String);

impl DurableCompletionExternalPublicationAcknowledgementTranscriptDigest {
    /// The lowercase hex representation.
    pub fn as_hex(&self) -> &str {
        &self.0
    }
...
/// Run 280 — deterministic, domain-separated receipt identity digest.
pub fn external_publication_acknowledgement_identity_digest(
    identity: &DurableCompletionExternalPublicationAcknowledgementIdentity,
) -> DurableCompletionExternalPublicationAcknowledgementDigest {
    ...
    DurableCompletionExternalPublicationAcknowledgementDigest(w.finish())
}

/// Run 280 — deterministic, domain-separated receipt request digest. Binds every
/// request field including the full Run 256 backend digest binding.
pub fn external_publication_acknowledgement_request_digest(
    request: &DurableCompletionExternalPublicationAcknowledgementRequest,
) -> DurableCompletionExternalPublicationAcknowledgementDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN);
    w.str_field(&request.external_publication_acknowledgement_record_id)
        ...
        .str_field(&request.external_publication_receipt_identity_digest)
        .str_field(&request.external_publication_receipt_request_digest)
        .str_field(&request.external_publication_receipt_response_digest)
        .str_field(&request.external_publication_receipt_record_digest)
        .str_field(&request.external_publication_receipt_transcript_digest)
        .str_field(&request.external_publication_receipt_record_id)
        .str_field(&request.domain_separation_tag)
        .str_field(external_publication_acknowledgement_identity_digest(&request.identity).as_hex());
    DurableCompletionExternalPublicationAcknowledgementDigest(w.finish())
}

/// Run 280 — deterministic, domain-separated receipt response digest. Binds the
/// receipt record id, the request digest it answers, the acceptance flag, and the
/// responding receipt kind.
pub fn external_publication_acknowledgement_response_digest(
    response: &DurableCompletionExternalPublicationAcknowledgementResponse,
) -> DurableCompletionExternalPublicationAcknowledgementDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN);
    w.str_field(&response.external_publication_acknowledgement_record_id)
        ...
        .str_field(response.external_publication_acknowledgement_kind.tag());
    DurableCompletionExternalPublicationAcknowledgementDigest(w.finish())
}

/// Run 280 — deterministic, domain-separated receipt record digest. Binds the
/// receipt record id, the request digest, and the receipt identity digest.
pub fn external_publication_acknowledgement_record_digest(
    record: &DurableCompletionExternalPublicationAcknowledgementRecord,
) -> DurableCompletionExternalPublicationAcknowledgementDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN);
    w.str_field(&record.external_publication_acknowledgement_record_id)
        .str_field(record.request_digest.as_hex())
        .str_field(record.identity_digest.as_hex());
    DurableCompletionExternalPublicationAcknowledgementDigest(w.finish())
}

/// Run 280 — deterministic, domain-separated receipt transcript digest. Binds the
/// request, response, and record digests into a single transcript binding.
pub fn external_publication_acknowledgement_transcript_digest(
    request_digest: &DurableCompletionExternalPublicationAcknowledgementDigest,
    response_digest: &DurableCompletionExternalPublicationAcknowledgementDigest,
    record_digest: &DurableCompletionExternalPublicationAcknowledgementDigest,
) -> DurableCompletionExternalPublicationAcknowledgementTranscriptDigest {
    ...
    DurableCompletionExternalPublicationAcknowledgementTranscriptDigest(w.finish())
}
...
/// Run 280 — the typed receipt request a future production audit ledger or
...
/// attestation / Run 252 external_publication_acknowledgement / Run 250 completion report / Run 248
/// receipt / Run 246 decision material — never a copy of any wire payload and never
/// a production durable record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementRequest {
    ...
    /// Run 252 external_publication_acknowledgement decision digest.
    ...
    /// Run 278 settlement-confirmation identity digest.
    pub external_publication_receipt_identity_digest: String,
    /// Run 278 settlement-confirmation request digest.
    pub external_publication_receipt_request_digest: String,
    /// Run 278 settlement-confirmation response digest.
    pub external_publication_receipt_response_digest: String,
    /// Run 278 settlement-confirmation record digest.
    pub external_publication_receipt_record_digest: String,
    /// Run 278 settlement-confirmation transcript digest.
    pub external_publication_receipt_transcript_digest: String,
    /// Run 278 settlement-confirmation record id.
    pub external_publication_receipt_record_id: String,
    /// External-publication-confirmation identity.
    pub identity: DurableCompletionExternalPublicationAcknowledgementIdentity,
    /// Domain separation tag.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAcknowledgementRequest {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.external_publication_acknowledgement_record_id.is_empty()
            ...
            && !self.external_publication_receipt_identity_digest.is_empty()
            && !self.external_publication_receipt_request_digest.is_empty()
            && !self.external_publication_receipt_response_digest.is_empty()
            && !self.external_publication_receipt_record_digest.is_empty()
            && !self.external_publication_receipt_transcript_digest.is_empty()
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationAcknowledgementDigest {
        external_publication_acknowledgement_request_digest(self)
    }

    /// The canonical immutable record derived from this request.
    pub fn to_record(&self) -> DurableCompletionExternalPublicationAcknowledgementRecord {
        DurableCompletionExternalPublicationAcknowledgementRecord {
            external_publication_acknowledgement_record_id: self.external_publication_acknowledgement_record_id.clone(),
...
/// Run 280 — the typed receipt response a receipt sink returns for a request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementResponse {
    /// The receipt record id the response answers.
    pub external_publication_acknowledgement_record_id: String,
    /// The request digest the response answers.
    pub request_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
    /// `true` iff the receipt sink accepted the request.
    pub accepted: bool,
    /// The responding receipt kind.
    pub external_publication_acknowledgement_kind: DurableCompletionExternalPublicationAcknowledgementKind,
}

impl DurableCompletionExternalPublicationAcknowledgementResponse {
    /// `true` iff the response is structurally well-formed.
    pub fn is_well_formed(&self) -> bool {
        !self.external_publication_acknowledgement_record_id.is_empty()
            && self.external_publication_acknowledgement_kind != DurableCompletionExternalPublicationAcknowledgementKind::Unknown
    }

    /// The deterministic receipt response digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAcknowledgementDigest {
        external_publication_acknowledgement_response_digest(self)
    }
}

/// Run 280 — the canonical immutable receipt record derived from a request. Two
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAcknowledgementRecord {
    /// The receipt record id.
    pub external_publication_acknowledgement_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
    /// The receipt identity digest.
    pub identity_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
}

impl DurableCompletionExternalPublicationAcknowledgementRecord {
    /// The deterministic receipt record digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAcknowledgementDigest {
        external_publication_acknowledgement_record_digest(self)
    }
}
...
/// Run 280 — the recorded status of a modeled audit/publication receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAcknowledgementLedgerStatus {
    /// The modeled receipt is recorded in the in-memory fixture ledger.
    Recorded,
}

/// Run 280 — a single modeled receipt record held in the in-memory fixture ledger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementLedgerRecord {
    /// The receipt record id (stable identity of the receipt).
    pub external_publication_acknowledgement_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
    /// The response digest.
    pub response_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
    /// The record digest.
    pub record_digest: DurableCompletionExternalPublicationAcknowledgementDigest,
    /// The transcript digest.
    pub transcript_digest: DurableCompletionExternalPublicationAcknowledgementTranscriptDigest,
    /// The recorded status.
    pub status: DurableCompletionExternalPublicationAcknowledgementLedgerStatus,
}

/// Run 280 — an immutable snapshot of the modeled receipt ledger used to model a
/// fixture rollback.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot {
    records: Vec<DurableCompletionExternalPublicationAcknowledgementLedgerRecord>,
}

impl DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot {
...
/// Run 280 — the modeled in-memory receipt ledger.
///
...
pub struct DurableCompletionExternalPublicationAcknowledgementLedger {
    records: Vec<DurableCompletionExternalPublicationAcknowledgementLedgerRecord>,
}

impl DurableCompletionExternalPublicationAcknowledgementLedger {
    ...
    pub fn records(&self) -> &[DurableCompletionExternalPublicationAcknowledgementLedgerRecord] {
        &self.records
    }

    /// The record for `external_publication_acknowledgement_record_id`, if present.
    pub fn find(
        &self,
        external_publication_acknowledgement_record_id: &str,
    ) -> Option<&DurableCompletionExternalPublicationAcknowledgementLedgerRecord> {
        self.records
            .iter()
            .find(|r| r.external_publication_acknowledgement_record_id == external_publication_acknowledgement_record_id)
    }

    /// `true` iff a receipt with `external_publication_acknowledgement_record_id` is recorded.
    pub fn contains(&self, external_publication_acknowledgement_record_id: &str) -> bool {
        self.find(external_publication_acknowledgement_record_id).is_some()
    }

    /// Capture an immutable snapshot for a modeled rollback.
    pub fn snapshot(&self) -> DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot {
        DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot {
    ...
    pub fn restore(
        &mut self,
        snapshot: &DurableCompletionExternalPublicationAcknowledgementLedgerSnapshot,
    ) {
    ...
    fn insert(&mut self, record: DurableCompletionExternalPublicationAcknowledgementLedgerRecord) {
...
/// Run 280 — the canonical binding a [`DurableCompletionExternalPublicationAcknowledgementInput`]
/// is checked against.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementExpectations {
    ...
    /// Expected Run 252 external_publication_acknowledgement decision digest.
    ...
    /// Expected Run 278 settlement-confirmation identity digest.
    pub expected_external_publication_receipt_identity_digest: String,
    /// Expected Run 278 settlement-confirmation request digest.
    pub expected_external_publication_receipt_request_digest: String,
    /// Expected Run 278 settlement-confirmation response digest.
    pub expected_external_publication_receipt_response_digest: String,
    /// Expected Run 278 settlement-confirmation record digest.
    pub expected_external_publication_receipt_record_digest: String,
    /// Expected Run 278 settlement-confirmation transcript digest.
    pub expected_external_publication_receipt_transcript_digest: String,
    /// Expected Run 278 settlement-confirmation record id.
    pub expected_external_publication_receipt_record_id: String,
    /// Expected settlement-confirmation identity.
    pub expected_identity: DurableCompletionExternalPublicationAcknowledgementIdentity,
    /// Expected settlement-confirmation kind.
    pub expected_external_publication_acknowledgement_kind: DurableCompletionExternalPublicationAcknowledgementKind,
    /// Expected settlement-confirmation policy.
    pub expected_external_publication_acknowledgement_policy: DurableCompletionExternalPublicationAcknowledgementPolicy,
    /// Expected domain separation tag.
    pub expected_domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAcknowledgementExpectations {
    /// Internal: the first environment / surface binding mismatch reason, if any.
    fn binding_mismatch_reason(
        &self,
        input: &DurableCompletionExternalPublicationAcknowledgementInput,
    ) -> Option<&'static str> {
    ...
    fn request_mismatch_reason(
        ...
        if request.finalization_decision_digest != self.expected_finalization_decision_digest {
            return Some("wrong external_publication_acknowledgement decision digest");
        }
        ...
        if request.external_publication_receipt_identity_digest
            != self.expected_external_publication_receipt_identity_digest
        {
            return Some("wrong settlement-confirmation identity digest");
        }
        if request.external_publication_receipt_request_digest
            != self.expected_external_publication_receipt_request_digest
        {
            return Some("wrong settlement-confirmation request digest");
        }
        if request.external_publication_receipt_response_digest
            != self.expected_external_publication_receipt_response_digest
        {
            return Some("wrong settlement-confirmation response digest");
        }
        if request.external_publication_receipt_record_digest
            != self.expected_external_publication_receipt_record_digest
        {
            return Some("wrong settlement-confirmation record digest");
        }
        if request.external_publication_receipt_transcript_digest
            != self.expected_external_publication_receipt_transcript_digest
        {
        ...
        if request.identity != self.expected_identity {
            return Some("wrong external-publication-acknowledgement identity");
        }
        if request.identity.kind != self.expected_external_publication_acknowledgement_kind {
            return Some("wrong external-publication-acknowledgement kind");
        }
        if request.identity.policy != self.expected_external_publication_acknowledgement_policy {
            return Some("wrong external-publication-acknowledgement policy");
        }
        None
    }
...
/// Run 280 — typed inputs for one modeled durable-completion audit/publication
/// receipt round-trip.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAcknowledgementInput {
    /// The receipt policy selector.
    pub policy: DurableCompletionExternalPublicationAcknowledgementPolicy,
    /// The trust-domain environment binding.
    pub environment_binding: DurableCompletionExternalPublicationAcknowledgementEnvironment,
    /// The runtime binding (governance + mutation surface + sequence).
    pub runtime_binding: DurableCompletionExternalPublicationAcknowledgementBinding,
    /// The Run 240/246 durable replay observation the pipeline authorized consume
    /// under.
    pub replay_binding: DurableCompletionExternalPublicationAcknowledgementReplayBinding,
    /// The Run 246 pipeline outcome.
    pub pipeline_binding: DurableCompletionExternalPublicationAcknowledgementPipelineBinding,
    /// The Run 248 sink outcome.
    pub sink_binding: DurableCompletionExternalPublicationAcknowledgementSinkBinding,
    /// The Run 250 reporter outcome.
    pub reporter_binding: DurableCompletionExternalPublicationAcknowledgementReporterBinding,
    /// The Run 252 external_publication_acknowledgement outcome.
    pub confirmation_binding: DurableCompletionExternalPublicationAcknowledgementFinalizationBinding,
    /// The Run 254 attestation outcome.
    pub attestation_binding: DurableCompletionExternalPublicationAcknowledgementAttestationBinding,
    /// The Run 256 backend outcome carried as backend-record context.
    pub backend_binding: DurableCompletionExternalPublicationAcknowledgementBackendBinding,
    /// The Run 258 audit/publication receipt outcome carried as receipt-record
    /// context.
    pub receipt_binding: DurableCompletionExternalPublicationAcknowledgementReceiptBinding,
    /// The Run 260 audit-receipt acknowledgement outcome carried as
    /// acknowledgement-record context.
    pub acknowledgement_binding:
        DurableCompletionExternalPublicationAcknowledgementAcknowledgementBinding,
    ...
    pub consumer_binding: DurableCompletionExternalPublicationAcknowledgementConsumerBinding,
    /// The Run 278 settlement-confirmation outcome the external-publication-acknowledgement boundary
    /// projects to a external-publication-acknowledgement request.
    pub external_publication_receipt_binding:
        DurableCompletionExternalPublicationAcknowledgementExternalPublicationReceiptBinding,
    /// The external-publication-acknowledgement request the call site would submit.
    pub request: DurableCompletionExternalPublicationAcknowledgementRequest,
}

impl DurableCompletionExternalPublicationAcknowledgementInput {
    ...
    pub fn surface(&self) -> DurableCompletionExternalPublicationAcknowledgementSurface {
    ...
    pub fn is_mainnet_peer_driven(&self) -> bool {
        (self.environment() == TrustBundleEnvironment::Mainnet && self.surface().is_peer_driven())
            ...
            || matches!(
                self.external_publication_receipt_binding,
                DurableCompletionExternalPublicationReceiptOutcome::MainNetPeerDrivenApplyRefusedNoReceipt
            )
    }
}
...
/// Run 280 — the typed outcome of one modeled durable-completion audit/publication
/// receipt round-trip.
///
/// Only [`Self::ExternalPublicationAcknowledgementRecorded`] authorizes a **new** modeled
/// audit/publication-receipt state. A [`Self::ExternalPublicationAcknowledgementDuplicateIdempotent`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAcknowledgementOutcome {
    /// Legacy bypass — a disabled receipt policy preserved the legacy
    /// no-audit-receipt path. No receipt invocation.
    LegacyBypassNoExternalPublicationAcknowledgement,
    ...
    RejectedBeforeExternalPublicationReceiptNoAcknowledgement,
    ...
    /// invocation.
    ExternalPublicationReceiptDidNotRecordNoAcknowledgement,
    ...
    ExternalPublicationAcknowledgementRecorded,
    /// A duplicate identical receipt — idempotent; no second receipt recorded.
    ExternalPublicationAcknowledgementDuplicateIdempotent,
    ...
    /// receipt.
    ExternalPublicationAcknowledgementRejectedBeforeRecord,
    /// The receipt record failed. No receipt.
    ExternalPublicationAcknowledgementRecordFailedNoAcknowledgement,
    /// The receipt record was rolled back. No receipt.
    ExternalPublicationAcknowledgementRolledBackNoAcknowledgement,
    /// The receipt rollback itself failed — fatal / fail-closed. No receipt.
    ExternalPublicationAcknowledgementRollbackFailedFatalNoAcknowledgement,
    /// The after-record receipt window was ambiguous — fails closed. No receipt.
    ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement,
    /// The production audit-ledger path was reached but is unavailable. No receipt.
    ProductionExternalPublicationAcknowledgementUnavailableNoAcknowledgement,
    /// The MainNet audit-ledger path was reached but is unavailable. No receipt.
    MainNetExternalPublicationAcknowledgementUnavailableNoAcknowledgement,
    /// The external-publication path was reached but is unavailable. No receipt.
    ExternalExternalPublicationAcknowledgementUnavailableNoAcknowledgement,
...
impl DurableCompletionExternalPublicationAcknowledgementOutcome {
    /// `true` iff this outcome authorizes a **new** modeled audit/publication
    /// receipt (only [`Self::ExternalPublicationAcknowledgementRecorded`]).
    pub fn authorizes_record(&self) -> bool {
        matches!(self, Self::ExternalPublicationAcknowledgementRecorded)
    }
    ...
    pub fn projects_to_recorded(&self) -> bool {
        matches!(
            self,
            Self::ExternalPublicationAcknowledgementRecorded | Self::ExternalPublicationAcknowledgementDuplicateIdempotent
        )
    }
    ...
    pub fn is_legacy_bypass(&self) -> bool {
        matches!(self, Self::LegacyBypassNoExternalPublicationAcknowledgement)
    }
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::LegacyBypassNoExternalPublicationAcknowledgement => "legacy-bypass-no-external-publication-acknowledgement",
            Self::RejectedBeforeExternalPublicationReceiptNoAcknowledgement => {
                "rejected-before-settlement-confirmation-no-outcome-publication"
            }
            Self::ExternalPublicationReceiptDidNotRecordNoAcknowledgement => {
                "settlement-confirmation-did-not-record-no-outcome-publication"
            }
            Self::ExternalPublicationAcknowledgementRecorded => "external-publication-acknowledgement-recorded",
            Self::ExternalPublicationAcknowledgementDuplicateIdempotent => {
                "external-publication-acknowledgement-duplicate-idempotent"
            }
            Self::ExternalPublicationAcknowledgementRejectedBeforeRecord => {
                "external-publication-acknowledgement-rejected-before-record"
            }
            Self::ExternalPublicationAcknowledgementRecordFailedNoAcknowledgement => {
                "external-publication-acknowledgement-record-failed-no-outcome-publication"
            }
            Self::ExternalPublicationAcknowledgementRolledBackNoAcknowledgement => {
                "external-publication-acknowledgement-rolled-back-no-outcome-publication"
            }
            Self::ExternalPublicationAcknowledgementRollbackFailedFatalNoAcknowledgement => {
                "external-publication-acknowledgement-rollback-failed-fatal-no-outcome-publication"
            }
            Self::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement => {
                "external-publication-acknowledgement-ambiguous-fail-closed-no-outcome-publication"
            }
            Self::ProductionExternalPublicationAcknowledgementUnavailableNoAcknowledgement => {
                "production-external-publication-acknowledgement-unavailable-no-outcome-publication"
            }
            Self::MainNetExternalPublicationAcknowledgementUnavailableNoAcknowledgement => {
                "mainnet-external-publication-acknowledgement-unavailable-no-outcome-publication"
            }
            Self::ExternalExternalPublicationAcknowledgementUnavailableNoAcknowledgement => {
                "external-external-publication-acknowledgement-unavailable-no-outcome-publication"
            }
            ...
            Self::PolicyChangeUnsupportedNoAcknowledgement => "policy-change-unsupported-no-outcome-publication",
        }
    }
}
...
/// Run 280 — the typed confirmation of a Run 262 acknowledgement consumer outcome
/// onto a settlement-confirmation request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAcknowledgementRequestIntent {
    ...
    NoReceipt(DurableCompletionExternalPublicationAcknowledgementOutcome),
}

impl DurableCompletionExternalPublicationAcknowledgementRequestIntent {
...
/// Run 280 — project a Run 278 settlement-confirmation outcome onto a
/// external-publication-acknowledgement request.
///
/// Only
/// [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptRecorded`]
/// creates a external-publication-acknowledgement request.
/// [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptDuplicateIdempotent`]
/// may only match an already-recorded external-publication-acknowledgement record and never creates
...
/// [`DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationReceiptDidNotRecordNoAcknowledgement`]).
/// Pure: performs no work and never records.
pub fn project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request(
    outcome: &DurableCompletionExternalPublicationAcknowledgementExternalPublicationReceiptBinding,
) -> DurableCompletionExternalPublicationAcknowledgementRequestIntent {
    use DurableCompletionExternalPublicationReceiptOutcome as Finalization;
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Receipt;
    use DurableCompletionExternalPublicationAcknowledgementRequestIntent as Intent;
    match outcome {
        Finalization::ExternalPublicationReceiptRecorded => Intent::CreateRequest,
        Finalization::ExternalPublicationReceiptDuplicateIdempotent => Intent::IdempotentOnly,
        Finalization::LegacyBypassNoExternalPublicationReceipt => {
            Intent::NoReceipt(Receipt::LegacyBypassNoExternalPublicationAcknowledgement)
        }
        Finalization::RejectedBeforeExternalPublicationConfirmationNoReceipt => {
            Intent::NoReceipt(Receipt::RejectedBeforeExternalPublicationReceiptNoAcknowledgement)
        }
        Finalization::MainNetPeerDrivenApplyRefusedNoReceipt => {
            Intent::NoReceipt(Receipt::MainNetPeerDrivenApplyRefusedNoAcknowledgement)
        }
        Finalization::ValidatorSetRotationUnsupportedNoReceipt => {
            Intent::NoReceipt(Receipt::ValidatorSetRotationUnsupportedNoAcknowledgement)
        }
        Finalization::PolicyChangeUnsupportedNoReceipt => {
            Intent::NoReceipt(Receipt::PolicyChangeUnsupportedNoAcknowledgement)
        }
        ...
        // record, so no external-publication-acknowledgement record may exist.
        _ => Intent::NoReceipt(Receipt::ExternalPublicationReceiptDidNotRecordNoAcknowledgement),
    }
}
...
/// Run 280 — a modeled fault the fixture receipt sink injects to exercise the
/// fail-closed record / rollback / ambiguous paths. Source/test only.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAcknowledgementFault {
...
/// Run 280 — the pure/mockable modeled durable-completion audit/publication receipt
/// boundary.
///
...
/// [`DurableCompletionExternalPublicationAcknowledgementLedger`].
pub trait GovernanceDurableCompletionExternalPublicationAcknowledgementSink {
    /// The receipt kind (used for typed recovery classification).
    fn kind(&self) -> DurableCompletionExternalPublicationAcknowledgementKind;
    ...
    fn project_durable_completion_external_publication_acknowledgement(
        &mut self,
        request: &DurableCompletionExternalPublicationAcknowledgementRequest,
        expectations: &DurableCompletionExternalPublicationAcknowledgementExpectations,
        idempotent_only: bool,
        ledger: &mut DurableCompletionExternalPublicationAcknowledgementLedger,
    ...
    fn recover_durable_completion_external_publication_acknowledgement_window(
        &self,
        input: &DurableCompletionExternalPublicationAcknowledgementInput,
        window: DurableCompletionExternalPublicationAcknowledgementWindow,
        recovered_record: Option<&DurableCompletionExternalPublicationAcknowledgementLedgerRecord>,
        expectations: &DurableCompletionExternalPublicationAcknowledgementExpectations,
    ) -> DurableCompletionExternalPublicationAcknowledgementOutcome {
...
/// Run 280 — the DevNet/TestNet in-memory fixture receipt sink.
///
/// Source-test only. It mutates only the in-memory
/// [`DurableCompletionExternalPublicationAcknowledgementLedger`] and exposes an invocation
/// counter so tests can prove non-submitting backend paths and pre-receipt
/// rejections never invoke it.
#[derive(Debug, Clone)]
pub struct FixtureDurableCompletionExternalPublicationAcknowledgementSink {
    fault: Option<DurableCompletionExternalPublicationAcknowledgementFault>,
    invocations: u32,
}

impl Default for FixtureDurableCompletionExternalPublicationAcknowledgementSink {
    fn default() -> Self {
        Self::new()
    }
}

impl FixtureDurableCompletionExternalPublicationAcknowledgementSink {
    ...
    pub fn with_fault(fault: DurableCompletionExternalPublicationAcknowledgementFault) -> Self {
        Self {
            fault: Some(fault),
            invocations: 0,
        }
...
impl GovernanceDurableCompletionExternalPublicationAcknowledgementSink
    for FixtureDurableCompletionExternalPublicationAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAcknowledgementKind {
    ...
    fn project_durable_completion_external_publication_acknowledgement(
        ...
        if let Some(fault) = self.fault {
            ...
            return match fault {
                DurableCompletionExternalPublicationAcknowledgementFault::RecordFailedNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAcknowledgementRecordFailedNoAcknowledgement
                }
                DurableCompletionExternalPublicationAcknowledgementFault::RolledBackNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAcknowledgementRolledBackNoAcknowledgement
                }
                DurableCompletionExternalPublicationAcknowledgementFault::RollbackFailedFatal => {
                    Receipt::ExternalPublicationAcknowledgementRollbackFailedFatalNoAcknowledgement
                }
                DurableCompletionExternalPublicationAcknowledgementFault::AmbiguousAfterRecord => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement
                }
            };
        }
        ...
        if !matches!(
            ...
            return Receipt::ExternalPublicationAcknowledgementRejectedBeforeRecord;
        }
        ...
        let response = DurableCompletionExternalPublicationAcknowledgementResponse {
            external_publication_acknowledgement_record_id: request.external_publication_acknowledgement_record_id.clone(),
            request_digest: request_digest.clone(),
            accepted: true,
            external_publication_acknowledgement_kind: DurableCompletionExternalPublicationAcknowledgementKind::FixtureInMemory,
        };
        let response_digest = response.digest();
        let record = request.to_record();
        let record_digest = record.digest();
        let transcript_digest = external_publication_acknowledgement_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
        );
        ...
        if let Some(existing) = ledger.find(&request.external_publication_acknowledgement_record_id) {
            ...
            {
                return Receipt::ExternalPublicationAcknowledgementDuplicateIdempotent;
            }
        ...
        ledger.insert(DurableCompletionExternalPublicationAcknowledgementLedgerRecord {
            external_publication_acknowledgement_record_id: request.external_publication_acknowledgement_record_id.clone(),
            request_digest,
            response_digest,
            record_digest,
            transcript_digest,
            status: DurableCompletionExternalPublicationAcknowledgementLedgerStatus::Recorded,
        });
        Receipt::ExternalPublicationAcknowledgementRecorded
    }
}
...
/// Run 280 — the production audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ProductionExternalPublicationAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAcknowledgementSink
    for ProductionExternalPublicationAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAcknowledgementKind {
        DurableCompletionExternalPublicationAcknowledgementKind::ProductionExternalPublicationAcknowledgementUnavailable
    }
    ...
    fn project_durable_completion_external_publication_acknowledgement(
        &mut self,
        _request: &DurableCompletionExternalPublicationAcknowledgementRequest,
        _expectations: &DurableCompletionExternalPublicationAcknowledgementExpectations,
        _idempotent_only: bool,
        _ledger: &mut DurableCompletionExternalPublicationAcknowledgementLedger,
    ) -> DurableCompletionExternalPublicationAcknowledgementOutcome {
        self.invocations += 1;
        DurableCompletionExternalPublicationAcknowledgementOutcome::ProductionExternalPublicationAcknowledgementUnavailableNoAcknowledgement
    }
}

/// Run 280 — the MainNet audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct MainNetExternalPublicationAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAcknowledgementSink
    for MainNetExternalPublicationAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAcknowledgementKind {
        DurableCompletionExternalPublicationAcknowledgementKind::MainNetExternalPublicationAcknowledgementUnavailable
    }
    ...
    fn project_durable_completion_external_publication_acknowledgement(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAcknowledgementOutcome::MainNetExternalPublicationAcknowledgementUnavailableNoAcknowledgement
    }
}

/// Run 280 — the external-publication receipt sink. Reachable but unavailable /
/// fail-closed. It performs no external publication, records no receipt, and
/// mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ExternalExternalPublicationAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAcknowledgementSink
    for ExternalExternalPublicationAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAcknowledgementKind {
        DurableCompletionExternalPublicationAcknowledgementKind::ExternalExternalPublicationAcknowledgementUnavailable
    }
    ...
    fn project_durable_completion_external_publication_acknowledgement(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalExternalPublicationAcknowledgementUnavailableNoAcknowledgement
    }
}
...
/// Run 280 — evaluate one modeled durable-completion audit/publication receipt
/// round-trip.
///
...
/// 2. legacy bypass — a
///    [`DurableCompletionExternalPublicationAcknowledgementPolicy::Disabled`] policy;
...
pub fn evaluate_durable_completion_external_publication_acknowledgement<S>(
    input: &DurableCompletionExternalPublicationAcknowledgementInput,
    expectations: &DurableCompletionExternalPublicationAcknowledgementExpectations,
    sink: &mut S,
    ledger: &mut DurableCompletionExternalPublicationAcknowledgementLedger,
...
where
    S: GovernanceDurableCompletionExternalPublicationAcknowledgementSink,
{
    ...
    if input.is_mainnet_peer_driven() {
        return Finalization::MainNetPeerDrivenApplyRefusedNoAcknowledgement;
    }
    ...
    if input.policy.is_disabled() {
        return Finalization::LegacyBypassNoExternalPublicationAcknowledgement;
    }
    ...
    let idempotent_only =
        match project_external_publication_receipt_outcome_to_external_publication_acknowledgement_request(
            &input.external_publication_receipt_binding,
        ) {
    ...
    if !expectations.binding_matches(input) {
        return Finalization::RejectedBeforeExternalPublicationReceiptNoAcknowledgement;
    }

    // Step 5: invoke the settlement-confirmation sink to record the modeled confirmation.
    sink.project_durable_completion_external_publication_acknowledgement(
        &input.request,
        expectations,
        idempotent_only,
        ledger,
...
/// Run 280 — the modeled durable-completion audit/publication receipt
/// crash/recovery window.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAcknowledgementWindow {
    ...
    /// Crashed after the report record but before a external_publication_acknowledgement intent.
    AfterPublicationRecordBeforeReceiptIntent,
    /// Crashed after a external_publication_acknowledgement intent but before any external_publication_acknowledgement record.
    AfterReceiptIntentBeforeReceiptRecord,
    /// Crashed after the external_publication_acknowledgement record but before an attestation intent.
    ...
    AfterConsumerSuccessBeforeExternalPublicationReceiptRequest,
    /// Crashed after a settlement-confirmation request but before any
    /// settlement-confirmation record.
    AfterExternalPublicationReceiptRequestBeforeExternalPublicationReceiptRecord,
    ...
    /// record exists.
    AfterExternalPublicationReceiptRecordBeforeExternalPublicationReceiptSuccess,
    /// Crashed after settlement-confirmation success but before a external-publication-acknowledgement
    /// request.
    AfterExternalPublicationReceiptSuccessBeforeExternalPublicationAcknowledgementRequest,
    /// Crashed after a external-publication-acknowledgement request but before any
    /// external-publication-acknowledgement record.
    AfterExternalPublicationAcknowledgementRequestBeforeExternalPublicationAcknowledgementRecord,
    /// Crashed after a external-publication-acknowledgement record but before external-publication-acknowledgement
    /// success — fails closed unless an explicit matching external-publication-acknowledgement
    /// record exists.
    AfterExternalPublicationAcknowledgementRecordBeforeExternalPublicationAcknowledgementSuccess,
    /// Recovered after a successful external-publication-acknowledgement record.
    AfterExternalPublicationAcknowledgementSuccess,
    /// Recovered after an ambiguous external-publication-acknowledgement window.
    AfterExternalPublicationAcknowledgementAmbiguous,
    /// The external-publication-acknowledgement record itself failed.
    ExternalPublicationAcknowledgementRecordFailed,
    /// The external-publication-acknowledgement record was rolled back.
    ExternalPublicationAcknowledgementRollbackCompleted,
    /// The external-publication-acknowledgement rollback itself failed — fatal.
    ExternalPublicationAcknowledgementRollbackFailed,
    /// An unknown window — fails closed.
    Unknown,
}

/// Run 280 — classify a modeled durable-completion audit/publication receipt
/// crash/recovery window.
///
...
/// [`DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRecorded`]. Pure:
/// performs no modeled mutation and never invokes Run 070.
pub fn recover_durable_completion_external_publication_acknowledgement_window(
    input: &DurableCompletionExternalPublicationAcknowledgementInput,
    window: DurableCompletionExternalPublicationAcknowledgementWindow,
    kind: DurableCompletionExternalPublicationAcknowledgementKind,
    recovered_record: Option<&DurableCompletionExternalPublicationAcknowledgementLedgerRecord>,
    expectations: &DurableCompletionExternalPublicationAcknowledgementExpectations,
) -> DurableCompletionExternalPublicationAcknowledgementOutcome {
    use DurableCompletionExternalPublicationAcknowledgementOutcome as Receipt;
    use DurableCompletionExternalPublicationAcknowledgementWindow as Window;
    ...
    match kind {
        DurableCompletionExternalPublicationAcknowledgementKind::ProductionExternalPublicationAcknowledgementUnavailable => {
            return Receipt::ProductionExternalPublicationAcknowledgementUnavailableNoAcknowledgement;
        }
        DurableCompletionExternalPublicationAcknowledgementKind::MainNetExternalPublicationAcknowledgementUnavailable => {
            return Receipt::MainNetExternalPublicationAcknowledgementUnavailableNoAcknowledgement;
        }
        DurableCompletionExternalPublicationAcknowledgementKind::ExternalExternalPublicationAcknowledgementUnavailable => {
            return Receipt::ExternalExternalPublicationAcknowledgementUnavailableNoAcknowledgement;
        }
        DurableCompletionExternalPublicationAcknowledgementKind::Disabled => {
            return Receipt::LegacyBypassNoExternalPublicationAcknowledgement;
        }
        DurableCompletionExternalPublicationAcknowledgementKind::Unknown => {
            return Receipt::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement;
        }
        DurableCompletionExternalPublicationAcknowledgementKind::FixtureInMemory => {}
    }
    ...
    let recovered_matches =
        |record: &DurableCompletionExternalPublicationAcknowledgementLedgerRecord| -> bool {
            record.external_publication_acknowledgement_record_id == expectations.expected_external_publication_acknowledgement_record_id
                && record.request_digest == input.request.digest()
                && record.status
                    == DurableCompletionExternalPublicationAcknowledgementLedgerStatus::Recorded
        };

    match window {
        // Through settlement-confirmation success but before a external-publication-acknowledgement
        // request there is nothing to record a settlement external_publication_acknowledgement for.
        ...
        | Window::AfterConsumerSuccessBeforeExternalPublicationReceiptRequest
        | Window::AfterExternalPublicationReceiptRequestBeforeExternalPublicationReceiptRecord
        | Window::AfterExternalPublicationReceiptRecordBeforeExternalPublicationReceiptSuccess
        | Window::AfterExternalPublicationReceiptSuccessBeforeExternalPublicationAcknowledgementRequest => {
            Receipt::ExternalPublicationReceiptDidNotRecordNoAcknowledgement
        }
        // A external-publication-acknowledgement request without a record never records a
        // settlement external_publication_acknowledgement.
        Window::AfterExternalPublicationAcknowledgementRequestBeforeExternalPublicationAcknowledgementRecord => {
            Receipt::ExternalPublicationAcknowledgementRejectedBeforeRecord
        }
        // After a external-publication-acknowledgement record but before success: fails closed
        // unless an explicit matching, well-formed external-publication-acknowledgement record
        // exists.
        Window::AfterExternalPublicationAcknowledgementRecordBeforeExternalPublicationAcknowledgementSuccess => {
            match recovered_record {
                Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAcknowledgementRecorded,
                _ => Receipt::ExternalPublicationAcknowledgementRejectedBeforeRecord,
            }
        }
        // An explicit successful settlement external_publication_acknowledgement recovers as recorded only if
        // it matches.
        Window::AfterExternalPublicationAcknowledgementSuccess => match recovered_record {
            Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAcknowledgementRecorded,
            _ => Receipt::ExternalPublicationAcknowledgementRejectedBeforeRecord,
        },
        Window::AfterExternalPublicationAcknowledgementAmbiguous => {
            Receipt::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement
        }
        Window::ExternalPublicationAcknowledgementRecordFailed => {
            Receipt::ExternalPublicationAcknowledgementRecordFailedNoAcknowledgement
        }
        Window::ExternalPublicationAcknowledgementRollbackCompleted => {
            Receipt::ExternalPublicationAcknowledgementRolledBackNoAcknowledgement
        }
        Window::ExternalPublicationAcknowledgementRollbackFailed => {
            Receipt::ExternalPublicationAcknowledgementRollbackFailedFatalNoAcknowledgement
        }
        // Any unknown window fails closed.
        Window::Unknown => Receipt::ExternalPublicationAcknowledgementAmbiguousFailClosedNoAcknowledgement,
    }
}

/// Run 280 — `true` iff a receipt outcome authorizes a **new** modeled
/// audit/publication receipt (only
/// [`DurableCompletionExternalPublicationAcknowledgementOutcome::ExternalPublicationAcknowledgementRecorded`]).
pub fn external_publication_acknowledgement_outcome_authorizes_record(
    outcome: &DurableCompletionExternalPublicationAcknowledgementOutcome,
) -> bool {
    outcome.authorizes_record()
}

/// Run 280 — `true` iff a receipt outcome projects to a recorded audit/publication
/// receipt (a newly recorded receipt or an idempotent duplicate of an
/// already-recorded receipt).
pub fn external_publication_acknowledgement_outcome_projects_to_recorded(
    outcome: &DurableCompletionExternalPublicationAcknowledgementOutcome,
) -> bool {
...
/// Run 280 — a receipt rejection is non-mutating: it records no receipt, mutates no
/// `LivePqcTrustState`, and writes no durable state.
pub fn durable_completion_external_publication_acknowledgement_rejection_is_non_mutating() -> bool {
    true
}

/// Run 280 — the receipt boundary never calls Run 070.
pub fn durable_completion_external_publication_acknowledgement_never_calls_run_070() -> bool {
    true
}

/// Run 280 — the receipt boundary never mutates live PQC trust state.
pub fn durable_completion_external_publication_acknowledgement_never_mutates_live_pqc_trust_state() -> bool {
    true
}

/// Run 280 — the receipt boundary never writes a sequence or a marker.
pub fn durable_completion_external_publication_acknowledgement_never_writes_sequence_or_marker() -> bool {
    true
}

/// Run 280 — the receipt boundary changes no RocksDB file schema / migration.
pub fn durable_completion_external_publication_acknowledgement_no_rocksdb_file_schema_migration_change() -> bool {
    true
}

/// Run 280 — the receipt boundary performs no external publication.
pub fn durable_completion_external_publication_acknowledgement_no_external_publication() -> bool {
    true
}

/// Run 280 — the receipt boundary performs no real audit-ledger persistence.
pub fn durable_completion_external_publication_acknowledgement_no_real_audit_ledger() -> bool {
    true
}

/// Run 280 — a receipt requires a successful Run 246 pipeline outcome upstream.
pub fn durable_completion_external_publication_acknowledgement_pipeline_success_required() -> bool {
    true
}

/// Run 280 — a receipt requires a Run 248 sink receipt record upstream.
pub fn durable_completion_external_publication_acknowledgement_sink_receipt_required() -> bool {
    true
}

/// Run 280 — a receipt requires a Run 250 completion report upstream.
pub fn durable_completion_external_publication_acknowledgement_completion_report_required() -> bool {
    true
}

/// Run 280 — a receipt requires a Run 252 external_publication_acknowledgement upstream.
pub fn durable_completion_external_publication_acknowledgement_finalization_projection_required() -> bool {
    true
}

/// Run 280 — a receipt requires a Run 254 attestation upstream.
pub fn durable_completion_external_publication_acknowledgement_attestation_required() -> bool {
    true
}

/// Run 280 — an acknowledgement requires a Run 256 backend submission upstream.
pub fn durable_completion_external_publication_acknowledgement_backend_submission_required() -> bool {
    true
}

/// Run 280 — an acknowledgement requires a Run 258 audit receipt upstream.
pub fn durable_completion_external_publication_acknowledgement_receipt_required() -> bool {
    true
}

/// Run 280 — a consumer requires a Run 260 audit-receipt acknowledgement upstream.
pub fn durable_completion_external_publication_acknowledgement_consumer_required() -> bool {
    true
}

/// Run 280 — a settlement external_publication_acknowledgement requires a Run 278 settlement confirmation
/// upstream; no settlement external_publication_acknowledgement is authorized without a recorded
/// settlement confirmation.
pub fn durable_completion_external_publication_acknowledgement_confirmation_required() -> bool {
    true
}

/// Run 280 — the consumer boundary never performs a real settlement; production /
/// MainNet / external settlement consumers are reachable but fail closed.
pub fn durable_completion_external_publication_acknowledgement_no_real_settlement() -> bool {
    true
}

/// Run 280 — the external-publication-acknowledgement boundary never confers real settlement
/// finality; the only external-publication-acknowledgement record is a modeled in-memory fixture
/// record. Production / MainNet / external external-publication-acknowledgement sinks are reachable
/// but unavailable / fail closed and never confer any real finality.
pub fn durable_completion_external_publication_acknowledgement_no_real_settlement_finality() -> bool {
    true
}

/// Run 280 — the external-publication-acknowledgement boundary never emits a real settlement
/// receipt; the only external-publication-acknowledgement record is a modeled in-memory fixture
/// record with no external publication, network I/O, or persistent backend.
pub fn durable_completion_external_publication_acknowledgement_no_real_settlement_receipt() -> bool {
    true
}

/// Run 280 — the external-publication-acknowledgement boundary never confers a real
/// external-publication-receipt; the only external-publication-receipt
/// record is a modeled in-memory fixture record. Production / MainNet / external
/// external-publication-receipt sinks are reachable but unavailable / fail closed
/// and never confer any real acknowledgement.
pub fn durable_completion_external_publication_acknowledgement_no_real_external_publication_receipt() -> bool {
    true
}
...
/// backend.
pub fn durable_completion_external_publication_acknowledgement_no_real_settlement_finality_projection() -> bool {
    true
}

/// Run 280 — the external-publication-acknowledgement boundary never confers a real
/// external-publication-receipt; the only external-publication-receipt is a modeled
/// in-memory fixture record with no external publication, network I/O, or persistent
/// backend.
pub fn durable_completion_external_publication_acknowledgement_no_real_external_publication_acknowledgement() -> bool {
    true
}

/// Run 280 — a receipt record is required before a receipt is acknowledged.
pub fn durable_completion_external_publication_acknowledgement_record_required_before_reported() -> bool {
    true
}

/// Run 280 — a failed receipt record never records a receipt.
pub fn durable_completion_external_publication_acknowledgement_failed_record_never_records() -> bool {
    true
}

/// Run 280 — a rolled-back receipt record never records a receipt.
pub fn durable_completion_external_publication_acknowledgement_rollback_never_records() -> bool {
    true
}

/// Run 280 — an ambiguous after-record receipt window fails closed.
pub fn durable_completion_external_publication_acknowledgement_ambiguous_window_fails_closed() -> bool {
    true
}

/// Run 280 — MainNet peer-driven apply refusal precedes every receipt stage.
pub fn durable_completion_external_publication_acknowledgement_mainnet_peer_driven_apply_refused_first(
    environment: TrustBundleEnvironment,
) -> bool {
    environment == TrustBundleEnvironment::Mainnet
}

/// Run 280 — production / MainNet audit-ledger sinks are reachable but unavailable.
pub fn durable_completion_external_publication_acknowledgement_production_mainnet_unavailable() -> bool {
    true
}

/// Run 280 — the external-publication sink is reachable but unavailable.
pub fn durable_completion_external_publication_acknowledgement_external_unavailable() -> bool {
    true
}

/// Run 280 — validator-set rotation remains unsupported at the receipt boundary.
pub fn durable_completion_external_publication_acknowledgement_validator_set_rotation_unsupported() -> bool {
    true
}

/// Run 280 — policy-change actions remain unsupported at the receipt boundary.
pub fn durable_completion_external_publication_acknowledgement_policy_change_unsupported() -> bool {
    true
}

/// Run 280 — a local operator cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_acknowledgement_local_operator_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}

/// Run 280 — a peer majority cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_acknowledgement_peer_majority_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}