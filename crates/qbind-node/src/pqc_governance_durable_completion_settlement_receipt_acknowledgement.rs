//! Run 270 — source/test durable-completion **settlement-receipt acknowledgement /
//! settlement-finality projection interface boundary**.
//!
//! Source/test only. Run 270 captures **no** release-binary evidence and enables
...
//! settlement-receipt acknowledgement backend, a real settlement-finality projection
...
//! finalization / completion-report / durable-consume / persistent-replay backend, a
...
//!
//! Run 268
//! ([`crate::pqc_governance_durable_completion_settlement_finalization`])
//! proves that a modeled durable-completion *settlement finalization* is recorded
//! **only** after the Run 266 settlement-commitment stage recorded a settlement
//! commitment, terminating in the single settlement-finalization-recording outcome
//! [`DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRecorded`].
//!
//! Run 270 defines the **first typed interface** a future production settlement-receipt
//! acknowledgement or settlement-finality projection subsystem would use to *consume* a
//! durable-completion settlement finalization and prepare a settlement-receipt
//! acknowledgement intent and modeled in-memory settlement-finality projection receipt
//! **after** the Run 268 settlement-finalization stage produced
//! `SettlementFinalizationRecorded`. It is an **interface / projection boundary only**:
//! production / MainNet / external settlement-receipt acknowledgement implementations
...
//!
//! The settlement-receipt acknowledgement layer is a **model only**. It does not
//! implement a real settlement, a real settlement finality, a real settlement receipt, a
//! real settlement-receipt acknowledgement, a real settlement-finality projection, a
...
//! fixture settlement-receipt acknowledgement sink mutates only the in-memory
...
//!    settlement-commitment invocation, and settlement-finalization invocation;
//! 2. **legacy bypass** — a
//!    [`DurableCompletionSettlementReceiptAcknowledgementPolicy::Disabled`] policy preserves
//!    the legacy no-settlement-receipt-acknowledgement bypass and never invokes the
//!    settlement-receipt acknowledgement sink;
//! 3. **settlement-finalization-outcome projection** — only
//!    [`DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRecorded`]
//!    creates a settlement-receipt acknowledgement request; every other Run 268 outcome
...
//!    and Run 268 settlement-finalization digest sets) must match expectations *before*
//!    the settlement-receipt acknowledgement sink is invoked; a mismatch fails closed
//!    and leaves the sink invocation count at zero;
//! 5. **settlement-receipt acknowledgement record** — only after every prior gate
//!    passes is the sink invoked; the record-identity fields must match exactly before
//!    any modeled settlement-receipt acknowledgement record is recorded;
//! 6. **settlement-receipt acknowledgement authorization** — only
//!    [`DurableCompletionSettlementReceiptAcknowledgementOutcome::SettlementReceiptAcknowledgementRecorded`]
//!    authorizes a new modeled settlement-receipt acknowledgement / settlement-finality
//!    projection state.
//!
//! A settlement-receipt acknowledgement record failure, rollback, rollback failure, or
//! ambiguous window never retroactively claims a durable settlement-receipt
//! acknowledgement record. A duplicate identical settlement-receipt acknowledgement
//! record is idempotent; the same record id with a different digest fails closed as
//! equivocation and records no second record. A Run 268
//! [`DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationDuplicateIdempotent`]
//! never creates a new settlement-receipt acknowledgement record by itself — it can only
...
/// Run 270 — the validation / mutation surface pair the receipt binds to.
pub type DurableCompletionSettlementReceiptAcknowledgementSurface =
    ModeledGovernanceTrustMutationSurface;

/// Run 270 — the trust-domain environment binding the receipt is bound to.
pub type DurableCompletionSettlementReceiptAcknowledgementEnvironment =
    ModeledGovernanceTrustMutationEnvironmentBinding;

/// Run 270 — the runtime binding (governance + mutation surface + sequence) the
/// receipt is bound to.
pub type DurableCompletionSettlementReceiptAcknowledgementBinding =
    ModeledGovernanceTrustMutationRuntimeBinding;

/// Run 270 — the Run 240/246 durable replay observation carried as freshness
/// context.
pub type DurableCompletionSettlementReceiptAcknowledgementReplayBinding = DurableReplayObservation;

/// Run 270 — the Run 246 pipeline outcome carried as consume authorization context.
pub type DurableCompletionSettlementReceiptAcknowledgementPipelineBinding =
    GovernanceModeledEndToEndPipelineOutcome;

/// Run 270 — the Run 248 sink outcome carried as receipt-record context.
pub type DurableCompletionSettlementReceiptAcknowledgementSinkBinding =
    GovernanceModeledDurableConsumeSinkOutcome;

/// Run 270 — the Run 250 reporter outcome carried as completion-report context.
pub type DurableCompletionSettlementReceiptAcknowledgementReporterBinding =
    GovernanceModeledDurableConsumeCompletionReporterOutcome;

/// Run 270 — the Run 252 receipt_acknowledgement outcome carried as receipt_acknowledgement context.
pub type DurableCompletionSettlementReceiptAcknowledgementFinalizationBinding =
    GovernanceModeledDurableCompletionFinalizationOutcome;

/// Run 270 — the Run 254 attestation outcome carried as attestation context.
pub type DurableCompletionSettlementReceiptAcknowledgementAttestationBinding =
    GovernanceModeledDurableCompletionAttestationOutcome;

/// Run 270 — the Run 256 backend outcome carried as backend-record context. The
/// acknowledgement boundary never reimplements the backend; it only carries its
/// terminal outcome.
pub type DurableCompletionSettlementReceiptAcknowledgementBackendBinding =
    DurableCompletionAttestationBackendOutcome;

/// Run 270 — the Run 258 audit/publication receipt outcome carried as
/// receipt-record context. The consumer boundary never reimplements the receipt; it
/// only carries its terminal outcome.
pub type DurableCompletionSettlementReceiptAcknowledgementReceiptBinding =
    DurableCompletionAuditPublicationReceiptOutcome;

/// Run 270 — the Run 260 audit-receipt acknowledgement outcome carried as
/// acknowledgement-record context. The settlement-finalization boundary never
/// reimplements the acknowledgement; it only carries its terminal outcome.
pub type DurableCompletionSettlementReceiptAcknowledgementAcknowledgementBinding =
    DurableCompletionAuditReceiptAcknowledgementOutcome;

/// Run 270 — the Run 262 acknowledgement consumer outcome the settlement-finalization
...
pub type DurableCompletionSettlementReceiptAcknowledgementConsumerBinding =
    DurableCompletionAcknowledgementConsumerOutcome;

/// Run 270 — the Run 268 settlement-finalization outcome the settlement-receipt-acknowledgement
/// boundary projects to a settlement-receipt-acknowledgement request. The settlement-receipt-acknowledgement
/// boundary never reimplements the settlement finalization; it only projects its
/// terminal outcome.
pub type DurableCompletionSettlementReceiptAcknowledgementSettlementFinalizationBinding =
...
/// Run 270 — the typed durable-completion audit/publication receipt kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionSettlementReceiptAcknowledgementKind {
    ...
    /// fail-closed).
    ProductionSettlementReceiptAcknowledgementUnavailable,
    /// MainNet audit-ledger receipt sink (reachable-but-unavailable / fail-closed).
    MainNetSettlementReceiptAcknowledgementUnavailable,
    /// External-publication receipt sink (reachable-but-unavailable / fail-closed).
    ExternalSettlementReceiptAcknowledgementUnavailable,
    /// An unknown receipt kind — fails closed.
    Unknown,
}

impl DurableCompletionSettlementReceiptAcknowledgementKind {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureInMemory => "fixture-in-memory",
            Self::ProductionSettlementReceiptAcknowledgementUnavailable => {
                "production-settlement-receipt-acknowledgement-unavailable"
            }
            Self::MainNetSettlementReceiptAcknowledgementUnavailable => {
                "mainnet-settlement-receipt-acknowledgement-unavailable"
            }
            Self::ExternalSettlementReceiptAcknowledgementUnavailable => {
                "external-settlement-receipt-acknowledgement-unavailable"
            }
...
/// Run 270 — the typed durable-completion audit/publication receipt policy
/// selector.
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionSettlementReceiptAcknowledgementPolicy {
    /// The receipt boundary is disabled (legacy bypass).
    Disabled,
    ...
    /// unavailable.
    ProductionSettlementReceiptAcknowledgementRequired,
    /// A real MainNet audit-ledger receipt sink is required — reachable but
    /// unavailable.
    MainNetSettlementReceiptAcknowledgementRequired,
    /// A real external-publication receipt sink is required — reachable but
    /// unavailable.
    ExternalSettlementReceiptAcknowledgementRequired,
}

impl DurableCompletionSettlementReceiptAcknowledgementPolicy {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
...
/// Run 270 — the typed receipt identity a receipt request is bound to.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionSettlementReceiptAcknowledgementIdentity {
    /// Stable receipt id.
    pub finalization_id: String,
    /// The receipt kind.
    pub kind: DurableCompletionSettlementReceiptAcknowledgementKind,
    /// The receipt policy.
    pub policy: DurableCompletionSettlementReceiptAcknowledgementPolicy,
    /// The domain separation tag the receipt operates under.
    pub domain_separation_tag: String,
}

impl DurableCompletionSettlementReceiptAcknowledgementIdentity {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.finalization_id.is_empty()
            ...
            && self.kind != DurableCompletionSettlementReceiptAcknowledgementKind::Unknown
    }

    /// The deterministic, domain-separated receipt identity digest.
    pub fn digest(&self) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
        settlement_receipt_acknowledgement_identity_digest(self)
    }
}
...
/// Run 270 — domain separator for the receipt identity digest.
const RECEIPT_ACKNOWLEDGEMENT_IDENTITY_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-settlement-receipt-acknowledgement-identity:v1";
/// Run 270 — domain separator for the receipt request digest.
const RECEIPT_ACKNOWLEDGEMENT_REQUEST_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-settlement-receipt-acknowledgement-request:v1";
/// Run 270 — domain separator for the receipt response digest.
const RECEIPT_ACKNOWLEDGEMENT_RESPONSE_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-settlement-receipt-acknowledgement-response:v1";
/// Run 270 — domain separator for the receipt record digest.
const RECEIPT_ACKNOWLEDGEMENT_RECORD_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-settlement-receipt-acknowledgement-record:v1";
/// Run 270 — domain separator for the receipt transcript digest.
const RECEIPT_ACKNOWLEDGEMENT_TRANSCRIPT_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-settlement-receipt-acknowledgement-transcript:v1";

/// Run 270 — a deterministic, domain-separated receipt digest (lowercase hex of a
/// SHA3-256 over length-prefixed, field-bound canonical material). It contains no
/// unstable display text.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionSettlementReceiptAcknowledgementDigest(String);

impl DurableCompletionSettlementReceiptAcknowledgementDigest {
...
/// Run 270 — a deterministic, domain-separated receipt transcript digest.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionSettlementReceiptAcknowledgementTranscriptDigest(String);

impl DurableCompletionSettlementReceiptAcknowledgementTranscriptDigest {
    /// The lowercase hex representation.
    pub fn as_hex(&self) -> &str {
        &self.0
    }
...
/// Run 270 — deterministic, domain-separated receipt identity digest.
pub fn settlement_receipt_acknowledgement_identity_digest(
    identity: &DurableCompletionSettlementReceiptAcknowledgementIdentity,
    ...
    w.str_field(&identity.finalization_id)
        .str_field(identity.kind.tag())
        .str_field(identity.policy.tag())
        .str_field(&identity.domain_separation_tag);
    DurableCompletionSettlementReceiptAcknowledgementDigest(w.finish())
}

/// Run 270 — deterministic, domain-separated receipt request digest. Binds every
/// request field including the full Run 256 backend digest binding.
pub fn settlement_receipt_acknowledgement_request_digest(
    ...
    w.str_field(&request.receipt_acknowledgement_record_id)
        ...
        .str_field(&request.settlement_finalization_identity_digest)
        .str_field(&request.settlement_finalization_request_digest)
        .str_field(&request.settlement_finalization_response_digest)
        .str_field(&request.settlement_finalization_record_digest)
        .str_field(&request.settlement_finalization_transcript_digest)
        .str_field(&request.settlement_finalization_record_id)
        .str_field(&request.domain_separation_tag)
        .str_field(settlement_receipt_acknowledgement_identity_digest(&request.identity).as_hex());
    DurableCompletionSettlementReceiptAcknowledgementDigest(w.finish())
}

/// Run 270 — deterministic, domain-separated receipt response digest. Binds the
/// receipt record id, the request digest it answers, the acceptance flag, and the
/// responding receipt kind.
pub fn settlement_receipt_acknowledgement_response_digest(
    response: &DurableCompletionSettlementReceiptAcknowledgementResponse,
) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
    let mut w = CanonicalWriter::new(RECEIPT_ACKNOWLEDGEMENT_RESPONSE_DOMAIN);
    w.str_field(&response.receipt_acknowledgement_record_id)
        ...
        .str_field(response.receipt_acknowledgement_kind.tag());
    DurableCompletionSettlementReceiptAcknowledgementDigest(w.finish())
}

/// Run 270 — deterministic, domain-separated receipt record digest. Binds the
/// receipt record id, the request digest, and the receipt identity digest.
pub fn settlement_receipt_acknowledgement_record_digest(
    record: &DurableCompletionSettlementReceiptAcknowledgementRecord,
) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
    let mut w = CanonicalWriter::new(RECEIPT_ACKNOWLEDGEMENT_RECORD_DOMAIN);
    w.str_field(&record.receipt_acknowledgement_record_id)
        .str_field(record.request_digest.as_hex())
        .str_field(record.identity_digest.as_hex());
    DurableCompletionSettlementReceiptAcknowledgementDigest(w.finish())
}

/// Run 270 — deterministic, domain-separated receipt transcript digest. Binds the
/// request, response, and record digests into a single transcript binding.
pub fn settlement_receipt_acknowledgement_transcript_digest(
    request_digest: &DurableCompletionSettlementReceiptAcknowledgementDigest,
    response_digest: &DurableCompletionSettlementReceiptAcknowledgementDigest,
    record_digest: &DurableCompletionSettlementReceiptAcknowledgementDigest,
    ...
    DurableCompletionSettlementReceiptAcknowledgementTranscriptDigest(w.finish())
}
...
/// Run 270 — the typed receipt request a future production audit ledger or
...
/// attestation / Run 252 receipt_acknowledgement / Run 250 completion report / Run 248
/// receipt / Run 246 decision material — never a copy of any wire payload and never
/// a production durable record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementRequest {
    /// Stable receipt record id (the idempotency key of the receipt).
    pub receipt_acknowledgement_record_id: String,
    ...
    /// Run 252 receipt_acknowledgement decision digest.
    ...
    /// Run 268 settlement-finalization identity digest.
    pub settlement_finalization_identity_digest: String,
    /// Run 268 settlement-finalization request digest.
    pub settlement_finalization_request_digest: String,
    /// Run 268 settlement-finalization response digest.
    pub settlement_finalization_response_digest: String,
    /// Run 268 settlement-finalization record digest.
    pub settlement_finalization_record_digest: String,
    /// Run 268 settlement-finalization transcript digest.
    pub settlement_finalization_transcript_digest: String,
    /// Run 268 settlement-finalization record id.
    pub settlement_finalization_record_id: String,
    /// Settlement-receipt-acknowledgement identity.
    pub identity: DurableCompletionSettlementReceiptAcknowledgementIdentity,
    /// Domain separation tag.
    pub domain_separation_tag: String,
}

impl DurableCompletionSettlementReceiptAcknowledgementRequest {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.receipt_acknowledgement_record_id.is_empty()
            ...
            && !self.settlement_finalization_identity_digest.is_empty()
            && !self.settlement_finalization_request_digest.is_empty()
            && !self.settlement_finalization_response_digest.is_empty()
            && !self.settlement_finalization_record_digest.is_empty()
            && !self.settlement_finalization_transcript_digest.is_empty()
            && !self.settlement_finalization_record_id.is_empty()
            && !self.domain_separation_tag.is_empty()
    ...
    pub fn digest(&self) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
        settlement_receipt_acknowledgement_request_digest(self)
    }

    /// The canonical immutable record derived from this request.
    pub fn to_record(&self) -> DurableCompletionSettlementReceiptAcknowledgementRecord {
        DurableCompletionSettlementReceiptAcknowledgementRecord {
            receipt_acknowledgement_record_id: self.receipt_acknowledgement_record_id.clone(),
...
/// Run 270 — the typed receipt response a receipt sink returns for a request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementResponse {
    /// The receipt record id the response answers.
    pub receipt_acknowledgement_record_id: String,
    /// The request digest the response answers.
    pub request_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
    /// `true` iff the receipt sink accepted the request.
    pub accepted: bool,
    /// The responding receipt kind.
    pub receipt_acknowledgement_kind: DurableCompletionSettlementReceiptAcknowledgementKind,
}

impl DurableCompletionSettlementReceiptAcknowledgementResponse {
    ...
    pub fn digest(&self) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
        settlement_receipt_acknowledgement_response_digest(self)
    }
}

/// Run 270 — the canonical immutable receipt record derived from a request. Two
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionSettlementReceiptAcknowledgementRecord {
    /// The receipt record id.
    pub receipt_acknowledgement_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
    /// The receipt identity digest.
    pub identity_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
}

impl DurableCompletionSettlementReceiptAcknowledgementRecord {
    /// The deterministic receipt record digest.
    pub fn digest(&self) -> DurableCompletionSettlementReceiptAcknowledgementDigest {
        settlement_receipt_acknowledgement_record_digest(self)
    }
}
...
/// Run 270 — the recorded status of a modeled audit/publication receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionSettlementReceiptAcknowledgementLedgerStatus {
    /// The modeled receipt is recorded in the in-memory fixture ledger.
    Recorded,
}

/// Run 270 — a single modeled receipt record held in the in-memory fixture ledger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementLedgerRecord {
    /// The receipt record id (stable identity of the receipt).
    pub receipt_acknowledgement_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
    /// The response digest.
    pub response_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
    /// The record digest.
    pub record_digest: DurableCompletionSettlementReceiptAcknowledgementDigest,
    /// The transcript digest.
    pub transcript_digest: DurableCompletionSettlementReceiptAcknowledgementTranscriptDigest,
    /// The recorded status.
    pub status: DurableCompletionSettlementReceiptAcknowledgementLedgerStatus,
}

/// Run 270 — an immutable snapshot of the modeled receipt ledger used to model a
/// fixture rollback.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot {
    records: Vec<DurableCompletionSettlementReceiptAcknowledgementLedgerRecord>,
}

impl DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot {
...
/// Run 270 — the modeled in-memory receipt ledger.
///
...
pub struct DurableCompletionSettlementReceiptAcknowledgementLedger {
    records: Vec<DurableCompletionSettlementReceiptAcknowledgementLedgerRecord>,
}

impl DurableCompletionSettlementReceiptAcknowledgementLedger {
    ...
    pub fn records(&self) -> &[DurableCompletionSettlementReceiptAcknowledgementLedgerRecord] {
        &self.records
    }

    /// The record for `receipt_acknowledgement_record_id`, if present.
    pub fn find(
        &self,
        receipt_acknowledgement_record_id: &str,
    ) -> Option<&DurableCompletionSettlementReceiptAcknowledgementLedgerRecord> {
        self.records
            .iter()
            .find(|r| r.receipt_acknowledgement_record_id == receipt_acknowledgement_record_id)
    }

    /// `true` iff a receipt with `receipt_acknowledgement_record_id` is recorded.
    pub fn contains(&self, receipt_acknowledgement_record_id: &str) -> bool {
        self.find(receipt_acknowledgement_record_id).is_some()
    }

    /// Capture an immutable snapshot for a modeled rollback.
    pub fn snapshot(&self) -> DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot {
    ...
    pub fn restore(
        &mut self,
        snapshot: &DurableCompletionSettlementReceiptAcknowledgementLedgerSnapshot,
    ) {
    ...
    fn insert(&mut self, record: DurableCompletionSettlementReceiptAcknowledgementLedgerRecord) {
...
/// Run 270 — the canonical binding a [`DurableCompletionSettlementReceiptAcknowledgementInput`]
/// is checked against.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementExpectations {
    /// Expected receipt record id.
    pub expected_receipt_acknowledgement_record_id: String,
    ...
    /// Expected Run 252 receipt_acknowledgement decision digest.
    ...
    /// Expected Run 268 settlement-finalization identity digest.
    pub expected_settlement_finalization_identity_digest: String,
    /// Expected Run 268 settlement-finalization request digest.
    pub expected_settlement_finalization_request_digest: String,
    /// Expected Run 268 settlement-finalization response digest.
    pub expected_settlement_finalization_response_digest: String,
    /// Expected Run 268 settlement-finalization record digest.
    pub expected_settlement_finalization_record_digest: String,
    /// Expected Run 268 settlement-finalization transcript digest.
    pub expected_settlement_finalization_transcript_digest: String,
    /// Expected Run 268 settlement-finalization record id.
    pub expected_settlement_finalization_record_id: String,
...
impl DurableCompletionSettlementReceiptAcknowledgementExpectations {
    /// Internal: the first environment / surface binding mismatch reason, if any.
    fn binding_mismatch_reason(
        &self,
        input: &DurableCompletionSettlementReceiptAcknowledgementInput,
    ) -> Option<&'static str> {
    ...
    fn request_mismatch_reason(
        ...
        if request.finalization_decision_digest != self.expected_finalization_decision_digest {
            return Some("wrong receipt_acknowledgement decision digest");
        }
        ...
        if request.settlement_finalization_identity_digest
            != self.expected_settlement_finalization_identity_digest
        {
            return Some("wrong settlement-finalization identity digest");
        }
        if request.settlement_finalization_request_digest
            != self.expected_settlement_finalization_request_digest
        {
            return Some("wrong settlement-finalization request digest");
        }
        if request.settlement_finalization_response_digest
            != self.expected_settlement_finalization_response_digest
        {
            return Some("wrong settlement-finalization response digest");
        }
        if request.settlement_finalization_record_digest
            != self.expected_settlement_finalization_record_digest
        {
            return Some("wrong settlement-finalization record digest");
        }
        if request.settlement_finalization_transcript_digest
            != self.expected_settlement_finalization_transcript_digest
        {
        ...
        if request.identity != self.expected_identity {
            return Some("wrong settlement-receipt-acknowledgement identity");
        }
        if request.identity.kind != self.expected_receipt_acknowledgement_kind {
            return Some("wrong settlement-receipt-acknowledgement kind");
        }
        if request.identity.policy != self.expected_receipt_acknowledgement_policy {
            return Some("wrong settlement-receipt-acknowledgement policy");
        }
        None
    }
...
/// Run 270 — typed inputs for one modeled durable-completion audit/publication
/// receipt round-trip.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionSettlementReceiptAcknowledgementInput {
    /// The receipt policy selector.
    pub policy: DurableCompletionSettlementReceiptAcknowledgementPolicy,
    /// The trust-domain environment binding.
    pub environment_binding: DurableCompletionSettlementReceiptAcknowledgementEnvironment,
    /// The runtime binding (governance + mutation surface + sequence).
    pub runtime_binding: DurableCompletionSettlementReceiptAcknowledgementBinding,
    /// The Run 240/246 durable replay observation the pipeline authorized consume
    /// under.
    pub replay_binding: DurableCompletionSettlementReceiptAcknowledgementReplayBinding,
    /// The Run 246 pipeline outcome.
    pub pipeline_binding: DurableCompletionSettlementReceiptAcknowledgementPipelineBinding,
    /// The Run 248 sink outcome.
    pub sink_binding: DurableCompletionSettlementReceiptAcknowledgementSinkBinding,
    /// The Run 250 reporter outcome.
    pub reporter_binding: DurableCompletionSettlementReceiptAcknowledgementReporterBinding,
    /// The Run 252 receipt_acknowledgement outcome.
    pub finalization_binding: DurableCompletionSettlementReceiptAcknowledgementFinalizationBinding,
    /// The Run 254 attestation outcome.
    pub attestation_binding: DurableCompletionSettlementReceiptAcknowledgementAttestationBinding,
    /// The Run 256 backend outcome carried as backend-record context.
    pub backend_binding: DurableCompletionSettlementReceiptAcknowledgementBackendBinding,
    /// The Run 258 audit/publication receipt outcome carried as receipt-record
    /// context.
    pub receipt_binding: DurableCompletionSettlementReceiptAcknowledgementReceiptBinding,
    /// The Run 260 audit-receipt acknowledgement outcome carried as
    /// acknowledgement-record context.
    pub acknowledgement_binding:
        DurableCompletionSettlementReceiptAcknowledgementAcknowledgementBinding,
    /// The Run 262 acknowledgement consumer outcome the settlement-finalization
    /// boundary projects to a settlement-finalization request.
    pub consumer_binding: DurableCompletionSettlementReceiptAcknowledgementConsumerBinding,
    /// The Run 268 settlement-finalization outcome the settlement-receipt-acknowledgement boundary
    /// projects to a settlement-receipt-acknowledgement request.
    pub settlement_finalization_binding:
        DurableCompletionSettlementReceiptAcknowledgementSettlementFinalizationBinding,
    /// The settlement-receipt-acknowledgement request the call site would submit.
    pub request: DurableCompletionSettlementReceiptAcknowledgementRequest,
}

impl DurableCompletionSettlementReceiptAcknowledgementInput {
    ...
    pub fn surface(&self) -> DurableCompletionSettlementReceiptAcknowledgementSurface {
    ...
    pub fn is_mainnet_peer_driven(&self) -> bool {
        (self.environment() == TrustBundleEnvironment::Mainnet && self.surface().is_peer_driven())
            ...
            || matches!(
                self.settlement_finalization_binding,
                DurableCompletionSettlementFinalizationOutcome::MainNetPeerDrivenApplyRefusedNoFinalization
            )
    }
}
...
/// Run 270 — the typed outcome of one modeled durable-completion audit/publication
/// receipt round-trip.
///
/// Only [`Self::SettlementReceiptAcknowledgementRecorded`] authorizes a **new** modeled
/// audit/publication-receipt state. A [`Self::SettlementReceiptAcknowledgementDuplicateIdempotent`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionSettlementReceiptAcknowledgementOutcome {
    /// Legacy bypass — a disabled receipt policy preserved the legacy
    /// no-audit-receipt path. No receipt invocation.
    LegacyBypassNoSettlementReceiptAcknowledgement,
    ...
    RejectedBeforeSettlementFinalizationNoReceiptAcknowledgement,
    ...
    /// invocation.
    SettlementFinalizationDidNotRecordNoReceiptAcknowledgement,
    ...
    SettlementReceiptAcknowledgementRecorded,
    /// A duplicate identical receipt — idempotent; no second receipt recorded.
    SettlementReceiptAcknowledgementDuplicateIdempotent,
    ...
    /// receipt.
    SettlementReceiptAcknowledgementRejectedBeforeRecord,
    /// The receipt record failed. No receipt.
    SettlementReceiptAcknowledgementRecordFailedNoReceiptAcknowledgement,
    /// The receipt record was rolled back. No receipt.
    SettlementReceiptAcknowledgementRolledBackNoReceiptAcknowledgement,
    /// The receipt rollback itself failed — fatal / fail-closed. No receipt.
    SettlementReceiptAcknowledgementRollbackFailedFatalNoReceiptAcknowledgement,
    /// The after-record receipt window was ambiguous — fails closed. No receipt.
    SettlementReceiptAcknowledgementAmbiguousFailClosedNoReceiptAcknowledgement,
    /// The production audit-ledger path was reached but is unavailable. No receipt.
    ProductionSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
    /// The MainNet audit-ledger path was reached but is unavailable. No receipt.
    MainNetSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
    /// The external-publication path was reached but is unavailable. No receipt.
    ExternalSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement,
    ...
    MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement,
    /// Validator-set rotation is unsupported. No receipt.
    ValidatorSetRotationUnsupportedNoReceiptAcknowledgement,
    /// Policy-change actions are unsupported. No receipt.
    PolicyChangeUnsupportedNoReceiptAcknowledgement,
}

impl DurableCompletionSettlementReceiptAcknowledgementOutcome {
    /// `true` iff this outcome authorizes a **new** modeled audit/publication
    /// receipt (only [`Self::SettlementReceiptAcknowledgementRecorded`]).
    pub fn authorizes_record(&self) -> bool {
        matches!(self, Self::SettlementReceiptAcknowledgementRecorded)
    }
    ...
    pub fn is_legacy_bypass(&self) -> bool {
        matches!(self, Self::LegacyBypassNoSettlementReceiptAcknowledgement)
    }
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::LegacyBypassNoSettlementReceiptAcknowledgement => "legacy-bypass-no-settlement-receipt-acknowledgement",
            Self::RejectedBeforeSettlementFinalizationNoReceiptAcknowledgement => {
                "rejected-before-settlement-finalization-no-receipt-acknowledgement"
            }
            Self::SettlementFinalizationDidNotRecordNoReceiptAcknowledgement => {
                "settlement-finalization-did-not-record-no-receipt-acknowledgement"
            }
            Self::SettlementReceiptAcknowledgementRecorded => "settlement-receipt-acknowledgement-recorded",
            Self::SettlementReceiptAcknowledgementDuplicateIdempotent => {
                "settlement-receipt-acknowledgement-duplicate-idempotent"
            }
            Self::SettlementReceiptAcknowledgementRejectedBeforeRecord => {
                "settlement-receipt-acknowledgement-rejected-before-record"
            }
            Self::SettlementReceiptAcknowledgementRecordFailedNoReceiptAcknowledgement => {
                "settlement-receipt-acknowledgement-record-failed-no-receipt-acknowledgement"
            }
            Self::SettlementReceiptAcknowledgementRolledBackNoReceiptAcknowledgement => {
                "settlement-receipt-acknowledgement-rolled-back-no-receipt-acknowledgement"
            }
            Self::SettlementReceiptAcknowledgementRollbackFailedFatalNoReceiptAcknowledgement => {
                "settlement-receipt-acknowledgement-rollback-failed-fatal-no-receipt-acknowledgement"
            }
            Self::SettlementReceiptAcknowledgementAmbiguousFailClosedNoReceiptAcknowledgement => {
                "settlement-receipt-acknowledgement-ambiguous-fail-closed-no-receipt-acknowledgement"
            }
            Self::ProductionSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement => {
                "production-settlement-receipt-acknowledgement-unavailable-no-receipt-acknowledgement"
            }
            Self::MainNetSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement => {
                "mainnet-settlement-receipt-acknowledgement-unavailable-no-receipt-acknowledgement"
            }
            Self::ExternalSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement => {
                "external-settlement-receipt-acknowledgement-unavailable-no-receipt-acknowledgement"
            }
            Self::MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement => {
                "mainnet-peer-driven-apply-refused-no-receipt-acknowledgement"
            }
            Self::ValidatorSetRotationUnsupportedNoReceiptAcknowledgement => {
                "validator-set-rotation-unsupported-no-receipt-acknowledgement"
            }
            Self::PolicyChangeUnsupportedNoReceiptAcknowledgement => "policy-change-unsupported-no-receipt-acknowledgement",
        }
    }
}

// ===========================================================================
// Consumer-outcome -> settlement-finalization request finalization
// ===========================================================================

/// Run 270 — the typed finalization of a Run 262 acknowledgement consumer outcome
/// onto a settlement-finalization request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionSettlementReceiptAcknowledgementRequestIntent {
    /// The consumer recorded a consumer record; the settlement-finalization sink may
    /// record a new settlement finalization.
    CreateRequest,
    /// The consumer reported an idempotent-duplicate consumer record; the
    /// settlement-finalization sink may only match an already-recorded settlement
    /// finalization and must never create a new one.
    IdempotentOnly,
    /// The consumer did not record; no settlement-finalization request. Carries the
    /// typed no-finalization outcome the settlement-finalization evaluation returns
    /// directly (without invoking the settlement-finalization sink).
    NoFinalization(DurableCompletionSettlementReceiptAcknowledgementOutcome),
}

impl DurableCompletionSettlementReceiptAcknowledgementRequestIntent {
    /// `true` iff this finalization creates a settlement-finalization request (i.e. the
...
/// Run 270 — project a Run 268 settlement-finalization outcome onto a
/// settlement-receipt-acknowledgement request.
///
/// Only
/// [`DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationRecorded`]
/// creates a settlement-receipt-acknowledgement request.
/// [`DurableCompletionSettlementFinalizationOutcome::SettlementFinalizationDuplicateIdempotent`]
/// may only match an already-recorded settlement-receipt-acknowledgement record and never creates
/// a new one. Every other settlement-finalization outcome maps to a no-receipt-acknowledgement
/// fail-closed outcome (a more specific one where one exists, otherwise the generic
/// [`DurableCompletionSettlementReceiptAcknowledgementOutcome::SettlementFinalizationDidNotRecordNoReceiptAcknowledgement`]).
/// Pure: performs no work and never records.
pub fn project_settlement_finalization_outcome_to_receipt_acknowledgement_request(
    outcome: &DurableCompletionSettlementReceiptAcknowledgementSettlementFinalizationBinding,
...
/// Run 270 — a modeled fault the fixture receipt sink injects to exercise the
/// fail-closed record / rollback / ambiguous paths. Source/test only.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionSettlementReceiptAcknowledgementFault {
...
/// Run 270 — the pure/mockable modeled durable-completion audit/publication receipt
/// boundary.
///
...
/// [`DurableCompletionSettlementReceiptAcknowledgementLedger`].
pub trait GovernanceDurableCompletionSettlementReceiptAcknowledgementSink {
    /// The receipt kind (used for typed recovery classification).
    fn kind(&self) -> DurableCompletionSettlementReceiptAcknowledgementKind;
    ...
    fn project_durable_completion_settlement_receipt_acknowledgement(
        &mut self,
        request: &DurableCompletionSettlementReceiptAcknowledgementRequest,
        expectations: &DurableCompletionSettlementReceiptAcknowledgementExpectations,
        idempotent_only: bool,
        ledger: &mut DurableCompletionSettlementReceiptAcknowledgementLedger,
    ...
    fn recover_durable_completion_settlement_receipt_acknowledgement_window(
        &self,
        input: &DurableCompletionSettlementReceiptAcknowledgementInput,
        window: DurableCompletionSettlementReceiptAcknowledgementWindow,
        recovered_record: Option<&DurableCompletionSettlementReceiptAcknowledgementLedgerRecord>,
        expectations: &DurableCompletionSettlementReceiptAcknowledgementExpectations,
    ) -> DurableCompletionSettlementReceiptAcknowledgementOutcome {
...
/// Run 270 — the DevNet/TestNet in-memory fixture receipt sink.
///
...
#[derive(Debug, Clone)]
pub struct FixtureDurableCompletionSettlementReceiptAcknowledgementSink {
    fault: Option<DurableCompletionSettlementReceiptAcknowledgementFault>,
    invocations: u32,
}

impl Default for FixtureDurableCompletionSettlementReceiptAcknowledgementSink {
    fn default() -> Self {
        Self::new()
    }
}

impl FixtureDurableCompletionSettlementReceiptAcknowledgementSink {
    ...
    pub fn with_fault(fault: DurableCompletionSettlementReceiptAcknowledgementFault) -> Self {
        Self {
            fault: Some(fault),
            invocations: 0,
        }
...
impl GovernanceDurableCompletionSettlementReceiptAcknowledgementSink
    for FixtureDurableCompletionSettlementReceiptAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionSettlementReceiptAcknowledgementKind {
    ...
    fn project_durable_completion_settlement_receipt_acknowledgement(
        ...
        use DurableCompletionSettlementReceiptAcknowledgementOutcome as Receipt;
        self.invocations += 1;
        ...
        if let Some(fault) = self.fault {
            ...
            return match fault {
                DurableCompletionSettlementReceiptAcknowledgementFault::RecordFailedNoFinalization => {
                    ...
                    Receipt::SettlementReceiptAcknowledgementRecordFailedNoReceiptAcknowledgement
                }
                DurableCompletionSettlementReceiptAcknowledgementFault::RolledBackNoFinalization => {
                    ledger.restore(&snapshot);
                    Receipt::SettlementReceiptAcknowledgementRolledBackNoReceiptAcknowledgement
                }
                DurableCompletionSettlementReceiptAcknowledgementFault::RollbackFailedFatal => {
                    Receipt::SettlementReceiptAcknowledgementRollbackFailedFatalNoReceiptAcknowledgement
                }
                DurableCompletionSettlementReceiptAcknowledgementFault::AmbiguousAfterRecord => {
                    ledger.restore(&snapshot);
                    Receipt::SettlementReceiptAcknowledgementAmbiguousFailClosedNoReceiptAcknowledgement
                }
            };
        }
        ...
        let response = DurableCompletionSettlementReceiptAcknowledgementResponse {
        ...
        let transcript_digest = settlement_receipt_acknowledgement_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
        );
        ...
        if let Some(existing) = ledger.find(&request.receipt_acknowledgement_record_id) {
            ...
            {
                return Receipt::SettlementReceiptAcknowledgementDuplicateIdempotent;
            }
...
/// Run 270 — the production audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ProductionSettlementReceiptAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionSettlementReceiptAcknowledgementSink
    for ProductionSettlementReceiptAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionSettlementReceiptAcknowledgementKind {
    ...
    fn project_durable_completion_settlement_receipt_acknowledgement(
        &mut self,
        _request: &DurableCompletionSettlementReceiptAcknowledgementRequest,
        _expectations: &DurableCompletionSettlementReceiptAcknowledgementExpectations,
        _idempotent_only: bool,
        _ledger: &mut DurableCompletionSettlementReceiptAcknowledgementLedger,
    ) -> DurableCompletionSettlementReceiptAcknowledgementOutcome {
        self.invocations += 1;
...
/// Run 270 — the MainNet audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct MainNetSettlementReceiptAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionSettlementReceiptAcknowledgementSink
    for MainNetSettlementReceiptAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionSettlementReceiptAcknowledgementKind {
...
/// Run 270 — the external-publication receipt sink. Reachable but unavailable /
/// fail-closed. It performs no external publication, records no receipt, and
/// mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ExternalSettlementReceiptAcknowledgementSink {
    invocations: u32,
}

impl GovernanceDurableCompletionSettlementReceiptAcknowledgementSink
    for ExternalSettlementReceiptAcknowledgementSink
{
    fn kind(&self) -> DurableCompletionSettlementReceiptAcknowledgementKind {
...
/// Run 270 — evaluate one modeled durable-completion audit/publication receipt
/// round-trip.
///
...
/// 2. legacy bypass — a
///    [`DurableCompletionSettlementReceiptAcknowledgementPolicy::Disabled`] policy;
/// 3. backend-outcome finalization — only
...
pub fn evaluate_durable_completion_settlement_receipt_acknowledgement<S>(
    input: &DurableCompletionSettlementReceiptAcknowledgementInput,
    expectations: &DurableCompletionSettlementReceiptAcknowledgementExpectations,
    sink: &mut S,
    ledger: &mut DurableCompletionSettlementReceiptAcknowledgementLedger,
...
where
    S: GovernanceDurableCompletionSettlementReceiptAcknowledgementSink,
{
    ...
    // acknowledgement invocation, consumer invocation, and settlement-finalization
    // invocation.
    if input.is_mainnet_peer_driven() {
        return Finalization::MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement;
    }

    // Step 2: legacy bypass — a disabled settlement-finalization policy preserves the
    // legacy no-settlement-finalization path and never invokes the finalization sink.
    if input.policy.is_disabled() {
        return Finalization::LegacyBypassNoSettlementReceiptAcknowledgement;
    }

    // Step 3: project the Run 262 acknowledgement consumer outcome onto a
    // settlement-finalization request. Every non-recording consumer outcome returns a
    // no-finalization outcome without invoking the settlement-finalization sink.
    let idempotent_only =
        match project_settlement_finalization_outcome_to_receipt_acknowledgement_request(
            &input.settlement_finalization_binding,
        ) {
            Intent::NoFinalization(outcome) => return outcome,
            Intent::CreateRequest => false,
            Intent::IdempotentOnly => true,
        };

    // Step 4: pre-finalization environment / surface binding validation. A mismatch
    // fails closed before the settlement-finalization sink is invoked, leaving the
    // settlement-finalization invocation count at zero.
    if !expectations.binding_matches(input) {
        return Finalization::RejectedBeforeSettlementFinalizationNoReceiptAcknowledgement;
    }

    // Step 5: invoke the settlement-finalization sink to record the modeled finalization.
    sink.project_durable_completion_settlement_receipt_acknowledgement(
        &input.request,
        expectations,
        idempotent_only,
        ledger,
...
/// Run 270 — the modeled durable-completion audit/publication receipt
/// crash/recovery window.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionSettlementReceiptAcknowledgementWindow {
    ...
    /// Crashed after the report record but before a receipt_acknowledgement intent.
    AfterReportRecordBeforeReceiptAcknowledgementIntent,
    /// Crashed after a receipt_acknowledgement intent but before any receipt_acknowledgement record.
    AfterReceiptAcknowledgementIntentBeforeReceiptAcknowledgementRecord,
    /// Crashed after the receipt_acknowledgement record but before an attestation intent.
    AfterReceiptAcknowledgementRecordBeforeAttestationIntent,
    ...
    /// Crashed after consumer success but before a settlement-finalization request.
    AfterConsumerSuccessBeforeSettlementFinalizationRequest,
    ...
    /// Crashed after settlement-finalization success but before a settlement-receipt-acknowledgement
    /// request.
    AfterSettlementFinalizationSuccessBeforeSettlementReceiptAcknowledgementRequest,
    /// Crashed after a settlement-receipt-acknowledgement request but before any
    /// settlement-receipt-acknowledgement record.
    AfterSettlementReceiptAcknowledgementRequestBeforeSettlementReceiptAcknowledgementRecord,
    /// Crashed after a settlement-receipt-acknowledgement record but before settlement-receipt-acknowledgement
    /// success — fails closed unless an explicit matching settlement-receipt-acknowledgement
    /// record exists.
    AfterSettlementReceiptAcknowledgementRecordBeforeSettlementReceiptAcknowledgementSuccess,
    /// Recovered after a successful settlement-receipt-acknowledgement record.
    AfterSettlementReceiptAcknowledgementSuccess,
    /// Recovered after an ambiguous settlement-receipt-acknowledgement window.
    AfterSettlementReceiptAcknowledgementAmbiguous,
    /// The settlement-receipt-acknowledgement record itself failed.
    SettlementReceiptAcknowledgementRecordFailed,
    /// The settlement-receipt-acknowledgement record was rolled back.
    SettlementReceiptAcknowledgementRollbackCompleted,
    /// The settlement-receipt-acknowledgement rollback itself failed — fatal.
    SettlementReceiptAcknowledgementRollbackFailed,
    /// An unknown window — fails closed.
    Unknown,
}

/// Run 270 — classify a modeled durable-completion audit/publication receipt
/// crash/recovery window.
///
...
/// [`DurableCompletionSettlementReceiptAcknowledgementOutcome::SettlementReceiptAcknowledgementRecorded`]. Pure:
/// performs no modeled mutation and never invokes Run 070.
pub fn recover_durable_completion_settlement_receipt_acknowledgement_window(
    input: &DurableCompletionSettlementReceiptAcknowledgementInput,
    window: DurableCompletionSettlementReceiptAcknowledgementWindow,
    ...
    if input.is_mainnet_peer_driven() {
        return Receipt::MainNetPeerDrivenApplyRefusedNoReceiptAcknowledgement;
    }
    ...
    match kind {
        DurableCompletionSettlementReceiptAcknowledgementKind::ProductionSettlementReceiptAcknowledgementUnavailable => {
            return Receipt::ProductionSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement;
        }
        DurableCompletionSettlementReceiptAcknowledgementKind::MainNetSettlementReceiptAcknowledgementUnavailable => {
            return Receipt::MainNetSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement;
        }
        DurableCompletionSettlementReceiptAcknowledgementKind::ExternalSettlementReceiptAcknowledgementUnavailable => {
            return Receipt::ExternalSettlementReceiptAcknowledgementUnavailableNoReceiptAcknowledgement;
        }
        DurableCompletionSettlementReceiptAcknowledgementKind::Disabled => {
            return Receipt::LegacyBypassNoSettlementReceiptAcknowledgement;
        }
        DurableCompletionSettlementReceiptAcknowledgementKind::Unknown => {
            return Receipt::SettlementReceiptAcknowledgementAmbiguousFailClosedNoReceiptAcknowledgement;
        }
        DurableCompletionSettlementReceiptAcknowledgementKind::FixtureInMemory => {}
    }
    ...
    match window {
        // Through settlement-finalization success but before a settlement-receipt-acknowledgement
        // request there is nothing to record a settlement receipt_acknowledgement for.
        ...
        | Window::AfterSettlementFinalizationSuccessBeforeSettlementReceiptAcknowledgementRequest => {
            Receipt::SettlementFinalizationDidNotRecordNoReceiptAcknowledgement
        }
        // A settlement-receipt-acknowledgement request without a record never records a
        ...
        Window::AfterSettlementReceiptAcknowledgementRequestBeforeSettlementReceiptAcknowledgementRecord => {
            Receipt::SettlementReceiptAcknowledgementRejectedBeforeRecord
        }
        // After a settlement-receipt-acknowledgement record but before success: fails closed
        // unless an explicit matching, well-formed settlement-receipt-acknowledgement record
        // exists.
        Window::AfterSettlementReceiptAcknowledgementRecordBeforeSettlementReceiptAcknowledgementSuccess => {
            match recovered_record {
                Some(record) if recovered_matches(record) => Receipt::SettlementReceiptAcknowledgementRecorded,
                _ => Receipt::SettlementReceiptAcknowledgementRejectedBeforeRecord,
            }
        }
        // An explicit successful settlement receipt_acknowledgement recovers as recorded only if
        // it matches.
        Window::AfterSettlementReceiptAcknowledgementSuccess => match recovered_record {
            Some(record) if recovered_matches(record) => Receipt::SettlementReceiptAcknowledgementRecorded,
            _ => Receipt::SettlementReceiptAcknowledgementRejectedBeforeRecord,
        },
        ...
        Window::Unknown => Receipt::SettlementReceiptAcknowledgementAmbiguousFailClosedNoReceiptAcknowledgement,
    }
}

/// Run 270 — `true` iff a receipt outcome authorizes a **new** modeled
/// audit/publication receipt (only
/// [`DurableCompletionSettlementReceiptAcknowledgementOutcome::SettlementReceiptAcknowledgementRecorded`]).
pub fn settlement_receipt_acknowledgement_outcome_authorizes_record(
    outcome: &DurableCompletionSettlementReceiptAcknowledgementOutcome,
) -> bool {
    outcome.authorizes_record()
}

/// Run 270 — `true` iff a receipt outcome projects to a recorded audit/publication
/// receipt (a newly recorded receipt or an idempotent duplicate of an
/// already-recorded receipt).
pub fn settlement_receipt_acknowledgement_outcome_projects_to_recorded(
    outcome: &DurableCompletionSettlementReceiptAcknowledgementOutcome,
) -> bool {
...
/// Run 270 — a receipt rejection is non-mutating: it records no receipt, mutates no
/// `LivePqcTrustState`, and writes no durable state.
pub fn durable_completion_settlement_receipt_acknowledgement_rejection_is_non_mutating() -> bool {
    true
}

/// Run 270 — the receipt boundary never calls Run 070.
pub fn durable_completion_settlement_receipt_acknowledgement_never_calls_run_070() -> bool {
    true
}
...
/// Run 270 — the receipt boundary performs no external publication.
pub fn durable_completion_settlement_receipt_acknowledgement_no_external_publication() -> bool {
    true
}

/// Run 270 — the receipt boundary performs no real audit-ledger persistence.
pub fn durable_completion_settlement_receipt_acknowledgement_no_real_audit_ledger() -> bool {
    true
}

/// Run 270 — a receipt requires a successful Run 246 pipeline outcome upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_pipeline_success_required() -> bool {
    true
}

/// Run 270 — a receipt requires a Run 248 sink receipt record upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_sink_receipt_required() -> bool {
    true
}

/// Run 270 — a receipt requires a Run 250 completion report upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_completion_report_required() -> bool {
    true
}
...
/// Run 270 — a receipt requires a Run 254 attestation upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_attestation_required() -> bool {
    true
}

/// Run 270 — an acknowledgement requires a Run 256 backend submission upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_backend_submission_required() -> bool {
    true
}

/// Run 270 — an acknowledgement requires a Run 258 audit receipt upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_receipt_required() -> bool {
    true
}

/// Run 270 — a consumer requires a Run 260 audit-receipt acknowledgement upstream.
pub fn durable_completion_settlement_receipt_acknowledgement_consumer_required() -> bool {
    true
}

/// Run 270 — a settlement receipt_acknowledgement requires a Run 268 settlement finalization
/// upstream; no settlement receipt_acknowledgement is authorized without a recorded
/// settlement finalization.
pub fn durable_completion_settlement_receipt_acknowledgement_finalization_required() -> bool {
    true
}

/// Run 270 — the consumer boundary never performs a real settlement; production /
/// MainNet / external settlement consumers are reachable but fail closed.
pub fn durable_completion_settlement_receipt_acknowledgement_no_real_settlement() -> bool {
    true
}

/// Run 270 — the settlement-receipt-acknowledgement boundary never confers real settlement
/// finality; the only settlement-receipt-acknowledgement record is a modeled in-memory fixture
/// record. Production / MainNet / external settlement-receipt-acknowledgement sinks are reachable
/// but unavailable / fail closed and never confer any real finality.
pub fn durable_completion_settlement_receipt_acknowledgement_no_real_settlement_finality() -> bool {
    true
}

/// Run 270 — the settlement-receipt-acknowledgement boundary never emits a real settlement
/// receipt; the only settlement-receipt-acknowledgement record is a modeled in-memory fixture
/// record with no external publication, network I/O, or persistent backend.
pub fn durable_completion_settlement_receipt_acknowledgement_no_real_settlement_receipt() -> bool {
    true
}

/// Run 270 — the settlement-receipt-acknowledgement boundary never confers a real
/// settlement-receipt acknowledgement; the only settlement-receipt acknowledgement
...
/// Run 270 — a failed receipt record never records a receipt.
pub fn durable_completion_settlement_receipt_acknowledgement_failed_record_never_records() -> bool {
    true
}

/// Run 270 — a rolled-back receipt record never records a receipt.
pub fn durable_completion_settlement_receipt_acknowledgement_rollback_never_records() -> bool {
    true
}
...
/// Run 270 — MainNet peer-driven apply refusal precedes every receipt stage.
pub fn durable_completion_settlement_receipt_acknowledgement_mainnet_peer_driven_apply_refused_first(
...
/// Run 270 — the external-publication sink is reachable but unavailable.
pub fn durable_completion_settlement_receipt_acknowledgement_external_unavailable() -> bool {
    true
}
...
/// Run 270 — policy-change actions remain unsupported at the receipt boundary.
pub fn durable_completion_settlement_receipt_acknowledgement_policy_change_unsupported() -> bool {
    true
}

/// Run 270 — a local operator cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_settlement_receipt_acknowledgement_local_operator_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}

/// Run 270 — a peer majority cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_settlement_receipt_acknowledgement_peer_majority_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}