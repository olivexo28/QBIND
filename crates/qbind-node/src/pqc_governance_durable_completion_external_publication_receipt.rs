//! Run 278 — source/test durable-completion **external-publication-confirmation
//! consumer / external-publication-receipt interface boundary**.
//!
//! Source/test only. Run 278 captures **no** release-binary evidence and enables
...
//! external-publication-confirmation backend, a real settlement-finality projection
//! backend, a real audit-ledger acknowledgement, a real external-publication
//! external_publication_receipt, a real external-publication system, a real production attestation /
...
//!
//! Run 276
//! ([`crate::pqc_governance_durable_completion_external_publication_confirmation`])
...
//!
//! Run 278 defines the **first typed interface** a future production settlement-receipt
...
//! **after** the Run 276 settlement-confirmation stage produced
//! `ExternalPublicationConfirmationRecorded`. It is an **interface / projection boundary only**:
//! production / MainNet / external external-publication-confirmation implementations
...
//!
//! The external-publication-confirmation layer is a **model only**. It does not
//! implement a real settlement, a real settlement finality, a real settlement receipt, a
//! real external-publication-confirmation, a real settlement-finality projection, a
...
//! fixture external-publication-confirmation sink mutates only the in-memory
...
//! 2. **legacy bypass** — a
//!    [`DurableCompletionExternalPublicationReceiptPolicy::Disabled`] policy preserves
//!    the legacy no-external-publication-receipt bypass and never invokes the
//!    external-publication-confirmation sink;
//! 3. **settlement-confirmation-outcome projection** — only
//!    [`DurableCompletionExternalPublicationConfirmationOutcome::ExternalPublicationConfirmationRecorded`]
//!    creates a external-publication-confirmation request; every other Run 276 outcome
...
//!    and Run 276 settlement-confirmation digest sets) must match expectations *before*
//!    the external-publication-confirmation sink is invoked; a mismatch fails closed
//!    and leaves the sink invocation count at zero;
//! 5. **external-publication-confirmation record** — only after every prior gate
//!    passes is the sink invoked; the record-identity fields must match exactly before
//!    any modeled external-publication-confirmation record is recorded;
//! 6. **external-publication-confirmation authorization** — only
//!    [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptRecorded`]
//!    authorizes a new modeled external-publication-confirmation / settlement-finality
//!    projection state.
//!
//! A external-publication-confirmation record failure, rollback, rollback failure, or
//! ambiguous window never retroactively claims a durable settlement-receipt
//! acknowledgement record. A duplicate identical external-publication-confirmation
//! record is idempotent; the same record id with a different digest fails closed as
//! equivocation and records no second record. A Run 276
//! [`DurableCompletionExternalPublicationConfirmationOutcome::ExternalPublicationConfirmationDuplicateIdempotent`]
//! never creates a new external-publication-confirmation record by itself — it can only
...
/// Run 278 — the runtime binding (governance + mutation surface + sequence) the
/// receipt is bound to.
pub type DurableCompletionExternalPublicationReceiptBinding =
    ModeledGovernanceTrustMutationRuntimeBinding;

/// Run 278 — the Run 240/246 durable replay observation carried as freshness
/// context.
pub type DurableCompletionExternalPublicationReceiptReplayBinding = DurableReplayObservation;

/// Run 278 — the Run 246 pipeline outcome carried as consume authorization context.
pub type DurableCompletionExternalPublicationReceiptPipelineBinding =
    GovernanceModeledEndToEndPipelineOutcome;

/// Run 278 — the Run 248 sink outcome carried as receipt-record context.
pub type DurableCompletionExternalPublicationReceiptSinkBinding =
    GovernanceModeledDurableConsumeSinkOutcome;

/// Run 278 — the Run 250 reporter outcome carried as completion-report context.
pub type DurableCompletionExternalPublicationReceiptReporterBinding =
    GovernanceModeledDurableConsumeCompletionReporterOutcome;

/// Run 278 — the Run 252 external_publication_receipt outcome carried as external_publication_receipt context.
pub type DurableCompletionExternalPublicationReceiptFinalizationBinding =
    GovernanceModeledDurableCompletionFinalizationOutcome;

/// Run 278 — the Run 254 attestation outcome carried as attestation context.
pub type DurableCompletionExternalPublicationReceiptAttestationBinding =
    GovernanceModeledDurableCompletionAttestationOutcome;

/// Run 278 — the Run 256 backend outcome carried as backend-record context. The
/// acknowledgement boundary never reimplements the backend; it only carries its
/// terminal outcome.
pub type DurableCompletionExternalPublicationReceiptBackendBinding =
    DurableCompletionAttestationBackendOutcome;

/// Run 278 — the Run 258 audit/publication receipt outcome carried as
/// receipt-record context. The consumer boundary never reimplements the receipt; it
/// only carries its terminal outcome.
pub type DurableCompletionExternalPublicationReceiptReceiptBinding =
    DurableCompletionAuditPublicationReceiptOutcome;

/// Run 278 — the Run 260 audit-receipt acknowledgement outcome carried as
...
pub type DurableCompletionExternalPublicationReceiptAcknowledgementBinding =
    DurableCompletionAuditReceiptAcknowledgementOutcome;

/// Run 278 — the Run 262 acknowledgement consumer outcome the settlement-confirmation
...
pub type DurableCompletionExternalPublicationReceiptConsumerBinding =
    DurableCompletionAcknowledgementConsumerOutcome;

/// Run 278 — the Run 276 settlement-confirmation outcome the external-publication-receipt
/// boundary projects to a external-publication-receipt request. The external-publication-receipt
/// boundary never reimplements the settlement confirmation; it only projects its
/// terminal outcome.
pub type DurableCompletionExternalPublicationReceiptExternalPublicationConfirmationBinding =
...
/// Run 278 — the typed durable-completion audit/publication receipt kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationReceiptKind {
    ...
    /// fail-closed).
    ProductionExternalPublicationReceiptUnavailable,
    /// MainNet audit-ledger receipt sink (reachable-but-unavailable / fail-closed).
    MainNetExternalPublicationReceiptUnavailable,
    /// External-publication receipt sink (reachable-but-unavailable / fail-closed).
    ExternalExternalPublicationReceiptUnavailable,
    /// An unknown receipt kind — fails closed.
    Unknown,
}

impl DurableCompletionExternalPublicationReceiptKind {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureInMemory => "fixture-in-memory",
            Self::ProductionExternalPublicationReceiptUnavailable => {
                "production-external-publication-receipt-unavailable"
            }
            Self::MainNetExternalPublicationReceiptUnavailable => {
                "mainnet-external-publication-receipt-unavailable"
            }
            Self::ExternalExternalPublicationReceiptUnavailable => {
                "external-external-publication-receipt-unavailable"
            }
...
/// Run 278 — the typed durable-completion audit/publication receipt policy
/// selector.
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationReceiptPolicy {
    /// The receipt boundary is disabled (legacy bypass).
    Disabled,
    ...
    /// unavailable.
    ProductionExternalPublicationReceiptRequired,
    /// A real MainNet audit-ledger receipt sink is required — reachable but
    /// unavailable.
    MainNetExternalPublicationReceiptRequired,
    /// A real external-publication receipt sink is required — reachable but
    /// unavailable.
    ExternalExternalPublicationReceiptRequired,
}

impl DurableCompletionExternalPublicationReceiptPolicy {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
...
/// Run 278 — the typed receipt identity a receipt request is bound to.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationReceiptIdentity {
    /// Stable receipt id.
    pub confirmation_id: String,
    /// The receipt kind.
    pub kind: DurableCompletionExternalPublicationReceiptKind,
    /// The receipt policy.
    pub policy: DurableCompletionExternalPublicationReceiptPolicy,
    /// The domain separation tag the receipt operates under.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationReceiptIdentity {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.confirmation_id.is_empty()
            ...
            && self.kind != DurableCompletionExternalPublicationReceiptKind::Unknown
    }

    /// The deterministic, domain-separated receipt identity digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationReceiptDigest {
        external_publication_receipt_identity_digest(self)
    }
}
...
/// Run 278 — domain separator for the receipt identity digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_IDENTITY_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-receipt-identity:v1";
/// Run 278 — domain separator for the receipt request digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-receipt-request:v1";
/// Run 278 — domain separator for the receipt response digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-receipt-response:v1";
/// Run 278 — domain separator for the receipt record digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-receipt-record:v1";
/// Run 278 — domain separator for the receipt transcript digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_TRANSCRIPT_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-receipt-transcript:v1";

/// Run 278 — a deterministic, domain-separated receipt digest (lowercase hex of a
/// SHA3-256 over length-prefixed, field-bound canonical material). It contains no
/// unstable display text.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationReceiptDigest(String);

impl DurableCompletionExternalPublicationReceiptDigest {
...
/// Run 278 — a deterministic, domain-separated receipt transcript digest.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationReceiptTranscriptDigest(String);

impl DurableCompletionExternalPublicationReceiptTranscriptDigest {
    /// The lowercase hex representation.
    pub fn as_hex(&self) -> &str {
        &self.0
    }
...
/// Run 278 — deterministic, domain-separated receipt identity digest.
pub fn external_publication_receipt_identity_digest(
    identity: &DurableCompletionExternalPublicationReceiptIdentity,
) -> DurableCompletionExternalPublicationReceiptDigest {
    ...
    DurableCompletionExternalPublicationReceiptDigest(w.finish())
}

/// Run 278 — deterministic, domain-separated receipt request digest. Binds every
/// request field including the full Run 256 backend digest binding.
pub fn external_publication_receipt_request_digest(
    request: &DurableCompletionExternalPublicationReceiptRequest,
) -> DurableCompletionExternalPublicationReceiptDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN);
    w.str_field(&request.external_publication_receipt_record_id)
        ...
        .str_field(&request.external_publication_confirmation_identity_digest)
        .str_field(&request.external_publication_confirmation_request_digest)
        .str_field(&request.external_publication_confirmation_response_digest)
        .str_field(&request.external_publication_confirmation_record_digest)
        .str_field(&request.external_publication_confirmation_transcript_digest)
        .str_field(&request.external_publication_confirmation_record_id)
        .str_field(&request.domain_separation_tag)
        .str_field(external_publication_receipt_identity_digest(&request.identity).as_hex());
    DurableCompletionExternalPublicationReceiptDigest(w.finish())
}

/// Run 278 — deterministic, domain-separated receipt response digest. Binds the
/// receipt record id, the request digest it answers, the acceptance flag, and the
/// responding receipt kind.
pub fn external_publication_receipt_response_digest(
    response: &DurableCompletionExternalPublicationReceiptResponse,
) -> DurableCompletionExternalPublicationReceiptDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN);
    w.str_field(&response.external_publication_receipt_record_id)
        ...
        .str_field(response.external_publication_receipt_kind.tag());
    DurableCompletionExternalPublicationReceiptDigest(w.finish())
}

/// Run 278 — deterministic, domain-separated receipt record digest. Binds the
/// receipt record id, the request digest, and the receipt identity digest.
pub fn external_publication_receipt_record_digest(
    record: &DurableCompletionExternalPublicationReceiptRecord,
) -> DurableCompletionExternalPublicationReceiptDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN);
    w.str_field(&record.external_publication_receipt_record_id)
        .str_field(record.request_digest.as_hex())
        .str_field(record.identity_digest.as_hex());
    DurableCompletionExternalPublicationReceiptDigest(w.finish())
}

/// Run 278 — deterministic, domain-separated receipt transcript digest. Binds the
/// request, response, and record digests into a single transcript binding.
pub fn external_publication_receipt_transcript_digest(
    request_digest: &DurableCompletionExternalPublicationReceiptDigest,
    response_digest: &DurableCompletionExternalPublicationReceiptDigest,
    record_digest: &DurableCompletionExternalPublicationReceiptDigest,
) -> DurableCompletionExternalPublicationReceiptTranscriptDigest {
    ...
    DurableCompletionExternalPublicationReceiptTranscriptDigest(w.finish())
}
...
/// Run 278 — the typed receipt request a future production audit ledger or
...
/// attestation / Run 252 external_publication_receipt / Run 250 completion report / Run 248
/// receipt / Run 246 decision material — never a copy of any wire payload and never
/// a production durable record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptRequest {
    ...
    /// Run 252 external_publication_receipt decision digest.
    ...
    /// Run 276 settlement-confirmation identity digest.
    pub external_publication_confirmation_identity_digest: String,
    /// Run 276 settlement-confirmation request digest.
    pub external_publication_confirmation_request_digest: String,
    /// Run 276 settlement-confirmation response digest.
    pub external_publication_confirmation_response_digest: String,
    /// Run 276 settlement-confirmation record digest.
    pub external_publication_confirmation_record_digest: String,
    /// Run 276 settlement-confirmation transcript digest.
    pub external_publication_confirmation_transcript_digest: String,
    /// Run 276 settlement-confirmation record id.
    pub external_publication_confirmation_record_id: String,
    /// External-publication-confirmation identity.
    pub identity: DurableCompletionExternalPublicationReceiptIdentity,
    /// Domain separation tag.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationReceiptRequest {
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationReceiptDigest {
        external_publication_receipt_request_digest(self)
    }
...
/// Run 278 — the typed receipt response a receipt sink returns for a request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptResponse {
    /// The receipt record id the response answers.
    pub external_publication_receipt_record_id: String,
    /// The request digest the response answers.
    pub request_digest: DurableCompletionExternalPublicationReceiptDigest,
    /// `true` iff the receipt sink accepted the request.
    pub accepted: bool,
    /// The responding receipt kind.
    pub external_publication_receipt_kind: DurableCompletionExternalPublicationReceiptKind,
}

impl DurableCompletionExternalPublicationReceiptResponse {
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationReceiptDigest {
        external_publication_receipt_response_digest(self)
    }
}

/// Run 278 — the canonical immutable receipt record derived from a request. Two
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationReceiptRecord {
    /// The receipt record id.
    pub external_publication_receipt_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationReceiptDigest,
    /// The receipt identity digest.
    pub identity_digest: DurableCompletionExternalPublicationReceiptDigest,
}

impl DurableCompletionExternalPublicationReceiptRecord {
    /// The deterministic receipt record digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationReceiptDigest {
        external_publication_receipt_record_digest(self)
    }
}
...
/// Run 278 — the recorded status of a modeled audit/publication receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationReceiptLedgerStatus {
    /// The modeled receipt is recorded in the in-memory fixture ledger.
    Recorded,
}

/// Run 278 — a single modeled receipt record held in the in-memory fixture ledger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptLedgerRecord {
    /// The receipt record id (stable identity of the receipt).
    pub external_publication_receipt_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationReceiptDigest,
    /// The response digest.
    pub response_digest: DurableCompletionExternalPublicationReceiptDigest,
    /// The record digest.
    pub record_digest: DurableCompletionExternalPublicationReceiptDigest,
    /// The transcript digest.
    pub transcript_digest: DurableCompletionExternalPublicationReceiptTranscriptDigest,
    /// The recorded status.
    pub status: DurableCompletionExternalPublicationReceiptLedgerStatus,
}

/// Run 278 — an immutable snapshot of the modeled receipt ledger used to model a
/// fixture rollback.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptLedgerSnapshot {
    records: Vec<DurableCompletionExternalPublicationReceiptLedgerRecord>,
}

impl DurableCompletionExternalPublicationReceiptLedgerSnapshot {
...
/// Run 278 — the modeled in-memory receipt ledger.
///
...
pub struct DurableCompletionExternalPublicationReceiptLedger {
    records: Vec<DurableCompletionExternalPublicationReceiptLedgerRecord>,
}

impl DurableCompletionExternalPublicationReceiptLedger {
    ...
    pub fn records(&self) -> &[DurableCompletionExternalPublicationReceiptLedgerRecord] {
        &self.records
    }

    /// The record for `external_publication_receipt_record_id`, if present.
    pub fn find(
        &self,
    ...
    pub fn contains(&self, external_publication_receipt_record_id: &str) -> bool {
        self.find(external_publication_receipt_record_id).is_some()
    }

    /// Capture an immutable snapshot for a modeled rollback.
    pub fn snapshot(&self) -> DurableCompletionExternalPublicationReceiptLedgerSnapshot {
        DurableCompletionExternalPublicationReceiptLedgerSnapshot {
    ...
    pub fn restore(
        &mut self,
        snapshot: &DurableCompletionExternalPublicationReceiptLedgerSnapshot,
    ) {
    ...
    fn insert(&mut self, record: DurableCompletionExternalPublicationReceiptLedgerRecord) {
...
/// Run 278 — the canonical binding a [`DurableCompletionExternalPublicationReceiptInput`]
/// is checked against.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptExpectations {
    ...
    /// Expected Run 252 external_publication_receipt decision digest.
    ...
    /// Expected Run 276 settlement-confirmation identity digest.
    pub expected_external_publication_confirmation_identity_digest: String,
    /// Expected Run 276 settlement-confirmation request digest.
    pub expected_external_publication_confirmation_request_digest: String,
    /// Expected Run 276 settlement-confirmation response digest.
    pub expected_external_publication_confirmation_response_digest: String,
    /// Expected Run 276 settlement-confirmation record digest.
    pub expected_external_publication_confirmation_record_digest: String,
    /// Expected Run 276 settlement-confirmation transcript digest.
    pub expected_external_publication_confirmation_transcript_digest: String,
    /// Expected Run 276 settlement-confirmation record id.
    pub expected_external_publication_confirmation_record_id: String,
    /// Expected settlement-confirmation identity.
    pub expected_identity: DurableCompletionExternalPublicationReceiptIdentity,
...
impl DurableCompletionExternalPublicationReceiptExpectations {
    /// Internal: the first environment / surface binding mismatch reason, if any.
    fn binding_mismatch_reason(
        &self,
        input: &DurableCompletionExternalPublicationReceiptInput,
    ) -> Option<&'static str> {
    ...
    fn request_mismatch_reason(
        ...
        if request.finalization_decision_digest != self.expected_finalization_decision_digest {
            return Some("wrong external_publication_receipt decision digest");
        }
        ...
        if request.external_publication_confirmation_identity_digest
            != self.expected_external_publication_confirmation_identity_digest
        {
            return Some("wrong settlement-confirmation identity digest");
        }
        if request.external_publication_confirmation_request_digest
            != self.expected_external_publication_confirmation_request_digest
        {
            return Some("wrong settlement-confirmation request digest");
        }
        if request.external_publication_confirmation_response_digest
            != self.expected_external_publication_confirmation_response_digest
        {
            return Some("wrong settlement-confirmation response digest");
        }
        if request.external_publication_confirmation_record_digest
            != self.expected_external_publication_confirmation_record_digest
        {
            return Some("wrong settlement-confirmation record digest");
        }
        if request.external_publication_confirmation_transcript_digest
            != self.expected_external_publication_confirmation_transcript_digest
        {
        ...
        if request.identity != self.expected_identity {
            return Some("wrong external-publication-receipt identity");
        }
        if request.identity.kind != self.expected_external_publication_receipt_kind {
            return Some("wrong external-publication-receipt kind");
        }
        if request.identity.policy != self.expected_external_publication_receipt_policy {
            return Some("wrong external-publication-receipt policy");
        }
        None
    }
...
/// Run 278 — typed inputs for one modeled durable-completion audit/publication
/// receipt round-trip.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationReceiptInput {
    /// The receipt policy selector.
    pub policy: DurableCompletionExternalPublicationReceiptPolicy,
    /// The trust-domain environment binding.
    pub environment_binding: DurableCompletionExternalPublicationReceiptEnvironment,
    /// The runtime binding (governance + mutation surface + sequence).
    pub runtime_binding: DurableCompletionExternalPublicationReceiptBinding,
    /// The Run 240/246 durable replay observation the pipeline authorized consume
    /// under.
    pub replay_binding: DurableCompletionExternalPublicationReceiptReplayBinding,
    /// The Run 246 pipeline outcome.
    pub pipeline_binding: DurableCompletionExternalPublicationReceiptPipelineBinding,
    /// The Run 248 sink outcome.
    pub sink_binding: DurableCompletionExternalPublicationReceiptSinkBinding,
    /// The Run 250 reporter outcome.
    pub reporter_binding: DurableCompletionExternalPublicationReceiptReporterBinding,
    /// The Run 252 external_publication_receipt outcome.
    pub confirmation_binding: DurableCompletionExternalPublicationReceiptFinalizationBinding,
    /// The Run 254 attestation outcome.
    pub attestation_binding: DurableCompletionExternalPublicationReceiptAttestationBinding,
    /// The Run 256 backend outcome carried as backend-record context.
    pub backend_binding: DurableCompletionExternalPublicationReceiptBackendBinding,
    ...
    /// The Run 276 settlement-confirmation outcome the external-publication-receipt boundary
    /// projects to a external-publication-receipt request.
    pub external_publication_confirmation_binding:
        DurableCompletionExternalPublicationReceiptExternalPublicationConfirmationBinding,
    /// The external-publication-receipt request the call site would submit.
    pub request: DurableCompletionExternalPublicationReceiptRequest,
}

impl DurableCompletionExternalPublicationReceiptInput {
    ...
    pub fn surface(&self) -> DurableCompletionExternalPublicationReceiptSurface {
    ...
    pub fn is_mainnet_peer_driven(&self) -> bool {
        (self.environment() == TrustBundleEnvironment::Mainnet && self.surface().is_peer_driven())
            ...
            || matches!(
                self.external_publication_confirmation_binding,
                DurableCompletionExternalPublicationConfirmationOutcome::MainNetPeerDrivenApplyRefusedNoConfirmation
            )
    }
}
...
/// Run 278 — the typed outcome of one modeled durable-completion audit/publication
/// receipt round-trip.
///
/// Only [`Self::ExternalPublicationReceiptRecorded`] authorizes a **new** modeled
/// audit/publication-receipt state. A [`Self::ExternalPublicationReceiptDuplicateIdempotent`]
...
pub enum DurableCompletionExternalPublicationReceiptOutcome {
    ...
    RejectedBeforeExternalPublicationConfirmationNoReceipt,
    ...
    /// invocation.
    ExternalPublicationConfirmationDidNotRecordNoReceipt,
    ...
    ExternalPublicationReceiptRecorded,
    /// A duplicate identical receipt — idempotent; no second receipt recorded.
    ExternalPublicationReceiptDuplicateIdempotent,
...
impl DurableCompletionExternalPublicationReceiptOutcome {
    /// `true` iff this outcome authorizes a **new** modeled audit/publication
    /// receipt (only [`Self::ExternalPublicationReceiptRecorded`]).
    pub fn authorizes_record(&self) -> bool {
        matches!(self, Self::ExternalPublicationReceiptRecorded)
    }
    ...
    pub fn is_legacy_bypass(&self) -> bool {
        matches!(self, Self::LegacyBypassNoExternalPublicationReceipt)
    }
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            ...
            Self::ExternalPublicationConfirmationDidNotRecordNoReceipt => {
                "settlement-confirmation-did-not-record-no-outcome-publication"
            }
            Self::ExternalPublicationReceiptRecorded => "external-publication-receipt-recorded",
            Self::ExternalPublicationReceiptDuplicateIdempotent => {
                "external-publication-receipt-duplicate-idempotent"
            }
            Self::ExternalPublicationReceiptRejectedBeforeRecord => {
                "external-publication-receipt-rejected-before-record"
            }
            Self::ExternalPublicationReceiptRecordFailedNoReceipt => {
                "external-publication-receipt-record-failed-no-outcome-publication"
            }
            Self::ExternalPublicationReceiptRolledBackNoReceipt => {
                "external-publication-receipt-rolled-back-no-outcome-publication"
            }
            Self::ExternalPublicationReceiptRollbackFailedFatalNoReceipt => {
                "external-publication-receipt-rollback-failed-fatal-no-outcome-publication"
            }
            Self::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt => {
                "external-publication-receipt-ambiguous-fail-closed-no-outcome-publication"
            }
            Self::ProductionExternalPublicationReceiptUnavailableNoReceipt => {
                "production-external-publication-receipt-unavailable-no-outcome-publication"
            }
            Self::MainNetExternalPublicationReceiptUnavailableNoReceipt => {
                "mainnet-external-publication-receipt-unavailable-no-outcome-publication"
            }
            Self::ExternalExternalPublicationReceiptUnavailableNoReceipt => {
                "external-external-publication-receipt-unavailable-no-outcome-publication"
            }
...
/// Run 278 — the typed confirmation of a Run 262 acknowledgement consumer outcome
/// onto a settlement-confirmation request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationReceiptRequestIntent {
    ...
    /// typed no-confirmation outcome the settlement-confirmation evaluation returns
    /// directly (without invoking the settlement-confirmation sink).
    NoConfirmation(DurableCompletionExternalPublicationReceiptOutcome),
}

impl DurableCompletionExternalPublicationReceiptRequestIntent {
...
/// Run 278 — project a Run 276 settlement-confirmation outcome onto a
/// external-publication-receipt request.
///
/// Only
/// [`DurableCompletionExternalPublicationConfirmationOutcome::ExternalPublicationConfirmationRecorded`]
/// creates a external-publication-receipt request.
/// [`DurableCompletionExternalPublicationConfirmationOutcome::ExternalPublicationConfirmationDuplicateIdempotent`]
/// may only match an already-recorded external-publication-receipt record and never creates
...
/// [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationConfirmationDidNotRecordNoReceipt`]).
/// Pure: performs no work and never records.
pub fn project_external_publication_confirmation_outcome_to_external_publication_receipt_request(
    outcome: &DurableCompletionExternalPublicationReceiptExternalPublicationConfirmationBinding,
    ...
    use DurableCompletionExternalPublicationReceiptOutcome as Receipt;
    use DurableCompletionExternalPublicationReceiptRequestIntent as Intent;
    match outcome {
        Finalization::ExternalPublicationConfirmationRecorded => Intent::CreateRequest,
        Finalization::ExternalPublicationConfirmationDuplicateIdempotent => Intent::IdempotentOnly,
        Finalization::LegacyBypassNoExternalPublicationConfirmation => {
            Intent::NoConfirmation(Receipt::LegacyBypassNoExternalPublicationReceipt)
        }
        Finalization::RejectedBeforeSettlementOutcomePublicationNoConfirmation => {
            Intent::NoConfirmation(Receipt::RejectedBeforeExternalPublicationConfirmationNoReceipt)
        }
        Finalization::MainNetPeerDrivenApplyRefusedNoConfirmation => {
            Intent::NoConfirmation(Receipt::MainNetPeerDrivenApplyRefusedNoReceipt)
        }
        Finalization::ValidatorSetRotationUnsupportedNoConfirmation => {
            Intent::NoConfirmation(Receipt::ValidatorSetRotationUnsupportedNoReceipt)
        }
        Finalization::PolicyChangeUnsupportedNoConfirmation => {
            Intent::NoConfirmation(Receipt::PolicyChangeUnsupportedNoReceipt)
        }
        ...
        // record, so no external-publication-receipt record may exist.
        _ => Intent::NoConfirmation(Receipt::ExternalPublicationConfirmationDidNotRecordNoReceipt),
    }
}
...
/// Run 278 — the pure/mockable modeled durable-completion audit/publication receipt
/// boundary.
///
...
/// [`DurableCompletionExternalPublicationReceiptLedger`].
pub trait GovernanceDurableCompletionExternalPublicationReceiptSink {
    /// The receipt kind (used for typed recovery classification).
    fn kind(&self) -> DurableCompletionExternalPublicationReceiptKind;
    ...
    fn project_durable_completion_external_publication_receipt(
        &mut self,
        request: &DurableCompletionExternalPublicationReceiptRequest,
        expectations: &DurableCompletionExternalPublicationReceiptExpectations,
        idempotent_only: bool,
        ledger: &mut DurableCompletionExternalPublicationReceiptLedger,
    ...
    fn recover_durable_completion_external_publication_receipt_window(
        &self,
        input: &DurableCompletionExternalPublicationReceiptInput,
        window: DurableCompletionExternalPublicationReceiptWindow,
        recovered_record: Option<&DurableCompletionExternalPublicationReceiptLedgerRecord>,
        expectations: &DurableCompletionExternalPublicationReceiptExpectations,
    ) -> DurableCompletionExternalPublicationReceiptOutcome {
...
/// Run 278 — the DevNet/TestNet in-memory fixture receipt sink.
///
...
#[derive(Debug, Clone)]
pub struct FixtureDurableCompletionExternalPublicationReceiptSink {
    fault: Option<DurableCompletionExternalPublicationReceiptFault>,
    invocations: u32,
}

impl Default for FixtureDurableCompletionExternalPublicationReceiptSink {
    fn default() -> Self {
        Self::new()
    }
}

impl FixtureDurableCompletionExternalPublicationReceiptSink {
    ...
    pub fn with_fault(fault: DurableCompletionExternalPublicationReceiptFault) -> Self {
        Self {
            fault: Some(fault),
            invocations: 0,
        }
...
impl GovernanceDurableCompletionExternalPublicationReceiptSink
    for FixtureDurableCompletionExternalPublicationReceiptSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationReceiptKind {
    ...
    fn project_durable_completion_external_publication_receipt(
        ...
        if let Some(fault) = self.fault {
            ...
            return match fault {
                DurableCompletionExternalPublicationReceiptFault::RecordFailedNoConfirmation => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationReceiptRecordFailedNoReceipt
                }
                DurableCompletionExternalPublicationReceiptFault::RolledBackNoConfirmation => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationReceiptRolledBackNoReceipt
                }
                DurableCompletionExternalPublicationReceiptFault::RollbackFailedFatal => {
                    Receipt::ExternalPublicationReceiptRollbackFailedFatalNoReceipt
                }
                DurableCompletionExternalPublicationReceiptFault::AmbiguousAfterRecord => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt
                }
            };
        }
        ...
        if !matches!(
            ...
            return Receipt::ExternalPublicationReceiptRejectedBeforeRecord;
        }
        ...
        let transcript_digest = external_publication_receipt_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
        );
        ...
        if let Some(existing) = ledger.find(&request.external_publication_receipt_record_id) {
            ...
            {
                return Receipt::ExternalPublicationReceiptDuplicateIdempotent;
            }
        ...
        ledger.insert(DurableCompletionExternalPublicationReceiptLedgerRecord {
            ...
            record_digest,
            transcript_digest,
            status: DurableCompletionExternalPublicationReceiptLedgerStatus::Recorded,
        });
        Receipt::ExternalPublicationReceiptRecorded
    }
}
...
/// Run 278 — the production audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ProductionExternalPublicationReceiptSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationReceiptSink
    for ProductionExternalPublicationReceiptSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationReceiptKind {
        DurableCompletionExternalPublicationReceiptKind::ProductionExternalPublicationReceiptUnavailable
    }
    ...
    fn project_durable_completion_external_publication_receipt(
        &mut self,
        _request: &DurableCompletionExternalPublicationReceiptRequest,
        _expectations: &DurableCompletionExternalPublicationReceiptExpectations,
        _idempotent_only: bool,
        _ledger: &mut DurableCompletionExternalPublicationReceiptLedger,
    ) -> DurableCompletionExternalPublicationReceiptOutcome {
        self.invocations += 1;
        DurableCompletionExternalPublicationReceiptOutcome::ProductionExternalPublicationReceiptUnavailableNoReceipt
    }
}

/// Run 278 — the MainNet audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct MainNetExternalPublicationReceiptSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationReceiptSink
    for MainNetExternalPublicationReceiptSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationReceiptKind {
        DurableCompletionExternalPublicationReceiptKind::MainNetExternalPublicationReceiptUnavailable
    }
    ...
    fn project_durable_completion_external_publication_receipt(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationReceiptOutcome::MainNetExternalPublicationReceiptUnavailableNoReceipt
    }
}

/// Run 278 — the external-publication receipt sink. Reachable but unavailable /
/// fail-closed. It performs no external publication, records no receipt, and
/// mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ExternalExternalPublicationReceiptSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationReceiptSink
    for ExternalExternalPublicationReceiptSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationReceiptKind {
        DurableCompletionExternalPublicationReceiptKind::ExternalExternalPublicationReceiptUnavailable
    }
    ...
    fn project_durable_completion_external_publication_receipt(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationReceiptOutcome::ExternalExternalPublicationReceiptUnavailableNoReceipt
    }
}
...
/// Run 278 — evaluate one modeled durable-completion audit/publication receipt
/// round-trip.
///
...
/// 2. legacy bypass — a
///    [`DurableCompletionExternalPublicationReceiptPolicy::Disabled`] policy;
...
pub fn evaluate_durable_completion_external_publication_receipt<S>(
    input: &DurableCompletionExternalPublicationReceiptInput,
    expectations: &DurableCompletionExternalPublicationReceiptExpectations,
    sink: &mut S,
    ledger: &mut DurableCompletionExternalPublicationReceiptLedger,
...
where
    S: GovernanceDurableCompletionExternalPublicationReceiptSink,
{
    ...
    if input.is_mainnet_peer_driven() {
        return Finalization::MainNetPeerDrivenApplyRefusedNoReceipt;
    }
    ...
    if input.policy.is_disabled() {
        return Finalization::LegacyBypassNoExternalPublicationReceipt;
    }
    ...
    // no-confirmation outcome without invoking the settlement-confirmation sink.
    let idempotent_only =
        match project_external_publication_confirmation_outcome_to_external_publication_receipt_request(
            &input.external_publication_confirmation_binding,
        ) {
            Intent::NoConfirmation(outcome) => return outcome,
    ...
    if !expectations.binding_matches(input) {
        return Finalization::RejectedBeforeExternalPublicationConfirmationNoReceipt;
    }

    // Step 5: invoke the settlement-confirmation sink to record the modeled confirmation.
    sink.project_durable_completion_external_publication_receipt(
        &input.request,
        expectations,
        idempotent_only,
        ledger,
...
/// Run 278 — the modeled durable-completion audit/publication receipt
/// crash/recovery window.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationReceiptWindow {
    ...
    /// Crashed after the report record but before a external_publication_receipt intent.
    AfterPublicationRecordBeforeReceiptIntent,
    /// Crashed after a external_publication_receipt intent but before any external_publication_receipt record.
    AfterReceiptIntentBeforeReceiptRecord,
    /// Crashed after the external_publication_receipt record but before an attestation intent.
    ...
    AfterConsumerSuccessBeforeExternalPublicationConfirmationRequest,
    ...
    /// Crashed after settlement-confirmation success but before a external-publication-receipt
    /// request.
    AfterExternalPublicationConfirmationSuccessBeforeExternalPublicationReceiptRequest,
    /// Crashed after a external-publication-receipt request but before any
    /// external-publication-receipt record.
    AfterExternalPublicationReceiptRequestBeforeExternalPublicationReceiptRecord,
    /// Crashed after a external-publication-receipt record but before external-publication-receipt
    /// success — fails closed unless an explicit matching external-publication-receipt
    /// record exists.
    AfterExternalPublicationReceiptRecordBeforeExternalPublicationReceiptSuccess,
    /// Recovered after a successful external-publication-receipt record.
    AfterExternalPublicationReceiptSuccess,
    /// Recovered after an ambiguous external-publication-receipt window.
    AfterExternalPublicationReceiptAmbiguous,
    /// The external-publication-receipt record itself failed.
    ExternalPublicationReceiptRecordFailed,
    /// The external-publication-receipt record was rolled back.
    ExternalPublicationReceiptRollbackCompleted,
    /// The external-publication-receipt rollback itself failed — fatal.
    ExternalPublicationReceiptRollbackFailed,
    /// An unknown window — fails closed.
    Unknown,
}

/// Run 278 — classify a modeled durable-completion audit/publication receipt
/// crash/recovery window.
///
...
/// [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptRecorded`]. Pure:
/// performs no modeled mutation and never invokes Run 070.
pub fn recover_durable_completion_external_publication_receipt_window(
    input: &DurableCompletionExternalPublicationReceiptInput,
    window: DurableCompletionExternalPublicationReceiptWindow,
    kind: DurableCompletionExternalPublicationReceiptKind,
    recovered_record: Option<&DurableCompletionExternalPublicationReceiptLedgerRecord>,
    expectations: &DurableCompletionExternalPublicationReceiptExpectations,
) -> DurableCompletionExternalPublicationReceiptOutcome {
    use DurableCompletionExternalPublicationReceiptOutcome as Receipt;
    use DurableCompletionExternalPublicationReceiptWindow as Window;
    ...
    match kind {
        DurableCompletionExternalPublicationReceiptKind::ProductionExternalPublicationReceiptUnavailable => {
            return Receipt::ProductionExternalPublicationReceiptUnavailableNoReceipt;
        }
        DurableCompletionExternalPublicationReceiptKind::MainNetExternalPublicationReceiptUnavailable => {
            return Receipt::MainNetExternalPublicationReceiptUnavailableNoReceipt;
        }
        DurableCompletionExternalPublicationReceiptKind::ExternalExternalPublicationReceiptUnavailable => {
            return Receipt::ExternalExternalPublicationReceiptUnavailableNoReceipt;
        }
        DurableCompletionExternalPublicationReceiptKind::Disabled => {
            return Receipt::LegacyBypassNoExternalPublicationReceipt;
        }
        DurableCompletionExternalPublicationReceiptKind::Unknown => {
            return Receipt::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt;
        }
        DurableCompletionExternalPublicationReceiptKind::FixtureInMemory => {}
    }
    ...
    match window {
        // Through settlement-confirmation success but before a external-publication-receipt
        // request there is nothing to record a settlement external_publication_receipt for.
        ...
        | Window::AfterConsumerSuccessBeforeExternalPublicationConfirmationRequest
        ...
        | Window::AfterExternalPublicationConfirmationSuccessBeforeExternalPublicationReceiptRequest => {
            Receipt::ExternalPublicationConfirmationDidNotRecordNoReceipt
        }
        // A external-publication-receipt request without a record never records a
        // settlement external_publication_receipt.
        Window::AfterExternalPublicationReceiptRequestBeforeExternalPublicationReceiptRecord => {
            Receipt::ExternalPublicationReceiptRejectedBeforeRecord
        }
        // After a external-publication-receipt record but before success: fails closed
        // unless an explicit matching, well-formed external-publication-receipt record
        // exists.
        Window::AfterExternalPublicationReceiptRecordBeforeExternalPublicationReceiptSuccess => {
            match recovered_record {
                Some(record) if recovered_matches(record) => Receipt::ExternalPublicationReceiptRecorded,
                _ => Receipt::ExternalPublicationReceiptRejectedBeforeRecord,
            }
        }
        // An explicit successful settlement external_publication_receipt recovers as recorded only if
        // it matches.
        Window::AfterExternalPublicationReceiptSuccess => match recovered_record {
            Some(record) if recovered_matches(record) => Receipt::ExternalPublicationReceiptRecorded,
            _ => Receipt::ExternalPublicationReceiptRejectedBeforeRecord,
        },
        Window::AfterExternalPublicationReceiptAmbiguous => {
            Receipt::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt
        }
        Window::ExternalPublicationReceiptRecordFailed => {
            Receipt::ExternalPublicationReceiptRecordFailedNoReceipt
        }
        Window::ExternalPublicationReceiptRollbackCompleted => {
            Receipt::ExternalPublicationReceiptRolledBackNoReceipt
        }
        Window::ExternalPublicationReceiptRollbackFailed => {
            Receipt::ExternalPublicationReceiptRollbackFailedFatalNoReceipt
        }
        // Any unknown window fails closed.
        Window::Unknown => Receipt::ExternalPublicationReceiptAmbiguousFailClosedNoReceipt,
    }
}

/// Run 278 — `true` iff a receipt outcome authorizes a **new** modeled
/// audit/publication receipt (only
/// [`DurableCompletionExternalPublicationReceiptOutcome::ExternalPublicationReceiptRecorded`]).
pub fn external_publication_receipt_outcome_authorizes_record(
    outcome: &DurableCompletionExternalPublicationReceiptOutcome,
) -> bool {
    outcome.authorizes_record()
}

/// Run 278 — `true` iff a receipt outcome projects to a recorded audit/publication
/// receipt (a newly recorded receipt or an idempotent duplicate of an
/// already-recorded receipt).
pub fn external_publication_receipt_outcome_projects_to_recorded(
    outcome: &DurableCompletionExternalPublicationReceiptOutcome,
) -> bool {
...
/// Run 278 — a receipt rejection is non-mutating: it records no receipt, mutates no
/// `LivePqcTrustState`, and writes no durable state.
pub fn durable_completion_external_publication_receipt_rejection_is_non_mutating() -> bool {
    true
}

/// Run 278 — the receipt boundary never calls Run 070.
pub fn durable_completion_external_publication_receipt_never_calls_run_070() -> bool {
    true
}
...
/// Run 278 — the receipt boundary never writes a sequence or a marker.
pub fn durable_completion_external_publication_receipt_never_writes_sequence_or_marker() -> bool {
    true
}
...
/// Run 278 — the receipt boundary performs no external publication.
pub fn durable_completion_external_publication_receipt_no_external_publication() -> bool {
    true
}

/// Run 278 — the receipt boundary performs no real audit-ledger persistence.
pub fn durable_completion_external_publication_receipt_no_real_audit_ledger() -> bool {
    true
}

/// Run 278 — a receipt requires a successful Run 246 pipeline outcome upstream.
pub fn durable_completion_external_publication_receipt_pipeline_success_required() -> bool {
    true
}

/// Run 278 — a receipt requires a Run 248 sink receipt record upstream.
pub fn durable_completion_external_publication_receipt_sink_receipt_required() -> bool {
    true
}

/// Run 278 — a receipt requires a Run 250 completion report upstream.
pub fn durable_completion_external_publication_receipt_completion_report_required() -> bool {
    true
}

/// Run 278 — a receipt requires a Run 252 external_publication_receipt upstream.
pub fn durable_completion_external_publication_receipt_finalization_projection_required() -> bool {
    true
}

/// Run 278 — a receipt requires a Run 254 attestation upstream.
pub fn durable_completion_external_publication_receipt_attestation_required() -> bool {
    true
}

/// Run 278 — an acknowledgement requires a Run 256 backend submission upstream.
pub fn durable_completion_external_publication_receipt_backend_submission_required() -> bool {
    true
}

/// Run 278 — an acknowledgement requires a Run 258 audit receipt upstream.
pub fn durable_completion_external_publication_receipt_receipt_required() -> bool {
    true
}

/// Run 278 — a consumer requires a Run 260 audit-receipt acknowledgement upstream.
pub fn durable_completion_external_publication_receipt_consumer_required() -> bool {
    true
}

/// Run 278 — a settlement external_publication_receipt requires a Run 276 settlement confirmation
/// upstream; no settlement external_publication_receipt is authorized without a recorded
/// settlement confirmation.
pub fn durable_completion_external_publication_receipt_confirmation_required() -> bool {
    true
}

/// Run 278 — the consumer boundary never performs a real settlement; production /
/// MainNet / external settlement consumers are reachable but fail closed.
pub fn durable_completion_external_publication_receipt_no_real_settlement() -> bool {
    true
}

/// Run 278 — the external-publication-receipt boundary never confers real settlement
/// finality; the only external-publication-receipt record is a modeled in-memory fixture
/// record. Production / MainNet / external external-publication-receipt sinks are reachable
/// but unavailable / fail closed and never confer any real finality.
pub fn durable_completion_external_publication_receipt_no_real_settlement_finality() -> bool {
    true
}

/// Run 278 — the external-publication-receipt boundary never emits a real settlement
/// receipt; the only external-publication-receipt record is a modeled in-memory fixture
/// record with no external publication, network I/O, or persistent backend.
pub fn durable_completion_external_publication_receipt_no_real_settlement_receipt() -> bool {
    true
}

/// Run 278 — the external-publication-receipt boundary never confers a real
/// external-publication-confirmation; the only external-publication-confirmation
...
/// Run 278 — a receipt record is required before a receipt is acknowledged.
pub fn durable_completion_external_publication_receipt_record_required_before_reported() -> bool {
    true
}

/// Run 278 — a failed receipt record never records a receipt.
pub fn durable_completion_external_publication_receipt_failed_record_never_records() -> bool {
    true
}

/// Run 278 — a rolled-back receipt record never records a receipt.
pub fn durable_completion_external_publication_receipt_rollback_never_records() -> bool {
    true
}

/// Run 278 — an ambiguous after-record receipt window fails closed.
pub fn durable_completion_external_publication_receipt_ambiguous_window_fails_closed() -> bool {
    true
}

/// Run 278 — MainNet peer-driven apply refusal precedes every receipt stage.
pub fn durable_completion_external_publication_receipt_mainnet_peer_driven_apply_refused_first(
    environment: TrustBundleEnvironment,
) -> bool {
    environment == TrustBundleEnvironment::Mainnet
}

/// Run 278 — production / MainNet audit-ledger sinks are reachable but unavailable.
pub fn durable_completion_external_publication_receipt_production_mainnet_unavailable() -> bool {
    true
}

/// Run 278 — the external-publication sink is reachable but unavailable.
pub fn durable_completion_external_publication_receipt_external_unavailable() -> bool {
    true
}
...
/// Run 278 — policy-change actions remain unsupported at the receipt boundary.
pub fn durable_completion_external_publication_receipt_policy_change_unsupported() -> bool {
    true
}

/// Run 278 — a local operator cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_receipt_local_operator_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}

/// Run 278 — a peer majority cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_receipt_peer_majority_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}