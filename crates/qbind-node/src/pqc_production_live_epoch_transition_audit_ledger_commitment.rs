...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self
                .expected_durable_audit_finalization_decision_id
                .is_empty()
            && !self
                .expected_durable_audit_finalization_request_id
                .is_empty()
            && !self
                .expected_durable_audit_finalization_intent_digest
                .is_empty()
            && !self
                .expected_durable_audit_finalization_transcript_digest
                .is_empty()
...
pub fn production_live_epoch_transition_audit_ledger_commitment_id(
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_ID_DOMAIN_TAG.as_bytes());
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
        b"guarded_mutation_intent_digest",
...
pub enum ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome {
    ...
    LiveEpochTransitionAuditLedgerCommitmentAmbiguous {
        reason: String,
    },
    MainNetRefused,
}
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            ...
            Self::DurableAuditFinalizationDecisionIdMismatch => {
                "durable-audit-finalization-decision-id-mismatch"
            }
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentDecision {
    ...
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only()
            && self.audit_ledger_commitment_artifact.is_some()
    }
}
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor {
    ...
    fn check_application_binding(
        ...
        // transcript binding.
        if decision.durable_audit_finalization_id
            != inputs.expected_durable_audit_finalization_decision_id
        {
        ...
        if decision.durable_audit_finalization_digest
            != inputs.expected_durable_audit_finalization_intent_digest
        {
            return Some(O::DurableAuditFinalizationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest
            != inputs.expected_durable_audit_finalization_transcript_digest
        {
        ...
        // nonce binding.
        if intent.durable_audit_finalization_nonce
            != inputs.expected_durable_audit_finalization_nonce
        {
    ...
    fn evaluate_core<R: LiveEpochTransitionAuditLedgerCommitmentReplaySet + ?Sized>(
        ...
        let staged_application_id =
            production_live_epoch_transition_audit_ledger_commitment_request_id(
        ...
        let mut record = ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact {
            ...
            durable_audit_finalization_intent_digest: decision
        ...
        let audit_ledger_commitment_id =
            production_live_epoch_transition_audit_ledger_commitment_id(
                self.config.protocol_version.0,
                &decision.durable_audit_finalization_digest,
                &inputs.audit_ledger_commitment_policy_id,
                request.proposed_epoch_transition_target,
...
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_default_is_disabled(
) -> bool {
...
impl LiveEpochTransitionAuditLedgerCommitmentFixtureState {
    ...
    pub fn has_applied(&self, audit_ledger_commitment_id: &str) -> bool {
        self.applied_execution_ids
            .iter()
            .any(|id| id == audit_ledger_commitment_id)
    }
    ...
    pub fn apply_prepared_execution(
        ...
        self.applied_execution_ids
            .push(audit_ledger_commitment_id.to_string());
        true
    }
}
