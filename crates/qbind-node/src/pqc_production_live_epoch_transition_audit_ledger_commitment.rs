//! Run 329 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 325/326 guarded epoch-transition audit-ledger-commitment boundary: the boundary
//! that consumes a **verified** Run 327/328 non-mutating guarded
//! epoch-transition audit-ledger-commitment *decision* (the accepted
//! [`ProductionLiveEpochTransitionDurableAuditFinalizationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionDurableAuditFinalizationArtifact`], as
//! release-binary-evidenced by Run 326) and translates it into a typed,
...
//!
//! Where the Run 325 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 329 answers the next question:
//! "given a verified guarded audit-ledger-commitment decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a durable-audit-finalization
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 329)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 330**.
//! * The default policy is
//!   [`ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::Disabled`]
//!   and fails closed **before** any binding, validator-set binding, or
//!   handoff-package construction.
//! * Only a **verified** Run 327/328 guarded epoch-transition
//!   audit-ledger-commitment decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionDurableAuditFinalizationArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 323/324 staged-application decision alone, a Run
//!   321/322 live-application authorization alone, a Run 319/320 application
//!   decision alone, Run 317 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact`]; it
...
use crate::pqc_production_live_epoch_transition_durable_audit_finalization::{
...
/// Run 329 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_PROTOCOL_VERSION: u16 = 1;

/// Run 329 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_INTENT_DOMAIN_TAG: &str =
    "QBIND:329-epoch-transition-durable-audit-finalization-intent:v1";

/// Run 329 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_ID_DOMAIN_TAG: &str =
    "QBIND:329-epoch-transition-durable-audit-finalization-id:v1";

/// Run 329 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_REQUEST_DOMAIN_TAG: &str =
    "QBIND:329-epoch-transition-durable-audit-finalization-request:v1";

/// Run 329 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:329-epoch-transition-durable-audit-finalization-transcript:v1";
...
/// Run 329 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuditLedgerCommitment` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 327/328 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuditLedgerCommitment` and
/// `MainnetProductionLiveEpochTransitionAuditLedgerCommitmentRequired` are
...
pub enum ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy {
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuditLedgerCommitment => {
                "allow-source-test-epoch-transition-durable-audit-finalization"
            }
            Self::RequireProductionLiveEpochTransitionAuditLedgerCommitment => {
                "require-production-epoch-transition-durable-audit-finalization"
            }
            Self::MainnetProductionLiveEpochTransitionAuditLedgerCommitmentRequired => {
                "mainnet-production-epoch-transition-durable-audit-finalization-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuditLedgerCommitment` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionAuditLedgerCommitment` kind is fail-closed as
/// unavailable in Run 329 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 329.
    ProductionLiveEpochTransitionAuditLedgerCommitment,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuditLedgerCommitment => {
                "source-test-epoch-transition-durable-audit-finalization"
            }
            Self::ProductionLiveEpochTransitionAuditLedgerCommitment => {
                "production-epoch-transition-durable-audit-finalization"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentConfig {
    pub fn new(kind: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::SourceTestLiveEpochTransitionAuditLedgerCommitment,
        )
    }
...
impl Default for ProductionLiveEpochTransitionAuditLedgerCommitmentConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::Disabled)
    }
}
...
/// Run 329 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 327/328
/// [`LiveEpochTransitionDurableAuditFinalizationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuditLedgerCommitmentKind {
...
impl LiveEpochTransitionAuditLedgerCommitmentKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 329 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 327/328 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionDurableAuditFinalizationKind,
    ) -> Self {
        use LiveEpochTransitionDurableAuditFinalizationKind as A;
        match kind {
...
/// Run 329 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedDurableAuditFinalizationDecision`] carrying a Run 327/328
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuditLedgerCommitmentAuthoritySource {
    /// A verified Run 327/328 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedDurableAuditFinalizationDecision {
        decision: ProductionLiveEpochTransitionDurableAuditFinalizationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingDurableAuditFinalizationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedDurableAuditFinalizationDecision {
        decision: ProductionLiveEpochTransitionDurableAuditFinalizationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedDurableAuditFinalizationWithoutPackage {
        decision: ProductionLiveEpochTransitionDurableAuditFinalizationDecision,
    },
    /// A Run 325/326 epoch-transition commit-authorization decision presented
    /// directly, without a Run 327/328 durable-audit-finalization decision. Rejected.
    CommitAuthorizationDecisionWithoutDurableAuditFinalization,
    /// A Run 325/326 epoch-transition mutation-execution decision presented
    /// directly, without a Run 327/328 durable-audit-finalization decision. Rejected.
    MutationExecutionDecisionWithoutDurableAuditFinalization,
    /// A Run 325/326 epoch-transition execution-preparation decision presented
    /// directly, without a Run 327/328 durable-audit-finalization decision. Rejected.
    ExecutionPreparationDecisionWithoutDurableAuditFinalization,
    /// A Run 325/326 epoch-transition runtime handoff decision presented
    /// directly, without a Run 327/328 durable-audit-finalization decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutDurableAuditFinalization,
    /// A Run 325/326 guarded epoch-transition audit-ledger-commitment decision
    /// presented directly, without a Run 327/328 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutDurableAuditFinalization,
    /// A Run 323/324 staged-application decision presented directly, without a
    /// Run 327/328 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutDurableAuditFinalization,
    /// A Run 321/322 live-application authorization presented directly, without
    /// a Run 327/328 guarded audit-ledger-commitment decision. Rejected.
    LiveApplicationAuthorizationWithoutDurableAuditFinalization,
    /// A Run 319/320 validator-set rotation *application decision* presented
    /// directly, without a Run 327/328 guarded audit-ledger-commitment decision.
    /// Rejected.
    ApplicationDecisionWithoutDurableAuditFinalization,
    /// A Run 317/318 validator-set rotation plan presented directly, without a
    /// Run 327/328 live-application authorization. Rejected.
    RotationPlanWithoutDurableAuditFinalization,
    /// A Run 315/316 governance execution intent presented directly, without a
    /// Run 327/328 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutDurableAuditFinalization,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 321/322 live-application authorization. Rejected.
    GovernanceProofWithoutDurableAuditFinalization,
    ...
    FixtureOnlyDurableAuditFinalization,
...
/// Run 329 — the explicit trusted inputs the executor binds a verified
/// Run 327/328 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentInputs {
    ...
    pub audit_ledger_commitment_policy_id: String,
    /// Expected Run 327/328 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 319/320 application policy id (re-exposed by the
    ...
    /// Expected Run 315 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 315 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 315 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 317 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 317 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 317 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 317 rotation plan digest.
    ...
    /// Expected Run 319/320 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 319/320 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 319/320 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 319/320 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 327/328 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 327/328 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 327/328 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 327/328 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 327/328 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 327/328 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 327/328 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 327/328 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 327/328 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 319/320 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 327/328 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 325/326 guarded audit-ledger-commitment decision id (re-exposed
    /// by the consumed Run 327/328 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 325/326 guarded audit-ledger-commitment request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 325/326 guarded audit-ledger-commitment intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 325/326 guarded audit-ledger-commitment transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 325/326 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 327/328 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_durable_audit_finalization_decision_id: String,
    /// Expected Run 327/328 runtime handoff request id.
    pub expected_durable_audit_finalization_request_id: String,
    /// Expected Run 327/328 runtime handoff package/content digest (the
    /// consumed decision's `audit_ledger_commitment_digest`, i.e. the handoff content
    /// digest).
    pub expected_durable_audit_finalization_intent_digest: String,
    /// Expected Run 327/328 runtime handoff transcript digest.
    pub expected_durable_audit_finalization_transcript_digest: String,
    /// Expected Run 327/328 durable-audit-finalization nonce (re-exposed by the
    /// consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_durable_audit_finalization_nonce: u64,
    /// Expected Run 325/326 commit-authorization decision id (re-exposed by
    /// the consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_commit_authorization_decision_id: String,
    /// Expected Run 325/326 commit-authorization request id.
    pub expected_commit_authorization_request_id: String,
    /// Expected Run 325/326 commit-authorization intent/content digest.
    pub expected_commit_authorization_intent_digest: String,
    /// Expected Run 325/326 commit-authorization transcript digest.
    pub expected_commit_authorization_transcript_digest: String,
    /// Expected Run 325/326 commit-authorization nonce (re-exposed by the
    /// consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_commit_authorization_nonce: u64,
    /// Expected Run 325/326 mutation-execution decision id (re-exposed by
    /// the consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 325/326 mutation-execution request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 325/326 mutation-execution intent/content digest.
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 325/326 mutation-execution transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 325/326 mutation-execution nonce (re-exposed by the
    /// consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 325/326 execution-preparation decision id (re-exposed by
    /// the consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 325/326 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 325/326 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 325/326 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 325/326 execution-preparation nonce (re-exposed by the
    /// consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 325/326 runtime handoff decision id (re-exposed by the
    /// consumed Run 327/328 durable-audit-finalization artifact).
    pub expected_runtime_handoff_decision_id: String,
    /// Expected Run 325/326 runtime handoff request id.
    pub expected_runtime_handoff_request_id: String,
    /// Expected Run 325/326 runtime handoff package/content digest.
    pub expected_runtime_handoff_intent_digest: String,
    /// Expected Run 325/326 runtime handoff transcript digest.
    pub expected_runtime_handoff_transcript_digest: String,
    /// Expected Run 325/326 runtime handoff nonce (re-exposed by the consumed
    /// Run 327/328 durable-audit-finalization artifact).
    ...
    /// wall-clock value and never a reject path in Run 329.
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.audit_ledger_commitment_policy_id.is_empty()
            ...
            && !self.expected_durable_audit_finalization_decision_id.is_empty()
            && !self.expected_durable_audit_finalization_request_id.is_empty()
            && !self.expected_durable_audit_finalization_intent_digest.is_empty()
            && !self.expected_durable_audit_finalization_transcript_digest.is_empty()
...
/// Run 329 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition audit-ledger-commitment decision), the explicit
/// epoch-transition target, a durable-audit-finalization nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentRequest {
    pub authority_source: LiveEpochTransitionAuditLedgerCommitmentAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The durable-audit-finalization nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and durable-audit-finalization nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuditLedgerCommitmentAuthoritySource,
        proposed_epoch_transition_target: u64,
        audit_ledger_commitment_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
            proposed_epoch_transition_target,
            audit_ledger_commitment_nonce,
...
/// Run 329 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuditLedgerCommitmentReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuditLedgerCommitmentReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuditLedgerCommitmentReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuditLedgerCommitmentReplaySet;

impl LiveEpochTransitionAuditLedgerCommitmentReplaySet
    for EmptyLiveEpochTransitionAuditLedgerCommitmentReplaySet
{
...
/// Run 329 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 330+); Run 329 never
...
/// preconditions, and carries deterministic `audit_ledger_commitment_id`, `request_id`,
/// `audit_ledger_commitment_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact {
    pub staged_kind: LiveEpochTransitionAuditLedgerCommitmentKind,
    pub protocol_version: u16,
    pub audit_ledger_commitment_policy_id: String,

    // ---- Re-exposed Run 327/328 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 319/320 application-decision tuple ------------
    ...
    // ---- Bound Run 327/328 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 327/328 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 327/328 guarded audit-ledger-commitment authority tuple -
    // (the consumed guarded audit-ledger-commitment decision transcript)
    ...
    // ---- Guarded audit-ledger-commitment binding (re-exposed consumed nonce)
    /// The re-exposed Run 325/326 guarded audit-ledger-commitment nonce.
    ...
    pub durable_audit_finalization_decision_id: String,
    pub durable_audit_finalization_request_id: String,
    pub durable_audit_finalization_intent_digest: String,
    pub durable_audit_finalization_transcript_digest: String,
    /// The re-exposed Run 327/328 durable-audit-finalization nonce (consumed).
    pub durable_audit_finalization_nonce: u64,

    // ---- Bound Run 325/326 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 327/328 durable-audit-finalization artifact)
    ...
    /// The re-exposed Run 325/326 commit-authorization nonce (consumed).
    pub commit_authorization_nonce: u64,

    // ---- Bound Run 325/326 mutation-execution authority tuple -------
    // (re-exposed by the consumed Run 327/328 durable-audit-finalization artifact)
    ...
    /// The re-exposed Run 325/326 mutation-execution nonce (consumed).
    pub mutation_execution_nonce: u64,

    // ---- Bound Run 325/326 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 327/328 durable-audit-finalization artifact)
    ...
    /// The re-exposed Run 325/326 execution-preparation nonce (consumed).
    pub execution_preparation_nonce: u64,

    // ---- Bound Run 325/326 runtime handoff decision authority tuple ----
    // (re-exposed by the consumed Run 327/328 durable-audit-finalization artifact)
    ...
    /// The re-exposed Run 325/326 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed audit-ledger-commitment nonce.
    pub audit_ledger_commitment_nonce: u64,
    ...
    pub audit_ledger_commitment_id: String,
    pub request_id: String,
    pub audit_ledger_commitment_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`audit_ledger_commitment_id`,
    /// `request_id`, `audit_ledger_commitment_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"audit_ledger_commitment_policy_id",
            self.audit_ledger_commitment_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"durable_audit_finalization_decision_id",
            self.durable_audit_finalization_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_finalization_request_id",
            self.durable_audit_finalization_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_finalization_intent_digest",
            self.durable_audit_finalization_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_finalization_transcript_digest",
            self.durable_audit_finalization_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_finalization_nonce",
            &self.durable_audit_finalization_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Custody binding canonical hashing (module-local; mirrors Run 315/317/319
...
/// Run 329 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_audit_ledger_commitment_content_digest(
    package: &ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact,
) -> String {
    package.content_digest()
}

/// Run 329 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and durable-audit-finalization nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_audit_ledger_commitment_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    audit_ledger_commitment_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"audit_ledger_commitment_policy_id",
        audit_ledger_commitment_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"audit_ledger_commitment_nonce",
        &audit_ledger_commitment_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 329 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and durable-audit-finalization nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_audit_ledger_commitment_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    audit_ledger_commitment_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"audit_ledger_commitment_policy_id", audit_ledger_commitment_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 329 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_audit_ledger_commitment_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUDIT_LEDGER_COMMITMENT_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 329 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuditLedgerCommitment`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuditLedgerCommitmentBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition audit-ledger-commitment
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuditLedgerCommitment {
        execution_kind: LiveEpochTransitionAuditLedgerCommitmentKind,
    ...
    VerifiedDurableAuditFinalizationDecisionRequired,
    UnverifiedDurableAuditFinalizationDecisionRejected,
    ...
    DurableAuditFinalizationDecisionIdMismatch,
    DurableAuditFinalizationDecisionRequestIdMismatch,
    DurableAuditFinalizationDecisionIntentDigestMismatch,
    DurableAuditFinalizationDecisionTranscriptMismatch,
    DurableAuditFinalizationDecisionIntegrityMismatch,
    WrongDurableAuditFinalizationNonce,
    ...
    LiveEpochTransitionAuditLedgerCommitmentAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuditLedgerCommitmentBoundaryUnavailable => {
                "live-epoch-transition-audit-ledger-commitment-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable => {
                "production-live-epoch-transition-audit-ledger-commitment-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable => {
                "mainnet-production-live-epoch-transition-audit-ledger-commitment-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuditLedgerCommitment { .. } => {
                "accepted-source-test-live-epoch-transition-audit-ledger-commitment"
            }
            Self::VerifiedDurableAuditFinalizationDecisionRequired => {
                "verified-durable-audit-finalization-decision-required"
            }
            Self::UnverifiedDurableAuditFinalizationDecisionRejected => {
                "unverified-durable-audit-finalization-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-durable-audit-finalization-decision-rejected-as-production-authority"
            }
            ...
            Self::DurableAuditFinalizationDecisionIdMismatch => "durable-audit-finalization-decision-id-mismatch",
            Self::DurableAuditFinalizationDecisionRequestIdMismatch => {
                "durable-audit-finalization-decision-request-id-mismatch"
            }
            Self::DurableAuditFinalizationDecisionIntentDigestMismatch => {
                "durable-audit-finalization-decision-intent-digest-mismatch"
            }
            Self::DurableAuditFinalizationDecisionTranscriptMismatch => {
                "durable-audit-finalization-decision-transcript-mismatch"
            }
            Self::DurableAuditFinalizationDecisionIntegrityMismatch => {
                "durable-audit-finalization-decision-integrity-mismatch"
            }
            Self::WrongDurableAuditFinalizationNonce => "wrong-durable-audit-finalization-nonce",
            ...
            Self::LiveEpochTransitionAuditLedgerCommitmentAmbiguous { .. } => {
                "epoch-transition-durable-audit-finalization-ambiguous"
            }
...
/// Run 329 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentDecision {
    pub outcome: ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome,
    pub audit_ledger_commitment_id: String,
    pub request_id: String,
    pub audit_ledger_commitment_artifact:
        Option<ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact>,
    pub audit_ledger_commitment_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.audit_ledger_commitment_artifact.is_some()
    }
}
...
/// Run 329 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome {
...
impl ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome {
...
/// Run 329 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor {
    pub config: ProductionLiveEpochTransitionAuditLedgerCommitmentConfig,
    pub policy: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuditLedgerCommitmentConfig,
        policy: ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy,
    ) -> Self {
        Self { config, policy }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::source_test(),
            ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::AllowSourceTestLiveEpochTransitionAuditLedgerCommitment,
        )
    }
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuditLedgerCommitmentAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionDurableAuditFinalizationDecision,
            &'a ProductionLiveEpochTransitionDurableAuditFinalizationArtifact,
        ),
        ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome,
    > {
        use ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome as O;
        use LiveEpochTransitionAuditLedgerCommitmentAuthoritySource as S;
        match source {
            S::VerifiedDurableAuditFinalizationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedDurableAuditFinalizationDecisionRejected);
                }
                match &decision.durable_audit_finalization_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedDurableAuditFinalizationDecisionRequired),
                }
            }
            S::MissingDurableAuditFinalizationDecision => {
                Err(O::VerifiedDurableAuditFinalizationDecisionRequired)
            }
            S::UnverifiedDurableAuditFinalizationDecision { .. } => {
                Err(O::UnverifiedDurableAuditFinalizationDecisionRejected)
            }
            S::AcceptedDurableAuditFinalizationWithoutPackage { .. } => {
                Err(O::VerifiedDurableAuditFinalizationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutDurableAuditFinalization => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutDurableAuditFinalization => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutDurableAuditFinalization => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutDurableAuditFinalization => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutDurableAuditFinalization => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutDurableAuditFinalization => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutDurableAuditFinalization => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutDurableAuditFinalization => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutDurableAuditFinalization => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutDurableAuditFinalization => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutDurableAuditFinalization => {
            ...
            S::FixtureOnlyDurableAuditFinalization => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome> {
        use ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::MainnetProductionLiveEpochTransitionAuditLedgerCommitmentRequired => {
                    O::MainNetProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuditLedgerCommitmentUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 329.
        if self.config.kind
            == ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::ProductionLiveEpochTransitionAuditLedgerCommitment
        {
            return Some(O::LiveEpochTransitionAuditLedgerCommitmentBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 327/328 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionDurableAuditFinalizationDecision,
        intent: &ProductionLiveEpochTransitionDurableAuditFinalizationArtifact,
        inputs: &ProductionLiveEpochTransitionAuditLedgerCommitmentInputs,
    ) -> Option<ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome> {
        use ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 327/328 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.durable_audit_finalization_id != inputs.expected_durable_audit_finalization_decision_id {
            return Some(O::DurableAuditFinalizationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_durable_audit_finalization_request_id {
            return Some(O::DurableAuditFinalizationDecisionRequestIdMismatch);
        }
        if decision.durable_audit_finalization_digest != inputs.expected_durable_audit_finalization_intent_digest {
            return Some(O::DurableAuditFinalizationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_durable_audit_finalization_transcript_digest {
            return Some(O::DurableAuditFinalizationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.durable_audit_finalization_digest {
            return Some(O::DurableAuditFinalizationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed durable-audit-finalization
        // nonce binding.
        if intent.durable_audit_finalization_nonce != inputs.expected_durable_audit_finalization_nonce {
            return Some(O::WrongDurableAuditFinalizationNonce);
        }

        // Re-exposed Run 325/326 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 327/328
        // durable-audit-finalization artifact).
        ...
        // Re-exposed Run 325/326 mutation-execution decision authority
        // tuple binding (carried through the consumed Run 327/328
        // durable-audit-finalization artifact).
        ...
        // Re-exposed Run 325/326 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 327/328
        // durable-audit-finalization artifact).
        ...
        // Re-exposed Run 325/326 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 327/328
        // durable-audit-finalization artifact).
        ...
        // Re-exposed Run 325/326 guarded epoch-transition audit-ledger-commitment
        ...
        // Re-exposed Run 327/328 staged-application decision authority tuple
        ...
        // Re-exposed Run 327/328 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 319/320 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuditLedgerCommitmentReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionAuditLedgerCommitmentRequest,
        inputs: &ProductionLiveEpochTransitionAuditLedgerCommitmentInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_audit_ledger_commitment_request_id(
            self.config.protocol_version.0,
            &decision.durable_audit_finalization_digest,
            &inputs.audit_ledger_commitment_policy_id,
            request.proposed_epoch_transition_target,
            request.audit_ledger_commitment_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionAuditLedgerCommitmentKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            audit_ledger_commitment_policy_id: inputs.audit_ledger_commitment_policy_id.clone(),
            ...
            // Re-exposed Run 325/326 guarded audit-ledger-commitment decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded audit-ledger-commitment nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 327/328 runtime handoff decision transcript (the
            // authority source this run consumes).
            durable_audit_finalization_decision_id: decision.durable_audit_finalization_id.clone(),
            durable_audit_finalization_request_id: decision.request_id.clone(),
            durable_audit_finalization_intent_digest: decision.durable_audit_finalization_digest.clone(),
            durable_audit_finalization_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed durable-audit-finalization nonce.
            durable_audit_finalization_nonce: application_intent.durable_audit_finalization_nonce,
            // Re-exposed Run 325/326 commit-authorization decision authority
            // tuple (carried through the consumed durable-audit-finalization artifact).
            ...
            // Re-exposed Run 325/326 runtime handoff decision authority tuple
            // (carried through the consumed durable-audit-finalization artifact).
            ...
            // Newly proposed audit-ledger-commitment nonce.
            audit_ledger_commitment_nonce: request.audit_ledger_commitment_nonce,
            // Exact future-executor preconditions.
            ...
            audit_ledger_commitment_id: String::new(),
            request_id: String::new(),
            audit_ledger_commitment_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuditLedgerCommitment {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            audit_ledger_commitment_nonce: request.audit_ledger_commitment_nonce,
        };
        let audit_ledger_commitment_id = production_live_epoch_transition_audit_ledger_commitment_id(
            self.config.protocol_version.0,
            &decision.durable_audit_finalization_digest,
            &inputs.audit_ledger_commitment_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_audit_ledger_commitment_request_id(
            self.config.protocol_version.0,
            &decision.durable_audit_finalization_digest,
            &inputs.audit_ledger_commitment_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.audit_ledger_commitment_digest = record.content_digest();
        record.audit_ledger_commitment_id = audit_ledger_commitment_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_audit_ledger_commitment_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.audit_ledger_commitment_digest,
                outcome.tag(),
            );
    ...
    /// Run 329 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_audit_ledger_commitment<
        R: LiveEpochTransitionAuditLedgerCommitmentReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionAuditLedgerCommitmentRequest,
        inputs: &ProductionLiveEpochTransitionAuditLedgerCommitmentInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuditLedgerCommitmentDecision {
                outcome,
                audit_ledger_commitment_id: pkg.audit_ledger_commitment_id.clone(),
                request_id: pkg.request_id.clone(),
                audit_ledger_commitment_artifact: record.clone(),
                audit_ledger_commitment_digest: pkg.audit_ledger_commitment_digest.clone(),
        ...
        let durable_audit_finalization_intent_digest = match &request.authority_source {
            LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::VerifiedDurableAuditFinalizationDecision {
                decision,
            }
            | LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::UnverifiedDurableAuditFinalizationDecision {
                decision,
            }
            | LiveEpochTransitionAuditLedgerCommitmentAuthoritySource::AcceptedDurableAuditFinalizationWithoutPackage {
                decision,
            } => decision.durable_audit_finalization_digest.clone(),
            _ => String::new(),
        };

        let audit_ledger_commitment_id = production_live_epoch_transition_audit_ledger_commitment_id(
            self.config.protocol_version.0,
            &durable_audit_finalization_intent_digest,
            &inputs.audit_ledger_commitment_policy_id,
            request.proposed_epoch_transition_target,
            request.audit_ledger_commitment_nonce,
        );
        ...
        let audit_ledger_commitment_digest = String::new();
        let transcript_digest =
            production_live_epoch_transition_audit_ledger_commitment_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &audit_ledger_commitment_digest,
                outcome.tag(),
            );

        ProductionLiveEpochTransitionAuditLedgerCommitmentDecision {
            outcome,
            audit_ledger_commitment_id,
            request_id,
            audit_ledger_commitment_artifact: None,
            audit_ledger_commitment_digest,
            transcript_digest,
        }
    }

    /// Run 329 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_audit_ledger_commitment_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact>,
        current: &ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact,
    ) -> ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome {
        use ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::Disabled
        {
...
/// Run 329 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::default()
        == ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionAuditLedgerCommitmentConfig::default().kind
            == ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind::Disabled
}

/// Run 329 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 330).
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 329 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_mainnet_refused() -> bool {
    true
}

/// Run 329 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_is_non_mutating() -> bool {
    true
}

/// Run 329 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_never_falls_back() -> bool {
    true
}

/// Run 329 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 329 — the executor only requires a verified Run 327/328 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_audit_ledger_commitment_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 329 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 329. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuditLedgerCommitmentFixtureState {
...
impl LiveEpochTransitionAuditLedgerCommitmentFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, audit_ledger_commitment_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == audit_ledger_commitment_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `audit_ledger_commitment_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact,
        audit_ledger_commitment_id: &str,
    ) -> bool {
        if self.has_applied(audit_ledger_commitment_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(audit_ledger_commitment_id.to_string());
        true
    }
}