//! Consensus networking adapter for the qbind node.
//!
...
//! `NetMessage`. This adapter is used by the consensus engine (`qbind-consensus`)
//! in tests and in the node.
//!
//! The adapter also implements `qbind_consensus::ConsensusNetwork`, allowing
...
use qbind_consensus::{ConsensusNetwork, ConsensusNetworkEvent, NetworkError};
...
/// Implementation of the abstract `ConsensusNetwork` trait from `qbind-consensus`.
///
...
/// `PeerId(u64)` wrapper defined in `qbind-node::peer`. If the consensus crate
