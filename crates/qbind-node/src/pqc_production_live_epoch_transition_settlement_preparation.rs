//! Run 335 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 329/330 guarded epoch-transition settlement-preparation boundary: the boundary
//! that consumes a **verified** Run 333/334 non-mutating guarded
//! epoch-transition settlement-preparation *decision* (the accepted
//! [`ProductionLiveEpochTransitionExternalPublicationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionExternalPublicationArtifact`], as
...
//! mutation record does it authorize?", Run 335 answers the next question:
//! "given a verified guarded settlement-preparation decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a external-publication
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 335)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 336**.
...
//! * Only a **verified** Run 333/334 guarded epoch-transition
//!   settlement-preparation decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionExternalPublicationArtifact`] can authorize a runtime
...
//!   [`ProductionLiveEpochTransitionSettlementPreparationArtifact`]; it
...
/// Run 335 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_PROTOCOL_VERSION: u16 = 1;

/// Run 335 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:335-epoch-transition-external-publication-intent:v1";

/// Run 335 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_ID_DOMAIN_TAG: &str =
    "QBIND:335-epoch-transition-external-publication-id:v1";

/// Run 335 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:335-epoch-transition-external-publication-request:v1";

/// Run 335 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:335-epoch-transition-external-publication-transcript:v1";
...
/// Run 335 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionSettlementPreparationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionSettlementPreparationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionSettlementPreparationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionSettlementPreparation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 333/334 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionSettlementPreparation` and
/// `MainnetProductionLiveEpochTransitionSettlementPreparationRequired` are
...
pub enum ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy {
...
impl ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionSettlementPreparation => {
                "allow-source-test-epoch-transition-external-publication"
            }
            Self::RequireProductionLiveEpochTransitionSettlementPreparation => {
                "require-production-epoch-transition-external-publication"
            }
            Self::MainnetProductionLiveEpochTransitionSettlementPreparationRequired => {
                "mainnet-production-epoch-transition-external-publication-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionSettlementPreparation` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionSettlementPreparation` kind is fail-closed as
/// unavailable in Run 335 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionSettlementPreparationExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 335.
    ProductionLiveEpochTransitionSettlementPreparation,
}

impl ProductionLiveEpochTransitionSettlementPreparationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionSettlementPreparation => {
                "source-test-epoch-transition-external-publication"
            }
            Self::ProductionLiveEpochTransitionSettlementPreparation => {
                "production-epoch-transition-external-publication"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionSettlementPreparationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionSettlementPreparationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionSettlementPreparationExecutorKind,
}

impl ProductionLiveEpochTransitionSettlementPreparationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionSettlementPreparationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionSettlementPreparationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionSettlementPreparationExecutorKind::SourceTestLiveEpochTransitionSettlementPreparation,
        )
    }
...
impl Default for ProductionLiveEpochTransitionSettlementPreparationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionSettlementPreparationExecutorKind::Disabled)
    }
}
...
/// Run 335 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 333/334
/// [`LiveEpochTransitionExternalPublicationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionSettlementPreparationKind {
...
impl LiveEpochTransitionSettlementPreparationKind {
    ...
    /// Every Run 335 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 333/334 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionExternalPublicationKind,
    ) -> Self {
        use LiveEpochTransitionExternalPublicationKind as A;
        match kind {
...
/// Run 335 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedExternalPublicationDecision`] carrying a Run 333/334
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionSettlementPreparationAuthoritySource {
    /// A verified Run 333/334 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedExternalPublicationDecision {
        decision: ProductionLiveEpochTransitionExternalPublicationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingExternalPublicationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedExternalPublicationDecision {
        decision: ProductionLiveEpochTransitionExternalPublicationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedExternalPublicationWithoutPackage {
        decision: ProductionLiveEpochTransitionExternalPublicationDecision,
    },
    /// A Run 329/330 epoch-transition commit-authorization decision presented
    /// directly, without a Run 333/334 external-publication decision. Rejected.
    CommitAuthorizationDecisionWithoutExternalPublication,
    /// A Run 329/330 epoch-transition mutation-execution decision presented
    /// directly, without a Run 333/334 external-publication decision. Rejected.
    MutationExecutionDecisionWithoutExternalPublication,
    /// A Run 329/330 epoch-transition execution-preparation decision presented
    /// directly, without a Run 333/334 external-publication decision. Rejected.
    ExecutionPreparationDecisionWithoutExternalPublication,
    /// A Run 329/330 epoch-transition runtime handoff decision presented
    /// directly, without a Run 333/334 external-publication decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutExternalPublication,
    /// A Run 329/330 guarded epoch-transition settlement-preparation decision
    /// presented directly, without a Run 333/334 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutExternalPublication,
    /// A Run 327/328 staged-application decision presented directly, without a
    /// Run 333/334 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutExternalPublication,
    /// A Run 325/326 live-application authorization presented directly, without
    /// a Run 333/334 guarded settlement-preparation decision. Rejected.
    LiveApplicationAuthorizationWithoutExternalPublication,
    /// A Run 323/324 validator-set rotation *application decision* presented
    /// directly, without a Run 333/334 guarded settlement-preparation decision.
    /// Rejected.
    ApplicationDecisionWithoutExternalPublication,
    /// A Run 321/322 validator-set rotation plan presented directly, without a
    /// Run 333/334 live-application authorization. Rejected.
    RotationPlanWithoutExternalPublication,
    /// A Run 319/320 governance execution intent presented directly, without a
    /// Run 333/334 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutExternalPublication,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 325/326 live-application authorization. Rejected.
    GovernanceProofWithoutExternalPublication,
    ...
    FixtureOnlyExternalPublication,
...
/// Run 335 — the explicit trusted inputs the executor binds a verified
/// Run 333/334 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementPreparationInputs {
    ...
    pub settlement_preparation_policy_id: String,
    /// Expected Run 333/334 live-application authorization policy id (bound
    ...
    /// Expected Run 333/334 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 333/334 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 333/334 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 333/334 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 333/334 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 333/334 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 333/334 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 333/334 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 333/334 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 333/334 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 329/330 guarded settlement-preparation decision id (re-exposed
    /// by the consumed Run 333/334 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 329/330 guarded settlement-preparation request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 329/330 guarded settlement-preparation intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 329/330 guarded settlement-preparation transcript digest.
    ...
    /// Expected Run 333/334 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_external_publication_decision_id: String,
    /// Expected Run 333/334 runtime handoff request id.
    pub expected_external_publication_request_id: String,
    /// Expected Run 333/334 runtime handoff package/content digest (the
    /// consumed decision's `settlement_preparation_digest`, i.e. the handoff content
    /// digest).
    pub expected_external_publication_intent_digest: String,
    /// Expected Run 333/334 runtime handoff transcript digest.
    pub expected_external_publication_transcript_digest: String,
    /// Expected Run 333/334 external-publication nonce (re-exposed by the
    /// consumed Run 333/334 external-publication artifact).
    pub expected_external_publication_nonce: u64,
    /// Expected Run 329/330 commit-authorization decision id (re-exposed by
    /// the consumed Run 333/334 external-publication artifact).
    ...
    /// Run 333/334 external-publication artifact).
    ...
    /// wall-clock value and never a reject path in Run 335.
...
impl ProductionLiveEpochTransitionSettlementPreparationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.settlement_preparation_policy_id.is_empty()
...
/// Run 335 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition settlement-preparation decision), the explicit
/// epoch-transition target, a external-publication nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementPreparationRequest {
    pub authority_source: LiveEpochTransitionSettlementPreparationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The external-publication nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionSettlementPreparationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and external-publication nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionSettlementPreparationAuthoritySource,
        proposed_epoch_transition_target: u64,
        settlement_preparation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 335 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionSettlementPreparationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionSettlementPreparationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionSettlementPreparationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionSettlementPreparationReplaySet;

impl LiveEpochTransitionSettlementPreparationReplaySet
    for EmptyLiveEpochTransitionSettlementPreparationReplaySet
{
...
/// Run 335 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 336+); Run 335 never
...
/// preconditions, and carries deterministic `settlement_preparation_id`, `request_id`,
/// `settlement_preparation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementPreparationArtifact {
    pub staged_kind: LiveEpochTransitionSettlementPreparationKind,
    pub protocol_version: u16,
    pub settlement_preparation_policy_id: String,

    // ---- Re-exposed Run 333/334 authorization intent tuple ------------
    ...
    // ---- Bound Run 333/334 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 333/334 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 333/334 guarded settlement-preparation authority tuple -
    // (the consumed guarded settlement-preparation decision transcript)
    ...
    // ---- Guarded settlement-preparation binding (re-exposed consumed nonce)
    /// The re-exposed Run 329/330 guarded settlement-preparation nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 333/334 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub external_publication_decision_id: String,
    pub external_publication_request_id: String,
    pub external_publication_intent_digest: String,
    pub external_publication_transcript_digest: String,
    /// The re-exposed Run 333/334 external-publication nonce (consumed).
    pub external_publication_nonce: u64,

    // ---- Bound Run 329/330 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 333/334 external-publication artifact)
    ...
    /// The newly proposed settlement-preparation nonce.
    pub settlement_preparation_nonce: u64,
    ...
    pub settlement_preparation_id: String,
    pub request_id: String,
    pub settlement_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementPreparationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`settlement_preparation_id`,
    /// `request_id`, `settlement_preparation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        ...
        hash_field(
            &mut h,
            b"external_publication_decision_id",
            self.external_publication_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"external_publication_request_id",
            self.external_publication_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"external_publication_intent_digest",
            self.external_publication_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"external_publication_transcript_digest",
            self.external_publication_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
...
pub fn production_live_epoch_transition_settlement_preparation_content_digest(
    package: &ProductionLiveEpochTransitionSettlementPreparationArtifact,
) -> String {
    package.content_digest()
}

/// Run 335 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and external-publication nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_settlement_preparation_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_preparation_policy_id: &str,
    ...
    hash_field(
        &mut h,
        b"settlement_preparation_nonce",
        &settlement_preparation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 335 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and external-publication nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_settlement_preparation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_preparation_policy_id: &str,
    epoch_transition_target: u64,
...
/// Run 335 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_settlement_preparation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_PREPARATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 335 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionSettlementPreparation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementPreparationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionSettlementPreparationBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionSettlementPreparationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition settlement-preparation
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionSettlementPreparation {
        execution_kind: LiveEpochTransitionSettlementPreparationKind,
    ...
    VerifiedExternalPublicationDecisionRequired,
    UnverifiedExternalPublicationDecisionRejected,
    ...
    ExternalPublicationDecisionIdMismatch,
    ExternalPublicationDecisionRequestIdMismatch,
    ExternalPublicationDecisionIntentDigestMismatch,
    ExternalPublicationDecisionTranscriptMismatch,
    ExternalPublicationDecisionIntegrityMismatch,
    WrongExternalPublicationNonce,
...
impl ProductionLiveEpochTransitionSettlementPreparationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionSettlementPreparationBoundaryUnavailable => {
                "live-epoch-transition-settlement-preparation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionSettlementPreparationUnavailable => {
                "production-live-epoch-transition-settlement-preparation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionSettlementPreparationUnavailable => {
                "mainnet-production-live-epoch-transition-settlement-preparation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionSettlementPreparation { .. } => {
                "accepted-source-test-live-epoch-transition-settlement-preparation"
            }
            Self::VerifiedExternalPublicationDecisionRequired => {
                "verified-external-publication-decision-required"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-external-publication-decision-rejected-as-production-authority"
            }
            ...
            Self::ExternalPublicationDecisionIntentDigestMismatch => {
                "external-publication-decision-intent-digest-mismatch"
            }
            Self::ExternalPublicationDecisionTranscriptMismatch => {
                "external-publication-decision-transcript-mismatch"
            }
            Self::ExternalPublicationDecisionIntegrityMismatch => {
                "external-publication-decision-integrity-mismatch"
            }
            Self::WrongExternalPublicationNonce => "wrong-external-publication-nonce",
            ...
            Self::LiveEpochTransitionSettlementPreparationAmbiguous { .. } => {
                "epoch-transition-external-publication-ambiguous"
            }
...
/// Run 335 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementPreparationDecision {
    pub outcome: ProductionLiveEpochTransitionSettlementPreparationOutcome,
    pub settlement_preparation_id: String,
    pub request_id: String,
    pub settlement_preparation_artifact:
        Option<ProductionLiveEpochTransitionSettlementPreparationArtifact>,
    pub settlement_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementPreparationDecision {
    pub fn is_accept(&self) -> bool {
...
/// Run 335 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome {
...
/// Run 335 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementPreparationExecutor {
    pub config: ProductionLiveEpochTransitionSettlementPreparationConfig,
    pub policy: ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy,
}

impl ProductionLiveEpochTransitionSettlementPreparationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionSettlementPreparationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionSettlementPreparationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionExternalPublicationDecision,
            &'a ProductionLiveEpochTransitionExternalPublicationArtifact,
        ),
        ...
        match source {
            S::VerifiedExternalPublicationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedExternalPublicationDecisionRejected);
                }
                match &decision.external_publication_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedExternalPublicationDecisionRequired),
                }
            }
            S::MissingExternalPublicationDecision => {
                Err(O::VerifiedExternalPublicationDecisionRequired)
            }
            S::UnverifiedExternalPublicationDecision { .. } => {
                Err(O::UnverifiedExternalPublicationDecisionRejected)
            }
            S::AcceptedExternalPublicationWithoutPackage { .. } => {
                Err(O::VerifiedExternalPublicationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutExternalPublication => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutExternalPublication => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutExternalPublication => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutExternalPublication => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutExternalPublication => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutExternalPublication => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutExternalPublication => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutExternalPublication => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutExternalPublication => Err(O::RotationPlanAloneRejected),
            ...
            S::FixtureOnlyExternalPublication => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionSettlementPreparationOutcome> {
        use ProductionLiveEpochTransitionSettlementPreparationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementPreparationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionSettlementPreparationRequired => {
                    O::MainNetProductionLiveEpochTransitionSettlementPreparationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionSettlementPreparationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionSettlementPreparationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 335.
        if self.config.kind
            == ProductionLiveEpochTransitionSettlementPreparationExecutorKind::ProductionLiveEpochTransitionSettlementPreparation
        {
            return Some(O::LiveEpochTransitionSettlementPreparationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 333/334 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionExternalPublicationDecision,
        intent: &ProductionLiveEpochTransitionExternalPublicationArtifact,
        inputs: &ProductionLiveEpochTransitionSettlementPreparationInputs,
    ) -> Option<ProductionLiveEpochTransitionSettlementPreparationOutcome> {
        use ProductionLiveEpochTransitionSettlementPreparationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 333/334 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.external_publication_id != inputs.expected_external_publication_decision_id {
            return Some(O::ExternalPublicationDecisionIdMismatch);
        }
        ...
        if decision.transcript_digest != inputs.expected_external_publication_transcript_digest {
            return Some(O::ExternalPublicationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.external_publication_digest {
            return Some(O::ExternalPublicationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed external-publication
        // nonce binding.
        if intent.external_publication_nonce != inputs.expected_external_publication_nonce {
            return Some(O::WrongExternalPublicationNonce);
        }
        ...
        // authority tuple binding (carried through the consumed Run 333/334
        // external-publication artifact).
        ...
        // Re-exposed Run 329/330 guarded epoch-transition settlement-preparation
        ...
        // Re-exposed Run 333/334 staged-application decision authority tuple
        ...
        // Re-exposed Run 333/334 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionSettlementPreparationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionSettlementPreparationRequest,
        inputs: &ProductionLiveEpochTransitionSettlementPreparationInputs,
        replay_set: &R,
        ...
        let staged_kind =
            LiveEpochTransitionSettlementPreparationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionSettlementPreparationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            settlement_preparation_policy_id: inputs.settlement_preparation_policy_id.clone(),
            ...
            // Re-exposed Run 329/330 guarded settlement-preparation decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded settlement-preparation nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 333/334 runtime handoff decision transcript (the
            // authority source this run consumes).
            external_publication_decision_id: decision.external_publication_id.clone(),
            external_publication_request_id: decision.request_id.clone(),
            external_publication_intent_digest: decision.external_publication_digest.clone(),
            external_publication_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed external-publication nonce.
            external_publication_nonce: application_intent.external_publication_nonce,
            // Re-exposed Run 329/330 commit-authorization decision authority
            // tuple (carried through the consumed external-publication artifact).
            ...
            settlement_preparation_id: String::new(),
            request_id: String::new(),
            settlement_preparation_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionSettlementPreparation {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            settlement_preparation_nonce: request.settlement_preparation_nonce,
        };
        let settlement_preparation_id = production_live_epoch_transition_settlement_preparation_id(
            self.config.protocol_version.0,
            &decision.external_publication_digest,
            &inputs.settlement_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_settlement_preparation_request_id(
            self.config.protocol_version.0,
            &decision.external_publication_digest,
            &inputs.settlement_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.settlement_preparation_digest = record.content_digest();
        record.settlement_preparation_id = settlement_preparation_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_settlement_preparation_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.settlement_preparation_digest,
                outcome.tag(),
            );
    ...
    /// Run 335 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_settlement_preparation<
        R: LiveEpochTransitionSettlementPreparationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionSettlementPreparationRequest,
        inputs: &ProductionLiveEpochTransitionSettlementPreparationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionSettlementPreparationDecision {
                outcome,
                settlement_preparation_id: pkg.settlement_preparation_id.clone(),
                request_id: pkg.request_id.clone(),
                settlement_preparation_artifact: record.clone(),
                settlement_preparation_digest: pkg.settlement_preparation_digest.clone(),
        ...
        let external_publication_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionSettlementPreparationAuthoritySource::AcceptedExternalPublicationWithoutPackage {
                decision,
            } => decision.external_publication_digest.clone(),
            _ => String::new(),
        };

        let settlement_preparation_id = production_live_epoch_transition_settlement_preparation_id(
            self.config.protocol_version.0,
            &external_publication_intent_digest,
            &inputs.settlement_preparation_policy_id,
            request.proposed_epoch_transition_target,
            request.settlement_preparation_nonce,
        );
        ...
        let settlement_preparation_digest = String::new();
        let transcript_digest =
            production_live_epoch_transition_settlement_preparation_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &settlement_preparation_digest,
                outcome.tag(),
            );
        ...
        ProductionLiveEpochTransitionSettlementPreparationDecision {
            ...
            request_id,
            settlement_preparation_artifact: None,
            settlement_preparation_digest,
            transcript_digest,
        }
    }

    /// Run 335 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_settlement_preparation_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionSettlementPreparationArtifact>,
        current: &ProductionLiveEpochTransitionSettlementPreparationArtifact,
    ) -> ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome {
        use ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementPreparationExecutorKind::Disabled
        {
...
/// Run 335 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_settlement_preparation_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy::default()
        == ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionSettlementPreparationConfig::default().kind
            == ProductionLiveEpochTransitionSettlementPreparationExecutorKind::Disabled
}

/// Run 335 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 336).
pub fn production_live_epoch_transition_settlement_preparation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 335 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_settlement_preparation_executor_mainnet_refused() -> bool {
    true
}

/// Run 335 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_settlement_preparation_executor_is_non_mutating() -> bool {
    true
}

/// Run 335 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_settlement_preparation_executor_never_falls_back() -> bool {
    true
}

/// Run 335 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_settlement_preparation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 335 — the executor only requires a verified Run 333/334 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_settlement_preparation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 335 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 335. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionSettlementPreparationFixtureState {
...
impl LiveEpochTransitionSettlementPreparationFixtureState {
    ...
    ///
    /// Idempotent: re-applying the same `settlement_preparation_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionSettlementPreparationArtifact,
        settlement_preparation_id: &str,
    ) -> bool {
        if self.has_applied(settlement_preparation_id) {
            return false;
        }
