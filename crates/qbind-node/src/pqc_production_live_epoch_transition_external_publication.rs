//! Run 333 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 329/330 guarded epoch-transition external-publication boundary: the boundary
//! that consumes a **verified** Run 331/332 non-mutating guarded
//! epoch-transition external-publication *decision* (the accepted
//! [`ProductionLiveEpochTransitionDurableAuditPublicationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionDurableAuditPublicationArtifact`], as
...
//! mutation record does it authorize?", Run 333 answers the next question:
//! "given a verified guarded external-publication decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a durable-audit-publication
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 333)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 334**.
//! * The default policy is
//!   [`ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::Disabled`]
//!   and fails closed **before** any binding, validator-set binding, or
//!   handoff-package construction.
//! * Only a **verified** Run 331/332 guarded epoch-transition
//!   external-publication decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionDurableAuditPublicationArtifact`] can authorize a runtime
...
//!   [`ProductionLiveEpochTransitionExternalPublicationArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_333.md`.
...
use crate::pqc_production_live_epoch_transition_durable_audit_publication::{
...
/// Run 333 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_PROTOCOL_VERSION: u16 = 1;

/// Run 333 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:333-epoch-transition-durable-audit-publication-intent:v1";

/// Run 333 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_ID_DOMAIN_TAG: &str =
    "QBIND:333-epoch-transition-durable-audit-publication-id:v1";

/// Run 333 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:333-epoch-transition-durable-audit-publication-request:v1";

/// Run 333 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:333-epoch-transition-durable-audit-publication-transcript:v1";
...
/// Run 333 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionExternalPublicationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionExternalPublicationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionExternalPublicationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionExternalPublication` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 331/332 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionExternalPublication` and
/// `MainnetProductionLiveEpochTransitionExternalPublicationRequired` are
...
pub enum ProductionLiveEpochTransitionExternalPublicationExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionExternalPublication,
    ...
    RequireProductionLiveEpochTransitionExternalPublication,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionExternalPublicationRequired,
}

impl ProductionLiveEpochTransitionExternalPublicationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionExternalPublication => {
                "allow-source-test-epoch-transition-durable-audit-publication"
            }
            Self::RequireProductionLiveEpochTransitionExternalPublication => {
                "require-production-epoch-transition-durable-audit-publication"
            }
            Self::MainnetProductionLiveEpochTransitionExternalPublicationRequired => {
                "mainnet-production-epoch-transition-durable-audit-publication-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionExternalPublication` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionExternalPublication` kind is fail-closed as
/// unavailable in Run 333 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionExternalPublicationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionExternalPublication,
    /// Reserved production authorization kind. Fail-closed in Run 333.
    ProductionLiveEpochTransitionExternalPublication,
}

impl ProductionLiveEpochTransitionExternalPublicationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionExternalPublication => {
                "source-test-epoch-transition-durable-audit-publication"
            }
            Self::ProductionLiveEpochTransitionExternalPublication => {
                "production-epoch-transition-durable-audit-publication"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionExternalPublicationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionExternalPublicationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionExternalPublicationExecutorKind,
}

impl ProductionLiveEpochTransitionExternalPublicationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionExternalPublicationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionExternalPublicationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionExternalPublicationExecutorKind::SourceTestLiveEpochTransitionExternalPublication,
        )
    }
...
impl Default for ProductionLiveEpochTransitionExternalPublicationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionExternalPublicationExecutorKind::Disabled)
    }
}
...
/// Run 333 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 331/332
/// [`LiveEpochTransitionDurableAuditPublicationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionExternalPublicationKind {
...
impl LiveEpochTransitionExternalPublicationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 333 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 331/332 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionDurableAuditPublicationKind,
    ) -> Self {
        use LiveEpochTransitionDurableAuditPublicationKind as A;
        match kind {
...
/// Run 333 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedDurableAuditPublicationDecision`] carrying a Run 331/332
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionExternalPublicationAuthoritySource {
    /// A verified Run 331/332 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedDurableAuditPublicationDecision {
        decision: ProductionLiveEpochTransitionDurableAuditPublicationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingDurableAuditPublicationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedDurableAuditPublicationDecision {
        decision: ProductionLiveEpochTransitionDurableAuditPublicationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedDurableAuditPublicationWithoutPackage {
        decision: ProductionLiveEpochTransitionDurableAuditPublicationDecision,
    },
    /// A Run 329/330 epoch-transition commit-authorization decision presented
    /// directly, without a Run 331/332 durable-audit-publication decision. Rejected.
    CommitAuthorizationDecisionWithoutDurableAuditPublication,
    /// A Run 329/330 epoch-transition mutation-execution decision presented
    /// directly, without a Run 331/332 durable-audit-publication decision. Rejected.
    MutationExecutionDecisionWithoutDurableAuditPublication,
    /// A Run 329/330 epoch-transition execution-preparation decision presented
    /// directly, without a Run 331/332 durable-audit-publication decision. Rejected.
    ExecutionPreparationDecisionWithoutDurableAuditPublication,
    /// A Run 329/330 epoch-transition runtime handoff decision presented
    /// directly, without a Run 331/332 durable-audit-publication decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutDurableAuditPublication,
    /// A Run 329/330 guarded epoch-transition external-publication decision
    /// presented directly, without a Run 331/332 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutDurableAuditPublication,
    /// A Run 327/328 staged-application decision presented directly, without a
    /// Run 331/332 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutDurableAuditPublication,
    /// A Run 325/326 live-application authorization presented directly, without
    /// a Run 331/332 guarded external-publication decision. Rejected.
    LiveApplicationAuthorizationWithoutDurableAuditPublication,
    /// A Run 323/324 validator-set rotation *application decision* presented
    /// directly, without a Run 331/332 guarded external-publication decision.
    /// Rejected.
    ApplicationDecisionWithoutDurableAuditPublication,
    /// A Run 321/322 validator-set rotation plan presented directly, without a
    /// Run 331/332 live-application authorization. Rejected.
    RotationPlanWithoutDurableAuditPublication,
    /// A Run 319/320 governance execution intent presented directly, without a
    /// Run 331/332 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutDurableAuditPublication,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 325/326 live-application authorization. Rejected.
    GovernanceProofWithoutDurableAuditPublication,
    ...
    FixtureOnlyDurableAuditPublication,
...
/// Run 333 — the explicit trusted inputs the executor binds a verified
/// Run 331/332 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExternalPublicationInputs {
    ...
    pub external_publication_policy_id: String,
    /// Expected Run 331/332 live-application authorization policy id (bound
    ...
    /// Expected Run 331/332 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 331/332 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 331/332 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 331/332 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 331/332 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 331/332 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 331/332 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 331/332 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 331/332 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 331/332 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 329/330 guarded external-publication decision id (re-exposed
    /// by the consumed Run 331/332 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 329/330 guarded external-publication request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 329/330 guarded external-publication intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 329/330 guarded external-publication transcript digest.
    ...
    /// Expected Run 331/332 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_durable_audit_publication_decision_id: String,
    /// Expected Run 331/332 runtime handoff request id.
    pub expected_durable_audit_publication_request_id: String,
    /// Expected Run 331/332 runtime handoff package/content digest (the
    /// consumed decision's `external_publication_digest`, i.e. the handoff content
    /// digest).
    pub expected_durable_audit_publication_intent_digest: String,
    /// Expected Run 331/332 runtime handoff transcript digest.
    pub expected_durable_audit_publication_transcript_digest: String,
    /// Expected Run 331/332 durable-audit-publication nonce (re-exposed by the
    /// consumed Run 331/332 durable-audit-publication artifact).
    pub expected_durable_audit_publication_nonce: u64,
    /// Expected Run 329/330 commit-authorization decision id (re-exposed by
    /// the consumed Run 331/332 durable-audit-publication artifact).
    ...
    /// Run 331/332 durable-audit-publication artifact).
    ...
    /// wall-clock value and never a reject path in Run 333.
...
impl ProductionLiveEpochTransitionExternalPublicationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.external_publication_policy_id.is_empty()
            ...
            && !self.expected_durable_audit_publication_decision_id.is_empty()
            && !self.expected_durable_audit_publication_request_id.is_empty()
            && !self.expected_durable_audit_publication_intent_digest.is_empty()
            && !self.expected_durable_audit_publication_transcript_digest.is_empty()
...
/// Run 333 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition external-publication decision), the explicit
/// epoch-transition target, a durable-audit-publication nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExternalPublicationRequest {
    pub authority_source: LiveEpochTransitionExternalPublicationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The durable-audit-publication nonce (idempotency / replay binding).
    pub external_publication_nonce: u64,
...
impl ProductionLiveEpochTransitionExternalPublicationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and durable-audit-publication nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionExternalPublicationAuthoritySource,
        proposed_epoch_transition_target: u64,
        external_publication_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
            proposed_epoch_transition_target,
            external_publication_nonce,
...
/// Run 333 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionExternalPublicationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionExternalPublicationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionExternalPublicationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionExternalPublicationReplaySet;

impl LiveEpochTransitionExternalPublicationReplaySet
    for EmptyLiveEpochTransitionExternalPublicationReplaySet
{
...
/// Run 333 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 334+); Run 333 never
...
/// preconditions, and carries deterministic `external_publication_id`, `request_id`,
/// `external_publication_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExternalPublicationArtifact {
    pub staged_kind: LiveEpochTransitionExternalPublicationKind,
    pub protocol_version: u16,
    pub external_publication_policy_id: String,

    // ---- Re-exposed Run 331/332 authorization intent tuple ------------
    ...
    // ---- Bound Run 331/332 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 331/332 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 331/332 guarded external-publication authority tuple -
    // (the consumed guarded external-publication decision transcript)
    ...
    // ---- Guarded external-publication binding (re-exposed consumed nonce)
    /// The re-exposed Run 329/330 guarded external-publication nonce.
    ...
    pub durable_audit_publication_decision_id: String,
    pub durable_audit_publication_request_id: String,
    pub durable_audit_publication_intent_digest: String,
    pub durable_audit_publication_transcript_digest: String,
    /// The re-exposed Run 331/332 durable-audit-publication nonce (consumed).
    pub durable_audit_publication_nonce: u64,

    // ---- Bound Run 329/330 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 331/332 durable-audit-publication artifact)
    ...
    /// The newly proposed external-publication nonce.
    pub external_publication_nonce: u64,
    ...
    pub external_publication_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionExternalPublicationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`external_publication_id`,
    /// `request_id`, `external_publication_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"external_publication_policy_id",
            self.external_publication_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"durable_audit_publication_decision_id",
            self.durable_audit_publication_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_publication_request_id",
            self.durable_audit_publication_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_publication_intent_digest",
            self.durable_audit_publication_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"durable_audit_publication_transcript_digest",
            self.durable_audit_publication_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"external_publication_nonce",
            &self.external_publication_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 333 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_external_publication_content_digest(
    package: &ProductionLiveEpochTransitionExternalPublicationArtifact,
) -> String {
    package.content_digest()
}

/// Run 333 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and durable-audit-publication nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_external_publication_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    external_publication_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"external_publication_policy_id",
        external_publication_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"external_publication_nonce",
        &external_publication_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 333 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and durable-audit-publication nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_external_publication_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    external_publication_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"external_publication_policy_id", external_publication_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 333 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_external_publication_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_EXTERNAL_PUBLICATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 333 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionExternalPublication`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionExternalPublicationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionExternalPublicationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionExternalPublicationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionExternalPublicationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition external-publication
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionExternalPublication {
        execution_kind: LiveEpochTransitionExternalPublicationKind,
    ...
    VerifiedDurableAuditPublicationDecisionRequired,
    UnverifiedDurableAuditPublicationDecisionRejected,
    ...
    DurableAuditPublicationDecisionIdMismatch,
    DurableAuditPublicationDecisionRequestIdMismatch,
    DurableAuditPublicationDecisionIntentDigestMismatch,
    DurableAuditPublicationDecisionTranscriptMismatch,
    DurableAuditPublicationDecisionIntegrityMismatch,
    WrongDurableAuditPublicationNonce,
    ...
    LiveEpochTransitionExternalPublicationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionExternalPublicationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionExternalPublicationBoundaryUnavailable => {
                "live-epoch-transition-external-publication-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionExternalPublicationUnavailable => {
                "production-live-epoch-transition-external-publication-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionExternalPublicationUnavailable => {
                "mainnet-production-live-epoch-transition-external-publication-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionExternalPublication { .. } => {
                "accepted-source-test-live-epoch-transition-external-publication"
            }
            Self::VerifiedDurableAuditPublicationDecisionRequired => {
                "verified-durable-audit-publication-decision-required"
            }
            Self::UnverifiedDurableAuditPublicationDecisionRejected => {
                "unverified-durable-audit-publication-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-durable-audit-publication-decision-rejected-as-production-authority"
            }
            ...
            Self::DurableAuditPublicationDecisionIdMismatch => "durable-audit-publication-decision-id-mismatch",
            Self::DurableAuditPublicationDecisionRequestIdMismatch => {
                "durable-audit-publication-decision-request-id-mismatch"
            }
            Self::DurableAuditPublicationDecisionIntentDigestMismatch => {
                "durable-audit-publication-decision-intent-digest-mismatch"
            }
            Self::DurableAuditPublicationDecisionTranscriptMismatch => {
                "durable-audit-publication-decision-transcript-mismatch"
            }
            Self::DurableAuditPublicationDecisionIntegrityMismatch => {
                "durable-audit-publication-decision-integrity-mismatch"
            }
            Self::WrongDurableAuditPublicationNonce => "wrong-durable-audit-publication-nonce",
            ...
            Self::LiveEpochTransitionExternalPublicationAmbiguous { .. } => {
                "epoch-transition-durable-audit-publication-ambiguous"
            }
...
/// Run 333 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExternalPublicationDecision {
    pub outcome: ProductionLiveEpochTransitionExternalPublicationOutcome,
    pub external_publication_id: String,
    pub request_id: String,
    pub external_publication_artifact:
        Option<ProductionLiveEpochTransitionExternalPublicationArtifact>,
    pub external_publication_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionExternalPublicationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.external_publication_artifact.is_some()
    }
}
...
/// Run 333 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome {
...
/// Run 333 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionExternalPublicationExecutor {
    pub config: ProductionLiveEpochTransitionExternalPublicationConfig,
    pub policy: ProductionLiveEpochTransitionExternalPublicationExecutorPolicy,
}

impl ProductionLiveEpochTransitionExternalPublicationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionExternalPublicationConfig,
        policy: ProductionLiveEpochTransitionExternalPublicationExecutorPolicy,
    ) -> Self {
        Self { config, policy }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionExternalPublicationConfig::source_test(),
            ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::AllowSourceTestLiveEpochTransitionExternalPublication,
        )
    }
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionExternalPublicationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionDurableAuditPublicationDecision,
            &'a ProductionLiveEpochTransitionDurableAuditPublicationArtifact,
        ),
        ProductionLiveEpochTransitionExternalPublicationOutcome,
    > {
        use ProductionLiveEpochTransitionExternalPublicationOutcome as O;
        use LiveEpochTransitionExternalPublicationAuthoritySource as S;
        match source {
            S::VerifiedDurableAuditPublicationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedDurableAuditPublicationDecisionRejected);
                }
                match &decision.durable_audit_publication_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedDurableAuditPublicationDecisionRequired),
                }
            }
            S::MissingDurableAuditPublicationDecision => {
                Err(O::VerifiedDurableAuditPublicationDecisionRequired)
            }
            S::UnverifiedDurableAuditPublicationDecision { .. } => {
                Err(O::UnverifiedDurableAuditPublicationDecisionRejected)
            }
            S::AcceptedDurableAuditPublicationWithoutPackage { .. } => {
                Err(O::VerifiedDurableAuditPublicationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutDurableAuditPublication => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutDurableAuditPublication => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutDurableAuditPublication => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutDurableAuditPublication => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutDurableAuditPublication => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutDurableAuditPublication => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutDurableAuditPublication => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutDurableAuditPublication => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutDurableAuditPublication => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutDurableAuditPublication => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutDurableAuditPublication => {
            ...
            S::FixtureOnlyDurableAuditPublication => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionExternalPublicationOutcome> {
        use ProductionLiveEpochTransitionExternalPublicationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionExternalPublicationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::MainnetProductionLiveEpochTransitionExternalPublicationRequired => {
                    O::MainNetProductionLiveEpochTransitionExternalPublicationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionExternalPublicationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionExternalPublicationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 333.
        if self.config.kind
            == ProductionLiveEpochTransitionExternalPublicationExecutorKind::ProductionLiveEpochTransitionExternalPublication
        {
            return Some(O::LiveEpochTransitionExternalPublicationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 331/332 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionDurableAuditPublicationDecision,
        intent: &ProductionLiveEpochTransitionDurableAuditPublicationArtifact,
        inputs: &ProductionLiveEpochTransitionExternalPublicationInputs,
    ) -> Option<ProductionLiveEpochTransitionExternalPublicationOutcome> {
        use ProductionLiveEpochTransitionExternalPublicationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 331/332 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.durable_audit_publication_id != inputs.expected_durable_audit_publication_decision_id {
            return Some(O::DurableAuditPublicationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_durable_audit_publication_request_id {
            return Some(O::DurableAuditPublicationDecisionRequestIdMismatch);
        }
        if decision.durable_audit_publication_digest != inputs.expected_durable_audit_publication_intent_digest {
            return Some(O::DurableAuditPublicationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_durable_audit_publication_transcript_digest {
            return Some(O::DurableAuditPublicationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.durable_audit_publication_digest {
            return Some(O::DurableAuditPublicationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed durable-audit-publication
        // nonce binding.
        if intent.durable_audit_publication_nonce != inputs.expected_durable_audit_publication_nonce {
            return Some(O::WrongDurableAuditPublicationNonce);
        }

        // Re-exposed Run 329/330 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 331/332
        // durable-audit-publication artifact).
        ...
        // authority tuple binding (carried through the consumed Run 331/332
        // durable-audit-publication artifact).
        ...
        // Re-exposed Run 329/330 guarded epoch-transition external-publication
        ...
        // Re-exposed Run 331/332 staged-application decision authority tuple
        ...
        // Re-exposed Run 331/332 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionExternalPublicationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionExternalPublicationRequest,
        inputs: &ProductionLiveEpochTransitionExternalPublicationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_external_publication_request_id(
            self.config.protocol_version.0,
            &decision.durable_audit_publication_digest,
            &inputs.external_publication_policy_id,
            request.proposed_epoch_transition_target,
            request.external_publication_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionExternalPublicationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionExternalPublicationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            external_publication_policy_id: inputs.external_publication_policy_id.clone(),
            ...
            // Re-exposed Run 329/330 guarded external-publication decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded external-publication nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 331/332 runtime handoff decision transcript (the
            // authority source this run consumes).
            durable_audit_publication_decision_id: decision.durable_audit_publication_id.clone(),
            durable_audit_publication_request_id: decision.request_id.clone(),
            durable_audit_publication_intent_digest: decision.durable_audit_publication_digest.clone(),
            durable_audit_publication_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed durable-audit-publication nonce.
            durable_audit_publication_nonce: application_intent.durable_audit_publication_nonce,
            // Re-exposed Run 329/330 commit-authorization decision authority
            // tuple (carried through the consumed durable-audit-publication artifact).
            ...
            // (carried through the consumed durable-audit-publication artifact).
            ...
            // Newly proposed external-publication nonce.
            external_publication_nonce: request.external_publication_nonce,
            // Exact future-executor preconditions.
            ...
            external_publication_id: String::new(),
            request_id: String::new(),
            external_publication_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionExternalPublication {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            external_publication_nonce: request.external_publication_nonce,
        };
        let external_publication_id = production_live_epoch_transition_external_publication_id(
            self.config.protocol_version.0,
            &decision.durable_audit_publication_digest,
            &inputs.external_publication_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_external_publication_request_id(
            self.config.protocol_version.0,
            &decision.durable_audit_publication_digest,
            &inputs.external_publication_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.external_publication_digest = record.content_digest();
        record.external_publication_id = external_publication_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_external_publication_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.external_publication_digest,
                outcome.tag(),
            );
    ...
    /// Run 333 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_external_publication<
        R: LiveEpochTransitionExternalPublicationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionExternalPublicationRequest,
        inputs: &ProductionLiveEpochTransitionExternalPublicationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionExternalPublicationDecision {
                outcome,
                external_publication_id: pkg.external_publication_id.clone(),
                request_id: pkg.request_id.clone(),
                external_publication_artifact: record.clone(),
                external_publication_digest: pkg.external_publication_digest.clone(),
        ...
        let durable_audit_publication_intent_digest = match &request.authority_source {
            LiveEpochTransitionExternalPublicationAuthoritySource::VerifiedDurableAuditPublicationDecision {
                decision,
            }
            | LiveEpochTransitionExternalPublicationAuthoritySource::UnverifiedDurableAuditPublicationDecision {
                decision,
            }
            | LiveEpochTransitionExternalPublicationAuthoritySource::AcceptedDurableAuditPublicationWithoutPackage {
                decision,
            } => decision.durable_audit_publication_digest.clone(),
            _ => String::new(),
        };

        let external_publication_id = production_live_epoch_transition_external_publication_id(
            self.config.protocol_version.0,
            &durable_audit_publication_intent_digest,
            &inputs.external_publication_policy_id,
            request.proposed_epoch_transition_target,
            request.external_publication_nonce,
        );
        ...
        let external_publication_digest = String::new();
        ...
        ProductionLiveEpochTransitionExternalPublicationDecision {
            ...
            request_id,
            external_publication_artifact: None,
            external_publication_digest,
            transcript_digest,
        }
    }

    /// Run 333 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_external_publication_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionExternalPublicationArtifact>,
        current: &ProductionLiveEpochTransitionExternalPublicationArtifact,
    ) -> ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome {
        use ProductionLiveEpochTransitionExternalPublicationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionExternalPublicationExecutorKind::Disabled
        {
...
/// Run 333 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_external_publication_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::default()
        == ProductionLiveEpochTransitionExternalPublicationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionExternalPublicationConfig::default().kind
            == ProductionLiveEpochTransitionExternalPublicationExecutorKind::Disabled
}

/// Run 333 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 334).
pub fn production_live_epoch_transition_external_publication_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 333 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_external_publication_executor_mainnet_refused() -> bool {
    true
}

/// Run 333 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_external_publication_executor_is_non_mutating() -> bool {
    true
}

/// Run 333 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_external_publication_executor_never_falls_back() -> bool {
    true
}

/// Run 333 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_external_publication_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 333 — the executor only requires a verified Run 331/332 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_external_publication_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 333 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 333. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionExternalPublicationFixtureState {
...
impl LiveEpochTransitionExternalPublicationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, external_publication_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == external_publication_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `external_publication_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionExternalPublicationArtifact,
        external_publication_id: &str,
    ) -> bool {
        if self.has_applied(external_publication_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(external_publication_id.to_string());
        true
    }
}