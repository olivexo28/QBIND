//! Run 319 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 315/316 guarded epoch-transition commit-authorization boundary: the boundary
//! that consumes a **verified** Run 317/318 non-mutating guarded
//! epoch-transition commit-authorization *decision* (the accepted
//! [`ProductionLiveEpochTransitionMutationExecutionDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionMutationExecutionArtifact`], as
//! release-binary-evidenced by Run 316) and translates it into a typed,
...
//!
//! Where the Run 315 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 319 answers the next question:
//! "given a verified guarded commit-authorization decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a mutation-execution
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 319)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 320**.
...
//! * Only a **verified** Run 317/318 guarded epoch-transition
//!   commit-authorization decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionMutationExecutionArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 313/314 staged-application decision alone, a Run
//!   311/312 live-application authorization alone, a Run 309/310 application
//!   decision alone, Run 307 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionCommitAuthorizationArtifact`]; it
...
use crate::pqc_production_live_epoch_transition_mutation_execution::{
...
/// Run 319 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_PROTOCOL_VERSION: u16 = 1;

/// Run 319 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:319-epoch-transition-mutation-execution-intent:v1";

/// Run 319 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_ID_DOMAIN_TAG: &str =
    "QBIND:319-epoch-transition-mutation-execution-id:v1";

/// Run 319 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:319-epoch-transition-mutation-execution-request:v1";

/// Run 319 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:319-epoch-transition-mutation-execution-transcript:v1";
...
/// Run 319 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionCommitAuthorizationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionCommitAuthorizationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionCommitAuthorization` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 317/318 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionCommitAuthorization` and
/// `MainnetProductionLiveEpochTransitionCommitAuthorizationRequired` are
...
pub enum ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionCommitAuthorization,
    ...
    RequireProductionLiveEpochTransitionCommitAuthorization,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionCommitAuthorizationRequired,
}

impl ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionCommitAuthorization => {
                "allow-source-test-epoch-transition-mutation-execution"
            }
            Self::RequireProductionLiveEpochTransitionCommitAuthorization => {
                "require-production-epoch-transition-mutation-execution"
            }
            Self::MainnetProductionLiveEpochTransitionCommitAuthorizationRequired => {
                "mainnet-production-epoch-transition-mutation-execution-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionCommitAuthorization` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionCommitAuthorization` kind is fail-closed as
/// unavailable in Run 319 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionCommitAuthorizationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionCommitAuthorization,
    /// Reserved production authorization kind. Fail-closed in Run 319.
    ProductionLiveEpochTransitionCommitAuthorization,
}

impl ProductionLiveEpochTransitionCommitAuthorizationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionCommitAuthorization => {
                "source-test-epoch-transition-mutation-execution"
            }
            Self::ProductionLiveEpochTransitionCommitAuthorization => {
                "production-epoch-transition-mutation-execution"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionCommitAuthorizationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionCommitAuthorizationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionCommitAuthorizationExecutorKind,
}

impl ProductionLiveEpochTransitionCommitAuthorizationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionCommitAuthorizationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionCommitAuthorizationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::SourceTestLiveEpochTransitionCommitAuthorization,
        )
    }
...
impl Default for ProductionLiveEpochTransitionCommitAuthorizationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::Disabled)
    }
}
...
/// Run 319 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 317/318
/// [`LiveEpochTransitionMutationExecutionKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionCommitAuthorizationKind {
...
impl LiveEpochTransitionCommitAuthorizationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 319 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 317/318 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionMutationExecutionKind,
    ) -> Self {
        use LiveEpochTransitionMutationExecutionKind as A;
        match kind {
...
/// Run 319 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedMutationExecutionDecision`] carrying a Run 317/318
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionCommitAuthorizationAuthoritySource {
    /// A verified Run 317/318 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedMutationExecutionDecision {
        decision: ProductionLiveEpochTransitionMutationExecutionDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingMutationExecutionDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedMutationExecutionDecision {
        decision: ProductionLiveEpochTransitionMutationExecutionDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedMutationExecutionWithoutPackage {
        decision: ProductionLiveEpochTransitionMutationExecutionDecision,
    },
    /// A Run 315/316 epoch-transition execution-preparation decision presented
    /// directly, without a Run 317/318 mutation-execution decision. Rejected.
    ExecutionPreparationDecisionWithoutMutationExecution,
    /// A Run 315/316 epoch-transition runtime handoff decision presented
    /// directly, without a Run 317/318 mutation-execution decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutMutationExecution,
    /// A Run 315/316 guarded epoch-transition commit-authorization decision
    /// presented directly, without a Run 317/318 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutMutationExecution,
    /// A Run 313/314 staged-application decision presented directly, without a
    /// Run 317/318 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutMutationExecution,
    /// A Run 311/312 live-application authorization presented directly, without
    /// a Run 317/318 guarded commit-authorization decision. Rejected.
    LiveApplicationAuthorizationWithoutMutationExecution,
    /// A Run 309/310 validator-set rotation *application decision* presented
    /// directly, without a Run 317/318 guarded commit-authorization decision.
    /// Rejected.
    ApplicationDecisionWithoutMutationExecution,
    /// A Run 307/308 validator-set rotation plan presented directly, without a
    /// Run 317/318 live-application authorization. Rejected.
    RotationPlanWithoutMutationExecution,
    /// A Run 305/306 governance execution intent presented directly, without a
    /// Run 317/318 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutMutationExecution,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 311/312 live-application authorization. Rejected.
    GovernanceProofWithoutMutationExecution,
...
/// Run 319 — the explicit trusted inputs the executor binds a verified
/// Run 317/318 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationInputs {
    ...
    pub commit_authorization_policy_id: String,
    /// Expected Run 317/318 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 309/310 application policy id (re-exposed by the
    ...
    /// Expected Run 305 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 305 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 305 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 307 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 307 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 307 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 307 rotation plan digest.
    ...
    /// Expected Run 309/310 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 309/310 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 309/310 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 309/310 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 317/318 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 317/318 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 317/318 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 317/318 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 317/318 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 317/318 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 317/318 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 317/318 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 317/318 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 309/310 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 317/318 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 315/316 guarded commit-authorization decision id (re-exposed
    /// by the consumed Run 317/318 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 315/316 guarded commit-authorization request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 315/316 guarded commit-authorization intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 315/316 guarded commit-authorization transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 315/316 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 317/318 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 317/318 runtime handoff request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 317/318 runtime handoff package/content digest (the
    /// consumed decision's `commit_authorization_digest`, i.e. the handoff content
    /// digest).
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 317/318 runtime handoff transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 317/318 mutation-execution nonce (re-exposed by the
    /// consumed Run 317/318 mutation-execution artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 315/316 execution-preparation decision id (re-exposed by
    /// the consumed Run 317/318 mutation-execution artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 315/316 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 315/316 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 315/316 execution-preparation transcript digest.
    ...
    /// Expected Run 315/316 runtime handoff decision id (re-exposed by the
    /// consumed Run 317/318 mutation-execution artifact).
    ...
    /// Expected Run 315/316 runtime handoff package/content digest.
    ...
    /// Expected Run 315/316 runtime handoff nonce (re-exposed by the consumed
    /// Run 317/318 mutation-execution artifact).
    ...
    /// wall-clock value and never a reject path in Run 319.
...
impl ProductionLiveEpochTransitionCommitAuthorizationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.commit_authorization_policy_id.is_empty()
...
/// Run 319 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition commit-authorization decision), the explicit
/// epoch-transition target, a mutation-execution nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationRequest {
    pub authority_source: LiveEpochTransitionCommitAuthorizationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The mutation-execution nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionCommitAuthorizationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and mutation-execution nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionCommitAuthorizationAuthoritySource,
        proposed_epoch_transition_target: u64,
        commit_authorization_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 319 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionCommitAuthorizationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionCommitAuthorizationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionCommitAuthorizationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionCommitAuthorizationReplaySet;

impl LiveEpochTransitionCommitAuthorizationReplaySet
    for EmptyLiveEpochTransitionCommitAuthorizationReplaySet
{
...
/// Run 319 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 320+); Run 319 never
...
/// preconditions, and carries deterministic `commit_authorization_id`, `request_id`,
/// `commit_authorization_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationArtifact {
    pub staged_kind: LiveEpochTransitionCommitAuthorizationKind,
    pub protocol_version: u16,
    pub commit_authorization_policy_id: String,

    // ---- Re-exposed Run 317/318 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 309/310 application-decision tuple ------------
    ...
    // ---- Bound Run 317/318 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 317/318 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 317/318 guarded commit-authorization authority tuple -
    // (the consumed guarded commit-authorization decision transcript)
    ...
    // ---- Guarded commit-authorization binding (re-exposed consumed nonce)
    /// The re-exposed Run 315/316 guarded commit-authorization nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 317/318 runtime handoff decision authority tuple ----
    ...
    // ---- Bound Run 315/316 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 317/318 mutation-execution artifact)
    ...
    /// The re-exposed Run 315/316 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed commit-authorization nonce.
    pub commit_authorization_nonce: u64,
    ...
    pub commit_authorization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionCommitAuthorizationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`commit_authorization_id`,
    /// `request_id`, `commit_authorization_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"commit_authorization_policy_id",
            self.commit_authorization_policy_id.as_bytes(),
        );
...
/// Custody binding canonical hashing (module-local; mirrors Run 305/307/309
...
/// Run 319 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_commit_authorization_content_digest(
    package: &ProductionLiveEpochTransitionCommitAuthorizationArtifact,
) -> String {
    package.content_digest()
}

/// Run 319 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and mutation-execution nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_commit_authorization_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    commit_authorization_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"commit_authorization_policy_id",
        commit_authorization_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"commit_authorization_nonce",
        &commit_authorization_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 319 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and mutation-execution nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_commit_authorization_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    commit_authorization_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"commit_authorization_policy_id", commit_authorization_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 319 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_commit_authorization_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 319 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionCommitAuthorization`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionCommitAuthorizationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionCommitAuthorizationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionCommitAuthorizationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionCommitAuthorizationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition commit-authorization
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionCommitAuthorization {
        execution_kind: LiveEpochTransitionCommitAuthorizationKind,
    ...
    VerifiedMutationExecutionDecisionRequired,
    UnverifiedMutationExecutionDecisionRejected,
    ...
    MutationExecutionDecisionIntegrityMismatch,
    ...
    LiveEpochTransitionCommitAuthorizationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionCommitAuthorizationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionCommitAuthorizationBoundaryUnavailable => {
                "live-epoch-transition-commit-authorization-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionCommitAuthorizationUnavailable => {
                "production-live-epoch-transition-commit-authorization-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionCommitAuthorizationUnavailable => {
                "mainnet-production-live-epoch-transition-commit-authorization-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionCommitAuthorization { .. } => {
                "accepted-source-test-live-epoch-transition-commit-authorization"
            }
            Self::VerifiedMutationExecutionDecisionRequired => {
                "verified-mutation-execution-decision-required"
            }
            Self::UnverifiedMutationExecutionDecisionRejected => {
                "unverified-mutation-execution-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-mutation-execution-decision-rejected-as-production-authority"
            }
            ...
            Self::MutationExecutionDecisionIntegrityMismatch => {
                "mutation-execution-decision-integrity-mismatch"
            }
            ...
            Self::LiveEpochTransitionCommitAuthorizationAmbiguous { .. } => {
                "epoch-transition-mutation-execution-ambiguous"
            }
...
/// Run 319 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationDecision {
    pub outcome: ProductionLiveEpochTransitionCommitAuthorizationOutcome,
    pub commit_authorization_id: String,
    pub request_id: String,
    pub commit_authorization_artifact:
        Option<ProductionLiveEpochTransitionCommitAuthorizationArtifact>,
    pub commit_authorization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionCommitAuthorizationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.commit_authorization_artifact.is_some()
    }
}
...
/// Run 319 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome {
...
/// Run 319 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitAuthorizationExecutor {
    pub config: ProductionLiveEpochTransitionCommitAuthorizationConfig,
    pub policy: ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy,
}

impl ProductionLiveEpochTransitionCommitAuthorizationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionCommitAuthorizationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionCommitAuthorizationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionMutationExecutionDecision,
            &'a ProductionLiveEpochTransitionMutationExecutionArtifact,
        ),
        ProductionLiveEpochTransitionCommitAuthorizationOutcome,
    > {
        use ProductionLiveEpochTransitionCommitAuthorizationOutcome as O;
        use LiveEpochTransitionCommitAuthorizationAuthoritySource as S;
        match source {
            S::VerifiedMutationExecutionDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedMutationExecutionDecisionRejected);
                }
                match &decision.execution_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedMutationExecutionDecisionRequired),
                }
            }
            S::MissingMutationExecutionDecision => {
                Err(O::VerifiedMutationExecutionDecisionRequired)
            }
            S::UnverifiedMutationExecutionDecision { .. } => {
                Err(O::UnverifiedMutationExecutionDecisionRejected)
            }
            S::AcceptedMutationExecutionWithoutPackage { .. } => {
                Err(O::VerifiedMutationExecutionDecisionRequired)
            }
            S::ExecutionPreparationDecisionWithoutMutationExecution => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutMutationExecution => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutMutationExecution => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutMutationExecution => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutMutationExecution => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutMutationExecution => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutMutationExecution => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutMutationExecution => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutMutationExecution => {
            ...
            S::FixtureOnlyMutationExecution => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionCommitAuthorizationOutcome> {
        use ProductionLiveEpochTransitionCommitAuthorizationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy::MainnetProductionLiveEpochTransitionCommitAuthorizationRequired => {
                    O::MainNetProductionLiveEpochTransitionCommitAuthorizationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionCommitAuthorizationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionCommitAuthorizationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 319.
        if self.config.kind
            == ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::ProductionLiveEpochTransitionCommitAuthorization
        {
            return Some(O::LiveEpochTransitionCommitAuthorizationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 317/318 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionMutationExecutionDecision,
        intent: &ProductionLiveEpochTransitionMutationExecutionArtifact,
        inputs: &ProductionLiveEpochTransitionCommitAuthorizationInputs,
    ) -> Option<ProductionLiveEpochTransitionCommitAuthorizationOutcome> {
        use ProductionLiveEpochTransitionCommitAuthorizationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 317/318 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.execution_id != inputs.expected_mutation_execution_decision_id {
            return Some(O::MutationExecutionDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_mutation_execution_request_id {
            return Some(O::MutationExecutionDecisionRequestIdMismatch);
        }
        if decision.execution_digest != inputs.expected_mutation_execution_intent_digest {
            return Some(O::MutationExecutionDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_mutation_execution_transcript_digest {
        ...
        if intent.content_digest() != decision.execution_digest {
            return Some(O::MutationExecutionDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed mutation-execution
        // nonce binding.
        if intent.mutation_execution_nonce != inputs.expected_mutation_execution_nonce {
            return Some(O::WrongMutationExecutionNonce);
        }

        // Re-exposed Run 315/316 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 317/318
        // mutation-execution artifact).
        ...
        // Re-exposed Run 315/316 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 317/318
        // mutation-execution artifact).
        ...
        // Re-exposed Run 315/316 guarded epoch-transition commit-authorization
        ...
        // Re-exposed Run 317/318 staged-application decision authority tuple
        ...
        // Re-exposed Run 317/318 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 309/310 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionCommitAuthorizationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionCommitAuthorizationRequest,
        inputs: &ProductionLiveEpochTransitionCommitAuthorizationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_commit_authorization_request_id(
            self.config.protocol_version.0,
            &decision.execution_digest,
            &inputs.commit_authorization_policy_id,
            request.proposed_epoch_transition_target,
            request.commit_authorization_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionCommitAuthorizationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionCommitAuthorizationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            commit_authorization_policy_id: inputs.commit_authorization_policy_id.clone(),
            ...
            // Re-exposed Run 315/316 guarded commit-authorization decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded commit-authorization nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 317/318 runtime handoff decision transcript (the
            // authority source this run consumes).
            mutation_execution_decision_id: decision.execution_id.clone(),
            mutation_execution_request_id: decision.request_id.clone(),
            mutation_execution_intent_digest: decision.execution_digest.clone(),
            mutation_execution_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed mutation-execution nonce.
            mutation_execution_nonce: application_intent.mutation_execution_nonce,
            // Re-exposed Run 315/316 execution-preparation decision authority
            // tuple (carried through the consumed mutation-execution artifact).
            ...
            // Re-exposed Run 315/316 runtime handoff decision authority tuple
            // (carried through the consumed mutation-execution artifact).
            ...
            // Newly proposed commit-authorization nonce.
            commit_authorization_nonce: request.commit_authorization_nonce,
            // Exact future-executor preconditions.
            ...
            commit_authorization_id: String::new(),
            request_id: String::new(),
            commit_authorization_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionCommitAuthorization {
        ...
        record.commit_authorization_digest = record.content_digest();
        record.commit_authorization_id = commit_authorization_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_commit_authorization_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.commit_authorization_digest,
                outcome.tag(),
            );
    ...
    /// Run 319 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_commit_authorization<
        R: LiveEpochTransitionCommitAuthorizationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionCommitAuthorizationRequest,
        inputs: &ProductionLiveEpochTransitionCommitAuthorizationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionCommitAuthorizationDecision {
                outcome,
                commit_authorization_id: pkg.commit_authorization_id.clone(),
                request_id: pkg.request_id.clone(),
                commit_authorization_artifact: record.clone(),
                commit_authorization_digest: pkg.commit_authorization_digest.clone(),
        ...
        let mutation_execution_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionCommitAuthorizationAuthoritySource::AcceptedMutationExecutionWithoutPackage {
                decision,
            } => decision.execution_digest.clone(),
            _ => String::new(),
        };
        ...
        let commit_authorization_digest = String::new();
        ...
        ProductionLiveEpochTransitionCommitAuthorizationDecision {
            ...
            request_id,
            commit_authorization_artifact: None,
            commit_authorization_digest,
            transcript_digest,
        }
    }

    /// Run 319 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_commit_authorization_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionCommitAuthorizationArtifact>,
        current: &ProductionLiveEpochTransitionCommitAuthorizationArtifact,
    ) -> ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome {
        use ProductionLiveEpochTransitionCommitAuthorizationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::Disabled
        {
...
/// Run 319 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_commit_authorization_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy::default()
        == ProductionLiveEpochTransitionCommitAuthorizationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionCommitAuthorizationConfig::default().kind
            == ProductionLiveEpochTransitionCommitAuthorizationExecutorKind::Disabled
}

/// Run 319 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 320).
pub fn production_live_epoch_transition_commit_authorization_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 319 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_commit_authorization_executor_mainnet_refused() -> bool {
    true
}

/// Run 319 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_commit_authorization_executor_is_non_mutating() -> bool {
    true
}

/// Run 319 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_commit_authorization_executor_never_falls_back() -> bool {
    true
}

/// Run 319 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_commit_authorization_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 319 — the executor only requires a verified Run 317/318 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_commit_authorization_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 319 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 319. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionCommitAuthorizationFixtureState {
...
impl LiveEpochTransitionCommitAuthorizationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, commit_authorization_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == commit_authorization_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `commit_authorization_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionCommitAuthorizationArtifact,
        commit_authorization_id: &str,
    ) -> bool {
        if self.has_applied(commit_authorization_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(commit_authorization_id.to_string());
        true
    }
}