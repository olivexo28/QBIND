...
impl ProductionLiveEpochTransitionCommitAuthorizationExecutorKind {
    ...
    pub const fn is_source_test(self) -> bool {
        matches!(self, Self::SourceTestLiveEpochTransitionCommitAuthorization)
    }
}
...
pub fn production_live_epoch_transition_commit_authorization_request_id(
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_REQUEST_DOMAIN_TAG.as_bytes());
...
pub fn production_live_epoch_transition_commit_authorization_id(
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_AUTHORIZATION_ID_DOMAIN_TAG.as_bytes());
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
        b"guarded_mutation_intent_digest",
...
pub enum ProductionLiveEpochTransitionCommitAuthorizationOutcome {
    ...
    LiveEpochTransitionCommitAuthorizationAmbiguous {
        reason: String,
    },
    MainNetRefused,
}
...
impl ProductionLiveEpochTransitionCommitAuthorizationDecision {
    ...
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only()
            && self.commit_authorization_artifact.is_some()
    }
}
...
impl ProductionLiveEpochTransitionCommitAuthorizationExecutor {
    ...
    > {
        ...
        match source {
            ...
            S::GovernanceProofWithoutMutationExecution => Err(O::GovernanceProofAloneRejected),
    ...
    fn evaluate_core<R: LiveEpochTransitionCommitAuthorizationReplaySet + ?Sized>(
        ...
        let staged_kind = LiveEpochTransitionCommitAuthorizationKind::from_staged_application_kind(
...
impl LiveEpochTransitionCommitAuthorizationFixtureState {
    ...
    pub fn has_applied(&self, commit_authorization_id: &str) -> bool {
        self.applied_execution_ids
            .iter()
            .any(|id| id == commit_authorization_id)
    }
    ...
    pub fn apply_prepared_execution(
        ...
        self.applied_execution_ids
            .push(commit_authorization_id.to_string());
        true
    }
}
