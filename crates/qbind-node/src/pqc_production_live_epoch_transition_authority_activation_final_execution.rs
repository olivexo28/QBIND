...
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig {
    ...
    pub protocol_version:
        ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion,
    /// The boundary kind.
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig {
    pub fn new(
        kind: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind,
    ) -> Self {
        Self {
            protocol_version:
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self
                .authority_activation_final_execution_policy_id
                .is_empty()
            ...
            && !self
                ...
                .is_empty()
            && !self
                .expected_authority_activation_execution_preparation_request_id
                .is_empty()
            && !self
                .expected_authority_activation_execution_preparation_intent_digest
                .is_empty()
            && !self
                .expected_authority_activation_execution_preparation_transcript_digest
                .is_empty()
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact {
    ...
    pub fn content_digest(&self) -> String {
        ...
        hash_field(
            ...
            self.authority_activation_final_execution_policy_id
                .as_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            ...
            self.authority_activation_execution_preparation_decision_id
                .as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_request_id",
            self.authority_activation_execution_preparation_request_id
                .as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_intent_digest",
            self.authority_activation_execution_preparation_intent_digest
                .as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_transcript_digest",
            self.authority_activation_execution_preparation_transcript_digest
                .as_bytes(),
        );
        hash_field(
            &mut h,
...
pub enum ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome {
    ...
    LiveEpochTransitionAuthorityActivationFinalExecutionAmbiguous {
        reason: String,
    },
    MainNetRefused,
}
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
    ...
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only()
            && self.authority_activation_final_execution_artifact.is_some()
    }
}
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor {
        ...
        match source {
            S::VerifiedAuthorityActivationExecutionPreparationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(
                        O::UnverifiedAuthorityActivationExecutionPreparationDecisionRejected,
                    );
                }
            ...
            S::RotationPlanWithoutAuthorityActivationExecutionPreparation => {
    ...
    fn preflight_gate(
        ...
        if self.policy.is_production() {
            return Some(
                O::ProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable,
            );
        }
        ...
        if !self.config.is_well_formed() || !inputs.is_well_formed() {
            return Some(
                O::LiveEpochTransitionAuthorityActivationFinalExecutionBoundaryUnavailable,
            );
        }
    ...
    fn check_application_binding(
        ...
        // transcript binding.
        if decision.authority_activation_execution_preparation_id
            != inputs.expected_authority_activation_execution_preparation_decision_id
        {
            return Some(O::AuthorityActivationExecutionPreparationDecisionIdMismatch);
        }
        if decision.request_id
            != inputs.expected_authority_activation_execution_preparation_request_id
        {
            return Some(O::AuthorityActivationExecutionPreparationDecisionRequestIdMismatch);
        }
        if decision.authority_activation_execution_preparation_digest
            != inputs.expected_authority_activation_execution_preparation_intent_digest
        {
            return Some(O::AuthorityActivationExecutionPreparationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest
            != inputs.expected_authority_activation_execution_preparation_transcript_digest
        {
        ...
        // nonce binding.
        if intent.authority_activation_execution_preparation_nonce
            != inputs.expected_authority_activation_execution_preparation_nonce
        {
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet + ?Sized>(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact {
            ...
            authority_activation_final_execution_policy_id: inputs
                .authority_activation_final_execution_policy_id
                .clone(),
            ...
            authority_activation_execution_preparation_decision_id: decision
            ...
            authority_activation_execution_preparation_intent_digest: decision
                .authority_activation_execution_preparation_digest
                .clone(),
            authority_activation_execution_preparation_transcript_digest: decision
                .transcript_digest
                .clone(),
            // Re-exposed consumed authority-activation-execution-preparation nonce.
            authority_activation_execution_preparation_nonce: application_intent
                .authority_activation_execution_preparation_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            ...
            authority_activation_final_execution_nonce: request
                .authority_activation_final_execution_nonce,
            // Exact future-executor preconditions.
        ...
        let authority_activation_final_execution_id =
    ...
    >(
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
                outcome,
                authority_activation_final_execution_id: pkg
                ...
                authority_activation_final_execution_digest: pkg
...
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_default_is_disabled(
) -> bool {
...
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_mainnet_refused(
) -> bool {
    true
}
...
/// non-mutating.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_is_non_mutating(
) -> bool {
    true
}
...
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_never_falls_back(
) -> bool {
    true
}
...
impl LiveEpochTransitionAuthorityActivationFinalExecutionFixtureState {
    ...
    pub fn has_applied(&self, authority_activation_final_execution_id: &str) -> bool {
        self.applied_execution_ids
            .iter()
            .any(|id| id == authority_activation_final_execution_id)
    }
    ...
    pub fn apply_prepared_execution(
        ...
        self.applied_execution_ids
            .push(authority_activation_final_execution_id.to_string());
        true
    }
}
