...
//! 347/348 live epoch-transition authority-activation execution-preparation /
//! final-execution preflight boundary: the boundary that consumes a
//! **verified** Run 347/348 non-mutating live epoch-transition
//! authority-activation-execution-preparation *decision* (the accepted
//! [`ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact`], as
//! source-test-evidenced by Run 347/348) and translates it into a typed,
...
//! 347/348 boundary answers "given a verified authority-activation-authorization
//! decision, what typed non-mutating authority-activation execution-preparation
//! / final-execution preflight artifact does it authorize?", Run 349 answers the
...
//! authority-activation-final-execution nonce?".
//!
//! ## Scope and honesty constraints (Run 349)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 350**.
...
//! * Only a **verified** Run 347/348 live epoch-transition
//!   authority-activation-execution-preparation / final-execution-readiness decision
//!   that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact`] can authorize an
...
//!   [`ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_349.md`.
...
use crate::pqc_production_live_epoch_transition_authority_activation_execution_preparation::{
    ...
    LiveEpochTransitionAuthorityActivationExecutionPreparationKind,
};
...
/// Run 349 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_PROTOCOL_VERSION: u16 = 1;

/// Run 349 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:349-epoch-transition-authority-activation-final-execution-intent:v1";

/// Run 349 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_ID_DOMAIN_TAG: &str =
    "QBIND:349-epoch-transition-authority-activation-final-execution-id:v1";

/// Run 349 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:349-epoch-transition-authority-activation-final-execution-request:v1";

/// Run 349 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:349-epoch-transition-authority-activation-final-execution-transcript:v1";
...
/// Run 349 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuthorityActivationFinalExecution` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 347/348 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuthorityActivationFinalExecution` and
/// `MainnetProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequired` are
...
pub enum ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy {
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuthorityActivationFinalExecution => {
                "allow-source-test-epoch-transition-authority-activation-execution-preparation"
            }
            Self::RequireProductionLiveEpochTransitionAuthorityActivationFinalExecution => {
                "require-production-epoch-transition-authority-activation-execution-preparation"
            }
            Self::MainnetProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequired => {
                "mainnet-production-epoch-transition-authority-activation-execution-preparation-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuthorityActivationFinalExecution` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionAuthorityActivationFinalExecution` kind is fail-closed as
/// unavailable in Run 349 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionAuthorityActivationFinalExecution,
    /// Reserved production authorization kind. Fail-closed in Run 349.
    ProductionLiveEpochTransitionAuthorityActivationFinalExecution,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuthorityActivationFinalExecution => {
                "source-test-epoch-transition-authority-activation-execution-preparation"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationFinalExecution => {
                "production-epoch-transition-authority-activation-execution-preparation"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig {
    pub fn new(kind: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionAuthorityActivationFinalExecutionProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind::SourceTestLiveEpochTransitionAuthorityActivationFinalExecution,
        )
    }
...
impl Default for ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind::Disabled)
    }
}
...
/// Run 349 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 347/348
/// [`LiveEpochTransitionAuthorityActivationExecutionPreparationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuthorityActivationFinalExecutionKind {
...
impl LiveEpochTransitionAuthorityActivationFinalExecutionKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 349 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 347/348 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionAuthorityActivationExecutionPreparationKind,
    ) -> Self {
        use LiveEpochTransitionAuthorityActivationExecutionPreparationKind as A;
        match kind {
...
/// Run 349 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedAuthorityActivationExecutionPreparationDecision`] carrying a Run 347/348
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource {
    /// A verified Run 347/348 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedAuthorityActivationExecutionPreparationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingAuthorityActivationExecutionPreparationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedAuthorityActivationExecutionPreparationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedAuthorityActivationExecutionPreparationWithoutPackage {
        decision: ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 347/348 authority-activation-execution-preparation decision. Rejected.
    CommitAuthorizationDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 347/348 authority-activation-execution-preparation decision. Rejected.
    MutationExecutionDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 347/348 authority-activation-execution-preparation decision. Rejected.
    ExecutionPreparationDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 347/348 authority-activation-execution-preparation decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 331/332 guarded epoch-transition authority-activation-final-execution decision
    /// presented directly, without a Run 347/348 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 347/348 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 347/348 guarded authority-activation-final-execution decision. Rejected.
    LiveApplicationAuthorizationWithoutAuthorityActivationExecutionPreparation,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 347/348 guarded authority-activation-final-execution decision.
    /// Rejected.
    ApplicationDecisionWithoutAuthorityActivationExecutionPreparation,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 347/348 live-application authorization. Rejected.
    RotationPlanWithoutAuthorityActivationExecutionPreparation,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 347/348 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutAuthorityActivationExecutionPreparation,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutAuthorityActivationExecutionPreparation,
    ...
    FixtureOnlyAuthorityActivationExecutionPreparation,
...
/// Run 349 — the explicit trusted inputs the executor binds a verified
/// Run 347/348 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs {
    ...
    pub authority_activation_final_execution_policy_id: String,
    /// Expected Run 347/348 live-application authorization policy id (bound
    ...
    /// Expected Run 347/348 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 347/348 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 347/348 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 347/348 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 347/348 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 347/348 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 347/348 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 347/348 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 347/348 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 347/348 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded authority-activation-final-execution decision id (re-exposed
    /// by the consumed Run 347/348 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded authority-activation-final-execution request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded authority-activation-final-execution intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded authority-activation-final-execution transcript digest.
    ...
    /// Expected Run 347/348 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_authority_activation_execution_preparation_decision_id: String,
    /// Expected Run 347/348 runtime handoff request id.
    pub expected_authority_activation_execution_preparation_request_id: String,
    /// Expected Run 347/348 runtime handoff package/content digest (the
    /// consumed decision's `authority_activation_final_execution_digest`, i.e. the handoff content
    /// digest).
    pub expected_authority_activation_execution_preparation_intent_digest: String,
    /// Expected Run 347/348 runtime handoff transcript digest.
    pub expected_authority_activation_execution_preparation_transcript_digest: String,
    /// Expected Run 347/348 authority-activation-execution-preparation nonce (re-exposed by the
    /// consumed Run 347/348 authority-activation-execution-preparation artifact).
    pub expected_authority_activation_execution_preparation_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 347/348 authority-activation-execution-preparation artifact).
    ...
    /// Run 347/348 authority-activation-execution-preparation artifact).
    ...
    /// wall-clock value and never a reject path in Run 349.
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.authority_activation_final_execution_policy_id.is_empty()
            ...
            && !self.expected_authority_activation_execution_preparation_decision_id.is_empty()
            && !self.expected_authority_activation_execution_preparation_request_id.is_empty()
            && !self.expected_authority_activation_execution_preparation_intent_digest.is_empty()
            && !self.expected_authority_activation_execution_preparation_transcript_digest.is_empty()
...
/// Run 349 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition authority-activation-final-execution decision), the explicit
/// epoch-transition target, a authority-activation-execution-preparation nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequest {
    pub authority_source: LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The authority-activation-execution-preparation nonce (idempotency / replay binding).
    pub authority_activation_final_execution_nonce: u64,
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and authority-activation-execution-preparation nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource,
        proposed_epoch_transition_target: u64,
        authority_activation_final_execution_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 349 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuthorityActivationFinalExecutionReplaySet;

impl LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet
    for EmptyLiveEpochTransitionAuthorityActivationFinalExecutionReplaySet
{
...
/// Run 349 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 350+); Run 349 never
...
/// preconditions, and carries deterministic `authority_activation_final_execution_id`, `request_id`,
/// `authority_activation_final_execution_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact {
    pub staged_kind: LiveEpochTransitionAuthorityActivationFinalExecutionKind,
    pub protocol_version: u16,
    pub authority_activation_final_execution_policy_id: String,

    // ---- Re-exposed Run 347/348 authorization intent tuple ------------
    ...
    // ---- Bound Run 347/348 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 347/348 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 347/348 guarded authority-activation-final-execution authority tuple -
    // (the consumed guarded authority-activation-final-execution decision transcript)
    ...
    // ---- Guarded authority-activation-final-execution binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded authority-activation-final-execution nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 347/348 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub authority_activation_execution_preparation_decision_id: String,
    pub authority_activation_execution_preparation_request_id: String,
    pub authority_activation_execution_preparation_intent_digest: String,
    pub authority_activation_execution_preparation_transcript_digest: String,
    /// The re-exposed Run 347/348 authority-activation-execution-preparation nonce (consumed).
    pub authority_activation_execution_preparation_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 347/348 authority-activation-execution-preparation artifact)
    ...
    /// The newly proposed authority-activation-final-execution nonce.
    pub authority_activation_final_execution_nonce: u64,
    ...
    pub authority_activation_final_execution_id: String,
    pub request_id: String,
    pub authority_activation_final_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`authority_activation_final_execution_id`,
    /// `request_id`, `authority_activation_final_execution_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_final_execution_policy_id",
            self.authority_activation_final_execution_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_decision_id",
            self.authority_activation_execution_preparation_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_request_id",
            self.authority_activation_execution_preparation_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_intent_digest",
            self.authority_activation_execution_preparation_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_execution_preparation_transcript_digest",
            self.authority_activation_execution_preparation_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 349 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_authority_activation_final_execution_content_digest(
    package: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact,
) -> String {
    package.content_digest()
}

/// Run 349 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and authority-activation-execution-preparation nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_authority_activation_final_execution_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_final_execution_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"authority_activation_final_execution_policy_id",
        authority_activation_final_execution_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"authority_activation_final_execution_nonce",
        &authority_activation_final_execution_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 349 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and authority-activation-execution-preparation nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_authority_activation_final_execution_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_final_execution_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"authority_activation_final_execution_policy_id", authority_activation_final_execution_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 349 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_authority_activation_final_execution_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 349 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationFinalExecution`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuthorityActivationFinalExecutionBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition authority-activation-final-execution
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuthorityActivationFinalExecution {
        execution_kind: LiveEpochTransitionAuthorityActivationFinalExecutionKind,
    ...
    VerifiedAuthorityActivationExecutionPreparationDecisionRequired,
    UnverifiedAuthorityActivationExecutionPreparationDecisionRejected,
    ...
    AuthorityActivationExecutionPreparationDecisionIdMismatch,
    AuthorityActivationExecutionPreparationDecisionRequestIdMismatch,
    AuthorityActivationExecutionPreparationDecisionIntentDigestMismatch,
    AuthorityActivationExecutionPreparationDecisionTranscriptMismatch,
    AuthorityActivationExecutionPreparationDecisionIntegrityMismatch,
    WrongAuthorityActivationExecutionPreparationNonce,
    ...
    LiveEpochTransitionAuthorityActivationFinalExecutionAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuthorityActivationFinalExecutionBoundaryUnavailable => {
                "live-epoch-transition-authority-activation-final-execution-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable => {
                "production-live-epoch-transition-authority-activation-final-execution-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable => {
                "mainnet-production-live-epoch-transition-authority-activation-final-execution-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationFinalExecution { .. } => {
                "accepted-source-test-live-epoch-transition-authority-activation-final-execution"
            }
            Self::VerifiedAuthorityActivationExecutionPreparationDecisionRequired => {
                "verified-authority-activation-execution-preparation-decision-required"
            }
            Self::UnverifiedAuthorityActivationExecutionPreparationDecisionRejected => {
                "unverified-authority-activation-execution-preparation-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-authority-activation-execution-preparation-decision-rejected-as-production-authority"
            }
            ...
            Self::AuthorityActivationExecutionPreparationDecisionIdMismatch => "authority-activation-execution-preparation-decision-id-mismatch",
            Self::AuthorityActivationExecutionPreparationDecisionRequestIdMismatch => {
                "authority-activation-execution-preparation-decision-request-id-mismatch"
            }
            Self::AuthorityActivationExecutionPreparationDecisionIntentDigestMismatch => {
                "authority-activation-execution-preparation-decision-intent-digest-mismatch"
            }
            Self::AuthorityActivationExecutionPreparationDecisionTranscriptMismatch => {
                "authority-activation-execution-preparation-decision-transcript-mismatch"
            }
            Self::AuthorityActivationExecutionPreparationDecisionIntegrityMismatch => {
                "authority-activation-execution-preparation-decision-integrity-mismatch"
            }
            Self::WrongAuthorityActivationExecutionPreparationNonce => "wrong-authority-activation-execution-preparation-nonce",
            ...
            Self::LiveEpochTransitionAuthorityActivationFinalExecutionAmbiguous { .. } => {
                "epoch-transition-authority-activation-execution-preparation-ambiguous"
            }
...
/// Run 349 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
    pub outcome: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome,
    pub authority_activation_final_execution_id: String,
    pub request_id: String,
    pub authority_activation_final_execution_artifact:
        Option<ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact>,
    pub authority_activation_final_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.authority_activation_final_execution_artifact.is_some()
    }
}
...
/// Run 349 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRecoveryOutcome {
...
impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRecoveryOutcome {
...
/// Run 349 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor {
    pub config: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig,
    pub policy: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision,
            &'a ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact,
        ),
        ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome,
    > {
        use ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome as O;
        use LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource as S;
        match source {
            S::VerifiedAuthorityActivationExecutionPreparationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedAuthorityActivationExecutionPreparationDecisionRejected);
                }
                match &decision.authority_activation_execution_preparation_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedAuthorityActivationExecutionPreparationDecisionRequired),
                }
            }
            S::MissingAuthorityActivationExecutionPreparationDecision => {
                Err(O::VerifiedAuthorityActivationExecutionPreparationDecisionRequired)
            }
            S::UnverifiedAuthorityActivationExecutionPreparationDecision { .. } => {
                Err(O::UnverifiedAuthorityActivationExecutionPreparationDecisionRejected)
            }
            S::AcceptedAuthorityActivationExecutionPreparationWithoutPackage { .. } => {
                Err(O::VerifiedAuthorityActivationExecutionPreparationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutAuthorityActivationExecutionPreparation => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutAuthorityActivationExecutionPreparation => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutAuthorityActivationExecutionPreparation => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutAuthorityActivationExecutionPreparation => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutAuthorityActivationExecutionPreparation => {
            ...
            S::FixtureOnlyAuthorityActivationExecutionPreparation => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequired => {
                    O::MainNetProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuthorityActivationFinalExecutionUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 349.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionAuthorityActivationFinalExecutionBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 347/348 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationDecision,
        intent: &ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationArtifact,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs,
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 347/348 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.authority_activation_execution_preparation_id != inputs.expected_authority_activation_execution_preparation_decision_id {
            return Some(O::AuthorityActivationExecutionPreparationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_authority_activation_execution_preparation_request_id {
            return Some(O::AuthorityActivationExecutionPreparationDecisionRequestIdMismatch);
        }
        if decision.authority_activation_execution_preparation_digest != inputs.expected_authority_activation_execution_preparation_intent_digest {
            return Some(O::AuthorityActivationExecutionPreparationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_authority_activation_execution_preparation_transcript_digest {
            return Some(O::AuthorityActivationExecutionPreparationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.authority_activation_execution_preparation_digest {
            return Some(O::AuthorityActivationExecutionPreparationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed authority-activation-execution-preparation
        // nonce binding.
        if intent.authority_activation_execution_preparation_nonce != inputs.expected_authority_activation_execution_preparation_nonce {
            return Some(O::WrongAuthorityActivationExecutionPreparationNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 347/348
        // authority-activation-execution-preparation artifact).
        ...
        // authority tuple binding (carried through the consumed Run 347/348
        // authority-activation-execution-preparation artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition authority-activation-final-execution
        ...
        // Re-exposed Run 347/348 staged-application decision authority tuple
        ...
        // Re-exposed Run 347/348 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_authority_activation_final_execution_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_execution_preparation_digest,
            &inputs.authority_activation_final_execution_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_final_execution_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionAuthorityActivationFinalExecutionKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            authority_activation_final_execution_policy_id: inputs.authority_activation_final_execution_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded authority-activation-final-execution decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded authority-activation-final-execution nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 347/348 runtime handoff decision transcript (the
            // authority source this run consumes).
            authority_activation_execution_preparation_decision_id: decision.authority_activation_execution_preparation_id.clone(),
            authority_activation_execution_preparation_request_id: decision.request_id.clone(),
            authority_activation_execution_preparation_intent_digest: decision.authority_activation_execution_preparation_digest.clone(),
            authority_activation_execution_preparation_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed authority-activation-execution-preparation nonce.
            authority_activation_execution_preparation_nonce: application_intent.authority_activation_execution_preparation_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed authority-activation-execution-preparation artifact).
            ...
            // (carried through the consumed authority-activation-execution-preparation artifact).
            ...
            // Newly proposed authority-activation-final-execution nonce.
            authority_activation_final_execution_nonce: request.authority_activation_final_execution_nonce,
            // Exact future-executor preconditions.
            ...
            authority_activation_final_execution_id: String::new(),
            request_id: String::new(),
            authority_activation_final_execution_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuthorityActivationFinalExecution {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            authority_activation_final_execution_nonce: request.authority_activation_final_execution_nonce,
        };
        let authority_activation_final_execution_id = production_live_epoch_transition_authority_activation_final_execution_id(
            self.config.protocol_version.0,
            &decision.authority_activation_execution_preparation_digest,
            &inputs.authority_activation_final_execution_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_authority_activation_final_execution_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_execution_preparation_digest,
            &inputs.authority_activation_final_execution_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.authority_activation_final_execution_digest = record.content_digest();
        record.authority_activation_final_execution_id = authority_activation_final_execution_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_authority_activation_final_execution_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.authority_activation_final_execution_digest,
                outcome.tag(),
            );
    ...
    /// Run 349 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_authority_activation_final_execution<
        R: LiveEpochTransitionAuthorityActivationFinalExecutionReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
                outcome,
                authority_activation_final_execution_id: pkg.authority_activation_final_execution_id.clone(),
                request_id: pkg.request_id.clone(),
                authority_activation_final_execution_artifact: record.clone(),
                authority_activation_final_execution_digest: pkg.authority_activation_final_execution_digest.clone(),
        ...
        let authority_activation_execution_preparation_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionAuthorityActivationFinalExecutionAuthoritySource::AcceptedAuthorityActivationExecutionPreparationWithoutPackage {
                decision,
            } => decision.authority_activation_execution_preparation_digest.clone(),
            _ => String::new(),
        };

        let authority_activation_final_execution_id = production_live_epoch_transition_authority_activation_final_execution_id(
            self.config.protocol_version.0,
            &authority_activation_execution_preparation_intent_digest,
            &inputs.authority_activation_final_execution_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_final_execution_nonce,
        );
        ...
        let authority_activation_final_execution_digest = String::new();
        ...
        ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision {
            ...
            request_id,
            authority_activation_final_execution_artifact: None,
            authority_activation_final_execution_digest,
            transcript_digest,
        }
    }

    /// Run 349 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_authority_activation_final_execution_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact>,
        current: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact,
    ) -> ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRecoveryOutcome {
        use ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind::Disabled
        {
...
/// Run 349 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy::default()
        == ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig::default().kind
            == ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind::Disabled
}

/// Run 349 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 350).
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 349 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_mainnet_refused() -> bool {
    true
}

/// Run 349 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_is_non_mutating() -> bool {
    true
}

/// Run 349 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_never_falls_back() -> bool {
    true
}

/// Run 349 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 349 — the executor only requires a verified Run 347/348 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_authority_activation_final_execution_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 349 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 349. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuthorityActivationFinalExecutionFixtureState {
...
impl LiveEpochTransitionAuthorityActivationFinalExecutionFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, authority_activation_final_execution_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == authority_activation_final_execution_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `authority_activation_final_execution_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuthorityActivationFinalExecutionArtifact,
        authority_activation_final_execution_id: &str,
    ) -> bool {
        if self.has_applied(authority_activation_final_execution_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(authority_activation_final_execution_id.to_string());
        true
    }
}