//! Run 343 — source/test live epoch-transition **authority-lifecycle
...
//! 341/342 live epoch-transition final-settlement / authority-lifecycle
//! completion boundary: the boundary that consumes a **verified** Run 341/342
//! non-mutating live epoch-transition final-settlement / authority-lifecycle
//! completion *decision* (the accepted
//! [`ProductionLiveEpochTransitionFinalSettlementCompletionDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionFinalSettlementCompletionArtifact`], as
//! source-test-evidenced by Run 341) and translates it into a typed,
...
//! 341 boundary answers "given a verified settlement-execution decision, what
//! typed non-mutating final-settlement / authority-lifecycle completion
//! artifact does it authorize?", Run 343 answers the next question: "given a
...
//! post-completion-attestation sink must re-verify does it authorize, under an
//! explicit post-completion-attestation policy, bound to the full
...
//! post-completion-attestation nonce?".
//!
//! ## Scope and honesty constraints (Run 343)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 344**.
...
//! * Only a **verified** Run 341/342 live epoch-transition final-settlement /
//!   authority-lifecycle completion decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionFinalSettlementCompletionArtifact`] can authorize a
...
//!   [`ProductionLiveEpochTransitionPostCompletionAttestationArtifact`]; it
...
//!   activation-readiness / post-completion-attestation sink.
...
use crate::pqc_production_live_epoch_transition_final_settlement_completion::{
...
/// Run 343 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_PROTOCOL_VERSION: u16 = 1;

/// Run 343 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_INTENT_DOMAIN_TAG: &str =
    "QBIND:343-epoch-transition-post-completion-attestation-intent:v1";

/// Run 343 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_ID_DOMAIN_TAG: &str =
    "QBIND:343-epoch-transition-post-completion-attestation-id:v1";

/// Run 343 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:343-epoch-transition-post-completion-attestation-request:v1";

/// Run 343 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:343-epoch-transition-post-completion-attestation-transcript:v1";
...
/// Run 343 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionPostCompletionAttestationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionPostCompletionAttestationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionPostCompletionAttestation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 341/342 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionPostCompletionAttestation` and
/// `MainnetProductionLiveEpochTransitionPostCompletionAttestationRequired` are
...
pub enum ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionPostCompletionAttestation,
    ...
    RequireProductionLiveEpochTransitionPostCompletionAttestation,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionPostCompletionAttestationRequired,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionPostCompletionAttestation => {
                "allow-source-test-epoch-transition-final-settlement-completion"
            }
            Self::RequireProductionLiveEpochTransitionPostCompletionAttestation => {
                "require-production-epoch-transition-final-settlement-completion"
            }
            Self::MainnetProductionLiveEpochTransitionPostCompletionAttestationRequired => {
                "mainnet-production-epoch-transition-final-settlement-completion-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionPostCompletionAttestation` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionPostCompletionAttestation` kind is fail-closed as
/// unavailable in Run 343 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionPostCompletionAttestation,
    /// Reserved production authorization kind. Fail-closed in Run 343.
    ProductionLiveEpochTransitionPostCompletionAttestation,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionPostCompletionAttestation => {
                "source-test-epoch-transition-final-settlement-completion"
            }
            Self::ProductionLiveEpochTransitionPostCompletionAttestation => {
                "production-epoch-transition-final-settlement-completion"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionPostCompletionAttestationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionPostCompletionAttestationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionPostCompletionAttestationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind::SourceTestLiveEpochTransitionPostCompletionAttestation,
        )
    }
...
impl Default for ProductionLiveEpochTransitionPostCompletionAttestationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind::Disabled)
    }
}
...
/// Run 343 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 341/342
/// [`LiveEpochTransitionFinalSettlementCompletionKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionPostCompletionAttestationKind {
...
impl LiveEpochTransitionPostCompletionAttestationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 343 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 341/342 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionFinalSettlementCompletionKind,
    ) -> Self {
        use LiveEpochTransitionFinalSettlementCompletionKind as A;
        match kind {
...
/// Run 343 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedFinalSettlementCompletionDecision`] carrying a Run 341/342
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionPostCompletionAttestationAuthoritySource {
    /// A verified Run 341/342 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedFinalSettlementCompletionDecision {
        decision: ProductionLiveEpochTransitionFinalSettlementCompletionDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingFinalSettlementCompletionDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedFinalSettlementCompletionDecision {
        decision: ProductionLiveEpochTransitionFinalSettlementCompletionDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedFinalSettlementCompletionWithoutPackage {
        decision: ProductionLiveEpochTransitionFinalSettlementCompletionDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 341/342 final-settlement-completion decision. Rejected.
    CommitAuthorizationDecisionWithoutFinalSettlementCompletion,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 341/342 final-settlement-completion decision. Rejected.
    MutationExecutionDecisionWithoutFinalSettlementCompletion,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 341/342 final-settlement-completion decision. Rejected.
    ExecutionPreparationDecisionWithoutFinalSettlementCompletion,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 341/342 final-settlement-completion decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutFinalSettlementCompletion,
    /// A Run 331/332 guarded epoch-transition post-completion-attestation decision
    /// presented directly, without a Run 341/342 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutFinalSettlementCompletion,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 341/342 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutFinalSettlementCompletion,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 341/342 guarded post-completion-attestation decision. Rejected.
    LiveApplicationAuthorizationWithoutFinalSettlementCompletion,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 341/342 guarded post-completion-attestation decision.
    /// Rejected.
    ApplicationDecisionWithoutFinalSettlementCompletion,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 341/342 live-application authorization. Rejected.
    RotationPlanWithoutFinalSettlementCompletion,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 341/342 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutFinalSettlementCompletion,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutFinalSettlementCompletion,
    ...
    FixtureOnlyFinalSettlementCompletion,
...
/// Run 343 — the explicit trusted inputs the executor binds a verified
/// Run 341/342 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationInputs {
    ...
    pub post_completion_attestation_policy_id: String,
    /// Expected Run 341/342 live-application authorization policy id (bound
    ...
    /// Expected Run 341/342 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 341/342 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 341/342 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 341/342 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 341/342 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 341/342 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 341/342 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 341/342 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 341/342 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 341/342 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded post-completion-attestation decision id (re-exposed
    /// by the consumed Run 341/342 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded post-completion-attestation request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded post-completion-attestation intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded post-completion-attestation transcript digest.
    ...
    /// Expected Run 341/342 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_final_settlement_completion_decision_id: String,
    /// Expected Run 341/342 runtime handoff request id.
    pub expected_final_settlement_completion_request_id: String,
    /// Expected Run 341/342 runtime handoff package/content digest (the
    /// consumed decision's `post_completion_attestation_digest`, i.e. the handoff content
    /// digest).
    pub expected_final_settlement_completion_intent_digest: String,
    /// Expected Run 341/342 runtime handoff transcript digest.
    pub expected_final_settlement_completion_transcript_digest: String,
    /// Expected Run 341/342 final-settlement-completion nonce (re-exposed by the
    /// consumed Run 341/342 final-settlement-completion artifact).
    pub expected_final_settlement_completion_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 341/342 final-settlement-completion artifact).
    ...
    /// Run 341/342 final-settlement-completion artifact).
    ...
    /// wall-clock value and never a reject path in Run 343.
...
impl ProductionLiveEpochTransitionPostCompletionAttestationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.post_completion_attestation_policy_id.is_empty()
            ...
            && !self.expected_final_settlement_completion_decision_id.is_empty()
            && !self.expected_final_settlement_completion_request_id.is_empty()
            && !self.expected_final_settlement_completion_intent_digest.is_empty()
            && !self.expected_final_settlement_completion_transcript_digest.is_empty()
...
/// Run 343 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition post-completion-attestation decision), the explicit
/// epoch-transition target, a final-settlement-completion nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationRequest {
    pub authority_source: LiveEpochTransitionPostCompletionAttestationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The final-settlement-completion nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionPostCompletionAttestationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and final-settlement-completion nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionPostCompletionAttestationAuthoritySource,
        proposed_epoch_transition_target: u64,
        post_completion_attestation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 343 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionPostCompletionAttestationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionPostCompletionAttestationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionPostCompletionAttestationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionPostCompletionAttestationReplaySet;

impl LiveEpochTransitionPostCompletionAttestationReplaySet
    for EmptyLiveEpochTransitionPostCompletionAttestationReplaySet
{
...
/// Run 343 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 344+); Run 343 never
...
/// preconditions, and carries deterministic `post_completion_attestation_id`, `request_id`,
/// `post_completion_attestation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationArtifact {
    pub staged_kind: LiveEpochTransitionPostCompletionAttestationKind,
    pub protocol_version: u16,
    pub post_completion_attestation_policy_id: String,

    // ---- Re-exposed Run 341/342 authorization intent tuple ------------
    ...
    // ---- Bound Run 341/342 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 341/342 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 341/342 guarded post-completion-attestation authority tuple -
    // (the consumed guarded post-completion-attestation decision transcript)
    ...
    // ---- Guarded post-completion-attestation binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded post-completion-attestation nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 341/342 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub final_settlement_completion_decision_id: String,
    pub final_settlement_completion_request_id: String,
    pub final_settlement_completion_intent_digest: String,
    pub final_settlement_completion_transcript_digest: String,
    /// The re-exposed Run 341/342 final-settlement-completion nonce (consumed).
    pub final_settlement_completion_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 341/342 final-settlement-completion artifact)
    ...
    /// The newly proposed post-completion-attestation nonce.
    pub post_completion_attestation_nonce: u64,
    ...
    pub post_completion_attestation_id: String,
    pub request_id: String,
    pub post_completion_attestation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`post_completion_attestation_id`,
    /// `request_id`, `post_completion_attestation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"post_completion_attestation_policy_id",
            self.post_completion_attestation_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"final_settlement_completion_decision_id",
            self.final_settlement_completion_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"final_settlement_completion_request_id",
            self.final_settlement_completion_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"final_settlement_completion_intent_digest",
            self.final_settlement_completion_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"final_settlement_completion_transcript_digest",
            self.final_settlement_completion_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"final_settlement_completion_nonce",
            &self.final_settlement_completion_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 343 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_post_completion_attestation_content_digest(
    package: &ProductionLiveEpochTransitionPostCompletionAttestationArtifact,
) -> String {
    package.content_digest()
}

/// Run 343 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and final-settlement-completion nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_post_completion_attestation_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    post_completion_attestation_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"post_completion_attestation_policy_id",
        post_completion_attestation_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"post_completion_attestation_nonce",
        &post_completion_attestation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 343 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and final-settlement-completion nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_post_completion_attestation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    post_completion_attestation_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"post_completion_attestation_policy_id", post_completion_attestation_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 343 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_post_completion_attestation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_FINAL_SETTLEMENT_COMPLETION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 343 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionPostCompletionAttestation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionPostCompletionAttestationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionPostCompletionAttestationBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionPostCompletionAttestationUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionPostCompletionAttestationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition post-completion-attestation
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionPostCompletionAttestation {
        execution_kind: LiveEpochTransitionPostCompletionAttestationKind,
    ...
    VerifiedFinalSettlementCompletionDecisionRequired,
    UnverifiedFinalSettlementCompletionDecisionRejected,
    ...
    FinalSettlementCompletionDecisionIdMismatch,
    FinalSettlementCompletionDecisionRequestIdMismatch,
    FinalSettlementCompletionDecisionIntentDigestMismatch,
    FinalSettlementCompletionDecisionTranscriptMismatch,
    FinalSettlementCompletionDecisionIntegrityMismatch,
    WrongFinalSettlementCompletionNonce,
    ...
    LiveEpochTransitionPostCompletionAttestationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionPostCompletionAttestationBoundaryUnavailable => {
                "live-epoch-transition-post-completion-attestation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionPostCompletionAttestationUnavailable => {
                "production-live-epoch-transition-post-completion-attestation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionPostCompletionAttestationUnavailable => {
                "mainnet-production-live-epoch-transition-post-completion-attestation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionPostCompletionAttestation { .. } => {
                "accepted-source-test-live-epoch-transition-post-completion-attestation"
            }
            Self::VerifiedFinalSettlementCompletionDecisionRequired => {
                "verified-final-settlement-completion-decision-required"
            }
            Self::UnverifiedFinalSettlementCompletionDecisionRejected => {
                "unverified-final-settlement-completion-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-final-settlement-completion-decision-rejected-as-production-authority"
            }
            ...
            Self::FinalSettlementCompletionDecisionIdMismatch => "final-settlement-completion-decision-id-mismatch",
            Self::FinalSettlementCompletionDecisionRequestIdMismatch => {
                "final-settlement-completion-decision-request-id-mismatch"
            }
            Self::FinalSettlementCompletionDecisionIntentDigestMismatch => {
                "final-settlement-completion-decision-intent-digest-mismatch"
            }
            Self::FinalSettlementCompletionDecisionTranscriptMismatch => {
                "final-settlement-completion-decision-transcript-mismatch"
            }
            Self::FinalSettlementCompletionDecisionIntegrityMismatch => {
                "final-settlement-completion-decision-integrity-mismatch"
            }
            Self::WrongFinalSettlementCompletionNonce => "wrong-final-settlement-completion-nonce",
            ...
            Self::LiveEpochTransitionPostCompletionAttestationAmbiguous { .. } => {
                "epoch-transition-final-settlement-completion-ambiguous"
            }
...
/// Run 343 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationDecision {
    pub outcome: ProductionLiveEpochTransitionPostCompletionAttestationOutcome,
    pub post_completion_attestation_id: String,
    pub request_id: String,
    pub post_completion_attestation_artifact:
        Option<ProductionLiveEpochTransitionPostCompletionAttestationArtifact>,
    pub post_completion_attestation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.post_completion_attestation_artifact.is_some()
    }
}
...
/// Run 343 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionPostCompletionAttestationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionPostCompletionAttestationRecoveryOutcome {
...
/// Run 343 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionPostCompletionAttestationExecutor {
    pub config: ProductionLiveEpochTransitionPostCompletionAttestationConfig,
    pub policy: ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy,
}

impl ProductionLiveEpochTransitionPostCompletionAttestationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionPostCompletionAttestationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionPostCompletionAttestationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionFinalSettlementCompletionDecision,
            &'a ProductionLiveEpochTransitionFinalSettlementCompletionArtifact,
        ),
        ProductionLiveEpochTransitionPostCompletionAttestationOutcome,
    > {
        use ProductionLiveEpochTransitionPostCompletionAttestationOutcome as O;
        use LiveEpochTransitionPostCompletionAttestationAuthoritySource as S;
        match source {
            S::VerifiedFinalSettlementCompletionDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedFinalSettlementCompletionDecisionRejected);
                }
                match &decision.final_settlement_completion_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedFinalSettlementCompletionDecisionRequired),
                }
            }
            S::MissingFinalSettlementCompletionDecision => {
                Err(O::VerifiedFinalSettlementCompletionDecisionRequired)
            }
            S::UnverifiedFinalSettlementCompletionDecision { .. } => {
                Err(O::UnverifiedFinalSettlementCompletionDecisionRejected)
            }
            S::AcceptedFinalSettlementCompletionWithoutPackage { .. } => {
                Err(O::VerifiedFinalSettlementCompletionDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutFinalSettlementCompletion => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutFinalSettlementCompletion => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutFinalSettlementCompletion => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutFinalSettlementCompletion => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutFinalSettlementCompletion => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutFinalSettlementCompletion => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutFinalSettlementCompletion => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutFinalSettlementCompletion => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutFinalSettlementCompletion => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutFinalSettlementCompletion => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutFinalSettlementCompletion => {
            ...
            S::FixtureOnlyFinalSettlementCompletion => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionPostCompletionAttestationOutcome> {
        use ProductionLiveEpochTransitionPostCompletionAttestationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy::MainnetProductionLiveEpochTransitionPostCompletionAttestationRequired => {
                    O::MainNetProductionLiveEpochTransitionPostCompletionAttestationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionPostCompletionAttestationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionPostCompletionAttestationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 343.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionPostCompletionAttestationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 341/342 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionFinalSettlementCompletionDecision,
        intent: &ProductionLiveEpochTransitionFinalSettlementCompletionArtifact,
        inputs: &ProductionLiveEpochTransitionPostCompletionAttestationInputs,
    ) -> Option<ProductionLiveEpochTransitionPostCompletionAttestationOutcome> {
        use ProductionLiveEpochTransitionPostCompletionAttestationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 341/342 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.final_settlement_completion_id != inputs.expected_final_settlement_completion_decision_id {
            return Some(O::FinalSettlementCompletionDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_final_settlement_completion_request_id {
            return Some(O::FinalSettlementCompletionDecisionRequestIdMismatch);
        }
        if decision.final_settlement_completion_digest != inputs.expected_final_settlement_completion_intent_digest {
            return Some(O::FinalSettlementCompletionDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_final_settlement_completion_transcript_digest {
            return Some(O::FinalSettlementCompletionDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.final_settlement_completion_digest {
            return Some(O::FinalSettlementCompletionDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed final-settlement-completion
        // nonce binding.
        if intent.final_settlement_completion_nonce != inputs.expected_final_settlement_completion_nonce {
            return Some(O::WrongFinalSettlementCompletionNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 341/342
        // final-settlement-completion artifact).
        ...
        // authority tuple binding (carried through the consumed Run 341/342
        // final-settlement-completion artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition post-completion-attestation
        ...
        // Re-exposed Run 341/342 staged-application decision authority tuple
        ...
        // Re-exposed Run 341/342 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionPostCompletionAttestationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionPostCompletionAttestationRequest,
        inputs: &ProductionLiveEpochTransitionPostCompletionAttestationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_post_completion_attestation_request_id(
            self.config.protocol_version.0,
            &decision.final_settlement_completion_digest,
            &inputs.post_completion_attestation_policy_id,
            request.proposed_epoch_transition_target,
            request.post_completion_attestation_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionPostCompletionAttestationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionPostCompletionAttestationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            post_completion_attestation_policy_id: inputs.post_completion_attestation_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded post-completion-attestation decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded post-completion-attestation nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 341/342 runtime handoff decision transcript (the
            // authority source this run consumes).
            final_settlement_completion_decision_id: decision.final_settlement_completion_id.clone(),
            final_settlement_completion_request_id: decision.request_id.clone(),
            final_settlement_completion_intent_digest: decision.final_settlement_completion_digest.clone(),
            final_settlement_completion_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed final-settlement-completion nonce.
            final_settlement_completion_nonce: application_intent.final_settlement_completion_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed final-settlement-completion artifact).
            ...
            // (carried through the consumed final-settlement-completion artifact).
            ...
            // Newly proposed post-completion-attestation nonce.
            post_completion_attestation_nonce: request.post_completion_attestation_nonce,
            // Exact future-executor preconditions.
            ...
            post_completion_attestation_id: String::new(),
            request_id: String::new(),
            post_completion_attestation_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionPostCompletionAttestation {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            post_completion_attestation_nonce: request.post_completion_attestation_nonce,
        };
        let post_completion_attestation_id = production_live_epoch_transition_post_completion_attestation_id(
            self.config.protocol_version.0,
            &decision.final_settlement_completion_digest,
            &inputs.post_completion_attestation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_post_completion_attestation_request_id(
            self.config.protocol_version.0,
            &decision.final_settlement_completion_digest,
            &inputs.post_completion_attestation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.post_completion_attestation_digest = record.content_digest();
        record.post_completion_attestation_id = post_completion_attestation_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_post_completion_attestation_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.post_completion_attestation_digest,
                outcome.tag(),
            );
    ...
    /// Run 343 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_post_completion_attestation<
        R: LiveEpochTransitionPostCompletionAttestationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionPostCompletionAttestationRequest,
        inputs: &ProductionLiveEpochTransitionPostCompletionAttestationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionPostCompletionAttestationDecision {
                outcome,
                post_completion_attestation_id: pkg.post_completion_attestation_id.clone(),
                request_id: pkg.request_id.clone(),
                post_completion_attestation_artifact: record.clone(),
                post_completion_attestation_digest: pkg.post_completion_attestation_digest.clone(),
        ...
        let final_settlement_completion_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionPostCompletionAttestationAuthoritySource::AcceptedFinalSettlementCompletionWithoutPackage {
                decision,
            } => decision.final_settlement_completion_digest.clone(),
            _ => String::new(),
        };

        let post_completion_attestation_id = production_live_epoch_transition_post_completion_attestation_id(
            self.config.protocol_version.0,
            &final_settlement_completion_intent_digest,
            &inputs.post_completion_attestation_policy_id,
            request.proposed_epoch_transition_target,
            request.post_completion_attestation_nonce,
        );
        ...
        let post_completion_attestation_digest = String::new();
        ...
        ProductionLiveEpochTransitionPostCompletionAttestationDecision {
            ...
            request_id,
            post_completion_attestation_artifact: None,
            post_completion_attestation_digest,
            transcript_digest,
        }
    }

    /// Run 343 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_post_completion_attestation_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionPostCompletionAttestationArtifact>,
        current: &ProductionLiveEpochTransitionPostCompletionAttestationArtifact,
    ) -> ProductionLiveEpochTransitionPostCompletionAttestationRecoveryOutcome {
        use ProductionLiveEpochTransitionPostCompletionAttestationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind::Disabled
        {
...
/// Run 343 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_post_completion_attestation_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy::default()
        == ProductionLiveEpochTransitionPostCompletionAttestationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionPostCompletionAttestationConfig::default().kind
            == ProductionLiveEpochTransitionPostCompletionAttestationExecutorKind::Disabled
}

/// Run 343 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 344).
pub fn production_live_epoch_transition_post_completion_attestation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 343 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_post_completion_attestation_executor_mainnet_refused() -> bool {
    true
}

/// Run 343 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_post_completion_attestation_executor_is_non_mutating() -> bool {
    true
}

/// Run 343 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_post_completion_attestation_executor_never_falls_back() -> bool {
    true
}

/// Run 343 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_post_completion_attestation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 343 — the executor only requires a verified Run 341/342 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_post_completion_attestation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 343 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 343. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionPostCompletionAttestationFixtureState {
...
impl LiveEpochTransitionPostCompletionAttestationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, post_completion_attestation_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == post_completion_attestation_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `post_completion_attestation_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionPostCompletionAttestationArtifact,
        post_completion_attestation_id: &str,
    ) -> bool {
        if self.has_applied(post_completion_attestation_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(post_completion_attestation_id.to_string());
        true
    }
}