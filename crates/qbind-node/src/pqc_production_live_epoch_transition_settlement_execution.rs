//! Run 339 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 331/332 guarded epoch-transition settlement-execution boundary: the boundary
//! that consumes a **verified** Run 337/338 non-mutating guarded
//! epoch-transition settlement-execution *decision* (the accepted
//! [`ProductionLiveEpochTransitionSettlementExecutionPreparationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact`], as
//! release-binary-evidenced by Run 332) and translates it into a typed,
...
//!
//! Where the Run 331 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 339 answers the next question:
//! "given a verified guarded settlement-execution decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a settlement-execution-preparation
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 339)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 340**.
...
//! * Only a **verified** Run 337/338 guarded epoch-transition
//!   settlement-execution decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 329/330 staged-application decision alone, a Run
//!   327/328 live-application authorization alone, a Run 325/326 application
//!   decision alone, Run 323 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionSettlementExecutionArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_337.md`.
...
use crate::pqc_production_live_epoch_transition_settlement_execution_preparation::{
...
/// Run 339 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PROTOCOL_VERSION: u16 = 1;

/// Run 339 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_INTENT_DOMAIN_TAG: &str =
    "QBIND:339-epoch-transition-settlement-execution-preparation-intent:v1";

/// Run 339 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_ID_DOMAIN_TAG: &str =
    "QBIND:339-epoch-transition-settlement-execution-preparation-id:v1";

/// Run 339 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:339-epoch-transition-settlement-execution-preparation-request:v1";

/// Run 339 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:339-epoch-transition-settlement-execution-preparation-transcript:v1";
...
/// Run 339 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionSettlementExecutionProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionSettlementExecutionProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionSettlementExecutionProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionSettlementExecution` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 337/338 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionSettlementExecution` and
/// `MainnetProductionLiveEpochTransitionSettlementExecutionRequired` are
...
pub enum ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionSettlementExecution,
    ...
    RequireProductionLiveEpochTransitionSettlementExecution,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionSettlementExecutionRequired,
}

impl ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionSettlementExecution => {
                "allow-source-test-epoch-transition-settlement-execution-preparation"
            }
            Self::RequireProductionLiveEpochTransitionSettlementExecution => {
                "require-production-epoch-transition-settlement-execution-preparation"
            }
            Self::MainnetProductionLiveEpochTransitionSettlementExecutionRequired => {
                "mainnet-production-epoch-transition-settlement-execution-preparation-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionSettlementExecution` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionSettlementExecution` kind is fail-closed as
/// unavailable in Run 339 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionSettlementExecutionExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionSettlementExecution,
    /// Reserved production authorization kind. Fail-closed in Run 339.
    ProductionLiveEpochTransitionSettlementExecution,
}

impl ProductionLiveEpochTransitionSettlementExecutionExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionSettlementExecution => {
                "source-test-epoch-transition-settlement-execution-preparation"
            }
            Self::ProductionLiveEpochTransitionSettlementExecution => {
                "production-epoch-transition-settlement-execution-preparation"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionSettlementExecutionConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionSettlementExecutionProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionSettlementExecutionExecutorKind,
}

impl ProductionLiveEpochTransitionSettlementExecutionConfig {
    pub fn new(kind: ProductionLiveEpochTransitionSettlementExecutionExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionSettlementExecutionProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionSettlementExecutionExecutorKind::SourceTestLiveEpochTransitionSettlementExecution,
        )
    }
...
impl Default for ProductionLiveEpochTransitionSettlementExecutionConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionSettlementExecutionExecutorKind::Disabled)
    }
}
...
/// Run 339 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 337/338
/// [`LiveEpochTransitionSettlementExecutionPreparationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionSettlementExecutionKind {
...
impl LiveEpochTransitionSettlementExecutionKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 339 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 337/338 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionSettlementExecutionPreparationKind,
    ) -> Self {
        use LiveEpochTransitionSettlementExecutionPreparationKind as A;
        match kind {
...
/// Run 339 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedSettlementExecutionPreparationDecision`] carrying a Run 337/338
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionSettlementExecutionAuthoritySource {
    /// A verified Run 337/338 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedSettlementExecutionPreparationDecision {
        decision: ProductionLiveEpochTransitionSettlementExecutionPreparationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingSettlementExecutionPreparationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedSettlementExecutionPreparationDecision {
        decision: ProductionLiveEpochTransitionSettlementExecutionPreparationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedSettlementExecutionPreparationWithoutPackage {
        decision: ProductionLiveEpochTransitionSettlementExecutionPreparationDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 337/338 settlement-execution-preparation decision. Rejected.
    CommitAuthorizationDecisionWithoutSettlementExecutionPreparation,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 337/338 settlement-execution-preparation decision. Rejected.
    MutationExecutionDecisionWithoutSettlementExecutionPreparation,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 337/338 settlement-execution-preparation decision. Rejected.
    ExecutionPreparationDecisionWithoutSettlementExecutionPreparation,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 337/338 settlement-execution-preparation decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutSettlementExecutionPreparation,
    /// A Run 331/332 guarded epoch-transition settlement-execution decision
    /// presented directly, without a Run 337/338 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutSettlementExecutionPreparation,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 337/338 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutSettlementExecutionPreparation,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 337/338 guarded settlement-execution decision. Rejected.
    LiveApplicationAuthorizationWithoutSettlementExecutionPreparation,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 337/338 guarded settlement-execution decision.
    /// Rejected.
    ApplicationDecisionWithoutSettlementExecutionPreparation,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 337/338 live-application authorization. Rejected.
    RotationPlanWithoutSettlementExecutionPreparation,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 337/338 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutSettlementExecutionPreparation,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutSettlementExecutionPreparation,
    ...
    FixtureOnlySettlementExecutionPreparation,
...
/// Run 339 — the explicit trusted inputs the executor binds a verified
/// Run 337/338 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionInputs {
    ...
    pub settlement_execution_policy_id: String,
    /// Expected Run 337/338 live-application authorization policy id (bound
    ...
    /// Expected Run 337/338 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 337/338 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 337/338 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 337/338 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 337/338 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 337/338 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 337/338 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 337/338 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 337/338 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 337/338 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded settlement-execution decision id (re-exposed
    /// by the consumed Run 337/338 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded settlement-execution request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded settlement-execution intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded settlement-execution transcript digest.
    ...
    /// Expected Run 337/338 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_settlement_execution_preparation_decision_id: String,
    /// Expected Run 337/338 runtime handoff request id.
    pub expected_settlement_execution_preparation_request_id: String,
    /// Expected Run 337/338 runtime handoff package/content digest (the
    /// consumed decision's `settlement_execution_digest`, i.e. the handoff content
    /// digest).
    pub expected_settlement_execution_preparation_intent_digest: String,
    /// Expected Run 337/338 runtime handoff transcript digest.
    pub expected_settlement_execution_preparation_transcript_digest: String,
    /// Expected Run 337/338 settlement-execution-preparation nonce (re-exposed by the
    /// consumed Run 337/338 settlement-execution-preparation artifact).
    pub expected_settlement_execution_preparation_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 337/338 settlement-execution-preparation artifact).
    ...
    /// Run 337/338 settlement-execution-preparation artifact).
    ...
    /// wall-clock value and never a reject path in Run 339.
...
impl ProductionLiveEpochTransitionSettlementExecutionInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.settlement_execution_policy_id.is_empty()
            ...
            && !self.expected_settlement_execution_preparation_decision_id.is_empty()
            && !self.expected_settlement_execution_preparation_request_id.is_empty()
            && !self.expected_settlement_execution_preparation_intent_digest.is_empty()
            && !self.expected_settlement_execution_preparation_transcript_digest.is_empty()
...
/// Run 339 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition settlement-execution decision), the explicit
/// epoch-transition target, a settlement-execution-preparation nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionRequest {
    pub authority_source: LiveEpochTransitionSettlementExecutionAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The settlement-execution-preparation nonce (idempotency / replay binding).
    pub settlement_execution_nonce: u64,
...
impl ProductionLiveEpochTransitionSettlementExecutionRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and settlement-execution-preparation nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionSettlementExecutionAuthoritySource,
        proposed_epoch_transition_target: u64,
        settlement_execution_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 339 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionSettlementExecutionReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionSettlementExecutionReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionSettlementExecutionReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionSettlementExecutionReplaySet;

impl LiveEpochTransitionSettlementExecutionReplaySet
    for EmptyLiveEpochTransitionSettlementExecutionReplaySet
{
...
/// Run 339 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 340+); Run 339 never
...
/// preconditions, and carries deterministic `settlement_execution_id`, `request_id`,
/// `settlement_execution_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionArtifact {
    pub staged_kind: LiveEpochTransitionSettlementExecutionKind,
    pub protocol_version: u16,
    pub settlement_execution_policy_id: String,

    // ---- Re-exposed Run 337/338 authorization intent tuple ------------
    ...
    // ---- Bound Run 337/338 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 337/338 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 337/338 guarded settlement-execution authority tuple -
    // (the consumed guarded settlement-execution decision transcript)
    ...
    // ---- Guarded settlement-execution binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded settlement-execution nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 337/338 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub settlement_execution_preparation_decision_id: String,
    pub settlement_execution_preparation_request_id: String,
    pub settlement_execution_preparation_intent_digest: String,
    pub settlement_execution_preparation_transcript_digest: String,
    /// The re-exposed Run 337/338 settlement-execution-preparation nonce (consumed).
    pub settlement_execution_preparation_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 337/338 settlement-execution-preparation artifact)
    ...
    /// The newly proposed settlement-execution nonce.
    pub settlement_execution_nonce: u64,
    ...
    pub settlement_execution_id: String,
    pub request_id: String,
    pub settlement_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementExecutionArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`settlement_execution_id`,
    /// `request_id`, `settlement_execution_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"settlement_execution_policy_id",
            self.settlement_execution_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"settlement_execution_preparation_decision_id",
            self.settlement_execution_preparation_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_preparation_request_id",
            self.settlement_execution_preparation_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_preparation_intent_digest",
            self.settlement_execution_preparation_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_execution_preparation_transcript_digest",
            self.settlement_execution_preparation_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"settlement_execution_nonce",
            &self.settlement_execution_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 339 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_settlement_execution_content_digest(
    package: &ProductionLiveEpochTransitionSettlementExecutionArtifact,
) -> String {
    package.content_digest()
}

/// Run 339 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and settlement-execution-preparation nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_settlement_execution_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_execution_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"settlement_execution_policy_id",
        settlement_execution_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"settlement_execution_nonce",
        &settlement_execution_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 339 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and settlement-execution-preparation nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_settlement_execution_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_execution_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"settlement_execution_policy_id", settlement_execution_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 339 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_settlement_execution_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 339 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionSettlementExecution`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementExecutionOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionSettlementExecutionBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionSettlementExecutionUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionSettlementExecutionUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition settlement-execution
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionSettlementExecution {
        execution_kind: LiveEpochTransitionSettlementExecutionKind,
    ...
    VerifiedSettlementExecutionPreparationDecisionRequired,
    UnverifiedSettlementExecutionPreparationDecisionRejected,
    ...
    SettlementExecutionPreparationDecisionIdMismatch,
    SettlementExecutionPreparationDecisionRequestIdMismatch,
    SettlementExecutionPreparationDecisionIntentDigestMismatch,
    SettlementExecutionPreparationDecisionTranscriptMismatch,
    SettlementExecutionPreparationDecisionIntegrityMismatch,
    WrongSettlementExecutionPreparationNonce,
    ...
    LiveEpochTransitionSettlementExecutionAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionSettlementExecutionOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionSettlementExecutionBoundaryUnavailable => {
                "live-epoch-transition-settlement-execution-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionSettlementExecutionUnavailable => {
                "production-live-epoch-transition-settlement-execution-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionSettlementExecutionUnavailable => {
                "mainnet-production-live-epoch-transition-settlement-execution-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionSettlementExecution { .. } => {
                "accepted-source-test-live-epoch-transition-settlement-execution"
            }
            Self::VerifiedSettlementExecutionPreparationDecisionRequired => {
                "verified-settlement-execution-preparation-decision-required"
            }
            Self::UnverifiedSettlementExecutionPreparationDecisionRejected => {
                "unverified-settlement-execution-preparation-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-settlement-execution-preparation-decision-rejected-as-production-authority"
            }
            ...
            Self::SettlementExecutionPreparationDecisionIdMismatch => "settlement-execution-preparation-decision-id-mismatch",
            Self::SettlementExecutionPreparationDecisionRequestIdMismatch => {
                "settlement-execution-preparation-decision-request-id-mismatch"
            }
            Self::SettlementExecutionPreparationDecisionIntentDigestMismatch => {
                "settlement-execution-preparation-decision-intent-digest-mismatch"
            }
            Self::SettlementExecutionPreparationDecisionTranscriptMismatch => {
                "settlement-execution-preparation-decision-transcript-mismatch"
            }
            Self::SettlementExecutionPreparationDecisionIntegrityMismatch => {
                "settlement-execution-preparation-decision-integrity-mismatch"
            }
            Self::WrongSettlementExecutionPreparationNonce => "wrong-settlement-execution-preparation-nonce",
            ...
            Self::LiveEpochTransitionSettlementExecutionAmbiguous { .. } => {
                "epoch-transition-settlement-execution-preparation-ambiguous"
            }
...
/// Run 339 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionDecision {
    pub outcome: ProductionLiveEpochTransitionSettlementExecutionOutcome,
    pub settlement_execution_id: String,
    pub request_id: String,
    pub settlement_execution_artifact:
        Option<ProductionLiveEpochTransitionSettlementExecutionArtifact>,
    pub settlement_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementExecutionDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.settlement_execution_artifact.is_some()
    }
}
...
/// Run 339 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome {
...
impl ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome {
...
/// Run 339 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionExecutor {
    pub config: ProductionLiveEpochTransitionSettlementExecutionConfig,
    pub policy: ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy,
}

impl ProductionLiveEpochTransitionSettlementExecutionExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionSettlementExecutionConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionSettlementExecutionAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionSettlementExecutionPreparationDecision,
            &'a ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact,
        ),
        ProductionLiveEpochTransitionSettlementExecutionOutcome,
    > {
        use ProductionLiveEpochTransitionSettlementExecutionOutcome as O;
        use LiveEpochTransitionSettlementExecutionAuthoritySource as S;
        match source {
            S::VerifiedSettlementExecutionPreparationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedSettlementExecutionPreparationDecisionRejected);
                }
                match &decision.settlement_execution_preparation_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedSettlementExecutionPreparationDecisionRequired),
                }
            }
            S::MissingSettlementExecutionPreparationDecision => {
                Err(O::VerifiedSettlementExecutionPreparationDecisionRequired)
            }
            S::UnverifiedSettlementExecutionPreparationDecision { .. } => {
                Err(O::UnverifiedSettlementExecutionPreparationDecisionRejected)
            }
            S::AcceptedSettlementExecutionPreparationWithoutPackage { .. } => {
                Err(O::VerifiedSettlementExecutionPreparationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutSettlementExecutionPreparation => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutSettlementExecutionPreparation => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutSettlementExecutionPreparation => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutSettlementExecutionPreparation => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutSettlementExecutionPreparation => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutSettlementExecutionPreparation => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutSettlementExecutionPreparation => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutSettlementExecutionPreparation => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutSettlementExecutionPreparation => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutSettlementExecutionPreparation => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutSettlementExecutionPreparation => {
            ...
            S::FixtureOnlySettlementExecutionPreparation => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionSettlementExecutionOutcome> {
        use ProductionLiveEpochTransitionSettlementExecutionOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementExecutionExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::MainnetProductionLiveEpochTransitionSettlementExecutionRequired => {
                    O::MainNetProductionLiveEpochTransitionSettlementExecutionUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionSettlementExecutionUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionSettlementExecutionUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 339.
        if self.config.kind
            == ProductionLiveEpochTransitionSettlementExecutionExecutorKind::ProductionLiveEpochTransitionSettlementExecution
        {
            return Some(O::LiveEpochTransitionSettlementExecutionBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 337/338 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionSettlementExecutionPreparationDecision,
        intent: &ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionInputs,
    ) -> Option<ProductionLiveEpochTransitionSettlementExecutionOutcome> {
        use ProductionLiveEpochTransitionSettlementExecutionOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 337/338 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.settlement_execution_preparation_id != inputs.expected_settlement_execution_preparation_decision_id {
            return Some(O::SettlementExecutionPreparationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_settlement_execution_preparation_request_id {
            return Some(O::SettlementExecutionPreparationDecisionRequestIdMismatch);
        }
        if decision.settlement_execution_preparation_digest != inputs.expected_settlement_execution_preparation_intent_digest {
            return Some(O::SettlementExecutionPreparationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_settlement_execution_preparation_transcript_digest {
            return Some(O::SettlementExecutionPreparationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.settlement_execution_preparation_digest {
            return Some(O::SettlementExecutionPreparationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed settlement-execution-preparation
        // nonce binding.
        if intent.settlement_execution_preparation_nonce != inputs.expected_settlement_execution_preparation_nonce {
            return Some(O::WrongSettlementExecutionPreparationNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 337/338
        // settlement-execution-preparation artifact).
        ...
        // authority tuple binding (carried through the consumed Run 337/338
        // settlement-execution-preparation artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition settlement-execution
        ...
        // Re-exposed Run 337/338 staged-application decision authority tuple
        ...
        // Re-exposed Run 337/338 live-authorization decision authority tuple
        // binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionSettlementExecutionReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionSettlementExecutionRequest,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_settlement_execution_request_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_preparation_digest,
            &inputs.settlement_execution_policy_id,
            request.proposed_epoch_transition_target,
            request.settlement_execution_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionSettlementExecutionKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionSettlementExecutionArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            settlement_execution_policy_id: inputs.settlement_execution_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded settlement-execution decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded settlement-execution nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 337/338 runtime handoff decision transcript (the
            // authority source this run consumes).
            settlement_execution_preparation_decision_id: decision.settlement_execution_preparation_id.clone(),
            settlement_execution_preparation_request_id: decision.request_id.clone(),
            settlement_execution_preparation_intent_digest: decision.settlement_execution_preparation_digest.clone(),
            settlement_execution_preparation_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed settlement-execution-preparation nonce.
            settlement_execution_preparation_nonce: application_intent.settlement_execution_preparation_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed settlement-execution-preparation artifact).
            ...
            // (carried through the consumed settlement-execution-preparation artifact).
            ...
            // Newly proposed settlement-execution nonce.
            settlement_execution_nonce: request.settlement_execution_nonce,
            // Exact future-executor preconditions.
            ...
            settlement_execution_id: String::new(),
            request_id: String::new(),
            settlement_execution_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionSettlementExecution {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            settlement_execution_nonce: request.settlement_execution_nonce,
        };
        let settlement_execution_id = production_live_epoch_transition_settlement_execution_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_preparation_digest,
            &inputs.settlement_execution_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_settlement_execution_request_id(
            self.config.protocol_version.0,
            &decision.settlement_execution_preparation_digest,
            &inputs.settlement_execution_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.settlement_execution_digest = record.content_digest();
        record.settlement_execution_id = settlement_execution_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_settlement_execution_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.settlement_execution_digest,
                outcome.tag(),
            );
    ...
    /// Run 339 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_settlement_execution<
        R: LiveEpochTransitionSettlementExecutionReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionSettlementExecutionRequest,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionSettlementExecutionDecision {
                outcome,
                settlement_execution_id: pkg.settlement_execution_id.clone(),
                request_id: pkg.request_id.clone(),
                settlement_execution_artifact: record.clone(),
                settlement_execution_digest: pkg.settlement_execution_digest.clone(),
        ...
        let settlement_execution_preparation_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionSettlementExecutionAuthoritySource::AcceptedSettlementExecutionPreparationWithoutPackage {
                decision,
            } => decision.settlement_execution_preparation_digest.clone(),
            _ => String::new(),
        };

        let settlement_execution_id = production_live_epoch_transition_settlement_execution_id(
            self.config.protocol_version.0,
            &settlement_execution_preparation_intent_digest,
            &inputs.settlement_execution_policy_id,
            request.proposed_epoch_transition_target,
            request.settlement_execution_nonce,
        );
        ...
        let settlement_execution_digest = String::new();
        ...
        ProductionLiveEpochTransitionSettlementExecutionDecision {
            ...
            request_id,
            settlement_execution_artifact: None,
            settlement_execution_digest,
            transcript_digest,
        }
    }

    /// Run 339 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_settlement_execution_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionSettlementExecutionArtifact>,
        current: &ProductionLiveEpochTransitionSettlementExecutionArtifact,
    ) -> ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome {
        use ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementExecutionExecutorKind::Disabled
        {
...
/// Run 339 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_settlement_execution_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::default()
        == ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionSettlementExecutionConfig::default().kind
            == ProductionLiveEpochTransitionSettlementExecutionExecutorKind::Disabled
}

/// Run 339 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 340).
pub fn production_live_epoch_transition_settlement_execution_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 339 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_settlement_execution_executor_mainnet_refused() -> bool {
    true
}

/// Run 339 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_settlement_execution_executor_is_non_mutating() -> bool {
    true
}

/// Run 339 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_settlement_execution_executor_never_falls_back() -> bool {
    true
}

/// Run 339 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_settlement_execution_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 339 — the executor only requires a verified Run 337/338 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_settlement_execution_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 339 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 339. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionSettlementExecutionFixtureState {
...
impl LiveEpochTransitionSettlementExecutionFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, settlement_execution_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == settlement_execution_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `settlement_execution_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionSettlementExecutionArtifact,
        settlement_execution_id: &str,
    ) -> bool {
        if self.has_applied(settlement_execution_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(settlement_execution_id.to_string());
        true
    }
}