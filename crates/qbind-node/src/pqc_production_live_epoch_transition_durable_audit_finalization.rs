//! Run 327 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 323/324 guarded epoch-transition durable-audit-finalization boundary: the boundary
//! that consumes a **verified** Run 325/326 non-mutating guarded
//! epoch-transition durable-audit-finalization *decision* (the accepted
//! [`ProductionLiveEpochTransitionPostCommitAuditDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionPostCommitAuditArtifact`], as
//! release-binary-evidenced by Run 324) and translates it into a typed,
...
//!
//! Where the Run 323 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 327 answers the next question:
//! "given a verified guarded durable-audit-finalization decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a post-commit-audit
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 327)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 328**.
...
//! * Only a **verified** Run 325/326 guarded epoch-transition
//!   durable-audit-finalization decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionPostCommitAuditArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 321/322 staged-application decision alone, a Run
//!   319/320 live-application authorization alone, a Run 317/318 application
//!   decision alone, Run 315 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionDurableAuditFinalizationArtifact`]; it
...
use crate::pqc_production_live_epoch_transition_post_commit_audit::{
...
/// Run 327 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_PROTOCOL_VERSION: u16 = 1;

/// Run 327 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:327-epoch-transition-post-commit-audit-intent:v1";

/// Run 327 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_ID_DOMAIN_TAG: &str =
    "QBIND:327-epoch-transition-post-commit-audit-id:v1";

/// Run 327 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:327-epoch-transition-post-commit-audit-request:v1";

/// Run 327 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:327-epoch-transition-post-commit-audit-transcript:v1";
...
/// Run 327 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionDurableAuditFinalizationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionDurableAuditFinalizationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionDurableAuditFinalization` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 325/326 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionDurableAuditFinalization` and
/// `MainnetProductionLiveEpochTransitionDurableAuditFinalizationRequired` are
...
pub enum ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy {
...
impl ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionDurableAuditFinalization => {
                "allow-source-test-epoch-transition-post-commit-audit"
            }
            Self::RequireProductionLiveEpochTransitionDurableAuditFinalization => {
                "require-production-epoch-transition-post-commit-audit"
            }
            Self::MainnetProductionLiveEpochTransitionDurableAuditFinalizationRequired => {
                "mainnet-production-epoch-transition-post-commit-audit-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionDurableAuditFinalization` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionDurableAuditFinalization` kind is fail-closed as
/// unavailable in Run 327 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 327.
    ProductionLiveEpochTransitionDurableAuditFinalization,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionDurableAuditFinalization => {
                "source-test-epoch-transition-post-commit-audit"
            }
            Self::ProductionLiveEpochTransitionDurableAuditFinalization => {
                "production-epoch-transition-post-commit-audit"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionDurableAuditFinalizationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionDurableAuditFinalizationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::SourceTestLiveEpochTransitionDurableAuditFinalization,
        )
    }
...
impl Default for ProductionLiveEpochTransitionDurableAuditFinalizationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::Disabled)
    }
}
...
/// Run 327 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 325/326
/// [`LiveEpochTransitionPostCommitAuditKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionDurableAuditFinalizationKind {
...
impl LiveEpochTransitionDurableAuditFinalizationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 327 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 325/326 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionPostCommitAuditKind,
    ) -> Self {
        use LiveEpochTransitionPostCommitAuditKind as A;
        match kind {
...
/// Run 327 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedPostCommitAuditDecision`] carrying a Run 325/326
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionDurableAuditFinalizationAuthoritySource {
    /// A verified Run 325/326 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedPostCommitAuditDecision {
        decision: ProductionLiveEpochTransitionPostCommitAuditDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingPostCommitAuditDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedPostCommitAuditDecision {
        decision: ProductionLiveEpochTransitionPostCommitAuditDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedPostCommitAuditWithoutPackage {
        decision: ProductionLiveEpochTransitionPostCommitAuditDecision,
    },
    /// A Run 323/324 epoch-transition commit-authorization decision presented
    /// directly, without a Run 325/326 post-commit-audit decision. Rejected.
    CommitAuthorizationDecisionWithoutPostCommitAudit,
    /// A Run 323/324 epoch-transition mutation-execution decision presented
    /// directly, without a Run 325/326 post-commit-audit decision. Rejected.
    MutationExecutionDecisionWithoutPostCommitAudit,
    /// A Run 323/324 epoch-transition execution-preparation decision presented
    /// directly, without a Run 325/326 post-commit-audit decision. Rejected.
    ExecutionPreparationDecisionWithoutPostCommitAudit,
    /// A Run 323/324 epoch-transition runtime handoff decision presented
    /// directly, without a Run 325/326 post-commit-audit decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutPostCommitAudit,
    /// A Run 323/324 guarded epoch-transition durable-audit-finalization decision
    /// presented directly, without a Run 325/326 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutPostCommitAudit,
    /// A Run 321/322 staged-application decision presented directly, without a
    /// Run 325/326 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutPostCommitAudit,
    /// A Run 319/320 live-application authorization presented directly, without
    /// a Run 325/326 guarded durable-audit-finalization decision. Rejected.
    LiveApplicationAuthorizationWithoutPostCommitAudit,
    /// A Run 317/318 validator-set rotation *application decision* presented
    /// directly, without a Run 325/326 guarded durable-audit-finalization decision.
    /// Rejected.
    ApplicationDecisionWithoutPostCommitAudit,
    /// A Run 315/316 validator-set rotation plan presented directly, without a
    /// Run 325/326 live-application authorization. Rejected.
    RotationPlanWithoutPostCommitAudit,
    /// A Run 313/314 governance execution intent presented directly, without a
    /// Run 325/326 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutPostCommitAudit,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 319/320 live-application authorization. Rejected.
    GovernanceProofWithoutPostCommitAudit,
...
/// Run 327 — the explicit trusted inputs the executor binds a verified
/// Run 325/326 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationInputs {
    ...
    pub durable_audit_finalization_policy_id: String,
    /// Expected Run 325/326 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 317/318 application policy id (re-exposed by the
    ...
    /// Expected Run 313 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 313 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 313 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 315 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 315 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 315 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 315 rotation plan digest.
    ...
    /// Expected Run 317/318 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 317/318 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 317/318 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 317/318 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 325/326 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 325/326 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 325/326 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 325/326 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 325/326 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 325/326 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 325/326 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 325/326 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 325/326 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 317/318 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 325/326 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 323/324 guarded durable-audit-finalization decision id (re-exposed
    /// by the consumed Run 325/326 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 323/324 guarded durable-audit-finalization request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 323/324 guarded durable-audit-finalization intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 323/324 guarded durable-audit-finalization transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 323/324 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 325/326 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_post_commit_audit_decision_id: String,
    /// Expected Run 325/326 runtime handoff request id.
    pub expected_post_commit_audit_request_id: String,
    /// Expected Run 325/326 runtime handoff package/content digest (the
    /// consumed decision's `durable_audit_finalization_digest`, i.e. the handoff content
    /// digest).
    pub expected_post_commit_audit_intent_digest: String,
    /// Expected Run 325/326 runtime handoff transcript digest.
    pub expected_post_commit_audit_transcript_digest: String,
    /// Expected Run 325/326 post-commit-audit nonce (re-exposed by the
    /// consumed Run 325/326 post-commit-audit artifact).
    pub expected_post_commit_audit_nonce: u64,
    /// Expected Run 323/324 commit-authorization decision id (re-exposed by
    /// the consumed Run 325/326 post-commit-audit artifact).
    pub expected_commit_authorization_decision_id: String,
    /// Expected Run 323/324 commit-authorization request id.
    pub expected_commit_authorization_request_id: String,
    /// Expected Run 323/324 commit-authorization intent/content digest.
    pub expected_commit_authorization_intent_digest: String,
    /// Expected Run 323/324 commit-authorization transcript digest.
    pub expected_commit_authorization_transcript_digest: String,
    /// Expected Run 323/324 commit-authorization nonce (re-exposed by the
    /// consumed Run 325/326 post-commit-audit artifact).
    pub expected_commit_authorization_nonce: u64,
    /// Expected Run 323/324 mutation-execution decision id (re-exposed by
    /// the consumed Run 325/326 post-commit-audit artifact).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 323/324 mutation-execution request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 323/324 mutation-execution intent/content digest.
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 323/324 mutation-execution transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 323/324 mutation-execution nonce (re-exposed by the
    /// consumed Run 325/326 post-commit-audit artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 323/324 execution-preparation decision id (re-exposed by
    /// the consumed Run 325/326 post-commit-audit artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 323/324 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 323/324 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 323/324 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 323/324 execution-preparation nonce (re-exposed by the
    /// consumed Run 325/326 post-commit-audit artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 323/324 runtime handoff decision id (re-exposed by the
    /// consumed Run 325/326 post-commit-audit artifact).
    ...
    /// Expected Run 323/324 runtime handoff package/content digest.
    ...
    /// Expected Run 323/324 runtime handoff nonce (re-exposed by the consumed
    /// Run 325/326 post-commit-audit artifact).
    ...
    /// wall-clock value and never a reject path in Run 327.
...
impl ProductionLiveEpochTransitionDurableAuditFinalizationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.durable_audit_finalization_policy_id.is_empty()
            ...
            && !self.expected_post_commit_audit_decision_id.is_empty()
            && !self.expected_post_commit_audit_request_id.is_empty()
            && !self.expected_post_commit_audit_intent_digest.is_empty()
            && !self.expected_post_commit_audit_transcript_digest.is_empty()
...
/// Run 327 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition durable-audit-finalization decision), the explicit
/// epoch-transition target, a post-commit-audit nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationRequest {
    pub authority_source: LiveEpochTransitionDurableAuditFinalizationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The post-commit-audit nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionDurableAuditFinalizationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and post-commit-audit nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionDurableAuditFinalizationAuthoritySource,
        proposed_epoch_transition_target: u64,
        durable_audit_finalization_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 327 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionDurableAuditFinalizationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionDurableAuditFinalizationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionDurableAuditFinalizationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionDurableAuditFinalizationReplaySet;

impl LiveEpochTransitionDurableAuditFinalizationReplaySet
    for EmptyLiveEpochTransitionDurableAuditFinalizationReplaySet
{
...
/// Run 327 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 328+); Run 327 never
...
/// preconditions, and carries deterministic `durable_audit_finalization_id`, `request_id`,
/// `durable_audit_finalization_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationArtifact {
    pub staged_kind: LiveEpochTransitionDurableAuditFinalizationKind,
    pub protocol_version: u16,
    pub durable_audit_finalization_policy_id: String,

    // ---- Re-exposed Run 325/326 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 317/318 application-decision tuple ------------
    ...
    // ---- Bound Run 325/326 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 325/326 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 325/326 guarded durable-audit-finalization authority tuple -
    // (the consumed guarded durable-audit-finalization decision transcript)
    ...
    // ---- Guarded durable-audit-finalization binding (re-exposed consumed nonce)
    /// The re-exposed Run 323/324 guarded durable-audit-finalization nonce.
    ...
    pub post_commit_audit_decision_id: String,
    pub post_commit_audit_request_id: String,
    pub post_commit_audit_intent_digest: String,
    pub post_commit_audit_transcript_digest: String,
    /// The re-exposed Run 325/326 post-commit-audit nonce (consumed).
    pub post_commit_audit_nonce: u64,

    // ---- Bound Run 323/324 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 325/326 post-commit-audit artifact)
    ...
    /// The re-exposed Run 323/324 commit-authorization nonce (consumed).
    pub commit_authorization_nonce: u64,

    // ---- Bound Run 323/324 mutation-execution authority tuple -------
    // (re-exposed by the consumed Run 325/326 post-commit-audit artifact)
    ...
    /// The re-exposed Run 323/324 mutation-execution nonce (consumed).
    pub mutation_execution_nonce: u64,

    // ---- Bound Run 323/324 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 325/326 post-commit-audit artifact)
    ...
    /// The re-exposed Run 323/324 execution-preparation nonce (consumed).
    ...
    /// The re-exposed Run 323/324 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed durable-audit-finalization nonce.
    pub durable_audit_finalization_nonce: u64,
    ...
    pub durable_audit_finalization_id: String,
    pub request_id: String,
    pub durable_audit_finalization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`durable_audit_finalization_id`,
    /// `request_id`, `durable_audit_finalization_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"durable_audit_finalization_policy_id",
            self.durable_audit_finalization_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"post_commit_audit_decision_id",
            self.post_commit_audit_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_commit_audit_request_id",
            self.post_commit_audit_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_commit_audit_intent_digest",
            self.post_commit_audit_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"post_commit_audit_transcript_digest",
            self.post_commit_audit_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
...
/// Custody binding canonical hashing (module-local; mirrors Run 313/315/317
...
/// Run 327 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_durable_audit_finalization_content_digest(
    package: &ProductionLiveEpochTransitionDurableAuditFinalizationArtifact,
) -> String {
    package.content_digest()
}

/// Run 327 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and post-commit-audit nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_durable_audit_finalization_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    durable_audit_finalization_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"durable_audit_finalization_policy_id",
        durable_audit_finalization_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"durable_audit_finalization_nonce",
        &durable_audit_finalization_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 327 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and post-commit-audit nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_durable_audit_finalization_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    durable_audit_finalization_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"durable_audit_finalization_policy_id", durable_audit_finalization_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 327 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_durable_audit_finalization_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_FINALIZATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 327 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionDurableAuditFinalization`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionDurableAuditFinalizationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionDurableAuditFinalizationBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionDurableAuditFinalizationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition durable-audit-finalization
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionDurableAuditFinalization {
        execution_kind: LiveEpochTransitionDurableAuditFinalizationKind,
    ...
    VerifiedPostCommitAuditDecisionRequired,
    UnverifiedPostCommitAuditDecisionRejected,
    ...
    PostCommitAuditDecisionIdMismatch,
    PostCommitAuditDecisionRequestIdMismatch,
    PostCommitAuditDecisionIntentDigestMismatch,
    PostCommitAuditDecisionTranscriptMismatch,
    PostCommitAuditDecisionIntegrityMismatch,
    WrongPostCommitAuditNonce,
    ...
    LiveEpochTransitionDurableAuditFinalizationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionDurableAuditFinalizationBoundaryUnavailable => {
                "live-epoch-transition-durable-audit-finalization-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionDurableAuditFinalizationUnavailable => {
                "production-live-epoch-transition-durable-audit-finalization-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionDurableAuditFinalizationUnavailable => {
                "mainnet-production-live-epoch-transition-durable-audit-finalization-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionDurableAuditFinalization { .. } => {
                "accepted-source-test-live-epoch-transition-durable-audit-finalization"
            }
            Self::VerifiedPostCommitAuditDecisionRequired => {
                "verified-post-commit-audit-decision-required"
            }
            Self::UnverifiedPostCommitAuditDecisionRejected => {
                "unverified-post-commit-audit-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-post-commit-audit-decision-rejected-as-production-authority"
            }
            ...
            Self::PostCommitAuditDecisionIdMismatch => "post-commit-audit-decision-id-mismatch",
            Self::PostCommitAuditDecisionRequestIdMismatch => {
                "post-commit-audit-decision-request-id-mismatch"
            }
            Self::PostCommitAuditDecisionIntentDigestMismatch => {
                "post-commit-audit-decision-intent-digest-mismatch"
            }
            Self::PostCommitAuditDecisionTranscriptMismatch => {
                "post-commit-audit-decision-transcript-mismatch"
            }
            Self::PostCommitAuditDecisionIntegrityMismatch => {
                "post-commit-audit-decision-integrity-mismatch"
            }
            Self::WrongPostCommitAuditNonce => "wrong-post-commit-audit-nonce",
            ...
            Self::LiveEpochTransitionDurableAuditFinalizationAmbiguous { .. } => {
                "epoch-transition-post-commit-audit-ambiguous"
            }
...
/// Run 327 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationDecision {
    pub outcome: ProductionLiveEpochTransitionDurableAuditFinalizationOutcome,
    pub durable_audit_finalization_id: String,
    pub request_id: String,
    pub durable_audit_finalization_artifact:
        Option<ProductionLiveEpochTransitionDurableAuditFinalizationArtifact>,
    pub durable_audit_finalization_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.durable_audit_finalization_artifact.is_some()
    }
}
...
/// Run 327 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionDurableAuditFinalizationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionDurableAuditFinalizationRecoveryOutcome {
...
/// Run 327 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditFinalizationExecutor {
    pub config: ProductionLiveEpochTransitionDurableAuditFinalizationConfig,
    pub policy: ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy,
}

impl ProductionLiveEpochTransitionDurableAuditFinalizationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionDurableAuditFinalizationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionDurableAuditFinalizationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionPostCommitAuditDecision,
            &'a ProductionLiveEpochTransitionPostCommitAuditArtifact,
        ),
        ProductionLiveEpochTransitionDurableAuditFinalizationOutcome,
    > {
        use ProductionLiveEpochTransitionDurableAuditFinalizationOutcome as O;
        use LiveEpochTransitionDurableAuditFinalizationAuthoritySource as S;
        match source {
            S::VerifiedPostCommitAuditDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedPostCommitAuditDecisionRejected);
                }
                match &decision.post_commit_audit_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedPostCommitAuditDecisionRequired),
                }
            }
            S::MissingPostCommitAuditDecision => {
                Err(O::VerifiedPostCommitAuditDecisionRequired)
            }
            S::UnverifiedPostCommitAuditDecision { .. } => {
                Err(O::UnverifiedPostCommitAuditDecisionRejected)
            }
            S::AcceptedPostCommitAuditWithoutPackage { .. } => {
                Err(O::VerifiedPostCommitAuditDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutPostCommitAudit => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutPostCommitAudit => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutPostCommitAudit => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutPostCommitAudit => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutPostCommitAudit => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutPostCommitAudit => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutPostCommitAudit => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutPostCommitAudit => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutPostCommitAudit => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutPostCommitAudit => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutPostCommitAudit => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionDurableAuditFinalizationOutcome> {
        use ProductionLiveEpochTransitionDurableAuditFinalizationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy::MainnetProductionLiveEpochTransitionDurableAuditFinalizationRequired => {
                    O::MainNetProductionLiveEpochTransitionDurableAuditFinalizationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionDurableAuditFinalizationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionDurableAuditFinalizationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 327.
        if self.config.kind
            == ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::ProductionLiveEpochTransitionDurableAuditFinalization
        {
            return Some(O::LiveEpochTransitionDurableAuditFinalizationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 325/326 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionPostCommitAuditDecision,
        intent: &ProductionLiveEpochTransitionPostCommitAuditArtifact,
        inputs: &ProductionLiveEpochTransitionDurableAuditFinalizationInputs,
    ) -> Option<ProductionLiveEpochTransitionDurableAuditFinalizationOutcome> {
        use ProductionLiveEpochTransitionDurableAuditFinalizationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 325/326 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.post_commit_audit_id != inputs.expected_post_commit_audit_decision_id {
            return Some(O::PostCommitAuditDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_post_commit_audit_request_id {
            return Some(O::PostCommitAuditDecisionRequestIdMismatch);
        }
        if decision.post_commit_audit_digest != inputs.expected_post_commit_audit_intent_digest {
            return Some(O::PostCommitAuditDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_post_commit_audit_transcript_digest {
            return Some(O::PostCommitAuditDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.post_commit_audit_digest {
            return Some(O::PostCommitAuditDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed post-commit-audit
        // nonce binding.
        if intent.post_commit_audit_nonce != inputs.expected_post_commit_audit_nonce {
            return Some(O::WrongPostCommitAuditNonce);
        }

        // Re-exposed Run 323/324 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 325/326
        // post-commit-audit artifact).
        ...
        // Re-exposed Run 323/324 mutation-execution decision authority
        // tuple binding (carried through the consumed Run 325/326
        // post-commit-audit artifact).
        ...
        // Re-exposed Run 323/324 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 325/326
        // post-commit-audit artifact).
        ...
        // Re-exposed Run 323/324 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 325/326
        // post-commit-audit artifact).
        ...
        // Re-exposed Run 323/324 guarded epoch-transition durable-audit-finalization
        ...
        // Re-exposed Run 325/326 staged-application decision authority tuple
        ...
        // Re-exposed Run 325/326 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 317/318 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionDurableAuditFinalizationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionDurableAuditFinalizationRequest,
        inputs: &ProductionLiveEpochTransitionDurableAuditFinalizationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_durable_audit_finalization_request_id(
            self.config.protocol_version.0,
            &decision.post_commit_audit_digest,
            &inputs.durable_audit_finalization_policy_id,
            request.proposed_epoch_transition_target,
            request.durable_audit_finalization_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionDurableAuditFinalizationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionDurableAuditFinalizationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            durable_audit_finalization_policy_id: inputs.durable_audit_finalization_policy_id.clone(),
            ...
            // Re-exposed Run 323/324 guarded durable-audit-finalization decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded durable-audit-finalization nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 325/326 runtime handoff decision transcript (the
            // authority source this run consumes).
            post_commit_audit_decision_id: decision.post_commit_audit_id.clone(),
            post_commit_audit_request_id: decision.request_id.clone(),
            post_commit_audit_intent_digest: decision.post_commit_audit_digest.clone(),
            post_commit_audit_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed post-commit-audit nonce.
            post_commit_audit_nonce: application_intent.post_commit_audit_nonce,
            // Re-exposed Run 323/324 commit-authorization decision authority
            // tuple (carried through the consumed post-commit-audit artifact).
            ...
            // Re-exposed Run 323/324 runtime handoff decision authority tuple
            // (carried through the consumed post-commit-audit artifact).
            ...
            // Newly proposed durable-audit-finalization nonce.
            durable_audit_finalization_nonce: request.durable_audit_finalization_nonce,
            // Exact future-executor preconditions.
            ...
            durable_audit_finalization_id: String::new(),
            request_id: String::new(),
            durable_audit_finalization_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionDurableAuditFinalization {
        ...
        record.durable_audit_finalization_digest = record.content_digest();
        record.durable_audit_finalization_id = durable_audit_finalization_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_durable_audit_finalization_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.durable_audit_finalization_digest,
                outcome.tag(),
            );
    ...
    /// Run 327 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_durable_audit_finalization<
        R: LiveEpochTransitionDurableAuditFinalizationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionDurableAuditFinalizationRequest,
        inputs: &ProductionLiveEpochTransitionDurableAuditFinalizationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionDurableAuditFinalizationDecision {
                outcome,
                durable_audit_finalization_id: pkg.durable_audit_finalization_id.clone(),
                request_id: pkg.request_id.clone(),
                durable_audit_finalization_artifact: record.clone(),
                durable_audit_finalization_digest: pkg.durable_audit_finalization_digest.clone(),
        ...
        let post_commit_audit_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionDurableAuditFinalizationAuthoritySource::AcceptedPostCommitAuditWithoutPackage {
                decision,
            } => decision.post_commit_audit_digest.clone(),
            _ => String::new(),
        };
        ...
        let durable_audit_finalization_digest = String::new();
        ...
        ProductionLiveEpochTransitionDurableAuditFinalizationDecision {
            outcome,
            durable_audit_finalization_id,
            request_id,
            durable_audit_finalization_artifact: None,
            durable_audit_finalization_digest,
            transcript_digest,
        }
    }

    /// Run 327 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_durable_audit_finalization_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionDurableAuditFinalizationArtifact>,
        current: &ProductionLiveEpochTransitionDurableAuditFinalizationArtifact,
    ) -> ProductionLiveEpochTransitionDurableAuditFinalizationRecoveryOutcome {
        use ProductionLiveEpochTransitionDurableAuditFinalizationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::Disabled
        {
...
/// Run 327 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_durable_audit_finalization_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy::default()
        == ProductionLiveEpochTransitionDurableAuditFinalizationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionDurableAuditFinalizationConfig::default().kind
            == ProductionLiveEpochTransitionDurableAuditFinalizationExecutorKind::Disabled
}

/// Run 327 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 328).
pub fn production_live_epoch_transition_durable_audit_finalization_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 327 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_durable_audit_finalization_executor_mainnet_refused() -> bool {
    true
}

/// Run 327 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_durable_audit_finalization_executor_is_non_mutating() -> bool {
    true
}

/// Run 327 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_durable_audit_finalization_executor_never_falls_back() -> bool {
    true
}

/// Run 327 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_durable_audit_finalization_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 327 — the executor only requires a verified Run 325/326 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_durable_audit_finalization_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 327 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 327. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionDurableAuditFinalizationFixtureState {
...
impl LiveEpochTransitionDurableAuditFinalizationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, durable_audit_finalization_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == durable_audit_finalization_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `durable_audit_finalization_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionDurableAuditFinalizationArtifact,
        durable_audit_finalization_id: &str,
    ) -> bool {
        if self.has_applied(durable_audit_finalization_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(durable_audit_finalization_id.to_string());
        true
    }
}