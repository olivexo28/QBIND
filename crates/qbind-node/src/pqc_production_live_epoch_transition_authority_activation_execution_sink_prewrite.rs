//! Run 353 — source/test live epoch-transition **authority-activation
//! final-execution** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 351/352 live epoch-transition authority-activation execution-preparation /
//! final-execution preflight boundary: the boundary that consumes a
//! **verified** Run 351/352 non-mutating live epoch-transition
//! authority-activation-post-final-execution-confirmation *decision* (the accepted
//! [`ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact`], as
//! source-test-evidenced by Run 351/352) and translates it into a typed,
//! deterministic, policy-gated **authority-activation final-execution
//! artifact** for a *future* production authority-activation / final-execution
//! sink — *without* ever activating authority state, executing final
//! settlement, executing final-execution, transitioning a consensus epoch,
//! applying a live validator-set change, or mutating any trust state.
//!
//! The name is deliberate: this is **typed authority-activation final-execution
//! artifact generation**, *not* runtime authority activation, *not* production
//! final execution, *not* settlement execution, *not* final-settlement
//! completion, and *not* production mutation. The artifact only *describes*
//! exactly what a future production final-execution sink would perform after
//! re-verification; it never applies a validator set, transitions an epoch,
//! activates authority state, executes final settlement, executes
//! final-execution, publishes anything, or mutates runtime state. Where the Run
//! 351/352 boundary answers "given a verified authority-activation-authorization
//! decision, what typed non-mutating authority-activation execution-preparation
//! / final-execution preflight artifact does it authorize?", Run 353 answers the
//! next question: "given a verified authority-activation execution-preparation /
//! final-execution preflight decision, what typed, non-mutating
//! **authority-activation final-execution artifact** carrying the exact
//! preconditions and postconditions a *future* production final-execution sink
//! must re-verify does it authorize, under an explicit authority-activation
//! final-execution policy, bound to the full execution-preparation-decision /
//! settlement / commit / audit / publication / governance / validator-set /
//! custody / attestation / durable-replay evidence tuple, an explicit
//! epoch-transition target, current/proposed validator-set epoch/version
//! preconditions, a required replay window, and a
//! authority-activation-execution-sink-prewrite nonce?".
//!
//! ## Scope and honesty constraints (Run 353)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 354**.
...
//!   preflight-artifact construction.
//! * Only a **verified** Run 351/352 live epoch-transition
//!   authority-activation-post-final-execution-confirmation / final-execution-readiness decision
//!   that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact`] can authorize an
//!   authority-activation final-execution
//!   artifact. Unverified decisions, accepted
//!   decisions without a prepared artifact, a post-completion-attestation
//!   decision alone, a settlement-execution decision
//!   alone, a settlement-preparation decision alone, an external-publication
//!   decision alone, a commit-receipt decision alone, a governance execution
//!   intent alone, governance proof alone, local-operator assertions,
//!   peer-majority assertions, custody-only, RemoteSigner-only,
//!   custody-attestation-only, fixture-only decisions, and arbitrary
//!   validator-set bytes are all rejected as production authority.
//! * The boundary produces only a typed
//!   [`ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact`]; it
//!   **never** applies the decision, never activates authority state, never
//!   executes final-execution, never mutates a live validator set, never writes
...
//!   block. Only a typed accepted outcome may authorize a *future* production
//!   authority-activation final-execution
//!   sink.
//! * MainNet remains **refused**: even a fully valid source/test
//!   DevNet/TestNet preflight artifact does not enable MainNet runtime
//!   behavior.
//! * The boundary is **non-mutating**: no Run 70 apply, no
...
//!   fallback, no settlement, no authority-lifecycle completion, no
//!   authority activation, no final execution, no external publication, and no
//!   default runtime wiring.
//! * No CLI flag and no default runtime wiring is added. Full C4 remains
//!   OPEN; C5 remains OPEN.
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_351.md`.
...
use crate::pqc_production_live_epoch_transition_authority_activation_post_final_execution_confirmation::{
...
/// Run 353 — the only live validator-set application authorization boundary
...
/// Run 353 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_INTENT_DOMAIN_TAG: &str =
    "QBIND:353-epoch-transition-authority-activation-execution-sink-prewrite-intent:v1";

/// Run 353 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_ID_DOMAIN_TAG: &str =
    "QBIND:353-epoch-transition-authority-activation-execution-sink-prewrite-id:v1";

/// Run 353 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:353-epoch-transition-authority-activation-execution-sink-prewrite-request:v1";

/// Run 353 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:353-epoch-transition-authority-activation-execution-sink-prewrite-transcript:v1";
...
/// Run 353 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_AUTHORITY_ACTIVATION_FINAL_EXECUTION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 351/352 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite` and
/// `MainnetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequired` are
...
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy {
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite => {
                "allow-source-test-epoch-transition-authority-activation-post-final-execution-confirmation"
            }
            Self::RequireProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite => {
                "require-production-epoch-transition-authority-activation-post-final-execution-confirmation"
            }
            Self::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequired => {
                "mainnet-production-epoch-transition-authority-activation-post-final-execution-confirmation-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite` kind is fail-closed as
/// unavailable in Run 353 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 353.
    ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite => {
                "source-test-epoch-transition-authority-activation-post-final-execution-confirmation"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite => {
                "production-epoch-transition-authority-activation-post-final-execution-confirmation"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig {
    pub fn new(kind: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind::SourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite,
        )
    }
...
impl Default for ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind::Disabled)
    }
}
...
/// Run 353 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 351/352
/// [`LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind {
...
impl LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 353 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 351/352 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind,
    ) -> Self {
        use LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationKind as A;
        match kind {
...
/// Run 353 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecision`] carrying a Run 351/352
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource {
    /// A verified Run 351/352 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedAuthorityActivationPostFinalExecutionConfirmationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingAuthorityActivationPostFinalExecutionConfirmationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecision {
        decision: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedAuthorityActivationPostFinalExecutionConfirmationWithoutPackage {
        decision: ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision,
    },
    /// A Run 331/332 epoch-transition commit-authorization decision presented
    /// directly, without a Run 351/352 authority-activation-post-final-execution-confirmation decision. Rejected.
    CommitAuthorizationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 331/332 epoch-transition mutation-execution decision presented
    /// directly, without a Run 351/352 authority-activation-post-final-execution-confirmation decision. Rejected.
    MutationExecutionDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 331/332 epoch-transition execution-preparation decision presented
    /// directly, without a Run 351/352 authority-activation-post-final-execution-confirmation decision. Rejected.
    ExecutionPreparationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 331/332 epoch-transition runtime handoff decision presented
    /// directly, without a Run 351/352 authority-activation-post-final-execution-confirmation decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 331/332 guarded epoch-transition authority-activation-execution-sink-prewrite decision
    /// presented directly, without a Run 351/352 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 329/330 staged-application decision presented directly, without a
    /// Run 351/352 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 327/328 live-application authorization presented directly, without
    /// a Run 351/352 guarded authority-activation-execution-sink-prewrite decision. Rejected.
    LiveApplicationAuthorizationWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 325/326 validator-set rotation *application decision* presented
    /// directly, without a Run 351/352 guarded authority-activation-execution-sink-prewrite decision.
    /// Rejected.
    ApplicationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 323/324 validator-set rotation plan presented directly, without a
    /// Run 351/352 live-application authorization. Rejected.
    RotationPlanWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A Run 321/322 governance execution intent presented directly, without a
    /// Run 351/352 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutAuthorityActivationPostFinalExecutionConfirmation,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 327/328 live-application authorization. Rejected.
    GovernanceProofWithoutAuthorityActivationPostFinalExecutionConfirmation,
    ...
    FixtureOnlyAuthorityActivationPostFinalExecutionConfirmation,
...
/// Run 353 — the explicit trusted inputs the executor binds a verified
/// Run 351/352 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs {
    ...
    pub authority_activation_execution_sink_prewrite_policy_id: String,
    /// Expected Run 351/352 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 325/326 application policy id (re-exposed by the
    ...
    /// Expected Run 321 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 321 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 321 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 323 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 323 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 323 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 323 rotation plan digest.
    ...
    /// Expected Run 325/326 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 325/326 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 325/326 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 325/326 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 351/352 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 351/352 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 351/352 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 351/352 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 351/352 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 351/352 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 351/352 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 351/352 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 351/352 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 325/326 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 351/352 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 331/332 guarded authority-activation-execution-sink-prewrite decision id (re-exposed
    /// by the consumed Run 351/352 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 331/332 guarded authority-activation-execution-sink-prewrite request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 331/332 guarded authority-activation-execution-sink-prewrite intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 331/332 guarded authority-activation-execution-sink-prewrite transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 331/332 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 351/352 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_authority_activation_post_final_execution_confirmation_decision_id: String,
    /// Expected Run 351/352 runtime handoff request id.
    pub expected_authority_activation_post_final_execution_confirmation_request_id: String,
    /// Expected Run 351/352 runtime handoff package/content digest (the
    /// consumed decision's `authority_activation_execution_sink_prewrite_digest`, i.e. the handoff content
    /// digest).
    pub expected_authority_activation_post_final_execution_confirmation_intent_digest: String,
    /// Expected Run 351/352 runtime handoff transcript digest.
    pub expected_authority_activation_post_final_execution_confirmation_transcript_digest: String,
    /// Expected Run 351/352 authority-activation-post-final-execution-confirmation nonce (re-exposed by the
    /// consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_authority_activation_post_final_execution_confirmation_nonce: u64,
    /// Expected Run 331/332 commit-authorization decision id (re-exposed by
    /// the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_commit_authorization_decision_id: String,
    /// Expected Run 331/332 commit-authorization request id.
    pub expected_commit_authorization_request_id: String,
    /// Expected Run 331/332 commit-authorization intent/content digest.
    pub expected_commit_authorization_intent_digest: String,
    /// Expected Run 331/332 commit-authorization transcript digest.
    pub expected_commit_authorization_transcript_digest: String,
    /// Expected Run 331/332 commit-authorization nonce (re-exposed by the
    /// consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_commit_authorization_nonce: u64,
    /// Expected Run 331/332 mutation-execution decision id (re-exposed by
    /// the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 331/332 mutation-execution request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 331/332 mutation-execution intent/content digest.
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 331/332 mutation-execution transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 331/332 mutation-execution nonce (re-exposed by the
    /// consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 331/332 execution-preparation decision id (re-exposed by
    /// the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 331/332 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 331/332 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 331/332 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 331/332 execution-preparation nonce (re-exposed by the
    /// consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 331/332 runtime handoff decision id (re-exposed by the
    /// consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    pub expected_runtime_handoff_decision_id: String,
    /// Expected Run 331/332 runtime handoff request id.
    pub expected_runtime_handoff_request_id: String,
    /// Expected Run 331/332 runtime handoff package/content digest.
    pub expected_runtime_handoff_intent_digest: String,
    /// Expected Run 331/332 runtime handoff transcript digest.
    pub expected_runtime_handoff_transcript_digest: String,
    /// Expected Run 331/332 runtime handoff nonce (re-exposed by the consumed
    /// Run 351/352 authority-activation-post-final-execution-confirmation artifact).
    ...
    /// wall-clock value and never a reject path in Run 353.
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.authority_activation_execution_sink_prewrite_policy_id.is_empty()
            ...
            && !self.expected_authority_activation_post_final_execution_confirmation_decision_id.is_empty()
            && !self.expected_authority_activation_post_final_execution_confirmation_request_id.is_empty()
            && !self.expected_authority_activation_post_final_execution_confirmation_intent_digest.is_empty()
            && !self.expected_authority_activation_post_final_execution_confirmation_transcript_digest.is_empty()
...
/// Run 353 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition authority-activation-execution-sink-prewrite decision), the explicit
/// epoch-transition target, a authority-activation-post-final-execution-confirmation nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest {
    pub authority_source: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The authority-activation-post-final-execution-confirmation nonce (idempotency / replay binding).
    pub authority_activation_execution_sink_prewrite_nonce: u64,
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and authority-activation-post-final-execution-confirmation nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource,
        proposed_epoch_transition_target: u64,
        authority_activation_execution_sink_prewrite_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 353 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet;

impl LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet
    for EmptyLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet
{
...
/// Run 353 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 354+); Run 353 never
...
/// preconditions, and carries deterministic `authority_activation_execution_sink_prewrite_id`, `request_id`,
/// `authority_activation_execution_sink_prewrite_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact {
    pub staged_kind: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind,
    pub protocol_version: u16,
    pub authority_activation_execution_sink_prewrite_policy_id: String,

    // ---- Re-exposed Run 351/352 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 325/326 application-decision tuple ------------
    ...
    // ---- Bound Run 351/352 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 351/352 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 351/352 guarded authority-activation-execution-sink-prewrite authority tuple -
    // (the consumed guarded authority-activation-execution-sink-prewrite decision transcript)
    ...
    // ---- Guarded authority-activation-execution-sink-prewrite binding (re-exposed consumed nonce)
    /// The re-exposed Run 331/332 guarded authority-activation-execution-sink-prewrite nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 351/352 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub authority_activation_post_final_execution_confirmation_decision_id: String,
    pub authority_activation_post_final_execution_confirmation_request_id: String,
    pub authority_activation_post_final_execution_confirmation_intent_digest: String,
    pub authority_activation_post_final_execution_confirmation_transcript_digest: String,
    /// The re-exposed Run 351/352 authority-activation-post-final-execution-confirmation nonce (consumed).
    pub authority_activation_post_final_execution_confirmation_nonce: u64,

    // ---- Bound Run 331/332 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact)
    ...
    /// The re-exposed Run 331/332 commit-authorization nonce (consumed).
    pub commit_authorization_nonce: u64,

    // ---- Bound Run 331/332 mutation-execution authority tuple -------
    // (re-exposed by the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact)
    ...
    /// The re-exposed Run 331/332 mutation-execution nonce (consumed).
    pub mutation_execution_nonce: u64,

    // ---- Bound Run 331/332 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact)
    ...
    /// The re-exposed Run 331/332 execution-preparation nonce (consumed).
    pub execution_preparation_nonce: u64,

    // ---- Bound Run 331/332 runtime handoff decision authority tuple ----
    // (re-exposed by the consumed Run 351/352 authority-activation-post-final-execution-confirmation artifact)
    ...
    /// The re-exposed Run 331/332 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed authority-activation-execution-sink-prewrite nonce.
    pub authority_activation_execution_sink_prewrite_nonce: u64,
    ...
    pub authority_activation_execution_sink_prewrite_id: String,
    pub request_id: String,
    pub authority_activation_execution_sink_prewrite_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`authority_activation_execution_sink_prewrite_id`,
    /// `request_id`, `authority_activation_execution_sink_prewrite_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        ...
        hash_field(
            &mut h,
            b"authority_activation_execution_sink_prewrite_policy_id",
            self.authority_activation_execution_sink_prewrite_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"authority_activation_post_final_execution_confirmation_decision_id",
            self.authority_activation_post_final_execution_confirmation_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_post_final_execution_confirmation_request_id",
            self.authority_activation_post_final_execution_confirmation_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_post_final_execution_confirmation_intent_digest",
            self.authority_activation_post_final_execution_confirmation_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"authority_activation_post_final_execution_confirmation_transcript_digest",
            self.authority_activation_post_final_execution_confirmation_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"authority_activation_execution_sink_prewrite_nonce",
            &self.authority_activation_execution_sink_prewrite_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 353 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_content_digest(
    package: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact,
) -> String {
    package.content_digest()
}

/// Run 353 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and authority-activation-post-final-execution-confirmation nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_execution_sink_prewrite_policy_id: &str,
    ...
    hash_field(
        &mut h,
        b"authority_activation_execution_sink_prewrite_policy_id",
        authority_activation_execution_sink_prewrite_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"authority_activation_execution_sink_prewrite_nonce",
        &authority_activation_execution_sink_prewrite_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 353 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and authority-activation-post-final-execution-confirmation nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    authority_activation_execution_sink_prewrite_policy_id: &str,
    epoch_transition_target: u64,
    ...
    hash_field(&mut h, b"authority_activation_execution_sink_prewrite_policy_id", authority_activation_execution_sink_prewrite_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 353 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
...
/// Run 353 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition authority-activation-execution-sink-prewrite
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite {
        execution_kind: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind,
    ...
    VerifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRequired,
    UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRejected,
    ...
    AuthorityActivationPostFinalExecutionConfirmationDecisionIdMismatch,
    AuthorityActivationPostFinalExecutionConfirmationDecisionRequestIdMismatch,
    AuthorityActivationPostFinalExecutionConfirmationDecisionIntentDigestMismatch,
    AuthorityActivationPostFinalExecutionConfirmationDecisionTranscriptMismatch,
    AuthorityActivationPostFinalExecutionConfirmationDecisionIntegrityMismatch,
    WrongAuthorityActivationPostFinalExecutionConfirmationNonce,
    ...
    LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteBoundaryUnavailable => {
                "live-epoch-transition-authority-activation-execution-sink-prewrite-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable => {
                "production-live-epoch-transition-authority-activation-execution-sink-prewrite-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable => {
                "mainnet-production-live-epoch-transition-authority-activation-execution-sink-prewrite-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite { .. } => {
                "accepted-source-test-live-epoch-transition-authority-activation-execution-sink-prewrite"
            }
            Self::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRequired => {
                "verified-authority-activation-post-final-execution-confirmation-decision-required"
            }
            Self::UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRejected => {
                "unverified-authority-activation-post-final-execution-confirmation-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-authority-activation-post-final-execution-confirmation-decision-rejected-as-production-authority"
            }
            ...
            Self::AuthorityActivationPostFinalExecutionConfirmationDecisionIdMismatch => "authority-activation-post-final-execution-confirmation-decision-id-mismatch",
            Self::AuthorityActivationPostFinalExecutionConfirmationDecisionRequestIdMismatch => {
                "authority-activation-post-final-execution-confirmation-decision-request-id-mismatch"
            }
            Self::AuthorityActivationPostFinalExecutionConfirmationDecisionIntentDigestMismatch => {
                "authority-activation-post-final-execution-confirmation-decision-intent-digest-mismatch"
            }
            Self::AuthorityActivationPostFinalExecutionConfirmationDecisionTranscriptMismatch => {
                "authority-activation-post-final-execution-confirmation-decision-transcript-mismatch"
            }
            Self::AuthorityActivationPostFinalExecutionConfirmationDecisionIntegrityMismatch => {
                "authority-activation-post-final-execution-confirmation-decision-integrity-mismatch"
            }
            Self::WrongAuthorityActivationPostFinalExecutionConfirmationNonce => "wrong-authority-activation-post-final-execution-confirmation-nonce",
            ...
            Self::LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAmbiguous { .. } => {
                "epoch-transition-authority-activation-post-final-execution-confirmation-ambiguous"
            }
...
/// Run 353 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteDecision {
    pub outcome: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome,
    pub authority_activation_execution_sink_prewrite_id: String,
    pub request_id: String,
    pub authority_activation_execution_sink_prewrite_artifact:
        Option<ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact>,
    pub authority_activation_execution_sink_prewrite_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.authority_activation_execution_sink_prewrite_artifact.is_some()
    }
}
...
/// Run 353 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRecoveryOutcome {
...
impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRecoveryOutcome {
...
/// Run 353 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor {
    pub config: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig,
    pub policy: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy,
}

impl ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision,
            &'a ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact,
        ),
        ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome,
    > {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome as O;
        use LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource as S;
        match source {
            S::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRejected);
                }
                match &decision.authority_activation_post_final_execution_confirmation_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRequired),
                }
            }
            S::MissingAuthorityActivationPostFinalExecutionConfirmationDecision => {
                Err(O::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRequired)
            }
            S::UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecision { .. } => {
                Err(O::UnverifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRejected)
            }
            S::AcceptedAuthorityActivationPostFinalExecutionConfirmationWithoutPackage { .. } => {
                Err(O::VerifiedAuthorityActivationPostFinalExecutionConfirmationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutAuthorityActivationPostFinalExecutionConfirmation => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutAuthorityActivationPostFinalExecutionConfirmation => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutAuthorityActivationPostFinalExecutionConfirmation => {
            ...
            S::FixtureOnlyAuthorityActivationPostFinalExecutionConfirmation => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy::MainnetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequired => {
                    O::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 353.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 351/352 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision,
        intent: &ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationArtifact,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs,
    ) -> Option<ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome> {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 351/352 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.authority_activation_post_final_execution_confirmation_id != inputs.expected_authority_activation_post_final_execution_confirmation_decision_id {
            return Some(O::AuthorityActivationPostFinalExecutionConfirmationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_authority_activation_post_final_execution_confirmation_request_id {
            return Some(O::AuthorityActivationPostFinalExecutionConfirmationDecisionRequestIdMismatch);
        }
        if decision.authority_activation_post_final_execution_confirmation_digest != inputs.expected_authority_activation_post_final_execution_confirmation_intent_digest {
            return Some(O::AuthorityActivationPostFinalExecutionConfirmationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_authority_activation_post_final_execution_confirmation_transcript_digest {
            return Some(O::AuthorityActivationPostFinalExecutionConfirmationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.authority_activation_post_final_execution_confirmation_digest {
            return Some(O::AuthorityActivationPostFinalExecutionConfirmationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed authority-activation-post-final-execution-confirmation
        // nonce binding.
        if intent.authority_activation_post_final_execution_confirmation_nonce != inputs.expected_authority_activation_post_final_execution_confirmation_nonce {
            return Some(O::WrongAuthorityActivationPostFinalExecutionConfirmationNonce);
        }

        // Re-exposed Run 331/332 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 351/352
        // authority-activation-post-final-execution-confirmation artifact).
        ...
        // Re-exposed Run 331/332 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 351/352
        // authority-activation-post-final-execution-confirmation artifact).
        ...
        // Re-exposed Run 331/332 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 351/352
        // authority-activation-post-final-execution-confirmation artifact).
        ...
        // Re-exposed Run 331/332 guarded epoch-transition authority-activation-execution-sink-prewrite
        ...
        // Re-exposed Run 351/352 staged-application decision authority tuple
        ...
        // Re-exposed Run 351/352 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 325/326 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_authority_activation_execution_sink_prewrite_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_post_final_execution_confirmation_digest,
            &inputs.authority_activation_execution_sink_prewrite_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_execution_sink_prewrite_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            authority_activation_execution_sink_prewrite_policy_id: inputs.authority_activation_execution_sink_prewrite_policy_id.clone(),
            ...
            // Re-exposed Run 331/332 guarded authority-activation-execution-sink-prewrite decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded authority-activation-execution-sink-prewrite nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 351/352 runtime handoff decision transcript (the
            // authority source this run consumes).
            authority_activation_post_final_execution_confirmation_decision_id: decision.authority_activation_post_final_execution_confirmation_id.clone(),
            authority_activation_post_final_execution_confirmation_request_id: decision.request_id.clone(),
            authority_activation_post_final_execution_confirmation_intent_digest: decision.authority_activation_post_final_execution_confirmation_digest.clone(),
            authority_activation_post_final_execution_confirmation_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed authority-activation-post-final-execution-confirmation nonce.
            authority_activation_post_final_execution_confirmation_nonce: application_intent.authority_activation_post_final_execution_confirmation_nonce,
            // Re-exposed Run 331/332 commit-authorization decision authority
            // tuple (carried through the consumed authority-activation-post-final-execution-confirmation artifact).
            ...
            // Re-exposed Run 331/332 runtime handoff decision authority tuple
            // (carried through the consumed authority-activation-post-final-execution-confirmation artifact).
            ...
            // Newly proposed authority-activation-execution-sink-prewrite nonce.
            authority_activation_execution_sink_prewrite_nonce: request.authority_activation_execution_sink_prewrite_nonce,
            // Exact future-executor preconditions.
            ...
            authority_activation_execution_sink_prewrite_id: String::new(),
            request_id: String::new(),
            authority_activation_execution_sink_prewrite_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            authority_activation_execution_sink_prewrite_nonce: request.authority_activation_execution_sink_prewrite_nonce,
        };
        let authority_activation_execution_sink_prewrite_id = production_live_epoch_transition_authority_activation_execution_sink_prewrite_id(
            self.config.protocol_version.0,
            &decision.authority_activation_post_final_execution_confirmation_digest,
            &inputs.authority_activation_execution_sink_prewrite_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_authority_activation_execution_sink_prewrite_request_id(
            self.config.protocol_version.0,
            &decision.authority_activation_post_final_execution_confirmation_digest,
            &inputs.authority_activation_execution_sink_prewrite_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.authority_activation_execution_sink_prewrite_digest = record.content_digest();
        record.authority_activation_execution_sink_prewrite_id = authority_activation_execution_sink_prewrite_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_authority_activation_execution_sink_prewrite_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.authority_activation_execution_sink_prewrite_digest,
                outcome.tag(),
            );
    ...
    /// Run 353 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_authority_activation_execution_sink_prewrite<
        R: LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest,
        inputs: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteDecision {
                outcome,
                authority_activation_execution_sink_prewrite_id: pkg.authority_activation_execution_sink_prewrite_id.clone(),
                request_id: pkg.request_id.clone(),
                authority_activation_execution_sink_prewrite_artifact: record.clone(),
                authority_activation_execution_sink_prewrite_digest: pkg.authority_activation_execution_sink_prewrite_digest.clone(),
        ...
        let authority_activation_post_final_execution_confirmation_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource::AcceptedAuthorityActivationPostFinalExecutionConfirmationWithoutPackage {
                decision,
            } => decision.authority_activation_post_final_execution_confirmation_digest.clone(),
            _ => String::new(),
        };

        let authority_activation_execution_sink_prewrite_id = production_live_epoch_transition_authority_activation_execution_sink_prewrite_id(
            self.config.protocol_version.0,
            &authority_activation_post_final_execution_confirmation_intent_digest,
            &inputs.authority_activation_execution_sink_prewrite_policy_id,
            request.proposed_epoch_transition_target,
            request.authority_activation_execution_sink_prewrite_nonce,
        );
        ...
        let authority_activation_execution_sink_prewrite_digest = String::new();
        ...
        ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteDecision {
            ...
            request_id,
            authority_activation_execution_sink_prewrite_artifact: None,
            authority_activation_execution_sink_prewrite_digest,
            transcript_digest,
        }
    }

    /// Run 353 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_authority_activation_execution_sink_prewrite_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact>,
        current: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact,
    ) -> ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRecoveryOutcome {
        use ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind::Disabled
        {
...
/// Run 353 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy::default()
        == ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig::default().kind
            == ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind::Disabled
}

/// Run 353 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 354).
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 353 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_mainnet_refused() -> bool {
    true
}

/// Run 353 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_is_non_mutating() -> bool {
    true
}

/// Run 353 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_never_falls_back() -> bool {
    true
}

/// Run 353 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 353 — the executor only requires a verified Run 351/352 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 353 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 353. It is not wired into node runtime, never touches production
...
/// calls Run 70, and never mutates `LivePqcTrustState`. It is a plain
/// in-memory struct owned by the caller (a test).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteFixtureState {
...
impl LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, authority_activation_execution_sink_prewrite_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == authority_activation_execution_sink_prewrite_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `authority_activation_execution_sink_prewrite_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact,
        authority_activation_execution_sink_prewrite_id: &str,
    ) -> bool {
        if self.has_applied(authority_activation_execution_sink_prewrite_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(authority_activation_execution_sink_prewrite_id.to_string());
        true
    }
}