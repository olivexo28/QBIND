//! Run 321 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 317/318 guarded epoch-transition commit-execution boundary: the boundary
//! that consumes a **verified** Run 319/320 non-mutating guarded
//! epoch-transition commit-execution *decision* (the accepted
//! [`ProductionLiveEpochTransitionCommitAuthorizationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionCommitAuthorizationArtifact`], as
//! release-binary-evidenced by Run 318) and translates it into a typed,
...
//!
//! Where the Run 317 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 321 answers the next question:
//! "given a verified guarded commit-execution decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a commit-authorization
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 321)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 322**.
//! * The default policy is
//!   [`ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::Disabled`]
//!   and fails closed **before** any binding, validator-set binding, or
//!   handoff-package construction.
//! * Only a **verified** Run 319/320 guarded epoch-transition
//!   commit-execution decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionCommitAuthorizationArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 315/316 staged-application decision alone, a Run
//!   313/314 live-application authorization alone, a Run 311/312 application
//!   decision alone, Run 309 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionCommitExecutionArtifact`]; it
...
use crate::pqc_production_live_epoch_transition_commit_authorization::{
...
/// Run 321 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION: u16 = 1;

/// Run 321 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_INTENT_DOMAIN_TAG: &str =
    "QBIND:321-epoch-transition-commit-authorization-intent:v1";

/// Run 321 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_ID_DOMAIN_TAG: &str =
    "QBIND:321-epoch-transition-commit-authorization-id:v1";

/// Run 321 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:321-epoch-transition-commit-authorization-request:v1";

/// Run 321 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:321-epoch-transition-commit-authorization-transcript:v1";
...
/// Run 321 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionCommitExecutionProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionCommitExecutionProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionCommitExecutionProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionCommitExecution` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 319/320 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionCommitExecution` and
/// `MainnetProductionLiveEpochTransitionCommitExecutionRequired` are
...
pub enum ProductionLiveEpochTransitionCommitExecutionExecutorPolicy {
    ...
    AllowSourceTestLiveEpochTransitionCommitExecution,
    ...
    RequireProductionLiveEpochTransitionCommitExecution,
    ...
    /// wired.
    MainnetProductionLiveEpochTransitionCommitExecutionRequired,
}

impl ProductionLiveEpochTransitionCommitExecutionExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionCommitExecution => {
                "allow-source-test-epoch-transition-commit-authorization"
            }
            Self::RequireProductionLiveEpochTransitionCommitExecution => {
                "require-production-epoch-transition-commit-authorization"
            }
            Self::MainnetProductionLiveEpochTransitionCommitExecutionRequired => {
                "mainnet-production-epoch-transition-commit-authorization-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionCommitExecution` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionCommitExecution` kind is fail-closed as
/// unavailable in Run 321 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionCommitExecutionExecutorKind {
    /// Inert default; every request is refused.
    #[default]
    Disabled,
    /// Real source/test live validator-set application authorization
    /// boundary.
    SourceTestLiveEpochTransitionCommitExecution,
    /// Reserved production authorization kind. Fail-closed in Run 321.
    ProductionLiveEpochTransitionCommitExecution,
}

impl ProductionLiveEpochTransitionCommitExecutionExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionCommitExecution => {
                "source-test-epoch-transition-commit-authorization"
            }
            Self::ProductionLiveEpochTransitionCommitExecution => {
                "production-epoch-transition-commit-authorization"
            }
        }
    }
    ...
    pub const fn is_source_test(self) -> bool {
        matches!(self, Self::SourceTestLiveEpochTransitionCommitExecution)
    }
}
...
pub struct ProductionLiveEpochTransitionCommitExecutionConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionCommitExecutionProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionCommitExecutionExecutorKind,
}

impl ProductionLiveEpochTransitionCommitExecutionConfig {
    pub fn new(kind: ProductionLiveEpochTransitionCommitExecutionExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionCommitExecutionProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionCommitExecutionExecutorKind::SourceTestLiveEpochTransitionCommitExecution,
        )
    }
...
impl Default for ProductionLiveEpochTransitionCommitExecutionConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionCommitExecutionExecutorKind::Disabled)
    }
}
...
/// Run 321 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 319/320
/// [`LiveEpochTransitionCommitAuthorizationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionCommitExecutionKind {
...
impl LiveEpochTransitionCommitExecutionKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 321 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 319/320 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionCommitAuthorizationKind,
    ) -> Self {
        use LiveEpochTransitionCommitAuthorizationKind as A;
        match kind {
...
/// Run 321 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedCommitAuthorizationDecision`] carrying a Run 319/320
...
pub enum LiveEpochTransitionCommitExecutionAuthoritySource {
    ...
    /// A Run 317/318 epoch-transition runtime handoff decision presented
    /// directly, without a Run 319/320 commit-authorization decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutCommitAuthorization,
    /// A Run 317/318 guarded epoch-transition commit-execution decision
    /// presented directly, without a Run 319/320 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutCommitAuthorization,
    /// A Run 315/316 staged-application decision presented directly, without a
    /// Run 319/320 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutCommitAuthorization,
    /// A Run 313/314 live-application authorization presented directly, without
    /// a Run 319/320 guarded commit-execution decision. Rejected.
    LiveApplicationAuthorizationWithoutCommitAuthorization,
    /// A Run 311/312 validator-set rotation *application decision* presented
    /// directly, without a Run 319/320 guarded commit-execution decision.
    /// Rejected.
    ApplicationDecisionWithoutCommitAuthorization,
    /// A Run 309/310 validator-set rotation plan presented directly, without a
    /// Run 319/320 live-application authorization. Rejected.
    RotationPlanWithoutCommitAuthorization,
    /// A Run 307/308 governance execution intent presented directly, without a
    /// Run 319/320 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutCommitAuthorization,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 313/314 live-application authorization. Rejected.
    GovernanceProofWithoutCommitAuthorization,
    ...
    FixtureOnlyCommitAuthorization,
...
/// Run 321 — the explicit trusted inputs the executor binds a verified
/// Run 319/320 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitExecutionInputs {
    ...
    pub commit_execution_policy_id: String,
    /// Expected Run 319/320 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 311/312 application policy id (re-exposed by the
    ...
    /// Expected Run 307 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 307 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 307 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 309 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 309 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 309 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 309 rotation plan digest.
    ...
    /// Expected Run 311/312 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 311/312 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 311/312 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 311/312 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 319/320 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 319/320 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 319/320 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 319/320 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 319/320 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 319/320 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 319/320 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 319/320 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 319/320 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 311/312 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 319/320 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 317/318 guarded commit-execution decision id (re-exposed
    /// by the consumed Run 319/320 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 317/318 guarded commit-execution request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 317/318 guarded commit-execution intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 317/318 guarded commit-execution transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 317/318 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 319/320 runtime handoff decision id (bound into the
    ...
    /// Expected Run 319/320 runtime handoff package/content digest (the
    /// consumed decision's `commit_execution_digest`, i.e. the handoff content
    /// digest).
    ...
    /// Expected Run 317/318 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 317/318 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 317/318 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 317/318 execution-preparation nonce (re-exposed by the
    /// consumed Run 319/320 commit-authorization artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 317/318 runtime handoff decision id (re-exposed by the
    /// consumed Run 319/320 commit-authorization artifact).
    ...
    /// Expected Run 317/318 runtime handoff package/content digest.
    ...
    /// Expected Run 317/318 runtime handoff nonce (re-exposed by the consumed
    /// Run 319/320 commit-authorization artifact).
    ...
    /// wall-clock value and never a reject path in Run 321.
...
impl ProductionLiveEpochTransitionCommitExecutionInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.commit_execution_policy_id.is_empty()
...
/// Run 321 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition commit-execution decision), the explicit
/// epoch-transition target, a commit-authorization nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitExecutionRequest {
    pub authority_source: LiveEpochTransitionCommitExecutionAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The commit-authorization nonce (idempotency / replay binding).
...
impl ProductionLiveEpochTransitionCommitExecutionRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and commit-authorization nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionCommitExecutionAuthoritySource,
        proposed_epoch_transition_target: u64,
        commit_execution_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 321 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionCommitExecutionReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionCommitExecutionReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionCommitExecutionReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionCommitExecutionReplaySet;

impl LiveEpochTransitionCommitExecutionReplaySet
    for EmptyLiveEpochTransitionCommitExecutionReplaySet
{
...
/// Run 321 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 322+); Run 321 never
...
/// preconditions, and carries deterministic `commit_execution_id`, `request_id`,
/// `commit_execution_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitExecutionArtifact {
    pub staged_kind: LiveEpochTransitionCommitExecutionKind,
    pub protocol_version: u16,
    pub commit_execution_policy_id: String,

    // ---- Re-exposed Run 319/320 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 311/312 application-decision tuple ------------
    ...
    // ---- Bound Run 319/320 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 319/320 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 319/320 guarded commit-execution authority tuple -
    // (the consumed guarded commit-execution decision transcript)
    ...
    // ---- Guarded commit-execution binding (re-exposed consumed nonce)
    /// The re-exposed Run 317/318 guarded commit-execution nonce.
    ...
    /// The re-exposed Run 317/318 execution-preparation nonce (consumed).
    ...
    /// The re-exposed Run 317/318 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed commit-execution nonce.
    pub commit_execution_nonce: u64,
    ...
    pub commit_execution_id: String,
    pub request_id: String,
    pub commit_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionCommitExecutionArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`commit_execution_id`,
    /// `request_id`, `commit_execution_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_INTENT_DOMAIN_TAG.as_bytes());
        ...
        hash_field(
            &mut h,
            b"commit_execution_policy_id",
            self.commit_execution_policy_id.as_bytes(),
        );
        hash_field(
            &mut h,
...
/// Custody binding canonical hashing (module-local; mirrors Run 307/309/311
...
/// Run 321 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_commit_execution_content_digest(
    package: &ProductionLiveEpochTransitionCommitExecutionArtifact,
) -> String {
    package.content_digest()
}

/// Run 321 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and commit-authorization nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_commit_execution_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    commit_execution_policy_id: &str,
    epoch_transition_target: u64,
    commit_execution_nonce: u64,
) -> String {
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_REQUEST_DOMAIN_TAG.as_bytes());
    ...
    hash_field(
        &mut h,
        b"commit_execution_policy_id",
        commit_execution_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
        b"epoch_transition_target",
    ...
    hash_field(
        &mut h,
        b"commit_execution_nonce",
        &commit_execution_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 321 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and commit-authorization nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_commit_execution_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    commit_execution_policy_id: &str,
    epoch_transition_target: u64,
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_ID_DOMAIN_TAG.as_bytes());
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
        b"guarded_mutation_intent_digest",
...
/// Run 321 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_commit_execution_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    use sha3::{Digest, Sha3_256};
    let mut h = Sha3_256::new();
    h.update(PRODUCTION_LIVE_EPOCH_TRANSITION_COMMIT_EXECUTION_TRANSCRIPT_DOMAIN_TAG.as_bytes());
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 321 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionCommitExecution`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionCommitExecutionOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionCommitExecutionBoundaryUnavailable,
    /// The production policy has no production prerequisites wired.
    ProductionLiveEpochTransitionCommitExecutionUnavailable,
    /// The MainNet production policy has no MainNet authority wired.
    MainNetProductionLiveEpochTransitionCommitExecutionUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition commit-execution
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionCommitExecution {
        execution_kind: LiveEpochTransitionCommitExecutionKind,
    ...
    VerifiedCommitAuthorizationDecisionRequired,
    UnverifiedCommitAuthorizationDecisionRejected,
    ...
    CommitAuthorizationDecisionIntegrityMismatch,
    ...
    LiveEpochTransitionCommitExecutionAmbiguous {
        reason: String,
    },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionCommitExecutionOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionCommitExecutionBoundaryUnavailable => {
                "live-epoch-transition-commit-execution-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionCommitExecutionUnavailable => {
                "production-live-epoch-transition-commit-execution-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionCommitExecutionUnavailable => {
                "mainnet-production-live-epoch-transition-commit-execution-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionCommitExecution { .. } => {
                "accepted-source-test-live-epoch-transition-commit-execution"
            }
            Self::VerifiedCommitAuthorizationDecisionRequired => {
                "verified-commit-authorization-decision-required"
            }
            Self::UnverifiedCommitAuthorizationDecisionRejected => {
                "unverified-commit-authorization-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-commit-authorization-decision-rejected-as-production-authority"
            }
            ...
            Self::CommitAuthorizationDecisionIntegrityMismatch => {
                "commit-authorization-decision-integrity-mismatch"
            }
            ...
            Self::LiveEpochTransitionCommitExecutionAmbiguous { .. } => {
                "epoch-transition-commit-authorization-ambiguous"
            }
...
/// Run 321 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitExecutionDecision {
    pub outcome: ProductionLiveEpochTransitionCommitExecutionOutcome,
    pub commit_execution_id: String,
    pub request_id: String,
    pub commit_execution_artifact: Option<ProductionLiveEpochTransitionCommitExecutionArtifact>,
    pub commit_execution_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionCommitExecutionDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.commit_execution_artifact.is_some()
    }
}
...
/// Run 321 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome {
...
impl ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome {
...
/// Run 321 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionCommitExecutionExecutor {
    pub config: ProductionLiveEpochTransitionCommitExecutionConfig,
    pub policy: ProductionLiveEpochTransitionCommitExecutionExecutorPolicy,
}

impl ProductionLiveEpochTransitionCommitExecutionExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionCommitExecutionConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionCommitExecutionAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionCommitAuthorizationDecision,
            &'a ProductionLiveEpochTransitionCommitAuthorizationArtifact,
        ),
        ProductionLiveEpochTransitionCommitExecutionOutcome,
    > {
        use LiveEpochTransitionCommitExecutionAuthoritySource as S;
        use ProductionLiveEpochTransitionCommitExecutionOutcome as O;
        match source {
            S::VerifiedCommitAuthorizationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedCommitAuthorizationDecisionRejected);
                }
                match &decision.commit_authorization_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedCommitAuthorizationDecisionRequired),
                }
            }
            S::MissingCommitAuthorizationDecision => {
                Err(O::VerifiedCommitAuthorizationDecisionRequired)
            }
            S::UnverifiedCommitAuthorizationDecision { .. } => {
                Err(O::UnverifiedCommitAuthorizationDecisionRejected)
            }
            ...
            S::RuntimeHandoffDecisionWithoutCommitAuthorization => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutCommitAuthorization => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutCommitAuthorization => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutCommitAuthorization => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutCommitAuthorization => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutCommitAuthorization => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutCommitAuthorization => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutCommitAuthorization => Err(O::GovernanceProofAloneRejected),
            ...
            S::FixtureOnlyCommitAuthorization => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionCommitExecutionOutcome> {
        use ProductionLiveEpochTransitionCommitExecutionOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionCommitExecutionExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::MainnetProductionLiveEpochTransitionCommitExecutionRequired => {
                    O::MainNetProductionLiveEpochTransitionCommitExecutionUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionCommitExecutionUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionCommitExecutionUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 321.
        if self.config.kind
            == ProductionLiveEpochTransitionCommitExecutionExecutorKind::ProductionLiveEpochTransitionCommitExecution
        {
            return Some(O::LiveEpochTransitionCommitExecutionBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 319/320 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionCommitAuthorizationDecision,
        intent: &ProductionLiveEpochTransitionCommitAuthorizationArtifact,
        inputs: &ProductionLiveEpochTransitionCommitExecutionInputs,
    ) -> Option<ProductionLiveEpochTransitionCommitExecutionOutcome> {
        use ProductionLiveEpochTransitionCommitExecutionOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 319/320 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.commit_authorization_id != inputs.expected_commit_authorization_decision_id {
            return Some(O::CommitAuthorizationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_commit_authorization_request_id {
            return Some(O::CommitAuthorizationDecisionRequestIdMismatch);
        }
        if decision.commit_authorization_digest
            != inputs.expected_commit_authorization_intent_digest
        {
            return Some(O::CommitAuthorizationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_commit_authorization_transcript_digest {
        ...
        if intent.content_digest() != decision.commit_authorization_digest {
            return Some(O::CommitAuthorizationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed commit-authorization
        // nonce binding.
        ...
        // Re-exposed Run 317/318 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 319/320
        // commit-authorization artifact).
        ...
        // Re-exposed Run 317/318 guarded epoch-transition commit-execution
        ...
        // Re-exposed Run 319/320 staged-application decision authority tuple
        ...
        // Re-exposed Run 319/320 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 311/312 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionCommitExecutionReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionCommitExecutionRequest,
        inputs: &ProductionLiveEpochTransitionCommitExecutionInputs,
        replay_set: &R,
        ...
        ProductionLiveEpochTransitionCommitExecutionOutcome,
        Option<ProductionLiveEpochTransitionCommitExecutionArtifact>,
    ) {
        use ProductionLiveEpochTransitionCommitExecutionOutcome as O;
        ...
        let staged_application_id = production_live_epoch_transition_commit_execution_request_id(
            self.config.protocol_version.0,
            &decision.commit_authorization_digest,
            &inputs.commit_execution_policy_id,
            request.proposed_epoch_transition_target,
            request.commit_execution_nonce,
        );
        ...
        let staged_kind = LiveEpochTransitionCommitExecutionKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionCommitExecutionArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            commit_execution_policy_id: inputs.commit_execution_policy_id.clone(),
            ...
            // Re-exposed Run 317/318 guarded commit-execution decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded commit-execution nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 319/320 runtime handoff decision transcript (the
            // authority source this run consumes).
            commit_authorization_decision_id: decision.commit_authorization_id.clone(),
            commit_authorization_request_id: decision.request_id.clone(),
            commit_authorization_intent_digest: decision.commit_authorization_digest.clone(),
            ...
            // Re-exposed Run 317/318 runtime handoff decision authority tuple
            // (carried through the consumed commit-authorization artifact).
            ...
            // Newly proposed commit-execution nonce.
            commit_execution_nonce: request.commit_execution_nonce,
            // Exact future-executor preconditions.
            ...
            commit_execution_id: String::new(),
            request_id: String::new(),
            commit_execution_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionCommitExecution {
        ...
        record.commit_execution_digest = record.content_digest();
        record.commit_execution_id = commit_execution_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_commit_execution_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
    ...
    /// Run 321 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_commit_execution<
        R: LiveEpochTransitionCommitExecutionReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionCommitExecutionRequest,
        inputs: &ProductionLiveEpochTransitionCommitExecutionInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionCommitExecutionDecision {
                outcome,
                commit_execution_id: pkg.commit_execution_id.clone(),
                request_id: pkg.request_id.clone(),
                commit_execution_artifact: record.clone(),
                commit_execution_digest: pkg.commit_execution_digest.clone(),
        ...
        let commit_authorization_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionCommitExecutionAuthoritySource::AcceptedCommitAuthorizationWithoutPackage {
                decision,
            } => decision.commit_authorization_digest.clone(),
            _ => String::new(),
        };
        ...
        let commit_execution_digest = String::new();
        ...
        ProductionLiveEpochTransitionCommitExecutionDecision {
            ...
            commit_execution_id,
            request_id,
            commit_execution_artifact: None,
            commit_execution_digest,
            transcript_digest,
        }
    }

    /// Run 321 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_commit_execution_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionCommitExecutionArtifact>,
        current: &ProductionLiveEpochTransitionCommitExecutionArtifact,
    ) -> ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome {
        use ProductionLiveEpochTransitionCommitExecutionRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionCommitExecutionExecutorKind::Disabled
        {
...
/// Run 321 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_commit_execution_executor_default_is_disabled() -> bool {
    ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::default()
        == ProductionLiveEpochTransitionCommitExecutionExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionCommitExecutionConfig::default().kind
            == ProductionLiveEpochTransitionCommitExecutionExecutorKind::Disabled
}

/// Run 321 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 322).
pub fn production_live_epoch_transition_commit_execution_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 321 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_commit_execution_executor_mainnet_refused() -> bool {
    true
}

/// Run 321 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_commit_execution_executor_is_non_mutating() -> bool {
    true
}

/// Run 321 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_commit_execution_executor_never_falls_back() -> bool {
    true
}

/// Run 321 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_commit_execution_executor_no_default_runtime_wiring() -> bool
{
    true
}

/// Run 321 — the executor only requires a verified Run 319/320 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_commit_execution_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 321 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 321. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionCommitExecutionFixtureState {
...
impl LiveEpochTransitionCommitExecutionFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, commit_execution_id: &str) -> bool {
        self.applied_execution_ids
            .iter()
            .any(|id| id == commit_execution_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `commit_execution_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionCommitExecutionArtifact,
        commit_execution_id: &str,
    ) -> bool {
        if self.has_applied(commit_execution_id) {
            return false;
        }
        ...
        self.applied_execution_ids
            .push(commit_execution_id.to_string());
        true
    }
}
