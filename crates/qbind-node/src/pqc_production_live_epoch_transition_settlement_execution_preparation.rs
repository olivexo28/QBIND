//! Run 337 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 329/330 guarded epoch-transition settlement-execution-preparation boundary: the boundary
//! that consumes a **verified** Run 335/336 non-mutating guarded
//! epoch-transition settlement-execution-preparation *decision* (the accepted
//! [`ProductionLiveEpochTransitionSettlementPreparationDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionSettlementPreparationArtifact`], as
//! release-binary-evidenced by Run 330) and translates it into a typed,
...
//!
//! Where the Run 329 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 337 answers the next question:
//! "given a verified guarded settlement-execution-preparation decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a settlement-preparation
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 337)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 338**.
...
//! * Only a **verified** Run 335/336 guarded epoch-transition
//!   settlement-execution-preparation decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionSettlementPreparationArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 327/328 staged-application decision alone, a Run
//!   325/326 live-application authorization alone, a Run 323/324 application
//!   decision alone, Run 321 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact`]; it
...
//!
//! See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_335.md`.
...
use crate::pqc_production_live_epoch_transition_settlement_preparation::{
...
/// Run 337 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_PROTOCOL_VERSION: u16 = 1;

/// Run 337 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:337-epoch-transition-settlement-preparation-intent:v1";

/// Run 337 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_ID_DOMAIN_TAG: &str =
    "QBIND:337-epoch-transition-settlement-preparation-id:v1";

/// Run 337 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:337-epoch-transition-settlement-preparation-request:v1";

/// Run 337 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:337-epoch-transition-settlement-preparation-transcript:v1";
...
/// Run 337 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionSettlementExecutionPreparation` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 335/336 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionSettlementExecutionPreparation` and
/// `MainnetProductionLiveEpochTransitionSettlementExecutionPreparationRequired` are
...
pub enum ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy {
...
impl ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionSettlementExecutionPreparation => {
                "allow-source-test-epoch-transition-settlement-preparation"
            }
            Self::RequireProductionLiveEpochTransitionSettlementExecutionPreparation => {
                "require-production-epoch-transition-settlement-preparation"
            }
            Self::MainnetProductionLiveEpochTransitionSettlementExecutionPreparationRequired => {
                "mainnet-production-epoch-transition-settlement-preparation-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionSettlementExecutionPreparation` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionSettlementExecutionPreparation` kind is fail-closed as
/// unavailable in Run 337 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 337.
    ProductionLiveEpochTransitionSettlementExecutionPreparation,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionSettlementExecutionPreparation => {
                "source-test-epoch-transition-settlement-preparation"
            }
            Self::ProductionLiveEpochTransitionSettlementExecutionPreparation => {
                "production-epoch-transition-settlement-preparation"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::SourceTestLiveEpochTransitionSettlementExecutionPreparation,
        )
    }
...
impl Default for ProductionLiveEpochTransitionSettlementExecutionPreparationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::Disabled)
    }
}
...
/// Run 337 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 335/336
/// [`LiveEpochTransitionSettlementPreparationKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionSettlementExecutionPreparationKind {
...
impl LiveEpochTransitionSettlementExecutionPreparationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 337 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 335/336 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionSettlementPreparationKind,
    ) -> Self {
        use LiveEpochTransitionSettlementPreparationKind as A;
        match kind {
...
/// Run 337 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedSettlementPreparationDecision`] carrying a Run 335/336
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionSettlementExecutionPreparationAuthoritySource {
    /// A verified Run 335/336 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedSettlementPreparationDecision {
        decision: ProductionLiveEpochTransitionSettlementPreparationDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingSettlementPreparationDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedSettlementPreparationDecision {
        decision: ProductionLiveEpochTransitionSettlementPreparationDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedSettlementPreparationWithoutPackage {
        decision: ProductionLiveEpochTransitionSettlementPreparationDecision,
    },
    /// A Run 329/330 epoch-transition commit-authorization decision presented
    /// directly, without a Run 335/336 settlement-preparation decision. Rejected.
    CommitAuthorizationDecisionWithoutSettlementPreparation,
    /// A Run 329/330 epoch-transition mutation-execution decision presented
    /// directly, without a Run 335/336 settlement-preparation decision. Rejected.
    MutationExecutionDecisionWithoutSettlementPreparation,
    /// A Run 329/330 epoch-transition execution-preparation decision presented
    /// directly, without a Run 335/336 settlement-preparation decision. Rejected.
    ExecutionPreparationDecisionWithoutSettlementPreparation,
    /// A Run 329/330 epoch-transition runtime handoff decision presented
    /// directly, without a Run 335/336 settlement-preparation decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutSettlementPreparation,
    /// A Run 329/330 guarded epoch-transition settlement-execution-preparation decision
    /// presented directly, without a Run 335/336 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutSettlementPreparation,
    /// A Run 327/328 staged-application decision presented directly, without a
    /// Run 335/336 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutSettlementPreparation,
    /// A Run 325/326 live-application authorization presented directly, without
    /// a Run 335/336 guarded settlement-execution-preparation decision. Rejected.
    LiveApplicationAuthorizationWithoutSettlementPreparation,
    /// A Run 323/324 validator-set rotation *application decision* presented
    /// directly, without a Run 335/336 guarded settlement-execution-preparation decision.
    /// Rejected.
    ApplicationDecisionWithoutSettlementPreparation,
    /// A Run 321/322 validator-set rotation plan presented directly, without a
    /// Run 335/336 live-application authorization. Rejected.
    RotationPlanWithoutSettlementPreparation,
    /// A Run 319/320 governance execution intent presented directly, without a
    /// Run 335/336 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutSettlementPreparation,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 325/326 live-application authorization. Rejected.
    GovernanceProofWithoutSettlementPreparation,
    ...
    FixtureOnlySettlementPreparation,
...
/// Run 337 — the explicit trusted inputs the executor binds a verified
/// Run 335/336 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationInputs {
    ...
    pub settlement_execution_preparation_policy_id: String,
    /// Expected Run 335/336 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 323/324 application policy id (re-exposed by the
    ...
    /// Expected Run 319 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 319 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 319 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 321 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 321 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 321 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 321 rotation plan digest.
    ...
    /// Expected Run 323/324 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 323/324 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 323/324 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 323/324 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 335/336 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 335/336 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 335/336 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 335/336 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 335/336 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 335/336 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 335/336 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 335/336 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 335/336 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 323/324 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 335/336 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 329/330 guarded settlement-execution-preparation decision id (re-exposed
    /// by the consumed Run 335/336 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 329/330 guarded settlement-execution-preparation request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 329/330 guarded settlement-execution-preparation intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 329/330 guarded settlement-execution-preparation transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 329/330 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 335/336 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_settlement_preparation_decision_id: String,
    /// Expected Run 335/336 runtime handoff request id.
    pub expected_settlement_preparation_request_id: String,
    /// Expected Run 335/336 runtime handoff package/content digest (the
    /// consumed decision's `settlement_execution_preparation_digest`, i.e. the handoff content
    /// digest).
    pub expected_settlement_preparation_intent_digest: String,
    /// Expected Run 335/336 runtime handoff transcript digest.
    pub expected_settlement_preparation_transcript_digest: String,
    /// Expected Run 335/336 settlement-preparation nonce (re-exposed by the
    /// consumed Run 335/336 settlement-preparation artifact).
    pub expected_settlement_preparation_nonce: u64,
    /// Expected Run 329/330 commit-authorization decision id (re-exposed by
    /// the consumed Run 335/336 settlement-preparation artifact).
    pub expected_commit_authorization_decision_id: String,
    /// Expected Run 329/330 commit-authorization request id.
    pub expected_commit_authorization_request_id: String,
    /// Expected Run 329/330 commit-authorization intent/content digest.
    pub expected_commit_authorization_intent_digest: String,
    /// Expected Run 329/330 commit-authorization transcript digest.
    pub expected_commit_authorization_transcript_digest: String,
    /// Expected Run 329/330 commit-authorization nonce (re-exposed by the
    /// consumed Run 335/336 settlement-preparation artifact).
    pub expected_commit_authorization_nonce: u64,
    /// Expected Run 329/330 mutation-execution decision id (re-exposed by
    /// the consumed Run 335/336 settlement-preparation artifact).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 329/330 mutation-execution request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 329/330 mutation-execution intent/content digest.
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 329/330 mutation-execution transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 329/330 mutation-execution nonce (re-exposed by the
    /// consumed Run 335/336 settlement-preparation artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 329/330 execution-preparation decision id (re-exposed by
    /// the consumed Run 335/336 settlement-preparation artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 329/330 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 329/330 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 329/330 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 329/330 execution-preparation nonce (re-exposed by the
    /// consumed Run 335/336 settlement-preparation artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 329/330 runtime handoff decision id (re-exposed by the
    /// consumed Run 335/336 settlement-preparation artifact).
    pub expected_runtime_handoff_decision_id: String,
    /// Expected Run 329/330 runtime handoff request id.
    pub expected_runtime_handoff_request_id: String,
    /// Expected Run 329/330 runtime handoff package/content digest.
    pub expected_runtime_handoff_intent_digest: String,
    /// Expected Run 329/330 runtime handoff transcript digest.
    pub expected_runtime_handoff_transcript_digest: String,
    /// Expected Run 329/330 runtime handoff nonce (re-exposed by the consumed
    /// Run 335/336 settlement-preparation artifact).
    ...
    /// wall-clock value and never a reject path in Run 337.
...
impl ProductionLiveEpochTransitionSettlementExecutionPreparationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.settlement_execution_preparation_policy_id.is_empty()
            ...
            && !self.expected_settlement_preparation_decision_id.is_empty()
            && !self.expected_settlement_preparation_request_id.is_empty()
            && !self.expected_settlement_preparation_intent_digest.is_empty()
            && !self.expected_settlement_preparation_transcript_digest.is_empty()
...
/// Run 337 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition settlement-execution-preparation decision), the explicit
/// epoch-transition target, a settlement-preparation nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationRequest {
    pub authority_source: LiveEpochTransitionSettlementExecutionPreparationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The settlement-preparation nonce (idempotency / replay binding).
    pub settlement_execution_preparation_nonce: u64,
...
impl ProductionLiveEpochTransitionSettlementExecutionPreparationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and settlement-preparation nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionSettlementExecutionPreparationAuthoritySource,
        proposed_epoch_transition_target: u64,
        settlement_execution_preparation_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
...
/// Run 337 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionSettlementExecutionPreparationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionSettlementExecutionPreparationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionSettlementExecutionPreparationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet;

impl LiveEpochTransitionSettlementExecutionPreparationReplaySet
    for EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet
{
...
/// Run 337 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 338+); Run 337 never
...
/// preconditions, and carries deterministic `settlement_execution_preparation_id`, `request_id`,
/// `settlement_execution_preparation_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact {
    pub staged_kind: LiveEpochTransitionSettlementExecutionPreparationKind,
    pub protocol_version: u16,
    pub settlement_execution_preparation_policy_id: String,

    // ---- Re-exposed Run 335/336 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 323/324 application-decision tuple ------------
    ...
    // ---- Bound Run 335/336 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 335/336 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 335/336 guarded settlement-execution-preparation authority tuple -
    // (the consumed guarded settlement-execution-preparation decision transcript)
    ...
    // ---- Guarded settlement-execution-preparation binding (re-exposed consumed nonce)
    /// The re-exposed Run 329/330 guarded settlement-execution-preparation nonce.
    pub guarded_mutation_nonce: u64,

    // ---- Bound Run 335/336 runtime handoff decision authority tuple ----
    // (the consumed runtime handoff decision transcript)
    pub settlement_preparation_decision_id: String,
    pub settlement_preparation_request_id: String,
    pub settlement_preparation_intent_digest: String,
    pub settlement_preparation_transcript_digest: String,
    /// The re-exposed Run 335/336 settlement-preparation nonce (consumed).
    pub settlement_preparation_nonce: u64,

    // ---- Bound Run 329/330 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 335/336 settlement-preparation artifact)
    ...
    /// The re-exposed Run 329/330 commit-authorization nonce (consumed).
    pub commit_authorization_nonce: u64,

    // ---- Bound Run 329/330 mutation-execution authority tuple -------
    // (re-exposed by the consumed Run 335/336 settlement-preparation artifact)
    ...
    /// The re-exposed Run 329/330 mutation-execution nonce (consumed).
    pub mutation_execution_nonce: u64,

    // ---- Bound Run 329/330 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 335/336 settlement-preparation artifact)
    ...
    /// The re-exposed Run 329/330 execution-preparation nonce (consumed).
    pub execution_preparation_nonce: u64,

    // ---- Bound Run 329/330 runtime handoff decision authority tuple ----
    // (re-exposed by the consumed Run 335/336 settlement-preparation artifact)
    ...
    /// The re-exposed Run 329/330 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed settlement-execution-preparation nonce.
    pub settlement_execution_preparation_nonce: u64,
    ...
    pub settlement_execution_preparation_id: String,
    pub request_id: String,
    pub settlement_execution_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`settlement_execution_preparation_id`,
    /// `request_id`, `settlement_execution_preparation_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"settlement_execution_preparation_policy_id",
            self.settlement_execution_preparation_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"settlement_preparation_decision_id",
            self.settlement_preparation_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_preparation_request_id",
            self.settlement_preparation_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_preparation_intent_digest",
            self.settlement_preparation_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_preparation_transcript_digest",
            self.settlement_preparation_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"settlement_preparation_nonce",
            &self.settlement_preparation_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"settlement_execution_preparation_nonce",
            &self.settlement_execution_preparation_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Run 337 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_settlement_execution_preparation_content_digest(
    package: &ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact,
) -> String {
    package.content_digest()
}

/// Run 337 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and settlement-preparation nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_settlement_execution_preparation_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_execution_preparation_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"settlement_execution_preparation_policy_id",
        settlement_execution_preparation_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"settlement_execution_preparation_nonce",
        &settlement_execution_preparation_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 337 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and settlement-preparation nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_settlement_execution_preparation_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    settlement_execution_preparation_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"settlement_execution_preparation_policy_id", settlement_execution_preparation_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 337 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_settlement_execution_preparation_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_SETTLEMENT_EXECUTION_PREPARATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 337 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionSettlementExecutionPreparation`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionSettlementExecutionPreparationBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition settlement-execution-preparation
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionSettlementExecutionPreparation {
        execution_kind: LiveEpochTransitionSettlementExecutionPreparationKind,
    ...
    VerifiedSettlementPreparationDecisionRequired,
    UnverifiedSettlementPreparationDecisionRejected,
    ...
    SettlementPreparationDecisionIdMismatch,
    SettlementPreparationDecisionRequestIdMismatch,
    SettlementPreparationDecisionIntentDigestMismatch,
    SettlementPreparationDecisionTranscriptMismatch,
    SettlementPreparationDecisionIntegrityMismatch,
    WrongSettlementPreparationNonce,
    ...
    LiveEpochTransitionSettlementExecutionPreparationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionSettlementExecutionPreparationBoundaryUnavailable => {
                "live-epoch-transition-settlement-execution-preparation-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable => {
                "production-live-epoch-transition-settlement-execution-preparation-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable => {
                "mainnet-production-live-epoch-transition-settlement-execution-preparation-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionSettlementExecutionPreparation { .. } => {
                "accepted-source-test-live-epoch-transition-settlement-execution-preparation"
            }
            Self::VerifiedSettlementPreparationDecisionRequired => {
                "verified-settlement-preparation-decision-required"
            }
            Self::UnverifiedSettlementPreparationDecisionRejected => {
                "unverified-settlement-preparation-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-settlement-preparation-decision-rejected-as-production-authority"
            }
            ...
            Self::SettlementPreparationDecisionIdMismatch => "settlement-preparation-decision-id-mismatch",
            Self::SettlementPreparationDecisionRequestIdMismatch => {
                "settlement-preparation-decision-request-id-mismatch"
            }
            Self::SettlementPreparationDecisionIntentDigestMismatch => {
                "settlement-preparation-decision-intent-digest-mismatch"
            }
            Self::SettlementPreparationDecisionTranscriptMismatch => {
                "settlement-preparation-decision-transcript-mismatch"
            }
            Self::SettlementPreparationDecisionIntegrityMismatch => {
                "settlement-preparation-decision-integrity-mismatch"
            }
            Self::WrongSettlementPreparationNonce => "wrong-settlement-preparation-nonce",
            ...
            Self::LiveEpochTransitionSettlementExecutionPreparationAmbiguous { .. } => {
                "epoch-transition-settlement-preparation-ambiguous"
            }
...
/// Run 337 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationDecision {
    pub outcome: ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome,
    pub settlement_execution_preparation_id: String,
    pub request_id: String,
    pub settlement_execution_preparation_artifact:
        Option<ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact>,
    pub settlement_execution_preparation_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.settlement_execution_preparation_artifact.is_some()
    }
}
...
/// Run 337 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome {
...
/// Run 337 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor {
    pub config: ProductionLiveEpochTransitionSettlementExecutionPreparationConfig,
    pub policy: ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy,
}

impl ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionSettlementExecutionPreparationConfig,
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionSettlementExecutionPreparationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionSettlementPreparationDecision,
            &'a ProductionLiveEpochTransitionSettlementPreparationArtifact,
        ),
        ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome,
    > {
        use ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome as O;
        use LiveEpochTransitionSettlementExecutionPreparationAuthoritySource as S;
        match source {
            S::VerifiedSettlementPreparationDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedSettlementPreparationDecisionRejected);
                }
                match &decision.settlement_preparation_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedSettlementPreparationDecisionRequired),
                }
            }
            S::MissingSettlementPreparationDecision => {
                Err(O::VerifiedSettlementPreparationDecisionRequired)
            }
            S::UnverifiedSettlementPreparationDecision { .. } => {
                Err(O::UnverifiedSettlementPreparationDecisionRejected)
            }
            S::AcceptedSettlementPreparationWithoutPackage { .. } => {
                Err(O::VerifiedSettlementPreparationDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutSettlementPreparation => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutSettlementPreparation => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutSettlementPreparation => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutSettlementPreparation => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutSettlementPreparation => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutSettlementPreparation => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutSettlementPreparation => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutSettlementPreparation => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutSettlementPreparation => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutSettlementPreparation => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutSettlementPreparation => {
            ...
            S::FixtureOnlySettlementPreparation => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::MainnetProductionLiveEpochTransitionSettlementExecutionPreparationRequired => {
                    O::MainNetProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionSettlementExecutionPreparationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 337.
        if self.config.kind
        ...
        {
            return Some(O::LiveEpochTransitionSettlementExecutionPreparationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 335/336 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionSettlementPreparationDecision,
        intent: &ProductionLiveEpochTransitionSettlementPreparationArtifact,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionPreparationInputs,
    ) -> Option<ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome> {
        use ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 335/336 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.settlement_preparation_id != inputs.expected_settlement_preparation_decision_id {
            return Some(O::SettlementPreparationDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_settlement_preparation_request_id {
            return Some(O::SettlementPreparationDecisionRequestIdMismatch);
        }
        if decision.settlement_preparation_digest != inputs.expected_settlement_preparation_intent_digest {
            return Some(O::SettlementPreparationDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_settlement_preparation_transcript_digest {
            return Some(O::SettlementPreparationDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.settlement_preparation_digest {
            return Some(O::SettlementPreparationDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed settlement-preparation
        // nonce binding.
        if intent.settlement_preparation_nonce != inputs.expected_settlement_preparation_nonce {
            return Some(O::WrongSettlementPreparationNonce);
        }

        // Re-exposed Run 329/330 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 335/336
        // settlement-preparation artifact).
        ...
        // Re-exposed Run 329/330 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 335/336
        // settlement-preparation artifact).
        ...
        // Re-exposed Run 329/330 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 335/336
        // settlement-preparation artifact).
        ...
        // Re-exposed Run 329/330 guarded epoch-transition settlement-execution-preparation
        ...
        // Re-exposed Run 335/336 staged-application decision authority tuple
        ...
        // Re-exposed Run 335/336 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 323/324 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionSettlementExecutionPreparationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionSettlementExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionPreparationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_settlement_execution_preparation_request_id(
            self.config.protocol_version.0,
            &decision.settlement_preparation_digest,
            &inputs.settlement_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
            request.settlement_execution_preparation_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionSettlementExecutionPreparationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            settlement_execution_preparation_policy_id: inputs.settlement_execution_preparation_policy_id.clone(),
            ...
            // Re-exposed Run 329/330 guarded settlement-execution-preparation decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded settlement-execution-preparation nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 335/336 runtime handoff decision transcript (the
            // authority source this run consumes).
            settlement_preparation_decision_id: decision.settlement_preparation_id.clone(),
            settlement_preparation_request_id: decision.request_id.clone(),
            settlement_preparation_intent_digest: decision.settlement_preparation_digest.clone(),
            settlement_preparation_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed settlement-preparation nonce.
            settlement_preparation_nonce: application_intent.settlement_preparation_nonce,
            // Re-exposed Run 329/330 commit-authorization decision authority
            // tuple (carried through the consumed settlement-preparation artifact).
            ...
            // Re-exposed Run 329/330 runtime handoff decision authority tuple
            // (carried through the consumed settlement-preparation artifact).
            ...
            // Newly proposed settlement-execution-preparation nonce.
            settlement_execution_preparation_nonce: request.settlement_execution_preparation_nonce,
            // Exact future-executor preconditions.
            ...
            settlement_execution_preparation_id: String::new(),
            request_id: String::new(),
            settlement_execution_preparation_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionSettlementExecutionPreparation {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            settlement_execution_preparation_nonce: request.settlement_execution_preparation_nonce,
        };
        let settlement_execution_preparation_id = production_live_epoch_transition_settlement_execution_preparation_id(
            self.config.protocol_version.0,
            &decision.settlement_preparation_digest,
            &inputs.settlement_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_settlement_execution_preparation_request_id(
            self.config.protocol_version.0,
            &decision.settlement_preparation_digest,
            &inputs.settlement_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.settlement_execution_preparation_digest = record.content_digest();
        record.settlement_execution_preparation_id = settlement_execution_preparation_id;
        ...
        record.transcript_digest =
            production_live_epoch_transition_settlement_execution_preparation_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.settlement_execution_preparation_digest,
                outcome.tag(),
            );
    ...
    /// Run 337 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_settlement_execution_preparation<
        R: LiveEpochTransitionSettlementExecutionPreparationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionSettlementExecutionPreparationRequest,
        inputs: &ProductionLiveEpochTransitionSettlementExecutionPreparationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionSettlementExecutionPreparationDecision {
                outcome,
                settlement_execution_preparation_id: pkg.settlement_execution_preparation_id.clone(),
                request_id: pkg.request_id.clone(),
                settlement_execution_preparation_artifact: record.clone(),
                settlement_execution_preparation_digest: pkg.settlement_execution_preparation_digest.clone(),
        ...
        let settlement_preparation_intent_digest = match &request.authority_source {
            ...
            | LiveEpochTransitionSettlementExecutionPreparationAuthoritySource::AcceptedSettlementPreparationWithoutPackage {
                decision,
            } => decision.settlement_preparation_digest.clone(),
            _ => String::new(),
        };

        let settlement_execution_preparation_id = production_live_epoch_transition_settlement_execution_preparation_id(
            self.config.protocol_version.0,
            &settlement_preparation_intent_digest,
            &inputs.settlement_execution_preparation_policy_id,
            request.proposed_epoch_transition_target,
            request.settlement_execution_preparation_nonce,
        );
        ...
        let settlement_execution_preparation_digest = String::new();
        ...
        ProductionLiveEpochTransitionSettlementExecutionPreparationDecision {
            ...
            request_id,
            settlement_execution_preparation_artifact: None,
            settlement_execution_preparation_digest,
            transcript_digest,
        }
    }

    /// Run 337 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_settlement_execution_preparation_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact>,
        current: &ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact,
    ) -> ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome {
        use ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::Disabled
        {
...
/// Run 337 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::default()
        == ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionSettlementExecutionPreparationConfig::default().kind
            == ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind::Disabled
}

/// Run 337 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 338).
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 337 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_mainnet_refused() -> bool {
    true
}

/// Run 337 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_is_non_mutating() -> bool {
    true
}

/// Run 337 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_never_falls_back() -> bool {
    true
}

/// Run 337 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 337 — the executor only requires a verified Run 335/336 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_settlement_execution_preparation_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 337 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 337. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionSettlementExecutionPreparationFixtureState {
...
impl LiveEpochTransitionSettlementExecutionPreparationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, settlement_execution_preparation_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == settlement_execution_preparation_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `settlement_execution_preparation_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact,
        settlement_execution_preparation_id: &str,
    ) -> bool {
        if self.has_applied(settlement_execution_preparation_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(settlement_execution_preparation_id.to_string());
        true
    }
}