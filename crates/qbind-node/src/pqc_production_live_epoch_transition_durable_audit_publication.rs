//! Run 331 — source/test epoch-transition **runtime handoff** /
//! live-mutation **preflight** boundary.
//!
//! This module implements the next source/test boundary above the Run
//! 327/328 guarded epoch-transition durable-audit-publication boundary: the boundary
//! that consumes a **verified** Run 329/330 non-mutating guarded
//! epoch-transition durable-audit-publication *decision* (the accepted
//! [`ProductionLiveEpochTransitionAuditLedgerCommitmentDecision`] carrying a prepared
//! [`ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact`], as
//! release-binary-evidenced by Run 328) and translates it into a typed,
...
//!
//! Where the Run 327 boundary answers "given a verified staged
//! epoch-transition application decision, what typed non-mutating guarded
//! mutation record does it authorize?", Run 331 answers the next question:
//! "given a verified guarded durable-audit-publication decision, what typed,
...
//! epoch/version preconditions, a required replay window, and a audit-ledger-commitment
//! nonce?".
//!
//! ## Scope and honesty constraints (Run 331)
//!
//! * This run is a **source/test implementation only**. It is **not**
//!   release-binary evidence — that is deferred to **Run 332**.
//! * The default policy is
//!   [`ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::Disabled`]
//!   and fails closed **before** any binding, validator-set binding, or
//!   handoff-package construction.
//! * Only a **verified** Run 329/330 guarded epoch-transition
//!   durable-audit-publication decision that `is_accept()` and carries a
//!   [`ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact`] can authorize a runtime
//!   handoff package. Unverified decisions, accepted decisions without a
//!   prepared record, a Run 325/326 staged-application decision alone, a Run
//!   323/324 live-application authorization alone, a Run 321/322 application
//!   decision alone, Run 319 rotation plans alone, governance execution intent
...
//!   [`ProductionLiveEpochTransitionDurableAuditPublicationArtifact`]; it
...
use crate::pqc_production_live_epoch_transition_audit_ledger_commitment::{
...
/// Run 331 — the only live validator-set application authorization boundary
/// protocol version this run accepts.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_PROTOCOL_VERSION: u16 = 1;

/// Run 331 — runtime handoff content digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_INTENT_DOMAIN_TAG: &str =
    "QBIND:331-epoch-transition-audit-ledger-commitment-intent:v1";

/// Run 331 — runtime handoff id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_ID_DOMAIN_TAG: &str =
    "QBIND:331-epoch-transition-audit-ledger-commitment-id:v1";

/// Run 331 — runtime handoff request-id domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_REQUEST_DOMAIN_TAG: &str =
    "QBIND:331-epoch-transition-audit-ledger-commitment-request:v1";

/// Run 331 — runtime handoff transcript digest domain tag.
pub const PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_TRANSCRIPT_DOMAIN_TAG: &str =
    "QBIND:331-epoch-transition-audit-ledger-commitment-transcript:v1";
...
/// Run 331 — typed live validator-set application authorization boundary
/// protocol version. Only
/// [`PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_PROTOCOL_VERSION`]
/// is supported.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationProtocolVersion(pub u16);

impl ProductionLiveEpochTransitionDurableAuditPublicationProtocolVersion {
    /// The single supported protocol version.
    pub const fn supported() -> Self {
        Self(PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_PROTOCOL_VERSION)
    }

    /// Returns `true` iff this is the supported protocol version.
    pub const fn is_supported(self) -> bool {
        self.0 == PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_PROTOCOL_VERSION
    }
}

impl Default for ProductionLiveEpochTransitionDurableAuditPublicationProtocolVersion {
    fn default() -> Self {
        Self::supported()
    }
}
...
/// `AllowSourceTestLiveEpochTransitionDurableAuditPublication` is the only
/// policy that can produce an accepted source/test authorization, and only on
/// DevNet/TestNet with a verified Run 329/330 validator-set rotation
/// application decision.
/// `RequireProductionLiveEpochTransitionDurableAuditPublication` and
/// `MainnetProductionLiveEpochTransitionDurableAuditPublicationRequired` are
...
pub enum ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy {
...
impl ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::AllowSourceTestLiveEpochTransitionDurableAuditPublication => {
                "allow-source-test-epoch-transition-audit-ledger-commitment"
            }
            Self::RequireProductionLiveEpochTransitionDurableAuditPublication => {
                "require-production-epoch-transition-audit-ledger-commitment"
            }
            Self::MainnetProductionLiveEpochTransitionDurableAuditPublicationRequired => {
                "mainnet-production-epoch-transition-audit-ledger-commitment-required"
            }
        }
    }
...
/// `SourceTestLiveEpochTransitionDurableAuditPublication` performs real
/// source/test authorization construction. A reserved
/// `ProductionLiveEpochTransitionDurableAuditPublication` kind is fail-closed as
/// unavailable in Run 331 (no production authority is wired).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind {
    ...
    /// Reserved production authorization kind. Fail-closed in Run 331.
    ProductionLiveEpochTransitionDurableAuditPublication,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind {
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::SourceTestLiveEpochTransitionDurableAuditPublication => {
                "source-test-epoch-transition-audit-ledger-commitment"
            }
            Self::ProductionLiveEpochTransitionDurableAuditPublication => {
                "production-epoch-transition-audit-ledger-commitment"
            }
        }
    }
...
pub struct ProductionLiveEpochTransitionDurableAuditPublicationConfig {
    /// Boundary protocol version. Must equal the supported version.
    pub protocol_version: ProductionLiveEpochTransitionDurableAuditPublicationProtocolVersion,
    /// The boundary kind.
    pub kind: ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationConfig {
    pub fn new(kind: ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind) -> Self {
        Self {
            protocol_version:
                ProductionLiveEpochTransitionDurableAuditPublicationProtocolVersion::supported(),
            kind,
        }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::SourceTestLiveEpochTransitionDurableAuditPublication,
        )
    }
...
impl Default for ProductionLiveEpochTransitionDurableAuditPublicationConfig {
    fn default() -> Self {
        Self::new(ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::Disabled)
    }
}
...
/// Run 331 — the typed kind of a prepared, non-mutating staged live
/// validator-set / epoch-transition application record.
///
/// Each kind corresponds one-to-one with a supported Run 329/330
/// [`LiveEpochTransitionAuditLedgerCommitmentKind`]; the reserved
/// [`Self::UnsupportedStagedApplication`] never authorizes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum LiveEpochTransitionDurableAuditPublicationKind {
...
impl LiveEpochTransitionDurableAuditPublicationKind {
    pub const fn tag(self) -> &'static str {
        match self {
    ...
    /// Every Run 331 staged application kind is a *prepared*, non-mutating
    /// record; none applies a live validator-set change.
    pub const fn is_non_mutating(self) -> bool {
        true
    }

    /// Maps a supported Run 329/330 staged live validator-set / epoch-transition
    ...
    pub const fn from_staged_application_kind(
        kind: LiveEpochTransitionAuditLedgerCommitmentKind,
    ) -> Self {
        use LiveEpochTransitionAuditLedgerCommitmentKind as A;
        match kind {
...
/// Run 331 — the staged live validator-set / epoch-transition application
/// authority source presented to the executor.
///
/// Only [`Self::VerifiedAuditLedgerCommitmentDecision`] carrying a Run 329/330
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LiveEpochTransitionDurableAuditPublicationAuthoritySource {
    /// A verified Run 329/330 epoch-transition runtime handoff / live-mutation
    /// preflight decision. The **only** accepted authority source. The
    /// decision must `is_accept()` and carry `Some(handoff_package)`.
    VerifiedAuditLedgerCommitmentDecision {
        decision: ProductionLiveEpochTransitionAuditLedgerCommitmentDecision,
    },
    /// No runtime handoff decision was supplied.
    MissingAuditLedgerCommitmentDecision,
    /// An unverified / non-accept runtime handoff decision. Rejected.
    UnverifiedAuditLedgerCommitmentDecision {
        decision: ProductionLiveEpochTransitionAuditLedgerCommitmentDecision,
    },
    /// An accepted runtime handoff decision that carries no prepared runtime
    /// handoff package. Rejected.
    AcceptedAuditLedgerCommitmentWithoutPackage {
        decision: ProductionLiveEpochTransitionAuditLedgerCommitmentDecision,
    },
    /// A Run 327/328 epoch-transition commit-authorization decision presented
    /// directly, without a Run 329/330 audit-ledger-commitment decision. Rejected.
    CommitAuthorizationDecisionWithoutAuditLedgerCommitment,
    /// A Run 327/328 epoch-transition mutation-execution decision presented
    /// directly, without a Run 329/330 audit-ledger-commitment decision. Rejected.
    MutationExecutionDecisionWithoutAuditLedgerCommitment,
    /// A Run 327/328 epoch-transition execution-preparation decision presented
    /// directly, without a Run 329/330 audit-ledger-commitment decision. Rejected.
    ExecutionPreparationDecisionWithoutAuditLedgerCommitment,
    /// A Run 327/328 epoch-transition runtime handoff decision presented
    /// directly, without a Run 329/330 audit-ledger-commitment decision.
    /// Rejected.
    RuntimeHandoffDecisionWithoutAuditLedgerCommitment,
    /// A Run 327/328 guarded epoch-transition durable-audit-publication decision
    /// presented directly, without a Run 329/330 runtime handoff decision.
    /// Rejected.
    GuardedMutationDecisionWithoutAuditLedgerCommitment,
    /// A Run 325/326 staged-application decision presented directly, without a
    /// Run 329/330 runtime handoff decision. Rejected.
    StagedApplicationDecisionWithoutAuditLedgerCommitment,
    /// A Run 323/324 live-application authorization presented directly, without
    /// a Run 329/330 guarded durable-audit-publication decision. Rejected.
    LiveApplicationAuthorizationWithoutAuditLedgerCommitment,
    /// A Run 321/322 validator-set rotation *application decision* presented
    /// directly, without a Run 329/330 guarded durable-audit-publication decision.
    /// Rejected.
    ApplicationDecisionWithoutAuditLedgerCommitment,
    /// A Run 319/320 validator-set rotation plan presented directly, without a
    /// Run 329/330 live-application authorization. Rejected.
    RotationPlanWithoutAuditLedgerCommitment,
    /// A Run 317/318 governance execution intent presented directly, without a
    /// Run 329/330 live-application authorization. Rejected.
    GovernanceExecutionIntentWithoutAuditLedgerCommitment,
    /// A raw on-chain governance proof presented directly, without a Run
    /// 323/324 live-application authorization. Rejected.
    GovernanceProofWithoutAuditLedgerCommitment,
    ...
    FixtureOnlyAuditLedgerCommitment,
...
/// Run 331 — the explicit trusted inputs the executor binds a verified
/// Run 329/330 live validator-set application authorization decision against.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationInputs {
    ...
    pub durable_audit_publication_policy_id: String,
    /// Expected Run 329/330 live-application authorization policy id (bound
    /// into the consumed authorization intent).
    pub expected_authorization_policy_id: String,
    /// Expected Run 321/322 application policy id (re-exposed by the
    ...
    /// Expected Run 317 governance execution decision id.
    pub expected_governance_decision_id: String,
    /// Expected Run 317 governance execution request id.
    pub expected_governance_request_id: String,
    /// Expected Run 317 governance execution intent digest.
    pub expected_governance_intent_digest: String,
    /// Expected Run 319 rotation decision id.
    pub expected_rotation_decision_id: String,
    /// Expected Run 319 rotation request id.
    pub expected_rotation_request_id: String,
    /// Expected Run 319 rotation transcript digest.
    pub expected_rotation_transcript_digest: String,
    /// Expected Run 319 rotation plan digest.
    ...
    /// Expected Run 321/322 application decision id (re-exposed by the
    /// authorization intent).
    pub expected_application_decision_id: String,
    /// Expected Run 321/322 application request id.
    pub expected_application_request_id: String,
    /// Expected Run 321/322 application intent digest.
    pub expected_application_intent_digest: String,
    /// Expected Run 321/322 application transcript digest.
    pub expected_application_transcript_digest: String,
    /// Expected Run 329/330 authorization decision id (bound into the
    /// consumed authorization decision).
    pub expected_authorization_decision_id: String,
    /// Expected Run 329/330 authorization request id.
    pub expected_authorization_request_id: String,
    /// Expected Run 329/330 authorization intent digest.
    pub expected_authorization_intent_digest: String,
    /// Expected Run 329/330 authorization transcript digest.
    pub expected_authorization_transcript_digest: String,
    /// Expected Run 329/330 staged-application decision id (bound into the
    /// consumed staged-application decision).
    pub expected_staged_application_decision_id: String,
    /// Expected Run 329/330 staged-application request id.
    pub expected_staged_application_request_id: String,
    /// Expected Run 329/330 staged-application intent digest.
    pub expected_staged_application_intent_digest: String,
    /// Expected Run 329/330 staged-application transcript digest.
    pub expected_staged_application_transcript_digest: String,
    /// Expected Run 329/330 staged-application nonce (re-exposed by the
    ...
    /// Expected Run 321/322 application nonce (re-exposed by the authorization
    /// intent).
    pub expected_application_nonce: u64,
    /// Expected Run 329/330 live-application nonce (re-exposed by the
    /// authorization intent).
    pub expected_live_application_nonce: u64,
    /// Expected Run 327/328 guarded durable-audit-publication decision id (re-exposed
    /// by the consumed Run 329/330 runtime handoff package).
    pub expected_guarded_mutation_decision_id: String,
    /// Expected Run 327/328 guarded durable-audit-publication request id.
    pub expected_guarded_mutation_request_id: String,
    /// Expected Run 327/328 guarded durable-audit-publication intent digest.
    pub expected_guarded_mutation_intent_digest: String,
    /// Expected Run 327/328 guarded durable-audit-publication transcript digest.
    pub expected_guarded_mutation_transcript_digest: String,
    /// Expected Run 327/328 guarded mutation nonce (re-exposed by the consumed
    /// runtime handoff package).
    pub expected_guarded_mutation_nonce: u64,
    /// Expected Run 329/330 runtime handoff decision id (bound into the
    /// consumed runtime handoff decision).
    pub expected_audit_ledger_commitment_decision_id: String,
    /// Expected Run 329/330 runtime handoff request id.
    pub expected_audit_ledger_commitment_request_id: String,
    /// Expected Run 329/330 runtime handoff package/content digest (the
    /// consumed decision's `durable_audit_publication_digest`, i.e. the handoff content
    /// digest).
    pub expected_audit_ledger_commitment_intent_digest: String,
    /// Expected Run 329/330 runtime handoff transcript digest.
    pub expected_audit_ledger_commitment_transcript_digest: String,
    /// Expected Run 329/330 audit-ledger-commitment nonce (re-exposed by the
    /// consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_audit_ledger_commitment_nonce: u64,
    /// Expected Run 327/328 commit-authorization decision id (re-exposed by
    /// the consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_commit_authorization_decision_id: String,
    /// Expected Run 327/328 commit-authorization request id.
    pub expected_commit_authorization_request_id: String,
    /// Expected Run 327/328 commit-authorization intent/content digest.
    pub expected_commit_authorization_intent_digest: String,
    /// Expected Run 327/328 commit-authorization transcript digest.
    pub expected_commit_authorization_transcript_digest: String,
    /// Expected Run 327/328 commit-authorization nonce (re-exposed by the
    /// consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_commit_authorization_nonce: u64,
    /// Expected Run 327/328 mutation-execution decision id (re-exposed by
    /// the consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_mutation_execution_decision_id: String,
    /// Expected Run 327/328 mutation-execution request id.
    pub expected_mutation_execution_request_id: String,
    /// Expected Run 327/328 mutation-execution intent/content digest.
    pub expected_mutation_execution_intent_digest: String,
    /// Expected Run 327/328 mutation-execution transcript digest.
    pub expected_mutation_execution_transcript_digest: String,
    /// Expected Run 327/328 mutation-execution nonce (re-exposed by the
    /// consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_mutation_execution_nonce: u64,
    /// Expected Run 327/328 execution-preparation decision id (re-exposed by
    /// the consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_execution_preparation_decision_id: String,
    /// Expected Run 327/328 execution-preparation request id.
    pub expected_execution_preparation_request_id: String,
    /// Expected Run 327/328 execution-preparation intent/content digest.
    pub expected_execution_preparation_intent_digest: String,
    /// Expected Run 327/328 execution-preparation transcript digest.
    pub expected_execution_preparation_transcript_digest: String,
    /// Expected Run 327/328 execution-preparation nonce (re-exposed by the
    /// consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_execution_preparation_nonce: u64,
    /// Expected Run 327/328 runtime handoff decision id (re-exposed by the
    /// consumed Run 329/330 audit-ledger-commitment artifact).
    pub expected_runtime_handoff_decision_id: String,
    /// Expected Run 327/328 runtime handoff request id.
    pub expected_runtime_handoff_request_id: String,
    /// Expected Run 327/328 runtime handoff package/content digest.
    pub expected_runtime_handoff_intent_digest: String,
    /// Expected Run 327/328 runtime handoff transcript digest.
    pub expected_runtime_handoff_transcript_digest: String,
    /// Expected Run 327/328 runtime handoff nonce (re-exposed by the consumed
    /// Run 329/330 audit-ledger-commitment artifact).
    ...
    /// wall-clock value and never a reject path in Run 331.
...
impl ProductionLiveEpochTransitionDurableAuditPublicationInputs {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.trust_domain.chain_id.is_empty()
            ...
            && !self.durable_audit_publication_policy_id.is_empty()
            ...
            && !self.expected_audit_ledger_commitment_decision_id.is_empty()
            && !self.expected_audit_ledger_commitment_request_id.is_empty()
            && !self.expected_audit_ledger_commitment_intent_digest.is_empty()
            && !self.expected_audit_ledger_commitment_transcript_digest.is_empty()
...
/// Run 331 — a runtime handoff request: the authority source (a verified
/// guarded epoch-transition durable-audit-publication decision), the explicit
/// epoch-transition target, a audit-ledger-commitment nonce, and any represented
/// custody / attestation / durable-replay evidence bindings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationRequest {
    pub authority_source: LiveEpochTransitionDurableAuditPublicationAuthoritySource,
    /// The epoch a future epoch-transition executor would transition to.
    pub proposed_epoch_transition_target: u64,
    /// The audit-ledger-commitment nonce (idempotency / replay binding).
    pub durable_audit_publication_nonce: u64,
...
impl ProductionLiveEpochTransitionDurableAuditPublicationRequest {
    /// Construct a request carrying only an authority source, epoch target,
    /// and audit-ledger-commitment nonce (no represented custody / attestation /
    /// durable-replay evidence).
    pub fn new(
        authority_source: LiveEpochTransitionDurableAuditPublicationAuthoritySource,
        proposed_epoch_transition_target: u64,
        durable_audit_publication_nonce: u64,
    ) -> Self {
        Self {
            authority_source,
            proposed_epoch_transition_target,
            durable_audit_publication_nonce,
...
/// Run 331 — caller-owned replay authorization-id set. The executor reads
/// from this set but never mutates it.
pub trait LiveEpochTransitionDurableAuditPublicationReplaySet {
    fn contains(&self, authorization_id: &str) -> bool;
}

impl LiveEpochTransitionDurableAuditPublicationReplaySet for &[String] {
    fn contains(&self, authorization_id: &str) -> bool {
        (*self).iter().any(|s| s == authorization_id)
    }
}

impl LiveEpochTransitionDurableAuditPublicationReplaySet for Vec<String> {
    fn contains(&self, authorization_id: &str) -> bool {
        self.iter().any(|s| s == authorization_id)
    }
}
...
pub struct EmptyLiveEpochTransitionDurableAuditPublicationReplaySet;

impl LiveEpochTransitionDurableAuditPublicationReplaySet
    for EmptyLiveEpochTransitionDurableAuditPublicationReplaySet
{
...
/// Run 331 — a typed, deterministic, **non-mutating** epoch-transition runtime
/// handoff package. Only a typed accepted outcome carrying this package may
/// authorize a *future* real live mutation run (Run 332+); Run 331 never
...
/// preconditions, and carries deterministic `durable_audit_publication_id`, `request_id`,
/// `durable_audit_publication_digest`, and `transcript_digest` identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationArtifact {
    pub staged_kind: LiveEpochTransitionDurableAuditPublicationKind,
    pub protocol_version: u16,
    pub durable_audit_publication_policy_id: String,

    // ---- Re-exposed Run 329/330 authorization intent tuple ------------
    ...
    // ---- Re-exposed Run 321/322 application-decision tuple ------------
    ...
    // ---- Bound Run 329/330 authorization-decision authority tuple -----
    ...
    // ---- Bound Run 329/330 staged-application-decision authority tuple -
    ...
    // ---- Bound Run 329/330 guarded durable-audit-publication authority tuple -
    // (the consumed guarded durable-audit-publication decision transcript)
    ...
    // ---- Guarded durable-audit-publication binding (re-exposed consumed nonce)
    /// The re-exposed Run 327/328 guarded durable-audit-publication nonce.
    ...
    pub audit_ledger_commitment_decision_id: String,
    pub audit_ledger_commitment_request_id: String,
    pub audit_ledger_commitment_intent_digest: String,
    pub audit_ledger_commitment_transcript_digest: String,
    /// The re-exposed Run 329/330 audit-ledger-commitment nonce (consumed).
    pub audit_ledger_commitment_nonce: u64,

    // ---- Bound Run 327/328 commit-authorization authority tuple -------
    // (re-exposed by the consumed Run 329/330 audit-ledger-commitment artifact)
    ...
    /// The re-exposed Run 327/328 commit-authorization nonce (consumed).
    pub commit_authorization_nonce: u64,

    // ---- Bound Run 327/328 mutation-execution authority tuple -------
    // (re-exposed by the consumed Run 329/330 audit-ledger-commitment artifact)
    ...
    /// The re-exposed Run 327/328 mutation-execution nonce (consumed).
    pub mutation_execution_nonce: u64,

    // ---- Bound Run 327/328 execution-preparation authority tuple -------
    // (re-exposed by the consumed Run 329/330 audit-ledger-commitment artifact)
    ...
    /// The re-exposed Run 327/328 execution-preparation nonce (consumed).
    pub execution_preparation_nonce: u64,

    // ---- Bound Run 327/328 runtime handoff decision authority tuple ----
    // (re-exposed by the consumed Run 329/330 audit-ledger-commitment artifact)
    ...
    /// The re-exposed Run 327/328 runtime handoff nonce (consumed).
    pub runtime_handoff_nonce: u64,

    // ---- Execution preparation binding --------------------------------
    /// The newly proposed durable-audit-publication nonce.
    pub durable_audit_publication_nonce: u64,
    ...
    pub durable_audit_publication_id: String,
    pub request_id: String,
    pub durable_audit_publication_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationArtifact {
    /// Deterministic, domain-separated SHA3-256 hex content digest over every
    /// field **except** the four identifier fields (`durable_audit_publication_id`,
    /// `request_id`, `durable_audit_publication_digest`, `transcript_digest`). `Debug`
    /// formatting is never used as canonical bytes.
    pub fn content_digest(&self) -> String {
        use sha3::{Digest, Sha3_256};
        let mut h = Sha3_256::new();
        h.update(
            PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_INTENT_DOMAIN_TAG.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"durable_audit_publication_policy_id",
            self.durable_audit_publication_policy_id.as_bytes(),
        );
        ...
        hash_field(
            &mut h,
            b"audit_ledger_commitment_decision_id",
            self.audit_ledger_commitment_decision_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"audit_ledger_commitment_request_id",
            self.audit_ledger_commitment_request_id.as_bytes(),
        );
        hash_field(
            &mut h,
            b"audit_ledger_commitment_intent_digest",
            self.audit_ledger_commitment_intent_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"audit_ledger_commitment_transcript_digest",
            self.audit_ledger_commitment_transcript_digest.as_bytes(),
        );
        hash_field(
            &mut h,
            b"audit_ledger_commitment_nonce",
            &self.audit_ledger_commitment_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
        ...
        hash_field(
            &mut h,
            b"durable_audit_publication_nonce",
            &self.durable_audit_publication_nonce.to_le_bytes(),
        );
        hash_field(
            &mut h,
...
/// Custody binding canonical hashing (module-local; mirrors Run 317/319/321
...
/// Run 331 — deterministic runtime handoff package content digest wrapper
/// exposed as a named symbol.
pub fn production_live_epoch_transition_durable_audit_publication_content_digest(
    package: &ProductionLiveEpochTransitionDurableAuditPublicationArtifact,
) -> String {
    package.content_digest()
}

/// Run 331 — deterministic, domain-separated runtime handoff request id
/// binding the protocol version, guarded mutation intent digest, handoff
/// policy id, epoch-transition target, and audit-ledger-commitment nonce.
/// Deterministic across identical inputs; never wall-clock.
pub fn production_live_epoch_transition_durable_audit_publication_request_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    durable_audit_publication_policy_id: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_REQUEST_DOMAIN_TAG.as_bytes(),
    );
    ...
    hash_field(
        &mut h,
        b"durable_audit_publication_policy_id",
        durable_audit_publication_policy_id.as_bytes(),
    );
    hash_field(
        &mut h,
    ...
    hash_field(
        &mut h,
        b"durable_audit_publication_nonce",
        &durable_audit_publication_nonce.to_le_bytes(),
    );
    hex::encode(h.finalize())
}

/// Run 331 — deterministic, domain-separated runtime handoff id binding the
/// protocol version, guarded mutation intent digest, handoff policy id,
/// epoch-transition target, and audit-ledger-commitment nonce. Deterministic across
/// identical inputs; never wall-clock. Distinct domain-separated from the
/// request id.
pub fn production_live_epoch_transition_durable_audit_publication_id(
    protocol_version: u16,
    guarded_mutation_intent_digest: &str,
    durable_audit_publication_policy_id: &str,
    epoch_transition_target: u64,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_ID_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
    hash_field(
        &mut h,
    ...
    hash_field(&mut h, b"durable_audit_publication_policy_id", durable_audit_publication_policy_id.as_bytes());
    hash_field(
        &mut h,
        b"epoch_transition_target",
        &epoch_transition_target.to_le_bytes(),
...
/// Run 331 — deterministic, domain-separated runtime handoff transcript
/// digest binding the protocol version, request id, handoff (content) digest,
/// and outcome tag.
pub fn production_live_epoch_transition_durable_audit_publication_transcript_digest(
    protocol_version: u16,
    request_id: &str,
    intent_digest: &str,
    outcome_tag: &str,
    ...
    let mut h = Sha3_256::new();
    h.update(
        PRODUCTION_LIVE_EPOCH_TRANSITION_DURABLE_AUDIT_PUBLICATION_TRANSCRIPT_DOMAIN_TAG.as_bytes(),
    );
    hash_field(&mut h, b"protocol_version", &protocol_version.to_le_bytes());
...
/// Run 331 — typed outcome of the staged live validator-set / epoch-transition
/// application executor boundary.
///
/// Only
/// [`Self::AcceptedSourceTestLiveEpochTransitionDurableAuditPublication`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionDurableAuditPublicationOutcome {
    // ---- Disabled / unavailable ---------------------------------------
    /// Policy is `Disabled`; no authority was bound.
    Disabled,
    /// The boundary kind is unavailable / misconfigured.
    LiveEpochTransitionDurableAuditPublicationBoundaryUnavailable,
    ...
    MainNetProductionLiveEpochTransitionDurableAuditPublicationUnavailable,

    // ---- Accepted ------------------------------------------------------
    /// A verified DevNet/TestNet guarded epoch-transition durable-audit-publication
    /// decision produced a typed non-mutating runtime handoff package under
    /// the source/test policy. **Evidence only.**
    AcceptedSourceTestLiveEpochTransitionDurableAuditPublication {
        execution_kind: LiveEpochTransitionDurableAuditPublicationKind,
    ...
    VerifiedAuditLedgerCommitmentDecisionRequired,
    UnverifiedAuditLedgerCommitmentDecisionRejected,
    ...
    AuditLedgerCommitmentDecisionIdMismatch,
    AuditLedgerCommitmentDecisionRequestIdMismatch,
    AuditLedgerCommitmentDecisionIntentDigestMismatch,
    AuditLedgerCommitmentDecisionTranscriptMismatch,
    AuditLedgerCommitmentDecisionIntegrityMismatch,
    WrongAuditLedgerCommitmentNonce,
    ...
    LiveEpochTransitionDurableAuditPublicationAmbiguous { reason: String },
    MainNetRefused,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationOutcome {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::LiveEpochTransitionDurableAuditPublicationBoundaryUnavailable => {
                "live-epoch-transition-durable-audit-publication-boundary-unavailable"
            }
            Self::ProductionLiveEpochTransitionDurableAuditPublicationUnavailable => {
                "production-live-epoch-transition-durable-audit-publication-unavailable"
            }
            Self::MainNetProductionLiveEpochTransitionDurableAuditPublicationUnavailable => {
                "mainnet-production-live-epoch-transition-durable-audit-publication-unavailable"
            }
            Self::AcceptedSourceTestLiveEpochTransitionDurableAuditPublication { .. } => {
                "accepted-source-test-live-epoch-transition-durable-audit-publication"
            }
            Self::VerifiedAuditLedgerCommitmentDecisionRequired => {
                "verified-audit-ledger-commitment-decision-required"
            }
            Self::UnverifiedAuditLedgerCommitmentDecisionRejected => {
                "unverified-audit-ledger-commitment-decision-rejected"
            }
            ...
            Self::FixtureStagedApplicationRejectedAsProductionAuthority => {
                "fixture-audit-ledger-commitment-decision-rejected-as-production-authority"
            }
            ...
            Self::AuditLedgerCommitmentDecisionIdMismatch => "audit-ledger-commitment-decision-id-mismatch",
            Self::AuditLedgerCommitmentDecisionRequestIdMismatch => {
                "audit-ledger-commitment-decision-request-id-mismatch"
            }
            Self::AuditLedgerCommitmentDecisionIntentDigestMismatch => {
                "audit-ledger-commitment-decision-intent-digest-mismatch"
            }
            Self::AuditLedgerCommitmentDecisionTranscriptMismatch => {
                "audit-ledger-commitment-decision-transcript-mismatch"
            }
            Self::AuditLedgerCommitmentDecisionIntegrityMismatch => {
                "audit-ledger-commitment-decision-integrity-mismatch"
            }
            Self::WrongAuditLedgerCommitmentNonce => "wrong-audit-ledger-commitment-nonce",
            ...
            Self::LiveEpochTransitionDurableAuditPublicationAmbiguous { .. } => {
                "epoch-transition-audit-ledger-commitment-ambiguous"
            }
...
/// Run 331 — the typed decision produced by the executor boundary: the
...
/// transcript digest.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationDecision {
    pub outcome: ProductionLiveEpochTransitionDurableAuditPublicationOutcome,
    pub durable_audit_publication_id: String,
    pub request_id: String,
    pub durable_audit_publication_artifact:
        Option<ProductionLiveEpochTransitionDurableAuditPublicationArtifact>,
    pub durable_audit_publication_digest: String,
    pub transcript_digest: String,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationDecision {
    pub fn is_accept(&self) -> bool {
    ...
    /// it.
    pub fn authorizes_future_mutation_only(&self) -> bool {
        self.outcome.authorizes_future_mutation_only() && self.durable_audit_publication_artifact.is_some()
    }
}
...
/// Run 331 — typed idempotency / recovery outcome for a prepared staged
/// application window. Every variant is non-mutating; no durable state is
/// written.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome {
...
impl ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome {
...
/// Run 331 — the source/test staged live validator-set / epoch-transition
/// application executor boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProductionLiveEpochTransitionDurableAuditPublicationExecutor {
    pub config: ProductionLiveEpochTransitionDurableAuditPublicationConfig,
    pub policy: ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy,
}

impl ProductionLiveEpochTransitionDurableAuditPublicationExecutor {
    pub fn new(
        config: ProductionLiveEpochTransitionDurableAuditPublicationConfig,
        policy: ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy,
    ) -> Self {
        Self { config, policy }
    }
    ...
    pub fn source_test() -> Self {
        Self::new(
            ProductionLiveEpochTransitionDurableAuditPublicationConfig::source_test(),
            ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::AllowSourceTestLiveEpochTransitionDurableAuditPublication,
        )
    }
    ...
    fn resolve_authority_source<'a>(
        &self,
        source: &'a LiveEpochTransitionDurableAuditPublicationAuthoritySource,
    ) -> Result<
        (
            &'a ProductionLiveEpochTransitionAuditLedgerCommitmentDecision,
            &'a ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact,
        ),
        ProductionLiveEpochTransitionDurableAuditPublicationOutcome,
    > {
        use ProductionLiveEpochTransitionDurableAuditPublicationOutcome as O;
        use LiveEpochTransitionDurableAuditPublicationAuthoritySource as S;
        match source {
            S::VerifiedAuditLedgerCommitmentDecision { decision } => {
                if !decision.is_accept() {
                    return Err(O::UnverifiedAuditLedgerCommitmentDecisionRejected);
                }
                match &decision.audit_ledger_commitment_artifact {
                    Some(intent) => Ok((decision, intent)),
                    None => Err(O::VerifiedAuditLedgerCommitmentDecisionRequired),
                }
            }
            S::MissingAuditLedgerCommitmentDecision => {
                Err(O::VerifiedAuditLedgerCommitmentDecisionRequired)
            }
            S::UnverifiedAuditLedgerCommitmentDecision { .. } => {
                Err(O::UnverifiedAuditLedgerCommitmentDecisionRejected)
            }
            S::AcceptedAuditLedgerCommitmentWithoutPackage { .. } => {
                Err(O::VerifiedAuditLedgerCommitmentDecisionRequired)
            }
            S::CommitAuthorizationDecisionWithoutAuditLedgerCommitment => {
                Err(O::CommitAuthorizationDecisionAloneRejected)
            }
            S::MutationExecutionDecisionWithoutAuditLedgerCommitment => {
                Err(O::MutationExecutionDecisionAloneRejected)
            }
            S::ExecutionPreparationDecisionWithoutAuditLedgerCommitment => {
                Err(O::ExecutionPreparationDecisionAloneRejected)
            }
            S::RuntimeHandoffDecisionWithoutAuditLedgerCommitment => {
                Err(O::RuntimeHandoffDecisionAloneRejected)
            }
            S::GuardedMutationDecisionWithoutAuditLedgerCommitment => {
                Err(O::GuardedMutationDecisionAloneRejected)
            }
            S::ApplicationDecisionWithoutAuditLedgerCommitment => {
                Err(O::ApplicationDecisionAloneRejected)
            }
            S::StagedApplicationDecisionWithoutAuditLedgerCommitment => {
                Err(O::StagedApplicationDecisionAloneRejected)
            }
            S::LiveApplicationAuthorizationWithoutAuditLedgerCommitment => {
                Err(O::LiveApplicationAuthorizationAloneRejected)
            }
            S::RotationPlanWithoutAuditLedgerCommitment => Err(O::RotationPlanAloneRejected),
            S::GovernanceExecutionIntentWithoutAuditLedgerCommitment => {
                Err(O::GovernanceExecutionIntentAloneRejected)
            }
            S::GovernanceProofWithoutAuditLedgerCommitment => {
            ...
            S::FixtureOnlyAuditLedgerCommitment => {
    ...
    fn preflight_gate(
    ...
    ) -> Option<ProductionLiveEpochTransitionDurableAuditPublicationOutcome> {
        use ProductionLiveEpochTransitionDurableAuditPublicationOutcome as O;

        // 1. Disabled fails closed before any binding.
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::Disabled
        {
            ...
            || binding_env == TrustBundleEnvironment::Mainnet
        {
            return Some(match self.policy {
                ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::MainnetProductionLiveEpochTransitionDurableAuditPublicationRequired => {
                    O::MainNetProductionLiveEpochTransitionDurableAuditPublicationUnavailable
                }
                _ => O::MainNetRefused,
            });
        }
        ...
        if self.policy.is_mainnet() {
            return Some(O::MainNetProductionLiveEpochTransitionDurableAuditPublicationUnavailable);
        }

        // 4. The production policy has no production prerequisites wired —
        //    fail closed.
        if self.policy.is_production() {
            return Some(O::ProductionLiveEpochTransitionDurableAuditPublicationUnavailable);
        }

        // 5. Reserved production boundary kind is fail-closed in Run 331.
        if self.config.kind
            == ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::ProductionLiveEpochTransitionDurableAuditPublication
        {
            return Some(O::LiveEpochTransitionDurableAuditPublicationBoundaryUnavailable);
        }
    ...
    /// Cross-check the verified Run 329/330 live-application authorization
    ...
    fn check_application_binding(
        &self,
        decision: &ProductionLiveEpochTransitionAuditLedgerCommitmentDecision,
        intent: &ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact,
        inputs: &ProductionLiveEpochTransitionDurableAuditPublicationInputs,
    ) -> Option<ProductionLiveEpochTransitionDurableAuditPublicationOutcome> {
        use ProductionLiveEpochTransitionDurableAuditPublicationOutcome as O;
        let td = &inputs.trust_domain;

        // Consumed Run 329/330 epoch-transition runtime handoff decision
        // transcript binding.
        if decision.audit_ledger_commitment_id != inputs.expected_audit_ledger_commitment_decision_id {
            return Some(O::AuditLedgerCommitmentDecisionIdMismatch);
        }
        if decision.request_id != inputs.expected_audit_ledger_commitment_request_id {
            return Some(O::AuditLedgerCommitmentDecisionRequestIdMismatch);
        }
        if decision.audit_ledger_commitment_digest != inputs.expected_audit_ledger_commitment_intent_digest {
            return Some(O::AuditLedgerCommitmentDecisionIntentDigestMismatch);
        }
        if decision.transcript_digest != inputs.expected_audit_ledger_commitment_transcript_digest {
            return Some(O::AuditLedgerCommitmentDecisionTranscriptMismatch);
        }
        // The prepared runtime handoff package must reproduce the bound
        // runtime handoff decision content (handoff) digest.
        if intent.content_digest() != decision.audit_ledger_commitment_digest {
            return Some(O::AuditLedgerCommitmentDecisionIntegrityMismatch);
        }
        // The consumed runtime handoff package's re-exposed audit-ledger-commitment
        // nonce binding.
        if intent.audit_ledger_commitment_nonce != inputs.expected_audit_ledger_commitment_nonce {
            return Some(O::WrongAuditLedgerCommitmentNonce);
        }

        // Re-exposed Run 327/328 commit-authorization decision authority
        // tuple binding (carried through the consumed Run 329/330
        // audit-ledger-commitment artifact).
        ...
        // Re-exposed Run 327/328 mutation-execution decision authority
        // tuple binding (carried through the consumed Run 329/330
        // audit-ledger-commitment artifact).
        ...
        // Re-exposed Run 327/328 execution-preparation decision authority
        // tuple binding (carried through the consumed Run 329/330
        // audit-ledger-commitment artifact).
        ...
        // Re-exposed Run 327/328 epoch-transition runtime handoff decision
        // authority tuple binding (carried through the consumed Run 329/330
        // audit-ledger-commitment artifact).
        ...
        // Re-exposed Run 327/328 guarded epoch-transition durable-audit-publication
        ...
        // Re-exposed Run 329/330 staged-application decision authority tuple
        ...
        // Re-exposed Run 329/330 live-authorization decision authority tuple
        // binding.
        ...
        // Re-exposed Run 321/322 application-decision authority tuple binding.
    ...
    fn evaluate_core<R: LiveEpochTransitionDurableAuditPublicationReplaySet + ?Sized>(
        &self,
        request: &ProductionLiveEpochTransitionDurableAuditPublicationRequest,
        inputs: &ProductionLiveEpochTransitionDurableAuditPublicationInputs,
        replay_set: &R,
        ...
        let staged_application_id = production_live_epoch_transition_durable_audit_publication_request_id(
            self.config.protocol_version.0,
            &decision.audit_ledger_commitment_digest,
            &inputs.durable_audit_publication_policy_id,
            request.proposed_epoch_transition_target,
            request.durable_audit_publication_nonce,
        );
        ...
        let staged_kind =
            LiveEpochTransitionDurableAuditPublicationKind::from_staged_application_kind(
        ...
        let mut record = ProductionLiveEpochTransitionDurableAuditPublicationArtifact {
            staged_kind,
            protocol_version: self.config.protocol_version.0,
            durable_audit_publication_policy_id: inputs.durable_audit_publication_policy_id.clone(),
            ...
            // Re-exposed Run 327/328 guarded durable-audit-publication decision
            ...
            guarded_mutation_transcript_digest: application_intent
                ...
                .clone(),
            // Re-exposed guarded durable-audit-publication nonce.
            guarded_mutation_nonce: application_intent.guarded_mutation_nonce,
            // Consumed Run 329/330 runtime handoff decision transcript (the
            // authority source this run consumes).
            audit_ledger_commitment_decision_id: decision.audit_ledger_commitment_id.clone(),
            audit_ledger_commitment_request_id: decision.request_id.clone(),
            audit_ledger_commitment_intent_digest: decision.audit_ledger_commitment_digest.clone(),
            audit_ledger_commitment_transcript_digest: decision.transcript_digest.clone(),
            // Re-exposed consumed audit-ledger-commitment nonce.
            audit_ledger_commitment_nonce: application_intent.audit_ledger_commitment_nonce,
            // Re-exposed Run 327/328 commit-authorization decision authority
            // tuple (carried through the consumed audit-ledger-commitment artifact).
            ...
            // Re-exposed Run 327/328 runtime handoff decision authority tuple
            // (carried through the consumed audit-ledger-commitment artifact).
            ...
            // Newly proposed durable-audit-publication nonce.
            durable_audit_publication_nonce: request.durable_audit_publication_nonce,
            // Exact future-executor preconditions.
            ...
            durable_audit_publication_id: String::new(),
            request_id: String::new(),
            durable_audit_publication_digest: String::new(),
            transcript_digest: String::new(),
        };

        // Step 10: typed accepted non-mutating outcome + deterministic
        // identifiers bound into the package.
        let outcome = O::AcceptedSourceTestLiveEpochTransitionDurableAuditPublication {
            execution_kind: staged_kind,
            environment: application_intent.environment,
            epoch_transition_target: request.proposed_epoch_transition_target,
            durable_audit_publication_nonce: request.durable_audit_publication_nonce,
        };
        let durable_audit_publication_id = production_live_epoch_transition_durable_audit_publication_id(
            self.config.protocol_version.0,
            &decision.audit_ledger_commitment_digest,
            &inputs.durable_audit_publication_policy_id,
            request.proposed_epoch_transition_target,
        ...
        let request_id = production_live_epoch_transition_durable_audit_publication_request_id(
            self.config.protocol_version.0,
            &decision.audit_ledger_commitment_digest,
            &inputs.durable_audit_publication_policy_id,
            request.proposed_epoch_transition_target,
        ...
        record.durable_audit_publication_digest = record.content_digest();
        record.durable_audit_publication_id = durable_audit_publication_id;
        record.request_id = request_id.clone();
        record.transcript_digest =
            production_live_epoch_transition_durable_audit_publication_transcript_digest(
                self.config.protocol_version.0,
                &request_id,
                &record.durable_audit_publication_digest,
                outcome.tag(),
            );
    ...
    /// Run 331 — evaluate a staged live validator-set / epoch-transition
    ...
    pub fn evaluate_live_epoch_transition_durable_audit_publication<
        R: LiveEpochTransitionDurableAuditPublicationReplaySet + ?Sized,
    >(
        &self,
        request: &ProductionLiveEpochTransitionDurableAuditPublicationRequest,
        inputs: &ProductionLiveEpochTransitionDurableAuditPublicationInputs,
        ...
        if let Some(pkg) = &record {
            return ProductionLiveEpochTransitionDurableAuditPublicationDecision {
                outcome,
                durable_audit_publication_id: pkg.durable_audit_publication_id.clone(),
                request_id: pkg.request_id.clone(),
                durable_audit_publication_artifact: record.clone(),
                durable_audit_publication_digest: pkg.durable_audit_publication_digest.clone(),
        ...
        let audit_ledger_commitment_intent_digest = match &request.authority_source {
            LiveEpochTransitionDurableAuditPublicationAuthoritySource::VerifiedAuditLedgerCommitmentDecision {
                decision,
            }
            | LiveEpochTransitionDurableAuditPublicationAuthoritySource::UnverifiedAuditLedgerCommitmentDecision {
                decision,
            }
            | LiveEpochTransitionDurableAuditPublicationAuthoritySource::AcceptedAuditLedgerCommitmentWithoutPackage {
                decision,
            } => decision.audit_ledger_commitment_digest.clone(),
            _ => String::new(),
        };

        let durable_audit_publication_id = production_live_epoch_transition_durable_audit_publication_id(
            self.config.protocol_version.0,
            &audit_ledger_commitment_intent_digest,
            &inputs.durable_audit_publication_policy_id,
            request.proposed_epoch_transition_target,
            request.durable_audit_publication_nonce,
        );
        ...
        let durable_audit_publication_digest = String::new();
        ...
        ProductionLiveEpochTransitionDurableAuditPublicationDecision {
            outcome,
            durable_audit_publication_id,
            request_id,
            durable_audit_publication_artifact: None,
            durable_audit_publication_digest,
            transcript_digest,
        }
    }

    /// Run 331 — idempotency / recovery over a prepared-authorization window.
    /// Non-mutating; writes no durable state.
    pub fn recover_live_epoch_transition_durable_audit_publication_window(
        &self,
        prior: Option<&ProductionLiveEpochTransitionDurableAuditPublicationArtifact>,
        current: &ProductionLiveEpochTransitionDurableAuditPublicationArtifact,
    ) -> ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome {
        use ProductionLiveEpochTransitionDurableAuditPublicationRecoveryOutcome as R;
        if self.policy.is_disabled()
            || self.config.kind
                == ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::Disabled
        {
...
/// Run 331 — the executor default policy is Disabled / fail-closed.
pub fn production_live_epoch_transition_durable_audit_publication_executor_default_is_disabled() -> bool
{
    ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::default()
        == ProductionLiveEpochTransitionDurableAuditPublicationExecutorPolicy::Disabled
        && ProductionLiveEpochTransitionDurableAuditPublicationConfig::default().kind
            == ProductionLiveEpochTransitionDurableAuditPublicationExecutorKind::Disabled
}

/// Run 331 — the executor is a source/test implementation, not
/// release-binary evidence (deferred to Run 332).
pub fn production_live_epoch_transition_durable_audit_publication_executor_is_source_test_not_release_binary_evidence(
) -> bool {
    true
}

/// Run 331 — the executor refuses MainNet absent production authority.
pub fn production_live_epoch_transition_durable_audit_publication_executor_mainnet_refused() -> bool {
    true
}

/// Run 331 — the executor never applies a live validator-set change,
/// consensus epoch transition, or trust-state mutation; every outcome is
/// non-mutating.
pub fn production_live_epoch_transition_durable_audit_publication_executor_is_non_mutating() -> bool {
    true
}

/// Run 331 — the executor never falls back to rotation-plan-alone /
...
pub fn production_live_epoch_transition_durable_audit_publication_executor_never_falls_back() -> bool {
    true
}

/// Run 331 — the executor adds no default runtime wiring and no CLI flag.
pub fn production_live_epoch_transition_durable_audit_publication_executor_no_default_runtime_wiring(
) -> bool {
    true
}

/// Run 331 — the executor only requires a verified Run 329/330 validator-set
/// rotation application decision as authority; nothing else can authorize a
/// live-application authorization.
pub fn production_live_epoch_transition_durable_audit_publication_executor_requires_verified_application_decision(
) -> bool {
    true
}
// ===========================================================================
...
/// Run 331 — an explicit, in-memory fixture ledger/state used *only* by tests
...
/// in Run 331. It is not wired into node runtime, never touches production
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LiveEpochTransitionDurableAuditPublicationFixtureState {
...
impl LiveEpochTransitionDurableAuditPublicationFixtureState {
    ...
    /// fixture ledger.
    pub fn has_applied(&self, durable_audit_publication_id: &str) -> bool {
        self.applied_execution_ids.iter().any(|id| id == durable_audit_publication_id)
    }
    ...
    ///
    /// Idempotent: re-applying the same `durable_audit_publication_id` is a no-op returning
    ...
    pub fn apply_prepared_execution(
        &mut self,
        record: &ProductionLiveEpochTransitionDurableAuditPublicationArtifact,
        durable_audit_publication_id: &str,
    ) -> bool {
        if self.has_applied(durable_audit_publication_id) {
            return false;
        }
        ...
        self.applied_execution_ids.push(durable_audit_publication_id.to_string());
        true
    }
}