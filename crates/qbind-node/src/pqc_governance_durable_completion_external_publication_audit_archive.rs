//! Run 286 — source/test durable-completion **external-publication-audit-completion
//! consumer / external-publication-audit-archive interface boundary**.
//!
//! Source/test only. Run 286 captures **no** release-binary evidence and enables
...
//! external-publication-audit-completion backend, a real settlement-finality projection
//! backend, a real audit-ledger acknowledgement, a real external-publication
//! external_publication_audit_archive, a real external-publication system, a real production attestation /
...
//!
//! Run 284
//! ([`crate::pqc_governance_durable_completion_external_publication_audit_completion`])
...
//!
//! Run 286 defines the **first typed interface** a future production settlement-receipt
...
//! **after** the Run 284 settlement-confirmation stage produced
//! `ExternalPublicationAuditCompletionRecorded`. It is an **interface / projection boundary only**:
//! production / MainNet / external external-publication-audit-completion implementations
...
//!
//! The external-publication-audit-completion layer is a **model only**. It does not
//! implement a real settlement, a real settlement finality, a real settlement receipt, a
//! real external-publication-audit-completion, a real settlement-finality projection, a
...
//! fixture external-publication-audit-completion sink mutates only the in-memory
...
//! 2. **legacy bypass** — a
//!    [`DurableCompletionExternalPublicationAuditArchivePolicy::Disabled`] policy preserves
//!    the legacy no-external-publication-audit-archive bypass and never invokes the
//!    external-publication-audit-completion sink;
//! 3. **settlement-confirmation-outcome projection** — only
//!    [`DurableCompletionExternalPublicationAuditCompletionOutcome::ExternalPublicationAuditCompletionRecorded`]
//!    creates a external-publication-audit-completion request; every other Run 284 outcome
...
//!    and Run 284 settlement-confirmation digest sets) must match expectations *before*
//!    the external-publication-audit-completion sink is invoked; a mismatch fails closed
//!    and leaves the sink invocation count at zero;
//! 5. **external-publication-audit-completion record** — only after every prior gate
//!    passes is the sink invoked; the record-identity fields must match exactly before
//!    any modeled external-publication-audit-completion record is recorded;
//! 6. **external-publication-audit-completion authorization** — only
//!    [`DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRecorded`]
//!    authorizes a new modeled external-publication-audit-completion / settlement-finality
//!    projection state.
//!
//! A external-publication-audit-completion record failure, rollback, rollback failure, or
//! ambiguous window never retroactively claims a durable settlement-receipt
//! acknowledgement record. A duplicate identical external-publication-audit-completion
//! record is idempotent; the same record id with a different digest fails closed as
//! equivocation and records no second record. A Run 284
//! [`DurableCompletionExternalPublicationAuditCompletionOutcome::ExternalPublicationAuditCompletionDuplicateIdempotent`]
//! never creates a new external-publication-audit-completion record by itself — it can only
...
/// Run 286 — the validation / mutation surface pair the receipt binds to.
pub type DurableCompletionExternalPublicationAuditArchiveSurface =
    ModeledGovernanceTrustMutationSurface;

/// Run 286 — the trust-domain environment binding the receipt is bound to.
pub type DurableCompletionExternalPublicationAuditArchiveEnvironment =
    ModeledGovernanceTrustMutationEnvironmentBinding;

/// Run 286 — the runtime binding (governance + mutation surface + sequence) the
/// receipt is bound to.
pub type DurableCompletionExternalPublicationAuditArchiveBinding =
    ModeledGovernanceTrustMutationRuntimeBinding;

/// Run 286 — the Run 240/246 durable replay observation carried as freshness
/// context.
pub type DurableCompletionExternalPublicationAuditArchiveReplayBinding = DurableReplayObservation;

/// Run 286 — the Run 246 pipeline outcome carried as consume authorization context.
pub type DurableCompletionExternalPublicationAuditArchivePipelineBinding =
    GovernanceModeledEndToEndPipelineOutcome;

/// Run 286 — the Run 248 sink outcome carried as receipt-record context.
pub type DurableCompletionExternalPublicationAuditArchiveSinkBinding =
    GovernanceModeledDurableConsumeSinkOutcome;

/// Run 286 — the Run 250 reporter outcome carried as completion-report context.
pub type DurableCompletionExternalPublicationAuditArchiveReporterBinding =
    GovernanceModeledDurableConsumeCompletionReporterOutcome;

/// Run 286 — the Run 252 external_publication_audit_archive outcome carried as external_publication_audit_archive context.
pub type DurableCompletionExternalPublicationAuditArchiveFinalizationBinding =
    GovernanceModeledDurableCompletionFinalizationOutcome;

/// Run 286 — the Run 254 attestation outcome carried as attestation context.
pub type DurableCompletionExternalPublicationAuditArchiveAttestationBinding =
    GovernanceModeledDurableCompletionAttestationOutcome;

/// Run 286 — the Run 256 backend outcome carried as backend-record context. The
/// acknowledgement boundary never reimplements the backend; it only carries its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAuditArchiveBackendBinding =
    DurableCompletionAttestationBackendOutcome;

/// Run 286 — the Run 258 audit/publication receipt outcome carried as
/// receipt-record context. The consumer boundary never reimplements the receipt; it
/// only carries its terminal outcome.
pub type DurableCompletionExternalPublicationAuditArchiveReceiptBinding =
    DurableCompletionAuditPublicationReceiptOutcome;

/// Run 286 — the Run 260 audit-receipt acknowledgement outcome carried as
...
pub type DurableCompletionExternalPublicationAuditArchiveAcknowledgementBinding =
    DurableCompletionAuditReceiptAcknowledgementOutcome;

/// Run 286 — the Run 262 acknowledgement consumer outcome the settlement-confirmation
...
pub type DurableCompletionExternalPublicationAuditArchiveConsumerBinding =
    DurableCompletionAcknowledgementConsumerOutcome;

/// Run 286 — the Run 284 settlement-confirmation outcome the external-publication-audit-archive
/// boundary projects to a external-publication-audit-archive request. The external-publication-audit-archive
/// boundary never reimplements the settlement confirmation; it only projects its
/// terminal outcome.
pub type DurableCompletionExternalPublicationAuditArchiveExternalPublicationAuditCompletionBinding =
...
/// Run 286 — the typed durable-completion audit/publication receipt kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditArchiveKind {
    ...
    /// fail-closed).
    ProductionExternalPublicationAuditArchiveUnavailable,
    /// MainNet audit-ledger receipt sink (reachable-but-unavailable / fail-closed).
    MainNetExternalPublicationAuditArchiveUnavailable,
    /// External-publication receipt sink (reachable-but-unavailable / fail-closed).
    ExternalExternalPublicationAuditArchiveUnavailable,
    /// An unknown receipt kind — fails closed.
    Unknown,
}

impl DurableCompletionExternalPublicationAuditArchiveKind {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::FixtureInMemory => "fixture-in-memory",
            Self::ProductionExternalPublicationAuditArchiveUnavailable => {
                "production-external-publication-audit-archive-unavailable"
            }
            Self::MainNetExternalPublicationAuditArchiveUnavailable => {
                "mainnet-external-publication-audit-archive-unavailable"
            }
            Self::ExternalExternalPublicationAuditArchiveUnavailable => {
                "external-external-publication-audit-archive-unavailable"
            }
...
/// Run 286 — the typed durable-completion audit/publication receipt policy
/// selector.
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditArchivePolicy {
    /// The receipt boundary is disabled (legacy bypass).
    Disabled,
    ...
    /// unavailable.
    ProductionExternalPublicationAuditArchiveRequired,
    /// A real MainNet audit-ledger receipt sink is required — reachable but
    /// unavailable.
    MainNetExternalPublicationAuditArchiveRequired,
    /// A real external-publication receipt sink is required — reachable but
    /// unavailable.
    ExternalExternalPublicationAuditArchiveRequired,
}

impl DurableCompletionExternalPublicationAuditArchivePolicy {
    /// Stable operator-facing tag.
    pub const fn tag(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
...
/// Run 286 — the typed receipt identity a receipt request is bound to.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditArchiveIdentity {
    /// Stable receipt id.
    pub confirmation_id: String,
    /// The receipt kind.
    pub kind: DurableCompletionExternalPublicationAuditArchiveKind,
    /// The receipt policy.
    pub policy: DurableCompletionExternalPublicationAuditArchivePolicy,
    /// The domain separation tag the receipt operates under.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAuditArchiveIdentity {
    ...
    pub fn is_well_formed(&self) -> bool {
        !self.confirmation_id.is_empty()
            ...
            && self.kind != DurableCompletionExternalPublicationAuditArchiveKind::Unknown
    }

    /// The deterministic, domain-separated receipt identity digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditArchiveDigest {
        external_publication_audit_archive_identity_digest(self)
    }
}
...
/// Run 286 — domain separator for the receipt identity digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_IDENTITY_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-archive-identity:v1";
/// Run 286 — domain separator for the receipt request digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-archive-request:v1";
/// Run 286 — domain separator for the receipt response digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-archive-response:v1";
/// Run 286 — domain separator for the receipt record digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-archive-record:v1";
/// Run 286 — domain separator for the receipt transcript digest.
const EXTERNAL_PUBLICATION_CONFIRMATION_TRANSCRIPT_DOMAIN: &[u8] =
    b"QBIND:run268:durable-completion-external-publication-audit-archive-transcript:v1";

/// Run 286 — a deterministic, domain-separated receipt digest (lowercase hex of a
/// SHA3-256 over length-prefixed, field-bound canonical material). It contains no
/// unstable display text.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditArchiveDigest(String);

impl DurableCompletionExternalPublicationAuditArchiveDigest {
...
/// Run 286 — a deterministic, domain-separated receipt transcript digest.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditArchiveTranscriptDigest(String);

impl DurableCompletionExternalPublicationAuditArchiveTranscriptDigest {
    /// The lowercase hex representation.
    pub fn as_hex(&self) -> &str {
        &self.0
    }
...
/// Run 286 — deterministic, domain-separated receipt identity digest.
pub fn external_publication_audit_archive_identity_digest(
    identity: &DurableCompletionExternalPublicationAuditArchiveIdentity,
    ...
    DurableCompletionExternalPublicationAuditArchiveDigest(w.finish())
}

/// Run 286 — deterministic, domain-separated receipt request digest. Binds every
/// request field including the full Run 256 backend digest binding.
pub fn external_publication_audit_archive_request_digest(
    request: &DurableCompletionExternalPublicationAuditArchiveRequest,
) -> DurableCompletionExternalPublicationAuditArchiveDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_REQUEST_DOMAIN);
    w.str_field(&request.external_publication_audit_archive_record_id)
        ...
        .str_field(&request.external_publication_audit_completion_identity_digest)
        .str_field(&request.external_publication_audit_completion_request_digest)
        .str_field(&request.external_publication_audit_completion_response_digest)
        .str_field(&request.external_publication_audit_completion_record_digest)
        .str_field(&request.external_publication_audit_completion_transcript_digest)
        .str_field(&request.external_publication_audit_completion_record_id)
        .str_field(&request.domain_separation_tag)
        .str_field(external_publication_audit_archive_identity_digest(&request.identity).as_hex());
    DurableCompletionExternalPublicationAuditArchiveDigest(w.finish())
}

/// Run 286 — deterministic, domain-separated receipt response digest. Binds the
/// receipt record id, the request digest it answers, the acceptance flag, and the
/// responding receipt kind.
pub fn external_publication_audit_archive_response_digest(
    response: &DurableCompletionExternalPublicationAuditArchiveResponse,
) -> DurableCompletionExternalPublicationAuditArchiveDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RESPONSE_DOMAIN);
    w.str_field(&response.external_publication_audit_archive_record_id)
        ...
        .str_field(response.external_publication_audit_archive_kind.tag());
    DurableCompletionExternalPublicationAuditArchiveDigest(w.finish())
}

/// Run 286 — deterministic, domain-separated receipt record digest. Binds the
/// receipt record id, the request digest, and the receipt identity digest.
pub fn external_publication_audit_archive_record_digest(
    record: &DurableCompletionExternalPublicationAuditArchiveRecord,
) -> DurableCompletionExternalPublicationAuditArchiveDigest {
    let mut w = CanonicalWriter::new(EXTERNAL_PUBLICATION_CONFIRMATION_RECORD_DOMAIN);
    w.str_field(&record.external_publication_audit_archive_record_id)
        .str_field(record.request_digest.as_hex())
        .str_field(record.identity_digest.as_hex());
    DurableCompletionExternalPublicationAuditArchiveDigest(w.finish())
}

/// Run 286 — deterministic, domain-separated receipt transcript digest. Binds the
/// request, response, and record digests into a single transcript binding.
pub fn external_publication_audit_archive_transcript_digest(
    request_digest: &DurableCompletionExternalPublicationAuditArchiveDigest,
    response_digest: &DurableCompletionExternalPublicationAuditArchiveDigest,
    record_digest: &DurableCompletionExternalPublicationAuditArchiveDigest,
) -> DurableCompletionExternalPublicationAuditArchiveTranscriptDigest {
    ...
    DurableCompletionExternalPublicationAuditArchiveTranscriptDigest(w.finish())
}
...
/// Run 286 — the typed receipt request a future production audit ledger or
...
/// attestation / Run 252 external_publication_audit_archive / Run 250 completion report / Run 248
/// receipt / Run 246 decision material — never a copy of any wire payload and never
/// a production durable record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveRequest {
    ...
    /// Run 252 external_publication_audit_archive decision digest.
    ...
    /// Run 284 settlement-confirmation identity digest.
    pub external_publication_audit_completion_identity_digest: String,
    /// Run 284 settlement-confirmation request digest.
    pub external_publication_audit_completion_request_digest: String,
    /// Run 284 settlement-confirmation response digest.
    pub external_publication_audit_completion_response_digest: String,
    /// Run 284 settlement-confirmation record digest.
    pub external_publication_audit_completion_record_digest: String,
    /// Run 284 settlement-confirmation transcript digest.
    pub external_publication_audit_completion_transcript_digest: String,
    /// Run 284 settlement-confirmation record id.
    pub external_publication_audit_completion_record_id: String,
    /// External-publication-confirmation identity.
    pub identity: DurableCompletionExternalPublicationAuditArchiveIdentity,
    /// Domain separation tag.
    pub domain_separation_tag: String,
}

impl DurableCompletionExternalPublicationAuditArchiveRequest {
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditArchiveDigest {
        external_publication_audit_archive_request_digest(self)
    }
...
/// Run 286 — the typed receipt response a receipt sink returns for a request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveResponse {
    /// The receipt record id the response answers.
    pub external_publication_audit_archive_record_id: String,
    /// The request digest the response answers.
    pub request_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
...
impl DurableCompletionExternalPublicationAuditArchiveResponse {
    ...
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditArchiveDigest {
        external_publication_audit_archive_response_digest(self)
    }
}

/// Run 286 — the canonical immutable receipt record derived from a request. Two
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DurableCompletionExternalPublicationAuditArchiveRecord {
    /// The receipt record id.
    pub external_publication_audit_archive_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
    /// The receipt identity digest.
    pub identity_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
}

impl DurableCompletionExternalPublicationAuditArchiveRecord {
    /// The deterministic receipt record digest.
    pub fn digest(&self) -> DurableCompletionExternalPublicationAuditArchiveDigest {
        external_publication_audit_archive_record_digest(self)
    }
}
...
/// Run 286 — the recorded status of a modeled audit/publication receipt.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditArchiveLedgerStatus {
    /// The modeled receipt is recorded in the in-memory fixture ledger.
    Recorded,
}

/// Run 286 — a single modeled receipt record held in the in-memory fixture ledger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveLedgerRecord {
    /// The receipt record id (stable identity of the receipt).
    pub external_publication_audit_archive_record_id: String,
    /// The request digest.
    pub request_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
    /// The response digest.
    pub response_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
    /// The record digest.
    pub record_digest: DurableCompletionExternalPublicationAuditArchiveDigest,
    /// The transcript digest.
    pub transcript_digest: DurableCompletionExternalPublicationAuditArchiveTranscriptDigest,
    /// The recorded status.
    pub status: DurableCompletionExternalPublicationAuditArchiveLedgerStatus,
}

/// Run 286 — an immutable snapshot of the modeled receipt ledger used to model a
/// fixture rollback.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot {
    records: Vec<DurableCompletionExternalPublicationAuditArchiveLedgerRecord>,
}

impl DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot {
...
/// Run 286 — the modeled in-memory receipt ledger.
///
...
pub struct DurableCompletionExternalPublicationAuditArchiveLedger {
    records: Vec<DurableCompletionExternalPublicationAuditArchiveLedgerRecord>,
}

impl DurableCompletionExternalPublicationAuditArchiveLedger {
    ...
    pub fn records(&self) -> &[DurableCompletionExternalPublicationAuditArchiveLedgerRecord] {
        &self.records
    }

    /// The record for `external_publication_audit_archive_record_id`, if present.
    pub fn find(
        &self,
    ...
    pub fn snapshot(&self) -> DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot {
        DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot {
    ...
    pub fn restore(
        &mut self,
        snapshot: &DurableCompletionExternalPublicationAuditArchiveLedgerSnapshot,
    ) {
    ...
    fn insert(&mut self, record: DurableCompletionExternalPublicationAuditArchiveLedgerRecord) {
...
/// Run 286 — the canonical binding a [`DurableCompletionExternalPublicationAuditArchiveInput`]
/// is checked against.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveExpectations {
    ...
    /// Expected Run 252 external_publication_audit_archive decision digest.
    ...
    /// Expected Run 284 settlement-confirmation identity digest.
    pub expected_external_publication_audit_completion_identity_digest: String,
    /// Expected Run 284 settlement-confirmation request digest.
    pub expected_external_publication_audit_completion_request_digest: String,
    /// Expected Run 284 settlement-confirmation response digest.
    pub expected_external_publication_audit_completion_response_digest: String,
    /// Expected Run 284 settlement-confirmation record digest.
    pub expected_external_publication_audit_completion_record_digest: String,
    /// Expected Run 284 settlement-confirmation transcript digest.
    pub expected_external_publication_audit_completion_transcript_digest: String,
    /// Expected Run 284 settlement-confirmation record id.
...
impl DurableCompletionExternalPublicationAuditArchiveExpectations {
    /// Internal: the first environment / surface binding mismatch reason, if any.
    fn binding_mismatch_reason(
        &self,
        input: &DurableCompletionExternalPublicationAuditArchiveInput,
    ) -> Option<&'static str> {
    ...
    fn request_mismatch_reason(
        ...
        if request.finalization_decision_digest != self.expected_finalization_decision_digest {
            return Some("wrong external_publication_audit_archive decision digest");
        }
        ...
        if request.external_publication_audit_completion_identity_digest
            != self.expected_external_publication_audit_completion_identity_digest
        {
            return Some("wrong settlement-confirmation identity digest");
        }
        if request.external_publication_audit_completion_request_digest
            != self.expected_external_publication_audit_completion_request_digest
        {
            return Some("wrong settlement-confirmation request digest");
        }
        if request.external_publication_audit_completion_response_digest
            != self.expected_external_publication_audit_completion_response_digest
        {
            return Some("wrong settlement-confirmation response digest");
        }
        if request.external_publication_audit_completion_record_digest
            != self.expected_external_publication_audit_completion_record_digest
        {
            return Some("wrong settlement-confirmation record digest");
        }
        if request.external_publication_audit_completion_transcript_digest
            != self.expected_external_publication_audit_completion_transcript_digest
        {
        ...
        if request.identity != self.expected_identity {
            return Some("wrong external-publication-audit-archive identity");
        }
        if request.identity.kind != self.expected_external_publication_audit_archive_kind {
            return Some("wrong external-publication-audit-archive kind");
        }
        if request.identity.policy != self.expected_external_publication_audit_archive_policy {
            return Some("wrong external-publication-audit-archive policy");
        }
        None
    }
...
/// Run 286 — typed inputs for one modeled durable-completion audit/publication
/// receipt round-trip.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCompletionExternalPublicationAuditArchiveInput {
    /// The receipt policy selector.
    pub policy: DurableCompletionExternalPublicationAuditArchivePolicy,
    /// The trust-domain environment binding.
    pub environment_binding: DurableCompletionExternalPublicationAuditArchiveEnvironment,
    /// The runtime binding (governance + mutation surface + sequence).
    pub runtime_binding: DurableCompletionExternalPublicationAuditArchiveBinding,
    /// The Run 240/246 durable replay observation the pipeline authorized consume
    /// under.
    pub replay_binding: DurableCompletionExternalPublicationAuditArchiveReplayBinding,
    /// The Run 246 pipeline outcome.
    pub pipeline_binding: DurableCompletionExternalPublicationAuditArchivePipelineBinding,
    /// The Run 248 sink outcome.
    pub sink_binding: DurableCompletionExternalPublicationAuditArchiveSinkBinding,
    /// The Run 250 reporter outcome.
    pub reporter_binding: DurableCompletionExternalPublicationAuditArchiveReporterBinding,
    /// The Run 252 external_publication_audit_archive outcome.
    pub confirmation_binding: DurableCompletionExternalPublicationAuditArchiveFinalizationBinding,
    /// The Run 254 attestation outcome.
    pub attestation_binding: DurableCompletionExternalPublicationAuditArchiveAttestationBinding,
    /// The Run 256 backend outcome carried as backend-record context.
    pub backend_binding: DurableCompletionExternalPublicationAuditArchiveBackendBinding,
    /// The Run 258 audit/publication receipt outcome carried as receipt-record
    /// context.
    pub receipt_binding: DurableCompletionExternalPublicationAuditArchiveReceiptBinding,
    /// The Run 260 audit-receipt acknowledgement outcome carried as
    /// acknowledgement-record context.
    pub acknowledgement_binding:
        DurableCompletionExternalPublicationAuditArchiveAcknowledgementBinding,
    ...
    pub consumer_binding: DurableCompletionExternalPublicationAuditArchiveConsumerBinding,
    /// The Run 284 settlement-confirmation outcome the external-publication-audit-archive boundary
    /// projects to a external-publication-audit-archive request.
    pub external_publication_audit_completion_binding:
        DurableCompletionExternalPublicationAuditArchiveExternalPublicationAuditCompletionBinding,
    /// The external-publication-audit-archive request the call site would submit.
    pub request: DurableCompletionExternalPublicationAuditArchiveRequest,
}

impl DurableCompletionExternalPublicationAuditArchiveInput {
    ...
    pub fn surface(&self) -> DurableCompletionExternalPublicationAuditArchiveSurface {
    ...
    pub fn is_mainnet_peer_driven(&self) -> bool {
        (self.environment() == TrustBundleEnvironment::Mainnet && self.surface().is_peer_driven())
            ...
            || matches!(
                self.external_publication_audit_completion_binding,
...
/// Run 286 — the typed outcome of one modeled durable-completion audit/publication
/// receipt round-trip.
///
/// Only [`Self::ExternalPublicationAuditArchiveRecorded`] authorizes a **new** modeled
/// audit/publication-receipt state. A [`Self::ExternalPublicationAuditArchiveDuplicateIdempotent`]
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAuditArchiveOutcome {
    /// Legacy bypass — a disabled receipt policy preserved the legacy
    /// no-audit-receipt path. No receipt invocation.
    LegacyBypassNoExternalPublicationAuditArchive,
    ...
    RejectedBeforeExternalPublicationAuditCompletionNoAuditArchive,
    ...
    /// invocation.
    ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive,
    ...
    ExternalPublicationAuditArchiveRecorded,
    /// A duplicate identical receipt — idempotent; no second receipt recorded.
    ExternalPublicationAuditArchiveDuplicateIdempotent,
    ...
    /// receipt.
    ExternalPublicationAuditArchiveRejectedBeforeRecord,
    /// The receipt record failed. No receipt.
    ExternalPublicationAuditArchiveRecordFailedNoAuditArchive,
    /// The receipt record was rolled back. No receipt.
    ExternalPublicationAuditArchiveRolledBackNoAuditArchive,
    /// The receipt rollback itself failed — fatal / fail-closed. No receipt.
    ExternalPublicationAuditArchiveRollbackFailedFatalNoAuditArchive,
    /// The after-record receipt window was ambiguous — fails closed. No receipt.
    ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive,
    /// The production audit-ledger path was reached but is unavailable. No receipt.
    ProductionExternalPublicationAuditArchiveUnavailableNoAuditArchive,
    /// The MainNet audit-ledger path was reached but is unavailable. No receipt.
    MainNetExternalPublicationAuditArchiveUnavailableNoAuditArchive,
    /// The external-publication path was reached but is unavailable. No receipt.
    ExternalExternalPublicationAuditArchiveUnavailableNoAuditArchive,
    ...
    MainNetPeerDrivenApplyRefusedNoAuditArchive,
    ...
    PolicyChangeUnsupportedNoAuditArchive,
}

impl DurableCompletionExternalPublicationAuditArchiveOutcome {
    /// `true` iff this outcome authorizes a **new** modeled audit/publication
    /// receipt (only [`Self::ExternalPublicationAuditArchiveRecorded`]).
    pub fn authorizes_record(&self) -> bool {
        matches!(self, Self::ExternalPublicationAuditArchiveRecorded)
    }
    ...
    pub fn is_legacy_bypass(&self) -> bool {
        matches!(self, Self::LegacyBypassNoExternalPublicationAuditArchive)
    }

    /// `true` iff this is the MainNet peer-driven-apply refusal.
    pub fn is_mainnet_peer_driven_apply_refused(&self) -> bool {
        matches!(self, Self::MainNetPeerDrivenApplyRefusedNoAuditArchive)
    }
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            ...
            Self::ExternalPublicationAuditArchiveRejectedBeforeRecord => {
                "external-publication-audit-archive-rejected-before-record"
            }
            Self::ExternalPublicationAuditArchiveRecordFailedNoAuditArchive => {
                "external-publication-audit-archive-record-failed-no-outcome-publication"
            }
            Self::ExternalPublicationAuditArchiveRolledBackNoAuditArchive => {
                "external-publication-audit-archive-rolled-back-no-outcome-publication"
            }
            Self::ExternalPublicationAuditArchiveRollbackFailedFatalNoAuditArchive => {
                "external-publication-audit-archive-rollback-failed-fatal-no-outcome-publication"
            }
            Self::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive => {
                "external-publication-audit-archive-ambiguous-fail-closed-no-outcome-publication"
            }
            Self::ProductionExternalPublicationAuditArchiveUnavailableNoAuditArchive => {
                "production-external-publication-audit-archive-unavailable-no-outcome-publication"
            }
            Self::MainNetExternalPublicationAuditArchiveUnavailableNoAuditArchive => {
                "mainnet-external-publication-audit-archive-unavailable-no-outcome-publication"
            }
            Self::ExternalExternalPublicationAuditArchiveUnavailableNoAuditArchive => {
                "external-external-publication-audit-archive-unavailable-no-outcome-publication"
            }
            Self::MainNetPeerDrivenApplyRefusedNoAuditArchive => {
...
/// Run 286 — the typed confirmation of a Run 262 acknowledgement consumer outcome
/// onto a settlement-confirmation request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DurableCompletionExternalPublicationAuditArchiveRequestIntent {
    ...
    NoReceipt(DurableCompletionExternalPublicationAuditArchiveOutcome),
}

impl DurableCompletionExternalPublicationAuditArchiveRequestIntent {
...
/// Run 286 — project a Run 284 settlement-confirmation outcome onto a
/// external-publication-audit-archive request.
///
/// Only
/// [`DurableCompletionExternalPublicationAuditCompletionOutcome::ExternalPublicationAuditCompletionRecorded`]
/// creates a external-publication-audit-archive request.
/// [`DurableCompletionExternalPublicationAuditCompletionOutcome::ExternalPublicationAuditCompletionDuplicateIdempotent`]
/// may only match an already-recorded external-publication-audit-archive record and never creates
...
/// [`DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive`]).
...
pub fn project_external_publication_audit_completion_outcome_to_external_publication_audit_archive_request(
    ...
    match outcome {
        ...
        Finalization::ValidatorSetRotationUnsupportedNoAuditCompletion => {
            Intent::NoReceipt(Receipt::ValidatorSetRotationUnsupportedNoAuditArchive)
        }
        Finalization::PolicyChangeUnsupportedNoAuditCompletion => {
            Intent::NoReceipt(Receipt::PolicyChangeUnsupportedNoAuditArchive)
        }
...
/// Run 286 — a modeled fault the fixture receipt sink injects to exercise the
/// fail-closed record / rollback / ambiguous paths. Source/test only.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditArchiveFault {
...
/// Run 286 — the pure/mockable modeled durable-completion audit/publication receipt
/// boundary.
///
...
/// [`DurableCompletionExternalPublicationAuditArchiveLedger`].
pub trait GovernanceDurableCompletionExternalPublicationAuditArchiveSink {
    /// The receipt kind (used for typed recovery classification).
    fn kind(&self) -> DurableCompletionExternalPublicationAuditArchiveKind;
    ...
    fn project_durable_completion_external_publication_audit_archive(
        &mut self,
        request: &DurableCompletionExternalPublicationAuditArchiveRequest,
        expectations: &DurableCompletionExternalPublicationAuditArchiveExpectations,
        idempotent_only: bool,
        ledger: &mut DurableCompletionExternalPublicationAuditArchiveLedger,
    ...
    fn recover_durable_completion_external_publication_audit_archive_window(
        &self,
        input: &DurableCompletionExternalPublicationAuditArchiveInput,
        window: DurableCompletionExternalPublicationAuditArchiveWindow,
        recovered_record: Option<&DurableCompletionExternalPublicationAuditArchiveLedgerRecord>,
        expectations: &DurableCompletionExternalPublicationAuditArchiveExpectations,
    ) -> DurableCompletionExternalPublicationAuditArchiveOutcome {
...
/// Run 286 — the DevNet/TestNet in-memory fixture receipt sink.
///
/// Source-test only. It mutates only the in-memory
/// [`DurableCompletionExternalPublicationAuditArchiveLedger`] and exposes an invocation
/// counter so tests can prove non-submitting backend paths and pre-receipt
/// rejections never invoke it.
#[derive(Debug, Clone)]
pub struct FixtureDurableCompletionExternalPublicationAuditArchiveSink {
    fault: Option<DurableCompletionExternalPublicationAuditArchiveFault>,
    invocations: u32,
}

impl Default for FixtureDurableCompletionExternalPublicationAuditArchiveSink {
    fn default() -> Self {
        Self::new()
    }
}

impl FixtureDurableCompletionExternalPublicationAuditArchiveSink {
    ...
    pub fn with_fault(fault: DurableCompletionExternalPublicationAuditArchiveFault) -> Self {
        Self {
            fault: Some(fault),
            invocations: 0,
        }
...
impl GovernanceDurableCompletionExternalPublicationAuditArchiveSink
    for FixtureDurableCompletionExternalPublicationAuditArchiveSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditArchiveKind {
    ...
    fn project_durable_completion_external_publication_audit_archive(
        ...
        if let Some(fault) = self.fault {
            ...
            return match fault {
                DurableCompletionExternalPublicationAuditArchiveFault::RecordFailedNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditArchiveRecordFailedNoAuditArchive
                }
                DurableCompletionExternalPublicationAuditArchiveFault::RolledBackNoReceipt => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditArchiveRolledBackNoAuditArchive
                }
                DurableCompletionExternalPublicationAuditArchiveFault::RollbackFailedFatal => {
                    Receipt::ExternalPublicationAuditArchiveRollbackFailedFatalNoAuditArchive
                }
                DurableCompletionExternalPublicationAuditArchiveFault::AmbiguousAfterRecord => {
                    ledger.restore(&snapshot);
                    Receipt::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive
                }
            };
        }
        ...
        let transcript_digest = external_publication_audit_archive_transcript_digest(
            &request_digest,
            &response_digest,
            &record_digest,
        );
        ...
        if let Some(existing) = ledger.find(&request.external_publication_audit_archive_record_id) {
            ...
            {
                return Receipt::ExternalPublicationAuditArchiveDuplicateIdempotent;
            }
...
/// Run 286 — the production audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ProductionExternalPublicationAuditArchiveSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditArchiveSink
    for ProductionExternalPublicationAuditArchiveSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditArchiveKind {
        DurableCompletionExternalPublicationAuditArchiveKind::ProductionExternalPublicationAuditArchiveUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_archive(
        &mut self,
        _request: &DurableCompletionExternalPublicationAuditArchiveRequest,
        _expectations: &DurableCompletionExternalPublicationAuditArchiveExpectations,
        _idempotent_only: bool,
        _ledger: &mut DurableCompletionExternalPublicationAuditArchiveLedger,
    ) -> DurableCompletionExternalPublicationAuditArchiveOutcome {
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditArchiveOutcome::ProductionExternalPublicationAuditArchiveUnavailableNoAuditArchive
    }
}

/// Run 286 — the MainNet audit-ledger receipt sink. Reachable but unavailable /
/// fail-closed. It records no receipt and mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct MainNetExternalPublicationAuditArchiveSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditArchiveSink
    for MainNetExternalPublicationAuditArchiveSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditArchiveKind {
        DurableCompletionExternalPublicationAuditArchiveKind::MainNetExternalPublicationAuditArchiveUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_archive(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditArchiveOutcome::MainNetExternalPublicationAuditArchiveUnavailableNoAuditArchive
    }
}

/// Run 286 — the external-publication receipt sink. Reachable but unavailable /
/// fail-closed. It performs no external publication, records no receipt, and
/// mutates no ledger.
#[derive(Debug, Clone, Default)]
pub struct ExternalExternalPublicationAuditArchiveSink {
    invocations: u32,
}

impl GovernanceDurableCompletionExternalPublicationAuditArchiveSink
    for ExternalExternalPublicationAuditArchiveSink
{
    fn kind(&self) -> DurableCompletionExternalPublicationAuditArchiveKind {
        DurableCompletionExternalPublicationAuditArchiveKind::ExternalExternalPublicationAuditArchiveUnavailable
    }
    ...
    fn project_durable_completion_external_publication_audit_archive(
        ...
        self.invocations += 1;
        DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalExternalPublicationAuditArchiveUnavailableNoAuditArchive
    }
}
...
/// Run 286 — evaluate one modeled durable-completion audit/publication receipt
/// round-trip.
///
...
/// 2. legacy bypass — a
///    [`DurableCompletionExternalPublicationAuditArchivePolicy::Disabled`] policy;
...
pub fn evaluate_durable_completion_external_publication_audit_archive<S>(
    input: &DurableCompletionExternalPublicationAuditArchiveInput,
    expectations: &DurableCompletionExternalPublicationAuditArchiveExpectations,
    sink: &mut S,
    ledger: &mut DurableCompletionExternalPublicationAuditArchiveLedger,
...
where
    S: GovernanceDurableCompletionExternalPublicationAuditArchiveSink,
{
    ...
    if input.is_mainnet_peer_driven() {
        return Finalization::MainNetPeerDrivenApplyRefusedNoAuditArchive;
    }
    ...
    if input.policy.is_disabled() {
        return Finalization::LegacyBypassNoExternalPublicationAuditArchive;
    }
    ...
    let idempotent_only =
        match project_external_publication_audit_completion_outcome_to_external_publication_audit_archive_request(
    ...
    if !expectations.binding_matches(input) {
        return Finalization::RejectedBeforeExternalPublicationAuditCompletionNoAuditArchive;
    }

    // Step 5: invoke the settlement-confirmation sink to record the modeled confirmation.
    sink.project_durable_completion_external_publication_audit_archive(
        &input.request,
        expectations,
        idempotent_only,
        ledger,
...
/// Run 286 — the modeled durable-completion audit/publication receipt
/// crash/recovery window.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DurableCompletionExternalPublicationAuditArchiveWindow {
    ...
    /// Crashed after the report record but before a external_publication_audit_archive intent.
    AfterPublicationRecordBeforeReceiptIntent,
    /// Crashed after a external_publication_audit_archive intent but before any external_publication_audit_archive record.
    AfterReceiptIntentBeforeReceiptRecord,
    /// Crashed after the external_publication_audit_archive record but before an attestation intent.
    ...
    AfterConsumerSuccessBeforeExternalPublicationAuditCompletionRequest,
    ...
    /// Crashed after settlement-confirmation success but before a external-publication-audit-archive
    /// request.
    AfterExternalPublicationAuditCompletionSuccessBeforeExternalPublicationAuditArchiveRequest,
    /// Crashed after a external-publication-audit-archive request but before any
    /// external-publication-audit-archive record.
    AfterExternalPublicationAuditArchiveRequestBeforeExternalPublicationAuditArchiveRecord,
    /// Crashed after a external-publication-audit-archive record but before external-publication-audit-archive
    /// success — fails closed unless an explicit matching external-publication-audit-archive
    /// record exists.
    AfterExternalPublicationAuditArchiveRecordBeforeExternalPublicationAuditArchiveSuccess,
    /// Recovered after a successful external-publication-audit-archive record.
    AfterExternalPublicationAuditArchiveSuccess,
    /// Recovered after an ambiguous external-publication-audit-archive window.
    AfterExternalPublicationAuditArchiveAmbiguous,
    /// The external-publication-audit-archive record itself failed.
    ExternalPublicationAuditArchiveRecordFailed,
    /// The external-publication-audit-archive record was rolled back.
    ExternalPublicationAuditArchiveRollbackCompleted,
    /// The external-publication-audit-archive rollback itself failed — fatal.
    ExternalPublicationAuditArchiveRollbackFailed,
    /// An unknown window — fails closed.
    Unknown,
}

/// Run 286 — classify a modeled durable-completion audit/publication receipt
/// crash/recovery window.
///
...
/// [`DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRecorded`]. Pure:
/// performs no modeled mutation and never invokes Run 070.
pub fn recover_durable_completion_external_publication_audit_archive_window(
    input: &DurableCompletionExternalPublicationAuditArchiveInput,
    window: DurableCompletionExternalPublicationAuditArchiveWindow,
    kind: DurableCompletionExternalPublicationAuditArchiveKind,
    recovered_record: Option<&DurableCompletionExternalPublicationAuditArchiveLedgerRecord>,
    expectations: &DurableCompletionExternalPublicationAuditArchiveExpectations,
) -> DurableCompletionExternalPublicationAuditArchiveOutcome {
    use DurableCompletionExternalPublicationAuditArchiveOutcome as Receipt;
    use DurableCompletionExternalPublicationAuditArchiveWindow as Window;

    // MainNet peer-driven apply refusal precedes recovery classification.
    if input.is_mainnet_peer_driven() {
        return Receipt::MainNetPeerDrivenApplyRefusedNoAuditArchive;
    }
    ...
    match kind {
        DurableCompletionExternalPublicationAuditArchiveKind::ProductionExternalPublicationAuditArchiveUnavailable => {
            return Receipt::ProductionExternalPublicationAuditArchiveUnavailableNoAuditArchive;
        }
        DurableCompletionExternalPublicationAuditArchiveKind::MainNetExternalPublicationAuditArchiveUnavailable => {
            return Receipt::MainNetExternalPublicationAuditArchiveUnavailableNoAuditArchive;
        }
        DurableCompletionExternalPublicationAuditArchiveKind::ExternalExternalPublicationAuditArchiveUnavailable => {
            return Receipt::ExternalExternalPublicationAuditArchiveUnavailableNoAuditArchive;
        }
        DurableCompletionExternalPublicationAuditArchiveKind::Disabled => {
            return Receipt::LegacyBypassNoExternalPublicationAuditArchive;
        }
        DurableCompletionExternalPublicationAuditArchiveKind::Unknown => {
            return Receipt::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive;
        }
        DurableCompletionExternalPublicationAuditArchiveKind::FixtureInMemory => {}
    }
    ...
    match window {
        // Through settlement-confirmation success but before a external-publication-audit-archive
        // request there is nothing to record a settlement external_publication_audit_archive for.
        ...
        | Window::AfterConsumerSuccessBeforeExternalPublicationAuditCompletionRequest
        ...
        | Window::AfterExternalPublicationAuditCompletionSuccessBeforeExternalPublicationAuditArchiveRequest => {
            Receipt::ExternalPublicationAuditCompletionDidNotRecordNoAuditArchive
        }
        // A external-publication-audit-archive request without a record never records a
        // settlement external_publication_audit_archive.
        Window::AfterExternalPublicationAuditArchiveRequestBeforeExternalPublicationAuditArchiveRecord => {
            Receipt::ExternalPublicationAuditArchiveRejectedBeforeRecord
        }
        // After a external-publication-audit-archive record but before success: fails closed
        // unless an explicit matching, well-formed external-publication-audit-archive record
        // exists.
        Window::AfterExternalPublicationAuditArchiveRecordBeforeExternalPublicationAuditArchiveSuccess => {
            match recovered_record {
                Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAuditArchiveRecorded,
                _ => Receipt::ExternalPublicationAuditArchiveRejectedBeforeRecord,
            }
        }
        // An explicit successful settlement external_publication_audit_archive recovers as recorded only if
        // it matches.
        Window::AfterExternalPublicationAuditArchiveSuccess => match recovered_record {
            Some(record) if recovered_matches(record) => Receipt::ExternalPublicationAuditArchiveRecorded,
            _ => Receipt::ExternalPublicationAuditArchiveRejectedBeforeRecord,
        },
        Window::AfterExternalPublicationAuditArchiveAmbiguous => {
            Receipt::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive
        }
        Window::ExternalPublicationAuditArchiveRecordFailed => {
            Receipt::ExternalPublicationAuditArchiveRecordFailedNoAuditArchive
        }
        Window::ExternalPublicationAuditArchiveRollbackCompleted => {
            Receipt::ExternalPublicationAuditArchiveRolledBackNoAuditArchive
        }
        Window::ExternalPublicationAuditArchiveRollbackFailed => {
            Receipt::ExternalPublicationAuditArchiveRollbackFailedFatalNoAuditArchive
        }
        // Any unknown window fails closed.
        Window::Unknown => Receipt::ExternalPublicationAuditArchiveAmbiguousFailClosedNoAuditArchive,
    }
}

/// Run 286 — `true` iff a receipt outcome authorizes a **new** modeled
/// audit/publication receipt (only
/// [`DurableCompletionExternalPublicationAuditArchiveOutcome::ExternalPublicationAuditArchiveRecorded`]).
pub fn external_publication_audit_archive_outcome_authorizes_record(
    outcome: &DurableCompletionExternalPublicationAuditArchiveOutcome,
) -> bool {
    outcome.authorizes_record()
}

/// Run 286 — `true` iff a receipt outcome projects to a recorded audit/publication
/// receipt (a newly recorded receipt or an idempotent duplicate of an
/// already-recorded receipt).
pub fn external_publication_audit_archive_outcome_projects_to_recorded(
    outcome: &DurableCompletionExternalPublicationAuditArchiveOutcome,
) -> bool {
...
/// Run 286 — a receipt rejection is non-mutating: it records no receipt, mutates no
/// `LivePqcTrustState`, and writes no durable state.
pub fn durable_completion_external_publication_audit_archive_rejection_is_non_mutating() -> bool {
    true
}

/// Run 286 — the receipt boundary never calls Run 070.
pub fn durable_completion_external_publication_audit_archive_never_calls_run_070() -> bool {
    true
}
...
/// Run 286 — the receipt boundary performs no external publication.
pub fn durable_completion_external_publication_audit_archive_no_external_publication() -> bool {
    true
}

/// Run 286 — the receipt boundary performs no real audit-ledger persistence.
pub fn durable_completion_external_publication_audit_archive_no_real_audit_ledger() -> bool {
    true
}

/// Run 286 — a receipt requires a successful Run 246 pipeline outcome upstream.
pub fn durable_completion_external_publication_audit_archive_pipeline_success_required() -> bool {
    true
}

/// Run 286 — a receipt requires a Run 248 sink receipt record upstream.
pub fn durable_completion_external_publication_audit_archive_sink_receipt_required() -> bool {
    true
}

/// Run 286 — a receipt requires a Run 250 completion report upstream.
pub fn durable_completion_external_publication_audit_archive_completion_report_required() -> bool {
    true
}
...
/// Run 286 — a receipt requires a Run 254 attestation upstream.
pub fn durable_completion_external_publication_audit_archive_attestation_required() -> bool {
    true
}

/// Run 286 — an acknowledgement requires a Run 256 backend submission upstream.
pub fn durable_completion_external_publication_audit_archive_backend_submission_required() -> bool {
    true
}

/// Run 286 — an acknowledgement requires a Run 258 audit receipt upstream.
pub fn durable_completion_external_publication_audit_archive_receipt_required() -> bool {
    true
}

/// Run 286 — a consumer requires a Run 260 audit-receipt acknowledgement upstream.
pub fn durable_completion_external_publication_audit_archive_consumer_required() -> bool {
    true
}

/// Run 286 — a settlement external_publication_audit_archive requires a Run 284 settlement confirmation
/// upstream; no settlement external_publication_audit_archive is authorized without a recorded
/// settlement confirmation.
pub fn durable_completion_external_publication_audit_archive_confirmation_required() -> bool {
    true
}

/// Run 286 — the consumer boundary never performs a real settlement; production /
/// MainNet / external settlement consumers are reachable but fail closed.
pub fn durable_completion_external_publication_audit_archive_no_real_settlement() -> bool {
    true
}

/// Run 286 — the external-publication-audit-archive boundary never confers real settlement
/// finality; the only external-publication-audit-archive record is a modeled in-memory fixture
/// record. Production / MainNet / external external-publication-audit-archive sinks are reachable
/// but unavailable / fail closed and never confer any real finality.
pub fn durable_completion_external_publication_audit_archive_no_real_settlement_finality() -> bool {
    true
}

/// Run 286 — the external-publication-audit-archive boundary never emits a real settlement
/// receipt; the only external-publication-audit-archive record is a modeled in-memory fixture
/// record with no external publication, network I/O, or persistent backend.
pub fn durable_completion_external_publication_audit_archive_no_real_settlement_receipt() -> bool {
    true
}

/// Run 286 — the external-publication-audit-archive boundary never confers a real
/// external-publication-audit-completion; the only external-publication-audit-completion
...
/// Run 286 — a failed receipt record never records a receipt.
pub fn durable_completion_external_publication_audit_archive_failed_record_never_records() -> bool {
    true
}

/// Run 286 — a rolled-back receipt record never records a receipt.
pub fn durable_completion_external_publication_audit_archive_rollback_never_records() -> bool {
    true
}
...
/// Run 286 — MainNet peer-driven apply refusal precedes every receipt stage.
pub fn durable_completion_external_publication_audit_archive_mainnet_peer_driven_apply_refused_first(
...
/// Run 286 — the external-publication sink is reachable but unavailable.
pub fn durable_completion_external_publication_audit_archive_external_unavailable() -> bool {
    true
}
...
/// Run 286 — policy-change actions remain unsupported at the receipt boundary.
pub fn durable_completion_external_publication_audit_archive_policy_change_unsupported() -> bool {
    true
}

/// Run 286 — a local operator cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_audit_archive_local_operator_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}

/// Run 286 — a peer majority cannot satisfy MainNet authority for a receipt.
pub fn durable_completion_external_publication_audit_archive_peer_majority_cannot_satisfy_mainnet_authority(
) -> bool {
    true
}