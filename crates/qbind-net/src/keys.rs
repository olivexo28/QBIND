...
/// session_id = HKDF-Expand-Label(PRK, "QBIND:session-id", info, 3)
...
impl SessionKeys {
    ...
    /// HKDF layout:
    ///  - prk = HKDF-Extract(salt = "QBIND:KDF" || transcript_hash, ikm = shared_secret)
    ///  - session_id = first 3 bytes of HKDF-Expand-Label(prk, "QBIND:session-id", info, 3)
    ///  - k_c2s = HKDF-Expand-Label(prk, "QBIND:k_c2s", info, key_len)
    ///  - k_s2c = HKDF-Expand-Label(prk, "QBIND:k_s2c", info, key_len)
    ///
    ...
    pub fn derive(
        ...
        let mut salt = b"QBIND:KDF".to_vec();
        ...
        let sid_bytes = hkdf_expand_label(&prk, b"QBIND:session-id", &info, 3);
        ...
        let k_c2s_raw = hkdf_expand_label(&prk, b"QBIND:k_c2s", &info, key_len);
        let k_s2c_raw = hkdf_expand_label(&prk, b"QBIND:k_s2c", &info, key_len);
