#!/usr/bin/env bash
# Run 346 — release-binary evidence for the Run 343 live epoch-transition
...
# live epoch-transition authority-lifecycle activation-readiness / post-completion attestation accept
# decision — is_accept() with Some(post_completion_attestation_artifact) —
# composing the real
...
# chain; full post-completion-attestation / settlement-execution / runtime-handoff /
...
# guarded-mutation / runtime-handoff / execution-preparation / post-completion-attestation
# nonce binding + post-completion-attestation-decision-integrity /
# current-validator-set epoch-version preflight / disabled /
# missing-post-completion-attestation-decision / unverified-post-completion-attestation-decision /
# accepted-without-artifact / post-completion-attestation-decision-alone / runtime-handoff-decision-alone /
...
# authority-activation-authorization replay-recovery-idempotency / stale governance-epoch /
...
# verified live epoch-transition authority-lifecycle activation-readiness / post-completion attestation
...
# `ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionAuthorityActivationAuthorizationError`
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_346_production_live_epoch_transition_authority_activation_authorization_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_346_production_live_epoch_transition_authority_activation_authorization_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_346"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_authorization.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_346_production_live_epoch_transition_authority_activation_authorization_release_binary_helper.rs"

log() { printf '[run-346] %s\n' "$*" >&2; }
fail() { printf '[run-346] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_346_production_live_epoch_transition_authority_activation_authorization_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_346_production_live_epoch_transition_authority_activation_authorization_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_346.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_346_path:    ${HELPER_BIN}"
  echo "helper_346_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_346_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 346 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_346.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_346.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_346 helper failed"
...
log "running Run 346 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_346_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_346_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_346_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_346 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_346_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_346_deterministic_digests.txt" >/dev/null; then
...
log "S1 help hides Run 343 live epoch-transition authority-activation-authorization surface (no new CLI flag)"
...
# No Run 346/341 live epoch-transition authority-activation-authorization boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-authority-activation-authorization|ProductionLiveEpochTransitionAuthorityActivationAuthorization|pqc_production_live_epoch_transition_authority_activation_authorization|authority-activation-authorization|authority-lifecycle-completion|live-epoch-transition-settlement-execution|ProductionLiveEpochTransitionSettlementExecution|pqc_production_live_epoch_transition_settlement_execution|settlement-execution|live-settlement-execution|live-epoch-transition-external-publication|ProductionLiveEpochTransitionExternalPublication|pqc_production_live_epoch_transition_external_publication|external-publication|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-346|run_346|run-341|run_341'
log "S2..S4 default surfaces silent on live epoch-transition authority-activation-authorization boundary claims"
...
log "S5 no live epoch-transition authority-activation-authorization CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-authority-activation-authorization-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented authority-activation-authorization selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on authority-activation-authorization claims"
...
RUN341_SYMS=(
  ...
  
  ...
  LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource
  LiveEpochTransitionAuthorityActivationAuthorizationReplaySet
  ...
  production_live_epoch_transition_authority_activation_authorization_content_digest
...
cat "${MOD}" "${FSC_MOD}" "${FINAL_SETTLEMENT_MOD}" "${SETTLE_EXEC_MOD}" "${SETTLE_EXEC_PREP_MOD}" "${SETTLE_PREP_MOD}" "${PCA_MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 346 combined reachability — Run 341 live epoch-transition authority-activation-authorization boundary symbols across source module + Run 339/340 settlement-execution input module + ancestor chain modules + release helper:"
...
{
  echo "Run 346 source reachability — Run 341 live epoch-transition authority-activation-authorization boundary symbols in ${MOD}:"
...
for sym in ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome ProductionLiveEpochTransitionAuthorityActivationAuthorizationProtocolVersion LiveEpochTransitionAuthorityActivationAuthorizationKind LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource LiveEpochTransitionAuthorityActivationAuthorizationReplaySet EmptyLiveEpochTransitionAuthorityActivationAuthorizationReplaySet LiveEpochTransitionAuthorityActivationAuthorizationFixtureState evaluate_live_epoch_transition_authority_activation_authorization recover_live_epoch_transition_authority_activation_authorization_window production_live_epoch_transition_authority_activation_authorization_content_digest production_live_epoch_transition_authority_activation_authorization_request_id production_live_epoch_transition_authority_activation_authorization_id production_live_epoch_transition_authority_activation_authorization_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 346 helper reachability — Run 341 symbols exercised by the release helper (plus the Run 339/340 settlement-execution + Run 337/338 settlement-execution-preparation + Run 335/336 settlement-preparation + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
...
for sym in ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor ProductionLiveEpochTransitionAuthorityActivationAuthorizationConfig ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy ProductionLiveEpochTransitionAuthorityActivationAuthorizationInputs ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome LiveEpochTransitionAuthorityActivationAuthorizationKind LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource EmptyLiveEpochTransitionAuthorityActivationAuthorizationReplaySet LiveEpochTransitionAuthorityActivationAuthorizationFixtureState evaluate_live_epoch_transition_authority_activation_authorization recover_live_epoch_transition_authority_activation_authorization_window ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionDecision ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_authority_activation_authorization' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_authority_activation_authorization\|fn recover_live_epoch_transition_authority_activation_authorization_window\|fn production_live_epoch_transition_authority_activation_authorization_content_digest\|fn production_live_epoch_transition_authority_activation_authorization_request_id\|fn production_live_epoch_transition_authority_activation_authorization_id\|fn production_live_epoch_transition_authority_activation_authorization_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationOutcome\|enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationRecoveryOutcome\|enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy\|enum ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorKind\|enum LiveEpochTransitionAuthorityActivationAuthorizationKind\|enum LiveEpochTransitionAuthorityActivationAuthorizationAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionAuthorityActivationAuthorizationReplaySet\|struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor\|struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationArtifact\|struct ProductionLiveEpochTransitionAuthorityActivationAuthorizationRequest\|struct EmptyLiveEpochTransitionAuthorityActivationAuthorizationReplaySet\|struct LiveEpochTransitionAuthorityActivationAuthorizationFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"


C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 346'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 346 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 346 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-authorization-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-authorization row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 346 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-authorization row must be scoped Green"
grep -F -q 'consumes verified live epoch-transition authority-lifecycle activation-readiness / post-completion attestation decisions and produces typed non-mutating live authority-activation authorization / final-execution readiness artifacts' "${C4C5_DOC}" || fail "authority-activation-authorization row must record post-completion-attestation consumption and typed non-mutating live authority-activation authorization / final-execution readiness artifacts"
# The live epoch-transition post-completion-attestation and authority-activation-authorization rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "authority-activation-authorization row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "authority-activation-authorization row must record no public CLI flag"
...
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "authority-activation-authorization row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "authority-activation-authorization row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live-authority-activation-authorization active' 'live-authority-activation-authorization enabled' 'authority-activation-authorization applied' 'authority-lifecycle-completion applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition authority-activation-authorization enabled' 'peer-driven apply enabled'
...
{
  echo "Run 346 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 346 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 341 ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor ONLY"
  ...
  echo "  live authority-lifecycle activation-readiness / post-completion attestation artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionAuthorityActivationAuthorizationFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutorPolicy is Disabled; the production binary is not wired to"
...

{
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_346_deterministic_digests.txt"
  echo
  ...
  echo
  echo "verdict: PASS (release-binary evidence only; live epoch-transition authority-activation-authorization Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN)"
} | tee "${SUMMARY}"