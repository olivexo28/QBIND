#!/usr/bin/env bash
# Run 354 — release-binary evidence for the Run 353 live epoch-transition
# authority-activation execution-sink prewrite / sink-commit-readiness boundary.
#
# Release-binary evidence for the Run 353 source/test live epoch-transition
# authority-activation execution-sink prewrite / sink-commit-readiness boundary
...
# Run 353 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 349/350 verified live
# epoch-transition authority-activation post-final-execution-confirmation
# accept decision — is_accept() with Some(authority_activation_post_final_execution_confirmation_artifact) —
# composing the real Run 303/304 → Run 305/306 → Run 307/308 → Run 309/310 →
# Run 311/312 → Run 313/314 → Run 315/316 → Run 317/318 → Run 319/320 →
# Run 321/322 → Run 323/324 → Run 325/326 → Run 327/328 → Run 329/330 →
# Run 331/332 → Run 333/334 → Run 335/336 → Run 337/338 → Run 339/340 →
# Run 341/342 → Run 343/344 → Run 345/346 → Run 347/348 → Run 349/350 accept chain; full
# authority-activation-authorization / authority-activation-execution-sink-prewrite
# transcript + epoch-transition target + application / live-application /
# staged-application / guarded-mutation / runtime-handoff / execution-preparation /
# mutation-execution / commit-authorization / authority-activation-authorization
# nonce binding + decision-integrity / current-validator-set epoch-version
# preflight / disabled / missing-authority-activation-authorization-decision /
# unverified-authority-activation-authorization-decision / accepted-without-artifact /
# authority-activation-authorization-decision-alone / commit-authorization-decision-alone /
...
# fixture-only / local-operator / peer-majority / custody-only / remote-signer-only /
# custody-attestation-only / arbitrary-bytes rejected / wrong-field rejections /
# authority-activation-execution-sink-prewrite replay-recovery-idempotency /
# stale governance-epoch / stale authority-sequence / stale validator-set
# epoch-version / production-policy-unavailable / MainNet refused / non-mutating).
# The executor consumes a verified live epoch-transition authority-activation
# post-final-execution-confirmation accept decision and produces only a
# typed non-mutating live authority-activation execution-sink prewrite /
# sink-commit-readiness artifact for a future live production authority-activation
# execution executor. The release helper remains dead code from the production
...
#
# Substitution note: the Run 353 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome`
# fail-closed variant; there is no separate
# `ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteError` enum,
# so that symbol is intentionally not required by the reachability greps below.
set -euo pipefail


REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_354_production_live_epoch_transition_authority_activation_execution_sink_prewrite_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_354_production_live_epoch_transition_authority_activation_execution_sink_prewrite_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_354"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_execution_sink_prewrite.rs"
AAEP_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_post_final_execution_confirmation.rs"
AAA_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_authorization.rs"
FSC_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_lifecycle_post_completion_attestation.rs"
FINAL_SETTLEMENT_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_final_settlement_completion.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_354_production_live_epoch_transition_authority_activation_execution_sink_prewrite_release_binary_helper.rs"

log() { printf '[run-354] %s\n' "$*" >&2; }
fail() { printf '[run-354] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_354_production_live_epoch_transition_authority_activation_execution_sink_prewrite_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_354_production_live_epoch_transition_authority_activation_execution_sink_prewrite_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_354.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_354_path:    ${HELPER_BIN}"
  echo "helper_354_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_354_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 354 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_354.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_354.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_354 helper failed"
...
log "running Run 354 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_354_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_354_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_354_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_354 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_354_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_354_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 353 live epoch-transition
# authority-activation execution-sink prewrite / sink-commit-readiness boundary
# has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition authority-activation-execution-sink-prewrite active|live epoch-transition authority-activation-execution-sink-prewrite enabled|authority-activation-execution-sink-prewrite executor active|authority-activation-execution-sink-prewrite executor enabled|post-final-execution-confirmation preflight active|post-final-execution-confirmation preflight enabled|ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor enabled|MainNet live epoch-transition authority-activation-execution-sink-prewrite enabled|authority-activation-execution-sink-prewrite applied|authority-activation applied|post-final-execution-confirmation applied|live epoch-transition authority-activation-authorization active|live epoch-transition authority-activation-authorization enabled|authority-activation-authorization executor active|authority-activation-authorization executor enabled|ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor enabled|authority-activation-authorization applied|authority-lifecycle-completion applied|live mutation applied'
  ...
  assert_not_grep "$logf" 'fallback to fixture proof|fallback to local operator proof|fallback to peer majority|fallback to on-chain proof|fallback to RemoteSigner|fallback to custody attestation|fallback to governance proof|fallback to governance execution intent|fallback to rotation plan|fallback to application decision|fallback to authorization decision|fallback to staged application decision|fallback to guarded mutation decision|fallback to runtime handoff decision|fallback to authority-activation-authorization decision|raw local production key|DummySig active|DummyKem active|DummyAead active'
...
log "S1 help hides Run 353 live epoch-transition authority-activation-execution-sink-prewrite surface (no new CLI flag)"
...
# No Run 354/347 live epoch-transition authority-activation-execution-sink-prewrite boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-authority-activation-execution-sink-prewrite|ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewrite|pqc_production_live_epoch_transition_authority_activation_execution_sink_prewrite|authority-activation-execution-sink-prewrite|post-final-execution-confirmation-preflight|post-final-execution-confirmation-readiness|authority-activation|post-final-execution-confirmation|live-epoch-transition-authority-activation-authorization|ProductionLiveEpochTransitionAuthorityActivationAuthorization|pqc_production_live_epoch_transition_authority_activation_authorization|authority-activation-authorization|post-completion-attestation|activation-readiness|authority-lifecycle-completion|settlement-execution|external-publication|receipt-write|audit-write|run-354|run_354|run-353|run_351'
log "S2..S4 default surfaces silent on live epoch-transition authority-activation-execution-sink-prewrite boundary claims"
...
log "S5 no live epoch-transition authority-activation-execution-sink-prewrite CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-authority-activation-execution-sink-prewrite-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented authority-activation-execution-sink-prewrite selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on authority-activation-execution-sink-prewrite claims"
...
# Run 353 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN353_SYMS=(
  ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor
  ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig
  ...
  LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind
  ...
  production_live_epoch_transition_authority_activation_execution_sink_prewrite_content_digest
  production_live_epoch_transition_authority_activation_execution_sink_prewrite_request_id
...
cat "${MOD}" "${AAEP_MOD}" "${AAA_MOD}" "${FSC_MOD}" "${FINAL_SETTLEMENT_MOD}" "${SETTLE_EXEC_MOD}" "${SETTLE_EXEC_PREP_MOD}" "${SETTLE_PREP_MOD}" "${PCA_MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 354 combined reachability — Run 353 live epoch-transition authority-activation-execution-sink-prewrite boundary symbols across source module + Run 347/348 authority-activation-authorization input module + ancestor chain modules + release helper:"
  for sym in "${RUN353_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN353_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 354 source reachability — Run 353 live epoch-transition authority-activation-execution-sink-prewrite boundary symbols in ${MOD}:"
  for sym in "${RUN353_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in "${RUN353_SYMS[@]}"; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 354 helper reachability — Run 353 symbols exercised by the release helper (plus the Run 347/348 authority-activation-authorization + Run 343/344 post-completion-attestation + Run 341/342 final-settlement-completion + Run 339/340 settlement-execution + Run 337/338 settlement-execution-preparation + Run 335/336 settlement-preparation + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN353_SYMS[@]}" pqc_production_live_epoch_transition_authority_activation_post_final_execution_confirmation ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision pqc_production_live_epoch_transition_authority_activation_authorization ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision pqc_production_live_epoch_transition_settlement_execution ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionDecision pqc_production_live_epoch_transition_settlement_execution_preparation ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision pqc_production_live_epoch_transition_settlement_preparation ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision pqc_production_live_epoch_transition_external_publication ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision pqc_production_live_epoch_transition_commit_receipt ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteConfig ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteInputs ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteDecision ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource EmptyLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteFixtureState evaluate_live_epoch_transition_authority_activation_execution_sink_prewrite recover_live_epoch_transition_authority_activation_execution_sink_prewrite_window ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor ProductionLiveEpochTransitionAuthorityActivationAuthorizationDecision ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionDecision ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_authority_activation_execution_sink_prewrite' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_authority_activation_execution_sink_prewrite\|fn recover_live_epoch_transition_authority_activation_execution_sink_prewrite_window\|fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_content_digest\|fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_request_id\|fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_id\|fn production_live_epoch_transition_authority_activation_execution_sink_prewrite_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteOutcome\|enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRecoveryOutcome\|enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy\|enum ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorKind\|enum LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteKind\|enum LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet\|struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor\|struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteArtifact\|struct ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteRequest\|struct EmptyLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteReplaySet\|struct LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"


C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 354'
  'boundary readiness'
  ...
  'Live epoch-transition **authority-activation authorization / final-execution readiness**'
  'Green for release-binary-evidenced live-epoch-transition-authority-activation-authorization-boundary behavior only'
  'Live epoch-transition **authority-activation final-execution**'
  'Green for release-binary-evidenced live-epoch-transition-authority-activation-final-execution-boundary behavior only'
  'Live epoch-transition **authority-activation post-final-execution confirmation / execution-sink-readiness**'
  'Green for release-binary-evidenced live-epoch-transition-authority-activation-post-final-execution-confirmation-boundary behavior only'
  'Live epoch-transition **authority-activation execution-sink prewrite / sink-commit-readiness**'
  'Green for release-binary-evidenced live-epoch-transition-authority-activation-execution-sink-prewrite-boundary behavior only'
  'becomes Green-for-scope only because Run 354 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 354 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# authority-activation-authorization row remains Green-for-scope and the
# authority-activation-execution-sink-prewrite row becomes Green-for-scope.
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-post-completion-attestation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition post-completion-attestation row must remain scoped Green"
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-authorization-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-authorization row must remain scoped Green"
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-post-final-execution-confirmation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-post-final-execution-confirmation row must remain scoped Green"
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-execution-sink-prewrite-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-execution-sink-prewrite row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 354 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-execution-sink-prewrite row must be scoped Green"
grep -F -q 'consumes verified live epoch-transition authority-activation post-final-execution-confirmation decisions and produces typed non-mutating live authority-activation execution-sink prewrite / sink-commit-readiness artifacts' "${C4C5_DOC}" || fail "authority-activation-execution-sink-prewrite row must record authority-activation-post-final-execution-confirmation consumption and typed non-mutating live authority-activation execution-sink prewrite / sink-commit-readiness artifacts"
# The live epoch-transition authority-activation-authorization and
# authority-activation-execution-sink-prewrite rows must record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "authority-activation-execution-sink-prewrite row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "authority-activation-execution-sink-prewrite row must record no public CLI flag"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "authority-activation-execution-sink-prewrite row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "authority-activation-execution-sink-prewrite row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition authority-activation-execution-sink-prewrite active' 'live epoch-transition authority-activation-execution-sink-prewrite enabled'
  'authority-activation-execution-sink-prewrite executor active' 'authority-activation-execution-sink-prewrite executor enabled'
  'post-final-execution-confirmation preflight active' 'post-final-execution-confirmation preflight enabled' 'authority-activation-execution-sink-prewrite applied' 'authority-activation applied' 'post-final-execution-confirmation applied'
  'live epoch-transition authority-activation-authorization active' 'live epoch-transition authority-activation-authorization enabled'
  'authority-activation-authorization executor active' 'authority-activation-authorization executor enabled'
  'authority-activation-authorization applied' 'authority-lifecycle-completion applied'
  'live epoch-transition settlement-execution active' 'live epoch-transition settlement-execution enabled'
  'settlement-execution executor active' 'settlement-execution executor enabled' 'settlement-execution applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition authority-activation-execution-sink-prewrite enabled' 'peer-driven apply enabled'
  ...
  'fallback to fixture proof' 'fallback to local operator proof' 'fallback to peer majority' 'fallback to on-chain proof' 'fallback to RemoteSigner' 'fallback to custody attestation' 'fallback to governance proof' 'fallback to governance execution intent' 'fallback to rotation plan' 'fallback to application decision' 'fallback to authorization decision' 'fallback to staged application decision' 'fallback to guarded mutation decision' 'fallback to runtime handoff decision' 'fallback to authority-activation-authorization decision'
  'raw local production key' 'DummySig active' 'DummyKem active' 'DummyAead active'
)
{
  echo "Run 354 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 354 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 353 ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 349/350 live epoch-transition"
  echo "  authority-activation post-final-execution-confirmation accept decision (a ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationDecision that is_accept() and"
  echo "  carries Some(authority_activation_post_final_execution_confirmation_artifact)), which itself composes the verified Run 347/348 authority-activation-authorization"
  echo "  accept decision and the full Run 303/304 → … → Run 349/350 chain. The Run 353 executor produces ONLY a typed non-mutating"
  echo "  live authority-activation execution-sink prewrite / sink-commit-readiness artifact describing"
  echo "  what a future live production authority-activation execution executor would apply. It applies no live production validator-set change and performs"
  ...
  echo "  no settlement-execution, no final-settlement, no authority-lifecycle-completion, no authority-activation-authorization,"
  echo "  no authority-activation execution-sink prewrite / sink-commit-readiness, no post-final-execution-confirmation-preflight, no post-final-execution-confirmation-readiness, no authority-activation,"
  echo "  no post-final-execution-confirmation, no external publication, and no raw local production signing key load. The only mutation any positive path performs is against a"
  echo "  caller-owned in-memory LiveEpochTransitionAuthorityActivationExecutionSinkPrewriteFixtureState used exclusively as source/test evidence,"
  echo "  which is explicitly distinct from production runtime state. Under a production or MainNet policy the executor fails"
  echo "  closed and never falls back to authority-activation-post-final-execution-confirmation-decision-alone / authority-activation-authorization-decision-alone /"
  echo "  commit-authorization-decision-alone / mutation-execution-decision-alone / execution-preparation-decision-alone / runtime-handoff-decision-alone /"
  ...
  echo "  authority-activation-post-final-execution-confirmation decisions are rejected as production authority; MainNet identities are refused before acceptance."
  echo "  The default ProductionLiveEpochTransitionAuthorityActivationExecutionSinkPrewriteExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 354 — release-binary evidence for the Run 353 live epoch-transition authority-activation execution-sink prewrite / sink-commit-readiness boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_354_deterministic_digests.txt"
  echo
  ...
  echo "c4c5_taxonomy: passed (${#C4C5_PHRASES[@]} phrases; prior boundary rows Green-for-scope only; live-epoch-transition-authority-activation-execution-sink-prewrite row Green-for-scope; Red rows unchanged; Full C4 OPEN; C5 OPEN)"
  echo "denylist: passed (${#DENY_PATTERNS[@]} patterns)"
  echo "tests:"
  for verdict in "${TEST_VERDICTS[@]}"; do echo "  ${verdict}"; done
  echo
  echo "verdict: PASS (release-binary evidence only; live epoch-transition authority-activation-execution-sink-prewrite Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN)"
} | tee "${SUMMARY}"