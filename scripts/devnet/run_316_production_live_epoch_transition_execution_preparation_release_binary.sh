#!/usr/bin/env bash
# Run 316 — release-binary evidence for the Run 315 live epoch-transition
...
# Run 315 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 313/314 verified
# epoch-transition runtime handoff accept decision — is_accept() with
# Some(handoff_package) — composing the real Run 311/312 guarded mutation
...
# preparation nonce binding + runtime-handoff-decision-integrity /
# current-validator-set epoch-version preflight / disabled /
# missing-runtime-handoff-decision / unverified-runtime-handoff-decision /
# accepted-without-package / guarded-mutation-alone / staged-application-alone /
...
# execution-preparation replay-recovery-idempotency / stale governance-epoch /
...
# verified epoch-transition runtime handoff accept decision and produces only a
# typed non-mutating live-execution preparation artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 315 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionExecutionPreparationOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionExecutionPreparationError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_316_production_live_epoch_transition_execution_preparation_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_316_BIN="${REPO_ROOT}/target/release/examples/run_316_production_live_epoch_transition_execution_preparation_release_binary_helper"
HELPER_316_OUT="${OUTDIR}/helper_evidence/run_316"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_execution_preparation.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_316_production_live_epoch_transition_execution_preparation_release_binary_helper.rs"

log() { printf '[run-316] %s\n' "$*" >&2; }
fail() { printf '[run-316] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "OUTDIR=${OUTDIR}"
mkdir -p "${OUTDIR}"
rm -rf "${HELPER_316_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
mkdir -p "${HELPER_316_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
: > "${PROVENANCE}"; : > "${DENYLIST}"; : > "${NOMUT_PROOF}"

{
  echo "run-316 provenance"
...
log "cargo build --release -p qbind-node --example run_316_production_live_epoch_transition_execution_preparation_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_316_production_live_epoch_transition_execution_preparation_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_316.log" 2>&1 || fail "helper build failed"
[[ -x "${NODE_BIN}" ]] || fail "missing ${NODE_BIN}"
[[ -x "${HELPER_316_BIN}" ]] || fail "missing ${HELPER_316_BIN}"
{
  echo "qbind_node_path:    ${NODE_BIN}"
  echo "qbind_node_sha256:  $(sha256_file "${NODE_BIN}")"
  echo "qbind_node_buildid: $(build_id "${NODE_BIN}")"
  echo "helper_316_path:    ${HELPER_316_BIN}"
  echo "helper_316_sha256:  $(sha256_file "${HELPER_316_BIN}")"
  echo "helper_316_buildid: $(build_id "${HELPER_316_BIN}")"
} >> "${PROVENANCE}"

log "running Run 316 helper (first invocation)"
set +e
"${HELPER_316_BIN}" "${HELPER_316_OUT}" > "${LOGS_DIR}/helper_run_316.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_316.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_316 helper failed"
assert_grep "${HELPER_316_OUT}/helper_summary.txt" 'verdict=PASS'
assert_grep "${HELPER_316_OUT}/helper_summary.txt" 'total_fail=0'

# Deterministic-digest stability across two independent helper invocations.
log "running Run 316 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_316_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_316_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_316_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_316_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_316 helper invocation failed"
if ! diff -q "${HELPER_316_OUT}/fixtures/run_316_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_316_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 315 live epoch-transition
# execution preparation boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition execution preparation active|live epoch-transition execution preparation enabled|execution preparation executor active|execution preparation executor enabled|live-execution preparation active|live-execution preparation enabled|ProductionLiveEpochTransitionExecutionPreparationExecutor enabled|MainNet live epoch-transition execution preparation enabled|execution preparation applied|live mutation applied'
...
log "S1 help hides Run 315 live epoch-transition execution preparation surface (no new CLI flag)"
...
# No Run 315/316 live epoch-transition execution preparation boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-execution-preparation|ProductionLiveEpochTransitionExecutionPreparation|pqc_production_live_epoch_transition_execution_preparation|execution-preparation|live-execution-preparation|run-316|run_316|run-315|run_315'
log "S2..S4 default surfaces silent on live epoch-transition execution preparation boundary claims"
...
log "S5 no live epoch-transition execution preparation CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-execution-preparation-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented execution-preparation selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on execution-preparation claims"
...
# Run 315 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN315_SYMS=(
  ProductionLiveEpochTransitionExecutionPreparationExecutor
  ProductionLiveEpochTransitionExecutionPreparationConfig
  ProductionLiveEpochTransitionExecutionPreparationExecutorKind
  ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy
  ProductionLiveEpochTransitionExecutionPreparationInputs
  ProductionLiveEpochTransitionExecutionPreparationRequest
  ...
  ProductionLiveEpochTransitionExecutionPreparationProtocolVersion
  LiveEpochTransitionExecutionPreparationKind
  LiveEpochTransitionExecutionPreparationAuthoritySource
  LiveEpochTransitionExecutionPreparationReplaySet
  EmptyLiveEpochTransitionExecutionPreparationReplaySet
  LiveEpochTransitionExecutionPreparationFixtureState
  ...
  production_live_epoch_transition_execution_preparation_id
  production_live_epoch_transition_execution_preparation_transcript_digest
)
COMBINED_CORPUS="${REACH_DIR}/combined_corpus.txt"
cat "${MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 316 combined reachability — Run 315 live epoch-transition execution preparation boundary symbols across source module + Run 313/314 epoch-transition runtime handoff module + Run 311/312 guarded mutation executor module + Run 309/310 staged application executor module + Run 307/308 live validator-set application authorization module + Run 305/306 validator-set rotation application executor module + Run 303/304 validator-set rotation intent module + release helper:"
  for sym in "${RUN315_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN315_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 316 source reachability — Run 315 live epoch-transition execution preparation boundary symbols in ${MOD}:"
  for sym in "${RUN315_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationConfig ProductionLiveEpochTransitionExecutionPreparationExecutorKind ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy ProductionLiveEpochTransitionExecutionPreparationInputs ProductionLiveEpochTransitionExecutionPreparationRequest ProductionLiveEpochTransitionExecutionPreparationDecision ProductionLiveEpochTransitionExecutionPreparationArtifact ProductionLiveEpochTransitionExecutionPreparationOutcome ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome ProductionLiveEpochTransitionExecutionPreparationProtocolVersion LiveEpochTransitionExecutionPreparationKind LiveEpochTransitionExecutionPreparationAuthoritySource LiveEpochTransitionExecutionPreparationReplaySet EmptyLiveEpochTransitionExecutionPreparationReplaySet LiveEpochTransitionExecutionPreparationFixtureState evaluate_live_epoch_transition_execution_preparation recover_live_epoch_transition_execution_preparation_window production_live_epoch_transition_execution_preparation_content_digest production_live_epoch_transition_execution_preparation_request_id production_live_epoch_transition_execution_preparation_id production_live_epoch_transition_execution_preparation_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 316 helper reachability — Run 315 symbols exercised by the release helper (plus the Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN315_SYMS[@]}" pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationConfig ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy ProductionLiveEpochTransitionExecutionPreparationInputs ProductionLiveEpochTransitionExecutionPreparationRequest ProductionLiveEpochTransitionExecutionPreparationDecision ProductionLiveEpochTransitionExecutionPreparationArtifact ProductionLiveEpochTransitionExecutionPreparationOutcome ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome LiveEpochTransitionExecutionPreparationKind LiveEpochTransitionExecutionPreparationAuthoritySource EmptyLiveEpochTransitionExecutionPreparationReplaySet LiveEpochTransitionExecutionPreparationFixtureState evaluate_live_epoch_transition_execution_preparation recover_live_epoch_transition_execution_preparation_window production_live_epoch_transition_execution_preparation_content_digest production_live_epoch_transition_execution_preparation_request_id production_live_epoch_transition_execution_preparation_id production_live_epoch_transition_execution_preparation_transcript_digest ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_execution_preparation' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_execution_preparation\|fn recover_live_epoch_transition_execution_preparation_window\|fn production_live_epoch_transition_execution_preparation_content_digest\|fn production_live_epoch_transition_execution_preparation_request_id\|fn production_live_epoch_transition_execution_preparation_id\|fn production_live_epoch_transition_execution_preparation_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionExecutionPreparationOutcome\|enum ProductionLiveEpochTransitionExecutionPreparationRecoveryOutcome\|enum ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy\|enum ProductionLiveEpochTransitionExecutionPreparationExecutorKind\|enum LiveEpochTransitionExecutionPreparationKind\|enum LiveEpochTransitionExecutionPreparationAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionExecutionPreparationReplaySet\|struct ProductionLiveEpochTransitionExecutionPreparationExecutor\|struct ProductionLiveEpochTransitionExecutionPreparationArtifact\|struct ProductionLiveEpochTransitionExecutionPreparationRequest\|struct EmptyLiveEpochTransitionExecutionPreparationReplaySet\|struct LiveEpochTransitionExecutionPreparationFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 316'
  'boundary readiness'
...
{
  echo "Run 316 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# execution preparation row becomes Green-for-scope.
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-execution-preparation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition execution preparation row must be scoped Green"
# The live epoch-transition execution preparation row wording must record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "execution preparation row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "execution preparation row must record no public CLI flag"
...
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "execution preparation row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "execution preparation row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition execution preparation enabled' 'peer-driven apply enabled'
...
{
  echo "Run 316 denylist (proven empty across captured logs/helper output except help and summary):"
  for pat in "${DENY_PATTERNS[@]}"; do
    if find "${LOGS_DIR}" "${HELPER_316_OUT}" -type f ! -name qbind_node_help.log ! -name helper_summary.txt -print0 | xargs -0 grep -E -i -l "$pat" 2>/dev/null | head -n1 | grep -q .; then echo "FAIL pattern present: ${pat}"; exit 7; else echo "ok-empty: ${pat}"; fi
  done
} > "${DENYLIST}"

{
  echo "Run 316 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 315 ProductionLiveEpochTransitionExecutionPreparationExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 313/314 epoch-transition"
  echo "  runtime handoff accept decision (a"
  echo "  ProductionEpochTransitionRuntimeHandoffDecision that is_accept() and carries"
  echo "  Some(handoff_package)) and produces ONLY a typed non-mutating live-execution preparation artifact describing what a"
  echo "  future live production mutation executor would apply. It applies no live production validator-set change and performs"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionExecutionPreparationFixtureState used exclusively as source/test evidence,"
  echo "  which is explicitly distinct from production runtime state. Under a production or MainNet policy the executor fails"
  echo "  closed and never falls back to runtime-handoff-decision-alone / guarded-mutation-decision-alone /"
  ...
  echo "  custody-attestation-only / arbitrary-bytes material. Missing / unverified / accepted-without-package runtime-handoff"
  echo "  decisions are rejected as production authority; MainNet identities are refused before acceptance. The default"
  echo "  ProductionLiveEpochTransitionExecutionPreparationExecutorPolicy is Disabled; the production binary is not wired to"
  echo "  construct the boundary and adds no CLI flag."
  echo "  helper corpus tables:"; grep -E 'verdict=PASS|^table |^total_(pass|fail)=' "${HELPER_316_OUT}/helper_summary.txt" | sed 's/^/    /'
} > "${NOMUT_PROOF}"
...
{
  echo "Run 316 — release-binary evidence for the Run 315 live epoch-transition execution preparation boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  echo "  helper_316_sha256:  $(sha256_file "${HELPER_316_BIN}")"
  echo
  echo "helper_summary: ${HELPER_316_OUT}/helper_summary.txt"
  sed 's/^/  /' "${HELPER_316_OUT}/helper_summary.txt"
  echo
  echo "deterministic_digests: stable across two independent helper invocations"
  sed 's/^/  /' "${HELPER_316_OUT}/fixtures/run_316_deterministic_digests.txt"
  echo
