#!/usr/bin/env bash
# Run 336 — release-binary evidence for the Run 335 live epoch-transition
# settlement-preparation boundary.
#
# Release-binary evidence for the Run 335 source/test live epoch-transition
# settlement-preparation boundary
...
# OPEN.
#
# Substitution note: the Run 335 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionSettlementPreparationOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionSettlementPreparationError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_336_production_live_epoch_transition_settlement_preparation_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_336_production_live_epoch_transition_settlement_preparation_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_336"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_settlement_preparation.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_336_production_live_epoch_transition_settlement_preparation_release_binary_helper.rs"

log() { printf '[run-336] %s\n' "$*" >&2; }
fail() { printf '[run-336] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_336_production_live_epoch_transition_settlement_preparation_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_336_production_live_epoch_transition_settlement_preparation_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_336.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_336_path:    ${HELPER_BIN}"
  echo "helper_336_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_336_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 336 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_336.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_336.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_336 helper failed"
...
log "running Run 336 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_336_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_336_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_336_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_336 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_336_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_336_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 335 live epoch-transition
# settlement-preparation boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition settlement-preparation active|live epoch-transition settlement-preparation enabled|settlement-preparation executor active|settlement-preparation executor enabled|live-settlement-preparation active|live-settlement-preparation enabled|ProductionLiveEpochTransitionSettlementPreparationExecutor enabled|MainNet live epoch-transition settlement-preparation enabled|settlement-preparation applied|live mutation applied|live epoch-transition external-publication active|live epoch-transition external-publication enabled|external-publication executor active|external-publication executor enabled|ProductionLiveEpochTransitionExternalPublicationExecutor enabled|external-publication applied|live epoch-transition commit receipt active|live epoch-transition commit receipt enabled|commit receipt executor active|commit receipt executor enabled|live-commit receipt active|live-commit receipt enabled|ProductionLiveEpochTransitionCommitReceiptExecutor enabled|MainNet live epoch-transition commit receipt enabled|commit receipt applied'
...
log "S1 help hides Run 335 live epoch-transition settlement-preparation surface (no new CLI flag)"
...
# No Run 336/319 live epoch-transition settlement-preparation boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-settlement-preparation|ProductionLiveEpochTransitionSettlementPreparation|pqc_production_live_epoch_transition_settlement_preparation|settlement-preparation|live-settlement-preparation|live-epoch-transition-external-publication|ProductionLiveEpochTransitionExternalPublication|pqc_production_live_epoch_transition_external_publication|external-publication|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-336|run_336|run-329|run_329'
log "S2..S4 default surfaces silent on live epoch-transition settlement-preparation boundary claims"
...
log "S5 no live epoch-transition settlement-preparation CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-settlement-preparation-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented settlement-preparation selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on settlement-preparation claims"
...
# Run 335 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN335_SYMS=(
  ProductionLiveEpochTransitionSettlementPreparationExecutor
  ProductionLiveEpochTransitionSettlementPreparationConfig
  ...
  ProductionLiveEpochTransitionSettlementPreparationProtocolVersion
  LiveEpochTransitionSettlementPreparationKind
  LiveEpochTransitionSettlementPreparationAuthoritySource
  LiveEpochTransitionSettlementPreparationReplaySet
  ...
  recover_live_epoch_transition_settlement_preparation_window
  production_live_epoch_transition_settlement_preparation_content_digest
  production_live_epoch_transition_settlement_preparation_request_id
  production_live_epoch_transition_settlement_preparation_id
...
{
  echo "Run 336 combined reachability — Run 335 live epoch-transition settlement-preparation boundary symbols across source module + Run 335/324 commit receipt module + Run 313/314 epoch-transition runtime handoff module + Run 311/312 guarded mutation executor module + Run 309/310 staged application executor module + Run 307/308 live validator-set application authorization module + Run 305/306 validator-set rotation application executor module + Run 303/304 validator-set rotation intent module + release helper:"
  for sym in "${RUN335_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN335_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 336 source reachability — Run 335 live epoch-transition settlement-preparation boundary symbols in ${MOD}:"
  for sym in "${RUN335_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationConfig ProductionLiveEpochTransitionSettlementPreparationExecutorKind ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy ProductionLiveEpochTransitionSettlementPreparationInputs ProductionLiveEpochTransitionSettlementPreparationRequest ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionSettlementPreparationArtifact ProductionLiveEpochTransitionSettlementPreparationOutcome ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome ProductionLiveEpochTransitionSettlementPreparationProtocolVersion LiveEpochTransitionSettlementPreparationKind LiveEpochTransitionSettlementPreparationAuthoritySource LiveEpochTransitionSettlementPreparationReplaySet EmptyLiveEpochTransitionSettlementPreparationReplaySet LiveEpochTransitionSettlementPreparationFixtureState evaluate_live_epoch_transition_settlement_preparation recover_live_epoch_transition_settlement_preparation_window production_live_epoch_transition_settlement_preparation_content_digest production_live_epoch_transition_settlement_preparation_request_id production_live_epoch_transition_settlement_preparation_id production_live_epoch_transition_settlement_preparation_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 336 helper reachability — Run 335 symbols exercised by the release helper (plus the Run 335/324 commit receipt + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN335_SYMS[@]}" pqc_production_live_epoch_transition_external_publication ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision pqc_production_live_epoch_transition_commit_receipt ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationConfig ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy ProductionLiveEpochTransitionSettlementPreparationInputs ProductionLiveEpochTransitionSettlementPreparationRequest ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionSettlementPreparationArtifact ProductionLiveEpochTransitionSettlementPreparationOutcome ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome LiveEpochTransitionSettlementPreparationKind LiveEpochTransitionSettlementPreparationAuthoritySource EmptyLiveEpochTransitionSettlementPreparationReplaySet LiveEpochTransitionSettlementPreparationFixtureState evaluate_live_epoch_transition_settlement_preparation recover_live_epoch_transition_settlement_preparation_window production_live_epoch_transition_settlement_preparation_content_digest production_live_epoch_transition_settlement_preparation_request_id production_live_epoch_transition_settlement_preparation_id production_live_epoch_transition_settlement_preparation_transcript_digest ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_settlement_preparation' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_settlement_preparation\|fn recover_live_epoch_transition_settlement_preparation_window\|fn production_live_epoch_transition_settlement_preparation_content_digest\|fn production_live_epoch_transition_settlement_preparation_request_id\|fn production_live_epoch_transition_settlement_preparation_id\|fn production_live_epoch_transition_settlement_preparation_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionSettlementPreparationOutcome\|enum ProductionLiveEpochTransitionSettlementPreparationRecoveryOutcome\|enum ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy\|enum ProductionLiveEpochTransitionSettlementPreparationExecutorKind\|enum LiveEpochTransitionSettlementPreparationKind\|enum LiveEpochTransitionSettlementPreparationAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionSettlementPreparationReplaySet\|struct ProductionLiveEpochTransitionSettlementPreparationExecutor\|struct ProductionLiveEpochTransitionSettlementPreparationArtifact\|struct ProductionLiveEpochTransitionSettlementPreparationRequest\|struct EmptyLiveEpochTransitionSettlementPreparationReplaySet\|struct LiveEpochTransitionSettlementPreparationFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 336'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 336 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 336 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# commit receipt row remains Green-for-scope and the settlement-preparation row
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-settlement-preparation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition settlement-preparation row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 336 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition settlement-preparation row must be scoped Green"
# The live epoch-transition commit receipt and settlement-preparation rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "settlement-preparation row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "settlement-preparation row must record no public CLI flag"
...
grep -F -q 'consumes verified live epoch-transition external-publication decisions and produces typed non-mutating live settlement / finalization-preparation artifacts for a future production settlement or finalization run' "${C4C5_DOC}" || fail "settlement-preparation row must record external-publication consumption and typed non-mutating live settlement / finalization-preparation artifacts"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "settlement-preparation row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "settlement-preparation row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition settlement-preparation active' 'live epoch-transition settlement-preparation enabled'
  'settlement-preparation executor active' 'settlement-preparation executor enabled'
  'live-settlement-preparation active' 'live-settlement-preparation enabled' 'settlement-preparation applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition settlement-preparation enabled' 'peer-driven apply enabled'
...
{
  echo "Run 336 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 336 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 335 ProductionLiveEpochTransitionSettlementPreparationExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 335/324 live epoch-transition"
  ...
  echo "  accept decision. The Run 335 executor produces ONLY a typed non-mutating live-settlement-preparation artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionSettlementPreparationFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionSettlementPreparationExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 336 — release-binary evidence for the Run 335 live epoch-transition settlement-preparation boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_336_deterministic_digests.txt"
  echo
