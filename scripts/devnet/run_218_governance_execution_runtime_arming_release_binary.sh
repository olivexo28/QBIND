...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_218_governance_execution_runtime_arming_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_218_BIN="${REPO_ROOT}/target/release/examples/run_218_governance_execution_runtime_arming_release_binary_helper"
HELPER_218_OUT="${OUTDIR}/helper_evidence/run_218"
...
log() { printf '[run-218] %s\n' "$*" >&2; }
fail() { printf '[run-218] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "OUTDIR=${OUTDIR}"
mkdir -p "${OUTDIR}"
rm -rf "${HELPER_218_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
mkdir -p "${HELPER_218_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
: > "${PROVENANCE}"; : > "${DENYLIST}"; : > "${MUT_PROOF}"; : > "${NOMUT_PROOF}"

{
  echo "run-218 provenance"
...
log "cargo build --release -p qbind-node --example run_218_governance_execution_runtime_arming_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_218_governance_execution_runtime_arming_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_218.log" 2>&1 || fail "helper build failed"
[[ -x "${NODE_BIN}" ]] || fail "missing ${NODE_BIN}"
[[ -x "${HELPER_218_BIN}" ]] || fail "missing ${HELPER_218_BIN}"
{
  echo "qbind_node_path:    ${NODE_BIN}"
  echo "qbind_node_sha256:  $(sha256_file "${NODE_BIN}")"
  echo "qbind_node_buildid: $(build_id "${NODE_BIN}")"
  echo "helper_218_path:    ${HELPER_218_BIN}"
  echo "helper_218_sha256:  $(sha256_file "${HELPER_218_BIN}")"
  echo "helper_218_buildid: $(build_id "${HELPER_218_BIN}")"
} >> "${PROVENANCE}"

log "running Run 218 helper"
set +e
"${HELPER_218_BIN}" "${HELPER_218_OUT}" > "${LOGS_DIR}/helper_run_218.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_218.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_218 helper failed"
assert_grep "${HELPER_218_OUT}/helper_summary.txt" 'verdict: PASS'

{
  echo "Run 218 governance-execution fixture inventory (helper-minted):"
  if [[ -d "${HELPER_218_OUT}/fixtures" ]]; then
    for f in "${HELPER_218_OUT}/fixtures"/*; do [[ -f "$f" ]] && echo "  $(basename "$f") sha256=$(sha256_file "$f")"; done
  fi
...
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'governance execution|production governance|mainnet governance|validator-set rotation|on-chain governance proof verifier|runtime arming|run-217|run-218'
...
assert_grep "${REACH_DIR}/source_reachability.txt" 'GovernanceExecutionRuntimeArmingConfig'
assert_grep "${REACH_DIR}/source_reachability.txt" 'governance_execution_runtime_arming'
assert_grep "${REACH_DIR}/source_reachability.txt" 'Run105ReloadCheckContextData'
...
# Prove the runtime carrier is embedded in the long-running LiveReloadConfig
# and consumed by the main.rs runtime hooks (reload-apply/startup/reload-check/
# SIGHUP/local-peer-candidate call sites).
grep -RIn --include='*.rs' 'governance_execution_runtime_arming\|GovernanceExecutionRuntimeArmingConfig::from_cli_or_env\|GovernanceExecutionRuntimeSurface::' "${SRC_DIR}/main.rs" "${SRC_DIR}/pqc_live_trust_reload.rs" > "${REACH_DIR}/runtime_hook_reachability.txt" || fail "missing runtime-hook reachability"
assert_grep "${REACH_DIR}/runtime_hook_reachability.txt" 'GovernanceExecutionRuntimeSurface::ReloadApply'
assert_grep "${REACH_DIR}/runtime_hook_reachability.txt" 'GovernanceExecutionRuntimeSurface::StartupP2pTrustBundle'
assert_grep "${REACH_DIR}/runtime_hook_reachability.txt" 'GovernanceExecutionRuntimeSurface::ReloadCheck'
assert_grep "${REACH_DIR}/runtime_hook_reachability.txt" 'GovernanceExecutionRuntimeSurface::LocalPeerCandidateCheck'

{
  echo "Run 218 denylist (proven empty across captured logs):"
  for pat in 'MainNet apply ENABLED' 'MainNet peer-driven apply ENABLED' 'autonomous apply' 'apply on receipt' 'peer-majority authority' 'fallback to --p2p-trusted-root' 'DummySig' 'DummyKem' 'DummyAead' 'governance execution active' 'production governance active' 'MainNet governance enabled' 'on-chain governance proof verifier active' 'real KMS backend' 'real HSM backend' 'real RemoteSigner backend' 'custody attestation production active' 'validator-set rotation enabled' 'schema drift' 'wire drift' 'metric drift'; do
    if find "${LOGS_DIR}" "${HELPER_218_OUT}" -type f ! -name qbind_node_help.log ! -name helper_summary.txt -print0 | xargs -0 grep -E -i -l "$pat" 2>/dev/null | head -n1 | grep -q .; then echo "FAIL pattern present: ${pat}"; exit 7; else echo "ok-empty: ${pat}"; fi
  done
} > "${DENYLIST}"

{
  echo "Run 218 no-mutation proof for rejected runtime-armed governance-execution scenarios:"
  echo "  data dir at ${DATA_DIR} contents:"; ls -la "${DATA_DIR}" 2>/dev/null || true
  echo "  R4..R28 helper corpus (all driven through GovernanceExecutionRuntimeArmingConfig): no Run 070 apply call, no live trust swap, no session eviction, no sequence write, no marker write, marker/sequence bytes unchanged where present, no .tmp residue, no fallback to --p2p-trusted-root, no DummySig/DummyKem/DummyAead. The runtime-arming carrier and per-surface preflight wrappers are pure typed outcome functions; a reject short-circuits before any mutation."
  grep -E 'verdict: PASS|^table |^total_(pass|fail):' "${HELPER_218_OUT}/helper_summary.txt" | sed 's/^/    /'
} > "${NOMUT_PROOF}"
{
  echo "Run 218 mutation proof (release-binary scope): selector resolution occurs before runtime policy arming; runtime policy arming occurs before governance-execution material parse; material parse before marker decision; governance execution, lifecycle, governance-proof, custody and custody-attestation validation occur before apply/mutation; accepted mutating compatibility remains subject to Run 055 sequence commit before v2 marker persist. No mutation is performed by this fixture-only helper."
} > "${MUT_PROOF}"
...
{
  echo "Run 218 — release-binary governance-execution runtime-arming evidence"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo
  echo "build:"
  echo "  rustc_version:      $(rustc --version 2>/dev/null || echo unknown)"
  ...
  echo "  helper_run_218rc=$(cat "${EXIT_DIR}/helper_run_218.rc")$(grep -E 'verdict:' "${HELPER_218_OUT}/helper_summary.txt" | head -n1)"
  echo
  echo "helper runtime-arming + A1-A22 / R1-R28 corpus verdicts (release mode, Run 217 carrier symbols):"
  grep -E '^table |^total_(pass|fail): |^verdict: ' "${HELPER_218_OUT}/helper_summary.txt" | sed 's/^/  /'
  echo
  ...
  echo
  echo "honest_limits: default Disabled; hidden CLI/env selectors reach runtime arming; CLI-over-env deterministic at the runtime config boundary; invalid values fail closed before mutation; fixture DevNet/TestNet only; emergency explicit non-production; production/on-chain/MainNet unavailable; MainNet peer-driven apply refused; live 0x05 runtime-config policy threading limitation documented; no real governance engine/on-chain verifier/KMS-HSM/RemoteSigner/validator-set rotation; existing paths compatible; full C4 and C5 remain OPEN."
  echo
  echo "verdict: PASS"
} > "${SUMMARY}"
