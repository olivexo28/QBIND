#!/usr/bin/env bash
# Run 338 — release-binary evidence for the Run 337 live epoch-transition
# settlement-execution-preparation boundary.
#
# Release-binary evidence for the Run 337 source/test live epoch-transition
# settlement-execution-preparation boundary
...
# Run 337 production library symbols are present and exercised in release mode,
...
# typed non-mutating live-settlement-execution-preparation artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 337 executor surfaces every failure as a typed
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_338"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_settlement_execution_preparation.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper.rs"

log() { printf '[run-338] %s\n' "$*" >&2; }
fail() { printf '[run-338] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_338.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_338_path:    ${HELPER_BIN}"
  echo "helper_338_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_338_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 338 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_338.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_338.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_338 helper failed"
...
log "running Run 338 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_338_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_338_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_338_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_338 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_338_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_338_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 337 live epoch-transition
# settlement-execution-preparation boundary has been constructed / enabled / wired.
...
log "S1 help hides Run 337 live epoch-transition settlement-execution-preparation surface (no new CLI flag)"
...
# No Run 338/319 live epoch-transition settlement-execution-preparation boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-settlement-execution-preparation|ProductionLiveEpochTransitionSettlementExecutionPreparation|pqc_production_live_epoch_transition_settlement_execution_preparation|settlement-execution-preparation|live-settlement-execution-preparation|live-epoch-transition-external-publication|ProductionLiveEpochTransitionExternalPublication|pqc_production_live_epoch_transition_external_publication|external-publication|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-338|run_338|run-329|run_329'
log "S2..S4 default surfaces silent on live epoch-transition settlement-execution-preparation boundary claims"
...
log "S5 no live epoch-transition settlement-execution-preparation CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-settlement-execution-preparation-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented settlement-execution-preparation selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on settlement-execution-preparation claims"
...
# Run 337 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN337_SYMS=(
  ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor
  ProductionLiveEpochTransitionSettlementExecutionPreparationConfig
  ...
  LiveEpochTransitionSettlementExecutionPreparationReplaySet
  EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet
  LiveEpochTransitionSettlementExecutionPreparationFixtureState
...
cat "${MOD}" "${SETTLE_PREP_MOD}" "${PCA_MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 338 combined reachability — Run 337 live epoch-transition settlement-execution-preparation boundary symbols across source module + Run 337/324 commit receipt module + Run 313/314 epoch-transition runtime handoff module + Run 311/312 guarded mutation executor module + Run 309/310 staged application executor module + Run 307/308 live validator-set application authorization module + Run 305/306 validator-set rotation application executor module + Run 303/304 validator-set rotation intent module + release helper:"
  for sym in "${RUN337_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN337_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 338 source reachability — Run 337 live epoch-transition settlement-execution-preparation boundary symbols in ${MOD}:"
  for sym in "${RUN337_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationConfig ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy ProductionLiveEpochTransitionSettlementExecutionPreparationInputs ProductionLiveEpochTransitionSettlementExecutionPreparationRequest ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome ProductionLiveEpochTransitionSettlementExecutionPreparationProtocolVersion LiveEpochTransitionSettlementExecutionPreparationKind LiveEpochTransitionSettlementExecutionPreparationAuthoritySource LiveEpochTransitionSettlementExecutionPreparationReplaySet EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet LiveEpochTransitionSettlementExecutionPreparationFixtureState evaluate_live_epoch_transition_settlement_execution_preparation recover_live_epoch_transition_settlement_execution_preparation_window production_live_epoch_transition_settlement_execution_preparation_content_digest production_live_epoch_transition_settlement_execution_preparation_request_id production_live_epoch_transition_settlement_execution_preparation_id production_live_epoch_transition_settlement_execution_preparation_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 338 helper reachability — Run 337 symbols exercised by the release helper (plus the Run 337/324 commit receipt + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN337_SYMS[@]}" pqc_production_live_epoch_transition_settlement_preparation ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision pqc_production_live_epoch_transition_external_publication ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision pqc_production_live_epoch_transition_commit_receipt ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationConfig ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy ProductionLiveEpochTransitionSettlementExecutionPreparationInputs ProductionLiveEpochTransitionSettlementExecutionPreparationRequest ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome LiveEpochTransitionSettlementExecutionPreparationKind LiveEpochTransitionSettlementExecutionPreparationAuthoritySource EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet LiveEpochTransitionSettlementExecutionPreparationFixtureState evaluate_live_epoch_transition_settlement_execution_preparation recover_live_epoch_transition_settlement_execution_preparation_window production_live_epoch_transition_settlement_execution_preparation_content_digest production_live_epoch_transition_settlement_execution_preparation_request_id production_live_epoch_transition_settlement_execution_preparation_id production_live_epoch_transition_settlement_execution_preparation_transcript_digest ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_settlement_execution_preparation' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_settlement_execution_preparation\|fn recover_live_epoch_transition_settlement_execution_preparation_window\|fn production_live_epoch_transition_settlement_execution_preparation_content_digest\|fn production_live_epoch_transition_settlement_execution_preparation_request_id\|fn production_live_epoch_transition_settlement_execution_preparation_id\|fn production_live_epoch_transition_settlement_execution_preparation_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome\|enum ProductionLiveEpochTransitionSettlementExecutionPreparationRecoveryOutcome\|enum ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy\|enum ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorKind\|enum LiveEpochTransitionSettlementExecutionPreparationKind\|enum LiveEpochTransitionSettlementExecutionPreparationAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionSettlementExecutionPreparationReplaySet\|struct ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor\|struct ProductionLiveEpochTransitionSettlementExecutionPreparationArtifact\|struct ProductionLiveEpochTransitionSettlementExecutionPreparationRequest\|struct EmptyLiveEpochTransitionSettlementExecutionPreparationReplaySet\|struct LiveEpochTransitionSettlementExecutionPreparationFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 338'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 338 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 338 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-settlement-execution-preparation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition settlement-execution-preparation row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 338 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition settlement-execution-preparation row must be scoped Green"
...
{
  echo "Run 338 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 338 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 337 ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 337/324 live epoch-transition"
  ...
  echo "  accept decision. The Run 337 executor produces ONLY a typed non-mutating live-settlement-execution-preparation artifact describing"
...
{
  echo "Run 338 — release-binary evidence for the Run 337 live epoch-transition settlement-execution-preparation boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_338_deterministic_digests.txt"
  echo
