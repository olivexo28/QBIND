#!/usr/bin/env bash
# Run 378: public DevNet M4 EXTERNAL seed/bootnode reachability attempt harness.
#
# This harness produces the Run 378 acceptance evidence for M4 live seed/bootnode
# reachability (see `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_378.md`). It drives the
# RELEASE `target/release/qbind-node` binary directly and records an HONEST,
...
# primary objective is Route A: expose a real externally reachable seed and prove
# reachability from an INDEPENDENT EXTERNAL vantage point. In this sandboxed CI
# environment there is NO external ingress and NO independent external vantage
# point, so a real endpoint CANNOT be exposed and external reachability CANNOT be
# proven. Per Route C the harness does NOT expose a real endpoint, does NOT invent
# an external vantage point, and does NOT mark any committed seed entry
# `status: live`. M4 stays YELLOW; the infrastructure prerequisites for Route A
# are documented in the reachability evidence record.
#
# What it still proves (continuity with Run 377):
...
#
# Run 378 opens NO externally reachable port, deploys NO seed/bootnode/faucet/RPC/
...
OUTDIR="${1:-/tmp/qbind-run378-public-devnet-external-seed-reachability}"
...
REACH_REF="docs/release/public-devnet/network/reachability/RUN_378_qbind-devnet-seed-1.md"
SUMMARY="${OUTDIR}/summary.txt"

log()  { printf '[run378] %s\n' "$*"; }
fail() { printf '[run378] FAIL: %s\n' "$*" >&2; exit 1; }
...
#    (referencing the Run 378 evidence record).
# ---------------------------------------------------------------------------
...
emit "external_reachability=NOT_PROVEN (Route C: no external ingress / no independent external vantage point in this environment; M4 stays Yellow)"
