#!/usr/bin/env bash
# Run 342 — release-binary evidence for the Run 341 live epoch-transition
# final settlement / authority-lifecycle completion boundary.
#
# Release-binary evidence for the Run 341 source/test live epoch-transition
# final settlement / authority-lifecycle completion boundary
...
# Run 341 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 339/340 verified
# live epoch-transition settlement / finalization execution accept decision —
# is_accept() with Some(settlement_execution_artifact) — composing the real
...
# Run 333/334 → Run 335/336 → Run 337/338 → Run 339/340 accept chain; full
# settlement-execution / runtime-handoff / guarded-mutation / staged-application /
...
# guarded-mutation / runtime-handoff / execution-preparation / settlement-execution
# nonce binding + settlement-execution-decision-integrity /
# current-validator-set epoch-version preflight / disabled /
# missing-settlement-execution-decision / unverified-settlement-execution-decision /
# accepted-without-artifact / settlement-execution-decision-alone / runtime-handoff-decision-alone /
...
# final-settlement-completion replay-recovery-idempotency / stale governance-epoch /
...
# verified live epoch-transition settlement / finalization execution accept decision
# and produces only a typed non-mutating live final-settlement / authority-lifecycle
# completion artifact for a future live production authority-lifecycle completion
# executor. The release helper remains dead code from the
...
# OPEN.
#
# Substitution note: the Run 341 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionFinalSettlementCompletionOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionFinalSettlementCompletionError`
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_342_production_live_epoch_transition_final_settlement_completion_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_342_production_live_epoch_transition_final_settlement_completion_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_342"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_final_settlement_completion.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_342_production_live_epoch_transition_final_settlement_completion_release_binary_helper.rs"

log() { printf '[run-342] %s\n' "$*" >&2; }
fail() { printf '[run-342] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_342_production_live_epoch_transition_final_settlement_completion_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_342_production_live_epoch_transition_final_settlement_completion_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_342.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_342_path:    ${HELPER_BIN}"
  echo "helper_342_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_342_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 342 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_342.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_342.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_342 helper failed"
...
log "running Run 342 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_342_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_342_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_342_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_342 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_342_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_342_deterministic_digests.txt" >/dev/null; then
...
log "S1 help hides Run 341 live epoch-transition final-settlement-completion surface (no new CLI flag)"
...
# No Run 342/341 live epoch-transition final-settlement-completion boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-final-settlement-completion|ProductionLiveEpochTransitionFinalSettlementCompletion|pqc_production_live_epoch_transition_final_settlement_completion|final-settlement-completion|authority-lifecycle-completion|live-epoch-transition-settlement-execution|ProductionLiveEpochTransitionSettlementExecution|pqc_production_live_epoch_transition_settlement_execution|settlement-execution|live-settlement-execution|live-epoch-transition-external-publication|ProductionLiveEpochTransitionExternalPublication|pqc_production_live_epoch_transition_external_publication|external-publication|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-342|run_342|run-341|run_341'
...
log "S5 no live epoch-transition final-settlement-completion CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-final-settlement-completion-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented final-settlement-completion selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on final-settlement-completion claims"
...
RUN341_SYMS=(
  ...
  ProductionLiveEpochTransitionFinalSettlementCompletionConfig
  ...
  ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion
  LiveEpochTransitionFinalSettlementCompletionKind
  LiveEpochTransitionFinalSettlementCompletionAuthoritySource
  LiveEpochTransitionFinalSettlementCompletionReplaySet
  ...
  recover_live_epoch_transition_final_settlement_completion_window
  production_live_epoch_transition_final_settlement_completion_content_digest
...
cat "${MOD}" "${SETTLE_EXEC_MOD}" "${SETTLE_EXEC_PREP_MOD}" "${SETTLE_PREP_MOD}" "${PCA_MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 342 combined reachability — Run 341 live epoch-transition final-settlement-completion boundary symbols across source module + Run 339/340 settlement-execution input module + ancestor chain modules + release helper:"
...
{
  echo "Run 342 source reachability — Run 341 live epoch-transition final-settlement-completion boundary symbols in ${MOD}:"
...
for sym in ProductionLiveEpochTransitionFinalSettlementCompletionExecutor ProductionLiveEpochTransitionFinalSettlementCompletionConfig ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy ProductionLiveEpochTransitionFinalSettlementCompletionInputs ProductionLiveEpochTransitionFinalSettlementCompletionRequest ProductionLiveEpochTransitionFinalSettlementCompletionDecision ProductionLiveEpochTransitionFinalSettlementCompletionArtifact ProductionLiveEpochTransitionFinalSettlementCompletionOutcome ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome ProductionLiveEpochTransitionFinalSettlementCompletionProtocolVersion LiveEpochTransitionFinalSettlementCompletionKind LiveEpochTransitionFinalSettlementCompletionAuthoritySource LiveEpochTransitionFinalSettlementCompletionReplaySet EmptyLiveEpochTransitionFinalSettlementCompletionReplaySet LiveEpochTransitionFinalSettlementCompletionFixtureState evaluate_live_epoch_transition_final_settlement_completion recover_live_epoch_transition_final_settlement_completion_window production_live_epoch_transition_final_settlement_completion_content_digest production_live_epoch_transition_final_settlement_completion_request_id production_live_epoch_transition_final_settlement_completion_id production_live_epoch_transition_final_settlement_completion_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 342 helper reachability — Run 341 symbols exercised by the release helper (plus the Run 339/340 settlement-execution + Run 337/338 settlement-execution-preparation + Run 335/336 settlement-preparation + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
...
for sym in ProductionLiveEpochTransitionFinalSettlementCompletionExecutor ProductionLiveEpochTransitionFinalSettlementCompletionConfig ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy ProductionLiveEpochTransitionFinalSettlementCompletionInputs ProductionLiveEpochTransitionFinalSettlementCompletionRequest ProductionLiveEpochTransitionFinalSettlementCompletionDecision ProductionLiveEpochTransitionFinalSettlementCompletionArtifact ProductionLiveEpochTransitionFinalSettlementCompletionOutcome ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome LiveEpochTransitionFinalSettlementCompletionKind LiveEpochTransitionFinalSettlementCompletionAuthoritySource EmptyLiveEpochTransitionFinalSettlementCompletionReplaySet LiveEpochTransitionFinalSettlementCompletionFixtureState evaluate_live_epoch_transition_final_settlement_completion recover_live_epoch_transition_final_settlement_completion_window ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionDecision ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_final_settlement_completion' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_final_settlement_completion\|fn recover_live_epoch_transition_final_settlement_completion_window\|fn production_live_epoch_transition_final_settlement_completion_content_digest\|fn production_live_epoch_transition_final_settlement_completion_request_id\|fn production_live_epoch_transition_final_settlement_completion_id\|fn production_live_epoch_transition_final_settlement_completion_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionFinalSettlementCompletionOutcome\|enum ProductionLiveEpochTransitionFinalSettlementCompletionRecoveryOutcome\|enum ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy\|enum ProductionLiveEpochTransitionFinalSettlementCompletionExecutorKind\|enum LiveEpochTransitionFinalSettlementCompletionKind\|enum LiveEpochTransitionFinalSettlementCompletionAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionFinalSettlementCompletionReplaySet\|struct ProductionLiveEpochTransitionFinalSettlementCompletionExecutor\|struct ProductionLiveEpochTransitionFinalSettlementCompletionArtifact\|struct ProductionLiveEpochTransitionFinalSettlementCompletionRequest\|struct EmptyLiveEpochTransitionFinalSettlementCompletionReplaySet\|struct LiveEpochTransitionFinalSettlementCompletionFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"


C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 342'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 342 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 342 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-final-settlement-completion-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition final-settlement-completion row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 342 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition final-settlement-completion row must be scoped Green"
...
# The live epoch-transition commit receipt and final-settlement-completion rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "final-settlement-completion row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "final-settlement-completion row must record no public CLI flag"
...
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "final-settlement-completion row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "final-settlement-completion row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition final-settlement-completion active' 'live epoch-transition final-settlement-completion enabled'
  'final-settlement-completion executor active' 'final-settlement-completion executor enabled'
  'live-final-settlement-completion active' 'live-final-settlement-completion enabled' 'final-settlement-completion applied' 'authority-lifecycle-completion applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition final-settlement-completion enabled' 'peer-driven apply enabled'
...
{
  echo "Run 342 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 342 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 341 ProductionLiveEpochTransitionFinalSettlementCompletionExecutor ONLY"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionFinalSettlementCompletionFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionFinalSettlementCompletionExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 342 — release-binary evidence for the Run 341 live epoch-transition final-settlement / authority-lifecycle-completion boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_342_deterministic_digests.txt"
  echo
