#!/usr/bin/env bash
# Run 352 — release-binary evidence for the Run 351 live epoch-transition
# authority-activation post-final-execution confirmation boundary.
#
# Release-binary evidence for the Run 351 source/test live epoch-transition
# authority-activation post-final-execution confirmation boundary
...
# Run 351 production library symbols are present and exercised in release mode,
...
# epoch-transition authority-activation final-execution
# accept decision — is_accept() with Some(authority_activation_final_execution_artifact) —
...
# authority-activation-authorization / authority-activation-post-final-execution-confirmation
...
# authority-activation-post-final-execution-confirmation replay-recovery-idempotency /
...
# final-execution accept decision and produces only a
# typed non-mutating live authority-activation post-final-execution confirmation /
# execution-sink-readiness artifact for a future live production authority-activation
...
#
# Substitution note: the Run 351 executor surfaces every failure as a typed
...
# `ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationError` enum,
# so that symbol is intentionally not required by the reachability greps below.
set -euo pipefail


REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_352_production_live_epoch_transition_authority_activation_post_final_execution_confirmation_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_352_production_live_epoch_transition_authority_activation_post_final_execution_confirmation_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_352"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_post_final_execution_confirmation.rs"
AAEP_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_final_execution.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_352_production_live_epoch_transition_authority_activation_post_final_execution_confirmation_release_binary_helper.rs"

log() { printf '[run-352] %s\n' "$*" >&2; }
fail() { printf '[run-352] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_352_production_live_epoch_transition_authority_activation_post_final_execution_confirmation_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_352_production_live_epoch_transition_authority_activation_post_final_execution_confirmation_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_352.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_352_path:    ${HELPER_BIN}"
  echo "helper_352_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_352_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 352 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_352.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_352.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_352 helper failed"
...
log "running Run 352 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_352_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_352_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_352_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_352 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_352_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_352_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 351 live epoch-transition
# authority-activation post-final-execution confirmation boundary
# has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition authority-activation-post-final-execution-confirmation active|live epoch-transition authority-activation-post-final-execution-confirmation enabled|authority-activation-post-final-execution-confirmation executor active|authority-activation-post-final-execution-confirmation executor enabled|final-execution preflight active|final-execution preflight enabled|ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor enabled|MainNet live epoch-transition authority-activation-post-final-execution-confirmation enabled|authority-activation-post-final-execution-confirmation applied|authority-activation applied|final-execution applied|live epoch-transition authority-activation-authorization active|live epoch-transition authority-activation-authorization enabled|authority-activation-authorization executor active|authority-activation-authorization executor enabled|ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor enabled|authority-activation-authorization applied|authority-lifecycle-completion applied|live mutation applied'
...
log "S1 help hides Run 351 live epoch-transition authority-activation-post-final-execution-confirmation surface (no new CLI flag)"
...
# No Run 352/347 live epoch-transition authority-activation-post-final-execution-confirmation boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-authority-activation-post-final-execution-confirmation|ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmation|pqc_production_live_epoch_transition_authority_activation_post_final_execution_confirmation|authority-activation-post-final-execution-confirmation|final-execution-preflight|final-execution-readiness|authority-activation|final-execution|live-epoch-transition-authority-activation-authorization|ProductionLiveEpochTransitionAuthorityActivationAuthorization|pqc_production_live_epoch_transition_authority_activation_authorization|authority-activation-authorization|post-completion-attestation|activation-readiness|authority-lifecycle-completion|settlement-execution|external-publication|receipt-write|audit-write|run-352|run_352|run-351|run_351'
log "S2..S4 default surfaces silent on live epoch-transition authority-activation-post-final-execution-confirmation boundary claims"
...
log "S5 no live epoch-transition authority-activation-post-final-execution-confirmation CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-authority-activation-post-final-execution-confirmation-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented authority-activation-post-final-execution-confirmation selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on authority-activation-post-final-execution-confirmation claims"
...
# Run 351 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN351_SYMS=(
  ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor
  ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationConfig
  ...
  ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationRequest
  ...
  ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationProtocolVersion
  ...
  evaluate_live_epoch_transition_authority_activation_post_final_execution_confirmation
...
C4C5_PHRASES=(
  ...
  'becomes Green-for-scope only because Run 352 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 352 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# authority-activation-post-final-execution-confirmation row becomes Green-for-scope.
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-final-execution-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-final-execution row must remain scoped Green"
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-post-final-execution-confirmation-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-post-final-execution-confirmation row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 352 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-post-final-execution-confirmation row must be scoped Green"
grep -F -q 'consumes verified live epoch-transition authority-activation final-execution decisions and produces typed non-mutating live authority-activation post-final-execution confirmation / execution-sink-readiness artifacts' "${C4C5_DOC}" || fail "authority-activation-post-final-execution-confirmation row must record authority-activation-final-execution consumption and typed non-mutating live authority-activation post-final-execution confirmation / execution-sink-readiness artifacts"
# The live epoch-transition authority-activation-authorization and
# authority-activation-post-final-execution-confirmation rows must record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "authority-activation-post-final-execution-confirmation row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "authority-activation-post-final-execution-confirmation row must record no public CLI flag"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "authority-activation-post-final-execution-confirmation row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "authority-activation-post-final-execution-confirmation row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition authority-activation-post-final-execution-confirmation active' 'live epoch-transition authority-activation-post-final-execution-confirmation enabled'
  'authority-activation-post-final-execution-confirmation executor active' 'authority-activation-post-final-execution-confirmation executor enabled'
  'final-execution preflight active' 'final-execution preflight enabled' 'authority-activation-post-final-execution-confirmation applied' 'authority-activation applied' 'final-execution applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition authority-activation-post-final-execution-confirmation enabled' 'peer-driven apply enabled'
...
{
  echo "Run 352 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 352 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 351 ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutor ONLY"
  ...
  echo "  authority-activation final-execution accept decision (a ProductionLiveEpochTransitionAuthorityActivationFinalExecutionDecision that is_accept() and"
  echo "  carries Some(authority_activation_final_execution_artifact)), which itself composes the verified Run 347/348 authority-activation-authorization"
  echo "  accept decision and the full Run 303/304 → … → Run 349/350 chain. The Run 351 executor produces ONLY a typed non-mutating"
  echo "  live authority-activation post-final-execution confirmation artifact describing"
  ...
  echo "  no authority-activation post-final-execution confirmation, no final-execution-preflight, no final-execution-readiness, no authority-activation,"
  echo "  no final-execution, no external publication, and no raw local production signing key load. The only mutation any positive path performs is against a"
  echo "  caller-owned in-memory LiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationFixtureState used exclusively as source/test evidence,"
  echo "  which is explicitly distinct from production runtime state. Under a production or MainNet policy the executor fails"
  echo "  closed and never falls back to authority-activation-final-execution-decision-alone / authority-activation-authorization-decision-alone /"
  ...
  echo "  authority-activation-final-execution decisions are rejected as production authority; MainNet identities are refused before acceptance."
  echo "  The default ProductionLiveEpochTransitionAuthorityActivationPostFinalExecutionConfirmationExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 352 — release-binary evidence for the Run 351 live epoch-transition authority-activation post-final-execution confirmation boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_352_deterministic_digests.txt"
  echo
  ...
  echo "c4c5_taxonomy: passed (${#C4C5_PHRASES[@]} phrases; prior boundary rows Green-for-scope only; live-epoch-transition-authority-activation-post-final-execution-confirmation row Green-for-scope; Red rows unchanged; Full C4 OPEN; C5 OPEN)"
  echo "denylist: passed (${#DENY_PATTERNS[@]} patterns)"
  echo "tests:"
  for verdict in "${TEST_VERDICTS[@]}"; do echo "  ${verdict}"; done
  echo
  echo "verdict: PASS (release-binary evidence only; live epoch-transition authority-activation-post-final-execution-confirmation Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN)"
} | tee "${SUMMARY}"