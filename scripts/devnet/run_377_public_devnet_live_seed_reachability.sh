#!/usr/bin/env bash
# Run 377: public DevNet M4 live seed/bootnode reachability PREFLIGHT harness.
#
# This harness produces the Run 377 acceptance evidence for M4 live seed/bootnode
# reachability (see `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_377.md`). It drives the
# RELEASE `target/release/qbind-node` binary directly and records an HONEST,
# BOUNDED verdict about whether M4 can move Yellow -> Green.
#
# DECISION GATE = Route B (Partial-positive). In this sandboxed CI environment
# only LOOPBACK / SAME-HOST reachability can be demonstrated. External
# reachability from OUTSIDE the seed operator's own host/NAT CANNOT be proven
# here, so M4 stays YELLOW. The harness does NOT invent an external vantage
# point and does NOT mark any committed seed entry `status: live`.
#
# What it proves:
...
#
# Run 377 opens NO externally reachable port, deploys NO seed/bootnode/faucet/RPC/
...
# LivePqcTrustState. All addresses are loopback (127.0.0.1) or RFC 5737/5737bis
...
OUTDIR="${1:-/tmp/qbind-run377-public-devnet-live-seed-reachability}"
...
REACH_REF="docs/release/public-devnet/network/reachability/RUN_377_qbind-devnet-seed-1.md"
SUMMARY="${OUTDIR}/summary.txt"

log()  { printf '[run377] %s\n' "$*"; }
fail() { printf '[run377] FAIL: %s\n' "$*" >&2; exit 1; }
...
# ---------------------------------------------------------------------------
# 6. LOOPBACK PREFLIGHT: boot a real release P2P listener; same-host TCP dial.
# ---------------------------------------------------------------------------
...
emit "external_reachability=NOT_PROVEN (no external vantage point in this environment; M4 stays Yellow)"
...
emit ""
emit "RESULT=PARTIAL-POSITIVE (Route B: live admission gate proven + loopback preflight; external reachability NOT proven; M4 stays Yellow; C4/C5 OPEN)"
