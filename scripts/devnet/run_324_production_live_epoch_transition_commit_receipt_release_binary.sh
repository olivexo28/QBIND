#!/usr/bin/env bash
# Run 324 — release-binary evidence for the Run 323 live epoch-transition
# commit receipt boundary.
#
# Release-binary evidence for the Run 323 source/test live epoch-transition
# commit receipt boundary
...
# Run 323 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 321/322 verified
# live epoch-transition commit execution accept decision — is_accept() with
...
# preparation nonce binding + commit-execution-decision-integrity /
# current-validator-set epoch-version preflight / disabled /
# missing-commit-execution-decision / unverified-commit-execution-decision /
...
# commit-receipt replay-recovery-idempotency / stale governance-epoch /
...
# verified live epoch-transition commit execution accept decision and produces only a
# typed non-mutating live-commit receipt artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 323 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionCommitReceiptOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionCommitReceiptError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_324_production_live_epoch_transition_commit_receipt_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_320_BIN="${REPO_ROOT}/target/release/examples/run_324_production_live_epoch_transition_commit_receipt_release_binary_helper"
HELPER_320_OUT="${OUTDIR}/helper_evidence/run_324"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_commit_receipt.rs"
CA_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_commit_execution.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_324_production_live_epoch_transition_commit_receipt_release_binary_helper.rs"

log() { printf '[run-324] %s\n' "$*" >&2; }
fail() { printf '[run-324] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_324_production_live_epoch_transition_commit_receipt_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_324_production_live_epoch_transition_commit_receipt_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_324.log" 2>&1 || fail "helper build failed"
...
log "running Run 324 helper (first invocation)"
set +e
"${HELPER_320_BIN}" "${HELPER_320_OUT}" > "${LOGS_DIR}/helper_run_324.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_324.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_324 helper failed"
...
log "running Run 324 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_324_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_320_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_324_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_324_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_324 helper invocation failed"
if ! diff -q "${HELPER_320_OUT}/fixtures/run_324_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_324_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 323 live epoch-transition
# commit receipt boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition commit receipt active|live epoch-transition commit receipt enabled|commit receipt executor active|commit receipt executor enabled|live-commit receipt active|live-commit receipt enabled|ProductionLiveEpochTransitionCommitReceiptExecutor enabled|MainNet live epoch-transition commit receipt enabled|commit receipt applied|live mutation applied|live epoch-transition commit execution active|live epoch-transition commit execution enabled|commit execution executor active|commit execution executor enabled|live-commit execution active|live-commit execution enabled|ProductionLiveEpochTransitionCommitExecutionExecutor enabled|MainNet live epoch-transition commit execution enabled|commit execution applied'
...
log "S1 help hides Run 323 live epoch-transition commit receipt surface (no new CLI flag)"
...
# No Run 324/319 live epoch-transition commit receipt boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|post-commit-audit|receipt-write|audit-write|live-epoch-transition-commit-execution|ProductionLiveEpochTransitionCommitExecution|pqc_production_live_epoch_transition_commit_execution|commit-execution|live-commit-execution|run-324|run_324|run-323|run_323'
log "S2..S4 default surfaces silent on live epoch-transition commit receipt boundary claims"
...
log "S5 no live epoch-transition commit receipt CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-commit-receipt-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented commit-receipt selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on commit-receipt claims"
...
# Run 323 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN323_SYMS=(
  ProductionLiveEpochTransitionCommitReceiptExecutor
  ProductionLiveEpochTransitionCommitReceiptConfig
  ProductionLiveEpochTransitionCommitReceiptExecutorKind
  ProductionLiveEpochTransitionCommitReceiptExecutorPolicy
  ProductionLiveEpochTransitionCommitReceiptInputs
  ProductionLiveEpochTransitionCommitReceiptRequest
  ...
  ProductionLiveEpochTransitionCommitReceiptProtocolVersion
  LiveEpochTransitionCommitReceiptKind
  LiveEpochTransitionCommitReceiptAuthoritySource
  LiveEpochTransitionCommitReceiptReplaySet
  EmptyLiveEpochTransitionCommitReceiptReplaySet
  LiveEpochTransitionCommitReceiptFixtureState
  evaluate_live_epoch_transition_commit_receipt
  recover_live_epoch_transition_commit_receipt_window
  production_live_epoch_transition_commit_receipt_content_digest
  production_live_epoch_transition_commit_receipt_request_id
  production_live_epoch_transition_commit_receipt_id
  production_live_epoch_transition_commit_receipt_transcript_digest
)
COMBINED_CORPUS="${REACH_DIR}/combined_corpus.txt"
cat "${MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 324 combined reachability — Run 323 live epoch-transition commit receipt boundary symbols across source module + Run 321/322 commit execution module + Run 313/314 epoch-transition runtime handoff module + Run 311/312 guarded mutation executor module + Run 309/310 staged application executor module + Run 307/308 live validator-set application authorization module + Run 305/306 validator-set rotation application executor module + Run 303/304 validator-set rotation intent module + release helper:"
  for sym in "${RUN323_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN323_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 324 source reachability — Run 323 live epoch-transition commit receipt boundary symbols in ${MOD}:"
  for sym in "${RUN323_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptConfig ProductionLiveEpochTransitionCommitReceiptExecutorKind ProductionLiveEpochTransitionCommitReceiptExecutorPolicy ProductionLiveEpochTransitionCommitReceiptInputs ProductionLiveEpochTransitionCommitReceiptRequest ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitReceiptArtifact ProductionLiveEpochTransitionCommitReceiptOutcome ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome ProductionLiveEpochTransitionCommitReceiptProtocolVersion LiveEpochTransitionCommitReceiptKind LiveEpochTransitionCommitReceiptAuthoritySource LiveEpochTransitionCommitReceiptReplaySet EmptyLiveEpochTransitionCommitReceiptReplaySet LiveEpochTransitionCommitReceiptFixtureState evaluate_live_epoch_transition_commit_receipt recover_live_epoch_transition_commit_receipt_window production_live_epoch_transition_commit_receipt_content_digest production_live_epoch_transition_commit_receipt_request_id production_live_epoch_transition_commit_receipt_id production_live_epoch_transition_commit_receipt_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 324 helper reachability — Run 323 symbols exercised by the release helper (plus the Run 321/322 commit execution + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN323_SYMS[@]}" pqc_production_live_epoch_transition_commit_execution ProductionLiveEpochTransitionCommitExecutionExecutor ProductionLiveEpochTransitionCommitExecutionDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptConfig ProductionLiveEpochTransitionCommitReceiptExecutorPolicy ProductionLiveEpochTransitionCommitReceiptInputs ProductionLiveEpochTransitionCommitReceiptRequest ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitReceiptArtifact ProductionLiveEpochTransitionCommitReceiptOutcome ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome LiveEpochTransitionCommitReceiptKind LiveEpochTransitionCommitReceiptAuthoritySource EmptyLiveEpochTransitionCommitReceiptReplaySet LiveEpochTransitionCommitReceiptFixtureState evaluate_live_epoch_transition_commit_receipt recover_live_epoch_transition_commit_receipt_window production_live_epoch_transition_commit_receipt_content_digest production_live_epoch_transition_commit_receipt_request_id production_live_epoch_transition_commit_receipt_id production_live_epoch_transition_commit_receipt_transcript_digest ProductionLiveEpochTransitionCommitExecutionExecutor ProductionLiveEpochTransitionCommitExecutionDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_commit_receipt' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_commit_receipt\|fn recover_live_epoch_transition_commit_receipt_window\|fn production_live_epoch_transition_commit_receipt_content_digest\|fn production_live_epoch_transition_commit_receipt_request_id\|fn production_live_epoch_transition_commit_receipt_id\|fn production_live_epoch_transition_commit_receipt_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionCommitReceiptOutcome\|enum ProductionLiveEpochTransitionCommitReceiptRecoveryOutcome\|enum ProductionLiveEpochTransitionCommitReceiptExecutorPolicy\|enum ProductionLiveEpochTransitionCommitReceiptExecutorKind\|enum LiveEpochTransitionCommitReceiptKind\|enum LiveEpochTransitionCommitReceiptAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionCommitReceiptReplaySet\|struct ProductionLiveEpochTransitionCommitReceiptExecutor\|struct ProductionLiveEpochTransitionCommitReceiptArtifact\|struct ProductionLiveEpochTransitionCommitReceiptRequest\|struct EmptyLiveEpochTransitionCommitReceiptReplaySet\|struct LiveEpochTransitionCommitReceiptFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 324'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 324 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 324 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# commit execution row remains Green-for-scope and the commit receipt row
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-commit-execution-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition commit execution row must remain scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 324 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition commit receipt row must be scoped Green"
# The live epoch-transition commit execution and commit receipt rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "commit receipt row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "commit receipt row must record no public CLI flag"
grep -F -q 'produces typed non-mutating live-mutation commit-execution artifacts for a future mutating run' "${C4C5_DOC}" || fail "commit execution row must record typed non-mutating live-mutation commit-execution artifacts"
grep -F -q 'consumes verified live epoch-transition commit-execution decisions and produces typed non-mutating live commit-receipt / audit-preparation artifacts for a future production receipt/audit run' "${C4C5_DOC}" || fail "commit receipt row must record commit-execution consumption and typed non-mutating live commit-receipt / audit-preparation artifacts"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "commit receipt row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "commit receipt row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition commit receipt enabled' 'peer-driven apply enabled'
...
{
  echo "Run 324 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 324 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 323 ProductionLiveEpochTransitionCommitReceiptExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 321/322 live epoch-transition"
  echo "  commit execution accept decision (a ProductionLiveEpochTransitionCommitExecutionDecision that is_accept() and"
  echo "  carries Some(preparation_artifact)), which itself composes the verified Run 313/314 epoch-transition runtime handoff"
  echo "  accept decision. The Run 323 executor produces ONLY a typed non-mutating live-commit receipt artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionCommitReceiptFixtureState used exclusively as source/test evidence,"
  echo "  which is explicitly distinct from production runtime state. Under a production or MainNet policy the executor fails"
  echo "  closed and never falls back to commit-execution-decision-alone / runtime-handoff-decision-alone /"
  ...
  echo "  commit-execution decisions are rejected as production authority; MainNet identities are refused before acceptance."
  echo "  The default ProductionLiveEpochTransitionCommitReceiptExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 324 — release-binary evidence for the Run 323 live epoch-transition commit receipt boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_320_OUT}/fixtures/run_324_deterministic_digests.txt"
  echo
  ...
  echo "c4c5_taxonomy: passed (${#C4C5_PHRASES[@]} phrases; RocksDB + RemoteSigner + KMS/HSM + custody-attestation + on-chain-governance-proof-verifier + governance-execution-engine + validator-set-rotation-intent-boundary + validator-set-rotation-application-executor-boundary + live-validator-set-application-authorization-boundary + staged-live-validator-set-epoch-transition-application-executor-boundary + guarded-epoch-transition-mutation-executor-boundary + epoch-transition-runtime-handoff-boundary + live-epoch-transition-commit-execution-boundary + live-epoch-transition-commit-receipt-boundary rows Green-for-scope only; Red rows unchanged; Full C4 OPEN; C5 OPEN)"
  echo "denylist: passed (${#DENY_PATTERNS[@]} patterns)"
  echo "tests:"
  for verdict in "${TEST_VERDICTS[@]}"; do echo "  ${verdict}"; done
  echo
  echo "verdict: PASS (release-binary evidence only; live epoch-transition commit receipt Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN)"
} | tee "${SUMMARY}"