#!/usr/bin/env bash
# Run 189 — Release-binary KMS/HSM authority-custody boundary evidence on
# real `target/release/qbind-node`. Closes the Run 188-deferred release-
# binary boundary for the typed source/test authority-custody surface
# added by `crates/qbind-node/src/pqc_authority_custody.rs`.
#
...
#     claim. Run 188 added no operator-visible selector, so the
#     release-binary boundary for Run 189 mirrors that surface contract;
...
#     Run 188 custody enablement claim;
...
#     exercises the Run 188 A1–A8 / R1–R29 corpus end-to-end in
...
#
# Strict scope (from `task/RUN_189_TASK.txt`):
#   * Release-binary evidence only.
#   * Use real `target/release/qbind-node`.
#   * Use the release-built Run 189 helper to exercise the Run 188
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_189_authority_custody_boundary_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_189_BIN="${REPO_ROOT}/target/release/examples/run_189_authority_custody_boundary_release_binary_helper"

HELPER_189_OUT="${OUTDIR}/helper_evidence/run_189"
...
log()  { printf '[run-189] %s\n' "$*" >&2; }
fail() { printf '[run-189] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "OUTDIR=${OUTDIR}"
mkdir -p "${OUTDIR}"
rm -rf "${HELPER_189_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" \
       "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${SCEN_DIR}" \
       "${DATA_DIR}"
mkdir -p "${HELPER_189_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" \
         "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${SCEN_DIR}" \
         "${DATA_DIR}"
: > "${PROVENANCE}"
: > "${DENYLIST}"
...
# ---------------------------------------------------------------------------
# Build qbind-node bin + Run 189 helper in release mode.
# ---------------------------------------------------------------------------
...
log "cargo build --release -p qbind-node --example run_189_authority_custody_boundary_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node \
    --example run_189_authority_custody_boundary_release_binary_helper ) \
  > "${LOGS_DIR}/build_helper_run_189.log" 2>&1 \
  || fail "release build of run_189 helper failed (see ${LOGS_DIR}/build_helper_run_189.log)"

[[ -x "${NODE_BIN}"       ]] || fail "missing ${NODE_BIN}"
[[ -x "${HELPER_189_BIN}" ]] || fail "missing ${HELPER_189_BIN}"

{
  echo "qbind_node_path:    ${NODE_BIN}"
  echo "qbind_node_sha256:  $(sha256_file "${NODE_BIN}")"
  echo "qbind_node_buildid: $(build_id "${NODE_BIN}")"
  echo "helper_189_path:    ${HELPER_189_BIN}"
  echo "helper_189_sha256:  $(sha256_file "${HELPER_189_BIN}")"
  echo "helper_189_buildid: $(build_id "${HELPER_189_BIN}")"
} >> "${PROVENANCE}"

# ---------------------------------------------------------------------------
# Drive the Run 189 release helper. Exits 0 iff every Run 188 A1..A8 /
# R1..R29 scenario, every combined-helper scenario, every custody-class
...
# release mode through the production library symbols.
# ---------------------------------------------------------------------------
log "running Run 189 custody-boundary release helper -> ${HELPER_189_OUT}"
HELPER_189_LOG="${LOGS_DIR}/helper_run_189.log"
set +e
"${HELPER_189_BIN}" "${HELPER_189_OUT}" > "${HELPER_189_LOG}" 2>&1
HELPER_189_RC=$?
set -e
echo "${HELPER_189_RC}" > "${EXIT_DIR}/helper_run_189.rc"
[[ "${HELPER_189_RC}" -eq 0 ]] || fail "run_189 helper exited rc=${HELPER_189_RC} (see ${HELPER_189_LOG})"
[[ -s "${HELPER_189_OUT}/helper_summary.txt" ]] || fail "run_189 helper did not write helper_summary.txt"
assert_grep "${HELPER_189_OUT}/helper_summary.txt" "verdict: PASS"

# ---------------------------------------------------------------------------
# Real-binary surface invariants — Run 188 added no operator-visible
# selector, so the release-binary surface contract for Run 189 is that
...
# ---------------------------------------------------------------------------
log "S1 — qbind-node --help surfaces no Run 188 custody flag / claim"
...
assert_not_grep "${HELP_LOG}" "run-189"
...
# ---------------------------------------------------------------------------
# Source/release reachability proof for the Run 188 typed authority-
...
# the Run 189 helper exercises is wired in production source — not in
# tests or fixtures.
# ---------------------------------------------------------------------------
log "writing source-reachability proof to ${REACH_DIR}/source_reachability.txt"
SRC_DIR="${REPO_ROOT}/crates/qbind-node/src"
{
  echo "Run 189 source-reachability proof — production callers within ${SRC_DIR}:"
  echo
  for sym in \
    'pqc_authority_custody' \
...
{
  echo "Run 189 denylist (proven empty across all captured logs):"
  for pat in \
    'apply on receipt' \
    'apply-on-receipt' \
    'autonomous apply' \
  ...
  do
    if find "${LOGS_DIR}" "${HELPER_189_OUT}" -type f ! -name qbind_node_help.log ! -name helper_summary.txt -print0 2>/dev/null \
...
{
  echo "Run 189 no-mutation proof for rejected custody-boundary scenarios:"
  echo "  data dir at ${DATA_DIR} contents (must be empty):"
  ls -la "${DATA_DIR}" 2>/dev/null || true
  echo
  echo "  helper-driven Run 188 custody-boundary rejection corpus (R1..R29):"
  ...
  grep -E 'verdict: PASS|R[0-9]+_|A[0-9]+_|no_mutation_pass|determinism_pass' \
    "${HELPER_189_OUT}/helper_summary.txt" 2>/dev/null \
    | sed 's/^/    /' || true
} > "${NOMUT_PROOF}"
...
# ---------------------------------------------------------------------------
{
  echo "Run 189 mutation proof (release-binary scope):"
  echo
  echo "  binary-side production-call-site Run 188 custody-boundary"
  echo "  reachability today:"
  echo "    - Run 188 added pqc_authority_custody.rs with the typed"
  echo "      AuthorityCustodyClass (FixtureLocalKey / LocalOperatorKey /"
  ...
  echo "    - Run 188 added no operator-visible selector, so the"
  echo "      release-binary surface contract preserves Run 187's binary"
  echo "      defaults bit-identically — the only new release-binary"
  echo "      reachability for Run 189 is the typed library boundary"
  echo "      exercised by the Run 189 release-built helper through"
  echo "      pqc_authority_custody::*;"
  echo "    - the Run 188 custody validator is wired source-side as a pure"
  ...
  echo "      refusal is layered ahead of the Run 188 policy gate via"
  ...
  echo "    - the Run 189 helper exercises every AuthorityCustodyPolicy"
  ...
  echo "      library symbols, across the A1..A8 acceptance corpus and"
  echo "      the R1..R29 rejection corpus from Run 188;"
  ...
  echo "    - real target/release/qbind-node --help surfaces no Run 188"
  echo "      custody flag and no KMS/HSM/remote-signer claim;"
  echo "    - real target/release/qbind-node --print-genesis-hash --env"
  echo "      {devnet,testnet,mainnet} emits no Run 188 custody enablement"
  ...
  echo "      backend is wired in Run 189. Every RemoteSigner / Kms / Hsm"
...
# ---------------------------------------------------------------------------
# Targeted cargo test cross-checks. Mirrors `task/RUN_189_TASK.txt
...
{
  echo "Run 189 — release-binary KMS/HSM authority-custody boundary evidence"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo 'unknown')"
  echo
  echo "build:"
  echo "  rustc_version:      $(rustc --version 2>/dev/null || echo 'unknown')"
  ...
  echo "  helper_run_189	rc=$(cat "${EXIT_DIR}/helper_run_189.rc" 2>/dev/null || echo 'na')	$(grep -E 'verdict:' "${HELPER_189_OUT}/helper_summary.txt" 2>/dev/null | head -n1 || true)"
  echo
  ...
  echo "  * no schema/wire/metric drift (Run 189 is release-binary"
  echo "    evidence only);"
  ...
  echo
  echo "verdict:"
  echo "  positive: real target/release/qbind-node preserves the Run 188"
  ...
  echo "  No Run 188 selector is exposed at the binary surface (no new"
  ...
  echo "  KMS/HSM/remote-signer backend. The Run 189 release-built helper"
  echo "  exercises the full Run 188 A1-A8 / R1-R29 corpus end-to-end"
