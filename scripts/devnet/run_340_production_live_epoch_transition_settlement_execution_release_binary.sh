#!/usr/bin/env bash
# Run 340 — release-binary evidence for the Run 339 live epoch-transition
# settlement-execution boundary.
#
# Release-binary evidence for the Run 339 source/test live epoch-transition
# settlement-execution boundary
...
# Run 339 production library symbols are present and exercised in release mode,
...
# settlement-execution-preparation replay-recovery-idempotency / stale governance-epoch /
...
# typed non-mutating live-settlement-execution artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 339 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionSettlementExecutionPreparationError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_340_production_live_epoch_transition_settlement_execution_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_340_production_live_epoch_transition_settlement_execution_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_340"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_settlement_execution.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_340_production_live_epoch_transition_settlement_execution_release_binary_helper.rs"

log() { printf '[run-340] %s\n' "$*" >&2; }
fail() { printf '[run-340] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_340_production_live_epoch_transition_settlement_execution_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_340_production_live_epoch_transition_settlement_execution_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_340.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_340_path:    ${HELPER_BIN}"
  echo "helper_340_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_340_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 340 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_340.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_340.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_340 helper failed"
...
log "running Run 340 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_340_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_340_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_340_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_340 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_340_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_340_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 339 live epoch-transition
# settlement-execution boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition settlement-execution-preparation active|live epoch-transition settlement-execution-preparation enabled|settlement-execution-preparation executor active|settlement-execution-preparation executor enabled|live-settlement-execution-preparation active|live-settlement-execution-preparation enabled|ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor enabled|MainNet live epoch-transition settlement-execution-preparation enabled|settlement-execution-preparation applied|live mutation applied|live epoch-transition external-publication active|live epoch-transition external-publication enabled|external-publication executor active|external-publication executor enabled|ProductionLiveEpochTransitionExternalPublicationExecutor enabled|external-publication applied|live epoch-transition commit receipt active|live epoch-transition commit receipt enabled|commit receipt executor active|commit receipt executor enabled|live-commit receipt active|live-commit receipt enabled|ProductionLiveEpochTransitionCommitReceiptExecutor enabled|MainNet live epoch-transition commit receipt enabled|commit receipt applied'
...
log "S1 help hides Run 339 live epoch-transition settlement-execution surface (no new CLI flag)"
...
# No Run 340/339 live epoch-transition settlement-execution boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-settlement-execution-preparation|ProductionLiveEpochTransitionSettlementExecutionPreparation|pqc_production_live_epoch_transition_settlement_execution_preparation|settlement-execution-preparation|live-settlement-execution-preparation|live-epoch-transition-external-publication|ProductionLiveEpochTransitionExternalPublication|pqc_production_live_epoch_transition_external_publication|external-publication|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-340|run_340|run-329|run_329'
...
log "S5 no live epoch-transition settlement-execution CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-settlement-execution-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented settlement-execution selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on settlement-execution claims"
...
# Run 339 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN339_SYMS=(
  ProductionLiveEpochTransitionSettlementExecutionExecutor
  ProductionLiveEpochTransitionSettlementExecutionConfig
  ProductionLiveEpochTransitionSettlementExecutionExecutorKind
  ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy
  ProductionLiveEpochTransitionSettlementExecutionInputs
  ProductionLiveEpochTransitionSettlementExecutionRequest
  ...
  ProductionLiveEpochTransitionSettlementExecutionProtocolVersion
  LiveEpochTransitionSettlementExecutionKind
  LiveEpochTransitionSettlementExecutionAuthoritySource
  LiveEpochTransitionSettlementExecutionReplaySet
  ...
  recover_live_epoch_transition_settlement_execution_window
  production_live_epoch_transition_settlement_execution_content_digest
  production_live_epoch_transition_settlement_execution_request_id
  production_live_epoch_transition_settlement_execution_id
  production_live_epoch_transition_settlement_execution_transcript_digest
)
COMBINED_CORPUS="${REACH_DIR}/combined_corpus.txt"
cat "${MOD}" "${SETTLE_EXEC_PREP_MOD}" "${SETTLE_PREP_MOD}" "${PCA_MOD}" "${CA_MOD}" "${CAUTH_MOD}" "${MUT_MOD}" "${PREP_MOD}" "${HANDOFF_MOD}" "${GUARDED_MOD}" "${STAGED_MOD}" "${AUTHORIZATION_MOD}" "${APPLICATION_MOD}" "${ROTATION_MOD}" "${HELPER_SRC}" > "${COMBINED_CORPUS}"
{
  echo "Run 340 combined reachability — Run 339 live epoch-transition settlement-execution boundary symbols across source module + Run 337/338 settlement-execution-preparation module + ancestor chain modules + release helper:"
  for sym in "${RUN339_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${COMBINED_CORPUS}" | head -n 20 || echo '(no occurrences)'; echo; done
} > "${REACH_DIR}/combined_reachability.txt"
for sym in "${RUN339_SYMS[@]}"; do assert_grep "${COMBINED_CORPUS}" "$sym"; done
{
  echo "Run 340 source reachability — Run 339 live epoch-transition settlement-execution boundary symbols in ${MOD}:"
  for sym in "${RUN339_SYMS[@]}"; do echo "=== symbol: ${sym} ==="; grep -RIn --include='*.rs' "$sym" "${MOD}" || echo '(no occurrences in production module)'; echo; done
} > "${REACH_DIR}/source_reachability.txt"
# Symbols expected to be defined/referenced directly in the production module.
for sym in ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionConfig ProductionLiveEpochTransitionSettlementExecutionExecutorKind ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy ProductionLiveEpochTransitionSettlementExecutionInputs ProductionLiveEpochTransitionSettlementExecutionRequest ProductionLiveEpochTransitionSettlementExecutionDecision ProductionLiveEpochTransitionSettlementExecutionArtifact ProductionLiveEpochTransitionSettlementExecutionOutcome ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome ProductionLiveEpochTransitionSettlementExecutionProtocolVersion LiveEpochTransitionSettlementExecutionKind LiveEpochTransitionSettlementExecutionAuthoritySource LiveEpochTransitionSettlementExecutionReplaySet EmptyLiveEpochTransitionSettlementExecutionReplaySet LiveEpochTransitionSettlementExecutionFixtureState evaluate_live_epoch_transition_settlement_execution recover_live_epoch_transition_settlement_execution_window production_live_epoch_transition_settlement_execution_content_digest production_live_epoch_transition_settlement_execution_request_id production_live_epoch_transition_settlement_execution_id production_live_epoch_transition_settlement_execution_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 340 helper reachability — Run 339 symbols exercised by the release helper (plus the Run 339/324 commit receipt + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN339_SYMS[@]}" pqc_production_live_epoch_transition_settlement_execution_preparation ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision pqc_production_live_epoch_transition_settlement_preparation ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision pqc_production_live_epoch_transition_external_publication ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision pqc_production_live_epoch_transition_commit_receipt ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionSettlementExecutionExecutor ProductionLiveEpochTransitionSettlementExecutionConfig ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy ProductionLiveEpochTransitionSettlementExecutionInputs ProductionLiveEpochTransitionSettlementExecutionRequest ProductionLiveEpochTransitionSettlementExecutionDecision ProductionLiveEpochTransitionSettlementExecutionArtifact ProductionLiveEpochTransitionSettlementExecutionOutcome ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome LiveEpochTransitionSettlementExecutionKind LiveEpochTransitionSettlementExecutionAuthoritySource EmptyLiveEpochTransitionSettlementExecutionReplaySet LiveEpochTransitionSettlementExecutionFixtureState evaluate_live_epoch_transition_settlement_execution recover_live_epoch_transition_settlement_execution_window production_live_epoch_transition_settlement_execution_content_digest production_live_epoch_transition_settlement_execution_request_id production_live_epoch_transition_settlement_execution_id production_live_epoch_transition_settlement_execution_transcript_digest ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ProductionLiveEpochTransitionSettlementExecutionPreparationDecision ProductionLiveEpochTransitionSettlementPreparationExecutor ProductionLiveEpochTransitionSettlementPreparationDecision ProductionLiveEpochTransitionExternalPublicationExecutor ProductionLiveEpochTransitionExternalPublicationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_settlement_execution' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_settlement_execution\|fn recover_live_epoch_transition_settlement_execution_window\|fn production_live_epoch_transition_settlement_execution_content_digest\|fn production_live_epoch_transition_settlement_execution_request_id\|fn production_live_epoch_transition_settlement_execution_id\|fn production_live_epoch_transition_settlement_execution_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionSettlementExecutionOutcome\|enum ProductionLiveEpochTransitionSettlementExecutionRecoveryOutcome\|enum ProductionLiveEpochTransitionSettlementExecutionExecutorPolicy\|enum ProductionLiveEpochTransitionSettlementExecutionExecutorKind\|enum LiveEpochTransitionSettlementExecutionKind\|enum LiveEpochTransitionSettlementExecutionAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionSettlementExecutionReplaySet\|struct ProductionLiveEpochTransitionSettlementExecutionExecutor\|struct ProductionLiveEpochTransitionSettlementExecutionArtifact\|struct ProductionLiveEpochTransitionSettlementExecutionRequest\|struct EmptyLiveEpochTransitionSettlementExecutionReplaySet\|struct LiveEpochTransitionSettlementExecutionFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 340'
  'boundary readiness'
  ...
  'becomes Green-for-scope only because Run 340 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 340 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-settlement-execution-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition settlement-execution row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 340 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition settlement-execution row must be scoped Green"
grep -F -q 'consumes verified live epoch-transition settlement / finalization execution-preparation decisions and produces typed non-mutating live settlement / finalization execution artifacts' "${C4C5_DOC}" || fail "settlement-execution row must record settlement-execution-preparation consumption and typed non-mutating live settlement / finalization execution artifacts"
# The live epoch-transition commit receipt and settlement-execution-preparation rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "settlement-execution-preparation row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "settlement-execution-preparation row must record no public CLI flag"
...
grep -F -q 'consumes verified live epoch-transition settlement / finalization-preparation decisions and produces typed non-mutating live settlement / finalization execution-preparation artifacts for a future production settlement-finalization or authority-lifecycle final execution run' "${C4C5_DOC}" || fail "settlement-execution-preparation row must record settlement-preparation consumption and typed non-mutating live settlement / finalization execution-preparation artifacts"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "settlement-execution-preparation row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "settlement-execution-preparation row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition settlement-execution-preparation active' 'live epoch-transition settlement-execution-preparation enabled'
  'settlement-execution-preparation executor active' 'settlement-execution-preparation executor enabled'
  'live-settlement-execution-preparation active' 'live-settlement-execution-preparation enabled' 'settlement-execution-preparation applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition settlement-execution-preparation enabled' 'peer-driven apply enabled'
...
{
  echo "Run 340 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 340 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 339 ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 339/324 live epoch-transition"
  ...
  echo "  accept decision. The Run 339 executor produces ONLY a typed non-mutating live-settlement-execution artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionSettlementExecutionPreparationFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionSettlementExecutionPreparationExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 340 — release-binary evidence for the Run 339 live epoch-transition settlement-execution boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_340_deterministic_digests.txt"
  echo
