#!/usr/bin/env bash
# Run 326 — release-binary evidence for the Run 325 live epoch-transition
# post-commit audit boundary.
#
# Release-binary evidence for the Run 325 source/test live epoch-transition
# post-commit audit boundary
...
# Run 325 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 325/324 verified
...
# post-commit-audit replay-recovery-idempotency / stale governance-epoch /
...
# typed non-mutating live-post-commit audit artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 325 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionPostCommitAuditOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionPostCommitAuditError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_326_production_live_epoch_transition_post_commit_audit_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_326_production_live_epoch_transition_post_commit_audit_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_326"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_post_commit_audit.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_326_production_live_epoch_transition_post_commit_audit_release_binary_helper.rs"

log() { printf '[run-326] %s\n' "$*" >&2; }
fail() { printf '[run-326] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_326_production_live_epoch_transition_post_commit_audit_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_326_production_live_epoch_transition_post_commit_audit_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_326.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_320_path:    ${HELPER_BIN}"
  echo "helper_320_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_320_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 326 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_326.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_326.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_326 helper failed"
...
log "running Run 326 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_326_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_326_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_326_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_326 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_326_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_326_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 325 live epoch-transition
# post-commit audit boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition post-commit audit active|live epoch-transition post-commit audit enabled|post-commit audit executor active|post-commit audit executor enabled|live-post-commit audit active|live-post-commit audit enabled|ProductionLiveEpochTransitionPostCommitAuditExecutor enabled|MainNet live epoch-transition post-commit audit enabled|post-commit audit applied|live mutation applied|live epoch-transition commit receipt active|live epoch-transition commit receipt enabled|commit receipt executor active|commit receipt executor enabled|live-commit receipt active|live-commit receipt enabled|ProductionLiveEpochTransitionCommitReceiptExecutor enabled|MainNet live epoch-transition commit receipt enabled|commit receipt applied'
...
log "S1 help hides Run 325 live epoch-transition post-commit audit surface (no new CLI flag)"
...
# No Run 326/319 live epoch-transition post-commit audit boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-post-commit-audit|ProductionLiveEpochTransitionPostCommitAudit|pqc_production_live_epoch_transition_post_commit_audit|post-commit-audit|live-post-commit-audit|post-commit-audit|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-326|run_326|run-325|run_325'
log "S2..S4 default surfaces silent on live epoch-transition post-commit audit boundary claims"
...
log "S5 no live epoch-transition post-commit audit CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-post-commit-audit-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented post-commit-audit selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on post-commit-audit claims"
...
# Run 325 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN325_SYMS=(
  ProductionLiveEpochTransitionPostCommitAuditExecutor
  ProductionLiveEpochTransitionPostCommitAuditConfig
  ...
  ProductionLiveEpochTransitionPostCommitAuditProtocolVersion
  LiveEpochTransitionPostCommitAuditKind
  LiveEpochTransitionPostCommitAuditAuthoritySource
  LiveEpochTransitionPostCommitAuditReplaySet
  EmptyLiveEpochTransitionPostCommitAuditReplaySet
  LiveEpochTransitionPostCommitAuditFixtureState
  evaluate_live_epoch_transition_post_commit_audit
  recover_live_epoch_transition_post_commit_audit_window
  production_live_epoch_transition_post_commit_audit_content_digest
  production_live_epoch_transition_post_commit_audit_request_id
  production_live_epoch_transition_post_commit_audit_id
...
C4C5_PHRASES=(
  ...
  'Live epoch-transition post-commit audit'
  'becomes Green-for-scope only because Run 326 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 326 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# commit receipt row remains Green-for-scope and the post-commit audit row
...
grep -F -q 'becomes Green-for-scope only because Run 326 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition post-commit audit row must be scoped Green"
# The live epoch-transition commit receipt and post-commit audit rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "post-commit audit row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "post-commit audit row must record no public CLI flag"
...
grep -F -q 'consumes verified live epoch-transition commit-receipt decisions and produces typed non-mutating live post-commit-audit / audit-preparation artifacts for a future production receipt/audit run' "${C4C5_DOC}" || fail "post-commit audit row must record commit-receipt consumption and typed non-mutating live post-commit-audit / audit-preparation artifacts"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "post-commit audit row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "post-commit audit row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'post-commit audit executor active' 'post-commit audit executor enabled'
  'live-post-commit audit active' 'live-post-commit audit enabled' 'post-commit audit applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition post-commit audit enabled' 'peer-driven apply enabled'
...
{
  echo "Run 326 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 326 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 325 ProductionLiveEpochTransitionPostCommitAuditExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 325/324 live epoch-transition"
  ...
  echo "  accept decision. The Run 325 executor produces ONLY a typed non-mutating live-post-commit audit artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionPostCommitAuditFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionPostCommitAuditExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 326 — release-binary evidence for the Run 325 live epoch-transition post-commit audit boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_326_deterministic_digests.txt"
  echo
