#!/usr/bin/env bash
# Run 350 — release-binary evidence for the Run 349 live epoch-transition
# authority-activation final-execution boundary.
#
# Release-binary evidence for the Run 349 source/test live epoch-transition
# authority-activation final-execution boundary
...
# Run 349 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 347/348 verified live
...
# Run 341/342 → Run 343/344 → Run 347/348 accept chain; full
# authority-activation-authorization / authority-activation-final-execution
...
# authority-activation-final-execution replay-recovery-idempotency /
...
# typed non-mutating live authority-activation final-execution /
...
#
# Substitution note: the Run 349 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionAuthorityActivationFinalExecutionOutcome`
# fail-closed variant; there is no separate
# `ProductionLiveEpochTransitionAuthorityActivationFinalExecutionError` enum,
# so that symbol is intentionally not required by the reachability greps below.
set -euo pipefail


REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_350"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_final_execution.rs"
AAEP_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_authority_activation_execution_preparation.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary_helper.rs"

log() { printf '[run-350] %s\n' "$*" >&2; }
fail() { printf '[run-350] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_350_production_live_epoch_transition_authority_activation_final_execution_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_350.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_350_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_348_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 350 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_350.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_350.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_350 helper failed"
...
log "running Run 350 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_350_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_350_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_350_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_350 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_350_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_350_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 349 live epoch-transition
# authority-activation final-execution boundary
# has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition authority-activation-final-execution active|live epoch-transition authority-activation-final-execution enabled|authority-activation-final-execution executor active|authority-activation-final-execution executor enabled|final-execution preflight active|final-execution preflight enabled|ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor enabled|MainNet live epoch-transition authority-activation-final-execution enabled|authority-activation-final-execution applied|authority-activation applied|final-execution applied|live epoch-transition authority-activation-authorization active|live epoch-transition authority-activation-authorization enabled|authority-activation-authorization executor active|authority-activation-authorization executor enabled|ProductionLiveEpochTransitionAuthorityActivationAuthorizationExecutor enabled|authority-activation-authorization applied|authority-lifecycle-completion applied|live mutation applied'
...
log "S1 help hides Run 349 live epoch-transition authority-activation-final-execution surface (no new CLI flag)"
...
# No Run 350/347 live epoch-transition authority-activation-final-execution boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-authority-activation-final-execution|ProductionLiveEpochTransitionAuthorityActivationFinalExecution|pqc_production_live_epoch_transition_authority_activation_final_execution|authority-activation-final-execution|final-execution-preflight|final-execution-readiness|authority-activation|final-execution|live-epoch-transition-authority-activation-authorization|ProductionLiveEpochTransitionAuthorityActivationAuthorization|pqc_production_live_epoch_transition_authority_activation_authorization|authority-activation-authorization|post-completion-attestation|activation-readiness|authority-lifecycle-completion|settlement-execution|external-publication|receipt-write|audit-write|run-350|run_350|run-349|run_349'
log "S2..S4 default surfaces silent on live epoch-transition authority-activation-final-execution boundary claims"
...
log "S5 no live epoch-transition authority-activation-final-execution CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-authority-activation-final-execution-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented authority-activation-final-execution selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on authority-activation-final-execution claims"
...
# Run 349 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN349_SYMS=(
  ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor
  ProductionLiveEpochTransitionAuthorityActivationFinalExecutionConfig
  ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorKind
  ...
  ProductionLiveEpochTransitionAuthorityActivationFinalExecutionRequest
  ...
  production_live_epoch_transition_authority_activation_final_execution_content_digest
...
C4C5_PHRASES=(
  ...
  'becomes Green-for-scope only because Run 350 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 350 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# authority-activation-final-execution row becomes Green-for-scope.
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-authority-activation-final-execution-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-final-execution row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 350 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition authority-activation-final-execution row must be scoped Green"
grep -F -q 'consumes verified live epoch-transition authority-activation execution-preparation / final-execution preflight decisions and produces typed non-mutating live authority-activation final-execution artifacts' "${C4C5_DOC}" || fail "authority-activation-final-execution row must record authority-activation-authorization consumption and typed non-mutating live authority-activation final-execution artifacts"
# The live epoch-transition authority-activation-authorization and
# authority-activation-final-execution rows must record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "authority-activation-final-execution row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "authority-activation-final-execution row must record no public CLI flag"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "authority-activation-final-execution row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "authority-activation-final-execution row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'live epoch-transition authority-activation-final-execution active' 'live epoch-transition authority-activation-final-execution enabled'
  'authority-activation-final-execution executor active' 'authority-activation-final-execution executor enabled'
  'final-execution preflight active' 'final-execution preflight enabled' 'authority-activation-final-execution applied' 'authority-activation applied' 'final-execution applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition authority-activation-final-execution enabled' 'peer-driven apply enabled'
...
{
  echo "Run 350 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 350 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 349 ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 347/348 live epoch-transition"
  ...
  echo "  accept decision and the full Run 303/304 → … → Run 347/348 chain. The Run 349 executor produces ONLY a typed non-mutating"
  echo "  live authority-activation final-execution artifact describing"
  ...
  echo "  no authority-activation final-execution, no final-execution-preflight, no final-execution-readiness, no authority-activation,"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionAuthorityActivationFinalExecutionFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionAuthorityActivationFinalExecutionExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 350 — release-binary evidence for the Run 349 live epoch-transition authority-activation final-execution boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_350_deterministic_digests.txt"
  echo
  ...
  echo "c4c5_taxonomy: passed (${#C4C5_PHRASES[@]} phrases; prior boundary rows Green-for-scope only; live-epoch-transition-authority-activation-final-execution row Green-for-scope; Red rows unchanged; Full C4 OPEN; C5 OPEN)"
  echo "denylist: passed (${#DENY_PATTERNS[@]} patterns)"
  echo "tests:"
  for verdict in "${TEST_VERDICTS[@]}"; do echo "  ${verdict}"; done
  echo
  echo "verdict: PASS (release-binary evidence only; live epoch-transition authority-activation-final-execution Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN)"
} | tee "${SUMMARY}"