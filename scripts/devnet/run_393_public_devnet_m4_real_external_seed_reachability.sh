#!/usr/bin/env bash
# Run 393: public DevNet M4 REAL EXTERNAL seed reachability execution harness.
#
# This harness produces the Run 393 acceptance evidence for M4 real external
# seed reachability (see `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_393.md`). It drives
# the RELEASE `target/release/qbind-node` binary directly and records an HONEST,
# BOUNDED verdict about whether M4 can move Yellow -> Green via Route A (real
# external TCP + KEMTLS/static-root reachability proven from an INDEPENDENT
# OFF-HOST vantage point).
#
...
# seed under strict KEMTLS mutual-auth + PQC static-root, pin the Run 356 genesis,
# and prove external TCP + KEMTLS reachability from an INDEPENDENT OFF-HOST vantage
# point. In this sandboxed CI environment there is NO external ingress and NO
# independent off-host vantage point, so a real endpoint CANNOT be exposed and
# external reachability CANNOT be proven. Per Route C the harness does NOT expose
# a real endpoint, does NOT invent an off-host vantage point, does NOT fabricate
# reachability, and does NOT mark any committed seed entry `status: live` or emit
# `devnet-seeds.live.json`. M4 stays YELLOW; the Route A infrastructure
# prerequisites are documented in the reachability evidence record. This is
# unchanged from the Run 378/388/391 finding, and continues the Run 392 seed-ops
# runbook / M4 Route-A checklist that was accepted as PASS.
#
# What it still proves (continuity with Run 377/378/388/391):
#   1. `cargo build -p qbind-node --release --locked --bin qbind-node`;
...
#   8. release binary SHA-256, ELF BuildID, and toolchain are captured;
#   9. a non-claim grep confirms no false M4-Green / live / launch claim is
#      recorded in the tracked Run 393 artifacts.
#
# Run 393 opens NO externally reachable port, deploys NO seed/bootnode/faucet/RPC/
...
# default posture), adds NO new public CLI flag, and mutates NO trust/validator/
# epoch/sequence/marker/LivePqcTrustState. All addresses are loopback (127.0.0.1)
# or RFC 5737 documentation-example values. Temporary data dirs and identity
# material are removed on exit; no private key, root signing key, KEM secret key,
# data dir, or log is committed.

set -euo pipefail

OUTDIR="${1:-/tmp/qbind-run393-public-devnet-m4-real-external-seed-reachability}"
...
REACH_REF="docs/release/public-devnet/network/reachability/RUN_393_qbind-devnet-seed-1.md"
SUMMARY="${OUTDIR}/summary.txt"

log()  { printf '[run393] %s\n' "$*"; }
fail() { printf '[run393] FAIL: %s\n' "$*" >&2; exit 1; }
...
# ---------------------------------------------------------------------------
# 2. Generate real durable seed identity material (Run 375 path).
# ---------------------------------------------------------------------------
...
#    (referencing the Run 393 evidence record).
# ---------------------------------------------------------------------------
...
emit "external_tcp_reachability=NOT_PROVEN (Route C: no external ingress / no independent off-host vantage point in this environment)"
emit "external_kemtls_reachability=NOT_PROVEN (Route C: no external endpoint / no independent off-host vantage point; not fabricated)"
emit "observed_remote_nodeid_match=NOT_CAPTURED (no external observer exists; not fabricated)"
...
#    NOT a live claim — external reachability was not proven). NO
#    devnet-seeds.live.json is created (Route A did not succeed).
# ---------------------------------------------------------------------------
...
LIVE_JSON="${REPO_ROOT}/docs/release/public-devnet/network/devnet-seeds.live.json"
if [ -e "${LIVE_JSON}" ]; then
  fail "devnet-seeds.live.json exists but Route A did not succeed (must not be published)"
fi
emit "devnet_seeds_live_json=ABSENT (Route C: not published; no live seed to promote)"

# ---------------------------------------------------------------------------
# 8. Non-claim grep over tracked Run 393 artifacts: the negative conclusion
#    markers are present and NO positive M4-Green / live-reachability claim
#    token appears for this run.
# ---------------------------------------------------------------------------
REACH_DOC="${REPO_ROOT}/${REACH_REF}"
EVIDENCE_DOC="${REPO_ROOT}/docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_393.md"

# 8a. The reachability record must pin all conclusion claims false.
for KV in external_tcp_reachability=false external_kemtls_reachability=false \
          live_reachability_claim=false m4_green_claim=false c4_c5_closure_claim=false; do
  grep -q "${KV}" "${REACH_DOC}" || fail "reachability record missing '${KV}'"
done

# 8b. No positive claim token may appear in any tracked Run 393 artifact.
for DOC in "${REACH_DOC}" "${EVIDENCE_DOC}" "${SUMMARY}"; do
  [ -f "${DOC}" ] || continue
  if grep -Eiq 'm4_green_claim=true|external_tcp_reachability=true|external_kemtls_reachability=true|live_reachability_claim=true|RESULT=PASS|RESULT=POSITIVE' "${DOC}"; then
    fail "non-claim grep: positive M4-Green/live/PASS token in ${DOC}"
  fi
done

# 8c. The harness summary must already carry the negative external markers.
grep -q "external_tcp_reachability=NOT_PROVEN" "${SUMMARY}" \
  || fail "summary missing NOT_PROVEN external markers"
emit "nonclaim_grep=OK (reachability conclusions all false; no positive M4-Green/live/PASS token in tracked Run 393 artifacts)"

# ---------------------------------------------------------------------------
# 9. No private material committed (temp material/data dirs removed on exit).
# ---------------------------------------------------------------------------
emit "committed_private_material=NONE (material/node-data dirs are temporary; removed on exit)"

emit ""
emit "RESULT=NEGATIVE-FOR-EXTERNAL (Route C: no safe external seed infrastructure available; live admission gate proven + loopback preflight only; external TCP + KEMTLS reachability NOT proven; no devnet-seeds.live.json; M4 stays Yellow; C4/C5 OPEN)"
