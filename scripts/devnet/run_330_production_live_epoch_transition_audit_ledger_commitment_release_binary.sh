#!/usr/bin/env bash
# Run 330 — release-binary evidence for the Run 329 live epoch-transition
# audit-ledger commitment boundary.
#
# Release-binary evidence for the Run 329 source/test live epoch-transition
# audit-ledger commitment boundary
...
# Run 329 production library symbols are present and exercised in release mode,
# and that the real executor behaves correctly under release-built conditions
# (DevNet/TestNet source-test accept over the real Run 329/324 verified
...
# audit-ledger-commitment replay-recovery-idempotency / stale governance-epoch /
...
# typed non-mutating live-audit-ledger commitment artifact for a future live
...
# OPEN.
#
# Substitution note: the Run 329 executor surfaces every failure as a typed
# `ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome` fail-closed variant;
# there is no separate `ProductionLiveEpochTransitionAuditLedgerCommitmentError`
# enum, so that symbol is intentionally not required by the reachability greps
# below.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_BIN="${REPO_ROOT}/target/release/examples/run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary_helper"
HELPER_OUT="${OUTDIR}/helper_evidence/run_330"
...
MOD="${SRC_DIR}/pqc_production_live_epoch_transition_audit_ledger_commitment.rs"
PCA_MOD="${SRC_DIR}/pqc_production_live_epoch_transition_durable_audit_finalization.rs"
...
HELPER_SRC="${REPO_ROOT}/crates/qbind-node/examples/run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary_helper.rs"

log() { printf '[run-330] %s\n' "$*" >&2; }
fail() { printf '[run-330] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "cargo build --release -p qbind-node --example run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node --example run_330_production_live_epoch_transition_audit_ledger_commitment_release_binary_helper ) > "${LOGS_DIR}/build_helper_run_330.log" 2>&1 || fail "helper build failed"
...
[[ -x "${HELPER_BIN}" ]] || fail "missing ${HELPER_BIN}"
{
  ...
  echo "helper_330_sha256:  $(sha256_file "${HELPER_BIN}")"
  echo "helper_330_buildid: $(build_id "${HELPER_BIN}")"
} >> "${PROVENANCE}"

log "running Run 330 helper (first invocation)"
set +e
"${HELPER_BIN}" "${HELPER_OUT}" > "${LOGS_DIR}/helper_run_330.log" 2>&1
HELPER_RC=$?
set -e
echo "${HELPER_RC}" > "${EXIT_DIR}/helper_run_330.rc"
[[ "${HELPER_RC}" -eq 0 ]] || fail "run_330 helper failed"
...
log "running Run 330 helper (second invocation for deterministic-digest comparison)"
SECOND_OUT="${DATA_DIR}/helper_run_330_second"
mkdir -p "${SECOND_OUT}"
set +e
"${HELPER_BIN}" "${SECOND_OUT}" > "${LOGS_DIR}/helper_run_330_second.log" 2>&1
HELPER_RC2=$?
set -e
echo "${HELPER_RC2}" > "${EXIT_DIR}/helper_run_330_second.rc"
[[ "${HELPER_RC2}" -eq 0 ]] || fail "second run_330 helper invocation failed"
if ! diff -q "${HELPER_OUT}/fixtures/run_330_deterministic_digests.txt" "${SECOND_OUT}/fixtures/run_330_deterministic_digests.txt" >/dev/null; then
  fail "deterministic digests differ across helper invocations"
fi

# The production binary must never announce that a Run 329 live epoch-transition
# audit-ledger commitment boundary has been constructed / enabled / wired.
assert_surface_silent() {
  local logf="$1"
  assert_not_grep "$logf" 'live epoch-transition audit-ledger commitment active|live epoch-transition audit-ledger commitment enabled|audit-ledger commitment executor active|audit-ledger commitment executor enabled|live-audit-ledger commitment active|live-audit-ledger commitment enabled|ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor enabled|MainNet live epoch-transition audit-ledger commitment enabled|audit-ledger commitment applied|live mutation applied|live epoch-transition durable-audit finalization active|live epoch-transition durable-audit finalization enabled|durable-audit finalization executor active|durable-audit finalization executor enabled|ProductionLiveEpochTransitionDurableAuditFinalizationExecutor enabled|durable-audit finalization applied|live epoch-transition commit receipt active|live epoch-transition commit receipt enabled|commit receipt executor active|commit receipt executor enabled|live-commit receipt active|live-commit receipt enabled|ProductionLiveEpochTransitionCommitReceiptExecutor enabled|MainNet live epoch-transition commit receipt enabled|commit receipt applied'
...
log "S1 help hides Run 329 live epoch-transition audit-ledger commitment surface (no new CLI flag)"
...
# No Run 330/319 live epoch-transition audit-ledger commitment boundary flag / symbol / run marker is exposed.
assert_not_grep "${LOGS_DIR}/qbind_node_help.log" 'live-epoch-transition-audit-ledger-commitment|ProductionLiveEpochTransitionAuditLedgerCommitment|pqc_production_live_epoch_transition_audit_ledger_commitment|audit-ledger-commitment|live-audit-ledger-commitment|live-epoch-transition-durable-audit-finalization|ProductionLiveEpochTransitionDurableAuditFinalization|pqc_production_live_epoch_transition_durable_audit_finalization|durable-audit-finalization|receipt-write|audit-write|live-epoch-transition-commit-receipt|ProductionLiveEpochTransitionCommitReceipt|pqc_production_live_epoch_transition_commit_receipt|commit-receipt|live-commit-receipt|run-330|run_330|run-329|run_329'
log "S2..S4 default surfaces silent on live epoch-transition audit-ledger commitment boundary claims"
...
log "S5 no live epoch-transition audit-ledger commitment CLI selector exists (invented flag fails closed as unknown)"
set +e; ( cd "${REPO_ROOT}" && "${NODE_BIN}" --print-genesis-hash --env devnet --p2p-audit-ledger-commitment-policy allow-source-test ) > "${LOGS_DIR}/S5_no_selector.log" 2>&1; S5_RC=$?; set -e
echo "${S5_RC}" > "${EXIT_DIR}/S5_no_selector.rc"; [[ "${S5_RC}" -ne 0 ]] || fail "invented audit-ledger-commitment selector must be rejected (no such flag)"
assert_grep "${LOGS_DIR}/S5_no_selector.log" 'unexpected argument'
assert_surface_silent "${LOGS_DIR}/S5_no_selector.log"
log "S6 default devnet genesis-hash surface fails closed (requires --genesis-path) and stays silent on audit-ledger-commitment claims"
...
# Run 329 executor symbols required to be reachable in release evidence (from source
# and/or the release helper).
RUN329_SYMS=(
  ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor
  ProductionLiveEpochTransitionAuditLedgerCommitmentConfig
  ...
  ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion
  LiveEpochTransitionAuditLedgerCommitmentKind
  LiveEpochTransitionAuditLedgerCommitmentAuthoritySource
  LiveEpochTransitionAuditLedgerCommitmentReplaySet
  ...
  recover_live_epoch_transition_audit_ledger_commitment_window
  production_live_epoch_transition_audit_ledger_commitment_content_digest
...
{
  echo "Run 330 source reachability — Run 329 live epoch-transition audit-ledger commitment boundary symbols in ${MOD}:"
...
for sym in ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor ProductionLiveEpochTransitionAuditLedgerCommitmentConfig ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy ProductionLiveEpochTransitionAuditLedgerCommitmentInputs ProductionLiveEpochTransitionAuditLedgerCommitmentRequest ProductionLiveEpochTransitionAuditLedgerCommitmentDecision ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome ProductionLiveEpochTransitionAuditLedgerCommitmentProtocolVersion LiveEpochTransitionAuditLedgerCommitmentKind LiveEpochTransitionAuditLedgerCommitmentAuthoritySource LiveEpochTransitionAuditLedgerCommitmentReplaySet EmptyLiveEpochTransitionAuditLedgerCommitmentReplaySet LiveEpochTransitionAuditLedgerCommitmentFixtureState evaluate_live_epoch_transition_audit_ledger_commitment recover_live_epoch_transition_audit_ledger_commitment_window production_live_epoch_transition_audit_ledger_commitment_content_digest production_live_epoch_transition_audit_ledger_commitment_request_id production_live_epoch_transition_audit_ledger_commitment_id production_live_epoch_transition_audit_ledger_commitment_transcript_digest; do
  assert_grep "${REACH_DIR}/source_reachability.txt" "$sym"
done
{
  echo "Run 330 helper reachability — Run 329 symbols exercised by the release helper (plus the Run 329/324 commit receipt + Run 313/314 runtime handoff + Run 311/312 guarded + Run 309/310 staged + Run 307/308 authorization + Run 305/306 application executor + Run 303/304 validator-set rotation intent composition):"
  for sym in "${RUN329_SYMS[@]}" pqc_production_live_epoch_transition_durable_audit_finalization ProductionLiveEpochTransitionDurableAuditFinalizationExecutor ProductionLiveEpochTransitionDurableAuditFinalizationDecision pqc_production_live_epoch_transition_commit_receipt ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision pqc_production_live_epoch_transition_commit_authorization ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision pqc_production_live_epoch_transition_mutation_execution ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision pqc_production_live_epoch_transition_execution_preparation ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision pqc_production_epoch_transition_runtime_handoff ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision pqc_production_guarded_epoch_transition_mutation_executor ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision pqc_production_staged_live_validator_set_epoch_transition_application_executor ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor pqc_production_live_validator_set_application_authorization ProductionLiveValidatorSetApplicationAuthorizationExecutor pqc_production_validator_set_rotation_application_executor ProductionValidatorSetRotationApplicationExecutor pqc_production_validator_set_rotation_intent ProductionValidatorSetRotationBoundary; do echo "=== symbol: ${sym} ==="; grep -In "$sym" "${HELPER_SRC}" || echo '(no occurrences in helper)'; echo; done
} > "${REACH_DIR}/helper_reachability.txt"
# Symbols the release helper must directly exercise.
for sym in ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor ProductionLiveEpochTransitionAuditLedgerCommitmentConfig ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy ProductionLiveEpochTransitionAuditLedgerCommitmentInputs ProductionLiveEpochTransitionAuditLedgerCommitmentRequest ProductionLiveEpochTransitionAuditLedgerCommitmentDecision ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome LiveEpochTransitionAuditLedgerCommitmentKind LiveEpochTransitionAuditLedgerCommitmentAuthoritySource EmptyLiveEpochTransitionAuditLedgerCommitmentReplaySet LiveEpochTransitionAuditLedgerCommitmentFixtureState evaluate_live_epoch_transition_audit_ledger_commitment recover_live_epoch_transition_audit_ledger_commitment_window production_live_epoch_transition_audit_ledger_commitment_content_digest production_live_epoch_transition_audit_ledger_commitment_request_id production_live_epoch_transition_audit_ledger_commitment_id production_live_epoch_transition_audit_ledger_commitment_transcript_digest ProductionLiveEpochTransitionDurableAuditFinalizationExecutor ProductionLiveEpochTransitionDurableAuditFinalizationDecision ProductionLiveEpochTransitionCommitReceiptExecutor ProductionLiveEpochTransitionCommitReceiptDecision ProductionLiveEpochTransitionCommitAuthorizationExecutor ProductionLiveEpochTransitionCommitAuthorizationDecision ProductionLiveEpochTransitionMutationExecutionExecutor ProductionLiveEpochTransitionMutationExecutionDecision ProductionLiveEpochTransitionExecutionPreparationExecutor ProductionLiveEpochTransitionExecutionPreparationDecision ProductionEpochTransitionRuntimeHandoffExecutor ProductionEpochTransitionRuntimeHandoffDecision ProductionGuardedEpochTransitionMutationExecutor ProductionGuardedEpochTransitionMutationDecision ProductionStagedLiveValidatorSetEpochTransitionApplicationExecutor ProductionLiveValidatorSetApplicationAuthorizationExecutor ProductionValidatorSetRotationApplicationExecutor ProductionValidatorSetRotationBoundary; do
  assert_grep "${REACH_DIR}/helper_reachability.txt" "$sym"
done
grep -RIn --include='*.rs' 'pub mod pqc_production_live_epoch_transition_audit_ledger_commitment' "${SRC_DIR}/lib.rs" > "${REACH_DIR}/module_registration.txt" || fail "missing module registration"
grep -RIn --include='*.rs' 'fn evaluate_live_epoch_transition_audit_ledger_commitment\|fn recover_live_epoch_transition_audit_ledger_commitment_window\|fn production_live_epoch_transition_audit_ledger_commitment_content_digest\|fn production_live_epoch_transition_audit_ledger_commitment_request_id\|fn production_live_epoch_transition_audit_ledger_commitment_id\|fn production_live_epoch_transition_audit_ledger_commitment_transcript_digest' "${MOD}" > "${REACH_DIR}/entry_points.txt" || fail "missing executor entry points"
grep -RIn --include='*.rs' 'enum ProductionLiveEpochTransitionAuditLedgerCommitmentOutcome\|enum ProductionLiveEpochTransitionAuditLedgerCommitmentRecoveryOutcome\|enum ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy\|enum ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorKind\|enum LiveEpochTransitionAuditLedgerCommitmentKind\|enum LiveEpochTransitionAuditLedgerCommitmentAuthoritySource' "${MOD}" > "${REACH_DIR}/outcome_taxonomy.txt" || fail "missing executor taxonomy"
grep -RIn --include='*.rs' 'trait LiveEpochTransitionAuditLedgerCommitmentReplaySet\|struct ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor\|struct ProductionLiveEpochTransitionAuditLedgerCommitmentArtifact\|struct ProductionLiveEpochTransitionAuditLedgerCommitmentRequest\|struct EmptyLiveEpochTransitionAuditLedgerCommitmentReplaySet\|struct LiveEpochTransitionAuditLedgerCommitmentFixtureState' "${MOD}" > "${REACH_DIR}/boundary_surface.txt" || fail "missing executor surface"

C4C5_DOC="${REPO_ROOT}/docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md"
C4C5_PHRASES=(
  'Status as of Run 330'
  'boundary readiness'
  ...
  'Live epoch-transition audit-ledger commitment'
  'Green for release-binary-evidenced live-epoch-transition-audit-ledger-commitment-boundary behavior only'
  'becomes Green-for-scope only because Run 330 landed its release-binary evidence'
  'Full MainNet release-binary evidence under production custody'
)
{
  echo "Run 330 C4/C5 matrix taxonomy reachability — ${C4C5_DOC}:"
...
# commit receipt row remains Green-for-scope and the audit-ledger commitment row
...
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-durable-audit-finalization-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition durable-audit finalization row must remain scoped Green"
grep -F -q 'Green for release-binary-evidenced live-epoch-transition-audit-ledger-commitment-boundary behavior only' "${C4C5_DOC}" || fail "live epoch-transition audit-ledger commitment row must be scoped Green"
grep -F -q 'becomes Green-for-scope only because Run 330 landed its release-binary evidence' "${C4C5_DOC}" || fail "live epoch-transition audit-ledger commitment row must be scoped Green"
# The live epoch-transition commit receipt and audit-ledger commitment rows must
# record the non-goals explicitly.
grep -F -q 'not wired by default into production runtime' "${C4C5_DOC}" || fail "audit-ledger commitment row must record no default runtime wiring"
grep -F -q 'no public CLI flag' "${C4C5_DOC}" || fail "audit-ledger commitment row must record no public CLI flag"
...
grep -F -q 'consumes verified live epoch-transition durable-audit finalization decisions and produces typed non-mutating live audit-ledger commitment / audit-ledger preparation artifacts for a future production audit-ledger commitment run' "${C4C5_DOC}" || fail "audit-ledger commitment row must record durable-audit-finalization consumption and typed non-mutating live audit-ledger commitment / audit-ledger preparation artifacts"
grep -F -q 'never calls `BasicHotStuffEngine::transition_to_epoch` on production runtime state' "${C4C5_DOC}" || fail "audit-ledger commitment row must record no transition_to_epoch call"
grep -F -q 'does not prove live production validator-set mutation, production epoch transition, MainNet readiness, or C4/C5 closure.' "${C4C5_DOC}" || fail "audit-ledger commitment row must record no MainNet mutation proof / no C4/C5 closure"
...
DENY_PATTERNS=(
  ...
  'audit-ledger commitment executor active' 'audit-ledger commitment executor enabled'
  'live-audit-ledger commitment active' 'live-audit-ledger commitment enabled' 'audit-ledger commitment applied'
  'live epoch-transition durable-audit finalization active' 'live epoch-transition durable-audit finalization enabled'
  'durable-audit finalization executor active' 'durable-audit finalization executor enabled' 'durable-audit finalization applied'
  ...
  'MainNet authority rotation enabled' 'MainNet live epoch-transition audit-ledger commitment enabled' 'peer-driven apply enabled'
...
{
  echo "Run 330 denylist (proven empty across captured logs/helper output except help and summary):"
...
{
  echo "Run 330 no-mutation / no-authority-extension proof:"
  echo "  The release helper drives the real Run 329 ProductionLiveEpochTransitionAuditLedgerCommitmentExecutor ONLY"
  echo "  through the source/test boundary, ONLY for DevNet/TestNet identities on the accept path, under explicit"
  echo "  source/test, production-required and MainNet-required policies. It consumes a verified Run 329/324 live epoch-transition"
  ...
  echo "  accept decision. The Run 329 executor produces ONLY a typed non-mutating live-audit-ledger commitment artifact describing"
  ...
  echo "  caller-owned in-memory LiveEpochTransitionAuditLedgerCommitmentFixtureState used exclusively as source/test evidence,"
  ...
  echo "  The default ProductionLiveEpochTransitionAuditLedgerCommitmentExecutorPolicy is Disabled; the production binary is not wired to"
...
{
  echo "Run 330 — release-binary evidence for the Run 329 live epoch-transition audit-ledger commitment boundary"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_branch: $(git -C "${REPO_ROOT}" rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
  ...
  sed 's/^/  /' "${HELPER_OUT}/fixtures/run_330_deterministic_digests.txt"
  echo
