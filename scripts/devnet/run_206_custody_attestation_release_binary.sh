#!/usr/bin/env bash
# Run 206 — Release-binary custody **attestation verifier** boundary evidence on
# real `target/release/qbind-node`. Closes the Run 205-deferred release-binary
# boundary for the production custody attestation verifier skeleton added by
# `crates/qbind-node/src/pqc_custody_attestation_verifier.rs` (Run 205), layered
# over the Run 188 authority-custody boundary
# (`crates/qbind-node/src/pqc_authority_custody.rs`), the Run 203 KMS/HSM
# backend boundary (`crates/qbind-node/src/pqc_authority_kms_hsm_backend.rs`),
# and the Run 201 RemoteSigner transport boundary.
#
# Driving spec: `task/RUN_206_TASK.txt`.
#
# This harness proves on real `target/release/qbind-node`:
#
#   * the existing Run 070 / 130–205 binary surfaces (`--help`,
...
#   * the release-built Run 206 helper
#     `run_206_custody_attestation_release_binary_helper` exercises the Run 205
#     custody attestation verifier corpus (`verify_custody_attestation`,
#     `validate_custody_metadata_and_attestation`,
#     `validate_lifecycle_custody_and_attestation`, the fixture custody
#     attestation verifier, the production/cloud-KMS/PKCS#11/HSM/RemoteSigner
#     unavailable attestation verifiers, and the
#     evidence/input/transcript/provider-identity digests) — all in **release
#     mode** through the production library symbols.
#
# Strict scope (from `task/RUN_206_TASK.txt`):
#   * Release-binary evidence only.
#   * Use real `target/release/qbind-node`.
#   * Use the release-built Run 206 helper to exercise the Run 205 custody
#     attestation verifier boundary in release mode through the production
#     library symbols.
...
#   * Do not weaken Runs 070, 130–205.
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/docs/devnet/run_206_custody_attestation_release_binary}"
NODE_BIN="${REPO_ROOT}/target/release/qbind-node"
HELPER_206_BIN="${REPO_ROOT}/target/release/examples/run_206_custody_attestation_release_binary_helper"

HELPER_206_OUT="${OUTDIR}/helper_evidence/run_206"
...
log()  { printf '[run-206] %s\n' "$*" >&2; }
fail() { printf '[run-206] FAIL: %s\n' "$*" >&2; exit 1; }
...
log "OUTDIR=${OUTDIR}"
mkdir -p "${OUTDIR}"
rm -rf "${HELPER_206_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" \
       "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
mkdir -p "${HELPER_206_OUT}" "${LOGS_DIR}" "${EXIT_DIR}" \
         "${GREP_DIR}" "${REACH_DIR}" "${TEST_LOGS}" "${DATA_DIR}"
: > "${PROVENANCE}"
: > "${DENYLIST}"
: > "${MUT_PROOF}"
...
# ---------------------------------------------------------------------------
# Build qbind-node bin + Run 206 helper in release mode.
# ---------------------------------------------------------------------------
...
log "cargo build --release -p qbind-node --example run_206_custody_attestation_release_binary_helper"
( cd "${REPO_ROOT}" && cargo build --release -p qbind-node \
    --example run_206_custody_attestation_release_binary_helper ) \
  > "${LOGS_DIR}/build_helper_run_206.log" 2>&1 \
  || fail "release build of run_206 helper failed (see ${LOGS_DIR}/build_helper_run_206.log)"

[[ -x "${NODE_BIN}"       ]] || fail "missing ${NODE_BIN}"
[[ -x "${HELPER_206_BIN}" ]] || fail "missing ${HELPER_206_BIN}"

{
  echo "qbind_node_path:    ${NODE_BIN}"
  echo "qbind_node_sha256:  $(sha256_file "${NODE_BIN}")"
  echo "qbind_node_buildid: $(build_id "${NODE_BIN}")"
  echo "helper_206_path:    ${HELPER_206_BIN}"
  echo "helper_206_sha256:  $(sha256_file "${HELPER_206_BIN}")"
  echo "helper_206_buildid: $(build_id "${HELPER_206_BIN}")"
} >> "${PROVENANCE}"

# ---------------------------------------------------------------------------
# Drive the Run 206 release helper. Exits 0 iff the accepted (A1..A15),
# rejection (R1..R40), separation/fail-closed, composition, determinism, and
# refusal-helper tables all matched in release mode through the production
# library symbols.
# ---------------------------------------------------------------------------
log "running Run 206 custody attestation release helper -> ${HELPER_206_OUT}"
HELPER_206_LOG="${LOGS_DIR}/helper_run_206.log"
set +e
"${HELPER_206_BIN}" "${HELPER_206_OUT}" > "${HELPER_206_LOG}" 2>&1
HELPER_206_RC=$?
set -e
echo "${HELPER_206_RC}" > "${EXIT_DIR}/helper_run_206.rc"
[[ "${HELPER_206_RC}" -eq 0 ]] || fail "run_206 helper exited rc=${HELPER_206_RC} (see ${HELPER_206_LOG})"
[[ -s "${HELPER_206_OUT}/helper_summary.txt" ]] || fail "run_206 helper did not write helper_summary.txt"
assert_grep "${HELPER_206_OUT}/helper_summary.txt" "verdict: PASS"

# ---------------------------------------------------------------------------
# Real-binary surface invariants. Run 205 added a pure additive library module
# only (no CLI flag, no env var, no runtime banner). The surface contract is
# therefore that every existing Run 070 / 130–205 surface emits no custody
...
# ---------------------------------------------------------------------------
# Source/release reachability proof for the Run 205 custody attestation
# verifier boundary layered over the Run 188 custody verifier. We grep the
# production source under crates/qbind-node/src so the artifact records that the
# typed surface the Run 206 helper exercises is wired in production source.
# ---------------------------------------------------------------------------
log "writing source-reachability proof to ${REACH_DIR}/source_reachability.txt"
SRC_DIR="${REPO_ROOT}/crates/qbind-node/src"
{
  echo "Run 206 source-reachability proof — production symbols within ${SRC_DIR}:"
  echo
  for sym in \
    ...
    'trait CustodyAttestationVerifier' \
    'FixtureCustodyAttestationVerifier' \
    'ProductionAttestationVerifier' \
    'CloudKmsAttestationVerifier' \
    'Pkcs11HsmAttestationVerifier' \
    'HsmAttestationVerifier' \
    'RemoteSignerAttestationVerifier' \
    'fn evidence_digest' \
    'fn input_digest' \
...
{
  echo "Run 206 denylist (proven empty across all captured logs):"
  for pat in \
    'apply on receipt' \
    'apply-on-receipt' \
    'autonomous apply' \
  ...
  do
    if find "${LOGS_DIR}" "${HELPER_206_OUT}" -type f ! -name qbind_node_help.log ! -name helper_summary.txt -print0 2>/dev/null \
...
# ---------------------------------------------------------------------------
# No-mutation proof for rejected custody-attestation-boundary scenarios.
# ---------------------------------------------------------------------------
log "writing no-mutation proof to ${NOMUT_PROOF}"
{
  echo "Run 206 no-mutation proof for rejected custody-attestation-boundary scenarios:"
  echo "  data dir at ${DATA_DIR} contents (must be empty):"
  ls -la "${DATA_DIR}" 2>/dev/null || true
  echo
  echo "  helper-driven custody attestation rejection corpus (R1..R40):"
  ...
  echo "    * the verifier verify_custody_attestation, the composition"
  echo "      validate_custody_metadata_and_attestation, and the alias"
  echo "      validate_lifecycle_custody_and_attestation are pure functions"
  echo "      returning typed owned outcomes; on a reject they touch no marker /"
  echo "      sequence / live trust and never invoke Run 070"
  echo "      (helper rejection.manifest R1..R40 + R38/R39 no-mutation asserts)."
  grep -E 'verdict: PASS|^table |^total_(pass|fail):' \
    "${HELPER_206_OUT}/helper_summary.txt" 2>/dev/null \
    | sed 's/^/    /' || true
} > "${NOMUT_PROOF}"

# ---------------------------------------------------------------------------
# Mutation proof scaffold for accepted fixture custody-attestation scenarios
# (release-binary scope).
# ---------------------------------------------------------------------------
{
  echo "Run 206 mutation proof (release-binary scope):"
  echo
  echo "  library-side production custody attestation verifier boundary"
  echo "  reachability today:"
  echo "    - Run 205 added pqc_custody_attestation_verifier.rs with the typed"
  echo "      CustodyAttestationClass / CustodyAttestationPolicy taxonomy, the"
  echo "      CustodyAttestationEvidence / CustodyAttestationInput structs with"
  echo "      deterministic domain-separated evidence/input/transcript/"
  echo "      provider-identity digests, the pure/mockable"
  echo "      CustodyAttestationVerifier trait, the DevNet/TestNet-only"
  echo "      FixtureCustodyAttestationVerifier, the fail-closed"
  echo "      production/cloud-KMS/PKCS#11/HSM/RemoteSigner attestation"
  echo "      verifiers, the typed CustodyAttestationOutcome taxonomy, the pure"
  echo "      verify_custody_attestation, and the compositions"
  echo "      validate_custody_metadata_and_attestation /"
  echo "      validate_lifecycle_custody_and_attestation;"
  ...
  echo "    - the production / cloud-KMS / PKCS#11 / HSM / RemoteSigner"
  echo "      attestation verifiers fail closed as typed unavailable outcomes;"
  echo "    - the fixture custody attestation verifier is DevNet/TestNet"
  echo "      evidence-only and is refused on a MainNet trust domain;"
  echo "    - MainNet peer-driven apply remains the Run 147 / 148 / 152 FATAL"
  echo "      refusal even with fixture attestation material."
  echo
  echo "  release-binary custody attestation corpus (this run):"
  echo "    - the Run 206 helper exercises the A1..A15 acceptance corpus and"
  echo "      the R1..R40 rejection corpus in release mode through the"
  ...
  echo "      deterministic and domain-bound; the attestation boundary composes"
  echo "      with the Run 188 custody classes and the Run 203 / Run 201"
  echo "      backend / RemoteSigner transport evidence;"
  echo "    - the production / cloud-KMS / PKCS#11 / HSM / RemoteSigner"
  echo "      attestation verifiers return the typed unavailable outcome;"
  ...
  echo "      verifier is wired in Run 206. The production / cloud-KMS / PKCS#11"
  echo "      / HSM / RemoteSigner attestation verifiers always return the typed"
  echo "      unavailable reject;"
...
# ---------------------------------------------------------------------------
# Targeted cargo test cross-checks. Mirrors `task/RUN_206_TASK.txt Validation
...
# `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_206.md`.
# ---------------------------------------------------------------------------
log "writing summary -> ${SUMMARY}"
{
  echo "Run 206 — release-binary custody attestation verifier boundary evidence"
  echo "git_commit: $(git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || echo 'unknown')"
  echo
  echo "build:"
  echo "  rustc_version:      $(rustc --version 2>/dev/null || echo 'unknown')"
  ...
  echo "  helper_run_206	rc=$(cat "${EXIT_DIR}/helper_run_206.rc" 2>/dev/null || echo 'na')	$(grep -E 'verdict:' "${HELPER_206_OUT}/helper_summary.txt" 2>/dev/null | head -n1 || true)"
  echo
  echo "helper A1-A15 / R1-R40 corpus verdicts (release mode, production library symbols):"
  grep -E '^table |^total_(pass|fail): |^verdict: ' \
    "${HELPER_206_OUT}/helper_summary.txt" 2>/dev/null | sed 's/^/  /' || true
  echo
  ...
  echo "honest_limits:"
  echo "  * no real KMS / HSM / cloud KMS / PKCS#11 / RemoteSigner attestation"
  echo "    verifier is wired; the production / cloud-KMS / PKCS#11 / HSM /"
  echo "    RemoteSigner attestation verifiers fail closed as typed unavailable"
  echo "    outcomes;"
  ...
  echo "    and domain-bound;"
  echo "  * the attestation boundary composes with the Run 188 custody classes"
  echo "    and the Run 203 / Run 201 backend / RemoteSigner transport evidence;"
  echo "  * rejected custody-attestation-boundary cases produce no mutation"
  ...
  echo "    even with a fixture attestation response;"
  ...
  echo "  / 130–205 surface custody-attestation-silent (no custody attestation /"
  echo "  KMS / HSM / cloud KMS / PKCS#11 attestation enablement banner, MainNet"
  echo "  peer-driven apply refusal preserved), and the release-built Run 206"
  echo "  helper exercises the Run 205 custody attestation verifier corpus"
  echo "  end-to-end in release mode through the production library symbols:"
  echo "  fixture custody attestation accepted on DevNet/TestNet only;"
  echo "  production / cloud-KMS / PKCS#11 / HSM / RemoteSigner attestation"
  echo "  verifiers fail-closed as unavailable; evidence/input/transcript/"
  echo "  provider-identity digests deterministic and domain-bound; attestation"
  echo "  boundary composes with the Run 188 custody classes and the Run 203 /"
  echo "  Run 201 backend / RemoteSigner transport evidence; rejected cases"
  echo "  produce no mutation; and MainNet peer-driven apply remains the Run 147"
  echo "  FATAL refusal even with fixture attestation material. Real KMS / HSM /"
  echo "  cloud KMS / PKCS#11 / RemoteSigner attestation verifiers, real"
  echo "  RemoteSigner backend, real on-chain governance proof verification,"
