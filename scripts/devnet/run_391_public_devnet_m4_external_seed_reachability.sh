#!/usr/bin/env bash
# Run 391: public DevNet M4 EXTERNAL seed/bootnode reachability execution harness.
#
# This harness produces the Run 391 acceptance evidence for M4 external
# seed/bootnode reachability (see `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_391.md`).
...
# Run 378/388 finding (Run 386/387 preflight posture is likewise unchanged).
#
# What it still proves (continuity with Run 377/378/388):
...
#
# Run 391 opens NO externally reachable port, deploys NO seed/bootnode/faucet/RPC/
...
OUTDIR="${1:-/tmp/qbind-run391-public-devnet-m4-external-seed-reachability}"
...
REACH_REF="docs/release/public-devnet/network/reachability/RUN_391_qbind-devnet-seed-1.md"
SUMMARY="${OUTDIR}/summary.txt"

log()  { printf '[run391] %s\n' "$*"; }
fail() { printf '[run391] FAIL: %s\n' "$*" >&2; exit 1; }
...
#    (referencing the Run 391 evidence record).
# ---------------------------------------------------------------------------
