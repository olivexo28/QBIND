...
remain Green; public DevNet stays NOT launch-ready; C4/C5 remain OPEN; MainNet/TestNet untouched.**
Updated Run 412 — **readiness table/checklist/blocker consistency lint published; no readiness item moves (Route B —
docs + shell only, no production source change, no `build.rs` change, no `Cargo.toml` change, no new CLI flag)**: adds a
lightweight per-milestone consistency lint that checks this readiness matrix's §10 canonical current-status table
against its §4 must-have checklist (M1–M20) and §5 should-have checklist (S1–S7), and against the public DevNet
blocker register (`docs/release/public-devnet/BLOCKER_REGISTER.md`) and launch go/no-go gate
(`docs/release/public-devnet/LAUNCH_GO_NO_GO.md`), extending consistency protection **beyond** the Run 411
cross-ledger run-narrative lint (which keeps the readiness matrix and the contradiction ledger consistent on the
per-run narratives) down to the per-milestone status rows. In the course of this run the stale §4 M12 must-have
checklist entry (which still read Yellow / Partial from Run 362) was reconciled to **Green (Run 371, Green-for-scope)**
to match the §10 status table, §11 next-run recommendation, and §12 blocker summary that already record M12 Green —
a **documentation-consistency reconciliation of an already-historically-recorded status, not a new Green move**.
Publishes `docs/release/public-devnet/READINESS_STATUS_BLOCKER_LINT.md` and
`scripts/devnet/run_412_public_devnet_readiness_status_blocker_lint.sh` (`RESULT=POSITIVE`), which fails closed if the
§10 status table disagrees with the §4/§5 checklists on any M1–M20 / S1–S7 status, if M4 is anything other than
Yellow / launch-blocking, M6 anything other than Yellow / Partial, or S5/S7 anything other than Yellow anywhere they
appear, if the blocker register omits or resolves M4/M6/S5/S7, if the launch gate does not remain NO-GO / list M4+M6
as must-have blockers / keep S5+S7 M4-gated, or if any table/checklist/blocker text claims a live
seed/bootnode/faucet/RPC/explorer/status-service deployment, a `devnet-seeds.live.json`, C4/C5 closure, TestNet/MainNet
readiness, or a runtime mutation. It adds `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_412.md` and
`docs/devnet/run_412_public_devnet_readiness_status_blocker_lint/`, and narrowly updates
`docs/release/public-devnet/ARTIFACT_INDEX.md`, `docs/release/public-devnet/OPERATOR_VERIFICATION_MAP.md`,
`docs/release/public-devnet/READINESS_CONTRADICTION_LEDGER_LINT.md` (companion pointer), this matrix, and
`docs/whitepaper/contradiction.md`. It adds **no** CLI flag, opens **no** port, deploys **no**
seed/bootnode/faucet/RPC/explorer/status service, applies **no** trust bundle, and mutates **no**
validator/epoch/sequence/marker/`LivePqcTrustState` state. **No readiness item moves Green** — this run lints
documentation only. **M4 stays Yellow/launch-blocking; M6 stays Yellow/Partial; S5/S7 stay Yellow; M1–M3/M5/M7–M20
...
discoverable through the index / operator map / a documented exception, or if a
...
documentation only. **M4 stays Yellow/launch-blocking; M6 stays Yellow/Partial; S5/S7 stay Yellow; M1–M3/M5/M7–M20
remain Green; public DevNet stays NOT launch-ready; C4/C5 remain OPEN; MainNet/TestNet untouched.**
Updated Run 416 — **M4 external-reachability blocker CLEARED via the first successful real external
reachability run (Route A POSITIVE); M4 stays Yellow/launch-blocking pending a durable published
`devnet-seeds.live.json`**: where Runs 378/388/391/393/397 all executed the Route A objective and
recorded **Route C / NEGATIVE-FOR-EXTERNAL** (no external ingress / no independent off-host vantage in
the sandbox), a real deployment on operator infrastructure proved external TCP **and** external KEMTLS
mutual-auth static-root reachability to a routable public DevNet seed (`188.166.227.87:30333`, host
`<seed-host>`, listening `0.0.0.0:30333`) from a genuinely independent off-host dialer on a
different network (Laptop 1 WSL, host `<dialer-host>`, observed public egress `<dialer-public-egress>`), under
`--p2p-mutual-auth required --p2p-pqc-root-mode pqc-static-root` with a per-peer KEM public key +
`--validator-id` dialer override and temporary DevNet PQC material (`examples/devnet_pqc_root_helper`,
discarded on both sides). Publishes the publish-safe evidence archive
`docs/devnet/run_416_public_devnet_m4_live_seed_route_a_evidence/` (`README.md`, `summary.txt`,
`SHA256SUMS.txt`, `.gitignore`, `laptop1/*`, `vps/*`), the Route A reachability record
`docs/release/public-devnet/network/reachability/RUN_416_qbind-devnet-seed-1.md`
(`external_tcp_reachability=true`, `external_kemtls_reachability=true`, `m4_green_claim=false`), and
`docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_416.md`. The runtime evidence commit recorded in both manifests
is `1c2ba28d1532474a9e1124bc9873cd54193f29b6` (the live binary was built from it; the Run 415 docs
commit `50eed16b` landed afterward and is **not** claimed as the live binary's source). **M4 does NOT
move Green** — the seed identity was temporary DevNet PQC material (discarded), the VPS is not a durably
operated seed, and **no `devnet-seeds.live.json` is published**; the committed
`devnet-seeds.live-candidate.json` stays `status: planned`. It adds **no** CLI flag, **no**
`build.rs`/Rust/`Cargo.toml` change, and mutates **no** validator/epoch/sequence/marker/`LivePqcTrustState`
state; no private key material is committed. **M4 stays Yellow/launch-blocking (external-reachability
...
Updated Run 418 — **F6 authenticated KEMTLS peer → consensus sender binding remediated in CODE and TEST only; no readiness item moves; RS1 stays OPEN**: Run 418 is a production-code security-remediation run (Route A — Rust code + tests) that implements and proves the transport-session-to-consensus-sender binding recorded as Run 417 finding **F6**. The authenticated KEMTLS peer identity (full 32-byte certificate-derived `NodeId` + validator identity) is bound to the claimed consensus sender via `PeerConsensusBindingGate::authorize` (`crates/qbind-node/src/peer_consensus_binding.rs`) wired into the in-crate consensus ingress `binary_consensus_loop::handle_inbound_consensus_msg`; the authoritative `NodeId→ValidatorId` map admits a certificate only after structural/suite validation, verification against the configured PQC root, canonical validator-id match, index `< num_validators`, revocation/environment checks, and full 32-byte NodeId derivation (`p2p_node_builder::validated_cert_bound_node_id`), with strict canonical validator-id parsing; the client-side verified server identity is surfaced (`secure_channel::VerifiedServerIdentity`) so an unexpected server certificate cannot inherit a configured address's origin; missing/unknown/ambiguous/mismatched identities **fail closed** with fixed-label metrics and no engine call, mutation, outbound action, or accepted/delivered increment. Dedicated unit + integration acceptance tests pass using real production ingress/gate/demux/verified-identity types. **F6 is transport-session-to-consensus-sender binding and accountability ONLY** — it does not make cryptographic proposal/vote/timeout/new-view/QC signatures meaningful and **does not close F3, F4, F5, F7, F8, RS1, C4, or C5**; F5 is **not** fixed or always enforced (NewView carries no single immediate transport-sender field, and Timeout/NewView crypto verification remains the independently unresolved F5 boundary). This is a **code/test remediation of F6 only**; **no real release-binary evidence exists** on a live multi-node network, so **RS1 remains OPEN / launch-blocking**. **No readiness item moves Green**: M4 stays Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; public DevNet NOT launch-ready; no `devnet-seeds.live.json`; MainNet/TestNet untouched; no keys/certs/private material committed. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_418.md`, archive `docs/devnet/run_418_authenticated_peer_consensus_sender_binding/`, harness `scripts/devnet/run_418_authenticated_peer_consensus_sender_binding.sh`.
Updated Run 419 — **F6 partial release-binary evidence captured (Route A — evidence only, no production source change); no readiness item moves; RS1 stays OPEN**: Run 419 exercises the Run 418 binding gate on the standalone `target/release/qbind-node` binary over real loopback KEMTLS/static-root sessions driven by a Cargo-example helper (`crates/qbind-node/examples/run_419_f6_release_binary_binding_driver.rs`) and a fail-closed harness (`scripts/devnet/run_419_f6_standalone_release_binary_binding_evidence.sh`). It records live `qbind_consensus_binding_total{result="…"}` deltas on the receiver `/metrics` for the core Required/static-root path: honest authenticated proposal admitted (`accepted +1`), five-class impersonation rejected (`claimed_sender_mismatch +5` with all downstream delivered/engine-accepted/commit counters unchanged at 0), NewView origin admission (`accepted +1`), and root-valid unconfigured alternate leaf suppressed pre-gate (`missing_origin +1`). The unauthenticated-ingress (S4/S5) and dedicated outbound-identity (S7) vectors could not be driven as decisive real-socket evidence with the present transport configuration, so the overall verdict is a defined **PARTIAL** (`RESULT=PARTIAL-FOR-F6-STANDALONE-RELEASE-BINARY-EVIDENCE`, `F6_STATUS=CODE-TEST-PLUS-PARTIAL-RUNTIME-EVIDENCE`) and F6 is **not** promoted to complete release-binary coverage. This is **automated local loopback cross-process evidence — not off-host attestation and not external reachability**. **No readiness item moves Green**: M4 stays Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 remains OPEN / launch-blocking**; public DevNet **NO-GO**; no `devnet-seeds.live.json`; MainNet/TestNet untouched; no keys/certs/private material committed. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_419.md`, archive `docs/devnet/run_419_f6_standalone_release_binary_binding_evidence/`.
Updated Run 420 — **F3/F4/F8 configured-path fail-closed Proposal/Vote verification boundary remediated in CODE and TEST only; no readiness item moves; RS1 stays OPEN**: Run 420 replaces the former fail-open `verification_ctx == None` Proposal/Vote bypass in `binary_consensus_loop` with an explicit typed `ConsensusVerificationPolicy` (`Required` default selected by the production `main.rs` constructor; test-only `LocalFixtureUnsigned` unreachable from production, with source guards), fail-closed inbound rejection and outbound suppression when required verification/signing authority is unavailable, configured-context signature and suite verification, and bounded context-unavailable metrics. This is a **code + test remediation of the configured boundary only**; the standalone `target/release/qbind-node` still has **no** authoritative consensus signer/key-provider configuration, so `RELEASE_BINARY_EVIDENCE=NOT-CAPTURED` and the deployed binary is expected to reject inbound and suppress outbound Proposal/Vote (intentional, safer than unsigned operation). **F3/F4/F8 are not fully remediated in production; F1/F2/F5/F7 unresolved; F6 partial; RS1 OPEN.** **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; no `devnet-seeds.live.json`; MainNet/TestNet untouched; no keys/certs/private material committed. Deliverable: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_420.md`.
Updated Run 421 — **F3/F4/F8 unavailable-authority fail-closed release-binary evidence captured (Route A — evidence only, no production source change); no readiness item moves; RS1 stays OPEN**: Run 421 exercises the already-deployed Run 420 gate on the standalone `target/release/qbind-node` binary over real loopback KEMTLS/static-root sessions with `verification_ctx == None`, driven by a Cargo-example helper (`crates/qbind-node/examples/run_421_f3_f4_f8_release_binary_driver.rs`, fixed enumerated scenarios) and a fail-closed harness (`scripts/devnet/run_421_f3_f4_f8_unavailable_authority_release_binary_evidence.sh`). Live `/metrics` deltas: authenticated honest Proposal(1) and Vote(1) rejected fail-closed (`inbound_{proposal,vote}_verification_context_unavailable_total +1`, F6 `accepted +1` each); F6 sender-mismatch Proposal(0)+Vote(0) rejected **before** the Run 420 gate (`claimed_sender_mismatch +2`, context counters unchanged); malformed-signature and wrong-suite variants cannot bypass the unavailable-authority gate (context counters `+1`, no `rejected_bad_signature`/`rejected_unsupported_suite`) — all captured downstream delivered/engine/aggregation/QC/view/commit/outbound counters unchanged at 0; real-binary `--help` exposes no `LocalFixtureUnsigned`/verification-policy/forged-message/injection flag (invented fixture flag exits non-zero, default remains `Required`). Outbound suppression scenarios S7/S8 are **UNREACHABLE** on the standalone binary (no natural leader-proposal opportunity, `leader_changes_total = 0`; inbound fail-closes before engine ingestion), so the verdict is **PARTIAL-POSITIVE** (`RESULT=PARTIAL-POSITIVE-FOR-F3-F4-F8-UNAVAILABLE-AUTHORITY-FAIL-CLOSED-RELEASE-BINARY-EVIDENCE`) and `CONFIGURED_AUTHORITY_RELEASE_BINARY_EVIDENCE=NOT-CAPTURED`. This is **automated local loopback cross-process evidence — not off-host attestation, not external reachability, not liveness-positive, and not configured-authority signing/verification**. **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; no `devnet-seeds.live.json`; candidate seed status/reachability unchanged; MainNet/TestNet untouched; no keys/certs/private material committed. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_421.md`, archive `docs/devnet/run_421_f3_f4_f8_unavailable_authority_release_binary_evidence/`.

Updated Run 422 — **genesis-bound consensus authority activation remediated in CODE and TEST only (Route A — production source: new opt-in fail-closed flag); no readiness item moves; RS1 stays OPEN**: Run 422 adds `--consensus-authority-from-genesis` and `crates/qbind-node/src/genesis_consensus_authority.rs`, deriving the consensus validator set and per-validator authorized ML-DSA-44 `(suite, public_key)` provider **directly from the boot-verified canonical genesis** (`GenesisConfig.validators[].pqc_public_key`), computing a domain-tagged authority commitment bound to `chain_id` + genesis hash, requiring a loaded local signer whose public key equals the genesis-committed key for the local validator, and feeding the **same** validated constructor `main` uses (`try_build_timeout_verification_context`). This closes the Runs 031–033 gap where consensus keys came from the **uncommitted** `--validator-consensus-key` CLI override rather than canonical genesis. Every explicit invalid/partial activation exits startup **nonzero before P2P/consensus start** (no silent downgrade to `None`/unsigned); absent the flag, behavior is byte-identical to Run 421 (`verification_ctx == None` + `Required`), re-verified this run with the Run 421 and Run 419 release-binary harnesses retaining their PARTIAL verdicts and zero drift to historical Run 417–421 archives. This is a **source/test-positive activation only** (14 unit + 6 integration tests pass on the real `MlDsa44Backend`; 24 Run 418/420 regression tests pass); **configured-authority ADVERSARIAL release-binary evidence is NOT captured** (`CONFIGURED_AUTHORITY_RELEASE_BINARY_EVIDENCE=NOT-YET-CAPTURED`, Run 423 scope). **F3/F4/F8 are configured-authority production-path code/test-positive only — not release-binary adversarial and not full closure; F1/F2/F5/F7 unresolved; F6 partial; RS1 OPEN.** **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; no `devnet-seeds.live.json`; candidate seed status/reachability unchanged; MainNet/TestNet untouched; no keys/certs/private material committed. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_422.md`, archive `docs/devnet/run_422_consensus_authority_activation/`.

Updated Run 422 CORRECTIVE CONTINUATION — **the initial Run 422 POSITIVE was NOT accepted by source review and is SUPERSEDED by a PARTIAL corrective result**: `RESULT=PARTIAL-FOR-DEVNET-CONSENSUS-AUTHORITY-ACTIVATION-CODE-TEST` / `PRODUCTION_ACTIVATION=UNAVAILABLE-FOR-UNRESOLVED-BOUNDARIES`. **Corrected and behaviorally tested** this continuation: (D1) single-snapshot genesis provenance — the external genesis is read **once** into an owned snapshot, re-validated in place, the canonical identity is derived from that same parse and required to equal the boot-accepted identity, via the shared `load_verify_and_build_genesis_authority` (the two-read `compute_print_genesis_hash` pairing is removed); (D2) engine/verifier membership consistency — `peer_derived_count != genesis_authority_count` is rejected (no silent resize); (D3) every-mode flag handling — `--consensus-authority-from-genesis` is rejected non-zero under `--network-mode local-mesh`. **Still UNRESOLVED (reported as blockers, activation kept fail-closed):** (D4) startup ordering vs `P2pNodeBuilder::build`, (D5) Proposal/Vote vs Timeout/NewView shared-context separation, (D6) signed-domain / replay isolation (round-trip and wrong-genesis rejection NOT demonstrated; needs a versioned protocol change), (D7) genesis-static lifetime across epoch/restore transitions. **F3/F4/F8 are therefore PARTIAL — not code/test-positive.** Withdrawn claims: "invalid activation always fails before P2P startup", "flag absence universally implies None", "peer-derived engine membership necessarily matches genesis membership", "digest sensitivity proves wrong-genesis signature rejection". **No readiness item moves Green; RS1/C4/C5 stay OPEN; public DevNet NO-GO.** Run 423 not started. See `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_422.md`.
Updated Run 422 D4/D5 SUCCESSOR — **two of the four corrective blockers are closed on the CODE + TEST axis only; no readiness item moves; RS1 stays OPEN**: `RESULT=POSITIVE-FOR-CONSENSUS-SECURITY-PREFLIGHT-AND-CONTEXT-SEPARATION-CODE-TEST`. **(D4)** the fatal consensus-security preflight `run_p2p_consensus_security_preflight` runs **before** `P2pNodeBuilder::build`, peer dialing, and consensus-task startup — proven by deadline-bounded process tests over the real `qbind-node` binary (stdout/stderr drained concurrently; a timeout is a TEST FAILURE; child killed+reaped on every path) that observe a nonzero natural exit and the `[binary] Run 032/033:` preflight positive-control **without** the post-build `[binary] P2P transport up.` marker, interpreted with the inspected source ordering. **Metrics exception preserved:** the optional loopback `/metrics` HTTP task is spawned earlier in `main` (out of scope) and the genesis-refusal guard sits after that spawn but before the per-mode dispatch — the boundary is NOT "before any networking". **(D5)** a valid `TimeoutVerificationContext` never establishes `ProposalVoteAuthority`; under `Required` with absent Proposal/Vote authority, inbound Proposal/Vote are rejected fail-closed (F6 sender-binding precedes the authority lookup) and outbound Proposal/Vote are suppressed with **no fallback to Timeout credentials** — combined-context inbound tests, the ACTUAL late-peer re-emission path (`maybe_reemit_on_late_peer_connect`, reaching the authority/signing boundary), and an ACTIVE restore-mode Proposal rejection (authority-unavailable fires before restore deferral; recording facade observes zero outbound). Legacy Timeout/NewView cryptographic coverage is attributed to the existing **run030** tests (not the D5 context-field tests); legacy Timeout/NewView policy is **preserved**; the legacy `--validator-consensus-key` CLI route **no longer** establishes Proposal/Vote authority; genesis activation stays **DISABLED**. **Still UNRESOLVED: (D6)** signed-domain / replay isolation and **(D7)** genesis-static / epoch-restore authority lifetime. Security (CodeQL) analysis was **NOT** run to a zero-alert conclusion (separate validation axis; a code review is not a substitute). Tests + documentation only — Run 421/419 release-binary evidence and the prior release build are **preserved at their original tested SHA**, not re-run. `CONFIGURED_AUTHORITY_RELEASE_BINARY_EVIDENCE=NOT-YET-CAPTURED`. **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; MainNet/TestNet untouched; no keys/certs/private material committed. Run 423 not started. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_422_D4_D5.md`, archive `docs/devnet/run_422_d4_d5_preflight_context_separation/`.
Updated Run 422 D6 SUCCESSOR — **the signed-domain replay-isolation blocker (D6) is closed on the CODE + TEST axis only; no readiness item moves; RS1 stays OPEN**: `RESULT=POSITIVE-FOR-PROPOSAL-VOTE-DOMAIN-ISOLATION-CODE-TEST`. An explicitly versioned Proposal/Vote signing domain (`QBIND:PVDOMAIN:v2`, `ProposalVoteSigningDomainV2` in `crates/qbind-wire/src/pv_signing_domain.rs`) prepends a typed, immutable header — format version, message family, the **full 64-bit runtime ChainId**, the accepted 32-byte genesis identity, and the 32-byte consensus authority commitment — to the **unchanged** v1 `canonical_body()` bytes, so a signature valid for one domain does not authenticate the same message under another domain even with the same key, validator index, payload, and wire `chain_id`. Proven with the real `MlDsa44Backend`: replay matrix cases AâK (two custom chain ids both mapping to legacy `UNK`, different accepted genesis, different authority commitment, v1âv2 and v2âv1 mutual rejection, wrong family, unsupported version with no fallback, tamper, the existing missing-sig/wrong-key/unknown-validator/wrong-suite/unsupported-suite taxonomy, closed construction on invalid domain metadata, expected-wire-chain binding) plus deterministic golden preimage vectors specified independently of the encoder; and real-handler accept/reject tests driving the actual inbound/outbound Proposal/Vote paths with the real F6 sender-binding gate under `ConsensusVerificationPolicy::Required` (foreign-domain messages rejected before delivered counters, restore deferral, reconfiguration observation, engine mutation, and outbound actions; F6 mismatch still precedes domain/crypto; missing Proposal/Vote authority still rejects with a valid Timeout context; outbound signs only under the selected domain). Wire bytes are **unchanged** and v1 preimage bytes are **byte-identical**. **Activation stays DISABLED:** production `proposal_vote_authority` remains `None`, no CLI/env/override was added, and the runtime-ChainId(u64)âwire-chain_id(u32) mapping is **UNRESOLVED** (`expected_wire_chain_id` from trusted fixtures/config only), so production construction remains unavailable; downstream engine/QC still reconstruct the legacy preimage, so a v2 boundary pass is **not** QC/engine validation; **(D7)** genesis-static / epoch-restore authority lifetime remains unresolved. Security (CodeQL) analysis was **NOT** run to a zero-alert conclusion (separate validation axis). **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; MainNet/TestNet untouched; no keys/certs/private material committed. Run 423 not started. Deliverables: `docs/protocol/QBIND_PROPOSAL_VOTE_SIGNING_DOMAIN_V2.md`, `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_422_D6.md`, archive `docs/devnet/run_422_d6_proposal_vote_domain_isolation/`.
Updated Run 422 D7 SUCCESSOR — **the genesis-static / epoch-restore authority-lifetime blocker (D7) is **PARTIAL** on the CODE + TEST axis only (earlier POSITIVE closure WITHDRAWN); no readiness item moves; RS1 stays OPEN**: `D7_STATUS=PARTIAL-CODE-TEST / PRODUCTION-LIFECYCLE-UNAVAILABLE`; `DURABLE_ANTI_ROLLBACK=NOT-ESTABLISHED`. An additive, fail-closed lifetime guard on the genesis-static consensus authority (`GenesisConsensusAuthority::authorize_configuration` in `crates/qbind-node/src/genesis_consensus_authority.rs`, with `ObservedConsensusConfiguration` and a bounded `AuthorityLifetimeError`) authorizes **only** the founding epoch-0 configuration and rejects — ordered chain-id → genesis-hash → membership-count → authority-commitment → epoch — any changed chain / genesis / membership / commitment or any non-founding epoch, because this run implements no key rotation, revocation, or membership transition (so no authorized transition exists and stale genesis keys cannot sign). Proven with the real `MlDsa44Backend` across 12 section-12.E tests: snapshot immutability when the source genesis file changes after validation, 8-thread parallel non-mixing of keys/suites/membership/results, restart/restore identity mismatch and tampered-membership fail-closed, epoch-advance rejection (1, 2, 7, `u64::MAX`), malformed/extreme input cannot panic, and fixture-bypass unreachable through the production constructors/parser. Activation stays **DISABLED**: production `proposal_vote_authority` remains `None`, the release binary still refuses `--consensus-authority-from-genesis` with exit 1 before any P2P/consensus service, no CLI/env/override was added, and the `CurrentEpochUnavailable` trust-bundle boundary is untouched; downstream engine/QC are unaffected (no QC/engine claim). Security (CodeQL) analysis is a separate axis and was not run to a zero-alert conclusion. **No readiness item moves Green**: M4 Yellow/launch-blocking; M6 Yellow/Partial; S5/S7 Yellow; C4/C5 OPEN; **RS1 OPEN / launch-blocking**; public DevNet **NO-GO**; MainNet/TestNet untouched; no keys/certs/private material committed. `CONFIGURED_AUTHORITY_RELEASE_BINARY_EVIDENCE=NOT-YET-CAPTURED`. Run 423 not started. Deliverables: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_422_D7.md`, archive `docs/devnet/run_422_d7_authority_lifetime/`.
...
- No durable seed-node/bootnode deployment or published live seed list (`devnet-seeds.live.json`) by this run. (Run 416 demonstrated external reachability against a **temporary, discarded** operator seed; no durable seed is operated or published.)
...
- [~] S7. Seed-node operational runbook (operating the published seeds). — **Yellow (Run 392)**: a seed-node operations runbook, an M4 Route-A deployment checklist, and a reachability evidence template are published (`docs/release/public-devnet/network/SEED_NODE_OPERATIONS.md`, `M4_ROUTE_A_DEPLOYMENT_CHECKLIST.md`, `SEED_REACHABILITY_EVIDENCE_TEMPLATE.md`) and verified against the real `qbind-node` CLI/help + seed-list schema surfaces. Operating a **live** seed still depends on M4 (no durable published live seed exists yet — external reachability itself is proven as of Run 416), so S7 stays Yellow, not Green.
...
| M4, M7 | Ops | CLI `--p2p-peer`; no durable published live seed list (`devnet-seeds.live.json`) yet — external reachability proven Run 416; `crates/qbind-remote-signer/` |
| M5, M6, M17 | Docs | `docs/release/public-devnet/operator/` external onboarding package (Run 358: `README.md`, `QUICKSTART.md`, `IDENTITY.md`, `SAFETY.md`, `VERIFY.md`); `docs/devnet/QBIND_DEVNET_OPERATIONAL_GUIDE.md`; M5/M17 Green, M6 Yellow/Partial (first-class `identity generate/verify/register-check` command is Green-for-scope Run 375/376; live registration path M4-gated) |
| M8, M9 | Ops / security | `docs/ops/QBIND_PQC_TRUST_LIFECYCLE_RUNBOOK.md`, `crates/qbind-node/src/pqc_trust_bundle.rs` |
| M10, M11, M12 | Security / net | `docs/release/public-devnet/p2p/` (Run 360: `README.md`, `P2P_PORT_POSTURE.md`, `PEER_ADMISSION_POLICY.md`, `ABUSE_DOS_POSTURE.md`, `VERIFY.md`); `crates/qbind-node/src/cli.rs`, `peer_rate_limiter.rs`, `QBIND_PEER_TRUST_BUNDLE_PROPAGATION_SAFETY.md`; M10/M11 Green, M12 Green (Run 371, scoped to the two abuse/DoS deployed live-socket controls) |
...
| M4 | 🟡 | **Format landed (Run 357):** seed-list schema + placeholder published (`docs/release/public-devnet/network/`). **Preflight (Run 377, Partial-positive):** schema-valid preflight live-seed candidate + reachability record + harness + tests; live admission gate release-proven; **only loopback/same-host reachability demonstrated — external reachability NOT proven (Route B), so no Green move.** **External reachability now PROVEN (Run 416, Route A POSITIVE):** a real routable public seed (`188.166.227.87:30333`) was reached from an independent off-host vantage under mutual-auth static-root — the external-reachability blocker is cleared, but **M4 stays Yellow** because the seed used temporary DevNet PQC material and **no `devnet-seeds.live.json` is published**. **Next:** provision a **durable** operator seed identity on the externally reachable host, re-capture reachability against it, then promote the candidate to `status: live` and publish `devnet-seeds.live.json` to move Green. |
...
Public DevNet is **NOT yet launch-ready**. The remaining launch blocker
...
- **Yellow (must reach Green):** M4 (seed/bootnodes — format+placeholder landed Run 357; external TCP + KEMTLS
  reachability now **PROVEN** via Route A in Run 416, clearing the external-reachability half — but still a launch
  blocker until a **durable, published `devnet-seeds.live.json`** live seed list lands),
...
| seed nodes / bootnodes | DevNet | network | 🟡 | `docs/release/public-devnet/network/` (Run 357; preflight Run 377; Route-C external Runs 378/388/391/393; Route-A POSITIVE Run 416) | Format + placeholder + preflight candidate published; external reachability now PROVEN (Run 416) but no durable published live seed | Outsiders cannot join a durable, published live seed | **Launch blocker** — provision a durable operator seed identity + publish `devnet-seeds.live.json` (external reachability proven; durable published-seed half OPEN) (M4) |
...
Public DevNet is still **not launch-ready**: **M4 remains Yellow-but-blocking** for seed/bootnodes until a
**durable, published `devnet-seeds.live.json`** live seed list lands (external TCP + KEMTLS mutual-auth
static-root reachability is now **PROVEN** via Route A in **Run 416** — a routable public seed
`188.166.227.87:30333` reached from a genuinely independent off-host dialer — which clears the
external-reachability half but does **not** move M4 Green because the seed used temporary discarded PQC
material and no durable list is published), and **M6 remains Yellow/Partial** (identity generation/verification +
...
remains Green-for-scope only. The most efficient remaining path forward is provisioning a **durable seed
identity and publishing a live `devnet-seeds.live.json` seed list** (M4 — external reachability itself is now
proven) plus the M4-gated live identity registration path (M6).

**Canonical current status (as of Run 416).** Must-haves: **M1, M2, M3 Green**; **M4 Yellow /
