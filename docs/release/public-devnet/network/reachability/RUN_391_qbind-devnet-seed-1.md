# Run 391 — Reachability Evidence: `qbind-devnet-seed-1` (EXTERNAL EXECUTION)

> **Safety label:** experimental · resettable · no value · no MainNet readiness
> claim · no C4/C5 closure claim. **DevNet only.**
>
> **Verdict: Route C — no safe external seed infrastructure available.** Run 391
...
> Run 378 and Run 388; Run 386/387 preflight posture (`signed_release=false`,
> `slsa_grade=false`) is likewise unchanged.

This is the reachability evidence record referenced by the Run 391 external-seed
reachability execution. It captures the bounded probe that IS reproducible in this
environment (loopback / same-host, continuity with Run 377/378/388) and states
honestly what was and was not proven for the **external** step required by
M4 Green.

## 1. Timestamp (UTC)

`2026-09-01T06:53:14Z` (recorded by the Run 391 harness at probe time; the value
...
- The loopback listener booted by the harness uses the node's **default**
  ...
  candidate identity, because Run 391 does not provision a durable operator seed
...
The **same host** as the seed listener — the Run 391 harness process on the
...
reachable. No real public host/port is exposed by Run 391. Operator note: the
...
- Target `qbind-node` binary SHA-256 / BuildID / toolchain: captured by the
  ...
  `docs/devnet/run_391_public_devnet_m4_external_seed_reachability/summary.txt`.
...
route. Run 391 does not claim `external` or `same-cloud-external`.
...
Run 391 also exercises on the live admission path. Because no external observer
...
`scripts/devnet/run_391_public_devnet_m4_external_seed_reachability.sh`. The
...
**None — no endpoint is exposed.** Run 391 opens no externally reachable port,
...
Run 377/378/388), but M4 Green requires reachability proven from **outside** the seed
