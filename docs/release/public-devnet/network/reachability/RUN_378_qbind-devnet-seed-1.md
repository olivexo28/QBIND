# Run 378 — Reachability Evidence: `qbind-devnet-seed-1` (EXTERNAL ATTEMPT)

> **Safety label:** experimental · resettable · no value · no MainNet readiness
> claim · no C4/C5 closure claim. **DevNet only.**
>
> **Verdict: Route C — no safe external seed infrastructure available.** Run 378
> attempts **Route A** (real external seed reachability from an independent
> external vantage point) but the sandboxed CI environment has **no external
> ingress and no independent external vantage point**. Exposing a real endpoint
> is therefore not possible here. **External reachability from outside the seed
> operator's own host/NAT was NOT proven.** **M4 stays Yellow.** This document
> records the attempt honestly and lists the infrastructure prerequisites that a
> real operator must satisfy to obtain M4 Green.

This is the reachability evidence record referenced by the Run 378 external-seed
reachability attempt. It captures the bounded probe that IS reproducible in this
environment (loopback / same-host, continuity with Run 377) and states honestly
what was and was not proven for the **external** step required by M4 Green.

## 1. Timestamp (UTC)

`2026-08-26T15:46:47Z` (recorded by the Run 378 harness at probe time; the value
...
- The loopback listener booted by the harness uses the node's **default**
  ...
  candidate identity, because Run 378 does not provision a durable operator seed
...
The **same host** as the seed listener — the Run 378 harness process on the
sandboxed CI runner (`127.0.0.1`). **No external vantage point is available in
this environment.** No independent second host, no cloud VM in a different
network, no residential/mobile vantage, and no public egress-to-ingress path
exists to dial a real endpoint from outside the operator's own host/NAT.
...
reachable. No real public host/port is exposed by Run 378.

## 5. Route type

**unknown → recorded as Route C (no external infrastructure).** The only probe
that succeeds is **loopback / same-host**, which is explicitly **not** an
external route. Run 378 does not claim `external` or `same-cloud-external`.
...
environment. The external TCP-dial requirement of M4 Green (rule 7) is therefore
**NOT satisfied**.

## 7. KEMTLS strict-auth handshake result

**Not performed against an external endpoint.** The standing in-process /
cross-process KEMTLS strict-auth + `PqcStaticRoot` evidence is Run 371–373 and
the loopback strict-auth **boot admission** evidence is Run 374/375; none prove
external reachability. The preferred external KEMTLS strict-auth handshake
(rule 8) requires a real externally reachable seed and an independent external
vantage point, neither of which is available here.
...
Run 378 also exercises on the live admission path. Because no external observer
exists, **no externally observed remote NodeId / cert-derived identity** was
captured; M4 Green rule 9 (observed remote identity matches the published entry)
is therefore **NOT satisfied**.

## 9. Metrics endpoint evidence

Not enabled. The node's `/metrics` endpoint is opt-in
...
#   from an independent external host, outside the seed's NAT:
#   nc -vz <public-host> <public-port>            # external TCP dial
#   qbind-node --network-mode p2p --enable-p2p \  # external KEMTLS strict-auth join
...
`scripts/devnet/run_378_public_devnet_external_seed_reachability.sh`. The external
commands are illustrative prerequisites, not executed in this environment.
...
**None — no endpoint is exposed.** Run 378 opens no externally reachable port,
...
Run 377), but M4 Green requires reachability proven from **outside** the seed
operator's own host/NAT (rule 6) with at minimum an external TCP dial (rule 7)
and preferably an external KEMTLS strict-auth handshake (rule 8). None of those
external conditions can be met in this sandboxed environment, so **M4 remains
Yellow**. The committed candidate is kept at `status: planned` with
`last_reachability_evidence: null`; it is **not** marked live.

## 14. Infrastructure prerequisites to obtain M4 Green (Route A)
...
4. **Independent external vantage point.** Use a genuinely separate network
   ...
   same-VPC, same-cloud-internal, or VPN-only reachability is **insufficient**
   (that would be Route B, and M4 stays Yellow).
5. **Capture external evidence.** Record a timestamped external TCP dial and,
   preferably, an external KEMTLS strict-auth handshake; capture the observed
   remote NodeId / cert-derived identity and confirm it matches the published
   seed entry.
