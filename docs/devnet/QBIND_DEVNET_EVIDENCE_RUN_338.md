...
Release-binary evidence for the Run 337 live epoch-transition settlement / finalization execution-preparation boundary.

Run 338 is **release-binary evidence only**. It proves the real Run 337 live epoch-transition settlement / finalization execution-preparation boundary in release mode. It does **not** add a new source/test boundary, does **not** wire the boundary into normal node operation, adds **no public CLI flag**, does **not** enable MainNet, performs **no** live production validator-set mutation, performs **no** production epoch transition, performs **no** production commit/finalization, and writes **no** production receipt, audit, audit seal, audit-finalization, audit-ledger, external-publication, durable replay, settlement, settlement-finalization, or publication record. Full **C4 remains OPEN**, **C5 remains OPEN**, and MainNet authority rotation/revocation remains **Red**.

## 1. Exact verdict

**PASS (release-binary evidence only; live epoch-transition settlement / finalization execution-preparation Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN).**

Run 338 is release-binary evidence for the Run 337 real live epoch-transition settlement / finalization execution-preparation boundary
(`crates/qbind-node/src/pqc_production_live_epoch_transition_settlement_execution_preparation.rs`,
`ProductionLiveEpochTransitionSettlementExecutionPreparationExecutor`). It adds no new production runtime wiring, no
public CLI flag, no default enablement, and no MainNet enablement. The release helper links and exercises the real
Run 337 boundary over the real Run 335/336 verified live epoch-transition settlement / finalization-preparation accept decision
(`is_accept()` with `Some(settlement_preparation_artifact)`; itself composing the Run 333/334 verified live
...
typed non-mutating `ProductionLiveEpochTransitionSettlementExecutionPreparationOutcome`. Any positive fixture-state application is
explicitly caller-owned, in-memory, source/test-only (`LiveEpochTransitionSettlementExecutionPreparationFixtureState`) and is not
...
* `crates/qbind-node/examples/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper.rs`
  — release helper mirroring the Run 337 test corpus as release-linked free-function cases plus a
...
* `docs/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary/`
  — evidence archive (`README.md`, `summary.txt`, `.gitignore`; per-run artifacts git-ignored).
* `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_338.md` — this canonical evidence file.
* `docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md` — status line advanced to Run 338; live epoch-transition
  settlement / finalization execution-preparation row moved Yellow → Green-for-scope only;
  Current-status paragraph updated; Run 338 timeline entry appended.
* `docs/whitepaper/contradiction.md` — Run 338 entry.
* `docs/protocol/QBIND_TRUST_ANCHOR_AUTHORITY_MODEL.md`,
  `docs/protocol/QBIND_GOVERNANCE_EXECUTION_RUNTIME_SURFACE_AUDIT.md`,
  `docs/protocol/QBIND_PEER_DRIVEN_TRUST_BUNDLE_APPLY_SAFETY.md`,
  `docs/ops/QBIND_PQC_TRUST_LIFECYCLE_RUNBOOK.md` — narrow Run 338 note appended to each (five narrative docs).

No change was made to the Run 337 boundary source or any other production runtime code.

## 3. Release artifacts and hashes

Captured in the tracked
`docs/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary/summary.txt`:

* `target/release/qbind-node` — SHA-256 `df5888bc922a46a6a6882dbc1b8fc4831d88b9daaa296b816509d0ad5bc809d5`
  (`qbind_node_sha256`).
* `target/release/examples/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper`
  — SHA-256 `79a9f802f416195e4fc18b6943f57fcb133d76be19eba46a5e81e84d0defc6b4` (`helper_338_sha256`).
* Toolchain: `rustc 1.97.1 (8bab26f4f 2026-07-14)` / `cargo 1.97.1 (c980f4866 2026-06-30)` recorded in `summary.txt`.

The real release binaries were built during this Run 338 run (`cargo build -p qbind-node --release` and
`cargo build -p qbind-node --example run_338_..._helper --release` both succeeded). The hashes above are the exact
values recorded by the committed harness `summary.txt`, which is treated as the final harness result for Run 338.
...
* settlement_execution_preparation_id `cf69411893d343ff0e52ddf5da5c0a8362be0a6342cf10536696e0ed64ecb1c2`
* request_id `7efcc3dfe4c75f9b6ccd6f44e66d5289de4e1e676933d6616c47dd07a0582169`
* settlement_execution_preparation_digest / content_digest `3776abc32acab5a1b3cefd17a83e116d4c9f4efd15904c2904ae39dc485ce455`
* transcript_digest `d7a7873029c929f8656eb70fa1e0ac865dd2fd6754358a58e5ee9e07580bf80d`
* epoch_transition_target `11`; staged_kind `StageApplyValidatorAdd`
* outcome_tag `accepted-source-test-live-epoch-transition-settlement-execution-preparation`

The named-digest free-function outputs (`named_settlement_execution_preparation_id`, `named_request_id`, `named_content_digest`,
`named_transcript_digest`) match the decision-derived values exactly.

## 5. Real-binary scenarios

* S1 `--help` → rc=0, hides the Run 337 live epoch-transition settlement / finalization execution-preparation boundary surface
...
* S2 DevNet, S3 TestNet, S4 MainNet (`--print-genesis-hash --env …`) → each surface is silent on any
  live-epoch-transition-settlement-execution-preparation enablement claim. (These return rc=1 because the binary fails closed on a
  missing `--genesis-path`; the harness asserts surface silence, not rc=0, for S2–S4.)
* S5 invented live-epoch-transition-settlement-execution-preparation CLI selector is rejected as an `unexpected argument` (rc=2),
...
* S6 default DevNet genesis-hash surface fails closed requiring `--genesis-path` (rc=1) and stays silent on
  settlement-execution-preparation claims.
...
Accepted-path cases show DevNet/TestNet source-test settlement-execution-preparation requests that bind a verified Run
335/336 live epoch-transition settlement / finalization-preparation accept decision (with `Some(settlement_preparation_artifact)`)
under the explicit source-test policy, producing typed non-mutating live settlement / finalization execution-preparation artifacts
with deterministic, stable `settlement_execution_preparation_id` / `request_id` /
`settlement_execution_preparation_digest` / `content_digest` / `transcript_digest` across two independent helper invocations,
...
accepted-without-artifact settlement-preparation decision; settlement-preparation / external-publication / durable-audit-publication /
...
`recover_live_epoch_transition_settlement_execution_preparation_window` recovery path proves a no-prior window is clean and a
...
`LiveEpochTransitionSettlementExecutionPreparationFixtureState`, across all scenarios, explicitly distinct from production runtime,
...
The Run 338 release harness
(`scripts/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary.sh`) was executed
end-to-end for this change set. It ran the full boundary regression corpus (30 test targets, run
from the newest Run 337 boundary suite back through the ancestor chain, plus `--lib pqc_authority` and the full
...
* `bash -n scripts/devnet/run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary.sh` — syntax OK.
* `cargo build -p qbind-node --release` — succeeded.
* `cargo build -p qbind-node --example run_338_production_live_epoch_transition_settlement_execution_preparation_release_binary_helper --release` — succeeded.

The harness `summary.txt` records `verdict: PASS` for the full Run 337 → ancestor chain of boundary test suites plus
`--lib pqc_authority` and the full `--lib` suite. The committed `summary.txt` is treated as the final harness result
for Run 338.

## 13. Security scans

* **Secret scanning** was run over all Run 338 changed files: the release helper
  (`crates/qbind-node/examples/run_338_..._helper.rs`), the harness script
  (`scripts/devnet/run_338_..._release_binary.sh`), the archive tracked files (`README.md`, `summary.txt`, `.gitignore`),
  this canonical evidence doc, the C4/C5 criteria doc, and the five narrative docs. **No secrets were found.**
* **CodeQL:** the `codeql_checker` tool was invoked for the Run 338 change set (declared **non-trivial**, because the
  ...
  **no** analysis of this change set — most plausibly because the ~5,000-line generated release helper plus the large
  evidence corpus pushed the CodeQL database over its size limit. **No CodeQL coverage is claimed for Run 338;** this
...
Full **C4 remains OPEN**. **C5 remains OPEN**. The live epoch-transition settlement / finalization execution-preparation matrix
row is **Green (for scope)** — for release-binary-evidenced
live-epoch-transition-settlement-execution-preparation-boundary behavior only (source/test in Run 337; release-binary evidence
positive in Run 338). MainNet authority rotation/revocation remains **Red**. No prior Green-for-scope row is weakened.

**CodeQL provenance (recorded verbatim).** The `codeql_checker` tool was invoked for the Run 338 change set, declared
...
**ran no analysis**, **no CodeQL coverage is claimed for Run 338**; this skipped result is explicitly **not** described as
clean coverage. Secret scanning (a separate tool) did run over all changed files and found no secrets.

## 15. Honest limitations

* Run 338 is release-binary evidence for the Run 337 boundary **only**; it does not prove a live production
...
* **`git_status: dirty` explanation.** The committed harness `summary.txt` records `git_status: dirty` at
  `git_commit` `a4ba6b427f9f95380bed4e4a9561faba4cfadefe` (the Run 338 progress commit that had landed the release helper
  and the first round of docs). The
  `summary.txt` is generated by the harness **while the run is in progress**, so at generation time the remaining Run 338
  ...
  the same Run 338 change set that publishes
...
* **CodeQL coverage:** the `codeql_checker` tool was invoked and returned *"Analysis was skipped because the database
  ...
  claimed for Run 338**; the skipped result is **not** described as clean coverage.

## 16. C4/C5 status

Full **C4 remains OPEN**. **C5 remains OPEN**. The live epoch-transition settlement / finalization execution-preparation row is
**Green-for-scope only** (Run 337 source/test + Run 338 release-binary evidence). MainNet
...
## 17. Suggested Run 339 next step

Run 339 (source/test, odd cadence): implement the next non-mutating boundary that consumes a verified Run 337/338 live
epoch-transition settlement / finalization execution-preparation accept decision (`is_accept()` with
`Some(settlement_execution_preparation_artifact)`) — e.g. a live epoch-transition **settlement / finalization execution**
boundary — producing only a typed, deterministic, policy-gated, non-mutating artifact, moving a **new**
matrix row Red → Yellow, with release-binary evidence deferred to Run 340. Full C4 / C5 remain OPEN. Do not begin
Run 339 as part of this Run 338 change set.