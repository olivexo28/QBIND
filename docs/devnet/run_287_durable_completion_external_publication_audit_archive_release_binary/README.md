...
Secret scanning was run over the changed files during the evidence pass and reported **no secrets**. CodeQL: Run 287 adds a release-mode example helper plus a harness shell script and evidence/Markdown documentation, and does not change the production library surface; the CodeQL checker result is reported honestly in `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_287.md`. The underlying Run 286 production module is re-run unchanged and is unaffected by this pass.
