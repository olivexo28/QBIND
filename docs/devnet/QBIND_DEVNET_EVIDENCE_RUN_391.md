# QBIND DevNet Evidence — Run 391

Public DevNet **M4 external seed/bootnode reachability** evidence. Run 391
...
harness, an archive, and a Run 391 integration test — **without** launching a
...
Run 391's primary objective is Route A: expose a real externally reachable seed
...
Run 391 records **Route C** — no safe external seed infrastructure is available —
keeps **M4 Yellow**, and documents the exact infrastructure prerequisites a real
operator must satisfy for M4 Green. For continuity, Run 391 re-proves the **live
...
`last_reachability_evidence: null`. This finding is unchanged from Run 378 and
Run 388; the
...
- `docs/release/public-devnet/network/reachability/RUN_391_qbind-devnet-seed-1.md`
  — **new** reachability evidence record (Route C; external reachability not
  proven; Route A infrastructure prerequisites documented).
- `crates/qbind-node/tests/run_391_public_devnet_m4_external_seed_reachability_tests.rs`
  — **new** integration coverage (4 tests).
- `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_391.md` — this evidence record.
- `docs/devnet/run_391_public_devnet_m4_external_seed_reachability/` — archive
  (README, summary, `.gitignore`).
- `docs/release/public-devnet/network/devnet-seeds.live-candidate.json` — updated
  `placeholder_statement` + entry `notes` to reference Run 391 (Route C); the
...
--reachability-evidence` gate already exists (Run 376) and is unchanged; Run 391
...
Per Route C, Run 391 declines to expose an endpoint, keeps **M4 Yellow**, and
...
(`null`), and `notes` with the safety label + evidence reference. Run 391 only
...
is **NOT satisfied**. Recorded in the Run 391 reachability record §7.
...
cert is proven by `register-check --cert` (Run 376), which Run 391 exercises on the
live-admission path (`cert_verified: true`).

## 10. Reachability record

`network/reachability/RUN_391_qbind-devnet-seed-1.md` captures: UTC timestamp,
...
On the release binary (referencing the Run 391 evidence record):
...
**No new public CLI flag.** Run 391 uses only pre-existing flags/subcommands
...
schema-valid preflight candidate + a fresh Run 391 reachability record now exist,
...
Green-for-scope (Run 376) and Run 391 re-exercises its *live* branch, but M6
...
- `cargo test -p qbind-node --test run_391_public_devnet_m4_external_seed_reachability_tests`
  — **4 passed**.
...
+ JSON text-only edits). Accordingly CodeQL is **not meaningful** for Run 391 (no
...
From `scripts/devnet/run_391_public_devnet_m4_external_seed_reachability.sh`:

```
release_binary sha256 = 8113cb759123ad5c641ab636b0d8d19967940992fa6e670a2b39e66d69667c1b
build_id              = 165bf83dc66583d05adddabdae382a04c9182597
...
harness; only publish-safe values are archived. The hash differs from Run 388's
...
- Route A cannot be executed here without exposing a real endpoint that this
  environment cannot route to; Run 391 therefore records Route C rather than
...
## 29. Suggested Run 392 next step

Execute **Route A** on real infrastructure per the prerequisites in
`network/reachability/RUN_391_qbind-devnet-seed-1.md` §15: provision a durable
