# Run 391 evidence archive — public DevNet M4 EXTERNAL seed reachability execution

Only `README.md`, `summary.txt`, and `.gitignore` are tracked in this directory.

`summary.txt` is the tee'd output of
`scripts/devnet/run_391_public_devnet_m4_external_seed_reachability.sh` and
...
```bash
bash scripts/devnet/run_391_public_devnet_m4_external_seed_reachability.sh
```
...
Run 391 executes the **Route A** objective (real external reachability from an
...
cannot be exposed and external reachability cannot be proven. Run 391 still proves
...
Run 377/378/388). It does **NOT** prove external reachability from outside the seed
...
This finding is unchanged from Run 378 and Run 388; Run 386/387 preflight posture
