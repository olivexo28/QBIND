...
Release-binary evidence for the Run 347 live epoch-transition authority-activation execution-preparation / final-execution preflight boundary.

Run 348 is **release-binary evidence only**. It proves the real Run 347 live epoch-transition authority-activation execution-preparation / final-execution preflight boundary in release mode. It does **not** add a new source/test boundary, does **not** wire the boundary into normal node operation, adds **no public CLI flag**, does **not** enable MainNet, performs **no** live production validator-set mutation, performs **no** production epoch transition, performs **no** production commit/finalization, and writes **no** production final-execution, authority-activation, authority-activation-authorization, final-settlement, authority-lifecycle-completion, receipt, audit, audit seal, audit-finalization, audit-ledger, external-publication, durable replay, settlement, settlement-finalization, settlement-execution, or publication record. Full **C4 remains OPEN**, **C5 remains OPEN**, and MainNet authority rotation/revocation remains **Red**.

## 1. Exact verdict

**PASS (release-binary evidence only; live epoch-transition authority-activation execution-preparation / final-execution preflight Green-for-scope; MainNet authority rotation/revocation Red; Full C4 OPEN; C5 OPEN).**
...
`ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationExecutor`). It adds no new production runtime wiring, no
public CLI flag, no default enablement, and no MainNet enablement. The release helper links and exercises the real
Run 347 boundary over the real Run 345/346 verified live epoch-transition authority-activation authorization / final-execution readiness accept decision
(`is_accept()` with `Some(authority_activation_authorization_artifact)`; itself composing the Run 343/344 verified live
...
typed non-mutating `ProductionLiveEpochTransitionAuthorityActivationExecutionPreparationOutcome`. Any positive fixture-state application is
explicitly caller-owned, in-memory, source/test-only (`LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState`) and is not
...
* `crates/qbind-node/examples/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary_helper.rs`
  — release helper mirroring the Run 347 test corpus as release-linked free-function cases plus a
...
* `docs/devnet/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary/`
  — evidence archive (`README.md`, `summary.txt`, `.gitignore`; per-run artifacts git-ignored).
* `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_348.md` — this canonical evidence file.
* `docs/protocol/QBIND_C4_C5_CLOSURE_CRITERIA.md` — status line advanced to Run 348; live epoch-transition
  authority-activation execution-preparation / final-execution preflight row moved Yellow → Green-for-scope only;
  Current-status paragraph updated; Run 347/348 timeline entry advanced.
* `docs/whitepaper/contradiction.md` — Run 348 entry.
* `docs/protocol/QBIND_TRUST_ANCHOR_AUTHORITY_MODEL.md`,
  `docs/protocol/QBIND_GOVERNANCE_EXECUTION_RUNTIME_SURFACE_AUDIT.md`,
  `docs/protocol/QBIND_PEER_DRIVEN_TRUST_BUNDLE_APPLY_SAFETY.md`,
  `docs/ops/QBIND_PQC_TRUST_LIFECYCLE_RUNBOOK.md` — narrow Run 348 note appended to each (five narrative docs).

No change was made to the Run 347 boundary source or any other production runtime code.

## 3. Release artifacts and hashes

Captured in the tracked
`docs/devnet/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary/summary.txt`:

* `target/release/qbind-node` — SHA-256 `393b6078ac3f8039c4f7a357caac067cee913aa7572f0fe8e692c95da9c85e7d`
  (`qbind_node_sha256`).
* `target/release/examples/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary_helper`
  — SHA-256 `3a948f63916598b1657d7a37a88a6e52c6b4ebdd6c24abd051d28e50ca1244bc` (`helper_348_sha256`).
* Toolchain: `rustc 1.97.1 (8bab26f4f 2026-07-14)` / `cargo 1.97.1 (c980f4866 2026-06-30)` recorded in `summary.txt`.

The real release binaries were built during this Run 348 run (`cargo build -p qbind-node --release` and
`cargo build -p qbind-node --example run_348_..._helper --release` both succeeded). The hashes above are the exact
values recorded by the committed harness `summary.txt`, which is treated as the final harness result for Run 348.
...
* authority_activation_execution_preparation_id `33972b39886ed5d046c96449ff9f6b4a1bd4edfd66f95cd1f9704caddabee240`
* request_id `18aa0acff4fcb0474c44b820553178cb6f67cbc2f71a2ef318e05b983c5fe6a7`
* authority_activation_execution_preparation_digest / content_digest `8ff5c5138cf6bfb897b7513bdb669e7f4cf3d0aebb54077799b479661afc2084`
* transcript_digest `bf543ad579e1f1fd67047a4bd78f0dc25f34fcffea2644fdc515ca4faac46b15`
...
The artifact `content_digest()` equals `authority_activation_execution_preparation_digest` and is stable across repeated evaluation, matching the decision-derived values exactly.

## 5. Real-binary scenarios

* S1 `--help` → rc=0, hides the Run 347 live epoch-transition authority-activation execution-preparation / final-execution preflight boundary surface
...
345/346 live epoch-transition authority-activation authorization / final-execution readiness accept decision (with `Some(authority_activation_authorization_artifact)`; itself composing the Run 343/344 verified live epoch-transition authority-lifecycle activation-readiness / post-completion attestation accept decision)
under the explicit source-test policy, producing typed non-mutating live authority-activation execution-preparation / final-execution preflight artifacts
with deterministic, stable `authority_activation_execution_preparation_id` / `request_id` /
`authority_activation_execution_preparation_digest` / `content_digest` / `transcript_digest` across two independent helper invocations,
...
`recover_live_epoch_transition_authority_activation_execution_preparation_window` recovery path proves a no-prior window is clean and a
...
`LiveEpochTransitionAuthorityActivationExecutionPreparationFixtureState`, across all scenarios, explicitly distinct from production runtime,
...
The Run 348 release harness
(`scripts/devnet/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary.sh`) was executed
end-to-end for this change set. It ran the full boundary regression corpus (36 test targets, run
from the newest Run 347 boundary suite back through the ancestor chain, plus `--lib pqc_authority` and the full
...
* `bash -n scripts/devnet/run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary.sh` — syntax OK.
* `cargo build -p qbind-node --release` — succeeded.
* `cargo build -p qbind-node --example run_348_production_live_epoch_transition_authority_activation_execution_preparation_release_binary_helper --release` — succeeded.

The harness `summary.txt` records `verdict: PASS` for the full Run 347 → ancestor chain of boundary test suites plus
`--lib pqc_authority` and the full `--lib` suite. The committed `summary.txt` is treated as the final harness result
for Run 348.

## 13. Security scans

* **Secret scanning** was run over all Run 348 changed files: the release helper
  (`crates/qbind-node/examples/run_348_..._helper.rs`), the harness script
  (`scripts/devnet/run_348_..._release_binary.sh`), the archive tracked files (`README.md`, `summary.txt`, `.gitignore`),
  this canonical evidence doc, the C4/C5 criteria doc, and the five narrative docs. **No secrets were found.**
* **CodeQL:** the `codeql_checker` tool was invoked for the Run 348 change set (declared **non-trivial**, because the
  ...
  Run 348** and the skipped result is explicitly **not** described as clean coverage.

## 14. C4/C5 matrix status

Full **C4 remains OPEN**. **C5 remains OPEN**. The live epoch-transition authority-activation execution-preparation / final-execution preflight matrix
row is **Green (for scope)** — for release-binary-evidenced
live-epoch-transition-authority-activation-execution-preparation-boundary behavior only (source/test in Run 347; release-binary evidence
positive in Run 348). MainNet authority rotation/revocation remains **Red**. No prior Green-for-scope row is weakened.

**CodeQL provenance (recorded verbatim).** The `codeql_checker` tool was invoked for the Run 348 change set, declared
**non-trivial** (the change set adds a new compiled Rust example and a bash harness script). The tool's verbatim result
is recorded in this section. Where a reported "0 alerts" is a consequence of the analysis being **skipped**
(a tool/infrastructure limitation — e.g. the CodeQL database exceeding its size limit), it is **not** a clean scan and **no CodeQL coverage is claimed for Run 348**. Secret
scanning (a separate tool) did run over all changed files and found no secrets.

## 15. Honest limitations

* Run 348 is release-binary evidence for the Run 347 boundary **only**; it does not prove a live production
...
* The `supp_*` supplemental Run 347 tests live in a private nested submodule that is not reachable from the helper
  entry point, so they are exercised by the `run_347_..._tests` test target (run by the harness) rather than by the
...
* **`git_status: dirty` explanation.** The committed harness `summary.txt` may record `git_status: dirty` at its
  ...
  time the remaining Run 348 deliverables may still be uncommitted/untracked: the evidence archive (`README.md`,
  `summary.txt`, `.gitignore`), this canonical evidence doc, and the final in-place doc refinements. Those files are
  committed as part of the same Run 348 change set that publishes this evidence doc; the `dirty` marker reflects only the
...
* **CodeQL coverage:** where the `codeql_checker` tool performs no analysis (e.g. a skipped result), **no CodeQL coverage
  is claimed for Run 348**; the skipped result is **not** described as clean coverage.

## 16. C4/C5 status

Full **C4 remains OPEN**. **C5 remains OPEN**. The live epoch-transition authority-activation execution-preparation / final-execution preflight row is
**Green-for-scope only** (Run 347 source/test + Run 348 release-binary evidence). MainNet
...
Run 349 (source/test, odd cadence): implement the next non-mutating boundary that consumes a verified Run 347/348 live
epoch-transition authority-activation execution-preparation / final-execution preflight accept decision (`is_accept()` with
`Some(authority_activation_execution_preparation_artifact)`) — a live epoch-transition **authority-activation
