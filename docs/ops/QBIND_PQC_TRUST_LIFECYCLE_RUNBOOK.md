...
Run 177 — release-binary live inbound `0x05` governance-proof carrier evidence. Run 177 closes the Run 176-deferred release-binary boundary on real `target/release/qbind-node` (DevNet N=3 V0/V1/V2 topology). Operationally, no behaviour changes for operators today: the validation-only `--p2p-trust-bundle-peer-candidate-wire-validation-enabled` and `--p2p-trust-bundle-governance-proof-required` flags (and their `QBIND_P2P_TRUST_BUNDLE_GOVERNANCE_PROOF_REQUIRED=1|true|yes|on` environment counterpart) continue to control Required-policy on the live `0x05` surface. To reach the release-binary boundary, Run 177 adds a single hidden harness-only CLI flag `--p2p-trust-bundle-peer-candidate-wire-publish-governance-proof-path` (clap `hide=true`, absent from `qbind-node --help`) that lets the Run 080 publish-once path inject a `GovernanceAuthorityProofWire` JSON into the live `0x05` `PeerCandidateWireEnvelopeV1.governance_authority_proof` field before encoding. Default behaviour is unchanged: when the flag is absent, the binary emits exactly the same wire bytes Run 080 / Run 176 already emit (no carrier). The flag is intended for harness use and is not part of the supported operator surface. Operators MUST NOT enable peer-driven apply on MainNet; the Run 147 FATAL invariant survives the Run 177 carrier wiring (R22). No new operator-facing CLI flag, no new environment variable, no schema / wire / metric drift beyond Run 176's optional envelope field. Full C4 / C5 remain OPEN. Evidence: `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_177.md`, `docs/devnet/run_177_live_0x05_governance_proof_release_binary/`.
