...
| **Description** | T228 implements infrastructure skeleton; T229+ adds penalty engine; M9 adds atomic O1/O2 enforcement; M11 extends to O3–O5 |
| **Security Impact** | Economic deterrent now enforced for all offense classes (O1-O5) |
| **Required Milestone** | Pre-MainNet |
| **Status** | ✅ Fully Mitigated (M9 + M11: All O1–O5 enforced) |
| **Note** | **M9**: O1/O2 economic penalties fully enforced with persistent atomic ledger integration. Implementation adds: (1) `AtomicSlashingBackend` trait for atomic penalty application, (2) `AtomicPenaltyRequest`/`AtomicPenaltyResult` types, (3) `apply_penalty_atomic()` commits stake reduction + jail + evidence marker + record in single RocksDB WriteBatch, (4) `build_validator_set_with_stake_and_jail_filter()` excludes jailed validators from active set at epoch boundary. **M11**: O3–O5 penalties now fully enforced using the same atomic backend from M9. Penalty parameters: O3 (Invalid Vote): 3% slash, 3 epoch jail; O4 (Censorship): 2% slash, 2 epoch jail; O5 (Availability Failure): 1% slash, 1 epoch jail. Deterministic verification added for O3-O5 evidence (block-height/view bound, signature verifiable, fail-closed on non-deterministic input). Mode enforcement: `EnforceCritical` or `EnforceAll` applies penalties for all O1-O5 offenses; `RecordOnly` records evidence without penalties; `Off` rejects evidence. Tests in `crates/qbind-node/tests/m9_slashing_penalty_tests.rs` (M9) and `crates/qbind-node/tests/m11_slashing_penalty_o3_o5_tests.rs` (M11, 21 tests). |
...
## 3.14 Evidence Ingestion Hardening (M15)

| Field | Value |
|-------|-------|
| **File Path** | `crates/qbind-consensus/src/slashing/mod.rs` |
| **Description** | Harden evidence ingestion to prevent DoS and abuse without introducing monetary rewards |
| **Security Impact** | Without hardening, evidence submission is vulnerable to spam attacks, oversized payloads, and abuse from non-validators |
| **Required Milestone** | Pre-MainNet |
| **Status** | ✅ Fully Mitigated (M15) |
| **Note** | **M15**: `HardenedEvidenceIngestionEngine` wraps `PenaltySlashingEngine` with deterministic admission controls. **Verification ordering** (cheap checks first): (1) Reporter validation (O(1)), (2) Size bounds (O(1)), (3) Per-block cap (O(1)), (4) Deduplication (O(1) hash lookup), (5) Structure validation, (6) Age bounds, (7) Future height check, (8) Cryptographic verification (expensive). **Configuration** (`EvidenceIngestionConfig`): `require_validator_reporter` (default true for MainNet/TestNet), `per_block_evidence_cap` (default 10), `max_evidence_age_blocks` (default 100K), size limits per offense (O1: 64KB, O2: 32KB, O3: 16KB, O4: 128KB, O5: 64KB). **Metrics** (`EvidenceIngestionMetrics`): evidence_received_total{offense}, evidence_rejected_total{reason,offense}, evidence_verified_total, evidence_duplicate_total, evidence_oversize_total, evidence_rate_limited_total. **No rewards**: Reporting has no economic incentive; hardening provides abuse resistance. **Environment configs**: `devnet()` (permissive), `testnet()` (moderate), `mainnet()` (strict). Tests in `crates/qbind-node/tests/m15_evidence_ingestion_hardening_tests.rs` (25 tests): duplicate rejection before verify (A1-A2), oversized rejection (B1-B2), non-validator reporter rejection (C1-C4), per-block cap enforcement (D1-D2), ordering tests (E1-E3), regression tests (F1-F2), age bounds (G1-G2), future height (H1-H2), config tests (I1-I6). |

## 3.15 Slashing State Persistence and Canonicalization Hardening (M19)

| Field | Value |
|-------|-------|
| **File Path** | `crates/qbind-ledger/src/slashing_ledger.rs`, `docs/whitepaper/QBIND_WHITEPAPER.md` (Section 16.8) |
| **Description** | Eliminate ambiguity about slashing persistence; make slashing state restart-safe and audit-grade |
| **Security Impact** | Without explicit canonicalization, the split between `ValidatorRecord` (canonical) and `ValidatorSlashingState` (mirror) could cause eligibility divergence between nodes or after restart |
| **Required Milestone** | Pre-MainNet |
| **Status** | ✅ Fully Mitigated (M19) |
| **Note** | **M19**: Slashing persistence model made explicit, consistent, and provably restart-safe. **Consensus-critical state defined**: (1) `EvidenceSeenSet` - content-addressed evidence markers for deduplication, (2) `SlashingRecordLog` - append-only audit trail, (3) `RecordSequenceCounter` - monotonic sequence, (4) Canonical economic fields in `ValidatorRecord` ONLY (stake, jailed_until_epoch). **Non-critical state classified**: `ValidatorSlashingState.stake` and `ValidatorSlashingState.jailed_until_epoch` are NON-AUTHORITATIVE mirrors that MUST NOT be used for eligibility/quorum decisions. **Fail-closed corruption detection**: `verify_slashing_consistency_on_startup()` checks evidence set consistency, record sequence monotonicity, and validator state integrity; returns `SlashingStateCorrupt` error on any inconsistency with NO automatic repair. **Documentation**: Whitepaper Section 16.8 updated with full persistence model including consensus-critical vs non-critical classification, atomic update guarantees, and fail-closed behavior. **Tests**: `crates/qbind-node/tests/m19_slashing_persistence_canonicalization_tests.rs` with: restart_after_o1_penalty_preserves_all_markers_and_records, restart_after_o5_penalty_preserves_all_markers_and_records, duplicate_evidence_never_double_penalizes_across_restart, corruption_detection_fails_closed, ensure_no_api_reads_mirrored_stake_for_eligibility. |
...
| Slashing ledger partial state | Storage | Atomic WriteBatch in `apply_slashing_update_atomic()` + failure-injection test (M1.3) + M19 fail-closed corruption detection via `verify_slashing_consistency_on_startup()` | Low (proven by test, fail-closed on corruption) | Mitigated |
| Economic state divergence | Storage | **M13/M19**: Canonical economic state unified. `ValidatorRecord.stake` and `ValidatorRecord.jailed_until_epoch` are the single source of truth. `ValidatorSlashingState` contains NON-AUTHORITATIVE mirrors documented with explicit M19 warnings. Slashing persistence model formalized in Whitepaper Section 16.8 with consensus-critical vs non-critical classification. Restart safety and corruption detection via `verify_slashing_consistency_on_startup()`. Tests in `m13_economic_state_unification_tests.rs` (12 tests) and `m19_slashing_persistence_canonicalization_tests.rs` (5 tests). | Low (canonical ledger-backed state with M19 hardening) | Mitigated |
...
| Key exposure on validator host | Crypto | M10.1: Signer isolation via remote signer with domain separation (`QBIND:remote-signer:v1`), replay protection (monotonic `request_id`), fail-closed behavior, **KEMTLS mutual auth enforced for TestNet/MainNet**, `LoopbackTesting` (plaintext keys) **FORBIDDEN** on TestNet/MainNet. Optional HSM/PKCS#11 integration feature-gated. **Residual operational risks**: remote signer availability/DoS, misconfiguration, HSM lifecycle/key rotation. | Low (strongly mitigated M10.1; residual operational risk) | Mitigated |
| Evidence ingestion abuse/DoS (M15) | Consensus | **M15**: Evidence ingestion hardening via `HardenedEvidenceIngestionEngine`. Admission controls: (1) Reporter must be active validator (MainNet/TestNet), (2) Per-block evidence cap (default 10), (3) Evidence age window (default 100K blocks), (4) Size bounds per offense type (O1: 64KB, O2: 32KB, O3: 16KB, O4: 128KB, O5: 64KB), (5) Deduplication before expensive verification, (6) Deterministic rules for consensus safety. No rewards implemented. | Low-Medium (mitigated M15; no incentives but safe ingestion) | Mitigated |
