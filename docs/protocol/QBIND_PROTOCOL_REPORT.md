...
| Economic state divergence | Storage | **M13**: Canonical economic state unified. `ValidatorRecord.stake` and `ValidatorRecord.jailed_until_epoch` are the single source of truth. Slashing ledger maintains mirror for operational tracking but eligibility predicates read from `ValidatorRecord`. Restart safety invariants tested via `m13_economic_state_unification_tests.rs` (12 tests). | Low (canonical ledger-backed state) | Mitigated |
