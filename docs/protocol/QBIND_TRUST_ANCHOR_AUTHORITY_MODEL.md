...
Run 357 — public DevNet seed/bootnode list format + placeholder seed-list publication (docs/artifact/verification only). Run 357 adds `docs/release/public-devnet/network/` (`README.md`, `devnet-seed-list.schema.json`, `devnet-seeds.placeholder.json`, `VERIFY.md`) and `docs/devnet/QBIND_DEVNET_EVIDENCE_RUN_357.md`. It makes no change to the trust-anchor authority model. The seed-list schema records, per seed, a `transport_security_mode`, a `pqc_suite`, and a `trust_bundle_required` flag that merely *describe* the pre-existing KEMTLS mutual-auth / PQC trust-bundle join surface (`--p2p-mutual-auth`, `--p2p-trust-bundle`); it defines no new trust root, mints no authority material, and consumes no authority block in any verifier. The committed placeholder list carries no live seeds, no node/peer identities (null), and only non-routable RFC 5737 documentation values. Run 357 wires no authority-lifecycle boundary into runtime, adds no public CLI flag, changes no P2P wire/peer-admission behaviour, enables no MainNet, and performs no validator-set mutation or epoch transition. MainNet custody and MainNet authority rotation/revocation remain Red. Full C4 remains OPEN; C5 remains OPEN.