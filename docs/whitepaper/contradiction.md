...
| Field | Value |
|-------|-------|
| **Status** | ⚠️ **OPEN — partial** (B1 and B2 landed; B3 snapshot-restore still outstanding) |
...
| **Code Location** | `crates/qbind-node/src/main.rs` — `run_local_mesh_node` and `run_p2p_node` now spawn `binary_consensus_loop::spawn_binary_consensus_loop` (drives `BasicHotStuffEngine` on a tokio interval) and `metrics_http::spawn_metrics_http_server_with_crypto` (gated by `QBIND_METRICS_HTTP_ADDR` via `MetricsHttpConfig::from_env`). `crates/qbind-node/src/binary_consensus_loop.rs` (new, 2026-Q2) is the thin wiring module that owns the binary's consensus loop. `crates/qbind-ledger/src/state_snapshot.rs` (snapshot creation only — **still no** `restore_from_snapshot`/`load_snapshot`/`apply_snapshot` path in `qbind-node`'s startup; B3 remains open). |
| **Description** | The canonical operations docs presume an operable validator binary that, on startup: (a) runs the consensus loop and produces/commits blocks, (b) exposes a Prometheus-compatible `/metrics` endpoint, and (c) supports recovery from a state snapshot. **(a) and (b) are now satisfied at the binary level** by the EXE-1→EXE-2 follow-up batch (B1 + B2). The binary drives `BasicHotStuffEngine` directly: in single-validator mode it self-quorums and commits a block per tick (the same behaviour exercised by `localmesh_integration_tests`), and the metrics HTTP server is spawned from the binary path under `QBIND_METRICS_HTTP_ADDR`. **(c) is still missing** — there is no `--restore-from-snapshot` path in `qbind-node`. Multi-validator P2P consensus interconnect (routing inbound P2P consensus messages back into the engine's `on_proposal_event`/`on_vote_event`) is also not yet wired at the binary level; until it is, multi-validator P2P bring-up still relies on `NodeHotstuffHarness`-style integration tests. |
| **Impact** | Reduced — DevNet readiness assessment (EXE-2) can now begin honestly against the binary path. Backup/recovery drills and full multi-node P2P validator clusters via the binary remain blocked on B3 and the consensus-net binary-path interconnect respectively. Protocol-correctness claims continue to be test-evidenced. |
| **Remaining (EXE-1 → EXE-2 follow-up)** | • ✅ B1 — `BasicHotStuffEngine` driver wired into `run_local_mesh_node` and `run_p2p_node` via `binary_consensus_loop`. • ✅ B2 — `metrics_http::spawn_metrics_http_server_with_crypto` spawned from the binary path, gated on `QBIND_METRICS_HTTP_ADDR`. • ⚠️ Wire P2P inbound consensus events into the engine driver from the binary path (the existing `P2pNodeBuilder` + `P2pConsensusNetwork` already exist; the routing into `on_proposal_event`/`on_vote_event` from the binary remains tracked under EXE-2). • ⚠️ B3 — implement `--restore-from-snapshot <path>` ingestion at startup using existing `StateSnapshotter` checkpoint format and pair with a drill script that produces template-shaped restore evidence. |
| **Tracking** | Recorded by `docs/protocol/QBIND_REPO_CODE_DOC_ALIGNMENT_AUDIT.md` §6.1–§6.3, §9 (B1, B2, B3) as the top three execution blockers exposed by EXE-1. B1 + B2 closed by `crates/qbind-node/src/binary_consensus_loop.rs` + `crates/qbind-node/src/main.rs` (regression-tested by `crates/qbind-node/tests/binary_path_b1_b2_b4_tests.rs`). B3 still open. |
